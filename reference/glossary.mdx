---
title: "Glossary"
description: "Find definitions for Mintlify terminology across product, content, navigation, deployment, Git, API playground, and AI features like the assistant and agent."
...
An AI tool for documentation maintainers that researches, writes, validates, and publishes proposed changes. The agent can open a pull request or commit to the deployment branch, depending on your configuration. See [Agent](/agent). Compare with [Assistant](#assistant), which answers reader questions and does not edit content.

### API playground

An interactive interface for exploring and testing API endpoints directly from your documentation. Mintlify generates the playground from an [OpenAPI](#openapi) or [AsyncAPI](#asyncapi) document. See [API playground](/api-playground/overview).

### AsyncAPI

A standard format for describing event-driven APIs, such as those that use WebSockets or message brokers. Mintlify can generate endpoint pages from an AsyncAPI document. See [AsyncAPI setup](/api-playground/asyncapi-setup).
...
An AI interface for documentation readers that answers questions from your content. It cites relevant pages and can navigate users directly to the relevant content. The assistant does not edit your documentation. See [Assistant](/assistant). Compare with [Agent](#agent), which proposes and publishes documentation changes.
...
The `mint` command-line tool. Use it to preview documentation locally, run quality checks, and manage your deployment from the terminal. See [Mintlify CLI](/cli/index).
...
A query language and standard for describing APIs as a schema of typed fields. Mintlify can generate endpoint pages and an interactive playground from a GraphQL schema. See [GraphQL setup](/api-playground/graphql-setup).
...
A page that renders on your site but is not linked from navigation. Hidden pages have live URLs, so anyone with the link can view them. See [Hidden pages](/organize/hidden-pages).

## I

### Index

A search service for coding agents that retrieves current technical context from publisher-maintained documentation and the web through an MCP server or REST API. See [Index](/search-index). Not to be confused with the `index.mdx` filename convention for a section's landing page, or with a search index that a site builds internally.
...
The continuously updated rendering of your unpublished edits inside the web editor. A live preview is not publicly deployed. See [Live preview](/editor/live-preview). Compare with [Preview deployment](#preview-deployment), which is a shareable build tied to a branch or pull request.
...
A collection of files and their Git history. The documentation repository is the source of truth for a Mintlify site and contains its pages, configuration, and assets. Compare with [Source repository](#source-repository) and [Context repository](#context-repository), which the agent can read for additional context.
...
See [Reusable snippet](#reusable-snippet).
