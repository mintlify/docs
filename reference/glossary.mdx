...
An authenticated [Model Context Protocol](#model-context-protocol-mcp) server that lets authorized AI tools read and update content and settings across one or more projects in an organization. Changes happen on a branch and require a pull request to merge. See [Admin MCP server](/ai/mintlify-mcp).
...
An interactive environment on endpoint pages where readers craft, send, and inspect real API requests without leaving your documentation. Mintlify generates the playground from an [OpenAPI](#openapi) document. See [API playground overview](/api-playground/overview).
...
The directory in a repository that contains `docs.json` and the files for a Mintlify project. It can be the repository root or a configured subdirectory, such as `docs/` in a monorepo. See [Monorepo setup](/deploy/monorepo).
...
Units that measure use of Mintlify AI features. Credits are pooled across an organization and shared by its projects and members. See [Credits](/credits).
...
A build of a project that Mintlify publishes. A production deployment updates the live site from the deployment branch, and a [preview deployment](#preview-deployment) publishes a temporary version for review.
...
A Mintlify workspace that contains team members, organization-level settings, shared credits, and one or more projects.

### Overlay

A JSON or YAML document that describes changes to apply to an OpenAPI specification without editing its source file. Mintlify applies overlays before validating and rendering a specification. See [Transform your spec with overlays](/api-playground/openapi-setup#transform-your-spec-with-overlays).
...
A top-level navigation structure for publishing documentation for multiple distinct products in one project. Each product has its own navigation. See [Products](/organize/navigation#products).

### Project

A documentation site within a Mintlify organization. A project connects source content and a deployment branch to a live site. Projects were previously called deployments, and some dashboard settings and APIs still use that term, such as the **Deployment name** setting and the `list_deployments` tool in the [Admin MCP server](#admin-mcp-server).
...
A browser API that lets sites register [Model Context Protocol](#model-context-protocol-mcp) tools directly on the page. WebMCP-capable browsers and in-browser agents call these tools without a separate server connection. See [WebMCP](https://developer.chrome.com/docs/ai/webmcp) in the Chrome Developers documentation.
