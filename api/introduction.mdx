...
All Mintlify REST API requests use the following base URL:
...
Authenticate every request by passing your API key in the `Authorization` header as a bearer token. For example:
...
Use the [admin API key](#admin-api-key) for deployment, agent, and analytics endpoints and the [assistant API key](#assistant-api-key) for assistant endpoints. See each endpoint's reference page for its full path, request body, and response schema.
