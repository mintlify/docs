...
Use the admin API key to authenticate requests to [Trigger update](/api/update/trigger), [Get update status](/api/update/status), [Create Mintie bot job](/api/agent/create-agent-job), [Get Mintie bot job](/api/agent/get-agent-job), [Get all Mintie bot jobs](/api/agent/get-all-jobs), [Get user feedback](/api/analytics/feedback), and [Get assistant conversations](/api/analytics/assistant-conversations).
