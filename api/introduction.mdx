...
- [Create agent job](/api/agent/create-agent-job): Create an agent job to automatically edit your documentation (streaming response).
...
The v2 agent API provides a fire-and-forget interface with polling-based status retrieval:

- [Create agent job (v2)](/api/agent-v2/create-agent-job): Create an agent job that runs in the background and returns immediately.
- [Get agent job (v2)](/api/agent-v2/get-agent-job): Poll for job status and retrieve the PR link.
- [Send message (v2)](/api/agent-v2/send-message): Send follow-up messages to an existing agent session.
