...
- [List preview deployments](/api/preview/list): List all preview deployments for a project.
- [Get preview deployment status](/api/preview/status): Check the build status of a preview deployment.
- [Delete preview deployment](/api/preview/delete): Delete a preview deployment.
...
- **Preview deployments**: Programmatically create, monitor, and manage preview deployments for branches with [Create preview deployment](/api/preview/create), [Get preview deployment status](/api/preview/status), [List preview deployments](/api/preview/list), and [Delete preview deployment](/api/preview/delete).
...
Use the admin API key to authenticate requests to [Trigger update](/api/update/trigger), [Get update status](/api/update/status), [Create preview deployment](/api/preview/create), [List preview deployments](/api/preview/list), [Get preview deployment status](/api/preview/status), [Delete preview deployment](/api/preview/delete), [Create agent job](/api/agent/v2/create-agent-job), [Get agent job](/api/agent/v2/get-agent-job), [Send follow-up message](/api/agent/v2/send-message), [Get user feedback](/api/analytics/feedback), [Get feedback by page](/api/analytics/feedback-by-page), [Get assistant conversations](/api/analytics/assistant-conversations), [Get search queries](/api/analytics/searches), [Get page views](/api/analytics/views), and [Get unique visitors](/api/analytics/visitors).
