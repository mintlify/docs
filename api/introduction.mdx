...
- [Create agent job (v2)](/api/agent-v2/create-job): Create an async agent job with a simple prompt interface.
- [Get agent job status (v2)](/api/agent-v2/get-job): Poll for async job status and PR link.
- [Send message to agent (v2)](/api/agent-v2/send-message): Send follow-up instructions to an existing agent session.
...
- **Automated editing**: Use agent jobs to programmatically update documentation at scale with [Create agent job](/api/agent/create-agent-job), [Get agent job](/api/agent/get-agent-job), and [Get all agent jobs](/api/agent/get-all-jobs). For simpler async workflows, use the [v2 agent API](/api/agent-v2/create-job).
...
Use the admin API key to authenticate requests to [Trigger update](/api/update/trigger), [Get update status](/api/update/status), [Create agent job](/api/agent/create-agent-job), [Get agent job](/api/agent/get-agent-job), [Get all agent jobs](/api/agent/get-all-jobs), all [v2 agent endpoints](/api/agent-v2/create-job), [Get user feedback](/api/analytics/feedback), and [Get assistant conversations](/api/analytics/assistant-conversations).
