---
title: "Create Mintie assistant message (v1)"
description: "Deprecated. Send messages to Mintie assistant using AI SDK v4. Use v2 endpoint for AI SDK v5 or later."
openapi: /discovery-openapi.json POST /v1/assistant/{domain}/message
tag: "Deprecated"
keywords: [ "Mintie assistant message", "embed", "chat", "integrate", "ai sdk v4" ]
---
<Badge color="orange">Deprecated</Badge>

<Info>
  The Mintie assistant message v1 endpoint is compatible with **AI SDK v4**. If you use AI SDK v5 or later, use the [Mintie assistant message v2 endpoint](/api-reference/assistant/create-assistant-message-v2) instead.
</Info>

## Integration with `useChat`

The `useChat` hook from Vercel's AI SDK is the recommended way to integrate Mintie assistant API into your application.
...
- `body.context` - Array of contextual information to provide to Mintie assistant. Each context object contains:
...
- `body.currentPath` - The path of the page the user is currently viewing. When provided, Mintie assistant uses this context to provide more relevant answers. Maximum length: 200 characters.
...
Mintie assistant API has the following limits:
