...
Static export bundles use explicit `.html` page URLs. For example, `/guides/getting-started` becomes `/guides/getting-started.html`, and the home page becomes `/index.html`.

This format matches bundle files and supports S3 or plain static hosting without extensionless URL rewrites. Preserve `.html` filenames when you upload the bundle. The Static Export API enables this format automatically, so you do not need to change your documentation configuration.

CloudFront deployments preserve client-side navigation and prefetching. Plain file hosts support full-page navigation through generated `.html` links. Cloud-hosted production sites remain unchanged.

Built-in controls that navigate internally, such as version and language switchers, search results, cards, assistant citations, and API playground links, also use `.html` destinations. Query strings remain intact, so API playground links continue to work.

<Note>
  Canonical and sitemap URLs remain extensionless. Custom client-side navigation URLs can also remain extensionless. CloudFront-backed deployments resolve them, while file-only hosts may need rewrites.
</Note>
