...
This format matches the files in the bundle and supports S3 or plain static hosting without extensionless URL rewrites. Preserve `.html` filenames when you upload the bundle. The Static Export API enables this format automatically, so you do not need to change your documentation configuration.
...
Built-in internal navigation, including version and language switchers, search results, cards, assistant citations, and API playground links, also uses `.html` destinations. Query strings remain intact, so API playground links continue to work.
