...
<Note>
  Static export is not available for deployments with authentication enabled. If your deployment uses partial or full authentication, [Start static export job](/api/static-export/start-job) returns a `403` response. Contact [support@mintlify.com](mailto:support@mintlify.com) if you need to export an authenticated deployment.
</Note>
