...
<Steps>
  <Step title="Start a static export job">
    ...
    To build the bundle for a specific base path, such as `/docs`, pass `basePath` in the request body. Links and asset URLs in the bundle are prefixed with this path. If omitted, the export uses the base path configured for your deployment.
