...
- **feedback**: The user's thumbs rating on the assistant's response. Either `positive`, `negative`, or `null` if the user did not rate the response. When a user regenerates an answer and rates it again, the latest rating is used.
