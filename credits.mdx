...
On Enterprise plans, you can supply your own LLM provider API key and choose the model behind the assistant, agent, and automations. AI usage on a deployment with [bring your own model](/ai/bring-your-own-model) enabled does not consume Mintlify credits. Your provider bills you directly for inference. You configure this per deployment, so other deployments in the same organization still draw from the shared credit balance.
