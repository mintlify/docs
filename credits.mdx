...
* Assistant responses on your documentation site
* Assistant API and discovery chat responses
...
| Assistant API response | 23 |
| Editor agent run | 115 |
| Slack agent run | 110 |

The [Usage](https://app.mintlify.com/settings/organization/usage) page splits credit consumption into distinct product categories so you can see exactly where credits are going:

| Category | What it includes |
|:---------|:-----------------|
| Assistant | Responses from the in-product web assistant on your documentation site, including the Slack assistant bot. |
| Assistant API | Responses from the [assistant API](/api/assistant/create-assistant-message-v2) and discovery chat surfaces. |
| Agent | Editor agent and Slack agent runs. |
| Editor agent | Editor agent runs. |
| Workflows | Scheduled and triggered workflow runs. |

Use the product filter on the usage chart to view consumption for a single category over time.
...
**Monitor your usage patterns.** The [Usage](https://app.mintlify.com/settings/organization/usage) page in your dashboard shows a breakdown by feature category, including separate totals for the in-product Assistant and Assistant API. If a particular product or workflow is consuming more credits than expected, review its trigger or any custom instructions.
