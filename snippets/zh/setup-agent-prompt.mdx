...
让我的文档在已发布的 Mintlify 站点上正式上线。当你把在线 URL 给我并确认它能加载时，就算成功完成任务。当某个步骤需要我参与时，请清楚地告诉我——其他所有事情请你自己完成。如果这个提示词之前已经被粘贴过（例如在重启之后），请从上次中断的地方继续，而不是从头开始。如果你没有终端或命令执行权限，请告诉我改为在 [mintlify.com/start](https://mintlify.com/start) 按照手动步骤操作。以下所有步骤都需要执行命令，并且需要 Node.js v20.17.0+（建议使用 LTS 版本）。
