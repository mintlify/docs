...
Customize code block themes globally using `styling.codeblocks` in your `docs.json` file. You can use simple themes like `"system"` or `"dark"`, or configure custom [Shiki themes](https://shiki.style/themes) for light and dark modes. See [Settings](/settings#styling) for detailed configuration options.
