...
3. Optionally, customize the export options:
    ...
    - **Include authenticated pages**: For sites with [authentication](/deploy/authentication-setup) enabled, toggle this on to include pages that require authentication. When disabled, only publicly accessible pages are exported.
