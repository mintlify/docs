...
Full-site PDF exports are supported for documentation sites that use authentication. The export includes all pages in your docs regardless of group restrictions, so anyone with access to the exported PDF can read content that may otherwise be gated behind user groups.
