...
Full-site PDF exports are not supported for documentation sites that use authentication. Authenticated users can still download individual pages as PDFs.

<Tip>
  To let users download individual pages as PDFs, use the [contextual menu's](/ai/contextual-menu) `download-pdf` option. Per-page PDF downloads work on both public and authenticated docs sites.
</Tip>
