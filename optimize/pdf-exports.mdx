...
3. Optionally, customize the export options:
    ...
    - **Include authenticated pages**: Include pages that require authentication. This option only appears for sites with [authentication](/deploy/authentication-setup) enabled. When disabled (default), only public pages are exported.
