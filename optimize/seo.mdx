...
Mintlify automatically adds [schema.org](https://schema.org) structured data to every indexable page as a JSON-LD script. This structured data helps search engines display rich results for your documentation.

Mintlify generates the structured data from your existing page frontmatter and `docs.json` configuration, including the page title, description, keywords, canonical URL, last updated date, and your site name and logo. Fields without a corresponding value are left out. Pages with `noindex: true` do not include structured data.

There is no configuration for structured data. To change what appears in it, update the corresponding frontmatter fields on the page.
