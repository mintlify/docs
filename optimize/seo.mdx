...
Mintlify automatically generates an Open Graph (OG) image for every page. This image appears as the social preview when you share a link on platforms like Slack, X, LinkedIn, Discord, and iMessage.

### Default behavior

By default, every page gets an auto-generated OG image (1200x630px) that includes:
...
- Your site's **brand colors** (from the `colors` field in `docs.json`)

You don't need to configure anything for this to work — it happens automatically for every page.
...
<AccordionGroup>
<Accordion title="Custom background image">
  ...
  This applies to all pages. Mintlify overlays your logo, the page title, and description on top of the background image. You can also customize the font and color scheme:
  ...
  See [`thumbnails`](/organize/settings-appearance#thumbnails) for the full list of options.
</Accordion>

<Accordion title="Static OG image for all pages">
...
<Accordion title="Static OG image for a specific page">
  ...
  This replaces the auto-generated image for that page only.
</Accordion>
</AccordionGroup>

<Note>
  Setting `og:image` in meta tags (globally or per-page) replaces the auto-generated social preview with a static image. If you want Mintlify to automatically overlay your logo, page title, and description on a custom background, use [`thumbnails.background`](/organize/settings-appearance#thumbnails) instead.
</Note>
