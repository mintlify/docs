...
<Note>
  **Don't index project** does not gate [`llms-full.txt`](/ai/llmstxt#llms-fulltxt), which stays available at `/llms-full.txt`. The Markdown export of each page also keeps its pointer to `/llms.txt` even though that file returns `404` while the setting is enabled. If your `llms.txt` returns `404` but `llms-full.txt` works, check this setting. See [Fix llms.txt returning 404 while llms-full.txt works](/help-center/llms-txt-returns-404-but-llms-full-txt-works).
</Note>
