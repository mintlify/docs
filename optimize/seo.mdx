...
When enabled, Mintlify adds a trailing slash to:

- Canonical tags and `og:url` meta tags
- `<loc>` entries in the auto-generated `sitemap.xml`
- Root-relative internal links between pages, preserving any query string or hash

Enabling `trailingSlash` only affects how URLs are emitted. It does not change your page routes, redirect existing URLs, or apply to explicit per-page `canonical` values you set in [frontmatter](/organize/pages). Leave `trailingSlash` unset or `false` if your site serves URLs without a trailing slash.
