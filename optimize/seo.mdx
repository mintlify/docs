...
<Note>
  For fully authenticated documentation sites, sitemaps and `robots.txt` files also require authenticating to access, and sitemaps exclude pages that belong to [user groups](/deploy/authentication-setup#control-access-with-groups).

  For partially authenticated sites that include public pages, sitemaps and `robots.txt` are publicly accessible. Anonymous visitors see only public pages in the sitemap. Authenticated users also see ungrouped pages.
</Note>
