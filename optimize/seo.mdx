...
<Tip>
  Pages with `hidden: true` in their frontmatter are automatically treated as `noindex: true`, unless you set `seo.indexing` to `"all"` in your `docs.json`. Explicit `noindex: true` in a page's frontmatter is always respected. See [Hidden pages](/organize/hidden-pages) for more details.
</Tip>
