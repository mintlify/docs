...
Folder tags override `seo.metatags` for the same key. When a page is under multiple entries, longer paths override shorter ones. Page frontmatter still overrides both. `canonical` is always sourced from `seo.metatags` and cannot be set per folder. See [`seo.paths`](/organize/settings-seo#seo) for the full schema.
...
<Note>
  Always verify canonical URL behavior on your deployed site. Local builds add `/src/_props` to URLs as an artifact that is not part of the canonical URL.
</Note>
...
Frontmatter can also override any custom meta tag declared in your `docs.json` under [`seo.metatags`](/organize/settings-seo#seo) or a matching [`seo.paths`](/organize/settings-seo#seo) entry. Provide a non-empty string value with the same key to replace the site-wide or folder-scoped value on that page. Frontmatter cannot introduce meta tags that the config does not declare, and non-string values are ignored.
...
By default, Mintlify indexes only the pages you include in your `docs.json` navigation. [Hidden pages](/organize/hidden-pages) include pages missing from your navigation, pages under a hidden tab or group, and pages with `hidden: true` in their frontmatter. Mintlify excludes hidden pages from the following locations:
...
<Note>
  For documentation sites that require authentication, users must authenticate to access sitemaps and `robots.txt` files. Sitemaps exclude pages that belong to [user groups](/deploy/authentication-setup#control-access-with-groups).
</Note>
...
<Tip>
  Mintlify automatically treats pages with `hidden: true` in their frontmatter as `noindex: true`. See [Hidden pages](/organize/hidden-pages) for more details.
</Tip>
...
To hide your entire deployment from search engines while keeping it publicly accessible, enable **Don't index project** on the [General](https://app.mintlify.com/settings/deployment/general) page of your dashboard.
...
<Note>
  **Don't index project** does not gate [`llms-full.txt`](/ai/llmstxt#llms-fulltxt), which stays available at `/llms-full.txt`. The Markdown export of each page also keeps its pointer to `/llms.txt` even though that file returns `404` while the setting is enabled.
</Note>

Disable indexing when you need to keep a deployment online for internal review, staging, or limited sharing. This prevents the site from appearing in public search results and prevents AI tools from using it as training or retrieval data.
