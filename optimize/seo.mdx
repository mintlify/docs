...
Folder tags override `seo.metatags` for the same key, longer paths override shorter ones, and page frontmatter overrides both. See [`seo.paths`](/organize/settings-seo#param-seo-paths) for path rules and exceptions.
...
Frontmatter can also override any custom meta tag declared in your `docs.json` under [`seo.metatags`](/organize/settings-seo#param-seo-metatags) or a matching [`seo.paths`](/organize/settings-seo#param-seo-paths) entry. Set the same key to a non-empty string to replace the site-wide or folder value on that page. Frontmatter can't add custom meta tags that your config doesn't declare, and Mintlify ignores non-string values.
