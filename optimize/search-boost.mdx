...
Control how many results your in-product search bar returns by setting **Max results** in your dashboard under **Settings > Search**. The value must be an integer between `1` and `100`. Mintlify returns up to six results by default.

Increase this value when your documentation covers many adjacent topics and users benefit from seeing more matches at once. Lower it to keep the results list focused and reduce visual noise.

This setting only affects the search bar on your Mintlify-hosted documentation site. The [Search API](/api/assistant/search) continues to honor the `pageSize` parameter on each request, which overrides the dashboard setting when provided.
