---
title: "Mintlify analytics for traffic and user engagement"
...
<Info>
  User engagements are in beta and subject to change.
</Info>

The User engagements section shows how visitors interact with your documentation beyond page views. Use it to understand which content leads visitors to click, copy, call APIs, respond to feedback, or take other actions.

The overview shows total interactions, average interactions per session, and CTA click rate. Each metric includes the change from the previous period.

The **Actions taken over time** chart compares CTA clicks with other actions. CTA clicks are presses of your [navbar call-to-action button](/organize/settings-structure#navbar). Actions include contextual-menu clicks, code-block copies, API Playground calls, feedback responses, and similar interactions.

The per-page table shows page views, clicks, CTA rate, actions, and action rate. Use these metrics to compare pages and identify content that leads to deeper engagement.

**User flow paths** shows common paths from an entry point through a visitor's first, second, and third interactions. Select an entry point to explore its paths.

**User funnel** measures two journeys:

- **Search-to-answer**: Opened search, entered a query, clicked a result, landed on a page, and took an action.
- **Assistant conversation**: Started a conversation, got an answer, asked a follow-up, and engaged with the answer.
