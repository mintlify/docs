...
Agent visitors are identified by IP address and user agent. The agent visitor count approximates distinct sources of AI traffic rather than individual agent sessions or conversations. Multiple requests from the same IP address count as one visitor.
...
The MCP searches tab displays AI agent search interactions through the Model Context Protocol (MCP). This metric counts calls to your MCP server's search tool, so it reflects agents that users connected to your MCP server and issued a search, not general AI crawler traffic to your docs pages.
