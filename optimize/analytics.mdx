...
<Warning>
**Deprecation notice**: The analytics dashboard will be sunset on March 7, 2026. Migrate to Google Analytics before this date to continue tracking your documentation metrics. See the migration guide below.
</Warning>
...
The analytics dashboard is being deprecated on March 7, 2026. Follow these steps to migrate to Google Analytics:

<Steps>
  <Step title="Create a Google Analytics account">
    If you don't already have one, [create a Google Analytics 4 property](https://support.google.com/analytics/answer/9304153).
  </Step>

  <Step title="Get your measurement ID">
    In your GA4 property, navigate to Admin > Data Streams and copy your measurement ID (format: `G-XXXXXXXXXX`).
  </Step>

  <Step title="Add Google Analytics to your docs">
    Add the Google Analytics integration to your `docs.json`:
    ...
    See the [Google Analytics integration guide](/integrations/analytics/google-analytics) for detailed setup instructions.
  </Step>

  <Step title="Verify tracking">
    Deploy your changes and verify that events are being tracked in your Google Analytics real-time reports.
  </Step>

  <Step title="Export existing data (optional)">
    Before March 7, 2026, export any historical data you want to preserve from the analytics dashboard using the export features in the Assistant and Feedback tabs.
  </Step>
</Steps>

### What you'll need to set up manually

Google Analytics provides equivalent or enhanced tracking for most analytics features:

- **Visitors**: Tracked automatically via GA4 page views
- **Searches**: Available via GA4 site search tracking
- **Feedback**: Implement using GA4 custom events
- **Assistant insights**: Export chat history before migration if you want to preserve historical data
- **MCP searches**: Export data before migration for historical records

For help with the migration or questions about Google Analytics setup, [contact support](https://mintlify.com/contact/sales).
