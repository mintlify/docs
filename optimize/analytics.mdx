...
The categories tab automatically assigns each conversation a granular category, then groups related categories under a broader theme. Categorization groups conversations by topic regardless of language.

Each theme and category displays:

- **Occurrence**: How many conversations it contains.
- **Sentiment**: Thumbs up and thumbs down counts from user feedback on assistant responses.
...
Use themes to identify common topics and patterns in user needs. Use categories to find targeted issues, like a specific task that users repeatedly ask about or a page that generates negative feedback.

Click a theme row to expand it and view its categories. Click a category to view related conversations.
