...
Export any analytics tab to CSV for deeper analysis, reporting, or archival. Exports are available for visitors, views, the assistant, searches, referrals, and feedback, and respect the current traffic source and time range filters.

1. Navigate to the [analytics](https://app.mintlify.com/products/analytics/v2/) page of your dashboard.
...
Exports run asynchronously in the background, so you can keep working in the dashboard while a large export is being prepared. The download link in the email is valid for three days.
