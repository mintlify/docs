...
The [analytics](https://app.mintlify.com/analytics) page in your dashboard shows data about visitors to your site, their interactions with the assistant, their searches, and their feedback. Use this information to identify which pages are most valuable to your users and track trends over time.
...
The user engagements section shows how visitors interact with your content and calls to action. It includes an overview, interaction trends by page, common paths through your documentation, and search and assistant funnels. All cards use the time range selected for this section.
...
- **CTA clicks**: Clicks on the [primary navbar button](/organize/settings-structure#navbar). This series requires a `navbar.primary` button in `docs.json`.
...
The search funnel includes these stages:
...
The assistant conversation funnel excludes sessions flagged as spam and includes these stages:
...
Review your user engagement analytics to:
...
2. In the **Analytics streaming** section, click **Configure**.
...
Select which of your organization's deployments send events. The dashboard lists each deployment by subdomain. If you create new deployments, you must return to the streaming configuration and select them. New deployments do not automatically start streaming events.
