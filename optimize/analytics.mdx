...
The assistant section of the analytics page displays assistant usage over time and a conversations card with two views.

- **Conversation categories** groups conversations into categories and topics. Select a category to see its topics, then select a topic to open a drawer with satisfaction metrics and every conversation in that topic.
...
Use the **Filter** dropdown to narrow either view to conversations with **Positive** or **Negative** feedback. Feedback filters are applied server-side, so results stay complete across pages.
