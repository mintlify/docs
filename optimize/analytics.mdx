...
For deeper analysis, export a CSV file of your queries, responses, sources, resolution status, and query category. Use your preferred tools to analyze the exported data.

The exported CSV includes the following columns to help you triage conversations:

- `resolutionStatus`: Indicates whether the assistant successfully answered the question (`answered` or `unanswered`). Use this column to identify documentation gaps surfaced by questions the assistant could not resolve.
- `queryCategory`: The topic cluster the query was grouped into, matching the categories shown in the Categories tab. Use this column to slice exports by theme without re-running categorization.
