...
Enable selection mode to select multiple feedback items and perform actions in bulk. You can:

- **Set status**: Change the status of all selected items at once.
- **Add note**: Add an internal note to all selected items.
- **Flag as abusive**: Mark selected items as abusive content.
- **Unflag**: Remove the abusive flag from selected items.
- **Delete**: Permanently remove selected feedback items.
