...
- **Agent feedback** <span style={{verticalAlign: "middle", fontSize: "0.75rem", padding: "2px 6px", borderRadius: "4px", backgroundColor: "var(--shiki-token-keyword)", color: "white"}}>Beta</span>: Structured feedback submitted by AI agents about content quality and accuracy. See [agent feedback](#agent-feedback) for details.

## Agent feedback

<Note>
  Agent feedback is in beta.
</Note>

When enabled, AI agents that read your documentation through [Markdown page views](/ai/markdown-export) receive instructions for submitting structured feedback about content quality, accuracy, and completeness. This gives you a signal about which pages are causing problems for AI-powered workflows.
...
<Steps>
  ...
  <Step title="Enable the toggle">
    Under **Agent feedback**, enable the **Enable agent feedback** toggle.
  </Step>
</Steps>

### How it works

When agent feedback is enabled, every [Markdown page view](/ai/markdown-export) includes an `<AgentInstructions>` block that tells AI agents how to submit feedback. Agents can then call the feedback endpoint with a description of the issue they encountered.

The feedback endpoint accepts a `POST` request to your documentation domain:

```bash
curl -X POST https://your-docs-domain.com/feedback \
  -H "Content-Type: application/json" \
  -d '{
    "path": "/getting-started",
    "feedback": "The installation command is outdated. The package was renamed from old-pkg to new-pkg in v3.0."
  }'
```

| Field | Type | Description |
|-------|------|-------------|
| `path` | string | The page path the feedback is about. |
| `feedback` | string | A description of the issue. |

### View agent feedback

Agent feedback appears in the [feedback dashboard](https://dashboard.mintlify.com/products/analytics/v2/feedback) alongside other feedback types. Use the **Show agent feedback** filter to view only feedback submitted by agents.
