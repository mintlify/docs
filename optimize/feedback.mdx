...
### Example agent feedback request

```bash
curl -X POST https://your-domain.com/feedback \
  -H "Content-Type: application/json" \
  -d '{
    "path": "/quickstart",
    "feedback": "The `mint dev` step fails on Node 18 because the lockfile requires Node 20. Add a prerequisites note."
  }'
```

Required fields:

- `path`: The root-relative path of the page the feedback is about. Example: `/quickstart`.
- `feedback`: A free-form string describing what is incorrect, outdated, or unclear.
