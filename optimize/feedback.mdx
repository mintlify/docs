...
The feedback section of the [analytics](https://app.mintlify.com/analytics) page displays quantitative thumbs-up and thumbs-down votes for your pages. It also displays any qualitative feedback users provide. A bar chart shows feedback over time, and the views below it list specific feedback items. Use this information to gauge the quality of your content and make improvements.
...
To collect and view feedback from users, you must enable feedback in the **Feedback** section of the [General](https://app.mintlify.com/settings/project/general) settings page in your dashboard.

1. Navigate to the [General](https://app.mintlify.com/settings/project/general) settings page.
2. In the **Feedback** section, enable your desired feedback types.
...
You can set the status of a piece of feedback and add internal notes to track your work resolving user feedback. To analyze feedback outside the dashboard, [export it to CSV](/analytics/export).
...
Connect an agent to your site's [search MCP server](/ai/model-context-protocol). The server exposes a `submit_feedback` tool that requires the page path and a description of the problem.

<Note>
  Web search and web fetch tools do not connect an agent to the search MCP server. Connect the agent to the server so it can discover and call the `submit_feedback` tool.
</Note>

Mintlify records each submission as unhelpful feedback for the specified page. The feedback appears in the dashboard alongside human feedback.
