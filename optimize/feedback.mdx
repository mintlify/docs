...
## Automatic abuse filtering

Mintlify automatically detects and filters abusive feedback from your analytics. When a user submits feedback, it is checked for profanity, hate speech, spam, and other non-genuine content. Flagged feedback is excluded from all metric counts and charts in the dashboard, so your analytics reflect only genuine user feedback.

Abuse detection runs automatically in the background using keyword matching and AI classification. No configuration is required.

Filtered feedback does not appear in the [analytics API](/api/analytics/feedback) responses or dashboard metrics.
