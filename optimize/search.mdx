...
Navigate to the [General](https://app.mintlify.com/settings/deployment/general) settings page in your dashboard. In the "Search" section, set **Results per query** to your desired value.

<Frame>
  <img
    alt="The Search section of the General settings page in the dashboard with Results per query set to 6."
...
Enable **Result snippets** to show a single-line preview of matching content under each search result, in addition to the page title and breadcrumbs. The snippet is windowed around the first match of the user's query, and the matching terms are highlighted. When a result has no query match, or the user hasn't typed a query yet, the snippet falls back to the start of the page's content.

Result snippets help users disambiguate results that share similar titles by showing where the query appears in context. Leave them off if you prefer a more compact results panel.

Enable or disable result snippets on the [General](https://app.mintlify.com/settings/deployment/general) settings page in your dashboard. The setting is off by default.
...
Enable or disable search filters on the [General](https://app.mintlify.com/settings/deployment/general) settings page in your dashboard.
