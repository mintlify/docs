...
When an unauthenticated user tries to access a protected page, Mintlify preserves their intended destination through a redirect flow:

1. The user attempts to visit a protected page (e.g., `/quickstart`)

2. Mintlify redirects them to your login page and adds the original destination as a `redirect` query parameter
