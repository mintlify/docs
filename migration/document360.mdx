...
Export a Document360 project as a ZIP, then convert its workspaces, languages, categories, and articles into a Mintlify project. Use the Document360 API and published site to audit content that the export does not represent completely.

<Note>
  The Mintlify scraper does not currently support Document360. Use a native project export as the primary source instead of running `@mintlify/scraping` against the published site.
</Note>

## Create a migration snapshot

Before exporting, create a named manual backup in **Settings > Knowledge base portal > Backup & Restore**. Backups are for recovery inside Document360. Download a project export for the migration itself.

In **Content tools > Import and export > Import and export project**:
...
Keep the ZIP unchanged as a source snapshot and perform conversion work in a copy.
...
Also export the **All articles** list as CSV. It contains metadata rather than article bodies, but it is useful for reconciling titles, publication status, dates, categories, and URLs.
...
Use the published sitemap and the Customer API to compare the export with public and private content. The API is particularly useful when you need to traverse category metadata or recover fields that are awkward to derive from exported files.
...
| --- | --- |
| Workspace | Version, product, tab, or separate navigation tree |
...
Choose the mapping for workspaces based on how readers use them. A workspace named `v1` is usually a version, while separate product knowledge bases may map better to products or tabs.
...
Document360 reserves the article title as the page H1, so article bodies usually begin at H2. Mintlify also renders the frontmatter title as the page heading. Do not add a duplicate H1 during conversion.
...
Check the rendered source site as well as the export. Some exports can contain expanded content instead of an obvious reusable-content reference.
...
Include media in the project export and copy owned files into the Mintlify repository. Check Document360 Drive links, thumbnails, SVGs, downloadable files, CSS background images, and content embedded from external services.
...
Recreate private documentation with [authentication and access control](/deploy/authentication-setup). Translate reader groups and role-based restrictions deliberately, and test as users in each access group before launch.

## Validate the result

- Reconcile the project ZIP, article CSV, API inventory, and published sitemap.
...
- Verify that API reference content comes from the maintained source specification.
...
- [Category API](https://apidocs.document360.com/apidocs/get-category)
