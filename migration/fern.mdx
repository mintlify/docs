...
<Note>
  The Mintlify scraper does not currently support Fern. Start from the source repository instead of running `@mintlify/scraping` against the published site.
</Note>

## Collect the source

Create a migration branch or copy of the repository and locate:

- `fern/docs.yml`, which defines site settings and navigation
- Markdown and MDX page directories, commonly `fern/docs/`
- Image, video, favicon, and logo assets
...
- Custom CSS, JavaScript, and MDX components

Validate and preview the existing Fern project before changing the source:
...
Record any environment variables, remote specifications, generated files, or private packages that these commands require. Avoid `fern generate --docs` during this audit because that command publishes the documentation unless you pass its preview options.
...
Remove Fern-only imports after conversion. Preview pages with nested components, because valid syntax and supported properties can differ even when both platforms use the same component name.

## Migrate API references

Prefer the original API definition over rendered reference pages.
...
Fern definitions can contain information that is not represented directly in an OpenAPI document. Review the generated OpenAPI output and move any missing descriptions or examples before retiring the Fern build.
...
Copy files from the configured asset directories and update relative paths after moving pages. Review `docs.yml` for logos, favicons, social images, fonts, colors, navbar links, announcement banners, redirects, analytics, custom CSS, and custom JavaScript.
...
- Compare every `docs.yml` navigation entry and discovered folder page with `docs.json`.
- Search for Fern component imports and unsupported JSX properties.
- Validate every product, version, tab, and API specification.
- Confirm that remote specifications and generated content have a maintained source.
- Test assets, internal links, heading anchors, and redirects against the published sitemap.
