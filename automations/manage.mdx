...
- A Mintlify project connected fkfkfk to a GitHub or GitLab repository
...
2. Click the toggle beside an automation to enable it.
   <Frame>
     <img
       alt="The automations dashboard."
       className="block dark:hidden"
       src="/images/workflows/workflows-dashboard-light.png"
     />
     <img
       alt="The automations dashboard."
       className="hidden dark:block"
       src="/images/workflows/workflows-dashboard-dark.png"
     />
   </Frame>
...
4. Select a run scope, then click **Run now**.

Manual runs save the automation's current configuration before starting, count toward your credit usage, and appear in the run history alongside scheduled runs. After a run starts, click **View runs** in the confirmation to open the run history filtered to that automation.

Automations with an **Integration** trigger can't run manually. They run when their trigger fires.

### Scope a manual run

Every manual run uses exactly one scope, which you select when starting the run:

- **Since a date**: Reviews content changed from the selected date up to now. Defaults to the automation's last run.
- **Everything**: Runs on your entire site with no date restriction. This typically takes more time and uses more credits than a targeted run.
- **Specific pull request(s)**: Limits the run to a pull request in a connected source code repository. Select a repository, then search its pull requests by title or number. For automations with a **Code change** trigger, the repository must be one of the configured source repositories.

You can also ask the [agent](/editor/agent) to start a scoped run. For example, `run the changelog automation over the last two weeks` or `run the update from code changes automation on PR #42 of acme/api`.
