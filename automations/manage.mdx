...
Click an individual run to open its detail page, which shows the run's prompt, summary, status, credits used, and any pull request it opened. The page also lists the conversations or feedback items that triggered the run, with **View feedback** links to inspect each source item in your dashboard.
