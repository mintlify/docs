...
- **Code change**: Runs when a pull request merges in a connected source code repository. You must specify at least one source repository. For GitHub repositories, you can also filter each repository by path and exclude specific pull request authors. See [Filter code change triggers](#filter-code-change-triggers).
...
- **Excluding PRs from**: Add GitHub usernames or bot accounts whose pull requests should not trigger the automation. Useful for skipping merges from automation accounts. Suggestions come from recent contributors; you can also type a custom username.

Each repository supports up to 20 paths and 20 excluded authors. Changing the repository on a card clears its filters. GitLab source repositories do not support per-repository filters and continue to use a single repository picker.
...
The **Preview** button only appears when a preview deployment is linked to the run. Preview deployments expire after a period of inactivity; once expired, the button is disabled.
