...
- **Slack channel message**: Runs when someone posts a new top-level message in a selected Slack channel. Available for custom automations. Invite the [Mintlify Slack app](/agent/slack) to each channel you want to select, then pick the channels in the settings panel. If the app is not a member of a selected channel, the settings panel blocks saving until you invite it. Mintlify skips thread replies, messages from bots, and messages that mention the Mintlify agent. Mintlify deduplicates by Slack event ID so a single message triggers each matching automation at most once. When the automation acts, it replies in the triggering thread with a summary and a link to any pull request it opens. Run detail in the dashboard shows the triggering message with a permalink back to Slack.
