...
You can retrigger individual failed runs from the runs list, or retry all eligible failed runs at once.

1. Go to the [Automation Runs](https://app.mintlify.com/products/automations) page in your dashboard.
2. To retry a single run, select **Retrigger** on the run's row.
3. To retry all eligible failed runs, set the status filter to **All runs** or include **Failed**, then select **Retry failed**. If an automation filter is active, only failed runs for that automation are retriggered.

Retriggered runs are queued in the background and appear as new entries in the run history.
