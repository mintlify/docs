---
title: "Manage automation settings in your Mintlify dashboard"
description: "Enable, disable, trigger, and delete automations. Configure repository, schedule, and integration triggers, context repositories, and automerge."
...
When you select **Custom schedule**, choose one of the following schedule options:
...
Use a preset for a common recurring cadence. Select **Custom cron** when you need a schedule that the presets do not cover. The settings panel shows a human-readable description for presets. It shows the cron input only when **Custom cron** is selected.

Custom cron expressions use five fields in this order: `minute hour day month weekday`. For example, the following expression runs the automation at 9:00 UTC from Monday through Friday:

```text
0 9 * * 1-5
```

Mintlify validates the expression before you save it. An existing schedule that does not match a preset opens with **Custom cron** selected so you can review or edit the expression.
