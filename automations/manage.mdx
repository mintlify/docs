...
A manual run started from the run button reviews everything that changed since the automation's last completed run. To run an automation over a different scope, ask the [agent](/editor/agent) to start the run. The agent supports three scopes:

- **Date range**: Review changes from a specific time window instead of the default one. For example, `run the changelog automation over the last two weeks`.
- **Everything**: Review all changes with no date restriction. For example, `run the style guide automation across everything`.
- **A single pull request**: Run from one pull request in a connected source code repository. For example, `run the update from code changes automation on PR #42 of acme/api`. For automations with a **Code change** trigger, the pull request must be in one of the configured source repositories.

A run uses exactly one scope. Giving a scope also lets you manually run automations that don't have a schedule trigger. Scoped runs count toward credit usage and appear in the run history like any other run.
