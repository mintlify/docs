...
You can only delete [custom automations](/automations/create). To stop a predefined automation, disable it instead. Deleting an automation is permanent and removes its configuration and run history.

1. Open the [Automations](https://app.mintlify.com/products/automations) page in your dashboard.
2. Click the <Icon icon="settings" /> settings button beside the automation.
3. Click **Delete**.
...
- **Failed**: The agent could not complete the run. Failed runs do not count toward your daily run limit. To retry, [run the automation manually](#run-an-automation-manually) or wait for its next scheduled trigger.
