...
- **Custom schedule**: Runs on a recurring schedule you define. Pick a preset (Daily, Every Monday, Every Friday, or Twice weekly) and a start hour, or select **Custom cron** and enter a standard 5-field cron expression (`minute hour day month weekday`). Cron values are stored in UTC; the dashboard converts preset hours to and from your local timezone. Automations queue within 10 minutes of the scheduled time.
...
You can delete custom automations from the dashboard. Predefined automations cannot be deleted from the dashboard—disable them instead, or use the CLI to remove them.
