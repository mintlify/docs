...
You can add up to 10 context repositories per automation. For each GitHub repository, install the Mintlify GitHub App. Add repositories in the **GitHub app** section of the [Git settings](https://app.mintlify.com/settings/project/git-settings) page.
...
You can act on results directly from the list. For runs awaiting review, click **Accept** to merge the changes or **Preview** to open the [preview deployment](/deploy/preview-deployments) for the run's branch. For failed results, click **Re-trigger** to start the run again.
...
To open a run in the editor, click **View changes** in the run's Slack notification or notification email. This option is only available while the run's pull request is still open.
