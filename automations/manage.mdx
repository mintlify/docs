...
- **Slack channel message**: Runs when someone posts a new top-level message in a selected Slack channel. Available for custom automations. Select one or more channels where the [Mintlify Slack app](/agent/slack) is a member. Mintlify skips thread replies, messages from bots, and messages that mention the Mintlify agent. Mintlify deduplicates by Slack event ID so a single message triggers each matching automation at most once. When the automation acts, it replies in the triggering thread with a summary and a link to any pull request it opens.
