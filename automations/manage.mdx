...
4. Select a run scope, then click **Run now** to confirm. Nothing runs until you confirm.

Each manual run covers one of three scopes:

- **Since a date**: Covers content changed from the date you pick up to now. Defaults to the automation's last run.
- **Everything**: Runs on your entire site. This typically takes more time and uses significantly more of your usage than a targeted run.
- **Specific pull request(s)**: Limits the run to pull requests you select in a connected repository.
...
Automations with an **Integration** trigger cannot run manually. They run when their trigger event fires. If the run button is unavailable, hover over it to see why.
