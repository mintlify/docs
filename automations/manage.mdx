...
1. Open the [Automation Runs](https://app.mintlify.com/products/automations) page in your dashboard.
2. Click into a completed run.
3. Click **View changes**.

The editor opens to the automation's branch with the agent panel already open. The agent panel shows each page the automation changed—click any page to open a diff view. The editor agent has full context on what the automation did, including the automation's prompt, a summary of the changes, and which pages the automation modified. You can ask the agent to refine or extend the work without re-explaining the background.

When you're satisfied with the changes, publish the branch from the editor to merge them.
