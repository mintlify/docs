...
A custom automation runs the agent on a schedule, on repository pushes, or on connected integration events using instructions that you write. Use a custom automation when you need behavior that isn't covered by a [predefined automation](/automations/reference), or when you want to combine your own prompt with specific context repositories and integrations. For an overview of triggers, update modes, and configuration options shared across all automations, see [Automations](/automations).
