...
Trigger a manual run before relying on a schedule or event to catch issues early. Manual runs use the automation's current configuration and appear in the run history alongside scheduled runs. See [Run an automation manually](/automations/manage#run-an-automation-manually) for the steps.

Open the run in the [Automations](https://app.mintlify.com/products/automations?tab=runs) page to review the prompt, the files the agent read or changed, and any pull request it opened. If the result isn't what you expected, edit the instructions and run it again.
