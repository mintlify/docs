...
## Reviewing automation merge requests in GitLab

When an automation opens or updates a merge request, Mintlify adds context directly to the MR so you can review without leaving GitLab.

- **MR title** includes the name of the automation that generated it, so you can identify automation MRs at a glance when scanning an open MR list.
- **MR description** summarizes what changed and why. Each time a run appends new changes to an existing MR, the description updates to show a running timeline of what each run contributed.
- **MR comments** for any runs that append changes, explaining the changes and which source code merge requests the agent used as context.
- **Reviewers** assigned based on who authored the source code changes that triggered the update. Applies to automations that use source code repositories as context.
