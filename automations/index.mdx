...
- **Webhook**: Runs when an authenticated request hits the automation's webhook endpoint. Available for [custom automations](/automations/create) only. See [Trigger automation webhook](/api/automations/webhook).
...
<Note>
  Draft changelog, Fill gaps from assistant conversations, Improve docs from user feedback, and Update from support feedback do not group or append. Each run opens its own pull request with the changes from that run. Update from support feedback can open up to three pull requests per run, one for each distinct theme it identifies.
</Note>

Custom automations open a new pull request by default. A single run can open several pull requests when the work splits into independent changes. To add changes to an open pull request from an earlier run, tell the agent to do so in your instructions. See [Open multiple pull requests or append to open ones](/automations/create#open-multiple-pull-requests-or-append-to-open-ones).
