...
keywords: ["integrations", "Slack agent", "Notion", "Linear", "Jira", "Confluence", "Salesforce", "Salesforce Service Cloud", "PostHog", "Intercom", "Google Drive", "OneDrive", "Gmail", "Outlook", "HubSpot", "Plain", "automation trigger", "context"]
---
...
PostHog provides read-only access to product analytics data, including projects, insights, dashboards, events, feature flags, experiments, and surveys. Use it when the agent or an automation needs to analyze product behavior or retrieve analytics context.

Salesforce Service Cloud provides read-only access to support cases and service records. Use it when the agent or an automation needs to retrieve case details, query service data with SOQL, or inspect Salesforce object fields.

Both integrations are shared. To configure either one, connect it from the [Integrations](https://app.mintlify.com/settings/organization/integrations) page, or select it in an automation's **Tools** field and complete the authorization flow. After connecting, the integration is available to the Slack agent and supported automations.
