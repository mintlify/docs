...
keywords: ["integrations", "Slack agent", "Notion", "Linear", "Jira", "LaunchDarkly", "Confluence", "Salesforce", "Salesforce Service Cloud", "PostHog", "Intercom", "Google Drive", "OneDrive", "Gmail", "Outlook", "HubSpot", "Plain", "automation trigger", "context"]
---
...
| Salesforce Service Cloud | Shared | No |
| Zendesk | Shared | Yes |

PostHog provides read-only access to product analytics data, including projects, insights, dashboards, events, feature flags, experiments, and surveys. Use it to analyze product behavior or retrieve analytics context.

Salesforce Service Cloud provides read-only access to support cases and service records. Use it to retrieve case details, query service data with SOQL, or inspect Salesforce object fields.

LaunchDarkly provides read-only access to feature flags, projects, environments, flag statuses, segments, experiments, metrics, audit log entries, and account members. Use it to review feature-flag configuration, rollout status, experimentation data, or audit activity. LaunchDarkly is available as an automation tool, but not as an automation trigger.

These integrations are shared. To configure any of them, connect it from the [Integrations](https://app.mintlify.com/settings/organization/integrations) page. You can also select it in an automation's **Tools** field and complete the authorization flow. After connecting, the integration is available to the Slack agent and supported automations.
