...
keywords: ["integrations", "Slack agent", "Notion", "Linear", "Jira", "LaunchDarkly", "Confluence", "Salesforce", "PostHog", "Intercom", "Google Drive", "OneDrive", "Gmail", "Outlook", "HubSpot", "Plain", "Pylon", "Stripe", "automation trigger", "context"]
---
...
| **Automation tool** | Gives an automation read-only context from the shared integrations selected in its **Integrations** setting, under **Advanced options**. |
| **Automation trigger** | Starts a custom automation when a selected event occurs in a supported shared integration or when a new message posts to a selected Slack channel. |

<Note>
  The [Mintlify Slack app](/agent/slack) is separate from the third-party integrations on this page. Installing the Slack app adds the agent to your workspace and gives it read access to Slack content. You do not connect Slack from the **Integrations** settings page or select it in an automation's **Integrations** field. After you install the Slack app, you can select Slack as a custom automation trigger and the [editor agent](/editor/agent#read-your-slack-workspace) can read your workspace for context.
</Note>
...
You can also connect an integration while configuring an automation. Under **Advanced options**, select an unconnected app in the automation's **Integrations** field, then complete the authorization flow.
...
For example, ask the agent to compare a Linear issue with a Google Drive product brief or answer a question using a Notion page. You can also ask it to update documentation using context from a Jira issue.
...
3. In **Integrations**, select the integrations that the automation can use.
4. Connect any selected integration that is not already connected.
...
Custom automations can run when an event occurs in a supported shared integration or when a new message posts to a selected Slack channel.
...
When you use Slack messages as a trigger, only new top-level messages in selected channels start a run. Thread replies do not trigger automations.
