...
Every pull request opened by this automation links back to the source code pull request or pull requests that triggered it. The links appear in the pull request description and in a comment on any subsequent run that appends new changes, so reviewers can trace each documentation change to the code change it came from.
...
Select a single support platform when you configure the automation and connect it from the settings panel if it isn't already connected for your organization. The automation currently supports Intercom and Plain. Other support platforms, including Zendesk and Salesforce Service Cloud, are not yet supported.

Add source code repositories as context to help the agent tie support themes back to the product areas they describe. Context repositories work the same way as for other automations, supporting both GitHub and GitLab. See [Context repositories](/automations/manage#context-repositories) for setup details.
...
Add optional instructions on the automation's settings panel to steer the agent toward specific product areas, ticket labels, or writing conventions. Instructions apply on every run and append to the automation's base prompt.

This automation requires the [automation integrations](/automations/integrations) that power Intercom and Plain connections, which are available on the Enterprise plan. Contact your account team if you don't see it in the dashboard.
