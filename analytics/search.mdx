...
A click counts only when a user selects a specific result, either with the pointer or by pressing Enter when a result is highlighted. Asking the AI assistant from the search bar instead of selecting a result does not count as a click.
