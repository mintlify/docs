...
CTR counts a search as clicked only when a user selects a specific result, either with the pointer or by activating a highlighted result with the keyboard. If a user asks the AI assistant instead of clicking a result, that search is not counted as a click.
