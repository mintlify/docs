...
description: "Configure Enterprise analytics streaming to send selected event categories and deployments to Amazon S3."
...
<Accordion title="Prepare Amazon S3 credentials">
  Create an AWS Identity and Access Management (IAM) user with an access key whose policy allows writing to the target bucket. Mintlify does not support role assumption or temporary credentials that require a session token. At minimum, grant `s3:PutObject` on the bucket and key prefix that you plan to use.
...
Select which of your organization's deployments send events. The dashboard lists each deployment by subdomain. New deployments do not automatically start streaming events.
