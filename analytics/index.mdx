...
The [analytics](https://app.mintlify.com/analytics) page in your dashboard shows how visitors and agents find and use your documentation. Use these signals to identify valuable pages, discover content gaps, and track changes over time.

<Tip>
  Ask the [agent](/editor/agent#ask-about-your-analytics) questions about your analytics data directly in the chat panel. The agent can answer questions about the data and provide insights on how to improve your documentation.
</Tip>
...
<Card title="Savings" horizontal icon="piggy-bank" href="/analytics/savings">
  Estimate hours saved by the agent and automations, and support costs saved by the assistant.
</Card>
...
Use the **Filter** menu to scope the dashboard to a single page or path. When you select a path, the analytics page displays data for every page under that path. For example, if you filter to `docs/authentication`, the analytics page displays data for every page in the `docs/authentication/*` path. The Assistant and Search views always show site-wide data.

In the Traffic view, filtering by page changes the popular pages ranking to start at the selected page and continue down the ranking, rather than showing only that page.
