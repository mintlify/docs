...
In interactive mode, the CLI fetches and displays available templates automatically. To pass a template by ID, use one of the following values with `--template`:

| Template ID | Description |
| --- | --- |
| `developer-docs` | The default Mintlify [starter template](https://github.com/mintlify/starter) for developer-facing product and API documentation. |
| `help-center` | A customer-facing [help center](/guides/help-center) with a hero, search entry, and card-based navigation. |
| `knowledge-base` | An internal [knowledge base](/guides/knowledge-base) with department pages, onboarding content, and policy sections. |
| `product-guide` | A multi-product guide site with a landing page and product selector. |
| `empty` | A minimal blank site with a single placeholder page. Use this to build from scratch. |

Browse the full source of each template in the [mintlify/templates](https://github.com/mintlify/templates) repository on GitHub.
