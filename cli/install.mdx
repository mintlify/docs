...
After installing the CLI, log in to your Mintlify account to enable commands that interact with your deployments (such as `mint analytics`):

```bash
mint login
```

This opens a browser window to complete authentication. See [`mint login`](/cli/commands#mint-login) for details.
