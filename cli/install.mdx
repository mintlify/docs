...
| `--template` | Use a pre-defined template as a starting point instead of the default starter. |
| `--force` | Overwrite the current directory without prompting. |

In interactive mode, the CLI prompts you to choose between starting from scratch or selecting from available templates.
