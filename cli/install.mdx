...
| `--template` | Create the project from a pre-defined template. |
| `--force` | Overwrite the current directory without prompting. |

In interactive mode, the CLI asks whether you want to start from scratch or use a template. To skip the prompt, pass the `--template` flag directly:
