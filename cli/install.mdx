...
<Note>
  The package name is `mint`, not `mintlify`. The legacy `mintlify` package is deprecated. If you previously installed `mintlify`, see [mintlify versus mint package](#troubleshooting) in the troubleshooting section below.
</Note>
