...
To use them, select the content you want to wrap, then run **Snippets: Surround With** from the command palette and select a component. Snippets are available for `AccordionGroup`, `CardGroup`, `CodeGroup`, `Expandable`, `Frame`, `RequestExample`, `ResponseExample`, and fenced code blocks.
