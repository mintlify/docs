...
The extension also opens `.mdx` files in a visual editor and opens `.mdx` files in a visual editor and runs a live preview inside your editor, so you can write and see rendered output without switching to a browser.
...
Type `<` to view every built-in component. Autocomplete suggests components' properties and values inside tags, closing tags after `</`, and enumerated prop values like `<Badge color="…">`, closing tags after `</`, and enumerated prop values like `<Badge color="…">`.
