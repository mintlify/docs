...
You must be logged in to use analytics commands. Run `mint login` to authenticate with your Mintlify account.
...
Use the `--agent` flag for JSON output. The CLI auto-detects agent environments when the `CLAUDECODE` environment variable is set to `1`.
