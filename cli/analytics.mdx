...
The CLI auto-detects agent environments when the `CLAUDECODE` environment variable is `1` and defaults to JSON output.
...
The `subdomain` is set automatically when you run `mint login`. If you have multiple projects, the CLI prompts you to choose one. You can override the default at any time with `mint config set subdomain`.

Available configuration keys:

| Key | Description |
| --- | --- |
| `subdomain` | Default subdomain for analytics commands. Set automatically during `mint login`. |
