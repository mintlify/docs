...
You must log in to use analytics commands. Run `mint login` to authenticate with your Mintlify account. During login, the CLI prompts you to select a default project, which analytics commands use automatically.
...
The CLI automatically detects AI agent environments (non-interactive terminals, Claude Code) and defaults to JSON output.
...
| Key | Description |
| --- | --- |
| `subdomain` | Default subdomain for analytics commands. Set automatically during `mint login` if you select a project. |
