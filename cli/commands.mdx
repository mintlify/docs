...
| `openApiSpec` | Any OpenAPI or Swagger specification served at a standard path is valid. This check passes when no spec is served and only fails when a served spec file is not a valid OpenAPI or Swagger document. |
...
| `pagesHaveMarkdown` | Pages sampled from the sitemap have a corresponding Markdown counterpart (`path.md` or `path/index.md`) that serves non-HTML content. Passes at 90% or higher coverage. |
