...
Run agent readiness checks against a public documentation site. Returns a score and a breakdown of individual checks. Requires authentication with `mint login`.
...
| Flag | Description |
| --- | --- |
| `--url` | URL of the docs site to check. Required. |
| `--format` | Output format: `table` (default, colored output), `plain` (pipeable TSV), or `json`. |
