...
The score evaluates the following checks:

| Check | What it verifies |
| --- | --- |
| `llmsTxtExists` | An `llms.txt` file is reachable at the site root. |
| `llmsTxtValid` | The `llms.txt` file is valid Markdown with a title heading. |
| `llmsTxtSize` | The `llms.txt` file is within reasonable size limits. |
| `llmsTxtLinksResolve` | Links inside `llms.txt` return successful responses. |
| `llmsTxtLinksMarkdown` | Linked pages serve Markdown content. |
| `llmsTxtDirective` | Pages include a `Link` or `X-Llms-Txt` header pointing to `llms.txt`. |
| `llmsTxtFullExists` | An `llms-full.txt` file is reachable at the site root. |
| `llmsTxtFullSize` | The `llms-full.txt` file is within reasonable size limits. |
| `llmsTxtFullValid` | The `llms-full.txt` file is valid Markdown with a title heading. |
| `llmsTxtFullLinksResolve` | Links inside `llms-full.txt` return successful responses. |
| `skillMd` | A `.well-known/skills/mintlify/skill.md` file is discoverable. |
| `contentNegotiationMarkdown` | Pages return Markdown when requested with `Accept: text/markdown`. |
| `contentNegotiationPlaintext` | Pages return plain text when requested with `Accept: text/plain`. |
| `mcpServerDiscoverable` | An MCP server endpoint is discoverable on the site. |
| `mcpToolCount` | The MCP server exposes at least one tool. |
| `openApiSpec` | An OpenAPI specification is available. |
| `robotsTxtAllowsAI` | The `robots.txt` file does not block known AI user agents. |
| `sitemapExists` | A `sitemap.xml` file is reachable at the site root. |
