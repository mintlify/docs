...
Each checked page consumes one AI credit. The command skips pages under 50 words and does not charge for them. The command exits with code `1` when any page is flagged at or above the threshold, and `0` when every page is clean. This lets you run it in a rewrite loop or in CI.
