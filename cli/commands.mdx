...
The CLI prompts you for your first name, last name, company name, and email, then opens a browser window to verify your email. After you click the verification link, authentication completes automatically and your credentials save in `~/.config/mintlify/config.json`. Once signed up, you are logged in and can run `mint status` to confirm the session.
