...
Opens a browser window to complete authentication. If the browser does not open, the CLI displays a URL to open manually and a prompt to paste the authorization code. After a successful login, the CLI prompts you to select a default project when your organization has multiple projects, or automatically sets it when there is only one. Credentials save in `~/.config/mintlify/config.json`.
