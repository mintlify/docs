...
| --- | --- | --- |
| `subdomain` | Default documentation subdomain. | `mint analytics`, `mint score` |
...
If you omit `url`, the CLI runs the checks against your default subdomain (`<subdomain>.mintlify.app`). The default comes from the `subdomain` key set by `mint login` or `mint config set subdomain <subdomain>`. If no default is configured, the CLI uses the first project on your account.

```bash
# Check your default subdomain
mint score
