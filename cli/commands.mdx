...
Commands exit with code `0` on success and a non-zero code on failure. Checks like `mint validate`, `mint broken-links`, `mint a11y`, `mint deslop`, and `mint format` exit non-zero when they find issues. This lets you run them in CI to fail a pipeline on regressions.
