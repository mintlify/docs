...
Opens a browser window to complete authentication. If the browser does not open, the CLI displays a URL to open manually and a prompt to paste the authorization code. After login, the CLI prompts you to select a default project. If your account has a single project, it is set automatically. Credentials and the default project save in `~/.config/mintlify/config.json`.
