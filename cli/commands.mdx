...
The command displays an overall readiness score, a letter grade (A–F), and a breakdown of individual checks with their status.

### Checks

Scoring is powered by [AFDocs](https://afdocs.dev), an open standard for evaluating how agent-friendly a documentation site is. Each run evaluates your site across these areas:

| Area | What it covers |
| --- | --- |
| `llms.txt` | Whether an [llms.txt](/ai/llmstxt) file is available, well-formed, appropriately sized, and links to live pages using Markdown. |
| `llms-full.txt` | Whether an [llms-full.txt](/ai/llmstxt/#llms-full-txt) file is available, contains a valid Markdown structure, and its links resolve. |
| `skill.md` | Whether a [skill.md](/ai/skillmd) file is reachable for agent tool use. |
| Content negotiation | Whether the site returns Markdown and plain text when agents request them. |
| MCP | Whether an [MCP server](/ai/model-context-protocol) is discoverable and exposes at least one tool. |
| API specifications | Whether an OpenAPI or Swagger specification is available at a standard path. |
| Crawler access | Whether `robots.txt` allows AI crawlers and a sitemap is available for page discovery. |
| Structured data | Whether the homepage includes [JSON-LD](https://json-ld.org/) structured data for agents to parse. |
| Response latency | Whether the site responds within an acceptable time for agents. |

Some checks only run if a check they depend on passes. For example, link validation for `llms.txt` only runs if the file is reachable.

The overall score uses weighted scoring, so higher-impact checks contribute more to your final score and letter grade.
