...
Create a Mintlify account from the CLI.
...
Run without flags to sign up interactively — the CLI prompts you for your first name, last name, company name, and email, then opens a browser window to verify your email. After you click the verification link, authentication completes automatically and your credentials save in `~/.config/mintlify/config.json`. Once signed up, you are logged in and can run `mint status` to confirm the session.

Pass the flags above to skip the prompts and sign up non-interactively, which is useful when a coding agent runs the command on your behalf:

```bash
mint signup --firstName Ada --lastName Lovelace --company Analytical --email ada@example.com
```

After signup completes, visit the [Mintlify dashboard](https://dashboard.mintlify.com) to connect your GitHub repository and finish setting up your deployment.
