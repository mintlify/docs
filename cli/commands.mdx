...
Commands exit with code `0` on success and a non-zero code on failure. Checks like `mint validate`, `mint broken-links`, `mint a11y`, `mint deslop`, and `mint format` exit with a non-zero code when issues are found, so you can run them in CI to fail a pipeline on regressions.
