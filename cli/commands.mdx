...
The `mint` CLI requires [Node.js](https://nodejs.org) v20.17.0 or higher. Install the CLI globally with npm:

```bash
npm i -g mint
```

Run commands from the root of your documentation project (the directory containing `docs.json`) unless a command explicitly accepts a path argument. See the [Quickstart](/quickstart) for a full walkthrough.
