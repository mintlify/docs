...
The command uses your configured subdomain from `mint config`. If none is set, it uses the first subdomain on your account.

After the domain is registered, the CLI waits up to 10 seconds for DNS records to generate, then prints the `TXT` and `CNAME` records to add at your domain provider:
...
Add the `TXT` records first, then add the `CNAME` once the verification records are validated. See [Custom domain](/customize/custom-domain) for full DNS setup instructions, apex domain requirements, and TLS provisioning details.
