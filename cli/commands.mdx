...
Install the hosted Mintlify Index MCP server for your coding agents. The server provides a `context` tool for researching libraries, frameworks, SDKs, APIs, and CLI tools.
...
The command is also available as `mintlify index`.
...
Run `mint index` without client options to detect installed clients and open an interactive picker. Detected clients are selected by default. Select the clients to configure, then confirm the prompt.
...
Use `--yes` to configure every detected client without prompts. If no clients are detected, pass an explicit client option such as `--claude` or `--cursor`.
