...
| --- | --- | --- |
| `subdomain` | Default documentation subdomain. | `mint dev`, `mint automations`, `mint analytics`, `mint add-domain`, `mint score` |
...
Format every `.mdx` file in the current directory to Mintlify's canonical style. The command parses each file with the same converter the web editor uses, then rewrites it in place if the canonical output differs. Formatted files match the output that the web editor writes when it publishes, so publishing from the editor on a formatted repository produces a diff that contains only the edited lines.

```bash
mint format
```

Run the command from the root of your docs project. It walks every subdirectory, skipping paths matched by `.gitignore` and any Mintlify ignore rules. Files that already match the canonical output are left untouched. If formatting would remove a file's frontmatter, the command leaves that file unchanged and reports it as a failure.

<Warning>
  `mint format` rewrites files in place. Commit or stash your changes before running it so you can review the diff. The first run on an existing project can produce a large one-time diff as files are normalized, including frontmatter quoting, list spacing, and comment indentation. After that run, repeated runs leave already-formatted files untouched.
</Warning>

When it finishes, the command prints how many MDX files were reformatted and how many failed. If any file fails, the command exits with code `1` and prints the file path and error, so you can run it in CI to enforce consistent formatting.
