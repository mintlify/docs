...
Some checks are children of other checks. When a parent check fails, its children are automatically marked as failed. For example, `llmsTxtValid` only passes if `llmsTxtExists` passes first. The overall score uses weighted scoring, so higher-impact checks contribute more to your score.
