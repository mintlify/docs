...
After you authenticate with the `mint login` command, the CLI prompts you to select a default project if your account has multiple projects. The CLI stores your credentials and default project in `~/.config/mintlify/config.json` so you stay logged in across sessions.
