...
After you authenticate with the `mint login` command, the CLI prompts you to select a default project (or auto-sets it if your organization has only one). Credentials save in `~/.config/mintlify/config.json` so you stay logged in across sessions.
