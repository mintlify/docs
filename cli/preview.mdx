...
## Troubleshoot the local preview

If `mint dev` fails to start or behaves unexpectedly, check the following:

- **Missing or invalid `docs.json`**: Run `mint dev` from the directory that contains your `docs.json`. If the file exists, run `mint validate` to confirm it parses correctly.
- **Outdated CLI**: Run `mint update` to install the latest version. Older versions may fail on newer configuration fields.
- **Node version**: The CLI requires a currently supported Node.js version. Upgrade Node if you see engine or syntax errors on startup.
- **Port conflicts**: If the CLI can't find an open port near 3000, pass `--port` with a free port.
- **Slow page loads on large sites**: Use `--disable-openapi` and `--disable-prefetch` to reduce background work.

For a full list of commands and flags, see the [CLI reference](/cli/commands).
