---
title: "Quickstart"
description: "Deploy your documentation site and make your first change."
keywords: ["quickstart", "deploy", "get started", "first steps"]
---

Deploy a live documentation site in minutes.

## Deploy your site

<Steps>
  <Step title="Create your account">
    Go to [mintlify.com/start](https://mintlify.com/start) and sign up.
  </Step>
  <Step title="Choose repository option">
    - **Mintlify-hosted**: Click **Skip** when prompted to connect GitHub. Mintlify creates a private repository for you.
    - **Your own repository**: Connect your GitHub account and follow the prompts. Install the [GitHub App](https://dashboard.mintlify.com/api/github/install-app) to enable automatic deployments.
  </Step>
  <Step title="Complete setup">
    Follow the onboarding prompts. Your site deploys automatically to `https://<your-project-name>.mintlify.app`.
  </Step>
</Steps>

<Tip>
  Start with Mintlify-hosted if you're unsure. You can migrate to your own repository later.
</Tip>

## Make your first change

<Steps>
  <Step title="Open the web editor">
    Navigate to the [web editor](https://dashboard.mintlify.com/editor) in your dashboard.
  </Step>
  <Step title="Edit a page">
    Open `index.mdx` and update the frontmatter:
    
    ```mdx
    ---
    title: "Introduction"
    description: "Your custom description here"
    ---
    ```
  </Step>
  <Step title="Publish">
    Click **Publish** in the top-right toolbar. Your changes deploy automatically.
  </Step>
</Steps>

## Next steps

<CardGroup cols={3}>
  <Card title="Learn about the web editor" icon="mouse-pointer-2" href="/editor">
    Explore editing features and workflows.
  </Card>

  <Card title="Set up local development" icon="terminal" href="/installation">
    Use the CLI to preview your documentation locally as you write and edit.
  </Card>

  <Card title="Add a custom domain" icon="globe" href="/customize/custom-domain">
    Use your own domain for your documentation site.
  </Card>
</CardGroup>
