...
After you complete this guide, you'll have a live docs site ready to customize and update.
...
- **Preview before publishing.** Run `mint dev` locally or use the web editor preview to catch formatting and broken links before your changes go live.
- **Keep pages focused.** Each page should cover a single topic or task. Split long pages into smaller ones and link between them for easier navigation.
- **Write descriptive frontmatter.** Clear `title` and `description` values improve search results, SEO, and how your pages appear when shared.