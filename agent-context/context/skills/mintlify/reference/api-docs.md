...
- `x-codeSamples`: Custom code examples per endpoint. Write these by hand, or if SDKs are generated with Speakeasy, point the `openapi` field in `docs.json` at the Speakeasy combined spec URL (the `*-with-code-samples` API Registry entry) to pull autogenerated samples.
