...
To also verify that every redirect destination in your `docs.json` resolves to a valid path, use the `--check-redirects` flag. This catches redirects that point to pages that no longer exist.
