...
The name you use to render an imported snippet as a JSX tag should start with an uppercase letter, such as `MySnippet`. MDX treats lowercase tags such as `<mySnippet />` as literal HTML or custom element names rather than references to imported snippets. Use PascalCase for snippet names as a convention.

Curly braces in an `import` statement depend on how the snippet is exported. Import the rendered contents of an `.md` or `.mdx` file as a default export without braces (`import MySnippet from "..."`). Use braces for named exports such as variables or React components (`import { myName } from "..."` or `import { MyComponent } from "..."`).
