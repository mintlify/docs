...
You can also reference snippet variables inside code blocks. Use the same `{variableName}` syntax in the body of a fenced code block, and the value passed in as a prop replaces the placeholder when the page renders. Placeholders that don't match a prop are left unchanged.

1. Add a code block to your snippet that references a variable.
...
2. Import the snippet and pass the value as a prop.

    ```mdx destination-file.mdx
    ---
    title: "An example page"
    description: "This is an example page that imports a snippet with a code block variable."
    ---
