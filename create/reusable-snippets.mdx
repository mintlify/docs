...
Any MDX, JSX, or MD file automatically becomes a snippet when it's imported by another file. When a file is imported:

- It will no longer render as a standalone page
- It becomes reusable content that can be imported into multiple files
- It won't appear in your site's navigation or sitemap

When you remove all imports of a file, it automatically becomes a page again and will render as a standalone page in your documentation.

## Create snippets

Create an MDX, JSX, or MD file anywhere in your project with the content you want to reuse. Snippets can be located in any directory—they don't need to be in a specific "snippets" folder. Snippets can contain all content types supported by Mintlify and they can import other snippets.

## Import snippets into pages

To use snippets, import them into pages using either an absolute or relative path. When you import a file, it automatically becomes a snippet and will no longer render as a standalone page.

- **Absolute imports**: Start with `/` for imports from the root of your project (e.g., `/snippets/my-snippet.mdx` or `/components/shared-content.mdx`).
