...
The name you assign in the `import` statement is the JSX tag you use to render the snippet. It must be a valid JSX component identifier: start with an uppercase letter and use PascalCase (for example, `MySnippet`, not `mySnippet` or `my-snippet`). Names starting with a lowercase letter are treated as HTML elements and do not render your snippet.
