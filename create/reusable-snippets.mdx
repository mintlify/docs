...
<Note>
  Snippets must be `.mdx`, `.md`, `.js`, or `.jsx` files. Importing a `.json` or `.yaml` file directly is not supported. Keep the data in a `.js` snippet, or [generate one from your JSON or YAML source](#generate-snippets-and-pages-from-json-or-yaml).
</Note>
...
If the data lives in a JSON or YAML file, for example one that another team already maintains, generate the docs from it instead of editing them by hand. A short script can write the data snippet, one page per entry, and the matching navigation group. Run it in CI whenever the source file changes and commit the result, so your docs update automatically.
...
<Steps>
  ...
  <Step title="Run it in a GitHub Action">
    ...
    If the source file lives in another repository, run the workflow there instead: check out the docs repository with a token that can push to it, run the script, and commit.
  </Step>
</Steps>

<Tip>
  Generator scripts like this are a good task for an AI coding agent. Point it at the source file and a page you want as output, and ask it to write the script. When the data gains a field or the page layout changes, ask it to update the script and regenerate.
</Tip>
