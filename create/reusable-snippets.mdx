...
Add [glob patterns](https://code.visualstudio.com/docs/editor/glob-patterns) to the `snippets` array in `docs.json` to specify which folders contain snippets. This allows you to organize snippets anywhere in your project structure.
...
The `snippets` field accepts glob patterns, which means you can:

- Match entire directories: `"components/**"` matches all files in the `components` folder and its subdirectories
- Match specific file patterns: `"shared/*.mdx"` matches all MDX files directly in the `shared` folder
- Match individual files: `"shared/common-component.mdx"` matches a specific file

The default `/snippets/` folder remains supported even when you configure custom patterns. You don't need to include it in the `snippets` array.
