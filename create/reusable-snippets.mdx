...
<Note>
  MDX expressions, including imported variables like `{myName}` and inline expressions like `{1 + 1}`, are evaluated in the browser. Their computed values are not present in pre-hydration HTML or in [offline exports](/deploy/export). Consumers that do not run JavaScript, such as crawlers and LLM ingestion, see the surrounding text without the computed values. Write a value as plain text when it must be visible to those consumers.
</Note>
