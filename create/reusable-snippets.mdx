...
You can also reference snippet variables inside code blocks. Use the same `{variableName}` syntax in the body of a fenced code block. When the page renders, Mintlify replaces each placeholder with the matching prop value and leaves any unmatched placeholders as-is.
