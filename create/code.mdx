...
Hide the copy-to-clipboard button on a code block using `noCopy`. Use this for content where copying is not meaningful, like ASCII diagrams or terminal output.

The `noCopy` flag also accepts `no-copy` and `nocopy` as aliases, in any casing. Pass `noCopy="false"` to explicitly show the copy button.

For a code block with no language, use `no-copy` on its own to render the block as plain text without a copy button.
...
Inside a `<CodeGroup>`, the shared copy button hides when the selected tab sets `noCopy` and reappears when you switch to a tab without it.

### Diff

Show a visual diff of added or removed lines in your code blocks. The visual diff highlights added lines in green and removed lines in red.
