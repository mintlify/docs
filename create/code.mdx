...
Use [fenced code blocks](https://www.markdownguide.org/extended-syntax/#fenced-code-blocks) by enclosing code in three backticks. Code blocks are copyable, and if you have the assistant enabled, readers can ask the assistant to explain the code.
...
In JavaScript and TypeScript code blocks, use `twoslash` to enable interactive type information. Readers can hover over variables, functions, and parameters to see types and errors like in an IDE.
...
Allow readers to expand and collapse long code blocks using `expandable`.
