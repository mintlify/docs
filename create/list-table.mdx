...
When every `<col>` declares a width, through the `width` attribute or an inline style, the table uses your widths and wraps long content within each column. If any `<col>` is missing a width, Mintlify ignores the declared widths and sizes columns based on content. Tables that are too wide for the page still scroll horizontally.
