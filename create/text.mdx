...
Custom IDs are preserved exactly as you write them, including casing and underscores. You can use camelCase, kebab-case, snake_case, or any combination:
...
These headings are reachable at `#quickStart`, `#setup_steps`, and `#advanced-options` respectively. Match the casing exactly when linking to a custom ID—anchors are case-sensitive.

<Tip>
  Preserved casing is helpful when migrating from documentation tools that use camelCase or snake_case anchors. Keep your existing custom IDs to avoid breaking inbound links.
</Tip>
