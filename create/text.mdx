...
| Heading | Anchor ID | Link fragment |
| --- | --- | --- |
| `Restrict Claude's skill access` | `restrict-claude’s-skill-access` | `#restrict-claude%E2%80%99s-skill-access` |
...
You can override the generated ID with a custom one using the `{#custom-id}` syntax:
