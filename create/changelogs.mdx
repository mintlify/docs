...
An `Update` is shown only when every one of its tags is included in the active filter selection. Updates without any tags are hidden whenever at least one filter is active. To make an update appear under multiple filters, list each of those tags on the `Update`.
