...
Add `tags` to your `Update` components to replace the table of contents with tag filters. Readers can filter the changelog by selecting one or more tags.
...
Selected tags are reflected in the page URL as a `?tags=` query parameter, so readers can share deep links to a filtered view of the changelog. Opening a link like `/changelog?tags=Spearmint` activates matching sidebar filters on load, and updating filters keeps the URL in sync. Separate multiple tags with commas, for example `/changelog?tags=Spearmint,Peppermint`.
