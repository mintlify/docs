...
Add `tags` to your `Update` components to replace the table of contents with tag filters. You can filter the changelog by selecting one or more tags.

When you select multiple tags, the changelog shows only updates that include every selected tag. The changelog hides updates without tags whenever a filter is active.
