...
description: "Preview deployments are not generated for pull requests opened from a fork or for fork branches passed to the preview API. Push the branch to the main repository instead."
...
If a pull request opened from a fork shows no preview deployment link, or a call to the [Trigger preview deployment](/api/preview/trigger) endpoint with a fork branch fails, this is expected behavior. Mintlify does not build preview deployments from fork branches.
