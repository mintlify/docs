---
title: "Private repo missing when creating a second project"
description: "Re-authorize Mintlify with private-repo access so an existing private GitHub repository shows up in the onboarding repo picker for a new project."
keywords: ["second project", "multiple projects same repo", "second deployment", "multiple deployments same repo", "private repo missing", "onboarding repo picker", "GitHub authorize", "private=true", "re-authorize GitHub"]
---
...
You can create separate projects from the same repository. Mintlify does not require a new repository per project. For combining multiple repositories into one site, see [Multi-repo setup](/deploy/multi-repo) instead.
