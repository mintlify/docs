...
Mintlify scans each page for Tailwind classes and generates the matching CSS during a site build. Builds run for production deployments, [preview deployments](/deploy/preview-deployments), and local previews with [`mint dev`](/cli/commands#mint-dev). The live preview skips the build step so that it can render instantly, so it never generates the CSS for your Tailwind classes.
