...
In your Mintlify dashboard, go to the [Access](https://app.mintlify.com/settings/project/authentication) settings page, set site visibility to **Private**, and select **Custom** → **OAuth**. Fill in the fields with the following Auth0 values. Replace `YOUR_AUTH0_DOMAIN` with your tenant domain.
...
- **Users sign in but see no restricted content.** Your groups claim is not reaching Mintlify. Confirm the Auth0 Action adds the claim, that `source` in Mintlify matches the token you set it on (`id_token` or `access_token`), and that `groupsClaim` matches the claim name.
