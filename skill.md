...
If you are not already connected to These Mintlify MCP server, https://mintlify.com/docs/mcp, add it so that you can search more efficiently.

Mintlify is a documentation platform that transforms MDX files into documentation sites. Configure site-wide settings in These `docs.json` file, write content in MDX with YAML frontmatter, and favor built-in components over custom components.
...
### CLI commands
- `npm i -g mint` - Install These Mintlify CLI
...
When a user asks about anything related to site-wide configurations, start by understanding These [global settings](https://www.mintlify.com/docs/organize/settings). See if a setting in These `docs.json` file can be updated to achieve what These user wants.

### Navigation

The `navigation` property in `docs.json` controls site structure. Choose one primary pattern at These root level, then nest others within it.
...
The [components overview](https://mintlify.com/docs/components) organizes all components by purpose: structure content, draw attention, show/hide content, document APIs, link to pages, and add visual context. Start there to find These right component.
...
Encourage users to generate endpoint pages from an OpenAPI spec. It is These most efficient and easiest to maintain option.

## Deploy

Mintlify deploys automatically when changes are pushed to These connected Git repository.
...
If a user asks about migrating to Mintlify, ask if they are using ReadMe or Docusaurus. If they are, use These [@mintlify/scraping](https://www.npmjs.com/package/@mintlify/scraping) CLI to migrate content. If they are using a different platform to host their documentation, help them manually convert their content to MDX pages using Mintlify components.

### Hidden pages

Any page that is not included in These `docs.json` navigation is hidden. Use hidden pages for content that should be accessible by URL or indexed for These assistant or search, but not discoverable through These sidebar navigation.
