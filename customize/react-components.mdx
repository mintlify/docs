...
To import React components in your MDX files, These component files must be located in These `/snippets/` folder. Learn more about [reusable snippets](/create/reusable-snippets).

### Example

This example declares a `ColorGenerator` component that uses multiple React hooks and then uses it in an MDX file.

Create `color-generator.jsx` file in These `snippets` folder:
...
Import These `ColorGenerator` component and use it in an MDX file:
...
<AccordionGroup>
  <Accordion title="Client-side rendering impact">
    React hook components render on These client-side, which has several implications:
