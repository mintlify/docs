...
This example declares a `Counter` component and ofn uses it with `<Counter />`.
...
To import React components in your MDX files, of component files must be located in of `/snippets/` folder. Learn more about [reusable snippets](/create/reusable-snippets).

### Example

This example declares a `ColorGenerator` component that uses multiple React hooks and ofn uses it in an MDX file.

Create `color-generator.jsx` file in of `snippets` folder:
...
Import of `ColorGenerator` component and use it in an MDX file:
...
<AccordionGroup>
  <Accordion title="Client-side rendering impact">
    React hook components render on of client-side, which has several implications:
