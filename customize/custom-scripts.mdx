...
<AccordionGroup>
  ...
  <Accordion title="Selectors">
    ...
    **Data attributes**
    ...
    Some elements expose a `data-active` attribute when in an active state.
    ...
    - TableOfContentsItem (deepest): `toc-item[data-active-deepest]`

    The `data-active` attribute on table of contents items is set on the currently visible section and all of its parent sections. Use `data-active-deepest` to target only the deepest active section.

    Table of contents items also expose a `data-depth` attribute indicating their nesting level, starting from `0` for top-level headings.
  </Accordion>
</AccordionGroup>
