...
Use `window.mintlify.api.playground.setServerVariables` to prefill [OpenAPI server variables](/api-playground/openapi-setup) from custom JavaScript. Use it when values become available after page load, such as after an authentication SDK initializes or a tenant changes. It updates open API Playgrounds and applies to future API Playgrounds.
...
Call `window.mintlify.api.playground.clearServerVariables()` when the values no longer apply, such as after logout. After clearing, the API Playground falls back to its other configured values.
