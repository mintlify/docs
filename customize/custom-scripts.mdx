...
Load third-party scripts from external URLs using the `customScripts` option in `docs.json`. This is useful for adding analytics, chat widgets, or other services that provide a hosted script.

Each entry requires a `src` URL (must be HTTPS) and accepts an optional `strategy` to control when the script loads:
...
<Note>
  You can add up to 20 custom scripts. The `src` URL must use HTTPS and must not point to disallowed file types such as `.svg`, `.exe`, or `.zip`.
</Note>