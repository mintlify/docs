...
The `<html>` element exposes a `data-current-path` attribute that reflects the normalized path of the current page. The attribute is set before the first paint on initial load and updated during client-side navigation, so you can read it from custom scripts without waiting for route changes to settle.

The value is normalized to provide a stable identifier for the page:

- Any configured base path is stripped.
- Query strings and hash fragments are removed.
- Trailing `/index` segments are collapsed to the parent path.
- The root page is always reported as `/`.

Use this attribute when you need a consistent page identifier for analytics, feature flags, or other custom scripts.

```js
// Read the current page path
const path = document.documentElement.dataset.currentPath;

// React to client-side navigation
const observer = new MutationObserver(() => {
  const nextPath = document.documentElement.dataset.currentPath;
  // Send a page view, toggle a widget, etc.
