...
<AccordionGroup>
  ...
  <Accordion title="Selectors">
    ...
    **Data attributes**
    ...
    Use `data-active-deepest` to target the most specific active section in the table of contents. When you scroll to an `h3` inside an `h2`, both TOC items have `data-active`, but only the `h3` item has `data-active-deepest`.
