...
Load external scripts from a URL on every page of your documentation using the `integrations.customScripts` configuration in `docs.json`. This is useful for adding third-party analytics or tracking scripts that are hosted on a CDN.

<Note>
  External scripts require the custom scripts entitlement to be enabled for your project. Contact support if you need access.
</Note>

### Configuration

Add the `customScripts` array to `integrations` in your `docs.json`:
...
<ResponseField name="src" type="string" required>
  The HTTPS URL of the script to load. Must point to a valid JavaScript file.
</ResponseField>

<ResponseField name="strategy" type="string" default="afterInteractive">
  Controls when the script loads. Accepts `afterInteractive` (loads after the page becomes interactive) or `lazyOnload` (loads during idle time).
</ResponseField>

You can add up to 20 external scripts.

<Warning>
  Only load scripts from trusted sources. External scripts have full access to the page and can affect performance and security.
</Warning>