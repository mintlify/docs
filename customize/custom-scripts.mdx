...
<AccordionGroup>
  ...
  <Accordion title="Data attributes">
    ...
    The `<html>` element exposes a `data-current-path` attribute containing the normalized path of the current page. The value has any base path prefix, query parameters, and hash fragments removed, always starts with `/`, and has no trailing slash. The root and `/index` paths are both represented as `/`. The attribute is set on initial page load and updated on client-side route changes, making it useful for scoping CSS rules or custom scripts to specific pages.
