...
You can load external scripts (such as third-party analytics libraries) on every page of your documentation site from the Mintlify dashboard. External scripts are loaded via their URL rather than inline code, giving you a way to add analytics or tracking scripts without committing files to your repository.

To add an external script:

<Steps>
  <Step title="Open the dashboard">
    Go to your project settings in the [Mintlify dashboard](https://dashboard.mintlify.com) and navigate to **Settings > Add-ons**.
  </Step>
  <Step title="Add a script URL">
    Under **Custom scripts**, click **Add script** and enter the HTTPS URL for the script you want to load. For example:

    ```
    https://cdn.example.com/analytics.js
    ```
  </Step>
  <Step title="Choose a loading strategy">
    Select when the script should load:

    - **After Interactive** (default) — loads after the page becomes interactive. Best for most analytics scripts.
    - **Before Interactive (head)** — loads in the `<head>` before the page is interactive. Use this for scripts that must run before any page content renders.
    - **Lazy Onload** — loads during idle time after all other resources. Use this for low-priority scripts that don't need to run immediately.
  </Step>
  <Step title="Save and deploy">
    Click **Save and deploy** to apply your changes. A redeployment is triggered automatically.
  </Step>
</Steps>

<Note>
  Only HTTPS URLs are allowed. Inline script code is not supported — use [custom JavaScript files](#adding-custom-javascript-files) for inline code. You can add up to 20 external scripts per project.
</Note>