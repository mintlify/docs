...
description: "Add custom JavaScript and CSS to your documentation site for analytics, widgets, styling, third-party integrations, and API Playground server variables."
...
For example, you can add the following `ga.js` file to enable [Google Analytics](https://marketingplatform.google.com/about/analytics) across your entire site.
...
Use `window.mintlify.api.playground.setServerVariables` to prefill [OpenAPI server variables](/api-playground/openapi-setup) from custom JavaScript. Use it when values become available after page load, such as after an authentication SDK initializes or a tenant changes. The method updates any open API Playground and applies to future API Playgrounds.

Pass an object with string values. Each call replaces the full runtime override. Omitted keys are removed, and invalid values are ignored. Runtime values take precedence over OpenAPI defaults and saved server-variable values.
...
Call `window.mintlify.api.playground.clearServerVariables()` when the values no longer apply, such as after logout. After clearing, the playground falls back to its other configured values.

```js Clear API Playground server variables
window.mintlify.api.playground.clearServerVariables();
```

Calls made before the client initializes are queued and applied when it initializes. The override stays in memory for the current page session. It does not write to `localStorage` or credential storage. A full page refresh removes it.

<Warning>
  Set only non-secret values from client-side code. Do not include API keys, tokens, or other credentials in server variables.
</Warning>
