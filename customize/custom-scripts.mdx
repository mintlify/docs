...
### Adding inline scripts with JS files
...
Use the `integrations.customScripts` field in your `docs.json` to load external JavaScript files by URL. This is useful for third-party scripts that are not covered by a built-in [integration](/organize/settings-integrations).
...
<ResponseField name="src" type="string" required>
  An HTTPS URL pointing to the script. URLs ending in `.svg`, `.exe`, or `.zip` are not allowed.
</ResponseField>

<ResponseField name="strategy" type="string">
  Controls when the script loads. Accepts `afterInteractive` (default browser behavior) or `lazyOnload` (deferred until the browser is idle). If omitted, the script loads after the page becomes interactive.
</ResponseField>

You can include up to 20 custom scripts.
