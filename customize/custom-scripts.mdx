...
<AccordionGroup>
  ...
  <Accordion title="Selectors">
    ...
    **Data attributes**
    ...
    - TabsBarItem: `.nav-tabs-item[data-active]` - TabsBarItem renders as a standard `<a>` element (unlike most other components, which use custom element names). This selector only applies to simple tabs. Tabs with dropdown menus do not receive `data-active`.
    ...
    - MermaidContainer: `[data-component-name="mermaid-container"]` — the outer wrapper for a Mermaid diagram, including its zoom controls
    - MermaidControlsWrapper: `[data-component-name="mermaid-controls-wrapper"]` — the zoom and pan controls overlay on a Mermaid diagram
    - PrimaryHeaderButton: `[data-component-name="primary-header-button"]` — Sequoia theme only
    - ThemeToggle: `[data-component-name="theme-toggle"]`
