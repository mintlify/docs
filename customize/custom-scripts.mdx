...
Because custom scripts run before user info resolves, listen for the `mintlify:user` event to react once the user object is available. The event fires when user info resolves and again on any change. Its `detail` is the user object, or `null` when the visitor is signed out.
...
<Warning>
  Anything you place in the user `content` field is exposed to client-side scripts. Do not include secrets or credentials that shouldn't be readable in the browser.
</Warning>