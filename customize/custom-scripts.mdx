...
Load third-party JavaScript from external URLs using the `customScripts` field in `docs.json`. This is useful when you need to include a hosted script (such as a widget, chatbot, or tracking pixel) without adding the source code to your repository.

Each script must use an HTTPS URL. You can add up to 20 external scripts.

### Configuration

Add the `customScripts` array to the `integrations` object in your `docs.json` file:
...
<ResponseField name="src" type="string" required>
  The HTTPS URL of the script to load. URLs ending in `.svg`, `.exe`, or `.zip` are not allowed.
</ResponseField>
...
<ResponseField name="strategy" type="string">
  ...
  - `lazyOnload` - defers loading until the browser is idle
</ResponseField>

### Example

The following configuration loads a third-party chat widget after the page is interactive and defers a secondary analytics script until the browser is idle:
...
"integrations": {
  "customScripts": [
    ...
    {
      "src": "https://cdn.example.com/secondary-analytics.js",
      "strategy": "lazyOnload"
    }
  ]
}
