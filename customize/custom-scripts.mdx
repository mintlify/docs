...
Custom CSS applies to every page of your site, including custom mode and landing pages. To scope styles to a specific page or section, use the `html[data-current-path="..."]` attribute selector described in [Data attributes](#data-attributes).
...
Mintlify includes any `.js` file inside your content directory on every page of your documentation site, including custom mode and landing pages. Custom JavaScript files run after the page becomes interactive. You cannot scope them to specific pages, and when multiple `.js` files are present they all run without a guaranteed order.
