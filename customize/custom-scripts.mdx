...
You can load external scripts on every page using the `integrations.customScripts` field in your `docs.json`. Each entry requires an HTTPS `src` URL and accepts an optional `strategy` to control when the script loads.

| Strategy | Description |
|---|---|
| `beforeInteractive` | Injects the script into the `<head>` before the page becomes interactive. Use this for scripts that must run before any page content loads. |
| `afterInteractive` | Loads the script after the page becomes interactive. This is the default behavior. |
...
You can add up to 20 external scripts. The `src` must be an HTTPS URL and cannot point to `.svg`, `.exe`, or `.zip` files.