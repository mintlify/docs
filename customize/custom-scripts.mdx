...
You can also load external scripts by URL using the `integrations.customScripts` array in your `docs.json`. Each entry requires an HTTPS `src` URL and accepts an optional `strategy` to control when the script loads.
...
<ParamField path="src" type="string" required>
  HTTPS URL of the script to load. Must not point to a disallowed file type (`.svg`, `.exe`, `.zip`).
</ParamField>

<ParamField path="strategy" type="string">
  Controls when the script loads. One of:

  - `beforeInteractive` — Loads the script before the page becomes interactive. Use this for scripts that must run as early as possible, such as consent managers or critical polyfills.
  - `afterInteractive` — Loads the script after the page becomes interactive. This is the default behavior. Use this for analytics or tracking scripts.
  - `lazyOnload` — Defers loading until the browser is idle. Use this for low-priority scripts.
</ParamField>
