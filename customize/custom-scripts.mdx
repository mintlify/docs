...
## Load external scripts from URLs

Use `integrations.customScripts` in your `docs.json` to load third-party JavaScript from external URLs on every page. This is useful for loading analytics, chat widgets, or other hosted scripts without adding files to your repository.
...
Each script requires an HTTPS URL. You can optionally set a `strategy` to control when the script loads:

| Strategy | When it loads | Use case |
|---|---|---|
| `beforeInteractive` | Before the page becomes interactive | Critical scripts that must run immediately |
| `afterInteractive` | After the page becomes interactive (default) | Analytics and tracking scripts |
| `lazyOnload` | During browser idle time | Low-priority scripts |

You can add up to 20 external scripts. URLs pointing to `.svg`, `.exe`, or `.zip` files are not allowed.
