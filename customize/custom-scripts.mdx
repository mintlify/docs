...
Add external script URLs from the dashboard to load third-party analytics or tracking scripts on every page. This is useful when you need to load a hosted script by URL rather than writing inline JavaScript in your repository.

1. Navigate to the [Add-ons](https://dashboard.mintlify.com/products/addons) page in your dashboard.
2. Under **Custom scripts**, click **Add script**.
3. Enter an HTTPS URL pointing to the script (for example, `https://cdn.example.com/analytics.js`).
4. Choose a loading strategy:
   - **After Interactive** (default) — loads after the page becomes interactive.
   - **Before Interactive (head)** — loads in the `<head>` before the page is interactive. Use for scripts that must run before any user interaction.
   - **Lazy Onload** — loads during idle time after the page has fully loaded. Use for low-priority scripts.
5. Click **Save and deploy**.

You can add up to 20 external scripts. Only HTTPS URLs are accepted and the URL must point to a JavaScript file.
