...
Load external scripts by URL using the `integrations.customScripts` array in your `docs.json`. Each entry requires an HTTPS `src` URL and accepts an optional `strategy` to control when the script loads.
...
<ResponseField name="strategy" type="string">
  ...
  - `beforeInteractive` — loads the script before the page becomes interactive. Use for scripts that must run as early as possible, such as consent managers or critical polyfills.
  - `afterInteractive` — loads the script after the page becomes interactive. This is the default. Use for analytics or tracking scripts.
  - `lazyOnload` — defers loading until the browser is idle. Use for low-priority scripts.
</ResponseField>
