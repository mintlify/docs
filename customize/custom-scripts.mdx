...
You can load external JavaScript from a URL by adding `customScripts` to the `integrations` object in your `docs.json` file. Each entry requires an HTTPS `src` URL and accepts an optional `strategy` to control when the script loads.
...
<ResponseField name="strategy" type="string">
  ...
  - `afterInteractive` — loads the script after the page becomes interactive. This is the default.
  - `lazyOnload` — loads the script during idle time after all page content has loaded.
</ResponseField>
