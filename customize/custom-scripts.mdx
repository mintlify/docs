...
Add external analytics or tracking scripts to your docs site from the Mintlify dashboard, without adding files to your repository. Each script is loaded on every page by its URL.

### Adding an external script

<Steps>
  <Step title="Open add-ons settings">
    In the dashboard, go to **Settings > Deployment > Add-ons** and find the **Custom scripts** section.
  </Step>
  <Step title="Add a script URL">
    Click **Add script**, enter an HTTPS URL, and choose a loading strategy:

    | Strategy | Description |
    | --- | --- |
    | **After Interactive** (default) | Loads after the page becomes interactive. Best for most analytics scripts. |
    | **Before Interactive** | Loads in the `<head>` before the page is interactive. Use for scripts that must run early, such as tag managers. |
    | **Lazy Onload** | Loads during browser idle time. Use for low-priority scripts like surveys or chat widgets. |
  </Step>
  <Step title="Save and deploy">
    Click **Save and deploy** to apply your changes. The scripts take effect on the next deployment.
  </Step>
</Steps>
...
- Inline code is not supported — each entry must be a script URL.
- You can add up to 20 external scripts.
- File types such as `.svg`, `.exe`, and `.zip` are not allowed.

<Note>
  External scripts are managed through the dashboard and do not appear in your `docs.json` configuration. For inline JavaScript, use [custom JavaScript files](#custom-javascript) instead.
</Note>