...
Mintlify automatically includes any `.css` file inside your content directory on every page of your documentation site, the same way it includes custom `.js` files. You do not need to import or reference the file from `docs.json` or your MDX files.

To add custom styles, create a `.css` file (for example, `style.css`) at any level of your content directory. Any class names, ID selectors, or element selectors you define become available across all of your MDX files.
...
Then use it in any MDX file with the `className` prop:
