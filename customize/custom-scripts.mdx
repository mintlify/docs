...
You can also load external JavaScript files hosted on a CDN or third-party service using the `customScripts` field in `docs.json`. Each script must use an HTTPS URL.
...
<ResponseField name="strategy" type="string">
  Controls when the script loads. Options are `afterInteractive` (default) and `lazyOnload`.
  - `afterInteractive` — loads the script after the page becomes interactive.
  - `lazyOnload` — defers loading until the browser is idle.
</ResponseField>

You can include up to 20 custom scripts. See the [integrations settings reference](/organize/settings-integrations#custom-scripts) for the full property definition.
