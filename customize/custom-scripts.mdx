...
You can add JavaScript to your documentation in two ways: local `.js` files in your content directory, or external scripts loaded via `docs.json`.

### Local JavaScript files

Mintlify includes any `.js` file inside your content directory on every page of your documentation site. This is the equivalent of adding a `<script>` tag with inline code to every page.

For example, you can add the following `ga.js` file to enable [Google Analytics](https://marketingplatform.google.com/about/analytics) across the entire documentation.
...
Use the `customScripts` field in your `docs.json` integrations to load external JavaScript files by URL. This is useful for third-party scripts that aren't covered by a [built-in integration](/integrations/analytics/overview).
...
Each script object accepts the following properties:

<ParamField path="src" type="string" required>
  The HTTPS URL of the script to load. URLs pointing to `.svg`, `.exe`, or `.zip` files are not allowed.
</ParamField>

<ParamField path="strategy" type="string" default="afterInteractive">
  Controls when the script loads. Options are:
  - `afterInteractive` — loads after the page becomes interactive. Best for most scripts.
  - `lazyOnload` — defers loading until browser idle time. Best for low-priority scripts.
</ParamField>

You can add up to 20 external scripts. For the full integrations reference, see [integrations settings](/organize/settings-integrations).