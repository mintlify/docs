...
After you add a verification `TXT` record, domain validation can enter a backoff period and retries can take up to several hours. If you have already added the correct DNS records and your domain is still pending validation, retry validation manually from your dashboard to bypass the wait.

1. Navigate to the [Custom domain setup](https://dashboard.mintlify.com/settings/deployment/custom-domain) page in your dashboard.
2. Find your pending custom domain.
3. Select **Retry validation**.

Use this only after you have confirmed your DNS records are correct. Repeated retries with incorrect records do not speed up validation.
