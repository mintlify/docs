...
<Note>
  Your default `<subdomain>.mintlify.app` address stays reachable after you add a custom domain, and search engines can still index it. If the default address appears in search results, see [Default mintlify.app subdomain indexed with custom domain](/help-center/default-subdomain-indexed-alongside-custom-domain) for information on removing the default domain from search indexing.
</Note>
