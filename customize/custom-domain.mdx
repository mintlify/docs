...
## Subdomain slug and project name limitations

Before adding a custom domain, understand how the underlying Mintlify subdomain and your project name behave:

- **Subdomain slugs are permanent.** The slug in your `*.mintlify.app` dashboard URL and your `*.mintlify.site` deployment URL is set when the organization is created and cannot be changed. Changing a slug requires deleting the organization and creating a new one, which permanently deletes all associated content, settings, deployments, and history. There is no migration path.
- **Renaming a project only changes the display name.** Updating the project name in your dashboard changes the label shown in the UI. It does not change your `*.mintlify.site` deployment URL, your canonical host, or the origin host used for `Disallowed origin host` checks. If a rename triggers a `Disallowed origin host` error, the deployment is still tied to the original subdomain — use that host, not the new display name.
- **The default `*.mintlify.site` and `*.mintlify.app` hosts always keep serving.** Adding a custom domain does not disable the underlying Mintlify hosts, and they cannot be turned off today. To direct users and search engines to your custom domain, [set a canonical URL](#set-a-canonical-url) and use redirects at your own edge if you need to hard-redirect traffic.

<Warning>
  Deleting an organization to change its subdomain is destructive and irreversible. Custom domains, DNS verification, TLS certificates, API keys, members, and analytics do not transfer to a new organization — you must reconfigure everything from scratch. Confirm the tradeoff with your team before proceeding in production.
</Warning>
