...
<Info>
  Looking to set up a custom subpath like `mintlify.com/docs`? 
  
  - **For `/docs` subpath**: Use the self-service toggle in your [dashboard](https://dashboard.mintlify.com/settings/deployment/custom-domain). See the [Vercel guide](/deploy/vercel#host-from-docs) for the required rewrites configuration.
  - **For other subpaths** (e.g., `/help`, `/guides`): Follow the full [custom subpath](/deploy/cloudflare) setup guide.
</Info>
