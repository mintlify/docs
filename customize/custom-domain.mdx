...
A custom domain connects to exactly one deployment. To document multiple products on one domain, combine them into a single site with path-based navigation or give each product its own subdomain. See [Can I host multiple docs sites on the same domain?](/help-center/host-multiple-docs-sites-on-one-domain).
...
If you proxy only your subpath to Mintlify, follow the reverse proxy guide for your provider instead of adding the `CNAME` record. See [Host docs at a subpath](/deploy/docs-subpath) for more information. The verification `TXT` records shown in your dashboard stay unverified in this setup. This is expected. See [_cf-custom-hostname TXT record stuck unverified](/help-center/cf-custom-hostname-txt-record-stuck-unverified).
