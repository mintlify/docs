---
title: "Custom domain"
description: "Host your documentation at your own domain, subdomain, or subpath with DNS configuration and automatic TLS certificates."
keywords: ["custom domain setup", "DNS configuration", "CNAME records", "TLS certificates", "subpath hosting", "base path"]
---
...
The [Custom domain setup](https://app.mintlify.com/settings/deployment/custom-domain) page has two hosting modes:

- **Domain or subdomain**: Host your documentation at a domain like `example.com` or a subdomain like `docs.example.com`. Mintlify serves all traffic for the domain.
- **Subpath**: Host your documentation at a path on your domain, like `example.com/docs`.

When you host at a subpath, choose how traffic reaches Mintlify:

- **Send all domain traffic to Mintlify**: Your domain's DNS points to Mintlify and Mintlify serves your documentation at your subpath. Only use this option if Mintlify can receive all paths on the domain, meaning your domain hosts nothing else.
- **Proxy only your subpath**: Your app or CDN keeps receiving traffic for the domain and proxies only your subpath to Mintlify. Use this option when another site runs on your domain. See [/docs subpath](/deploy/docs-subpath) for proxy setup guides.
...
2. Select your hosting mode: **Domain or subdomain** or **Subpath**.
3. If you selected **Subpath**, enter your base path, like `/docs`, and choose a traffic routing option.
4. Enter your domain name. For example, `docs.example.com` or `example.com`.
5. Click **Add domain**.
...
If your domain traffic routes to Mintlify, the dashboard displays the DNS records to add at your domain provider. If you proxy only your subpath to Mintlify, the dashboard displays reverse proxy setup guides instead. Follow the guide for your provider and see [/docs subpath](/deploy/docs-subpath) for more information.
...
When you change your base path or switch between hosting modes, Mintlify rebuilds and redeploys your documentation. Your documentation stays live at the current path until the deployment finishes, then traffic switches to the new path. If the deployment fails, your documentation continues serving from the current path and you can retry the change from your dashboard.
...
`TXT` records are typically validated within 5 minutes. If your domain is still pending validation after adding the verification `TXT` records, you can retry validation manually from your dashboard.
