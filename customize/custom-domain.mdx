...
### Default subdomain remains active

Your deployment's default `<subdomain>.mintlify.site` URL cannot be disabled or redirected today, even after a custom domain is live. Both URLs serve the same content. Set a [canonical URL](#set-a-canonical-url) so search engines index the custom domain as the primary one.

### Renaming a project

Renaming a project in your dashboard changes only the display name. It does not change the deployment URL or the default `<subdomain>.mintlify.site` hostname.

If you rename a project and then see a `Disallowed origin host` error at your default URL, the deployment's hostname no longer matches what your dashboard expects. To change the URL, delete the deployment and recreate it with the desired name. There is no in-place rename for the deployment URL.

### Releasing a domain

A custom domain can only be attached to one Mintlify deployment at a time. If your domain is still attached to a previous deployment or organization (for example, after moving to a new workspace), the new deployment cannot claim it until the old one releases it.

To release a domain you control:

1. Contact [support](mailto:support@mintlify.com) and request release of the domain.
2. Support provides a verification `TXT` record with a unique value.
3. Add the record at your DNS provider:

   ```text
   TXT | <your-domain> | <value provided by support>
   ```

4. Reply to support once the record is live. Support verifies the record and detaches the domain from the previous deployment. You can then add it to your new deployment following [Add your custom domain](#add-your-custom-domain).
