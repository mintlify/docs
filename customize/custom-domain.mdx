...
There are three ways to host your documentation on a custom domain:

**Domain**: Host your documentation at a domain like `example.com`. Mintlify serves all traffic for the domain.

**Subdomain**: Host your documentation at a subdomain like `docs.example.com`. Mintlify serves all traffic for the subdomain.

**Subpath**: Host your documentation at a path on your domain, like `example.com/docs`. Choose how traffic reaches Mintlify:
- **Send all domain traffic to Mintlify**: Your domain's DNS points to Mintlify and Mintlify serves your documentation at your subpath. Only use this option if your domain hosts no other content and Mintlify can receive all paths on the domain.
- **Proxy only your subpath**: Your app or CDN keeps receiving traffic for the domain and proxies only your subpath to Mintlify. Use this option when another site runs on your domain.
...
3. If you select **Subpath**, enter your base path, like `/docs`, and choose a traffic routing option.
...
If you proxy only your subpath to Mintlify, the dashboard displays reverse proxy setup guides instead. Follow the guide for your provider and see [/docs subpath](/deploy/docs-subpath) for more information.

### Base path requirements

<Note>
  Base paths other than `/docs` require a [Pro or Enterprise plan](https://mintlify.com/pricing?ref=custom-domain).
</Note>
...
When you change your base path or switch between hosting modes, Mintlify rebuilds and redeploys your site. Your site stays live at the current path until the deployment finishes, then traffic switches to the new path. If the deployment fails, your site continues serving from the current path and you can retry the change from your dashboard.
