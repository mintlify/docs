...
<Warning>
**Important**: The order of behaviors matters in CloudFront. CloudFront evaluates behaviors from most specific to least specific. Make sure to create behaviors in the order listed below, with static assets behaviors listed above the default behavior.
</Warning>

### `/mintlify-assets/_next/static/*`

To prevent layout shifts and ensure stable navigation, create a behavior for static assets with a **Path pattern** of `/mintlify-assets/_next/static/*`.

- Set **Origin and origin groups** to your `.mintlify.dev` URL
- Set **Cache policy** to **CachingOptimized** 
- Set **Origin request policy** to **AllViewerExceptHostHeader**

This ensures static assets (CSS, JavaScript, fonts) are properly cached and don't cause layout shifts during navigation.
...
Lastly, we're going to edit the `Default (*)` behavior to handle HTML and app router navigation properly.
...
2. Set **Cache policy** to **CachingDisabled** (since RSC `?_rsc=` payloads vary and shouldn't be cached).
...
<Info>
The CachingDisabled policy for the default behavior ensures that dynamic content and React Server Components payloads are not cached, preventing stale content issues during navigation.
</Info>

### Check behaviors are set up correctly

If you follow the above steps, your behaviors should look like this (ordered from most specific to least specific):

1. `/mintlify-assets/_next/static/*` - Static assets caching
2. `/.well-known/*` - Vercel domain verification  
3. `/docs/*` - Documentation pages
4. `/docs` - Documentation root
5. `Default (*)` - Landing page fallback

<Frame>
  ![CloudFront "Behaviors" page with 5 behaviors: `/mintlify-assets/_next/static/*`, `/.well-known/*`, `/docs/*`, `/docs`, and `Default`.](/images/cloudfront/all-behaviors.png)
</Frame>

<Warning>
Make sure the `/mintlify-assets/_next/static/*` behavior is listed first (most specific) to ensure static assets are properly cached and prevent layout shifts during navigation.
</Warning>
