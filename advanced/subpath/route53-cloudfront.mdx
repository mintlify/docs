...
When using CloudFront as a reverse proxy, you may experience layout shifts where the sidebar snaps up and down during navigation. This occurs because CloudFront may not properly cache static assets, causing layout resources to reload on each page change.

To fix this issue, configure specific behaviors for static assets and navigation:

### Required behaviors for stable layouts

Configure these behaviors in your CloudFront distribution (ordered from most specific to least specific):

1. **Static assets behavior** - `/mintlify-assets/_next/static/*`
   - **Path pattern**: `/mintlify-assets/_next/static/*`
   - **Cache policy**: CachingOptimized
   - **Origin request policy**: AllViewerExceptHostHeader

2. **Default behavior** - `*` (handles HTML and app router navigation)
...
With these changes, CloudFront will only fetch new page content while keeping layout assets cached, preventing layout shifts during navigation.
