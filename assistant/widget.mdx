---
title: "Embed and configure the Mintlify AI assistant widget"
sidebarTitle: "Widget"
...
For example, use the same generated nonce on both page scripts and pass it to `init()`:
...
Your policy must allow `https://widget.mintlify.com` in `script-src`, the generated nonce in `style-src`, and the required widget and API origins in `connect-src`.
