...
The only required browser option is the public widget ID. Manage the enabled state, allowed origins, attachments, and bot protection in your dashboard. Set embed-specific starter questions, a support email, retrieval filters, and analytics preferences in the browser configuration.
...
Use `update()` to change appearance, labels, support email, starter questions, retrieval filters, analytics, or hooks without clearing the current conversation. Only the supplied fields change.
...
await window.MintlifyAssistant.update({
  ...
  // Restores analytics to its default of true.
  analytics: null,
  hooks: null,
});
```
...
| `analytics`        | boolean                                         | Enables privacy-safe Mintlify analytics. The default is `true`.                |
...
| `analytics`        | boolean or `null`                                       | Stops or resumes collection. Pass `null` to restore `true`.       |
...
Analytics events are internal to Mintlify and do not change [`hooks.event`](#assistanthooks). Changing `analytics` with `update()` does not remount the widget, reset its conversation, or affect customer hooks.

If your privacy or consent policy requires opt-in collection, initialize with analytics disabled, then enable it after consent:
...
// After the visitor provides any consent your policy requires:
await window.MintlifyAssistant.update({ analytics: true });
```

Your site remains responsible for its privacy notice and consent requirements. Setting `analytics: false` before `init()` prevents the analytics client from loading.
...
| `connect-src`                                | `https://ph.mintlify.com`           | Analytics, when enabled     |
...
Setting `analytics: false` removes the `https://ph.mintlify.com` requirement. `https://api.mintlify.com` remains required when you use the hosted endpoint because the browser sends configuration, message, and feedback requests to it. If you set a custom `endpoint`, allow that origin instead.
