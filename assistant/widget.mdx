...
## Configure analytics privacy

Mintlify captures internal usage analytics for the widget. By default, each event includes the embedding page's current pathname so you can see which pages drive assistant usage. The pathname is capped at 500 characters and sent without its query string or fragment. Query parameters, fragments, full URLs, and referrers are never collected.

If your site places tokens, personal data, or other private values in path segments, disable pathname capture at initialization:
...
Changing `analytics.capturePathname` at runtime requires `destroy()` followed by a new `init()`.
