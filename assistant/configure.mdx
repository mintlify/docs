...
Assistant credit usage is split into two categories on the [Usage](https://app.mintlify.com/settings/organization/usage) page:

- **Assistant**: Messages from the in-product web assistant embedded on your documentation site.
- **Assistant API**: Messages from the [assistant API](/api/assistant/create-assistant-message-v2) and discovery chat surfaces.

Both categories draw from the same credit allowance. Use the product filter on the usage chart to view consumption for each surface independently.
