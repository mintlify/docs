...
keywords: ["assistant", "settings", "deflection", "bot protection", "billing", "BYOK", "bring your own model"]
---
...
<Info>
  Bringing your own model is an add-on for [Enterprise plans](https://mintlify.com/pricing?ref=byok). The **AI model** section appears in your dashboard only when the add-on is included in your contract. [Contact sales](mailto:sales@mintlify.com) to add it.
</Info>

By default, Mintlify selects and manages the model that powers the assistant. Bring your own model to run Mintlify AI products on a model and API key that you control.

To configure your own model:

1. Go to the [API keys](https://app.mintlify.com/settings/organization/api-keys) page of your dashboard.
2. In the **AI model** section, toggle **Bring your own model** on.
3. Select a provider: **OpenRouter**, **Anthropic**, or **OpenAI-compatible**. OpenRouter is selected by default and works with all Mintlify AI products. Anthropic and OpenAI-compatible providers have limited compatibility: not all AI products work with them.
4. If you select **OpenAI-compatible**, enter a provider name and the provider's base URL. The base URL must use HTTPS and point to a publicly reachable hostname.
5. Select a model from the list for your provider.
6. Enter your provider API key and select **Save**.

Mintlify stores your API key securely and never displays it after saving. When you change models within the same provider, Mintlify reuses your saved key. When you change the provider or base URL, enter your key again. To rotate a key, enter a new key and save.

Toggle **Bring your own model** off to delete your configuration and return to Mintlify-managed models.
