...
View and change your current tier on the [Usage](https://dashboard.mintlify.com/settings/organization/usage) page of your dashboard.
...
If you want to disallow overages, disable them in the **Billing Controls** section of the [Usage](https://dashboard.mintlify.com/settings/organization/usage) page of your dashboard.
...
### View usage breakdown by source

To see how your credits are being used across different products, toggle **Breakdown by source** on the **Usage & Credits Overview** card on the [Usage](https://dashboard.mintlify.com/settings/organization/usage) page.

When enabled, the usage bar is replaced with a stacked bar that shows how many credits each source has used in the current billing period, along with a legend listing the credit count and percentage for each source.

Credits are grouped into the following sources:

- **Assistant**: Credits used by the in-product assistant chat and discovery search.
- **Workflows**: Credits used by [workflows](/workflows).
- **Slack agent**: Credits used by the [Slack agent](/agent/slack).
- **Linear agent**: Credits used by the Linear agent.
- **Other**: Any usage that does not fall into the categories above.

Toggle **Breakdown by source** off to return to the standard period-versus-quota usage bar.
