...
The [Usage](https://dashboard.mintlify.com/settings/organization/usage) page breaks down credit consumption by source so you can see where your credits are spent across a date range.

Credits are grouped into the following categories:

- **Assistant**: Credits spent on assistant interactions from your docs site, the API, and the Slack assistant.
- **Agent**: Credits spent on background agents, including the Slack agent, Discord, Telegram, and Linear agents.
- **Workflows**: Credits spent on automated workflows.

Below the breakdown, an event log shows individual credit-consuming events with their timestamp, source, and credit cost. Use the event log to investigate spikes or unexpected charges.
