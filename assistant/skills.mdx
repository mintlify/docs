...
The assistant uses the `name` and `description` to decide when to load a skill, so write descriptions that state when the skill applies. Keep the instructions concise. The assistant truncates skill content at approximately 6,000 tokens. Content beyond that limit is dropped from the end of the file, so put the most important guidance first and split long procedures into separate skills.

<Note>
  After you add or change a `skill.md` file, redeploy your site to make the skill available to the assistant. To verify a skill loads as expected, ask the assistant a question that matches the skill's `description`. If the wrong skill loads, tighten the `description` to be more specific about when the skill applies.
</Note>
...
Skills are on by default when at least one `skill.md` file is present. To disable a single skill, delete its directory under `.mintlify/assistant/skills/` and redeploy your site. To remove all skills, delete the `.mintlify/assistant/skills/` directory and redeploy your site. To turn off the skills feature for your deployment, contact [support](mailto:support@mintlify.com).
