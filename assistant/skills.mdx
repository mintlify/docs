...
The assistant uses `name` and `description` to decide whether to load the skill, so write descriptions that clearly state when the guide applies. Keep the body focused: the assistant truncates skill content at roughly 6,000 tokens when loading it.

<Note>
  Mintlify discovers skills on your next documentation update. After you add or change a `skill.md` file, publish your docs to make the skill available to the assistant.
</Note>
...
- The assistant treats loaded skill content as team-authored guidance that takes precedence over general search results.
- On follow-up turns, the assistant removes previously loaded skill content from the conversation history to keep the context small; the assistant reloads a skill if it needs it again.
...
- Use a unique, URL-safe slug for each directory name. Slugs must contain only lowercase letters, numbers, and hyphens. Publishing skips duplicate slugs with a warning.
...
Skills are on by default when at least one `skill.md` file is present. To turn skills off entirely for your deployment, contact [support](mailto:support@mintlify.com) to disable the assistant skills feature. Removing the `.mintlify/assistant/skills/` directory and republishing also clears the catalog.
