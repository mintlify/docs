...
The parameter is removed from the URL when the user closes the assistant, so the link stays clean if they share the page afterward.

The parameter only triggers for visitors who have access to the assistant. If a visitor does not have permission to use the assistant, the parameter is ignored and the page loads normally.

You can prefill the search modal in the same way with the `?search=<query>` parameter.
