...
<Info>
  The assistant is available on the [Pro plan and above](https://mintlify.com/pricing?ref=assistant). Every Pro plan contributes 10,000 credits per month to your organization's shared credit balance, used across the assistant, agent, and automations on every project. See [Manage billing](/assistant/configure#manage-billing) for more information about credit tiers and options.
</Info>
...
The assistant uses tool calling, which lets it invoke tools to answer questions. When users ask questions, the assistant:
...
* **Reads images** on your documentation pages, such as screenshots and diagrams, to answer questions about what they show. The assistant only views images that appear on pages it can access, so it cannot fetch arbitrary image URLs.
...
You can [set a deflection email](/assistant/configure#set-deflection-emails) so that the assistant provides your support email to users whose questions it cannot answer. This gives users a path forward, even if the documentation doesn't address their specific question.

## Assistant insights

Use assistant insights to understand how users interact with your documentation and identify improvement opportunities.

The [assistant page](https://app.mintlify.com/products/assistant) shows usage trends for the month to date. View more detailed insights in [assistant analytics](/analytics/assistant).
...
<Card title="Structure and organization">
  ...
  - Include comprehensive metadata in the frontmatter at the top of each page.
