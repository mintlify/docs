...
<Info>
  The assistant is enabled by default. All organizations receive 5,000 trial credits shared across the assistant, Assistant API, agent, and workflows. The in-product web assistant and the [Assistant API](/api/assistant/create-assistant-message-v2) (including discovery chat) draw from the same credit allowance but appear as separate categories on the [Usage](https://app.mintlify.com/settings/organization/usage) page. See [Manage billing](/assistant/configure#manage-billing) for more information about credit tiers and options.
</Info>
