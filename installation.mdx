...
The CLI collects anonymous usage telemetry to help improve Mintlify. Telemetry data includes the command name, CLI version, operating system, and architecture. No personally identifiable information, project content, or file paths are collected.

Telemetry is enabled by default. You can opt out at any time using the `--telemetry` flag:
...
Your preference is saved in `~/.config/mintlify/config.json` and persists across CLI sessions.
