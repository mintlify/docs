...
Create a new [agent workflow](/agent/workflows) file interactively:

```bash
mint workflow
```

The command prompts you for a workflow name, trigger type (cron or push), context repositories, auto-merge settings, and opens your default editor for the agent instructions.

The workflow file is saved to `.mintlify/workflows/` in your repository. Commit and push the file to activate the workflow.

<Tip>
  The CLI automatically detects non-interactive environments. If you're running the command in a CI/CD pipeline or with an AI coding agent, it outputs usage instructions instead of hanging on prompts.
</Tip>
