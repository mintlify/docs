...
You can use flags to check for specific accessibility issues, which is useful for CI/CD workflows:
...
The command will return an error if you attempt to use both `--skip-contrast` and `--skip-alt-text` flags simultaneously.
