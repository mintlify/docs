...
Export your documentation as a self-contained static site for air-gapped or offline deployment. The command generates a zip archive containing all pages, static assets, and platform-specific launchers for macOS and Windows.

```bash
mint export
```

By default, the output is saved as `export.zip` in the current directory. Use the `--output` flag to customize the output path:

```bash
mint export --output docs.zip
```

To view the exported site, extract the zip and run the included server:

```bash
node serve.js
```

Alternatively, double-click `Start Docs.command` on macOS or `Start Docs.bat` on Windows to launch the docs in your browser.

| Flag | Description |
| --- | --- |
| `--output` | Set the output zip file path. Defaults to `export.zip`. |
| `--groups [groupname]` | Mock user groups for the export. |
| `--disable-openapi` | Disable OpenAPI file generation during export. |

<Note>
  `mint export` requires Node.js v20.17.0 or higher and is not supported on Node.js v25+. An internet connection is required the first time you run the command or after updating the CLI.
</Note>
