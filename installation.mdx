...
Create a workflow file interactively with the following command:

```bash
mint workflow
```

The CLI prompts you for a name, trigger type, and other settings, then creates a `.md` file in `.mintlify/workflows/`. See [Workflows](/agent/workflows) for more information.
