...
This command runs a full build validation and exits with a non-zero status code if any warnings or errors are found. Use this command in CI/CD pipelines to prevent broken documentation from being deployed.

#### Flags

| Flag | Description |
| --- | --- |
| `--groups` | Mock user groups for validation when using [partial authentication](/authentication/partial-authentication). |
| `--disable-openapi` | Disable OpenAPI file generation during validation. |

#### CI/CD example

Add the following step to your CI workflow to validate documentation before deployment:
