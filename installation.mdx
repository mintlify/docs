...
This command prompts you for workflow configuration including the name, trigger type (cron or push), context repositories, automerge setting, and agent instructions. It then creates the workflow file in `.mintlify/workflows/`.

<Tip>
  The CLI automatically detects non-interactive environments. In non-interactive environments, it outputs usage instructions for creating workflow files manually instead of hanging on prompts.
</Tip>
