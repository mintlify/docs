...
The `validate` command automatically validates any OpenAPI files referenced in your `docs.json` configuration. Use flags to configure the validation command.

- `--groups [groupname]`: Mock user groups for validation (useful when testing group-based access control)
- `--disable-openapi`: Skip OpenAPI file validation and generation

<Note>
  The `openapi-check` command is deprecated. Use `mint validate` instead, which
  includes OpenAPI validation automatically.
</Note>
