...
<Note>
  You must be logged in to use the analytics commands. Run `mint login` to authenticate.
</Note>

View [analytics](/optimize/analytics) data for your documentation directly from the terminal with the `mint analytics` command. This is useful for quick checks, scripting, and CI workflows.
...
Filter stats by date range or page:

```bash
mint analytics stats --from 2025-01-01 --to 2025-01-31 --page /quickstart
```

Use `--humans` or `--agents` to show only human or AI agent traffic:
...
Display feedback items or aggregate feedback by page:

```bash
# List individual feedback items
mint analytics feedback

# Aggregate feedback by page
mint analytics feedback --type page
```

#### View assistant conversations

List and view assistant conversations:
...
View conversation category buckets to see common topics:
...
All analytics commands support a `--format` flag to control output:

| Format | Description |
| --- | --- |
| `plain` | Tab-separated columns, suitable for piping to other tools. This is the default. |
| `table` | Pretty-printed table with colors and formatting. |
| `json` | Raw JSON response from the API. |
| `graph` | Horizontal bar chart visualization. |

```bash
mint analytics stats --format table
mint analytics search --format json
```

Use `--agent` to output JSON, which is equivalent to `--format json`. The CLI also auto-detects agent environments.

#### Common options

All analytics commands support these options:

| Flag | Description |
| --- | --- |
| `--subdomain` | Target a specific deployment. Optional for single-deployment organizations. |
| `--from` | Start date in `YYYY-MM-DD` format. Defaults to 7 days ago. |
| `--to` | End date in `YYYY-MM-DD` format. Defaults to today. |

### Manage CLI configuration

Set default values for CLI options with the `mint config` command. Configuration is stored in `~/.mintlify/config.json`.

```bash
# Set a default subdomain for analytics commands
...
| `dateFrom` | Default start date for analytics commands. |
| `dateTo` | Default end date for analytics commands. |
