...
keywords: ["CLI", "npm", "local development", "Node.js", "pnpm", "mint dev", "broken links", "accessibility", "mint workflow"]
---
...
Generate a [workflow](/agent/workflows) file interactively with guided prompts:

```bash
mint workflow
```

This command walks you through creating a workflow file in `.mintlify/workflows/`. It prompts you for:

- A workflow name
- A trigger type (cron schedule or push event)
- Context repositories (optional)
- Auto-merge setting
- Agent instructions

<Tip>
  When running in non-interactive environments like AI coding agents, the command outputs usage instructions for creating workflow files directly instead of prompting.
</Tip>
