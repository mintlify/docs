...
Search is available in local preview when you are logged in to the CLI. Run `mint login` to authenticate, then start `mint dev`. Your local preview will have the same search experience as your deployed site.

If you are not logged in, the search bar displays a prompt to log in.

## Log in to the CLI

Authenticate to the CLI to enable features like search in local preview:

```bash
mint login
```

A browser window opens for authentication. After logging in, the CLI stores your credentials locally.
