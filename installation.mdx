...
The `validate` command also checks any OpenAPI specifications referenced in your `docs.json` under the `api.openapi` field. If an OpenAPI spec is invalid, the command exits with an error.
...
<Warning>
  The `openapi-check` command is deprecated. Use `mint validate` instead, which validates both your documentation build and any OpenAPI specifications referenced in your `docs.json`.
</Warning>
