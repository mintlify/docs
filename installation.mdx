...
Create workflow files interactively with the following command:

```bash
mint workflow
```

This command guides you through creating a workflow file in `.mintlify/workflows/`. It prompts for a workflow name, trigger type (cron or push), context repositories, automerge settings, and opens your default editor to write the agent instructions.

<Tip>
  The CLI automatically detects non-interactive environments. In non-interactive environments, it outputs usage instructions instead of hanging on prompts.
</Tip>

See [Workflows](/agent/workflows) for more information about workflow files and configuration options.
