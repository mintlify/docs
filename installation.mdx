...
Create a new [workflow](/agent/workflows) file interactively:

```bash
mint workflow
```

This command guides you through setting up a workflow by prompting for:
- **Workflow name**: Display name shown in the dashboard.
- **Trigger type**: Schedule (cron) or push events.
- **Cron expression** (for scheduled triggers): When to run the workflow.
- **Trigger repos** (for push triggers): Repositories to watch for pushes.
- **Context repos**: Additional repositories for the agent to reference.
- **Automerge**: Whether to merge changes automatically or open a pull request.
- **Agent instructions**: The prompt for the agent to follow.

The command creates a workflow file in `.mintlify/workflows/` with the configured settings.

<Tip>
You can also create workflow files manually. See [Workflows](/agent/workflows) for the file format and configuration options.
</Tip>
