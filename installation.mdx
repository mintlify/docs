...
- [Node.js](https://nodejs.org/en) v20.17.0+ (LTS versions recommended) installed. Node 25+ is not supported.
...
This command is also available as `mint accessibility`, `mint accessibility-check`, and `mint a11y-check`.
...
Use the `--force` flag to rename the file and skip errors:
...
This command is also available as `mint v`.
