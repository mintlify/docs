...
To check external links for broken URLs, use the `--check-external` flag:

```bash
mint broken-links --check-external
```

To check links inside `<Snippet>` components, use the `--check-snippets` flag:

```bash
mint broken-links --check-snippets
```
