...
This command generates a ZIP archive containing your entire documentation site as static HTML files, along with helper scripts to serve the site locally. The exported archive includes a Node.js-based static server and launcher scripts for macOS/Linux and Windows.

By default, the output file is named `export.zip`. Use the `--output` flag to specify a custom output path:
...
| Flag | Description |
| --- | --- |
| `--output` | Output ZIP file path. Defaults to `export.zip`. |
| `--groups` | Mock user groups for the export (useful when exporting group-restricted content). |
| `--disable-openapi` | Disable OpenAPI file generation during the export. |

<Note>
  An internet connection is required on first run to download the client application. Subsequent runs use the cached client from `~/.mintlify/`.
</Note>
