---
title: "Claude 提示词缓存"
description: "提示缓存是一个强大的功能，通过允许从你的提示中的特定前缀恢复，优化你的 API 使用。这种方法显著减少了重复任务或包含一致元素的提示的处理时间。"
icon: ''
---

以下是如何使用 Messages API 实现提示缓存的一个示例：  
...
- Claude 3.7 Sonnet、Claude 3.5 Sonnet 和 Claude 3 Opus 为 1024 个词元
- Claude 3.5 Haiku 和 Claude 3 Haiku 为 2048 个词元

即使标记了 `cache_control`,更短的提示词也无法缓存。任何请求缓存少于这个数量的词元都会在不使用缓存的情况下处理。要查看提示词是否被缓存,请查看响应使用情况[字段](https://docs.anthropic.com/en/docs/build-with-claude/prompt-caching#tracking-cache-performance)。
...
- `cache_creation_input_tokens`: 创建新缓存条目时写入缓存的词元数
- `cache_read_input_tokens`: 从缓存中检索的词元数
- `input_tokens`: 未从缓存读取或用于创建缓存的输入词元数
...
- **输出词元生成:** 提示词缓存不会影响输出词元生成。你收到的响应将与不使用提示词缓存时完全相同。
