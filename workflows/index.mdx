...
The **Improve docs from assistant chats** workflow runs on a schedule and uses recent conversations with your [AI assistant](/ai/contextual-menu) to identify documentation gaps. The agent samples user questions from the last run window, looks for patterns where the existing docs didn't answer well, and opens a pull request with suggested updates.

Use this workflow to keep your docs aligned with what users are actually asking. Review the generated pull request before merging — the agent proposes changes based on chat evidence, but you decide what ships.

Enable it from the [Workflows](https://dashboard.mintlify.com/products/workflows?tab=workflows) page. See [Enable workflows](/workflows/enable) for configuration steps. 
