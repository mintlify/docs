...
When you enable the **Assistant docs updates** workflow, the agent reviews recent [assistant](/assistant/index) conversations on a weekly schedule. It then proposes targeted documentation updates that close the gaps behind those questions.

The workflow groups recent assistant queries into themes and prioritizes themes with the most unanswered questions, repeat questions, and negative feedback. The agent verifies each theme against your existing docs and opens a pull request with small, focused edits to the pages that best address the recurring questions. If there is not enough conversation signal for a meaningful update, the workflow runs without making changes.
