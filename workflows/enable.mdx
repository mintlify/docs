...
For custom workflows, changelog workflows, and other non-maintenance workflows that run on a schedule, you can add context repositories so the agent has access to source code, history, or related projects when it runs.

Context repositories are read-only references. The agent clones them alongside your content repository and uses them to inform updates, but does not push commits or open pull requests against them.

For GitHub repositories, you must install the Mintlify GitHub App on each repository you add as context.
