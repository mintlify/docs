...
Context repositories are supported on all workflow types, including the changelog workflow. Use them when the agent needs information that lives outside your documentation repository, such as a product codebase that drives release notes or an SDK repository that drives reference updates.
