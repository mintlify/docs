...
When you enable the **Assistant docs updates** workflow, the agent reviews recent [assistant](/assistant/index) conversations on a weekly schedule and proposes targeted documentation updates that close the gaps surfacing in those questions.

The workflow groups recent assistant queries into themes and prioritizes themes with the most unanswered questions, repeat questions, and negative feedback. The agent then verifies each theme against your existing docs and opens a pull request with small, focused edits to the pages that best address the recurring questions. If there is not enough conversation signal for a meaningful update, the workflow runs without making changes.

Use this workflow if you want your docs to evolve automatically based on what users are actually asking the assistant.

**Prerequisites:**

- The [assistant](/assistant/index) is enabled on your project and has accumulated conversation history.

**Defaults:**

- Runs weekly on Mondays at 09:00 UTC. Customize the schedule in the workflow settings.
- Opens a pull request for review. Turn on automerge if you want updates to merge automatically.

User-specific text, personal data, and secrets from assistant conversations are not copied into the docs, pull request title, pull request body, commit messages, or branch names. The agent summarizes themes instead.
