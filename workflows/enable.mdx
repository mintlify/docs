...
If you connect Slack to your workflow and use **Require review**, Mintlify posts a reminder to your configured Slack channels when a workflow's pull request has been waiting for review for three days. The reminder includes the workflow name and a **Review PR** button that links directly to the open pull request.

Each pull request triggers at most one reminder. When the pull request is merged or closed, Mintlify strikes through the reminder message and updates the original notification.
