...
For workflows that don't run on push events, you can add up to 10 context repositories that the agent clones and references when the workflow runs. Context repositories are available on custom workflows, changelog workflows, and other workflows that run on a schedule.

For GitHub repositories, you must install the Mintlify GitHub App on each repository. Add repositories on the [GitHub app](https://dashboard.mintlify.com/settings/organization/github-app) settings page.

### Trigger integrations

Some workflows can use a third-party integration (for example, Linear or Slack) as a trigger. The picker shows a curated shortlist of supported integrations by default. Click **Show more integrations** to browse the full catalog of trigger-capable integrations, or use the search field to find one that isn't in the shortlist.

To install or remove the integrations themselves, see [Manage integrations](/agent/integrations).
...
When you enable the **Improve docs from assistant chats** workflow, the agent reviews recent conversations from your documentation assistant on a recurring schedule, identifies common gaps, and opens a pull request with proposed updates.

This workflow runs on a custom schedule that you choose. It uses your project's assistant conversation data as context. It does not accept context repositories or third-party trigger integrations.

To get useful output, make sure your assistant is receiving real user questions before enabling the workflow.
