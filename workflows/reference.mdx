...
When you run the workflow on every source code pull request, the agent creates a content update pull request whenever you merge a source code pull request that requires content updates. Use the push-based variant when you want content updates to appear as soon as code ships, rather than batched on a schedule.
...
Running on a content update trigger maintains your content with minimal lag. Running on a schedule is generally more credit-efficient since the agent batches work across multiple changes in a single run, but there is a delay between when content changes and when the maintenance workflow acts on it.

Any enabled maintenance workflows run on pull requests created by self-updating content workflows. If any changes are needed for the maintenance workflow, the agent combines the work into a single pull request. For example, if you enable the **translate content** workflow, the agent adds translations to any pull requests opened by the **update from code changes** workflow.
