...
Translates updated content into your target languages whenever changes push to your project repository. Mintlify reads the [languages](/organize/navigation#languages) defined in your `docs.json` and always translates into every language configured there, in addition to any extra target languages you select for the workflow.

For example, if your `docs.json` defines English as the source language and French as an additional language, and you also add Japanese as a target on the workflow:
...
The workflow translates each English content change into both French (from `docs.json`) and Japanese (from the workflow target).

To disable translations for a language, remove it from your `docs.json` and delete all files in that language's directory. You cannot disable translations for a language that still has published content.
