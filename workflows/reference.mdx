...
Any enabled maintenance workflows run on self-updating content workflows. If changes are needed, the agent commits the maintenance changes to the self-updating pull request. For example, if you enable the **translate content** workflow, the agent adds translations for any updates made with the **update from code changes** workflow to the same pull request.
