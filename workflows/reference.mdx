...
Any enabled maintenance workflows run on pull requests created by self-updating content workflows. If the maintenance workflow needs any changes, the agent combines the work into a single pull request. For example, if you enable the **translate content** workflow, the agent adds translations to any pull requests opened by the **update from code changes** workflow.
