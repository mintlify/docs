...
Set `iconPlacement` to `feature` to display the icon centered over a prominent feature area at the top of the card. This works well for visually highlighting cards in a grid.

<Card title="Feature card" icon="sparkles" iconPlacement="feature">
  This card displays the icon in the feature area.
</Card>

```mdx Feature icon placement example
<Card title="Feature card" icon="sparkles" iconPlacement="feature">
  This card displays the icon in the feature area.
</Card>
```

You can combine `iconPlacement="feature"` with `img` to overlay the icon on top of the image.

<Card title="Feature with image" icon="sparkles" iconPlacement="feature" img="https://mintlify-assets.b-cdn.net/yosemite.jpg">
  The icon appears centered over the image.
</Card>

```mdx Feature icon with image example
<Card
...
<ResponseField name="iconPlacement" type="string" default="default">
  Controls where the icon is rendered. Set to `default` for the standard position next to the title, or `feature` to display the icon centered over a feature area at the top of the card.
</ResponseField>
