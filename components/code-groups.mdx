...
Code groups inherit global styling from your `docs.json` file. You can customize themes using `styling.codeblocks` with simple options like `"system"` or `"dark"`, or configure custom [Shiki themes](https://shiki.style/themes) for light and dark modes. See [Settings](/settings#styling) for detailed configuration options.
