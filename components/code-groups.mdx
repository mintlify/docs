...
<Tip>
  Try to keep code blocks within a group at similar heights. When tabs have significantly different heights—for example, "Before" and "After" examples in a migration guide—the page can reflow when readers switch tabs, which is disorienting on long pages with many code groups. If you need to compare snippets of very different lengths, consider showing them side by side with [columns](/components/columns) instead.
</Tip>
