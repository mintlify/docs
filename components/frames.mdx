...
When you place a `<video>` element with `autoPlay` inside a frame, the component automatically adds `playsInline`, `loop`, and `muted` attributes. This ensures autoplay works reliably across browsers and devices without requiring you to set each attribute manually.

```mdx Frame with autoplay video
