...
<Note>
While all font formats listed above can be served as static files, custom font configuration in `docs.json` only supports `.woff` and `.woff2` formats. See [font settings](/settings#fonts) for custom font setup.
</Note>
