...
These properties apply to the `<Tabs>` wrapper component.

<ResponseField name="defaultTabIndex" type="number" default="0">
  The index of the tab to display by default when the page loads.
</ResponseField>
...
These properties apply to each `<Tab>` component inside `<Tabs>`.
