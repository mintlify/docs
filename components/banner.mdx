...
1. **Language-specific banner**: If the current language has a banner configured, it displays first.
2. **Global banner**: If no language-specific banner exists, the global banner (defined at the root level) displays.

### Dismissal behavior

Banner dismissals are stored per-language. When a user dismisses a banner:

- The dismissal only applies to the current language's banner.
- Switching to a different language shows that language's banner (if configured and not previously dismissed).
- Each language maintains its own dismissal state independently.