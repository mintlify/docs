...
<CodeGroup>
  ...
  "banner": {
    ...
    "color": { "light": "#0D9373", "dark": "#0D9373" }
  }
  ```
</CodeGroup>
...
<ResponseField name="type" type='"info" | "warning" | "critical"'>
  The visual style of the banner. `info` uses your primary brand color for general announcements. `warning` uses an amber background for cautionary notices like maintenance or deprecations. `critical` uses a red background for urgent notices. Defaults to `info`.
</ResponseField>

<ResponseField name="color" type="string or object">
  Override the banner background color with a custom hex color. Accepts a string hex code or a `{ light, dark }` object for theme-aware colors. When set, takes precedence over the color implied by `type`. Banner text is white, so choose a dark enough background for legibility.
</ResponseField>
