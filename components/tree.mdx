...
You can author trees with either a Markdown list or the `Tree.Folder` and `Tree.File` components. `<Tree>` and `<FileTree>` are aliases, so either tag works with either syntax.

## Markdown list syntax

Write a nested Markdown list inside `<FileTree>` (or `<Tree>`) to describe a folder structure. Add a trailing slash to mark a folder, or nest items under it. Folders that contain nested items are expanded by default.
...
Leave a blank line after the opening tag and before the closing tag so the list is parsed as Markdown. Inline formatting like `` `code` ``, `**bold**`, or `_italic_` is stripped from names, so use them only when they render correctly in your build.
