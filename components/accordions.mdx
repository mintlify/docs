...
You can use JSX in the `title` prop to render rich content like badges and formatted text.

<Accordion title={<>Release notes <Badge color="blue">Beta</Badge></>}>
  This accordion has a badge in its title.
</Accordion>

````mdx Rich title example
<Accordion title={<>Release notes <Badge color="blue">Beta</Badge></>}>
  This accordion has a badge in its title.
</Accordion>
````
...
<Note>
  Only static JSX is supported in component attributes. Dynamic expressions like function calls or event handlers are not allowed.
</Note>

## Properties

<ResponseField name="title" type="string | JSX" required>
  Title in the Accordion preview. Supports plain text or static JSX elements like `<Badge>`.
</ResponseField>
