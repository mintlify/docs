...
The table of contents (TOC) automatically updates to show only the headings from the currently selected view. When you switch between views using the multi-view dropdown, the TOC refreshes to display the relevant headings for that view.

This means you can have different heading structures in each view, and users will only see the headings that are relevant to their selected language or framework.
