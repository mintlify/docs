...
Mintlify builds the multi-view dropdown automatically from the `title` of each `View` component on the page. You don't configure the dropdown separately. Use one `View` per title per page: duplicate titles are ignored, and a `View` without a title is never rendered.
