...
Your site is automatically optimized for AI tools and search engines to help users discover your documentation. Every page is available as Markdown by appending `.md` to the URL or by sending an `Accept: text/markdown` header. Markdown helps AI tools process your content faster and use fewer tokens. Use the [visibility](/components/visibility) component to tailor content for each audience.
