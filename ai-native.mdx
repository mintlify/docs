...
Mintlify hosts [`llms.txt`](/ai/llmstxt), [`llms-full.txt`](/ai/llmstxt#llms-full-txt), and [`skill.md`](/ai/skillmd) files for your documentation. These files help LLMs respond efficiently with relevant information to user queries and provide a list of capabilities for agents to use. As a result, users are more successful with your product.
