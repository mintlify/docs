...
Create [workflows](/agent/workflows) to run the agent when content is updated, on a schedule, or when source code changes. Each workflow defines a prompt for the agent and a trigger for when to run it.
...
<CardGroup>
  ...
  <Card title="Workflows" icon="message-circle-plus" href="/agent/workflows">
    Get documentation updates automatically when content changes, on a schedule, or when source code changes.
  </Card>
