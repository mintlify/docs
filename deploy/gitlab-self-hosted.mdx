...
<Note>
  This guide is only for the **OAuth** integration that powers automations. You must configure the deployment-side connection (used for content sync and previews) separately with a deploy token. See the [GitLab guide](/deploy/gitlab) for instructions. The OAuth integration depends on the deployment-side connection.
</Note>
