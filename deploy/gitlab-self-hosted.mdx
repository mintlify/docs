...
<Note>
  This guide is only for the **OAuth** integration that powers automations. You must configure the project-side connection (used for content sync and previews) separately with an access token. See the [GitLab guide](/deploy/gitlab) for instructions. The OAuth integration depends on the project-side connection.
</Note>
