...
<Steps>
  <Step title="Register an OAuth application on your GitLab instance">
    ...
    <Tip>
      Editing an OAuth application on GitLab can rotate the client secret silently. If you make changes later, select **Renew secret** and update the new value in Mintlify.
    </Tip>
  </Step>
  ...
  <Step title="Register the instance in Mintlify">
    ...
    Select **Connect Self-Hosted GitLab** and enter:
    ...
    Select **Save instance**. Mintlify encrypts the secret at rest and never returns it to the browser after saving.
  </Step>
  <Step title="Authorize">
    Select **Authorize self-hosted GitLab**. GitLab redirects you to your instance, prompts you to sign in if needed, and shows a consent screen listing the requested scopes.

    After you select **Authorize** on GitLab, you'll be redirected back to Mintlify and the new connection appears in the installations list, badged with your instance hostname.
  </Step>
...
<Steps>
  <Step title="Revoke each connection">
    Select **Revoke** on every installation listed under the self-hosted instance. This removes the webhook on every connected project and revokes the OAuth token on GitLab.
  </Step>
  <Step title="Remove the instance">
    In the **Self-hosted GitLab** card, select **Remove instance**.
  </Step>
