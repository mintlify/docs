...
<Steps>
  ...
  <Step title="Choose projects">
    Expand the connection in the dashboard. Mintlify lists every top-level group your authorizing user has **Developer role or higher** on, plus a **Personal projects** entry for projects in the user's personal namespace. Groups where the user only holds project-level access are not listed.
    ...
    <Note>
      The authorizing user needs two levels of access to fully connect a project:

      - **Developer or higher on the project's top-level group** for the group (and its projects) to appear in the picker. Adding a user as Maintainer on an individual project is not enough—the group itself must grant Developer+ or the project will not surface here.
      - **Maintainer or higher on the project itself** for Mintlify to mint short-lived project access tokens during automation runs. Without Maintainer, the agent can read but cannot push commits or open merge requests.

      If you cannot get Developer+ on the top-level group, ask a group owner to grant it for the initial authorization, or have someone who already has that access authorize the connection.
    </Note>
  </Step>
</Steps>
