...
{/* TODO: Confirm exact output filename and whether it's configurable */}
...
<Tabs>
  ...
  <Tab title="Windows">
    ...
    <Note>
      Windows SmartScreen may block the file because it was downloaded from the internet. You can unblock it in one of two ways:

      - **From the SmartScreen prompt**: Select **More info**, then select **Run anyway**.
      - **From file properties**: Right-click **Start Docs.bat**, select **Properties**, check the **Unblock** checkbox in the Security section, then select **Apply**.
    </Note>
  </Tab>
  <Tab title="Manual">
    1. Unzip the archive.
