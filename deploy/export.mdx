...
- **Search doesn't work**: The search bar relies on a hosted backend that isn't included in the archive, so it has no effect in an offline or air-gapped environment. To add offline search, index the exported HTML with a static search tool such as [Pagefind](https://pagefind.app).
- **AI features don't work**: Ask AI and other AI-powered features depend on a hosted backend and don't work in the exported archive.
