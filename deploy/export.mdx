...
|---|---|---|
| `--output` | Set the output zip file path. | `export.zip` |
| `--groups` | Mock user groups for export. Useful when your docs use [group-based access control](/deploy/authentication-setup#control-access-with-groups) and you want to export pages visible to specific groups. | None |
| `--disable-openapi` | Skip OpenAPI file generation during the export. Use this to speed up export when you don't need API reference pages. | `false` |

For example, to export to a custom filename and include only pages visible to the `enterprise` group:

```bash
mint export --output enterprise-docs.zip --groups enterprise
```
