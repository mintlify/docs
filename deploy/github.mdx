...
Because the app only sees the repositories where it's installed, it cannot build preview deployments for pull requests opened from a fork. See [Fork pull requests](/deploy/preview-deployments#fork-pull-requests) for the workaround.
