...
description: "Show custom content based on user authentication and profile data."
...
For all personalization methods, logout is handled client-side. When users click the logout button, Mintlify clears their stored session data in the browser. You do not need to implement a logout endpoint for personalization.

If you need to limit how long Mintlify keeps personalization data, set the `expiresAt` field in your user info response. When the current time passes `expiresAt`, the user's stored data is automatically deleted and they must authenticate again to access personalized content. This does not revoke sessions or tokens in your identity provider. Handle server-side revocation in your own system, if needed.
...
<Note>
  Control session length with the `expiresAt` field in your user info response. This is a Unix timestamp--seconds since epoch--indicating when the session should expire. When the session expires, the user's stored data is deleted and they must log in again.
</Note>
...
<Steps>
  <Step title="Create user info API endpoint.">
    Create an API endpoint that:
    * Responds to `GET` requests.
    * Uses your existing session cookies to identify users. Mintlify calls this endpoint with credentials included, so your session cookies are automatically sent. No `Authorization` header is used.
