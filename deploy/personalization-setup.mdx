...
If you need to invalidate sessions server-side, use the `expiresAt` field in your user info response to control session duration. When the session expires, the user's stored data is automatically deleted and they must authenticate again to access personalized content.
