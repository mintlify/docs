...
<Note>
Control session length by including `expiresAt` in your response. This is a Unix timestamp (seconds since epoch) indicating when the session should expire. When the session expires, the user's stored data is deleted and they must log in again.
</Note>
