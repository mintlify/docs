...
| Microsoft Azure | Helm chart (AKS) |
| Google Cloud | Helm chart (GKE) |
| Oracle Cloud | Helm chart (OKE) |
...
| Infrastructure | Mintlify operates everything | You operate the cluster, network, and data stores. Mintlify ships versioned releases with upgrade runbooks and supports the application tier |
| Platform updates | Continuous, automatic | Versioned releases that you review and roll out on your own schedule |
...
Self-hosted rollouts usually start narrow and expand. A common path is public documentation first, then authenticated content, the web editor, and AI features as your security reviews complete.
...
| Search | <Icon icon="circle-check" color="#16a34a" /> | Runs inside your deployment, reindexed on publish |
...
| Versioned releases and rollback | <Icon icon="circle-check" color="#16a34a" /> | Image versions pinned per release, roll back by redeploying the previous version |
...
| Cloud-dependent integrations | <Icon icon="circle-xmark" color="#9ca3af" /> | Integrations that depend on Mintlify cloud services are unavailable |
...
| Search and indexing | Docs search, reindexed on publish | <Icon icon="circle-check" color="#16a34a" /> |
...
As a starting point, a production deployment runs on roughly 45 to 60 vCPU, 160 to 220 GB of memory, and about 1 TB of SSD storage across services, with non-production environments at about half that. Your account team sizes the deployment with you based on page count, traffic, and which features you enable.

## Set up your platform

<Tabs>
  <Tab title="AWS" icon="/images/logos/aws-mark.svg">
    AWS deployments use an AWS CDK app that provisions and updates the full stack in your account. Container images are pinned to specific versions, so every rollout is reproducible and reviewable.
  ...
  <Tab title="Kubernetes" icon="/images/logos/kubernetes.svg">
    Kubernetes deployments use the Mintlify enterprise Helm chart and run on any conformant cluster. The same chart covers managed Kubernetes on any cloud and on-premises clusters:
...
Platform updates and content updates are decoupled. You control when the platform changes, and your documentation stays current on its own.

### Platform updates

Mintlify ships versioned releases with upgrade runbooks and release notes from your account team. Each release pins specific image versions, so you can test a release in a non-production environment before rolling it out and roll back to the previous version if needed.
...
Content flows through your Git source, not through platform releases. When you push to your documentation repository, the build workers rebuild the site and publish it to object storage automatically. No platform deploy is involved.
...
<Columns cols={2}>
  <Card title="Talk to your account team" icon="messages-square" href="https://www.mintlify.com/enterprise">
    Scope a self-hosted deployment on your platform and plan your rollout.
  </Card>
