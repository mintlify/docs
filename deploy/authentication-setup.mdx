...
When token claims are configured, Mintlify reads groups directly from the token returned by your token exchange and does not call the Info API URL, even if one is set. To configure token claims for your deployment, contact Mintlify support.
