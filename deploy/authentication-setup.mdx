...
### Offer multiple login URLs

If your organization has more than one identity provider or region-specific login flow, you can configure between 2 and 10 named login URLs for your JWT-authenticated site. When more than one URL is set, unauthenticated visitors see a login selection screen with your site's branding, and each option redirects to the URL you configured.

To configure multiple login URLs, provide a list of `{ name, url }` pairs when you update your JWT authentication settings. Each name is displayed to users on the selection screen and must be 1 to 100 characters. Each URL must be a valid URL. With a single login URL, visitors continue to redirect directly to that URL as before.
...
Multiple login URLs apply to site-wide JWT authentication only. JWT [personalization](/create/personalization) accepts a single `loginUrl`.
