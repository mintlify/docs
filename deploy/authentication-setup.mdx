...
<ParamField path="host" type="string">
  **Required for JWT authentication.** The hostname of your documentation site (for example, `docs.foo.com`). This must exactly match the host where your documentation is deployed. Mintlify validates that the JWT's host matches the requesting host to prevent token reuse across different sites.
</ParamField>
