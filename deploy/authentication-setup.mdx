...
<Note>
  **Token claims** takes precedence over **Info API URL**. If you configure both, Mintlify uses the token claim for groups and does not call the info API, so `apiPlaygroundInputs`, `content`, and `expiresAt` from that endpoint are ignored. Use **Info API URL** instead if you need those fields.
</Note>
