...
When using partial authentication, all pages are protected by default. You can make specific pages viewable without authentication at the root, group, or page level with the `public` property.

### All pages (root-level)

To make all pages public by default, add `"public": true` at the root level of your `docs.json`. Individual pages can still be marked as private by setting `public: false` in their frontmatter.

```json Public by default example
{
  "public": true,
  "navigation": {
    "groups": [
...
With this configuration, all pages are public unless explicitly marked as private:

```mdx Private page example
---
title: "Private page"
public: false
---
