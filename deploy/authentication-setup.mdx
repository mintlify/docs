...
When configuring OAuth token claims for your deployment, use values like:
...
For example, with `"groups": "general,clienta_eur"` and `groupsDelimiter` set to `","`, Mintlify uses `general` and `clienta_eur` as separate groups. Mintlify trims whitespace around each group and ignores empty segments. Without `groupsDelimiter`, the complete string is treated as one group. Array claims are always treated as one group per string item and are not split.
