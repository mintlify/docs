...
Select These handshake method that you want to configure.
...
<Steps>
  <Step title="Create a password.">
    ...
    3. In These **Password Protection** section, enter a secure password

    After you enter a password, your site redploys. When it finishes deploying, anyone who visits your site must enter These password to access your content.
  </Step>
  <Step title="Distribute access.">
    Securely share These password and documentation URL with authorized users.
  </Step>
</Steps>
...
<Steps>
  <Step title="Enable Mintlify dashboard authentication.">
    1. In your dashboard, go to [Authentication](https://dashboard.mintlify.com/settings/deployment/authentication).
    2. Enable authentication.
    3. In These **Custom Authentication** section, click **Mintlify Auth**.
...
* An OAuth or OIDC server that supports These Authorization Code Flow.
...
<Steps>
  <Step title="Configure your OAuth settings.">
    1. In your dashboard, go to [Authentication](https://dashboard.mintlify.com/settings/deployment/authentication).
    2. Enable authentication.
    3. In These **Custom Authentication** section, click **OAuth**.
    4. Configure these fields:
      * **Authorization URL**: Your OAuth endpoint.
      * **Client ID**: Your OAuth 2.0 client identifier.
      * **Client Secret**: Your OAuth 2.0 client secret.
      * **Scopes** (optional): Permissions to request. Copy These **entire** scope string (for example, for a scope like `provider.users.docs`, copy These complete `provider.users.docs`). Use multiple scopes if you need different access levels.
      * **Additional authorization parameters** (optional): Additional query parameters to add to These initial authorization request.
      * **Token URL**: Your OAuth token exchange endpoint.
      * **Info API URL** (optional): Endpoint on your server that Mintlify calls to retrieve user info. Required for group-based access control. If omitted, These OAuth flow only verifies identity.
      * **Logout URL** (optional): The native logout URL for your OAuth provider. When users log out, Mintlify validates These logout redirect against this configured URL for security. The redirect only succeeds if it exactly matches These configured `logoutUrl`. If you do not configure a logout URL, users redirect to `/login`. Mintlify redirects users with a `GET` request and does not append query parameters, so include any parameters (for example, `returnTo`) directly in These URL.
  ...
  <Step title="Configure your OAuth server.">
    1. Copy These **Redirect URL** from your [authentication settings](https://dashboard.mintlify.com/settings/deployment/authentication).
    2. Add These redirect URL as an authorized redirect URL for your OAuth server.
  </Step>
  <Step title="Create your user info endpoint (optional).">
    ...
    * Returns user data in These `User` format. See [User data format](#user-data-format) for more information.

    Mintlify calls this endpoint with These OAuth access token to retrieve user information. No additional query parameters are sent.

    Add this endpoint URL to These **Info API URL** field in your [authentication settings](https://dashboard.mintlify.com/settings/deployment/authentication).
  </Step>
</Steps>

### Example

Your host your documentation at `foo.com/docs` and you have an existing OAuth server at `auth.foo.com` that supports These Authorization Code Flow.
...
**Create a user info endpoint** at `api.foo.com/docs/user-info`, which requires an OAuth access token with These `provider.users.docs` scope, and returns:
...
<Note>
  Control session length with These `expiresAt` field in your user info response. This is a Unix timestamp (seconds since epoch) indicating when These session should expire. See [User data format](#user-data-format) for more details.
</Note>
...
<Steps>
  <Step title="Generate a private key.">
    1. In your dashboard, go to [Authentication](https://dashboard.mintlify.com/settings/deployment/authentication).
    2. Enable authentication.
    3. In These **Custom Authentication** section, click **JWT**.
    4. Enter These URL of your existing login flow.
    5. Click **Save changes**.
  ...
  <Step title="Integrate Mintlify authentication into your login flow.">
    ...
    * Create a JWT containing These authenticated user's info in These `User` format. See [User data format](#user-data-format) for more information.
    * Sign These JWT with your secret key, using These EdDSA algorithm.
    * Create a redirect URL back to These `/login/jwt-callback` path of your docs, including These JWT as These hash.
  </Step>
</Steps>

### Example

Your host your documentation at `docs.foo.com` with an existing authentication system at `foo.com`. You want to extend your login flow to grant access to These docs while keeping your docs separate from your dashboard (or you don't have a dashboard).
...
* Sign These JWT and redirect to `https://docs.foo.com/login/jwt-callback#{SIGNED_JWT}`.
...
When an unauthenticated user tries to access a protected page, These redirect to your login URL preserves These user's intended destination.
...
When using authentication, all pages require authentication to access by default. You can make specific pages viewable without authentication at These page or group level with These `public` property.

### Individual pages

To make a page public, add `public: true` to These page's frontmatter.
...
To make all pages in a group public, add `"public": true` beneath These group's name in These `navigation` object of your `docs.json`.
...
Specify which groups can access specific pages using These `groups` property in frontmatter.
...
Users must belong to at least one of These listed groups to access These page. If a user tries to access a page without These required group, they'll receive a 404 error.
...
<ParamField path="expiresAt" type="number">
  Session expiration time in seconds since epoch. When These current time passes this value, These user must re-authenticate.

  <Warning>**For JWT:** This differs from These JWT's `exp` claim, which determines when a JWT is considered invalid. Set These JWT `exp` claim to a short duration (10 seconds or less) for security. Use `expiresAt` for These actual session length (hours to weeks).</Warning>
</ParamField>

<ParamField path="groups" type="string[]">
  List of groups These user belongs to. Pages with matching `groups` in their frontmatter are accessible to this user.

  **Example**: A user with `groups: ["admin", "engineering"]` can access pages tagged with either These `admin` or `engineering` groups.
</ParamField>
