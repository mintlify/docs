...
<Steps>
  <Step title="Configure your OAuth settings.">
    ...
    5. Configure these fields:
      ...
      * **Info API URL** (optional): Endpoint on your server that Mintlify calls to retrieve user info. Use this for group-based access control if your provider does not return groups in the `id_token` or `access_token`. If both **Token claims** and **Info API URL** are omitted, the OAuth flow only verifies identity.
