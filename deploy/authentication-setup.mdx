...
When authentication is enabled, all pages are protected by default. Users must log in before accessing content unless specific pages are marked as public. You can make individual pages or groups publicly viewable while keeping other pages protected.
...
When using authentication, all pages are protected by default. You can make specific pages viewable without authentication at the page or group level with the `public` property.
