...
<Steps>
  <Step title="Configure your OAuth settings.">
    ...
    3. Select **OAuth** and configure these fields:
      ...
      * **Logout URL**: The native logout URL for your OAuth provider. Mintlify redirects users to this URL with a `GET` request when they log out. Mintlify does not append query parameters, so include any parameters (for example, `returnTo`) directly in the URL. Provide a landing page users see after the logout process completes.
