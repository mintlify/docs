...
<Steps>
  <Step title="Configure your OAuth settings.">
    ...
    3. Select **OAuth** and configure these fields:
      ...
      * **Logout URL**: The native logout URL for your OAuth provider. Mintlify redirects users to this URL with a `GET` request when they log out. Mintlify does not append query parameters, so include any parameters (for example, `returnTo`) directly in the URL. Configure a page to redirect users to on a successful logout.
...
<Note>
  Control session length with the `expiresAt` field in your user info response. This is a Unix timestamp--seconds since epoch--indicating when the session should expire. See [User data format](/deploy/personalization-setup#user-data-format) for more details.
</Note>
...
With both full and partial authentication modes, you can make specific pages viewable without authentication at the page or group level with the `public` property.

When using **partial authentication**, all pages are protected by default unless marked as public.

When using **full authentication**, all pages are protected by default. However, you can still use the `public` property to make specific pages accessible without authentication.
...
- Pages with `public: true` are accessible to everyone without authentication.

**Partial authentication with groups:**
- Pages require authentication by default unless you make them public.
...
```mdx Only authenticated users in specific groups can view this page
---
