...
The dashboard site visibility toggle only offers **Public** or **Private**. Partial authentication is not a separate mode. To set it up, switch site visibility to **Private** so all pages require authentication by default, then mark the pages or navigation groups you want open to everyone with `public: true`. See [Make pages public](#make-pages-public) for the frontmatter and `docs.json` syntax.
