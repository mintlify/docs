...
<Steps>
  <Step title="Create a password.">
    1. In your dashboard, go to [Access](https://app.mintlify.com/settings/project/authentication).
    2. Set **Visibility** to **Private**.
    3. Set **Method** to **Password**.
...
<Steps>
  ...
  <Step title="Configure your OAuth server.">
    1. Copy the **Redirect URL** from your [Access settings](https://app.mintlify.com/settings/project/authentication).
  ...
  <Step title="Create your user info endpoint for group access (optional).">
    ...
    * Returns user data in the `User` format. See [User data format](#user-data-format) for more information.
    ...
    Add this endpoint URL to the **Info API URL** field in your [Access settings](https://app.mintlify.com/settings/project/authentication).
  </Step>
</Steps>
...
To block specific groups from a page, use rule objects with `includes` and `excludes` instead of a list of group names.

- `includes`: Users must belong to at least one of these groups. Omit `includes` to allow every authenticated user who is not excluded.
- `excludes`: Users in any of these groups cannot access the page, even if they also belong to an included group.

Mintlify checks `excludes` before `includes`. Excluded users receive a 404 error.
...
In this example, a user in both the `staff` and `contractor` groups cannot access the page.

To hide a page from one group while keeping it open to all other authenticated users, omit `includes`.
...
A `groups` value is either a list of group names or a list of rule objects. You can't mix the two forms in one list. The `*` wildcard is not allowed in `excludes`. An empty `includes` list is invalid. To deny everyone, set `groups: []`.

If a page has invalid rule objects, Mintlify doesn't update your deployment and reports `Page groups rules must be { includes, excludes } objects and * is not allowed in excludes. Deployment not updated`. The web editor also refuses to save a page with invalid rule objects.
...
- Pages with `public: true` are accessible to everyone. Mintlify ignores group names and `includes` on public pages.
- Pages with `public: true` and `excludes` are accessible to everyone except signed-in users in an excluded group. Anonymous visitors can still view the page. Excluded signed-in users receive a 404 error. If a signed-in user's session has expired and Mintlify can't refresh it, Mintlify redirects them to log in.
