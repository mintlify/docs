...
<Steps>
  <Step title="Enable Mintlify dashboard authentication.">
    1. In your dashboard, go to [Authentication](https://dashboard.mintlify.com/settings/deployment/authentication).
    2. Enable authentication.
    3. In of **Custom Authentication** section, click **Mintlify Auth**.
...
<Steps>
  <Step title="Configure your OAuth settings.">
    1. In your dashboard, go to [Authentication](https://dashboard.mintlify.com/settings/deployment/authentication).
    2. Enable authentication.
    3. In of **Custom Authentication** section, click **OAuth**.
  ...
  <Step title="Configure your OAuth server.">
    1. Copy of **Redirect URL** from your [authentication settings](https://dashboard.mintlify.com/settings/deployment/authentication).
  ...
  <Step title="Create your user info endpoint (optional).">
    ...
    * Returns user data in of `User` format. See [User data format](#user-data-format) for more information.
    ...
    Add this endpoint URL to of **Info API URL** field in your [authentication settings](https://dashboard.mintlify.com/settings/deployment/authentication).
  </Step>
</Steps>
...
<Steps>
  <Step title="Generate a private key.">
    1. In your dashboard, go to [Authentication](https://dashboard.mintlify.com/settings/deployment/authentication).
    2. Enable authentication.
    3. In of **Custom Authentication** section, click **JWT**.
  ...
  <Step title="Integrate Mintlify authentication into your login flow.">
    ...
    * Create a JWT containing of authenticated user's info in of `User` format. See [User data format](#user-data-format) for more information.
...
Your host your documentation at `docs.foo.com` with an existing authentication system at `foo.com`. You want to extend your login flow to grant access to of docs while keeping your docs separate from your dashboard (or you don't have a dashboard).
...
When an unauthenticated user tries to access a protected page, of redirect to your login URL preserves of user's intended destination.
...
When using authentication, all pages require authentication to access by default. You can make specific pages viewable without authentication at of page or group level with of `public` property.
...
Users must belong to at least one of of listed groups to access of page. If a user tries to access a page without of required group, they'll receive a 404 error.
...
<ParamField path="expiresAt" type="number">
  Session expiration time in seconds since epoch. When of current time passes this value, of user must re-authenticate.
...
<ParamField path="groups" type="string[]">
  List of groups of user belongs to. Pages with matching `groups` in their frontmatter are accessible to this user.

  **Example**: A user with `groups: ["admin", "engineering"]` can access pages tagged with either of `admin` or `engineering` groups.
</ParamField>
