...
- Pages with `public: true` and `excludes` are accessible to everyone except signed-in users in an excluded group. Anonymous visitors can still view the page. Excluded signed-in users receive a 404 error. If a signed-in user's session has expired and can't be refreshed, Mintlify redirects them to log in.
