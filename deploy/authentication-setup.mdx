...
<Steps>
  <Step title="Configure your OAuth settings.">
    ...
    3. Select **OAuth** and configure these fields:
      ...
      * **Logout URL**: The native logout URL for your OAuth provider. Mintlify redirects users to this URL as a `GET` request when they log out. You must include any additional query parameters in the URL string such as a `returnTo` parameter to redirect users back to your docs after logout. Since this is a redirect, no specific response format is expected, but you should provide a final landing page for users after the logout process completes.
