...
<Steps>
  <Step title="Configure your OAuth settings.">
    ...
    3. Select **OAuth** and configure these fields:
      ...
      * **Logout URL**: The native logout URL for your OAuth provider. Mintlify redirects users to this URL as a `GET` request when they log out. No additional query parameters are added unless you include them in the URL string (for example, a `returnTo` parameter to redirect users back to your docs after logout). Since this is a redirect, no specific response format is expected, but you should provide a final landing page for users after the logout process completes.
...
<Note>
Control session length by including `expiresAt` in your user info response. This is a Unix timestamp (seconds since epoch) indicating when the session should expire. See [User data format](/deploy/personalization-setup#user-data-format) for more details.
</Note>
