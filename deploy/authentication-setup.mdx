...
If your organization has more than one identity provider or region-specific login flows, configure multiple named login URLs for your JWT-authenticated site. You can configure between 1 and 10 login URLs.

When you set more than one URL, unauthenticated visitors see a login selection screen with your site's branding, and each option redirects to your configured URL.

To configure multiple login URLs, provide a list of `{ name, url }` pairs when you update your JWT authentication settings. Names must be between 1 and 100 characters. URLs must be valid.
