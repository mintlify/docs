...
Mintlify supports two reverse proxy configurations depending on your subpath requirements. Both target your `<your-subdomain>.mintlify.site` hostname.
...
- **Custom subpath**: Use this when you need a subpath other than `/docs` (such as `/help` or `/resources`). This approach requires additional routing rules.
...
<Warning>
  When you enable **Host at `/docs`**, your canonical docs URL becomes `<your-subdomain>.mintlify.site`. Proxy to that hostname so cache invalidations and updates reach your site.
</Warning>
...
**Cause**: Your reverse proxy points to a hostname other than your canonical Mintlify subdomain, so cache invalidations don't reach your site.
...
- Set `Origin` header to your Mintlify subdomain (`<your-subdomain>.mintlify.site`)
