...
- **Host at `/docs`**: Use `mintlify.dev` as the proxy target. This approach has a simpler configuration with fewer routes and supports automatic cache invalidation when you publish updates.
- **Custom subpath**: Use `mintlify.app` as the proxy target. This approach supports any subpath (like `/help` or `/resources`) and requires additional routing rules.
...
| `/.well-known/skills/*`         | `<your-subdomain>.mintlify.dev/docs` | No cache |
