...
`sourceRef` entries resolve during a multi-repository deploy, when Mintlify has access to every source. Local preview with `mint dev` only sees a single repository, so it cannot inline the referenced navigation.

When you run `mint dev` in a repository whose `docs.json` contains `sourceRef` entries, Mintlify strips those entries from the preview and logs a warning. The rest of the navigation renders normally, but sections that come from another source are missing until you deploy.
