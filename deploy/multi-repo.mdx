...
One source in a multi-repository deployment can use a root URL path (`/`). Content from that source is served at the root of the site with no path prefix, while other sources are served under their configured URL paths. This is useful when one repository holds the main documentation and other repositories add sections like an API reference or SDK guides.
