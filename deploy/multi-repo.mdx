...
Add a `sourceRef` entry to a navigation array in the base source's `docs.json`. The value must match the URL path of another repository source in the deployment.
...
In this example, the base source at `/docs` shows a `Guides` tab and inlines the `tabs` from a second source configured at `/api`.
...
- `sourceRef` values must match the URL path of a repository source in the same deployment.
