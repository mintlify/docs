...
<Steps>
  ...
  <Step title="Set the URL path">
    ...
    To serve a source at the root of your site with no path prefix, leave the URL path empty or set it to `/`. Only one source per deployment can use the root URL path.
...
You can remove a repository source from the [Git settings page](https://app.mintlify.com/settings/project/git-settings) in your dashboard. When only one repository source remains, Mintlify removes the source URL path and treats the remaining repository as the root source for the deployment.
