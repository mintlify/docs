...
sidebarTitle: "AWS"
description: "Deploy documentation on AWS with Route 53 DNS and CloudFront CDN."
...
<Note>
  **For `/docs` subpath only**: You can use the simpler self-service option. Enable the "Host at `/docs`" toggle in your [dashboard](https://dashboard.mintlify.com/settings/deployment/custom-domain) and follow the [Vercel rewrites guide](/deploy/vercel#host-from-docs). The AWS CloudFront setup below is only required for custom subpaths other than `/docs`.
</Note>
