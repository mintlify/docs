...
Create a behavior with a **Path pattern** of `/_mintlify/*` and set **Origin and origin groups** to the `.mintlify.site` URL. API playground and analytics requests are sent to `_mintlify` paths under your base path, such as `/docs/_mintlify/`, which your `/docs/*` behavior already covers.
