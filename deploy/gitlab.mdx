...
<Steps>
  ...
  <Step title="Select events">
    ...
    <Note>
      Automation workflows that run on push to the deployment branch currently only trigger when a merge request is merged. Direct pushes to the deployment branch on GitLab do not trigger workflow runs.
    </Note>
  </Step>
