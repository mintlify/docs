...
<Steps>
  <Step title="Select GitLab as your provider">
    On the [Git Settings](https://app.mintlify.com/settings/deployment/git-settings) page, select **Connect to GitLab** and then select **Continue**.
  </Step>
  <Step title="Download your content">
    <Tip>
      If you already have a GitLab repository with your documentation, you can skip the download and select **Continue setup** directly.
    </Tip>
    ...
    * Push the contents to your repository.
    ...
    Select **Continue setup** to proceed.
  </Step>
  ...
  <Step title="Generate an access token">
    ...
    Select **Create project access token** and copy the token.
...
<Steps>
  ...
  <Step title="Revalidate your settings">
    Select the green **Active** badge in the corner of the GitLab settings box to revalidate your Git source.
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="Get webtoken">
    In your Mintlify dashboard, select **Show Webtoken**. Copy the webtoken.
    <Frame>
  ...
  <Step title="Test the webhook">
    After you create the webhook, select the **Test** dropdown. Select **Push events** to send a sample payload. If the test returns `Hook executed successfully: HTTP 200`, you configured the webhook correctly.
