...
If you are running a self-hosted GitLab instance, you can connect it to Mintlify by providing your instance's host URL.

<Steps>
  <Step title="Enable self-hosted setup">
    In the [Mintlify dashboard](https://dashboard.mintlify.com/settings/deployment/git-settings), toggle on **Set up as self-hosted** in the GitLab configuration panel.
  </Step>
  <Step title="Enter your host URL">
    Enter the URL of your self-hosted GitLab instance in the **Host URL** field (for example, `https://gitlab.example.com`).
  </Step>
  <Step title="Complete the setup">
    Fill in your project ID, access token, and deploy branch as described in the [connection setup above](#set-up-the-connection), then click **Save Changes**.
  </Step>
</Steps>

<Note>
  The access token and webhook configuration steps are the same for self-hosted GitLab instances. Make sure your instance is accessible from the internet so Mintlify can connect to it.
</Note>
