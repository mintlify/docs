---
title: "GitLab"
description: "Connect your GitLab repository to Mintlify for automated documentation deployments, merge request previews, workflows, and continuous synchronization."
keywords: ["GitLab integration", "access tokens", "merge request previews", "self-hosted", "instance", "OAuth", "workflows"]
---
...
If you use [workflows](/agent/workflows), you must also connect GitLab via OAuth so that workflows can push commits and create merge requests. See [Connect GitLab via OAuth](#connect-gitlab-via-oauth).
...
[Workflows](/agent/workflows) need OAuth access to push commits and create merge requests on your GitLab repositories. The access token and webhook setup above handles deployments and sync, but workflows require a separate OAuth connection.

<Steps>
  <Step title="Open GitLab OAuth settings">
    In the [Mintlify dashboard](https://dashboard.mintlify.com/settings/organization/gitlab-oauth), go to **Settings** > **Organization** > **GitLab OAuth**.
  </Step>
  <Step title="Connect your GitLab account">
    Click **Connect GitLab**. You are redirected to GitLab to authorize Mintlify. After authorizing, you are returned to the dashboard.
  </Step>
  <Step title="Enable projects">
    Expand your connection to see your GitLab groups. Expand a group and check the box next to each project you want workflows to access. Enabling a project registers a webhook on that project automatically.
  </Step>
</Steps>

You can connect multiple GitLab accounts and enable projects from each. To remove a connection, click **Revoke** next to it. Revoking removes all webhooks associated with that connection.

<Note>
  GitLab OAuth is only required for workflows. Standard deployments and merge request previews use the access token and webhook configured in [Set up the connection](#set-up-the-connection) and [Create the webhook](#create-the-webhook).
</Note>
