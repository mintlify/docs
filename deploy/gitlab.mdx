...
We use access tokens and webhooks to authenticate and sync changes between GitLab and Mintlify. This works with both GitLab.com and self-hosted GitLab instances.
...
<Steps>
  ...
  <Step title="Set up the connection">
    ...
    2. If you are using a self-hosted GitLab instance, enter your GitLab host URL (e.g., `https://gitlab.example.com`). The URL must use HTTPS.
    3. Complete any other required configurations.
    4. Click **Save Changes**.

    Mintlify validates your access token and repository access when you save, so you can confirm your connection is working immediately.
