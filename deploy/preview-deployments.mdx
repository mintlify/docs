...
## Features that behave differently in previews

Some features are scoped to production deployments and do not appear on branch previews:

- **[Related topics](/organize/related-pages)**: Both automatic and manual `related` frontmatter suggestions are skipped on previews because no search index is built for preview deployments. Verify configuration on production after merging.
