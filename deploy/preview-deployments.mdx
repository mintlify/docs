...
Preview deployments are served on two hosts:

- `<preview-subdomain>.mintlify.app`
- `<preview-subdomain>.mintlify.site`

The URL shown in the **Previews** tab of your [dashboard](https://app.mintlify.com/) is the authoritative preview URL. Use that URL when sharing a preview for review or when a customer reports an issue with a specific deployment.

The Mintlify GitHub bot may post a `.mintlify.app` link in the pull request timeline while the dashboard shows a `.mintlify.site` link for the same preview. Both hosts point at the same deployment, but a given preview is only served from one host at a time, so the other host can return a `404` for the same path. If a preview URL 404s, open the preview from the dashboard to get the current URL instead of editing the host by hand.
...
- **`ERR_TOO_MANY_REDIRECTS` or "dangerous site" warnings**: These symptoms usually mean the preview is being loaded on the wrong host. Open the preview from the **Previews** tab of your [dashboard](https://app.mintlify.com/) to get the authoritative URL (see [Preview URL format](#preview-url-format)) and load that URL directly instead of the link posted in a pull request comment.
