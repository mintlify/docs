...
5. Optionally, [password-protect the preview](#password-protected-previews).
...
You can password-protect individual manual preview deployments for sharing with external reviewers who aren't members of your Mintlify organization.

<Info>
  Password-protected previews require a plan that includes preview deployment authentication. This option is not available when your deployment already has [authentication](/deploy/authentication-setup) enabled.
</Info>

To create a password-protected preview:

1. Go to your [dashboard](https://dashboard.mintlify.com/).
2. Select **Previews**, then **Create custom preview**.
3. Enter the branch name.
4. Enable the **Make private** toggle.
5. Enter a password (minimum 8 characters).
6. Select **Create deployment**.

Anyone who visits the preview URL is prompted to enter the password before they can view the content.
