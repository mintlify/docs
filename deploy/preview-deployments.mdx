...
Automatic previews are not generated for pull requests opened from a fork. The Mintlify GitHub App is installed on your documentation repository and can only access repositories where it has been explicitly installed, so it cannot read the contributor's fork or build a preview against it.

To preview changes from a fork, a maintainer with write access to the main repository can push the contributor's branch to a branch in the main repository (or merge the fork into an integration branch). The GitHub App can then build a preview for a pull request opened from that branch.

<Note>
  Native preview support for pull requests from forks against public repositories is a tracked feature request. Follow [this GitHub discussion](https://github.com/orgs/mintlify/discussions/1724) for updates.
</Note>
