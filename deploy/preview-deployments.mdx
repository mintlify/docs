...
- **Automatic previews**: The preview for a pull request remains available while the PR is open and after it's merged or closed, as long as the source branch still exists. Deleting the branch removes the preview on the next dashboard sync.
...
Preview URLs are unique per branch. If you delete a preview and later re-create one for the same branch, Mintlify may issue a new URL.
