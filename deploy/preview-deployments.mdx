...
### Preview deployments with authenticated docs

If your production docs use [authentication](/deploy/authentication-setup) (SSO, OAuth, JWT, or password), preview deployments also require authentication. Visitors are redirected to a login page before they can view the preview.

This means:

- **SSO-protected docs**: Preview URLs redirect to the SSO login page. Only users who can authenticate through your identity provider can view previews.
- **OAuth or JWT-protected docs**: Preview URLs redirect to your configured login flow before granting access.
- **Password-protected docs**: Preview URLs prompt for the password before showing content.

<Note>
  If your organization has multiple deployments, each preview inherits the authentication settings of its corresponding deployment. A preview for a deployment with SSO enabled requires SSO login, while a preview for a public deployment is accessible without authentication.
</Note>

This behavior is separate from the **Preview authentication** toggle above, which restricts access to Mintlify organization members. When both are enabled, users must satisfy both authentication requirements.
