...
5. Optionally, enable **Password protection** and enter a password to restrict access to the preview.
...
By default, preview deployments are publicly accessible to anyone with the URL. You can restrict access using organization-level authentication or per-preview password protection.

### Organization authentication

Require all preview viewers to be authenticated members of your Mintlify organization.
...
You can set a password on individual manual preview deployments. When enabled, visitors must enter the password before they can view the preview content.

To add password protection, enable the **Password protection** toggle and enter a password when [creating a manual preview](#manual-previews). The password must be at least 8 characters long.
