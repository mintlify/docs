...
By default, preview deployments are publicly accessible to anyone with the URL. You can restrict access using organization-level authentication or password protection.

### Organization authentication

Restrict access to authenticated members of your Mintlify organization.
...
You can password-protect individual preview deployments. Anyone with the preview link must enter the password before viewing the content. This is useful for sharing previews with external reviewers who are not members of your Mintlify organization.

1. Go to your [dashboard](https://dashboard.mintlify.com/).
2. Select **Previews**.
3. Select **Create custom preview**.
4. Enter the branch name and set a password (minimum 8 characters).
5. Select **Create deployment**.

The password applies only to that specific preview deployment. Each preview can have its own password or no password at all.
