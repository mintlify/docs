---
title: "Cloudflare"
description: "Host documentation on a custom subpath with Cloudflare Workers."
keywords: ["Cloudflare Workers", "custom subpath", "reverse proxy setup", "Worker configuration"]
---
...
<Note>
  **For `/docs` subpath only**: You can use the simpler self-service option. Enable the "Host at `/docs`" toggle in your [dashboard](https://dashboard.mintlify.com/settings/deployment/custom-domain) and follow the [Vercel rewrites guide](/deploy/vercel#host-from-docs). The Cloudflare Worker setup below is only required for custom subpaths other than `/docs`.
</Note>
