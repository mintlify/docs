...
<Note>
  The example script only proxies documentation traffic. If you add the Worker as a custom domain, requests outside your subpath, `/mintlify-assets/*`, `/_mintlify/*`, and `/.well-known/*` are not handled. If your main site is served on the same domain, either scope the Worker to the documentation paths with route patterns or route all other traffic to your main site as shown in [Webflow custom routing](#webflow-custom-routing).
</Note>
