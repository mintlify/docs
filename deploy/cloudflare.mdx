...
<Warning>
  In addition to your subpath, your Worker must proxy `/mintlify-assets/*`, which serves the CSS, JavaScript, and favicons for your documentation, and `/_mintlify/*`. API playground and analytics requests are sent to `_mintlify` paths under your subpath, such as `/docs/_mintlify/`, which your subpath route already covers.
