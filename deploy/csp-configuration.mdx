...
- **HTTP resources:** Scripts, styles, fonts, and other resources must load over HTTPS. Connections to plain HTTP URLs, including local development servers such as `http://localhost:8545`, are blocked.
- **Blob and data scripts:** Scripts loaded from `blob:` or `data:` URLs are blocked. Some third-party tag loaders use this pattern.
- **Plugins:** `<object>` and `<embed>` elements are blocked.
