...
Editors and admins can roll back a deployment. The **Roll back** and **Promote** buttons do not appear for viewers.
...
<Steps>
  ...
  <Step title="Find the build you want to serve.">
    ...
    The most recent successful build does not have a **Roll back** button because your site already serves it.
  </Step>
...
- Projects with [authentication](/deploy/authentication-setup) enabled do not support rollbacks.
...
Your [audit logs](/dashboard/audit-logs) record rollbacks and promotions under the **Project** category, including who performed each action and when.
