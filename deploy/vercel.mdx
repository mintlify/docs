...
<Note>
  Base paths other than `/docs` require a [Pro or Enterprise plan](https://mintlify.com/pricing?ref=vercel).
</Note>

To use a custom subpath (any path other than `/docs`):

1. Navigate to [Custom domain setup](https://app.mintlify.com/settings/deployment/custom-domain) in your dashboard.
2. Select the **Subpath** hosting mode and enter your base path. For example, `/help`.
3. Choose **Proxy only** for traffic routing.
4. Enter your domain.
5. Click **Add domain**.
6. Use the generator below to create your rewrites configuration. Add the rewrites to your `vercel.json` file.
