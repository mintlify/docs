...
The `/docs` subpath is the only self-service option. You can enable it directly from your dashboard without additional infrastructure setup.

<Note>
  For any subpath other than `/docs` (e.g., `/help`, `/guides`), you must use a reverse proxy. See the [Cloudflare](/deploy/cloudflare) or [AWS](/deploy/route53-cloudfront) guides.
</Note>
