...
For GitHub Apps, you can choose to give permissions to only a single repository. It is highly recommended to do so as only access to the repository where docs content exists is required.
...
Similar to how the [CLI link checker](/create/broken-links#broken-links) works on your local machine, documentation content is automatically checked for broken links.
...
#### Configuration
If you have a `.vale.ini` file in the root content directory for your deployment, that configuration file is used. Any configuration files in your specified `stylesPath` are also used.
...
```text Example Vale file structure expandable
  - docs.json
  - .vale.ini
  - styles/...
  - text.md
...
```text Example monorepo Vale file structure expandable
  - main.ts
  - docs/
    - docs.json
    - .vale.ini
