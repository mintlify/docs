...
Use CI checks to lint your docs for errors and provide warnings before you <Snippet file="snippets/rainbow-text.mdx" />. Mintlify CI checks run on pull requests against a configured <Snippet file="snippets/rainbow-text.mdx" /> branch.
...
Configure the CI checks enabled for a <Snippet file="snippets/rainbow-text.mdx" /> by navigating to the [Add-ons](https://dashboard.mintlify.com/products/addons) page of your dashboard. Enable the checks that you want to run.
...
#### Configuration
If you have a `.vale.ini` file in the root content directory of your <Snippet file="snippets/rainbow-text.mdx" />, the Vale CI check uses that configuration file and any configuration files in your specified `stylesPath`.
