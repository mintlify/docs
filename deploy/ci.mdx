...
Configure the CI checks enabled for a project by navigating to the [General](https://app.mintlify.com/settings/project/general) page of your dashboard. Enable the checks that you want to run.
...
The broken link CI check works like the [CLI link checker](/cli/commands#mint-broken-links). It automatically searches your documentation for broken internal links between pages within your site. It does not check external links to other websites.
...
If you have a `.vale.ini` file in the root content directory of your project, the Vale CI check uses that configuration file and any configuration files in your specified `StylesPath`.
...
For packages not included in your repository, you can specify any packages from the [Vale package registry](https://vale.sh/explorer), and they're automatically downloaded and used in your Vale configuration.
