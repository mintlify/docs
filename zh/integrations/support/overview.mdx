...
将客服小部件集成到文档中。在 `docs.json` 文件中添加 `integrations` 字段，并填入相应的应用 ID。

```json
  "integrations": {
    "intercom": "应用标识（APP_ID）",
    "frontchat": "聊天标识（CHAT_ID）"
  }
```