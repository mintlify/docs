...
你可以在文档中接入数据隐私平台。在 `docs.json` 中添加 `integrations` 字段，并填入相应的脚本。
...
如果你需要为满足 GDPR 合规而检查用户是否已同意使用 Cookie，可以在 `cookies` 下指定一个本地存储的键和值：
...
如果设置了这些值，系统会检查本地存储以确认用户是否已同意使用 Cookie。若未同意，将禁用遥测。

如果你想为所有用户禁用遥测，可以在你的 `docs.json` 文档配置文件中添加以下内容：
