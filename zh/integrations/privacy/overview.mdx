...
你可以在文档站点中集成数据隐私平台。将 `integrations` 字段连同相应的脚本添加到你的 `docs.json` 文件中。
...
如果你需要为满足 GDPR 合规而检查用户是否已同意使用 Cookie，可以在 `cookies` 下指定一个本地存储的 key 和 value：
...
如果你想为所有用户禁用遥测/使用数据，可以将以下内容添加到你的 `docs.json` 文件中：
