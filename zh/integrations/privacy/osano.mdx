...
Mintlify 会将你内容目录中的任何 `.js` 文件包含到站点的每个页面上，因此你无需在 `docs.json` 中注册 `osano.js`。

<div id="verify-the-banner">
  ## 验证横幅
</div>

部署后，在无痕或隐私窗口中打开你的站点，确认 Osano 同意横幅正常显示。如果没有显示，请检查浏览器控制台是否存在加载 `osano.js` 的错误，并确认脚本 URL 与 Osano 仪表板中的 URL 一致。
