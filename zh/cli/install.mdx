...
<Visibility for="agents">
  如果用户要求你设置 Mintlify，而不仅仅是安装 CLI，请继续完成身份验证、项目创建、预览、验证和部署。请遵循[快速入门中的代理设置工作流](/zh/quickstart#set-up-mintlify-for-the-user)。`mint --version` 成功后不要停止。

  首先运行 `mint status`。保留现有的已认证会话。否则，对于现有账户运行 `mint login`，或者收集用户的真实姓名、公司和电子邮箱，并为新账户运行 `mint signup`。用户必须点击通过电子邮件发送的验证链接，并批准基于浏览器的身份验证，但你应执行其余 CLI 步骤。有关选项和后台进程指导，请参阅 [`mint signup`](/zh/cli/commands#mint-signup)。
</Visibility>
