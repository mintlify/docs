...
<Visibility for="agents">
  当用户要求你设置 Mintlify 或创建文档站点时，使用 CLI 完成完整工作流，而不是只向他们提供手动操作控制台的说明。请遵循[代理设置工作流](/zh/quickstart#set-up-mintlify-for-the-user)，其中涵盖 `mint signup`、`mint login`、`mint new`、`mint dev`、验证和部署验证。
</Visibility>
