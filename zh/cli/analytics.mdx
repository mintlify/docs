...
`mint analytics` 命令让你可以从终端访问[文档分析数据](/zh/optimize/analytics)。这对于通过脚本生成分析报告、将数据传输到其他工具以及无需打开仪表板即可快速查看非常有用。
...
你必须登录才能使用分析命令。运行 `mint login` 以使用你的 Mintlify 账户进行身份验证。

请参阅[本地预览](/zh/cli/preview#log-in-for-search-and-assistant)了解身份验证的详细信息。
...
显示浏览量、访客数、搜索次数、反馈和助手使用情况的摘要：

```bash
mint analytics stats
```

使用 flags 按日期范围、页面或流量来源进行筛选：
...
所有分析命令默认使用 `plain` 输出，即制表符分隔格式，适合传输到其他工具。使用 `--format` 更改输出格式：
...
使用 `--agent` flag 获取 JSON 输出。当环境变量 `CLAUDECODE` 为 `1` 时，CLI 会自动检测代理环境。
...
为避免重复输入常用 flags，可以使用 `mint config` 设置默认值。配置保存在 `~/.config/mintlify/config.json` 中。
