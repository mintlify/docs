...
不带任何 flag 运行以进行交互式注册 — CLI 会提示你输入名字、姓氏、公司名称和电子邮件，然后打开浏览器窗口以验证你的电子邮件。点击验证链接后，身份验证会自动完成，你的凭据保存在 `~/.config/mintlify/config.json` 中。注册完成后，你即已登录，可以运行 `mint status` 来确认会话。

传入上述 flag 可跳过提示并以非交互方式注册，这在代码代理代表你运行该命令时非常有用：

```bash
mint signup --firstName Ada --lastName Lovelace --company Analytical --email ada@example.com
```

注册完成后，访问 [Mintlify 控制台](https://dashboard.mintlify.com)以连接你的 GitHub 仓库并完成部署设置。
