...
将当前目录中的每个 `.mdx` 文件格式化为 Mintlify 的规范样式。该命令使用与 Web 编辑器相同的转换器解析每个文件，如果规范化输出与原文不同，则就地重写文件。格式化后的文件与 Web 编辑器发布时写入的输出完全一致，因此在已格式化的仓库中从编辑器发布时，diff 只包含被编辑的行。

```bash
mint format
```

在文档项目的根目录中运行该命令。它会遍历所有子目录，跳过 `.gitignore` 匹配的路径和任何 Mintlify 忽略规则匹配的路径。已经与规范化输出一致的文件将保持不变。如果格式化会移除某个文件的 frontmatter，该命令会保持该文件不变，并将其报告为失败。

<Warning>
  `mint format` 会就地重写文件。运行前请先提交或暂存你的更改，以便审查 diff。在现有项目上首次运行时，文件规范化可能产生一次性的大 diff，包括 frontmatter 引号、列表间距和注释缩进。此后再次运行时，已格式化的文件将保持不变。
</Warning>

命令完成后，会打印重新格式化了多少个 MDX 文件以及有多少文件失败。如果有任何文件失败，命令将以退出码 `1` 结束，并打印文件路径和错误信息，这样你就可以在 CI 中运行它以强制执行一致的格式。
