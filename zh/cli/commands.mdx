...
| `--threshold` | 当页面的 AI 撰写比例达到此值时判定为不通过，取值范围为 `0` 到 `1`。默认为 `0.5`。 |
...
每检查一个页面消耗 1 个 AI 积分。少于 50 个词的页面会被跳过，不计费。当任意页面达到或超过阈值时，命令以退出码 `1` 结束；当所有页面都通过时，以退出码 `0` 结束，方便你在改写循环或 CI 中运行。
