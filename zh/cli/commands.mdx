...
在文档目录中运行该命令。它会扫描页面中的围栏代码块，检查哪些页面包含可测试的示例，让编码代理为每个页面生成测试，运行生成的测试，并报告哪些页面通过或失败。
...
在交互式终端中，该命令会打开一个引导式界面，你可以在其中选择编码代理和要测试的页面，实时查看进度，并审查结果，包括预览生成的测试文件。如果之前的运行被中断，可以恢复它；历史运行也会被保存，方便你随时回看报告。

在非交互式环境（例如 CI）中，该命令使用 Claude 对自动选择的页面运行测试，打印通过和失败页面的摘要，并在所有测试通过时以退出码 `0` 结束，否则以 `1` 结束。
