---
title: "命令"
description: "Mintlify CLI 所有命令和选项的完整参考。"
keywords: ["CLI", "mint", "commands", "flags", "reference"]
---

<div id="global-flags">
## 全局选项
</div>

这些选项适用于所有命令。

| Flag | Description |
| --- | --- |
| `--telemetry`, `-t` | 启用或禁用匿名使用遥测。 |
| `--help`, `-h` | 显示命令帮助。 |
| `--version`, `-v` | 显示 CLI 版本。`mint version` 的别名。 |

<div id="mint-dev">
## `mint dev`
</div>

启动文档的本地预览。

```bash
mint dev [flags]
```

| Flag | Description |
| --- | --- |
| `--port` | 运行本地预览的端口。默认为 `3000`。 |
| `--no-open` | 不自动打开浏览器。 |
| `--groups` | 用逗号分隔的用户组列表，用于在预览中模拟。 |
| `--disable-openapi` | 跳过 OpenAPI 文件处理以提高性能。 |
| `--local-schema` | 允许通过 HTTP 提供的本地托管 OpenAPI 文件。 |

---

<div id="mint-login">
## `mint login`
</div>

使用你的 Mintlify 账户进行身份验证。

```bash
mint login
```

打开浏览器窗口完成身份验证。如果浏览器未打开，CLI 会显示一个需要手动打开的 URL 以及一个粘贴授权码的提示。凭据保存在 `~/.config/mintlify/config.json` 中。

---

<div id="mint-logout">
## `mint logout`
</div>

删除已存储的凭据。

```bash
mint logout
```

---

<div id="mint-status">
## `mint status`
</div>

显示与当前会话关联的账户和组织。

```bash
mint status
```

---

<div id="mint-analytics">
## `mint analytics`
</div>

查看文档的分析数据。需要使用 `mint login` 进行身份验证。

```bash
mint analytics <subcommand> [flags]
```

所有子命令接受以下共享选项：

| Flag | Description |
| --- | --- |
| `--subdomain` | 文档子域名。默认为使用 `mint config set subdomain` 设置的值。 |
| `--from` | 开始日期，格式为 `YYYY-MM-DD`。默认为 7 天前，或使用 `mint config set dateFrom` 设置的值。 |
| `--to` | 结束日期，格式为 `YYYY-MM-DD`。默认为今天，或使用 `mint config set dateTo` 设置的值。 |
| `--format` | 输出格式：`plain`（默认）、`table`、`json` 或 `graph`。 |
| `--agent` | 输出 JSON。等同于 `--format json`。当环境变量 `CLAUDECODE` 等于 `1` 时也会自动激活。 |

<div id="mint-analytics-stats">
### `mint analytics stats`
</div>

显示浏览量、访客、搜索、反馈和助手使用情况的摘要。

```bash
mint analytics stats [flags]
```

| Flag | Description |
| --- | --- |
| `--page` | 按特定页面路径筛选。 |
| `--humans` | 仅显示人类流量。 |
| `--agents` | 仅显示 AI 代理流量。 |

<div id="mint-analytics-search">
### `mint analytics search`
</div>

显示搜索查询及其结果数量和点击率。

```bash
mint analytics search [flags]
```

| Flag | Description |
| --- | --- |
| `--query` | 按搜索查询子字符串筛选。 |
| `--page` | 按点击最多的页面筛选。 |

<div id="mint-analytics-feedback">
### `mint analytics feedback`
</div>

显示用户提交的反馈。

```bash
mint analytics feedback [flags]
```

| Flag | Description |
| --- | --- |
| `--type` | 反馈类型：`page`（按页面聚合）或 `code`（代码片段反馈）。 |
| `--page` | 按特定页面路径筛选。 |

<div id="mint-analytics-conversation">
### `mint analytics conversation`
</div>

查看助手对话数据。

```bash
mint analytics conversation <subcommand> [flags]
```

<div id="mint-analytics-conversation-list">
#### `mint analytics conversation list`
</div>

列出助手对话。每个条目包含一个对话 ID。

```bash
mint analytics conversation list [flags]
```

| Flag | Description |
| --- | --- |
| `--page` | 筛选在来源中引用特定页面的对话。 |

<div id="mint-analytics-conversation-view">
#### `mint analytics conversation view <conversation-id>`
</div>

按 ID 查看单个对话。使用 `mint analytics conversation list` 获取 ID。

```bash
mint analytics conversation view <conversation-id>
```

<div id="mint-analytics-conversation-buckets-list">
#### `mint analytics conversation buckets list`
</div>

列出分组的对话类别。每个条目包含一个类别 ID。

```bash
mint analytics conversation buckets list
```

<div id="mint-analytics-conversation-buckets-view">
#### `mint analytics conversation buckets view <bucket-id>`
</div>

查看类别中的对话。使用 `mint analytics conversation buckets list` 获取 ID。

```bash
mint analytics conversation buckets view <bucket-id>
```

---

<div id="mint-config">
## `mint config`
</div>

管理 CLI 命令的持久默认值。配置保存在 `~/.config/mintlify/config.json` 中。

```bash
mint config <subcommand> <key> [value]
```

| Subcommand | Description |
| --- | --- |
| `set <key> <value>` | 设置配置值。 |
| `get <key>` | 显示配置值。 |
| `clear <key>` | 删除配置值。 |

<div id="configuration-keys">
### 配置键
</div>

| Key | Description | Used by |
| --- | --- | --- |
| `subdomain` | 默认文档子域名。 | `mint analytics` |
| `dateFrom` | 分析查询的默认开始日期（`YYYY-MM-DD`）。 | `mint analytics` |
| `dateTo` | 分析查询的默认结束日期（`YYYY-MM-DD`）。 | `mint analytics` |

---

<div id="mint-broken-links">
## `mint broken-links`
</div>

检查文档中的内部断链。

```bash
mint broken-links [flags]
```

该命令排除匹配 [.mintignore](/zh/organize/mintignore) 模式的文件。指向被忽略文件的链接将被报告为断链。

| Flag | Description |
| --- | --- |
| `--check-anchors` | 同时验证锚点链接（例如 `/page#section`）是否与标题 slug 匹配。 |
| `--check-external` | 同时检查外部 URL 是否存在断链。 |
| `--check-snippets` | 同时检查 `<Snippet>` 组件内的链接。 |

---

<div id="mint-a11y">
## `mint a11y`
</div>

检查文档中的无障碍问题。

```bash
mint a11y [flags]
```

检查颜色对比度和图片及视频上缺失的替代文本。

| Flag | Description |
| --- | --- |
| `--skip-contrast` | 跳过颜色对比度检查。 |
| `--skip-alt-text` | 跳过缺失替代文本检查。 |

---

<div id="mint-validate">
## `mint validate`
</div>

在严格模式下验证文档构建。如果存在任何警告或错误，将以错误退出。包括自动验证 `docs.json` 中引用的 OpenAPI 规范。

```bash
mint validate [flags]
```

| Flag | Description |
| --- | --- |
| `--groups` | 用逗号分隔的用户组列表，用于在验证中模拟。 |
| `--disable-openapi` | 跳过 OpenAPI 文件处理和验证。 |
| `--local-schema` | 允许验证通过 HTTP 提供的本地托管 OpenAPI 文件。在生产环境中仅支持 HTTPS。 |

<Note>
  独立命令 `mint openapi-check` 已弃用。请使用 `mint validate` 代替。
</Note>

---

<div id="mint-workflow">
## `mint workflow`
</div>

交互式创建[工作流](/zh/agent/workflows)文件。

```bash
mint workflow
```

CLI 会提示输入名称、触发器类型和其他设置，然后在 `.mintlify/workflows/` 中创建一个 `.md` 文件。

---

<div id="mint-export">
## `mint export`
</div>

将文档导出为自包含的 zip 归档文件，用于离线查看和分发。

```bash
mint export [flags]
```

| Flag | Description |
| --- | --- |
| `--output` | 输出文件名。默认为 `export.zip`。 |
| `--groups` | 用逗号分隔的用户组列表，用于包含受限页面。 |
| `--disable-openapi` | 跳过 OpenAPI 处理。 |

详情请参阅[离线导出](/zh/deploy/export)。

---

<div id="mint-new">
## `mint new`
</div>

从 Mintlify 入门模板创建新的文档项目。

```bash
mint new [directory] [flags]
```

| Flag | Description |
| --- | --- |
| `--name` | 项目名称。在交互模式下未提供时，CLI 会提示输入。 |
| `--theme` | 项目[主题](/zh/customize/themes)。在交互模式下未提供时，CLI 会提示输入。 |
| `--force` | 不提示直接覆盖目录。 |

---

<div id="mint-update">
## `mint update`
</div>

将 CLI 更新到最新版本。

```bash
mint update
```

---

<div id="mint-version">
## `mint version`
</div>

显示当前 CLI 和客户端版本。

```bash
mint version
```

---

<div id="coming-soon">
## 即将推出
</div>

这些命令可以运行但尚未正式可用。运行它们会通过 CLI 遥测记录你的兴趣，并帮助确定下一步发布的优先级。

| Command | Description |
| --- | --- |
| `mint ai` | AI 驱动的文档工具。 |
| `mint test` | 文档测试。 |
| `mint signup` | 从 CLI 注册账户。 |
| `mint mcp` | 文档 MCP 服务器。 |

---

<div id="telemetry">
## 遥测
</div>

CLI 收集匿名使用遥测数据以帮助改进 Mintlify。遥测数据包括命令名称、CLI 版本、操作系统和架构。Mintlify **不会**收集个人身份信息、项目内容或文件路径。

默认情况下，CLI 会收集遥测数据。你可以随时使用 `--telemetry` 选项退出：

```bash
# Disable telemetry
mint --telemetry false

# Re-enable telemetry
mint --telemetry true
```

你也可以通过设置以下环境变量之一来禁用遥测：

| Variable | Value | Description |
| --- | --- | --- |
| `MINTLIFY_TELEMETRY_DISABLED` | `1` | 禁用 Mintlify CLI 遥测。 |
| `DO_NOT_TRACK` | `1` | 使用 [Console Do Not Track](https://consoledonottrack.com/) 标准禁用遥测。 |

你的偏好保存在 `~/.config/mintlify/config.json` 中，并在 CLI 会话之间持续有效。
