...
| `llmsTxtFullExists` | 提供了 [llms-full.txt](/zh/ai/llmstxt/#llms-full-txt) 文件，供需要完整内容的代理使用。 |
...
| `responseLatency` | 站点在延迟阈值（2 秒）内响应。 |

子检查项仅在其父检查项通过时才会运行。例如，`llmsTxtValid` 仅在 `llmsTxtExists` 先通过后才会运行。
