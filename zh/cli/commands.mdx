...
| `openApiSpec` | 在标准路径下提供的任何 OpenAPI 或 Swagger 规范均有效。未提供规范时此检查通过，仅当提供的规范文件不是有效的 OpenAPI 或 Swagger 文档时才会失败。 |
| `robotsTxtAllowsAI` | `robots.txt` 文件没有阻止 AI 爬虫。 |
| `sitemapExists` | 有可用的站点地图供页面发现使用。 |
| `pagesHaveMarkdown` | 从站点地图中抽样的页面具有对应的 Markdown 版本（`path.md` 或 `path/index.md`），且提供非 HTML 内容。覆盖率达到 90% 或更高时通过。 |
