---
title: "Agent"
description: "使用 agent 实现文档更新自动化。可基于 Slack 消息、PR 或 API 调用创建拉取请求。"
keywords: ["automation", "automate", "Slack integration", "maintenance"]
---

<Info>
  任何拥有你控制台访问权限的成员都可使用 agent，功能适用于[专业版和自定义方案](https://mintlify.com/pricing?ref=agent)。
</Info>

agent 会根据你的提示为文档生成修改建议并创建拉取请求。你向 agent 发出请求后，它会参考你的文档、已连接的代码仓库以及 Slack 消息，生成符合技术写作最佳实践并遵循 Mintlify 架构的内容。你可以在 Slack 工作区中使用 agent，或通过 API 将其嵌入自定义应用。
