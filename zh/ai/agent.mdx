---
title: "Agent"
description: "用 Agent 自动化更新文档。可通过 Slack 消息、PR 或 API 调用创建拉取请求（PR）。"
---

<Info>
  Agent 适用于拥有你控制台访问权限的用户，并在[Pro 和 Custom 方案](https://mintlify.com/pricing?ref=agent)中提供。
</Info>

Agent 会根据你的提示为文档创建包含建议更改的拉取请求（PR）。当你向 Agent 发送请求时，它会参考你的文档、已连接的代码仓库以及 Slack 消息，生成符合技术写作最佳实践并遵循 Mintlify 模式的内容。你可以在 Slack 工作区中使用 Agent，或通过 API 将其嵌入自定义应用。

你可以用 Agent 来：

* 基于你的提示、PR 链接或 Slack 线程撰写新内容
* 修订过时的代码示例和 API 参考
* 搜索并更新现有内容
* 回答关于你的文档和技术写作的相关问题

开始使用：将 Agent 添加到你的 Slack 工作区，并在频道中通过 `@mintlify` 提及它。
