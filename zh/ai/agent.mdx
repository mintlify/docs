...
在控制台中，可以通过键盘快捷键 <kbd>⌘</kbd>+<kbd>I</kbd>（macOS）或 <kbd>Ctrl</kbd>+<kbd>I</kbd>（Windows/Linux），或者点击 **Ask agent** 按钮来打开 agent。桌面端可以调整 agent 面板大小；在移动设备上，agent 会以全屏方式打开。

agent 面板包含三种视图：

- **Chat**：向 agent 发送指令以更新文档。agent 会根据你的指令创建拉取请求（PR；亦称“合并请求”/Merge Request），并显示链接，方便你查看这些拉取请求或在 Web 编辑器中打开变更。
- **History**：浏览历史对话，并继续处理之前的请求。点击任一对话即可在 Chat 视图中打开。
- **Settings**：配置 agent 的集成以及对存储库的访问权限。
...
2. 在 Slack 集成部分，点击 **Connect**。
3. 按照 Slack 的提示将 `mintlify` 应用添加到你的工作区。
4. 按照 Slack 的提示将你的 Mintlify 账号关联到你的 Slack 工作区。
5. 测试 agent 是否正常工作并响应以下操作：
   - 向它发送一条私信。
   - 在频道中通过 `@mintlify` 提及它。
...
通过 API 创建任务时，你可以使用 `asDraft` 参数（默认为 `true`）来控制是否以草稿模式创建拉取请求（PR；亦称“合并请求”/Merge Request）。将 `asDraft` 设置为 `false` 可创建非草稿的拉取请求，以便在自动化工作流中立即进行审核和合并。
...
请参阅教程：[当代码合并时自动更新文档](/guides/automate-agent) 了解如何操作。