---
title: "Agent"
description: "使用 agent 自动完成文档更新。可基于 Slack 消息、拉取请求（PR）或 API 调用创建拉取请求。"
keywords: ["automation", "automate", "Slack integration", "maintenance"]
---

<Info>
  agent 在 [Pro 和 Custom 套餐](https://mintlify.com/pricing?ref=agent) 中提供，所有有权访问你控制台的成员都可以使用。
</Info>

agent 会根据你的提示为文档创建包含建议修改的拉取请求（PR）。当你向 agent 发送请求时，它会基于你的文档、已连接的代码仓库以及 Slack 消息，生成符合技术写作最佳实践并遵循 Mintlify schema 的内容。你可以在控制台中直接使用 agent，在 Slack 工作区中调用它，或者通过 API 将它嵌入自定义应用中。

你可以使用 agent 来：

* 基于你的提示、拉取请求链接或 Slack 讨论串撰写新内容
* 修改过时的代码示例和 API 参考文档
* 搜索并更新现有内容
* 回答与你的文档和技术写作相关的问题
...
你可以在控制台中通过键盘快捷键 <kbd>⌘</kbd>+<kbd>I</kbd>（Mac）或 <kbd>Ctrl</kbd>+<kbd>I</kbd>（Windows/Linux）直接打开 agent 面板，或者点击 **Ask agent** 按钮。

agent 面板包括：

* **Chat**：发送指令并接收带有文档修改建议的回复。
* **History**：查看并继续之前的对话。
* **Settings**：配置 GitHub 应用权限和 Slack 集成。

当 agent 进行更改时，你可以直接在对话中查看拉取请求（PR），或在 Web 编辑器中打开这些更改。

<div id="add-the-agent-to-your-slack-workspace">
  ## 将助手添加到你的 Slack 工作区
</div>

你也可以在 Slack 中使用助手，与团队协作完成文档更新。

<Note>
  如果你的 Slack 工作区所有者要求对应用安装进行管理员审批，请先请他们批准 Mintlify 应用，然后再进行连接。
</Note>

1. 在控制台中打开助手面板并前往 **Settings**。
2. 在 Slack 集成部分点击 **Connect** 按钮。
3. 按照 Slack 的提示将 `mintlify` 应用添加到你的工作区。
4. 按照 Slack 的提示将你的 Mintlify 账户关联到你的 Slack 工作区。
5. 测试助手是否正常工作，并在以下情况下会进行响应：
   * 向它发送一条私信。
   * 在频道中通过 `@mintlify` 提及它。
...
agent 只能访问你通过 Mintlify GitHub 应用连接的代码仓库。你可以在 agent 面板的 **Settings** 中，或者在 [GitHub App settings](https://github.com/apps/mintlify/installations/new) 中配置 agent 可访问的代码仓库。
...
将代理集成到你现有的自动化工具中，在代码变更时自动更新文档、触发内容评审，或在多个代码库之间同步文档更新。

请参阅[代码合并后自动更新文档](/zh/guides/automate-agent)教程了解具体方法。