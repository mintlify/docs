...
在你的存储库中添加一个 `AGENTS.md` 文件，以自定义 agent 的行为。agent 会读取此文件并遵循你提供的所有指令。
...
代码示例始终使用 TypeScript,不要使用 JavaScript。
...
无论你是在控制台、Slack 中使用该 agent，还是通过 API 调用它，这些指令都会应用到所有任务。
