...
| **下载 PDF** | `download-pdf` | 将当前页面下载为 PDF。适用于 [Enterprise 套餐](https://mintlify.com/pricing)。 |
...
| **下载 API 规范** | `download-spec` | 下载你的部署的 OpenAPI 规范。如果有多个规范，则以 zip 压缩包形式下载。仅在 API 参考页面上显示。 |
...
在你的 `docs.json` 文件中添加 `contextual` 字段，并指定要包含的选项。选项按你在数组中列出的顺序在菜单中显示。
...
`docs.json` 中相同的字段和校验规则同样适用，包括[自定义选项](#add-custom-options)以及 `display` 设置。如果页面级的覆盖配置无效，Mintlify 将回退到全局的 `contextual` 配置。
