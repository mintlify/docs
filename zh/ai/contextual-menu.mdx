...
上下文菜单可快速访问经AI优化的内容，并直接对接主流AI工具。用户在任意页面打开上下文菜单时，可以将内容复制为AI工具的上下文，或在 ChatGPT、Claude、Perplexity，或你自定义的工具中发起对话，并自动加载你的文档作为上下文。
...
上下文菜单包含多个预设选项，你可以在配置中添加它们的标识符以启用。

| 选项 | 标识符 | 描述 |
|:--------|:------------|:-------------|
| **复制页面** | `copy` | 将当前页面复制为 Markdown，便于作为上下文粘贴到 AI 工具 |
| **以 Markdown 查看** | `view` | 以 Markdown 形式打开当前页面 |
| **在 ChatGPT 中打开** | `chatgpt` | 以当前页面为上下文新建一段 ChatGPT 对话 |
| **在 Claude 中打开** | `claude` | 以当前页面为上下文新建一段 Claude 对话 |
| **在 Perplexity 中打开** | `perplexity` | 以当前页面为上下文新建一段 Perplexity 对话 |
...
通过向 `options` 数组添加一个对象，可在上下文菜单中创建自定义选项。每个自定义选项需要以下属性：
...
```json {9-14} 换行
{
    "contextual": {
        "options": [
            ...
            {
                "title": "功能请求",
                "description": "前往 GitHub 参与讨论并提出你的功能需求",
                "icon": "plus"
