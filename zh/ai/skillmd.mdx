...
[skill.md 规范](https://agentskills.io/specification) 是一种结构化、机器可读的格式，它将产品的能力、必填输入以及约束条件显式列出。这样，agent 就能更可靠地使用这些产品。
...
当用户连接到你的 Model Context Protocol (MCP) 服务器时，他们的代理可以将你的 `skill.md` 文件作为 [MCP 资源](/zh/ai/model-context-protocol#mcp-resources)来发现和使用，无需单独安装这些 skills。
...
<Prompt description="将某个产品的 skill.md 文件安装到你的代理中。" actions={["copy", "cursor"]}>
将某个 Mintlify 托管的文档站点的 `skill.md` 文件安装到我的代理上下文中，让你了解该产品可以做什么。

1. 如果我尚未提供文档站点的 URL，请先向我索取，然后使用该 URL 运行 `npx skills add <url>`。
2. 打印出已安装的 skills 列表，包括每个 skill 的名称和描述。
3. 用一段话总结关键能力，让我了解你现在可以帮我用这个产品完成什么。
4. 如果任何 skill 在其 frontmatter 中声明了必需的输入或约束条件，请在我要求你使用它之前先指出来。
</Prompt>
