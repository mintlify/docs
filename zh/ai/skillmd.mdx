...
<Note>
  如果你使用 [反向代理](/zh/deploy/reverse-proxy)，请将其配置为将对 `/skill.md` 和 `/.well-known/skills/*` 路径的请求转发到你的 Mintlify 子域。
</Note>
...
Mintlify 在 `/.well-known/skills/` 托管了一个 skills 目录，代理可以使用它来以编程方式发现和获取你的 skill 文件。

<div id="skills-index">
  ### Skills 索引
</div>

`GET /.well-known/skills/index.json` 返回一个列出所有可用 skills 的 JSON 清单：
...
`name` 字段是从你的 `skill.md` frontmatter 中 `name` 字段派生的 URL 安全 slug。

<div id="individual-skill-files">
  ### 单个 skill 文件
</div>

`GET /.well-known/skills/{name}/skill.md` 返回索引中通过 slugified 名称标识的特定 skill 的 `skill.md` 文件。
