...
Mintlify 在 `/.well-known/skills/` 托管一个 skills 目录，代理可以用它以编程方式发现和获取你的 skill 文件。
...
`name` 字段是从你 `skill.md` frontmatter 中的 `name` 字段派生的 URL 安全 slug。

### 单个 skill 文件

`GET /.well-known/skills/{name}/skill.md` 返回通过索引中的 slugified 名称标识的特定 skill 的 `skill.md` 文件。
