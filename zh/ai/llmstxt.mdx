...
<Note>
  `llms.txt` 和 `llms-full.txt` 的行为取决于你的认证配置：

  - **公开站点**：两个文件均可公开访问，并包含所有页面。
  - **部分认证的站点**（至少包含一个 `public: true` 页面的站点）：两个文件均可公开访问，但仅列出并包含公开页面的内容。OpenAPI 和 AsyncAPI 规范会被排除。
  - **完全认证的站点**（没有任何公开页面）：`llms.txt` 仅向已认证用户提供，并会排除限制在[用户分组](/zh/deploy/authentication-setup#control-access-with-groups)中的页面，以及 OpenAPI 和 AsyncAPI 规范。`llms-full.txt` 不可用。
