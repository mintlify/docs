---
title: "llms.txt"
description: "自动生成 llms.txt 和 llms-full.txt 文件，帮助 AI 工具更高效地索引并理解你的文档。"
...
每个页面的说明来自其 frontmatter 中的 `description` 字段。超过 300 个字符的说明会被截断至前 300 个字符。对于 API 参考页面，说明中还会包含来自 `openapi` 或 `api` frontmatter 字段的规范信息。没有 `description` 字段的页面将在 `llms.txt` 中显示，但不含说明。
