---
title: "llms.txt"
description: "为 LLM 优化文档，提升其可读性与索引效果。"
...
<Note>
  如果你的站点在访问文档时需要认证，你的 `llms.txt` 和 `llms-full.txt` 文件将不会对 LLM 和 AI 工具开放。LLM 和 AI 工具只能索引公开可访问的内容。
</Note>

将 `/llms.txt` 追加到你的文档站点 URL 后查看 `llms.txt`。
...
* **API 规范链接**，指向你的 OpenAPI 和 AsyncAPI 规范（如已配置）。
...
如果你的项目包含 [OpenAPI](/zh/api-playground/openapi-setup) 或 [AsyncAPI](/zh/api-playground/asyncapi-setup) 规范，Mintlify 会自动在你的 `llms.txt` 文件中包含这些规范的链接。这使 LLM 能够访问你完整的 API 架构，从而对你的 API 端点、参数和数据类型提供更准确的响应。

规范文件按文件名去重，当多个文件在不同目录中共享相同名称时，优先选择路径最短的文件。
