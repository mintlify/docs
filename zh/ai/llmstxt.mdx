...
<Note>
  `llms.txt` 和 `llms-full.txt` 的行为取决于你的站点配置：

  - **公开站点**：两个文件均可公开访问，并列出文档中的所有页面。
  - **部分需认证的站点**（部分页面公开）：两个文件均可公开访问，仅列出公开页面。
  - **完全需认证的站点**：`llms.txt` 返回 404。已认证的最终用户可以获取 `llms-full.txt` 作为上下文，该文件会省略所有限定给特定[用户分组](/zh/deploy/authentication-setup#control-access-with-groups)或在搜索中被隐藏的页面。

  在需要认证的站点上，OpenAPI 和 AsyncAPI 规范链接会从 `llms.txt` 中省略。
