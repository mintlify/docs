...
<Note>
  `llms.txt` 和 `llms-full.txt` 的服务方式取决于你的认证设置：

  - **公开站点**：两个文件均公开可用，且包含所有页面。
  - **部分认证站点** (某些页面标记为 [`public`](/zh/deploy/authentication-setup#make-pages-public))：两个文件均无需认证即可公开访问，但仅包含公开页面。OpenAPI 和 AsyncAPI 规范会被省略。
  - **完全认证站点** (没有公开页面)：`llms.txt` 仅在认证后才会被提供，并会排除任何属于[用户分组](/zh/deploy/authentication-setup#control-access-with-groups)的页面。`llms-full.txt` 返回 404，无法访问。`llms.txt` 中也会省略 OpenAPI 和 AsyncAPI 规范。

  无法通过你站点认证的 LLM 和 AI 工具将无法访问受保护的内容。
