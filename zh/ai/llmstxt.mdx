...
在不同场景下使用 `llms.txt` 和 `llms-full.txt`：

- **`llms.txt`** 是文档的轻量级索引。AI 工具用它来发现有哪些页面，并仅获取与用户查询相关的页面。适用于能够跟随链接、希望尽量减少 context 用量的代理。
- **`llms-full.txt`** 是所有页面的完整文本合并到单个文件中。适用于希望在一次请求中将全部文档加载到 AI 工具 context 的场景，例如用于评估、离线分析，或用于不跟随链接的工具。
