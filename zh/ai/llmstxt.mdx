...
<Note>
  认证会影响 `llms.txt` 和 `llms-full.txt` 的提供方式：

  - **完全认证的站点**：查看这两个文件都需要认证。无法通过你站点认证的 LLM 和 AI 工具将无法访问这些文件。
  - **部分认证的站点**：无需认证即可公开访问这两个文件，但其内容仅限于你的公开页面。属于[用户分组](/zh/deploy/authentication-setup#control-access-with-groups)的页面会被排除。
