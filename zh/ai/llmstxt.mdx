...
每个页面的说明来自其 frontmatter 中的 `description` 字段。说明会在 300 个字符处以及首个换行符处截断。对于 API 参考页面，说明中还会包含来自 `openapi` 或 `api` frontmatter 字段的规范信息。没有 `description` 字段的页面将在 `llms.txt` 中显示，但不含说明。
