...
<Warning>
  如果你的站点需要[认证](/zh/deploy/authentication-setup)，`llms.txt` 和 `llms-full.txt` 将在认证后才能访问。大多数 AI 工具和 LLM 无法通过你站点的认证，因此无法访问这些文件。这意味着 ChatGPT、Claude 和 Cursor 等 AI 工具将无法发现或索引你的文档。

  无法在已认证的站点上将 `llms.txt` 或 `llms-full.txt` 设为公开。如果你需要 AI 工具索引你的文档，请考虑使用[部分认证](/zh/deploy/authentication-setup#make-pages-public)来保持部分页面公开，而不是对整个站点进行认证。

  这些文件也会排除属于[用户分组](/zh/deploy/authentication-setup#control-access-with-groups)的页面。
</Warning>
