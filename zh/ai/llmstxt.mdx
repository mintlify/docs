...
Mintlify 会在每个页面的响应中添加 HTTP 头信息，以便 AI 工具无需预先了解文件位置即可发现你的 `llms.txt` 文件：

- `Link: </llms.txt>; rel="llms-txt", </llms-full.txt>; rel="llms-full-txt"`：遵循标准 HTTP `Link` 头格式，用于资源发现。
- `X-Llms-Txt: /llms.txt`：一个便捷的头信息，供检查 `llms.txt` 支持的工具使用。
