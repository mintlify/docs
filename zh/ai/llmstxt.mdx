...
<Note>
  自动生成的 `llms.txt` 文件上限为 100,000 个字符。当文件接近此限制时，Mintlify 会先缩短页面说明，然后再删除条目，并附加一条说明列出被省略的内容。此限制不适用于 `llms-full.txt`。若要不经截断列出所有页面，请在项目根目录添加一个[自定义 `llms.txt` 文件](#custom-files)。
</Note>
