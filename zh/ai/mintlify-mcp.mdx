...
- **`read`**: 获取会话 branch 上任意页面的完整 MDX。可以传入页面路径（如 `/quickstart`），也可以传入编辑器 URL 中的页面 ID（`~/` 之后的部分），让 AI 工具直接通过编辑器链接打开页面。要读取[私有页面](/zh/editor/pages#private-pages)，请传入通过 `list_nodes` 加 `visibility: "private"` 获取的 `private-page-<uuid>` 节点 ID。私有读取无需 checkout，但需要 OAuth 会话（客户端令牌和机器到机器令牌会被拒绝）。
