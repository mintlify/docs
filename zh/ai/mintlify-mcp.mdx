...
将任意 MCP 客户端（例如 Claude、Claude Code、ChatGPT 或 Cursor）连接到管理员 MCP 服务器，即可使用与编写代码相同的工具协同处理你的 Mintlify 内容和设置。使用管理员 MCP 服务器时，所有更改都会发生在某个分支上，并需要通过拉取请求合并。如果你的组织有多个部署，单个管理员 MCP 连接即可访问所有这些部署并在它们之间切换。
...
| **权限** | 读取、编辑、调整结构、保存、创建工作流、管理设置 | 读取并搜索一个站点的已发布页面 | 读取并搜索所有 Mintlify 站点 |
| **端点** | `https://mcp.mintlify.com` | 你的站点域名上的 `/mcp` | `https://index.mintlify.com` |
...
- **Git 提供方访问权限**：为该项目安装的 Mintlify GitHub App 或 GitLab 连接必须对部署分支所在的仓库具有写入权限。`save` 会通过与常规部署相同的集成打开 PR。
...
每个管理员 MCP 会话都绑定到单个 Git 分支。流程如下：
...
<Steps>
  ...
  <Step title="检出分支">
    首次必需的调用是 `checkout {subdomain}`。它会从该部署的部署分支基础上创建一个全新的 `mintlify-mcp/<slug>-<sha>` 分支（或附加到你指定的现有分支），并返回一个 `editorUrl`，你可以打开它在控制台编辑器中实时跟进。

    如果你需要发现或筛选某个部署仓库中现有的分支，请在 `checkout` 之前调用 `list_branches`。
  </Step>
  <Step title="读取、搜索和编辑">
    AI 使用 `search`、`read`、`list_nodes`、`edit_page`、`write_page`、`create_node` 和 `update_config` 等工具进行更改。所有编辑都会实时缓冲在会话分支上，目前还不会影响你的部署分支。
  </Step>
  <Step title="查看差异">
    随时调用 `diff` 查看与 `main` 相比发生了哪些更改。在控制台中打开 `editorUrl`，可以看到相同更改的渲染效果。
  </Step>
  <Step title="保存">
    调用 `save` 将分支推送到 Git。`mode: "auto"`（默认值）会创建一个拉取请求；如果该部署的 agent review 设置为 push-to-main 且部署分支未受保护，则会立即将其合并（响应中包含 `merged: true`）。使用 `mode: "pr"` 始终创建拉取请求并保持其打开以供审查，或使用 `mode: "commit"` 直接推送到现有的 PR 分支，而不打开新的 PR。
  </Step>
  <Step title="必要时丢弃">
    调用 `discard_session` 丢弃所有会话内更改并释放该分支。
  </Step>
</Steps>

<Tip>
  如果你的连接可以访问多个部署，每个已 checkout 的部署都会同时在内存中保留其各自的会话和分支。

  使用不同的 `subdomain` 或分支再次调用 `checkout` 会切换当前活动的会话，而不会丢弃其他会话。若要放弃进行中的草稿而不仅仅是切换离开它，请调用 `discard_session`。
</Tip>
...
- **`checkout`**: 将某个会话绑定到给定部署 `subdomain` 对应的分支，或切换当前活动的部署会话。
- **`list_branches`**: 列出某个部署项目可用的 Git 分支，可选用 `query` 进行筛选。返回分支名称、总数以及部署分支。在 `checkout` 之前调用此项，可按名称附加到现有分支。
- **`get_session_state`**: 查看当前分支、已编辑的文件和待处理的导航差异。
- **`diff`**: 列出会话与 `main` 之间的所有更改。
- **`save`**: 打开拉取请求或提交到会话分支。当部署配置为将 agent 更改推送到 main 且部署分支未受保护时，会自动合并该 PR。
...
- _“检出一个名为 `add-billing-faq` 的分支，并在 FAQ 组下创建一个名为 'Billing' 的新页面。为这个 Linear 工单中的五个问题草拟答案。”_
...
<AccordionGroup>
  ...
  <Accordion title="使用 slug 作为分支名称">
    向 `checkout` 传入 `slug`（例如 `add-quickstart`），使自动生成的分支易于阅读。如果不传入，分支名称会基于会话令牌生成，在你的仓库中很难辨识。
  </Accordion>
...
<Note>
  会话在 Mintlify 端会持有一个内存中的分支。如果你在没有保存或丢弃的情况下放弃会话，该分支会一直保留到你下一次 checkout 覆盖它为止。请避免在你的仓库中留下陈旧的 `mintlify-mcp/*` 分支，并定期清理它们。
</Note>
