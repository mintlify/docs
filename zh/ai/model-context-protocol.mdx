...
AI 应用会根据用户 query 的上下文来决定何时应用这些筛选条件。例如，如果用户询问特定 API 版本，AI 应用可能会自动应用相应的筛选条件，以提供更相关的结果。
...
### 使用 OpenAPI 过滤 API 端点

如果你的文档包含 OpenAPI 规范，默认情况下所有 API 操作都会作为 MCP 工具暴露。你可以使用 OpenAPI 规范中的 [`x-mint: mcp` 扩展](/api-playground/openapi-setup#mcp)来控制哪些操作可用，并自定义它们的工具名称和描述。
