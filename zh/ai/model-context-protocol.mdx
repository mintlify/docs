...
你的 MCP 服务器会向 AI 应用提供搜索文档和浏览文档页面只读文件系统的工具。你的用户必须将你的 MCP 服务器连接到他们的工具中。

<div id="how-mcp-servers-work">
  ### MCP 服务器的工作方式
</div>

当某个 AI 应用接入你的文档 MCP 服务器后，它可以直接根据用户提示搜索你的文档并浏览文档文件，而不是依赖其训练数据中的信息或执行通用的网页搜索。你的 MCP 服务器会提供对文档站点上所有已建立索引内容的访问权限。
...
- **Search**：在你的文档中搜索相关内容，返回带有标题和链接的摘要片段。适用于广泛或概念性的查询。
- **Query docs filesystem**：对包含文档页面（`.mdx` 文件）和 OpenAPI 规范的虚拟文件系统运行只读 shell 命令。AI 代理使用此工具来阅读完整页面（`head -80 /quickstart.mdx`）、浏览文档结构（`tree / -L 2`）、搜索精确关键词（`rg -il "rate limit" /`），以及在一次调用中阅读多个页面。每次调用都是无状态的，不会写入任何数据。

AI 应用会根据对话的上下文来决定何时使用每个工具。例如，AI 应用可能会先搜索你的文档以查找相关页面，然后使用 query docs filesystem 工具阅读最相关结果的完整内容。
...
- **`command`**：对虚拟文档文件系统运行的 shell 命令。例如，`head -80 /api-reference/create-customer.mdx` 用于阅读页面，`tree / -L 2` 用于列出目录结构，或 `rg -il "keyword" /` 用于搜索文件内容。
