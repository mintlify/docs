...
* 对于部分需要认证的文档，也就是某些页面公开、其他页面需要登录的情况，你必须先启用你的 MCP 服务器，用户才能访问它。用户必须先完成认证，然后才能连接到你的 MCP 服务器。完成认证后，用户可以根据其 [user groups](/zh/deploy/authentication-setup) 搜索他们有权访问的所有内容。
...
如果你的文档采用部分认证，同时包含公开页面和受保护页面，用户必须先完成认证，然后才能连接到你的 MCP 服务器。完成认证后，用户可以访问他们有权查看的内容。
