...
当某个 AI 应用接入你的文档 MCP 服务器后，它可以在响应用户提示时直接搜索你的文档，而不是依赖其训练数据中的信息或执行通用的网页搜索。你的 MCP 服务器会提供对文档站点上所有已建立索引内容的访问权限。

* AI 应用可以在生成回复时主动搜索你的文档，即使用户没有明确要求在你的文档中搜索答案。
* AI 应用会根据对话的 context 以及你的文档与当前话题的相关性来决定何时使用搜索工具。
* 每次搜索（也称为工具调用）都发生在生成过程中，因此 AI 应用会从你的文档中检索最新信息来生成回复。
...
MCP 搜索工具支持可选的筛选参数，AI 应用使用这些参数来缩小搜索结果范围。
...
* 对于公开文档，你的 MCP 服务器对任何人开放，可搜索所有已索引的公开页面。
* 对于部分认证的文档（即部分页面公开、部分页面受保护），你必须先启用 MCP 服务器，用户才能使用。未认证用户可以搜索公开内容。已认证用户可以搜索其根据[用户组](/zh/deploy/authentication-setup)有权访问的所有内容。
* 对于所有页面都需要认证的文档，你必须先启用 MCP 服务器，用户才能使用。用户在连接到你的 MCP 服务器之前必须先完成认证。你的 MCP 服务器只搜索每位用户根据其[用户组](/zh/deploy/authentication-setup)有权访问的内容。
...
如果你的文档需要认证，你的 MCP 服务器会要求用户在连接之前完成认证。当用户将你的 MCP 服务器 URL 添加到其 AI 工具时，他们必须使用现有凭据登录。认证完成后，重定向会将他们送回其工具。MCP 服务器仅返回每位用户根据其[用户组](/zh/deploy/authentication-setup)有权访问的内容。

如果你的文档采用部分认证，同时包含公开页面和受保护页面，用户可以在不认证的情况下连接到你的 MCP 服务器以访问公开内容。已认证的用户可以获得对其可访问的受保护内容的权限。

默认情况下，你的 MCP 服务器仅对本地主机工具可用。要允许基于 Web 的工具连接，请添加 AI 工具的重定向域。重定向域是 AI 工具在用户完成认证后使用的主机名，例如 `claude.ai` 或 `app.cursor.ai`。除非用户 AI 工具的重定向域在此列表中，否则它们无法完成认证。

<Steps>
  <Step title="在控制台中启用认证 MCP">
    1. 导航到控制台中的 [MCP 服务器页面](https://dashboard.mintlify.com/products/mcp)。
    2. 点击 **Enable MCP Server** 开关。
  </Step>
  <Step title="添加重定向域">
    添加你希望授予用户访问权限的 AI 工具的重定向域。除非用户 AI 工具的重定向域在此列表中，否则它们无法完成认证。常见的重定向域包括 `claude.ai` 和 `vscode.dev/redirect`。
...
对于需要认证的文档，你的 MCP 服务器只索引[公开用户组](/zh/deploy/authentication-setup#make-pages-public)中的页面。对于部分认证的文档，你的 MCP 服务器会为未认证用户索引公开页面，以及公开用户组中的所有页面。
