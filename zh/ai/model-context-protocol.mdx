...
* **搜索**：在你的文档中搜索相关信息、代码示例、API 参考和指南。返回带有标题和文档页面直接链接的上下文片段。
* **获取页面**：通过路径获取特定文档页面的完整内容。当 AI 已经知道页面路径（例如从搜索结果中获取）并需要完整内容而非片段时，可使用此工具。

AI 应用会根据对话的上下文来决定使用哪个工具。通常，AI 会先搜索以找到相关页面，然后在需要时使用获取页面工具来获取特定页面的完整内容。

<div id="search-parameters">
  ### 搜索参数
</div>

搜索工具支持可选参数，AI 应用使用它们来控制和优化搜索结果。
...
| 每个文档站点 (domain) | 每小时 10,000 个请求 | 限制所有用户在你的 MCP 服务器上的总搜索次数。 |
