---
title: "搜索 Model Context Protocol (MCP) 服务器"
sidebarTitle: "搜索 MCP"
description: "将 Claude、Cursor、ChatGPT 等 AI 工具连接到托管的搜索 MCP 服务器，让它们能够搜索并检索你站点上的内容。"
keywords: ["MCP", "AI 工具", "Claude", "Cursor", "搜索"]
---
...
<Tip>
  若要让受信任的代理编辑你的内容，请使用[管理员 MCP 服务器](/zh/ai/mintlify-mcp)。若要为编码代理提供一个跨所有 Mintlify 站点的检索来源，请使用[Mintlify Index](/zh/search-index)。
</Tip>
...
- **Submit feedback**：当代理发现某个页面不正确、过时、令人困惑或不完整时，将文档问题反馈给你的团队。代理会提供页面路径和问题描述，Mintlify 会将其记录为分析仪表板中的负面反馈。

代理会根据对话的上下文来决定何时使用每个工具。例如，代理可能会先搜索你的文档以查找相关页面，然后使用 query docs filesystem 工具读取最相关结果的完整内容。如果它在过程中发现页面存在问题，可以提交反馈。
...
如果你的文档启用了[身份验证](/zh/deploy/authentication-setup)，带有 `groups` 字段的技能仅会向组匹配的已认证终端用户开放。参见[将技能限制为特定用户组](/zh/ai/skillmd#restrict-skills-to-user-groups)。
...
* 对于部分需要认证的文档，也就是某些页面公开、其他页面需要登录的情况，你必须先启用你的 MCP 服务器，用户才能访问它。未认证的用户可以搜索公开内容。已完成认证的用户可以根据其[用户组](/zh/deploy/authentication-setup#control-access-with-groups)搜索他们有权访问的所有内容。
* 对于所有页面都需要认证的文档，你必须先启用你的 MCP 服务器，用户才能使用它。用户必须先完成认证，然后才能连接到你的 MCP 服务器。你的 MCP 服务器只会根据每个用户有权访问的[用户组](/zh/deploy/authentication-setup#control-access-with-groups)搜索相应内容。

如果启用了认证，请在控制台的 **Settings → Security & access → MCP** 中配置你的托管 Search MCP 服务器。已认证的 Search MCP 服务器位于你的站点 URL 的 `/authed/mcp` 路径。

<Note>
  仅当你为站点启用了[认证](/zh/deploy/authentication-setup)时，**MCP** 设置页面才会显示。如果你的站点不需要认证，你的 Search MCP 服务器仍可通过站点 URL 的 `/mcp` 路径访问。
</Note>

Mintlify 为搜索 MCP 服务器保留 `/mcp` 和 `/authed/mcp` 路径。你不能将它们用于其他导航元素。
...
如果你的文档需要认证，你的 MCP 服务器会要求用户在连接前先完成认证。当用户将你的 MCP 服务器 URL 添加到其 AI 工具时，必须使用现有凭据登录。认证完成后，系统会通过重定向将用户带回其工具。MCP 服务器只会根据每位用户的[用户组](/zh/deploy/authentication-setup#control-access-with-groups)返回其有权访问的内容。

如果你的文档采用部分认证，同时包含公开页面和受保护页面，你有两个 MCP 服务器端点：

- `/mcp`：无需认证。仅返回公开内容。将此端点分享给需要访问公开内容的用户。
- `/authed/mcp`：始终需要认证。根据每位用户的[用户组](/zh/deploy/authentication-setup#control-access-with-groups)权限返回相应内容。将此端点分享给需要访问受保护内容的用户。

`/authed/mcp` 端点使用其专属的 OAuth 流程，位于 `/authed/mcp/oauth/*`。为你的 MCP 服务器配置的重定向域名同时适用于 `/mcp` 和 `/authed/mcp`。

Claude、ChatGPT、Cursor 和 Devin 等常见 MCP 客户端始终被允许对你的 MCP 服务器完成认证。你无需将这些客户端添加到任何允许列表。

要允许其他基于 Web 的工具连接，请在控制台中添加它们的重定向域名。重定向域名是指用户完成认证后，AI 工具使用的主机名。如果用户的 AI 工具的重定向域名不在此列表中，且该工具不属于始终允许的客户端，就无法完成认证。

当你的站点配置了[认证](/zh/deploy/authentication-setup)时，托管 MCP 服务器会自动启用。

<Steps>
  <Step title="打开托管 MCP 设置">
    在控制台中，前往 **Settings → Security & access → MCP**。
  </Step>

  <Step title="添加自定义重定向域名">
    在 **Allowed OAuth redirect URIs** 下，添加你希望向用户开放访问权限的其他 AI 工具的重定向域名。你只需为不在始终允许列表中的客户端添加域名。
...
<Note>
  MCP 客户端凭据与 [Mintlify API 密钥](/zh/api/introduction) 分开。管理员和助手 API 密钥用于验证对 Mintlify REST API 的请求，不能用于验证你的 MCP 服务器。要以编程方式调用你的 MCP 服务器，请在控制台的 **Settings → Security & access → MCP** 中创建客户端凭据，并将其兑换为访问令牌。
</Note>

<Steps>
  <Step title="创建客户端凭据">
    1. 在控制台中，前往 **Settings → Security & access → MCP**。
    2. 在 **Client credentials** 部分，单击 **Generate**。
    3. 输入凭据标签以标识其用途，然后确认。
    4. 从 **Secret generated** 对话框复制 **client ID** 和 **client secret**。client secret 只显示一次，之后无法找回。
  </Step>
...
你可以在控制台的 **Settings → Security & access → MCP** 管理你的客户端凭据。

- **删除凭据**将永久撤销访问权限。此操作无法撤销。
- **轮换凭据**的方式是：先创建一个新凭据，将所有使用旧凭据的集成更新为新的 client ID 和 secret，然后删除旧凭据。每个凭据都有唯一的 secret，因此在删除旧凭据之前，创建新凭据不会影响现有凭据。
...
| 每个文档站点的 Search（域名） | 每小时 10,000 个请求 | 限制所有用户在你的 MCP 服务器上的搜索工具调用总次数。 |
| 每个文档站点的 Query docs filesystem（域名） | 每小时 10,000 个请求 | 限制所有用户在你的 MCP 服务器上的 query docs filesystem 工具调用总次数。 |
| 每个文档站点的认证 Search（域名） | 每小时 5,000 个请求 | 限制所有用户在你的 MCP 服务器上的认证搜索工具调用总次数。 |
| 每个文档站点的认证 Query docs filesystem（域名） | 每小时 5,000 个请求 | 限制所有用户在你的 MCP 服务器上的认证 query docs filesystem 工具调用总次数。 |

当超过限制时，MCP 服务器会返回 HTTP `429 Too Many Requests` 响应。MCP 客户端应稍作退避后再重试 — 从几秒开始的指数退避是一个安全的默认策略。限制在滚动的每小时窗口中重置，因此大多数调用方都会在一小时内自动恢复。

如果你的站点持续触发这些限制，请联系 [support@mintlify.com](mailto:support@mintlify.com) 申请更高的限额。
...
<Tabs>
  ...
  <Tab title="Claude">
    <Steps>
      <Step title="获取你的 MCP 服务器 URL。">
        你的 MCP 服务器 URL 是站点 URL 的 `/mcp` 路径。例如 `https://your-docs.com/mcp`。如果你的站点需要认证，请改用 `/authed/mcp` 路径。
      </Step>
  ...
  <Tab title="Codex">
    <Steps>
      ...
      <Step title="为用户发布 MCP 服务器 URL。">
        为用户创建一份指南，包含你的 MCP 服务器 URL 以及将其连接到 Codex CLI 的步骤。
...
<Tabs>
  ...
  <Tab title="Codex">
    要将 Mintlify MCP 服务器连接到 Codex CLI，请将其添加到位于 `~/.codex/config.toml` 的配置文件中：
    ...
    通过启动 Codex 会话并询问以下问题来测试连接：

    ```text
    What tools do you have available?
    ```

    Codex 应当将 Mintlify MCP 服务器列为可用工具。

    查看 [Codex MCP 文档](https://developers.openai.com/codex/mcp)了解更多详情。
  </Tab>
