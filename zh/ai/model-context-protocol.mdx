---
title: "Model Context Protocol 服务器"
description: "将 Claude、Cursor、ChatGPT 等 AI 工具连接到托管的搜索 MCP 服务器，让它们能够搜索并检索你站点上的文档内容和 API 端点。"
...
<Info>
  WebMCP 需要 [Pro 或 Enterprise 方案](https://mintlify.com/pricing?ref=webmcp)。
</Info>

WebMCP 使用浏览器的 `navigator.modelContext` API 将你的 MCP 工具直接注册到页面上。支持 WebMCP 的浏览器和浏览器内代理可以在用户浏览你的站点时调用这些工具，无需先连接到你托管的 Search MCP 服务器。

WebMCP 公开五个工具：

- **`search_docs`**：搜索你的站点并返回带有绝对 URL 的摘要片段。
- **`read_page`**：返回指定页面的 Markdown 内容。
- **`get_site_overview`**：返回你站点的 `llms.txt`。
- **`read_skill`**：返回 [skill.md 文件](/zh/ai/skillmd)的内容。
- **`navigate_page`**：将当前标签页导航到你站点上的另一个页面。

WebMCP 无需任何设置或配置。当访问者的浏览器支持 WebMCP 规范时，Mintlify 会为符合条件的站点自动注册这些工具。
