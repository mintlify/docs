...
- `/mcp`：不需要认证。仅返回公开内容。将此端点分享给需要访问公开内容的用户。
- `/authed/mcp`：始终需要认证。根据每位用户的[用户组](/zh/deploy/authentication-setup)权限返回其有权访问的内容。将此端点分享给需要访问受保护内容的用户。

`/authed/mcp` 端点使用其自己的 OAuth 流程，位于 `/authed/mcp/oauth/*`。为你的 MCP 服务器配置的重定向域名同时适用于 `/mcp` 和 `/authed/mcp`。
