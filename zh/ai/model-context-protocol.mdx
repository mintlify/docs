...
<Steps>
  ...
  <Step title="启用预定义客户端">
    控制台包含常见 AI 工具（如 Claude Desktop 和 Cursor）的预定义开关。为你想要允许的每个客户端启用开关。每个开关会自动添加该客户端相应的重定向 URI。
  </Step>

  <Step title="添加自定义域名">
    对于未列为预定义客户端的 AI 工具，请在 **Custom Domains** 部分添加其重定向域名。如果用户的 AI 工具的重定向域名不在此列表中，就无法完成认证。
