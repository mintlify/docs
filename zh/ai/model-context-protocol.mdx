...
对于需要认证的文档，你必须先启用 MCP 服务器，用户才能使用。用户在连接到你的 MCP 服务器之前必须先完成认证。你的 MCP 服务器只搜索每位用户根据其[用户组](/zh/deploy/authentication-setup)有权访问的内容。
...
对于需要认证的文档，你的 MCP 服务器只索引[公开用户组](/zh/deploy/authentication-setup#make-pages-public)中的页面。
