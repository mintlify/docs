...
- **`submit_feedback`**：当代理发现某个页面不正确、过时、令人困惑或不完整时，将文档问题反馈给你的团队。代理会提供页面路径和问题描述，并将其记录为针对该页面的负面反馈，方便你的团队进行修复。这是唯一会写入数据的工具；服务器的其他部分均为只读。

代理会根据对话的上下文来决定何时使用每个工具。例如，代理可能会先搜索你的文档以查找相关页面，然后使用 query docs filesystem 工具读取最相关结果的完整内容。如果它在过程中发现页面存在问题，可以调用 `submit_feedback`。

代理必须连接你站点的 Search MCP 服务器,才能发现并调用 `submit_feedback`。仅有网页搜索或网页获取权限不会公开 MCP 工具。未连接 MCP 的代理只有在能够直接向站点的反馈端点发出 HTTP `POST` 请求时,才能[提交代理反馈](/zh/optimize/feedback#how-agent-feedback-works)。
