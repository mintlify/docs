---
title: "AI 助手"
description: "为文档站点添加 AI 聊天助手，回答用户问题、引用来源并按需生成代码示例。"
...
在控制台查看 AI 助手的使用情况，以了解用户行为和文档效果。导出并分析查询数据，以帮助识别：
...
AI 助手会为你已发布的文档和 API 规范建立索引，以便准确回答问题。当你发布变更时，AI 助手会立即为新增、更新或删除的内容建立或更新索引。AI 助手不会为草稿 branch 或预览部署建立索引。
...
你可以[设置分流邮箱](/zh/assistant/configure#set-deflection-email)，让 AI 助手在无法回答用户问题时，向用户提供你的支持邮箱。即使文档没有覆盖他们的具体问题，这也为用户提供了一个后续解决途径。
...
在 Bot Protection 部分，启用隐形验证码来保护你的 AI 助手免受自动化滥用。机器人防护注重隐私保护，对几乎所有访问者都是不可见的。

要启用机器人防护：

1. 前往控制台中的 [Assistant 设置](https://dashboard.mintlify.com/products/assistant/settings) 页面。
2. 在 **Bot Protection** 部分，开启 **Invisible Captcha**。

更改最多需要 10 分钟才能生效。

<Note>
  如果你使用了自定义的[内容安全策略头](/zh/deploy/csp-configuration)，你必须将 `hcaptcha.com` 和 `*.hcaptcha.com` 添加到你的允许列表中。
</Note>
...
在你的文档项目根目录下添加一个 `.mintlify/Assistant.md` 文件，为 AI 助手提供自定义指令，以影响它的回答方式（`Assistant.md` 也是可接受的文件名）。AI 助手会将这些指令作为每条回复的系统级上下文。

<Warning>
  将你的 `Assistant.md` 文件放在 `.mintlify/` 目录内，而不是放在项目根目录下。如果你将文件存放在项目根目录下，你的自定义指令将通过 `/assistant.md` 路径公开访问。`.mintlify/` 目录不会被公开提供服务，因此当你的 `Assistant.md` 文件位于该目录中时，它不会被公开访问。
  
  如果你必须将 `Assistant.md` 文件存放在项目根目录或其他目录中，请将其添加到你的 [`.mintignore`](/zh/organize/mintignore) 文件中，以将其从文档站点中排除。
</Warning>
...
在使用 CLI 登录后，AI 助手可在你的本地预览中使用。这让你可以在本地开发文档时测试助手的行为、`Assistant.md` 中的自定义指令和响应。

1. 如果尚未安装，请使用 `npm i -g mint` 安装 CLI。
2. 运行 `mint login` 进行身份验证。
3. 运行 `mint dev` 启动本地预览。

本地预览中的 AI 助手使用与已部署文档站点相同的索引内容。
