...
在控制台的 [Assistant Configurations](https://dashboard.mintlify.com/products/assistant/settings/billing) 页面查看并更改你当前的等级。
...
2. 在 **Spending Controls** 部分，从下拉菜单中选择你所需的等级。
...
默认情况下，AI 助手不允许超额使用。如果你达到消息配额上限，AI 助手将不可用，直到你的消息配额重置。

如果你希望允许超额使用，可以在控制台中 [Assistant Configurations](https://dashboard.mintlify.com/products/assistant/settings/billing) 页面里的 **Billing Controls** 部分启用该选项。

你也可以设置用量提醒，当你的消息配额使用达到某一百分比时，通过电子邮件收到通知。
...
“Categories” 标签页使用 LLM 自动为会话归类。类别会显示话题或主题的摘要、上一次有人就该类别提问的时间，以及一段时间内关于该类别的问题数量。
