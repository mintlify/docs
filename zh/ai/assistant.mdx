...
* **构建上下文**，根据用户提问时正在浏览的页面。
* **引用来源** 并提供可点击的链接，将用户直接带到被引用的页面。
* **返回详细的 API 信息**，在回答有关 API 的问题时，包括来自 OpenAPI 规范的方法、参数、请求体和响应模式。
* **生成可复制的代码示例**，帮助用户基于你的文档实现解决方案。
* **支持多模态**，允许用户添加文本、图片和其他文件作为上下文。
...
AI 助手可以根据用户正在查看的页面生成特定于页面的问题，你也可以添加在所有页面都可用的持久性问题。

最多可添加 3 个示例问题。点击 **Ask AI**，即可根据你的文档获取推荐问题。

<Frame>
  <img src="/images/assistant/search-suggestions-light.png" alt="控制台中的搜索建议面板，已启用上下文相关的起始问题。" className="block dark:hidden" />

  <img src="/images/assistant/search-suggestions-dark.png" alt="控制台中的搜索建议面板，已启用上下文相关的起始问题。" className="hidden dark:block" />
</Frame>
