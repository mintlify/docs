...
Mintlify 会完全按照你书写的形式保留自定义 ID，包括大小写和下划线。你可以使用 camelCase、kebab-case、snake_case 或任意组合：
...
这些标题分别可以通过 `#quickStart`、`#setup_steps` 和 `#advanced-options` 访问。链接到自定义 ID 时，请完全匹配大小写——锚点区分大小写。

<Tip>
  当你从使用 camelCase 或 snake_case 锚点的文档工具迁移过来时，保留大小写非常有用。保留现有的自定义 ID，可以避免破坏外部入站链接。
</Tip>
