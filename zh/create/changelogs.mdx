---
title: "更新日志"
description: "创建包含基于日期条目、RSS 订阅支持和订阅者通知功能的产品更新日志，让用户及时了解最新动态。本文介绍内容创建步骤和使用要点，帮助你高效编写并维护文档页面。这些说明适合开发者和文档维护者查阅。你可以结合页面示例完成相关操作。相关做法也便于团队后续维护和协作。请根据项目需求选择合适的配置方式。"
...
所选标签会以 `?tags=` 查询参数的形式反映在页面 URL 中，因此用户可以分享指向筛选后更新日志视图的深层链接。打开像 `/zh/changelog?tags=Spearmint` 这样的链接会在加载时激活对应的侧边栏筛选器，更新筛选器时 URL 也会保持同步。使用逗号分隔多个标签，例如 `/zh/changelog?tags=Spearmint,Peppermint`。
