...
你在 `import` 语句中指定的名称就是用于渲染代码片段的 JSX 标签。它必须是有效的 JSX 组件标识符：以大写字母开头并使用 PascalCase（例如 `MySnippet`，而不是 `mySnippet` 或 `my-snippet`）。以小写字母开头的名称会被视为 HTML 元素，不会渲染你的代码片段。
