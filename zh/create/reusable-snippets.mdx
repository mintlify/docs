...
`import` 语句中是否使用花括号取决于代码片段的导出方式。导入 `.md` 或 `.mdx` 文件渲染后的内容时，请将其作为默认导出导入，不使用花括号（`import MySnippet from "..."`）。导入变量或 React 组件等命名导出时，请使用花括号（`import { myName } from "..."` 或 `import { MyComponent } from "..."`）。
