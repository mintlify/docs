...
你也可以在代码块内引用代码片段变量。在围栏代码块的内容中使用相同的 `{variableName}` 语法。页面渲染时，Mintlify 会用对应的 prop 值替换每个占位符，未匹配到 prop 的占位符则保持不变。

1. 在代码片段中添加一个引用变量的代码块。
...
2. 导入该代码片段并将值作为 prop 传入。

    ```mdx destination-file.mdx
    ---
    title: "示例页面"
    description: "这是一个导入带有代码块变量的代码片段的示例页面。"
    ---
