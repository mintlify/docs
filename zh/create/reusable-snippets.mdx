---
title: "可复用片段"
description: "创建可复用内容片段，在多个文档页面间同步内容并减少重复维护。本文介绍片段的工作方式、在 snippets 文件夹创建片段的步骤，以及导入文本、嵌套片段、传入变量和导入 React 组件等引用方法。更新片段文件后所有引用页面自动同步。片段文件不会作为独立页面发布，只在被导入时渲染。"
