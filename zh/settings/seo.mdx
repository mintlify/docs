---
title: "SEO"
description: "SEO（搜索引擎优化）最佳实践与 meta 标签配置，提升搜索可见性"
icon: "search"
---
...
你可以在 `docs.json` 或页面的 frontmatter 中添加 `metatags` 字段，完全自定义站点的 meta 标签。

<div id="global-meta-tags">
  ## 全局 meta 标签
</div>

你可以在 `docs.json` 中添加 `metadata` 字段，以设置默认的 meta 标签。
...
按如下方式将你的 meta 标签添加到页面的 metadata 中：
...
<Warning>
  在页面的 metadata 中使用时，带有冒号的 meta 标签需要用引号括起来。
</Warning>
...
你可以使用 [metatags.io](https://metatags.io/) 预览这些 Meta 标签在不同平台上的展示效果。
...
Mintlify 会自动生成 `sitemap.xml` 和 `robots.txt` 文件。你可以在文档站点的 URL 末尾加上 `/sitemap.xml` 来查看站点地图。

默认情况下，仅 `docs.json` 中包含的页面会被收录。若要包含隐藏链接，请在 `docs.json` 中添加 `seo.indexing`：
...
要添加自定义的 `sitemap.xml` 或 `robots.txt` 文件，请在项目根目录创建对应的 `sitemap.xml` 或 `robots.txt` 文件。添加自定义文件后，将覆盖同名的自动生成文件。删除自定义文件后，将恢复使用默认文件。

<div id="disabling-indexing">
  ## 禁用索引编入
</div>

如果你想让某个页面不被搜索引擎收录，可以在该页面的 [frontmatter](zh/pages) 中加入以下内容：
...
你也可以在 `docs.json` 中将 `metadata.robots` 字段设置为 `"noindex"`，以为文档的所有页面指定 `noindex`：
