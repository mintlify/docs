---
title: "自定义脚本"
description: "通过自定义 CSS 和 JS 深度定制你的文档"
icon: "code"
---
...
使用 Tailwind CSS v3 为 HTML 元素设置样式。你可以控制布局、间距、颜色等视觉属性。常见的类包括：
...
不支持 Tailwind CSS 的任意值语法。若需自定义数值，请改用 `style` 属性。
...
将 CSS 文件添加到你的存储库后，其中定义的类名将会生效，并可在所有 `MDX` 文件中使用。
...
Mintlify 提供了一组常用的标识符和选择器，帮助你为界面中的关键元素添加标签。

<Tip>
  使用“检查元素”来找到你要自定义的元素引用。
</Tip>
...
<Warning>
  随着平台的迭代，引用和常用元素的样式可能会发生变化。请谨慎使用自定义样式。
</Warning>
...
自定义 JS 允许你在全局添加自定义可执行代码。它相当于在每个页面都插入一个包含 JS 代码的 `<script>` 标签。

<div id="adding-custom-javascript">
  ### 添加自定义 JavaScript
</div>

文档的 content 目录中的任意 `.js` 文件都会被包含到每个文档页面中。比如，你可以添加以下 `ga.js` 文件，在整个文档中启用 [Analytics](https://marketingplatform.google.com/about/analytics)。
