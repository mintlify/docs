---
title: "自定义脚本"
description: "通过自定义 CSS 与 JS 深度定制你的文档"
icon: "code"
---
...
使用 Tailwind CSS v3 为 HTML 元素设置样式。你可以控制布局、间距、颜色和其他视觉属性。常见的类包括：

* `w-full` - 全宽
* `aspect-video` - 16:9 宽高比
* `rounded-xl` - 大圆角
* `block`, `hidden` - 显示控制
* `dark:hidden`, `dark:block` - 深色模式可见性

不支持 Tailwind CSS 的任意值语法。对于自定义值，请改用 `style` 属性。
...
将 CSS 文件添加到你的代码仓库后，其中定义的类名会被应用，并可在所有 `MDX` 文件中使用。
...
Mintlify 提供了一组通用的标识符和选择器，帮助你为 UI 中的重要元素添加标签。

<Tip>
  使用“检查元素”以找到你想自定义的元素引用。
</Tip>
...
<Warning>
  随着平台演进，引用和常见元素的样式可能会发生变化。请谨慎使用自定义样式。
</Warning>
...
自定义 JS 允许你在全站范围内添加自定义可执行代码，相当于在每个页面插入包含 JS 代码的 `<script>` 标签。

<div id="adding-custom-javascript">
  ### 添加自定义 JavaScript
</div>

文档内容目录中的任意 `.js` 文件都会在每个文档页面中加载。比如，你可以添加一个 `ga.js` 文件，在整个文档站点启用 [Google Analytics](https://marketingplatform.google.com/about/analytics)。
