---
title: "Mintlify 分析完整使用指南：跟踪流量、搜索、助手使用情况和用户互动，改进文档内容与用户路径分析"
description: "在 Mintlify 分析仪表板中跟踪流量、搜索趋势、助手使用情况、用户反馈和互动指标。了解如何识别热门内容、发现文档缺口、分析用户路径，并根据搜索和行为数据改进页面与整体文档体验。使用这些数据衡量内容改动的效果，并持续优化文档和用户体验。按时间范围查看变化趋势。"
...
用户互动部分展示访客在页面浏览之外如何与文档互动。您可以使用它来了解哪些内容会促使访客点击、复制、调用 API、回应反馈或执行其他操作。

概览显示互动总数、每次会话的平均互动次数和 CTA 点击率。每项指标都包含与上一时间段相比的变化。

**一段时间内采取的操作**图表比较 CTA 点击和其他操作。CTA 点击是对[导航栏号召性用语按钮](/zh/organize/settings-structure#navbar)的点击。操作包括上下文菜单点击、代码块复制、API Playground 调用、反馈响应和类似互动。

每页表格显示页面浏览量、点击量、CTA 比率、操作次数和操作率。使用这些指标比较页面，并识别能够带来更深层互动的内容。

**用户流转路径**显示从入口点开始，经过访客第一次、第二次和第三次互动的常见路径。选择入口点以探索其路径。

**用户漏斗**衡量两种旅程：

- **从搜索到答案**：打开搜索、输入查询、点击结果、进入页面并采取操作。
- **助手对话**：开始对话、获得答案、提出后续问题并与答案互动。
