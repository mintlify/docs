---
title: "分析仪表板：文档流量、用户流程与内容优化洞察"
description: "在 Mintlify 分析仪表板中查看文档流量、用户流程、搜索趋势、助手使用情况和用户反馈，了解访客路径、发现内容问题，并持续改进文档体验和效果。通过入口页面和时间范围筛选数据，分析常见导航路径、页面之间的流失情况以及访客停止访问的位置，为内容规划和文档优化提供依据。"
...
用户流程部分显示访客从选定页面进入文档后所经过的路径。您可以使用它了解访客接下来查看哪些页面，以及他们在哪里离开网站。

查看用户流程：

1. 在仪表板中打开[分析](https://app.mintlify.com/analytics)页面。
2. 选择时间范围。
3. 在**用户流程路径**中选择入口页面。
4. 将鼠标悬停在节点上，查看进入该节点的会话数（**进入**）、继续前往其他页面的会话数（**离开**）以及适用时的放弃率。

每个互动列最多显示六个按到达量排序的命名页面，其余页面会合并到 **Other（N 个页面）**中。这样可以保留继续访问的会话，因此放弃率只反映未继续访问的访客。该分组会显示在列底部。最后一个互动列不显示放弃率，因为这些会话在当前显示的流程中没有后续页面。
...
您无需配置网站。符合条件的 Pro 和 Enterprise 套餐在文档获得访客流量后会显示用户流程数据。
...
仪表板首先显示 CTA 图表和表格，接着显示用户流程，最后显示用户漏斗。

该部分显示**随时间发生的操作**图表，分别统计操作和 CTA 点击次数。操作包括上下文菜单点击、代码块复制、API playground 调用和反馈响应，与 CTA 点击分开统计。页面表格包含页面浏览量、点击量、CTA 点击率、操作次数和操作率。

**用户漏斗**按五个阶段跟踪搜索会话：打开搜索、输入查询、点击结果、访问页面和执行操作。最后一个阶段统计点击 CTA 或执行页面级操作的会话，例如使用上下文菜单选项、复制代码块、调用 API playground 或提交反馈。每个阶段都会显示到达和流失的会话数。执行操作的会话数不会超过访问页面的会话数。
