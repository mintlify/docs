---
title: "Mintlify 文档流量、搜索与助手互动分析仪表板"
description: "在 Mintlify 分析仪表板中跟踪文档流量、页面浏览、搜索趋势、搜索漏斗、助手对话漏斗、助手使用情况和用户反馈数据，了解用户如何发现、阅读和使用您的内容，并据此改进文档内容、搜索体验和助手回答效果。按时间范围比较趋势，发现内容和用户体验中的改进机会，持续提升效果。"
...
在“展示次数”部分，使用“用户漏斗”选择器在“搜索到回答”和“助手对话”之间切换。默认选择“搜索到回答”。选择“助手对话”以查看助手阶段。

助手对话漏斗展示会话从开始对话到与回答互动的进展过程。使用它可以找出用户停止与助手互动的环节。

| 阶段 | 描述 |
| --- | --- |
| **已开始** | 会话中有未被标记为垃圾内容的助手活动。 |
| **已回答** | 助手在会话中至少完成了一次回答。 |
| **后续提问** | 助手在会话中至少完成了两次回答。 |
| **已互动** | 会话通过点击来源、复制回答、点击网页搜索结果或提交反馈与回答进行了互动。 |

查看各阶段之间的流失情况，了解用户是否收到回答以及是否继续与回答互动。漏斗统计所选时间范围内的会话，排除被标记为垃圾内容的会话，并要求互动发生在回答之后。

无需配置漏斗。打开分析页面，选择时间范围，然后查看展示次数部分。
...
搜索漏斗展示会话从打开搜索到访问搜索结果页面后采取操作的过程。使用它可以找出用户停止互动的环节。
...
| **已查询** | 会话提交了搜索查询。 |
| **已点击** | 会话点击了搜索结果。 |
| **已访问** | 会话查看了通过搜索结果打开的页面。 |
| **已采取操作** | 会话在访问页面后执行了操作，例如点击 CTA、使用上下文菜单、复制代码块、调用 API playground 或提交页面反馈。 |

查看各阶段之间的流失情况，判断用户是否难以找到结果、理解页面或继续采取操作。漏斗统计所选时间范围内的会话，并要求操作发生在页面访问之后。

无需配置。打开[分析](https://app.mintlify.com/analytics)页面，选择时间范围，然后查看搜索部分。分析功能需要 [Pro 或 Enterprise 套餐](https://mintlify.com/pricing?ref=analytics)。
...
互动概览卡片位于用户互动部分的顶部,汇总所选时间范围内的互动情况。它显示互动总数、每次会话的平均互动次数以及 CTA 点击率。

- **互动总数**: CTA 点击次数与页面级操作次数的总和。
- **每次会话的平均互动次数**: 互动总数除以发生过 CTA、操作或页面浏览活动的会话数。
- **CTA 点击率**: CTA 点击次数除以人类页面浏览量。该比率不能超过 100%。

每项指标都包含一个变化徽章,用于比较所选时间段与上一时间段的变化。更改分析时间范围后,指标值和对比变化都会更新。CTA 点击率以百分比显示。

使用这些指标了解访客与您的内容和号召性用语互动的活跃程度。
