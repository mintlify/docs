---
title: "搜索"
description: "配置 Mintlify 文档站点内置搜索栏的行为，包括排名加权和每次查询返回的结果数量。"
keywords: ["搜索", "搜索排名", "搜索加权", "加权", "搜索优先级", "最大搜索结果数", "最大结果数"]
---

配置 Mintlify 托管的文档站点上搜索栏的行为。这些设置只影响站内搜索，不会改变外部搜索引擎对你页面的索引方式。
...
在仪表盘中控制搜索栏每次查询返回的结果数量。默认值为 `6` 个结果。你可以设置 `1` 到 `100` 之间的任意值。

在你的仪表盘中，依次进入 **Settings** > **Deployment** > **Search**，将 **Maximum search results** 设置为你想要的值，然后选择 **Save changes**。

<Frame>
  <img
    alt="仪表盘中的 Search 设置页面，Maximum search results 步进器设置为 6。"
...
较高的值会在每次查询中显示更多匹配项，适用于主题覆盖范围较广、用户希望浏览多个候选结果的站点。较低的值可以保持结果面板紧凑，并引导用户关注排名最高的匹配项。
