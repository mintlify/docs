...
默认情况下，文档站点的搜索栏每次查询最多返回 6 个结果。要返回更多（或更少）结果，请在仪表盘中更改 **Maximum search results** 设置。

1. 在仪表盘中前往 [Project Settings → Search](https://dashboard.mintlify.com/settings/deployment/search)。
2. 使用步进控件选择 `1` 到 `100` 之间的值。
3. 选择 **Save changes**。

新的上限会应用于来自文档站点的每个搜索请求。返回更多结果可以提高合适页面被看到的概率，但数值过大可能会让搜索面板更难浏览。

<Note>
  你需要 `deployment.update` 权限才能更改此设置。该设置在 Atlas（自托管）部署中不可用。
</Note>
