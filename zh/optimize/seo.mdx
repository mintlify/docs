...
<Note>
  对于完全需要认证的文档站点，站点地图和 `robots.txt` 文件也需要完成认证后才能访问，且站点地图会排除属于 [用户分组](/zh/deploy/authentication-setup#control-access-with-groups) 的页面。

  对于包含公开页面的部分认证站点，站点地图和 `robots.txt` 可公开访问。匿名访客只能在站点地图中看到公开页面，已认证的用户还可以看到未分组的页面。
</Note>
