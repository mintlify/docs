...
对于相同的键，文件夹级标签会覆盖 `seo.metatags`。当页面位于多个条目下时，较长的路径会覆盖较短的路径。页面的 frontmatter 仍会覆盖上述两者。`canonical` 始终来源于 `seo.metatags`，无法按文件夹设置。完整的架构请参见 [`seo.paths`](/zh/organize/settings-seo#seo)。
...
frontmatter 也可以覆盖 `docs.json` 中在 [`seo.metatags`](/zh/organize/settings-seo#seo) 或匹配的 [`seo.paths`](/zh/organize/settings-seo#seo) 条目下声明的任何自定义元标签。使用相同的键提供一个非空字符串值，即可在该页面上替换站点级或文件夹级的值。frontmatter 不能引入配置中未声明的元标签，非字符串值将被忽略。
