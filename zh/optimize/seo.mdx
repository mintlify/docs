...
下面是可添加到你的 `docs.json` 中的 meta 标签的完整列表。这些 meta 标签有助于提升站点的 SEO（搜索引擎优化）、社交分享效果和浏览器兼容性。
...
如果你想阻止页面被搜索引擎收录，可以在页面的 [frontmatter](zh/pages) 中加入以下内容：
...
你也可以在 `docs.json` 中将 `metatags.robots` 字段设置为 `"noindex"`，以为文档的所有页面指定 `noindex`：
