...
要为一部分页面（例如版本化文档或特定的产品区域）应用不同的元标签，请在 `docs.json` 中添加 `seo.paths`。每个条目指向相对于文档根目录的文件夹路径，并将其标签应用到该文件夹下的每个页面。
...
对于相同的键，文件夹级标签会覆盖 `seo.metatags`，较长的路径会覆盖较短的路径，页面的 frontmatter 会覆盖上述两者。路径规则和例外情况请参见 [`seo.paths`](/zh/organize/settings-seo#param-seo-paths)。
...
frontmatter 也可以覆盖 `docs.json` 中在 [`seo.metatags`](/zh/organize/settings-seo#param-seo-metatags) 或匹配的 [`seo.paths`](/zh/organize/settings-seo#param-seo-paths) 条目下声明的任何自定义元标签。为相同的键设置一个非空字符串值，即可在该页面上替换站点级或文件夹级的值。frontmatter 不能添加配置中未声明的自定义元标签，Mintlify 会忽略非字符串值。
...
默认情况下，Mintlify 仅索引你在 `docs.json` 导航中包含的页面。它会将[隐藏页面](/zh/organize/hidden-pages)——即未列入导航的页面、位于隐藏标签页或分组下的页面，以及 frontmatter 中带有 `hidden: true` 的页面——排除在以下内容之外：
  - 搜索引擎站点地图
  - 内部文档搜索
...
要在保持文档公开可访问的同时让搜索引擎不收录整个项目，请在仪表盘的 [常规](https://app.mintlify.com/settings/project/general) 页面启用 **Don't index project**。

启用后，Mintlify 会：

- 为每个页面渲染 `noindex, nofollow` 的 robots meta 标签。
- 返回空的 `sitemap.xml`。
- 对 `llms.txt` 返回 `404`，包括 `/.well-known/llms.txt` 位置。
- 将你的页面从 [Mintlify Index](/zh/search-index) 中移除，使编程代理无法再从中检索你的内容。
...
你的站内搜索、[AI 助手](/zh/assistant/index)和 [MCP 服务器](/zh/ai/model-context-protocol)将继续工作。若要将某些页面从这些渠道中移除，请对相应页面使用 `noindex: true` 或 `searchable: false`。

<Note>
  **Don't index project** 不会限制 [`llms-full.txt`](/zh/ai/llmstxt#llms-fulltxt)，该文件在 `/llms-full.txt` 仍然可用。即使在此设置启用期间 `llms.txt` 会返回 `404`，每个页面的 Markdown 导出仍会保留指向 `/llms.txt` 的引用。如果你的 `llms.txt` 返回 `404` 而 `llms-full.txt` 正常，请检查此设置。参见[修复 llms.txt 返回 404 而 llms-full.txt 正常的问题](/zh/help-center/llms-txt-returns-404-but-llms-full-txt-works)。
</Note>

当你希望保持项目在线以便内部审阅、预演或有限分享，但不希望它出现在公开搜索结果中，或被 AI 工具用作训练或检索数据时，可以使用此选项。
