---
title: "标注框"
description: "使用标注框组件在文档中突出显示重要信息、警告和提示。"
...
通用 `Callout` 组件支持自定义图标和颜色。带类型的标注框（`Note`、`Warning`、`Info`、`Tip`、`Check`、`Danger`）使用预设的图标和颜色。所有标注框类型都支持 `className` 进行自定义样式设置。
