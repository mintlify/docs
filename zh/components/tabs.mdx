...
当标签相同时，Tabs 会在同一页面上自动与其他标签页和[代码分组](/zh/components/code-groups)同步。当你选择某个标签页时，所有具有相同标签的其他标签页和代码分组都会同步更新为你的选择。你可以将 `sync` 设置为 `false` 来禁用该行为。
...
<ResponseField name="sync" type="boolean" default="true">
  当为 `true` 时，标签页会与页面上其他具有相同标签的标签页和代码分组保持同步。将其设置为 `false` 可使标签页彼此独立。
</ResponseField>
