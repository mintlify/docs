...
使用横幅在整个文档站点中展示重要的公告、更新或通知。横幅会显示在每个页面顶部，支持 Markdown 格式，并可设置为可关闭。
...
<CodeGroup>
  ...
  "banner": {
    "content": "**需要操作：** 请在 1 月 1 日前迁移到新版本。[迁移指南](/migration)",
    "dismissible": true
  }
  ```
</CodeGroup>
...
<ResponseField name="dismissible" type="boolean">
  是否允许用户关闭横幅。为 `true` 时，用户可将横幅关闭，本次会话内将不再显示。默认值为 `false`。
</ResponseField>