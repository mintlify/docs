---
title: "静态导出"
description: "通过 Mintlify REST API 生成自包含的文档静态导出，并以单个包的形式下载。了解如何预渲染文档、部署静态文件，并在 S3、CDN 或其他静态托管服务上托管。"
...
<Note>
  已启用身份验证的部署不支持静态导出。如果你的部署使用部分或完整身份验证，[启动静态导出作业](/zh/api/static-export/start-job) 将返回 `403` 响应。如需导出已启用身份验证的部署，请联系 [support@mintlify.com](mailto:support@mintlify.com)。
</Note>
...
静态导出包使用明确的 `.html` 页面 URL。`/guides/getting-started` 会变为 `/guides/getting-started.html`，首页则为 `/index.html`。这样 URL 与包中的文件相对应，可以直接部署到 S3 或普通静态托管，无需将无扩展名 URL 重写为文件路径。上传时请保留 `.html` 文件名。静态导出 API 会自动启用此格式，因此你无需修改文档配置。
