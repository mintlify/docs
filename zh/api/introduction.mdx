---
title: "Mintlify REST API 使用简介"
description: "使用 Mintlify REST API 触发部署、嵌入 AI 助手、导出分析数据、管理文档，并创建静态导出任务、查询进度和生成可下载的自包含文档包。本文概述端点、使用场景、身份验证方式、API 密钥类型和服务器端安全要求，帮助你安全地集成文档工作流，适用于服务端应用和自动化流程。"
...
* [启动静态导出作业](/zh/api/static-export/start-job)：为部署启动静态导出作业。
* [获取静态导出作业状态](/zh/api/static-export/get-job-status)：轮询静态导出作业的状态和进度。
* [生成导出包](/zh/api/static-export/generate-bundle)：打包已完成的静态导出作业并返回下载链接。
* [Trigger automation](/zh/api/automations/trigger)：按需运行已计划的自动化。
* [Detect AI-sounding prose](/zh/api/admin/deslop)：分析页面中由 AI 生成的文字，并获取人工风格的改写建议。
...
* **静态导出**：使用 [启动静态导出作业](/zh/api/static-export/start-job)、[获取静态导出作业状态](/zh/api/static-export/get-job-status) 和 [生成导出包](/zh/api/static-export/generate-bundle)，生成并下载用于自托管的自包含静态包。
...
在控制台的 [API keys 页面](https://app.mintlify.com/settings/organization/api-keys)生成 API key。管理员和 Index API key 属于组织。你可以在同一组织内的多个部署中使用相同的 key。Assistant API key 属于创建它的部署。
...
| key 类型          | 前缀        | 用途                                                     |
| ----------------- | ----------- | -------------------------------------------------------- |
| 管理员 API key    | `mint_`     | 更新、代理任务、Analytics 导出和静态导出。仅限服务器端。   |
...
在控制台的 [API keys 页面](https://app.mintlify.com/settings/organization/api-keys) 创建 key 时设置允许列表。允许列表在 key 的生命周期内固定不变——要更改它，请删除该 key 并创建一个新的。如果不设置允许列表，key 会接受来自任何 IP 地址的请求。
...
在控制台的 [API keys 页面](https://app.mintlify.com/settings/organization/api-keys) 创建 key 时设置 scope。scope 在 key 的生命周期内固定不变——要更改它们，请删除该 key 并创建一个新的。如果不设置任何 scope，该 key 可以调用所有管理员端点（现有 key 保持不变）。
...
在控制台的 [API keys 页面](https://app.mintlify.com/settings/organization/api-keys) 设置过期时间。过期时间在 key 的生命周期内固定不变——要更改它，请删除该 key 并创建一个新的。如果不设置过期时间，该 key 永不过期。
