...
<Note>
  如果编辑器与你的仓库出现不同步的情况 —— 例如文件树为空，或即使文件确实存在于你的部署分支上仍显示 `Unable to find docs.json or mint.json` —— 请使用 [Reset editor](/zh/editor/settings#reset-editor) 强制从 Git 重新同步。
</Note>
...
<Card title="草稿" icon="file-pen" horizontal href="/zh/editor/drafts">
  在隔离的草稿中处理更改，草稿会保持未发布状态，直到你请求评审并发布。
</Card>
