...
- 你的组织有一个已连接到 GitHub 或 GitLab 的 Mintlify 部署。如果尚未设置，请先参照[快速入门](/zh/quickstart)。
- 你在该组织中拥有 [admin 或 editor 角色](/zh/dashboard/roles)。Viewers 可以浏览并留下[建议](/zh/editor/suggestions)，但无法发布。
- 你的 Git 提供商允许编辑器在仓库上打开拉取请求。

<Steps>
  <Step title="Create a branch">
    分支是你内容的临时副本。这是你在不影响线上站点的情况下进行更改的地方。你可以将分支合并到线上站点，或在选择不进行更改时安全地丢弃它。
...
如果你决定不发布这些更改，可以放弃该分支而不是将其合并。切换回你的部署用分支，然后从你的 Git 提供商（GitHub 或 GitLab）中删除该分支。分支上任何未发布的提交都会随之被删除，你的线上站点不受影响。

更多关于管理分支和拉取请求的内容，请参见[分支与发布](/zh/editor/branching-and-publishing)。
