...
如果没有待处理的更改，或者当前 branch 已经有打开的拉取请求，则 **Publish** 按钮不可点击。

<Note>
  你的线上文档站点会在 Mintlify 构建并部署你发布的更改后更新。这通常需要 30 秒到几分钟。请在你的[仪表板](https://dashboard.mintlify.com)上查看更改的部署状态。
</Note>
...
当你尝试合并对相同文件有不兼容更改的分支时，就会发生冲突。

- 你和另一位团队成员在不同的分支上编辑了同一文件中的相同行。
- 你在一个分支上移动、重命名或删除了文件，并在另一个分支上以不同方式修改了这些文件。
