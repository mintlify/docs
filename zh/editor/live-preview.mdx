...
<Note>
  实时预览适用于公开站点，以及使用[身份验证](/zh/deploy/authentication-setup)的站点，包括密码、OAuth、JWT 和 Mintlify 托管的访问方式。为经过身份验证的站点打开实时预览时，你会通过控制台登录，并以编辑者身份查看你有权访问的所有页面，包括受基于分组的访问控制保护的页面。
</Note>
