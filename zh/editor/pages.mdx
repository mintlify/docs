...
使用版本历史查看正在编辑的页面的已保存版本。当编辑器保存的内容与最近记录的版本不同时，Mint 会记录新条目。保存未更改的内容不会创建重复条目。保存后的页面也可以为空。每条记录显示作者和相对保存时间。最新记录标记为 **Latest**。
...
抽屉显示当前分支中当前页面的版本。点击 **Show more versions** 加载更早的记录。当你拥有页面编辑权限时，较早的版本会显示 **Restore** 操作。点击 **Restore** 并确认，即可使用该保存版本替换当前页面内容。此操作会覆盖未保存的更改。被阻止的部署分支和 **Latest** 版本不提供 Restore 操作。你无法从抽屉比较版本。
