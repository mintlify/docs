...
在左侧面板的 **Navigation** 标签页中浏览文档页面。切换到 **File explorer** 标签页可以查看仓库中的所有文件，包括 snippet、配置文件或图片等非内容文件。你也可以按 <kbd>Shift</kbd> + <kbd>Ctrl/Cmd</kbd> + <kbd>F</kbd> 切换文件浏览器。
...
如需更改默认打开的视图，请参阅[默认侧边栏视图](/zh/editor/settings#default-sidebar-view)。
...
你可以在文件浏览器中直接打开仓库里的大多数文本文件。常见格式都可编辑，包括：

* 配置文件，如 `docs.json`、`yaml`、`toml` 和 `ini`。
* snippet 和可复用的 Markdown。
* 源代码文件，如 `js`、`ts`、`tsx`、`py`、`rb`、`go`、`sh`、`sql`、`html` 和 `css`。

编辑会像 MDX 页面一样实时同步，因此队友会立即看到你的更改。发布时，编辑器会像处理其他更改一样提交该文件。

二进制文件和可执行文件（例如 `pdf`、字体、压缩包、Office 文档、`exe`、`dll`、`dmg`、`jar` 和 `iso`）会以 **No preview available** 面板打开。你可以在文件浏览器中看到它们，但无法在编辑器中编辑或通过编辑器上传新文件。
