...
* 点击导航元素以展开或折叠。
* 点击页面将在编辑器中打开它。
* 点击搜索 icon，或按下 <kbd>Cmd</kbd> + <kbd>P</kbd>（macOS）或 <kbd>Ctrl</kbd> + <kbd>P</kbd>（Windows），按文件名搜索文件。搜索将精确匹配文件名，因此请在单词之间使用连字符（例如，使用 `get-agent-job` 而不是 `get agent job`）。

<div id="view-unused-pages">
  ### 查看未使用页面
</div>

侧边栏 **Settings** 部分底部的 **Unused pages** 区域会显示存在于你的存储库中、但未包含在导航中的页面和其他文件。这些是[隐藏](/zh/organize/hidden-pages)页面，在你将它们添加到导航之前，不会出现在已发布站点中。
...
可以将 **Unused pages（未使用页面）** 区域中的页面添加到导航中。

**拖放操作：**

1. 在 **Unused pages（未使用页面）** 区域中找到该页面。
2. 将页面拖拽到导航树中目标位置。

**在导航树中：**

1. 在你想添加文件的位置点击加号按钮 <Icon icon="plus" />。
2. 点击 **Add existing file（添加已有文件）**。
3. 选择你想要添加的页面。
...
要在不删除文件的情况下将页面从导航中移除，请右键单击该页面并单击 **Hide Page**。该页面会移动到 **Unused pages** 区域，并且不会出现在已发布的网站上。文件仍保留在你的存储库中，你可以稍后将其重新添加到导航中。