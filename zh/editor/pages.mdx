...
* **插入或编辑链接**：选中文本并按 <kbd>Cmd</kbd> + <kbd>K</kbd>，或点击已有链接以打开链接编辑器。链接编辑器包含两个字段：
  * **Page or URL**：开始输入以搜索项目中的页面，或粘贴外部 URL。使用方向键选择结果并按 <kbd>Enter</kbd>，或在输入完 URL 后按 <kbd>Enter</kbd> 保存。
  * **Link title**：编辑显示在页面上的链接文本。按 <kbd>Enter</kbd> 保存，或按 <kbd>Esc</kbd> 关闭编辑器而不保存。点击 **Remove link** 以取消链接。

  你也可以将页面从导航树中拖入编辑器，以使用该页面标题创建链接。
