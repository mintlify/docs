---
title: "创建和编辑页面"
description: "了解如何在 Mintlify Web 编辑器中创建、编辑、复制和组织文档页面，管理文件和媒体，并搜索页面内容以维护完整的文档站点。"
...
复制页面可将现有页面的结构和内容作为新页面的起点。副本会保留页面正文、标题、描述和图标，标题末尾会添加 `(copy)`，方便你识别和重命名。

你可以通过以下任一方式复制页面：

* 打开页面，点击顶部栏中的 **More options**，然后选择 **Duplicate**。
* 在 **Navigation** 或 **Files** 视图中右键点击页面，然后选择 **Duplicate**。

复制当前打开的页面时，副本会包含未保存的更改。如果原页面已列入导航，编辑器会将副本放在文件树和导航中原页面旁边。检查副本，更新页面设置，然后准备好后发布。

<Note>
  你需要编辑权限，并且当前分支必须允许修改。复制功能适用于 Markdown 和 MDX 页面，无需额外配置。
</Note>
