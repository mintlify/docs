---
title: "创建和编辑页面"
description: "在 Mintlify Web 编辑器中创建和编辑文档页面，管理文件与文件夹，使用可视化或 Markdown 模式编辑内容，配置页面设置，插入图片、视频和外部媒体。了解如何搜索文件、管理未列出的页面、调整导航顺序、设置页面标题、Slug、描述和布局，以及使用版本历史查看页面保存记录。"
keywords: ["编辑器","内容","页面","文件","版本历史"]
---
...
* **添加嵌入内容**：输入 <kbd>/embed</kbd> 并粘贴 YouTube、Loom 或 Vimeo 的 URL 即可嵌入视频，或切换到 **手动嵌入** 以编写自定义 HTML。
...
使用版本历史查看正在编辑的页面的已保存版本。

当编辑器保存的内容与最近记录的版本不同时，Mint 会记录新条目。保存未更改的内容不会创建重复条目。保存后的页面也可以为空。

版本历史按日期对条目进行分组。每条记录显示作者和保存时间。Mint 将最新记录标记为 **Current**。

要打开版本历史：

1. 在编辑器中打开文档页面。版本历史不适用于 `docs.json`。
2. 在右上角点击 **More options**。
3. 选择 **Version history**。

当页面支持版本历史时，你还可以点击编辑器标题栏中的 **Edited** 时间戳以打开抽屉。对于不支持版本历史的页面，该时间戳仍不可交互。

抽屉显示当前页面的版本。公开页面的版本历史仅包含当前分支，私有页面的版本历史则包含所有分支的记录。查看者可以查看和比较私有页面的已保存版本，编辑者和管理员可以恢复这些版本。点击 **Show more versions** 加载更早的记录。

当你拥有页面编辑权限时，较早的版本会显示 **Restore** 操作。点击 **Restore** 并确认，即可使用该保存版本替换当前页面内容。此操作会覆盖未保存的更改。

Restore 会将所选快照应用到当前页面。对于公开页面，它会恢复当前分支上的页面。对于私有页面，它会更新共享的私有内容。

要将较早的保存版本与当前页面进行比较，请点击该版本所在的行。编辑器会打开 diff 视图，将保存版本作为基准，并与当前页面内容进行比较。活动行会显示 **Viewing**。再次点击活动行即可退出比较并返回编辑。

被阻止的部署分支和 **Current** 版本不提供 Restore 操作。
