...
<Note>
  导航标签页按标题列出页面。文件标签页按文件名和扩展名列出页面。
</Note>
...
* **Slug**：设置页面的 URL 路径。Slug 默认为相对于存储库根目录的路径，不含文件扩展名。例如，`guides/getting-started.mdx` 变为 `guides/getting-started`。更改 slug 会重命名并移动文件以匹配新路径。
