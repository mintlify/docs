...
右键点击页面并选择 **Unlist**，或打开其设置面板并点击删除按钮旁的取消列出图标。编辑器会将该页面从导航中移除，但会将文件保留在仓库中，因此知道 URL 的人仍可以访问。可将其用于不希望出现在侧边栏中的[隐藏页面](/zh/organize/hidden-pages)。要重新列出某个页面，请从某个导航元素的 **Add existing file** 菜单中重新添加它。
