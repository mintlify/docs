...
当编辑器保存的内容与最近记录的版本不同时，Mint 会记录新条目。保存未更改的内容不会创建重复条目。保存后的页面也可以为空。每条记录显示作者和相对保存时间。最新记录标记为 **Latest**。
...
当页面支持版本历史时，你还可以点击编辑器标题栏中的 **Edited ... ago** 标签以打开抽屉。对于不支持版本历史的页面，该标签仍不可交互。

抽屉显示当前页面的版本。公开页面的版本历史仅限于当前分支，而私有页面的版本历史在各分支之间共享。查看者可以查看和比较私有页面的已保存版本，编辑者和管理员可以恢复这些版本。点击 **Show more versions** 加载更早的记录。

当你拥有页面编辑权限时，较早的版本会显示 **Restore** 操作。点击 **Restore** 并确认，即可使用该保存版本替换当前页面内容。此操作会覆盖未保存的更改。

Restore 会将所选快照应用到当前页面。对于公开页面，它会写入当前站点、部署和分支。对于私有页面，它会更新共享的私有内容，而不是特定分支的副本。它不会创建新页面，也不会只是预览保存的内容。

要将较早的保存版本与当前页面进行比较，请点击该版本所在的行。编辑器会打开 diff 视图，将保存版本作为基准，并与当前页面内容进行比较。活动行会显示 **Viewing**。再次点击活动行即可退出比较并返回编辑。

被阻止的部署分支和 **Latest** 版本不提供 Restore 操作。
