...
<Note>
  导航标签页按页面标题列出页面。文件标签页按文件名和扩展名列出页面。
</Note>
...
* **添加嵌入内容**：输入 <kbd>/embed</kbd> 来插入 YouTube、Loom 或自定义 HTML 的嵌入内容。详情参见 [Add media](/zh/editor/media)。
...
* **Slug**：设置页面的 URL 路径。slug 默认为相对于存储库根目录的路径（不含文件扩展名）。例如，`guides/getting-started.mdx` 会变成 `guides/getting-started`。更改 slug 会重命名并移动文件以匹配新路径。
