...
要从侧边栏提升私有页面或文件夹，请打开其 **More options** 菜单，然后选择 **Promote to workspace**。此操作适用于你拥有的 **Private** 分区中的项目，且你需要对该项目及其所有后代拥有 Manager 权限。确认后，Mintlify 会将页面或文件夹添加到工作区根目录。提升文件夹时，其所有嵌套文件夹和页面也会一并提升，重新创建原有结构并复制内容。Mintlify 会根据页面标题生成路径，并自动解决重复路径。确认对话框会提示提升操作不可撤销。你无法将已提升的内容移回私有页面。
