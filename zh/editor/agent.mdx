...
当智能体在继续之前需要更多指引时，会在聊天中提出问题。选择建议的选项之一；如果问题允许，也可以在聊天框中输入自己的回答。点击 **Skip** 让智能体在没有该额外信息的情况下继续。

问题会保持打开状态，直到您回答或跳过为止。待回答的问题在智能体完成回复后、页面重新加载后，以及从[会话历史](#session-history)返回对话时仍然可用。您的回答会继续同一段对话。
