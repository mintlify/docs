...
输入 <kbd>@</kbd> 提及特定页面，智能体就会聚焦该页面。如果没有提及，它会使用你当前打开的页面。在仪表板设置页面上，智能体知道你正在查看哪个设置界面，因此你无需说明名称即可询问当前可见的设置。
...
当智能体在继续之前需要更多指引时，会在聊天中提出问题。选择建议的选项之一；如果问题允许，也可以在聊天框中输入自己的回答。点击 **Skip** 让智能体按照问题中说明的默认值继续。

有问题待回答时，智能体会暂停工作。它会等待您的回复，而不是凭猜测继续。问题会保持打开状态，直到您回答或跳过为止。待回答的问题在智能体完成回复后、页面重新加载后，以及从[会话历史](#session-history)返回对话时仍然可用。您的回答会继续同一段对话。

跳过和未回答的问题会以可折叠摘要的形式显示在聊天中。展开摘要并点击 **Answer questions** 即可重新打开这些问题。
