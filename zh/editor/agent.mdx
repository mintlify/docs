---
title: "向代理提问"
description: "使用 Mintlify 编辑器内置的 AI 代理完成日常文档工作：撰写和编辑页面内容、重新组织导航并更新 docs.json 配置、留下评论和修改建议、跨整个文档仓库搜索内容、上传图片与附件文件，并通过 code mode 代你编写脚本管理控制台设置与部署相关配置。"
keywords: ["编辑器", "AI", "助手", "代理", "聊天", "评论", "建议", "code mode", "文件上传"]
---
...
最大大小：每个文件 5 MB，SVG 文件除外，其上限为 256 KB。最多：每条消息 10 个文件。

在代理读取 SVG 文件之前，Mintlify 会移除其中的脚本和其他活动内容。
...
代理默认会直接编辑内容。如果你希望获得反馈而不立即修改页面，请明确要求代理审阅页面、留下评论或提出建议。

- [建议](/zh/editor/suggestions)会提议一处具体的替换，你可以接受或拒绝。在你做出决定之前，原文会以删除线的形式保持可见，代理还可以在建议线程中附上说明。
- [评论](/zh/editor/comments)会针对具体文本留下反馈或问题，不会修改页面本身。
...
除非你指定另一页，否则代理会使用你当前打开的页面。它可以标注页面上渲染出的文本，包括组件内的正文，但不能标注 frontmatter、页面元数据、配置或 MDX 组件的原始标签与属性。
