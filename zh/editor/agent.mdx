...
智能体可以回答有关文档的流量、搜索、assistant 和反馈数据的问题。智能体会以文字、图表或排名列表的形式返回信息。使用智能体请求具体的解释、对比页面表现,或调查流量变化。
...
如果您安装了 [Mintlify Slack 应用](/zh/agent/slack)，编辑器智能体可以在研究或回答您的问题时，读取您的 Slack 工作区作为上下文。编辑器智能体对您的工作区只有只读访问权限。它无法在您的工作区中发送消息、添加表情反应或执行其他操作。

在请求中让智能体使用 Slack。例如：

- `查看 #engineering 频道，获取有关新认证流程的上下文`
- `总结团队昨天关于速率限制做出的决定`

如果您尚未安装 Slack 应用，请让智能体为您连接。智能体会返回一个授权链接，并引导您完成安装流程。
