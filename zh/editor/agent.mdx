---
title: "使用编辑器 AI 代理编辑和管理页面、更新导航、运行命令、管理私有页面并配置 Mintlify 站点"
...
每个文件最大为 5 MB；每条消息最多 10 个文件。

<div id="add-a-selection-to-the-agent">
  ## 将选中内容添加到代理
</div>

在可视化模式下，选中页面上的文本。选区上方会出现一个浮动工具栏。点击 **Add to agent**，将所选文本作为上下文发送给代理。
...
代理可以对你的仓库运行 `grep`、`rg` 和其他 bash 命令，用于跨多个文件执行批量操作。
...
如果你有权访问私有页面，代理可以从私有页面根目录读取和编辑这些页面。你可以使用此功能查看或更新不应出现在公共导航中的内容。编辑器无需额外启用功能开关即可使用私有页面。你的角色和页面权限决定你可以访问和修改哪些页面。

你可以从私有页面编辑器中打开代理。具有 viewer 角色的用户不会看到代理按钮。当代理或其他编辑者更改私有页面树时，私有导航会自动刷新。

私有页面分为侧边栏中的两个分区。**Personal** 包含你拥有的页面和文件夹。**Shared with me** 包含与你共享的页面和文件夹。每个分区都可以独立折叠。使用 **Personal** 标题上的添加菜单，或右键点击该标题，创建私有页面或文件夹。**Shared with me** 标题没有添加菜单。

要从侧边栏提升私有页面或文件夹，请打开其 **More options** 菜单，然后选择 **Promote to workspace**。此操作适用于 **Personal** 分区中的项目，前提是你对该项目及其所有后代拥有 Manager 权限。确认后，Mintlify 会将页面或文件夹添加到工作区根目录。提升文件夹也会提升所有嵌套文件夹和页面。Mintlify 会重新创建它们的结构并复制其内容。Mintlify 会根据标题生成页面路径，并自动解决重复路径。确认对话框会警告提升操作不可撤销。你无法将已提升的内容移回私有页面。
...
在代理聊天框中输入 `@`，即可按标题搜索私有页面。私有页面会在提及选择器中显示为 `Private page`。选择一个页面后，其提及标签不会链接到该私有页面。代理会将提及解析为其 UUID 路径。列表中只包含你可以访问的页面。

要在 **Personal** 中创建私有文件夹，请使用人类可读的标题作为路径的最后一段。再次列出目录后，文件夹会显示为新的 `private-folder-<uuid>` 路径：
...
在 `/home/daytona/private` 中，使用 `mv` 将 **Personal** 中的私有页面或文件夹重新归入另一个私有文件夹。该项目会保留其基于 UUID 的名称。此操作需要 Manager 权限。你无法重命名页面，也无法将文件夹移动到自身内部：
...
要复制私有页面，请使用 `cp`，并将人类可读的标题作为路径的最后一段。再次列出目录后，副本会显示为新的 `private-page-<uuid>.mdx` 路径：
...
你只能复制私有页面。不支持复制文件夹，也不支持在私有根目录和工作区根目录之间复制。
...
移动操作会将该页面添加到公共导航中，并撤销其私有页面的访问权限。你无需配置其他文件系统或额外授权。代理会使用你现有的私有页面权限。

<div id="configure-your-site-code-mode">
  ### 配置你的站点（code mode）
</div>

对于超出页面编辑范围的请求，例如设置身份验证、管理工作流或更改部署设置，代理会切换到 code mode。它会代表你针对 Mintlify 控制台编写并运行脚本。
...
当你通过点击 **View changes** 从已完成的自动化运行中打开编辑器时，代理面板会自动打开。面板会包含有关该自动化所执行操作的上下文。聊天顶部会显示一张 **Changed pages** 卡片，列出该自动化修改的每个页面。点击任意页面可打开差异视图。

代理可以了解该自动化的提示词和所做更改的摘要。它还知道自己修改了哪些页面。你可以让它细化或扩展该自动化的工作，无需再次说明背景。
...
要为代理提供持久性指引，请在[编辑器设置](/zh/editor/settings#ai-instructions)中配置 AI 指令。使用这些指令定义语气规则、术语或格式约定。代理会在每次请求时遵循这些指令，无需你重复说明。
