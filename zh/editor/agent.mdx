...
如果您的组织已 [连接 Mintlify Slack 应用](/zh/agent/slack)，编辑器智能体可以在工作时从 Slack 读取内容。请智能体从频道、话题串或同事那里获取上下文，以支持它所做的修改。

示例提示：

- `查看 #launches 中关于账单改版的最新细节，并更新定价页面`
- `总结 #eng 中的 API 设计话题串，并把决策加入到 API 概览中`
- `搜索团队由谁负责？在 Slack 中找到他，并在评审说明中提到他`

编辑器智能体对 Slack 具有只读访问权限。它可以列出并搜索该应用所在的频道、获取消息和话题串，并查询工作区成员，但绝不会代您发送消息或添加反应。

如果 Slack 尚未连接，请让智能体去连接。智能体会返回一个 **Connect to Slack** 链接以启动安装流程。工作区所有者完成安装后，再发送一条消息，智能体就能从 Slack 读取内容。

Slack 访问权限取决于您在控制台中的角色。如果您的角色无法在控制台中读取或安装 Slack 集成，智能体同样无法进行。
