...
开启该选项后，编辑器会在你停止编辑后自动将部署分支（通常是 `main`）上的更改提交到 Git。关闭时，部署分支上的更改会在编辑器中保持挂起状态，直到你手动发布。

当你的团队直接在部署分支上编辑，并希望更新在无需手动发布的情况下进入 Git 时，请启用此设置。如果你希望在更改进入部署分支之前先在编辑器中审阅，请保持关闭。

此设置仅影响部署分支。从编辑器创建的功能分支上的编辑无论此设置如何都会继续自动提交，并且锁定部署分支具有更高优先级。
