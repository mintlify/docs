---
title: "编辑器外观、AI 与发布设置"
description: "在 Mintlify Web 编辑器中配置外观偏好、AI 指令、pull request 默认值和合并方式，管理编辑体验、编辑器行为、提交变更以及发布流程，并了解如何安全处理未发布的修改、调整编辑器布局和查看实时预览，并掌握外观、聊天、发布和危险区域设置的作用。"
keywords: ["编辑器", "设置", "ai", "指令", "发布", "pull request", "合并", "锁定分支"]
---

编辑器设置分为四个部分：

- **外观**和**聊天**仅对你生效。
- **发布**对部署中的所有人都生效，并控制提交变更及将其转换为 pull request 时的处理方式。
- **危险区域**包含可能丢弃未发布变更的操作。

你可以在设置面板中配置这些选项。打开编辑器工具栏中的 <Icon icon="settings" /> 设置图标。设置页面会显示全部四个部分。

<div id="appearance">
  ## 外观
</div>

外观设置控制你工作时编辑器的显示方式。这些偏好仅对你生效。

<div id="fit-editor-to-full-width">
  ### 让编辑器适应全宽
</div>

当你需要更多水平空间来编辑宽内容或源文件时，启用 **Fit editor to full width**。编辑器会将编辑画布扩展到整个宽度，而不是使用默认的 768px 列宽。

<div id="open-live-preview-in-a-new-tab">
  ### 在新标签页中打开实时预览
</div>

当你希望保持编辑器打开，同时在独立的浏览器标签页中查看渲染后的页面时，启用 **Open live preview in new tab**。预览网站时，Mintlify 会在新标签页中打开渲染后的页面。

<div id="show-all-files">
  ### 显示所有文件
</div>

启用 **Show all files** 可显示点文件、代码、PDF 以及默认隐藏的其他文件。Mintlify 会在导航侧边栏中显示这些文件。你也可以在 macOS 上按 <kbd>⌘⇧.</kbd>，或在其他平台上按 <kbd>Ctrl+Shift+.</kbd> 来切换此偏好设置。

配置外观偏好设置：

1. 点击编辑器工具栏中的 <Icon icon="settings" /> 设置图标。
2. 找到 **Appearance** 部分，或滚动到该部分。
3. 打开或关闭偏好设置。
...
1. 点击编辑器工具栏中的 <Icon icon="settings" /> 设置图标。
2. 在 **AI instructions** 字段中输入希望 AI 遵循的指令。
3. 保存更改。
...
当你的团队希望 Git 继续作为线上站点的唯一可信来源，并让所有编辑器更改都通过从功能分支发起的 pull request 提交时，可使用此设置。
