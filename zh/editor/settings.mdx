...
- **Publishing**：整个 deployment 范围内的默认设置，决定编辑器如何提交更改和打开 pull request，对所有发布者生效。更改这些设置需要具备更新 deployment 的权限。如果这些控件对你处于禁用状态，请联系管理员。
...
选择性发布让你在直接发布到 deployment branch 时选择要包含哪些待处理更改。启用该设置后，发布流程会在每个待处理更改旁边显示复选框。取消勾选你想暂缓的更改。取消勾选的更改会保持待处理状态，你可以稍后再发布。该设置默认关闭。

选择性发布仅在你直接在没有分支保护的 deployment branch 上发布时生效。相互依赖的更改会一起选中和取消选中。例如，删除页面会在同一个选择中同时移除页面文件及其导航条目。

如果所选更改依赖的某个更改未被选中，Mintlify 会在创建提交之前拒绝本次发布。在你调整选择并重新发布之前，任何更改都不会进入你的站点。
