...
- **Show all files**：侧边栏默认只列出 Markdown 页面。启用此选项可同时显示图片、视频、规范和数据文件、代码、PDF 以及 dotfile。你也可以按 <kbd>Cmd</kbd> + <kbd>Shift</kbd> + <kbd>.</kbd>（macOS）或 <kbd>Ctrl</kbd> + <kbd>Shift</kbd> + <kbd>.</kbd>（Windows）进行切换。
