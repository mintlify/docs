...
开启 **Auto publish** 可完全跳过分支和发布步骤。编辑器会在你停止编辑后不久，直接将更改提交到部署分支，站点重新构建后这些更改即刻上线。

当 Auto publish 处于开启状态时：

- 编辑器会隐藏分支选择器和 **Publish** 按钮。
- 每次编辑都会写入部署分支。没有草稿状态，也没有审阅环节。
- 提交部署后，所有读者立刻能看到更改。

当你的文档是 wiki 风格、更看重速度而非审阅，或者小团队希望更改直接进入 Git 而不用打开 pull request 时，请使用 Auto publish。如果你希望在更改发布前进行审阅，或团队依赖草稿分支和预览 URL，请保持关闭。

如果你之前使用 **主分支自动提交**，你的 deployment 会被迁移到 Auto publish，行为保持一致——提交直接进入部署分支——只是编辑器中不再有发布步骤。
...
新的 deployment 默认开启此设置。现有 deployment 保留你原有的取值。
