...
使用侧边栏文件浏览器查看文档文件。单击任意文件即可在编辑器中打开。
...
<Steps>
  <Step title="创建新文件">
    ...
    <img
      ...
      alt="深色模式下的 Web 编辑器中的文件菜单"/>
  </Step>
  <Step title="输入文件名">
    文件名必须以 `.mdx` 结尾。例如：`api-reference.mdx` 或 `getting-started.mdx`。
  </Step>
  <Step title="创建并打开">
    按 <kbd>Enter</kbd> 创建文件。文件将自动在编辑器中打开，随后即可添加内容。
  </Step>
</Steps>
...
<Steps>
  <Step title="Open docs.json">
    使用文件管理器在根目录中找到并打开 `docs.json` 文件。
  </Step>
  <Step title="Locate the navigation section">
    在 `docs.json` 文件中找到你要添加新页面的位置。
  </Step>
...
在可视化模式下，按 <kbd>/</kbd> 打开组件菜单。添加内容块、提示框、代码块及其他组件，自定义你的文档。

<Frame>
  <img 
    src="/images/editor/component-menu-light.png" 
    alt="Mintlify Web 编辑器中展开并高亮显示的组件菜单" 
...
在 Markdown 模式下，你可以直接编辑页面的 `MDX`。以下情况会很有帮助：
