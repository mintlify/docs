...
<Tip>
  使用描述性且富含关键词的标题，清晰指示后续内容。这将同时提升用户导航体验和搜索引擎优化效果。
</Tip>
...
默认情况下，标题会包含可点击的锚点链接，允许用户直接跳转到特定章节。你可以在 HTML 或 React 标题中使用 `noAnchor` 属性来禁用这些锚点链接。
...
链接可帮助用户在页面之间导航并访问外部资源。使用清晰、具描述性的链接文本，以提升可访问性和用户体验。

<div id="internal-links">
  ### 内部链接
</div>

使用以根目录为基准的路径链接到文档中的其他页面：
...
<Note>
  请避免使用类似 `[page](../page)` 的相对链接，因为它们加载更慢，且无法像根路径相对链接那样得到同等程度的优化。
</Note>
...
<Tip>
  谨慎使用引用，以保持其视觉效果和语义。对于注释、警示和其他信息，建议使用[callouts](/zh/components/callouts)。
</Tip>
...
<Info>
  使用 LaTeX 时需遵循规范的数学语法。有关完整的语法指南，请参阅 [LaTeX 文档](https://www.latex-project.org/help/documentation/)。
</Info>
