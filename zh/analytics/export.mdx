...
助手导出包含每个问题的查询、响应和来源,以及以下列。

| 列 | 描述 |
| --- | --- |
| `resolutionStatus` | 助手是否回答了该问题:`answered` 或 `unanswered`。可利用它识别助手无法解决的问题所暴露出的文档缺口。 |
| `feedback` | 用户对该响应的点赞或点踩评价:`positive` 或 `negative`。用户未评价时为空。 |
...
<Tip>
  ...
  - 汇总 `feedback` 为 `negative` 的响应中引用的来源。
</Tip>
