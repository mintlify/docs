...
助手导出包含查询、响应、来源、`resolutionStatus` 列以及 `feedback` 列。

- `resolutionStatus` 表示助手是否成功回答了每个问题,其值为 `answered` 或 `unanswered`。可利用它识别助手无法解决的问题所暴露出的文档缺口。
- `feedback` 记录用户对助手响应的点赞或点踩评价,值为 `positive` 或 `negative`。用户未评价的响应此列为空。如果用户重新生成回答并再次评价,则使用最新的评价。
