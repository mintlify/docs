...
keywords: ["AI", "助手", "代理", "llms.txt", "MCP", "llms-full.txt"]
---
...
除了阅读单个页面，用户还可以在你的文档中与[AI 助手](/zh/assistant)聊天，及时获得问题的答案和相关内容的链接。AI 助手会基于你的文档提供准确信息，帮助用户更好地了解你的产品。你也可以通过[API](/zh/api/assistant/create-assistant-message-v2)将 AI 助手嵌入自定义应用，扩展用户访问你文档的触达渠道。
...
配置常用工具（如 [Cursor](/zh/guides/cursor)、[Claude Code](/zh/guides/claude-code) 和 [Devin Desktop](/zh/guides/devin-desktop)），以参考 Mintlify schema、你的风格指南和最佳实践。

<div id="discovering">
  ### 发现
</div>

你的网站会自动针对 AI 工具和搜索引擎进行优化，帮助用户更容易找到你的文档。所有页面都会以 Markdown 而非 HTML 的形式发送给 AI 代理，这有助于这些工具更快处理你的内容并使用更少的令牌。通过在 URL 末尾追加 `.md`，每个页面也都可以以 Markdown 形式查看。使用 [visibility](/zh/components/visibility) 组件为不同受众定制内容。
...
<Tip>
  ...
  ```mdx
  <Prompt description="为你的产品安装 skill。" actions={["copy", "cursor"]}>
...
对于跨越多个产品或需要网页搜索的实现问题，[Mintlify Index](/zh/search-index) 为编码代理提供一个 MCP 服务器和 REST API，用于从发布者维护的文档和 Web 中检索上下文。
...
<CardGroup>
  <Card title="AI 助手" icon="bot-message-square" href="/zh/assistant/configure">
    配置 AI 助手以搜索外部网站，或在其无法回答问题时将用户引导至你的支持团队。
  </Card>

  <Card title="自动化" icon="message-circle-plus" href="/zh/automations/index">
