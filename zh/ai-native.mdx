...
你的网站会自动针对 AI 工具和搜索引擎进行优化，帮助用户更容易找到你的文档。通过在 URL 末尾追加 `.md`，或发送 `Accept: text/markdown` 请求头，每个页面都可以以 Markdown 形式获取，这有助于 AI 工具更快处理你的内容并使用更少的 token。使用 [visibility](/zh/components/visibility) 组件为不同受众定制内容。
