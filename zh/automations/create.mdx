...
通过手动运行来测试新的自动化，这样就无需等待其触发器发生。手动运行使用自动化的当前配置，并与自动运行一起显示在运行历史中。由集成事件触发的自动化无法手动运行。要为测试运行选择范围，请参见[手动运行自动化](/zh/automations/manage#run-an-automation-manually)。
