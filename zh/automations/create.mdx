...
自定义自动化使用你编写的指令，按计划、在仓库推送时、在已连接的集成事件发生时或在 Slack 频道收到新消息时运行 agent。当[预定义自动化](/zh/automations/reference)无法覆盖你需要的行为，或者你希望将自己的提示词与特定的上下文仓库和集成组合在一起时，使用自定义自动化。有关所有自动化共享的触发器、更新模式和配置选项的概述，请参见[自动化](/zh/automations)。
