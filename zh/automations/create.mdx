...
在依赖计划或事件触发之前先手动运行一次，可以尽早发现问题。手动运行会使用自动化的当前配置，并与计划运行一同显示在运行历史中。有关步骤，请参见[手动运行自动化](/zh/automations/manage#run-an-automation-manually)。

在 [Automations](https://app.mintlify.com/products/automations?tab=runs) 页面打开该次运行，查看提示词、agent 读取或更改的文件，以及它打开的任何拉取请求。如果结果与预期不符，请编辑指令并再次运行。
