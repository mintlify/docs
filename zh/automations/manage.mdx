...
4. 选择运行范围，然后点击 **Run now**。

手动运行会在开始前保存自动化的当前配置，会计入你的积分使用量，并与按计划运行的记录一起出现在运行历史中。运行开始后，点击确认提示中的 **View runs**，即可打开按该自动化筛选的运行历史。

使用**集成**触发器的自动化无法手动运行。它们在触发器触发时运行。

<div id="scope-a-manual-run">
### 限定手动运行的范围
</div>

每次手动运行只使用一个范围，在启动运行时选择：

- **自某个日期起**：审查从所选日期到现在更改的内容。默认为该自动化的上次运行时间。
- **全部**：在整个站点上运行，不受日期限制。这通常比限定范围的运行耗时更长、使用更多积分。
- **特定拉取请求**：将运行限定为已连接源代码仓库中的一个拉取请求。选择仓库，然后按标题或编号搜索其拉取请求。对于使用**代码更改**触发器的自动化，该仓库必须是已配置的源仓库之一。

你也可以让[代理](/zh/editor/agent)启动限定范围的运行。例如：`run the changelog automation over the last two weeks` 或 `run the update from code changes automation on PR #42 of acme/api`。
