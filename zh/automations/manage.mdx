...
- **Slack 频道消息（Slack channel message）**：当有人在所选 Slack 频道中发布新的顶层消息时运行。此选项适用于自定义自动化。将 [Mintlify Slack 应用](/zh/agent/slack)邀请到你想要选择的每个频道，然后在设置面板中选择这些频道。如果该应用不是所选频道的成员，设置面板会阻止保存，直到你将其邀请加入。Mintlify 会跳过线程回复、来自机器人的消息，以及提及 Mintlify agent 的消息。Mintlify 会按 Slack 事件 ID 去重，因此单条消息最多只会触发每个匹配的自动化一次。当自动化执行操作时，会在触发它的线程中回复摘要，并附上它开启的任何 pull request 的链接。运行详情在仪表板中会显示触发消息以及返回 Slack 的永久链接。
