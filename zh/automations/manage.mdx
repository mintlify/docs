...
选择 **自定义计划（Custom schedule）** 后，请从以下计划选项中选择一个：
...
对于常见的重复节奏，请使用预设。若需要的计划不在预设范围内，请选择 **自定义 cron（Custom cron）**。设置面板会为预设显示易于理解的说明。只有在选择 **自定义 cron（Custom cron）** 时，才会显示 cron 输入框。

自定义 cron 表达式（Custom cron）按以下顺序使用五个字段： `minute hour day month weekday`。例如，以下表达式会在周一至周五的 UTC 9:00 运行自动化：

```text
0 9 * * 1-5
```

Mintlify 会在你保存前验证该表达式。如果现有计划与任何预设都不匹配，打开时会选中 **自定义 cron（Custom cron）**，以便你查看或编辑该表达式。
