...
- **Slack 消息（Slack message）**：当团队成员在所选的某个 Slack 频道中发布顶层消息时运行。此选项适用于自定义自动化。需要安装 [Mintlify Slack 应用](/zh/agent/slack)。选择一个或多个要监听的频道。此触发器会忽略话题串回复、编辑，以及来自机器人或 Mintlify 应用自身的消息。agent 会读取每条新消息并判断是否需要采取操作。
