...
<Tip>
  你也可以通过 [`mint automations`](/zh/cli/commands#mint-automations) 在终端中创建、列出和删除自动化。CLI 适合用于脚本和 CI。控制台是配置和监控自动化运行最简单的方式。
</Tip>
...
要更改已激活自动化的设置，点击其卡片上的 <Icon icon="settings-2" /> 设置按钮以打开其配置页面。使用页面头部的开关可以启用或禁用自动化，然后点击 **Save** 应用你的更改。

<div id="configurations">
  ## 配置
</div>

每个自动化的配置页面将设置分为 **Trigger**、**Scope** 和 **Advanced options** 三个部分。
...
- **自定义计划（Custom schedule）**：按你定义的周期性计划运行。选择一个预设（**Daily**、**Every Monday**、**Every Friday** 或 **Twice weekly**）以及起始小时，或选择 **Custom cron** 并输入标准的 5 段式 cron 表达式（`minute hour day month weekday`）。自动化会在预定时间的 10 分钟内进入队列。
- **集成（Integration）**：当已连接的共享集成中发生所选事件时运行，或当在所选 Slack 频道中发布新消息时运行。此选项适用于自定义自动化。请选择集成和事件，并填写出现的任何其他事件字段。对于 Slack 触发器，请选择一个或多个已添加 [Mintlify Slack 应用](/zh/agent/slack)的频道。
- **Webhook**：当一个已认证的 `POST` 请求命中自动化的 webhook 端点时运行。仅适用于自定义自动化。先保存自动化；触发器卡片随后会显示 webhook URL 和用于 `Authorization: Bearer <api-key>` 请求头的 **Copy auth header** 操作。在 [API keys](https://app.mintlify.com/settings/organization/api-keys) 页面提供一个具有写入权限且未过期的组织 API 密钥。可用于从 CI/CD 流水线、发布脚本或内部工具触发运行。有关端点和速率限制，请参见[触发自动化 webhook](/zh/api/automations/webhook)。
...
每个仓库最多支持 20 条路径和 20 个被排除的作者。更换自动化的仓库会清除其过滤器。
...
对于自定义自动化和其他受支持的预定义自动化，你可以启用[集成](/zh/automations/integrations)，让 agent 在运行时从 Notion、Jira 或 Linear 等共享工具获取上下文。

要为某个自动化启用集成，打开其配置页面，展开 **Advanced options**，然后在 **Integrations** 中选择你想使用的集成。

如果为自定义自动化选择 **集成（Integration）** 作为触发器，触发该自动化的集成始终是该自动化的工具之一。有关连接范围、支持的事件和权限，请参见[集成](/zh/automations/integrations)。
...
2. 在控制台的 [Automations](https://app.mintlify.com/products/automations) 页面点击 **Configure Slack**。
3. 选择一个或多个用于接收通知的频道。
4. 点击 **Save changes**。
...
若要在自动化运行成功或失败时收到电子邮件，请在控制台的[通知](/zh/dashboard/notifications)页面开启自动化运行邮件。
...
1. 在控制台中打开 [Automations](https://app.mintlify.com/products/automations) 页面。
2. 点击自定义自动化卡片上的 <Icon icon="settings-2" /> 设置按钮以打开其配置页面。
3. 点击页面底部的 **Delete automation** 并确认。

要在终端中删除任意自动化，请使用 [`mint automations`](/zh/cli/commands#mint-automations)。删除操作是永久性的，无法撤销。
...
1. 在控制台中打开 [Automations](https://app.mintlify.com/products/automations) 页面。
2. 点击自动化卡片上的 <Icon icon="settings-2" /> 设置按钮以打开其配置页面。
3. 点击运行按钮（根据自动化不同，为 **Test run** 或 **Run now**）。
4. 选择运行范围。
...
对于使用**自定义计划**触发器的自动化，你可以从自己的工具中启动一次运行，而无需等待下一次计划时间。使用 [Trigger automation](/zh/api/automations/trigger) 端点，可以从 CI/CD 流水线、发布脚本或任何可以发起经过身份验证的 HTTP 请求的服务触发一次运行。
...
具有 **Webhook** 触发器的自定义自动化会在已认证的 `POST` 请求到达其 webhook 端点时运行。保存自动化后，打开其配置页面即可复制 webhook URL 并查看 `Authorization: Bearer <api-key>` 请求头模板。将 `<api-key>` 替换为在 [API keys](https://app.mintlify.com/settings/organization/api-keys) 页面创建的、具有写入权限且未过期的组织 API 密钥。自动化不会为你创建、存储或轮换密钥。

通过 webhook 触发的运行使用自动化保存的提示词，读取完整的仓库历史，并在运行历史中以 **Webhook request** 标签显示。有关请求格式、响应代码和速率限制，请参见[触发自动化 webhook](/zh/api/automations/webhook)。

<div id="view-run-history">
  ## 查看运行历史
</div>

自动化页面上的 [Runs](https://app.mintlify.com/products/automations?tab=runs) 标签页会显示所有自动化的全部运行列表。

一次运行是自动化的一次执行。一次运行可以创建新的 pull request、更新现有的 pull request、运行失败，或未发现需要更改的内容。

1. 进入控制台中的 [Automations](https://app.mintlify.com/products/automations?tab=runs) 页面。
...
你可以直接在列表中对结果执行操作。对于等待审查的运行，点击 **Accept** 合并更改，或点击 **Preview** 打开该运行分支的 [preview deployment](/zh/deploy/preview-deployments)。要在编辑器中打开该运行的分支，请点击 **View changes**，或打开 <Icon icon="ellipsis" /> 菜单并点击 **Open in editor**。对于失败的结果，点击 **Re-trigger** 重新开始运行。

只有当运行关联了 preview deployment 时，才会显示 **Preview** 按钮。preview deployment 在闲置一段时间后会过期。过期后，该按钮会显示 **Preview expired** 且不再可用。

<div id="resolve-merge-conflicts">
  ### 解决合并冲突
</div>

如果某次运行的 pull request 与你的部署分支存在冲突，当你点击该自动化运行的 **Accept** 时，agent 会提示你解决冲突。

点击 **Resolve and merge**，agent 会将部署分支的最新更改合并到 pull request 分支、解决冲突并自动合并该 pull request。如果 agent 无法解决冲突，请在你的仓库中手动解决冲突，然后点击 **Accept**。

要查看某次运行的提示词、读取或更改的文件，以及任何 pull request，请点击该运行的 <Icon icon="ellipsis" /> 操作菜单，然后点击 **View run details**。
...
1. 在控制台中打开 [Automations](https://app.mintlify.com/products/automations) 页面。
2. 在 **Runs** 标签页中，点击该运行上的 **View changes**，或打开 <Icon icon="ellipsis" /> 操作菜单并点击 **Open in editor**。这两个选项仅在该运行的 pull request 仍然开启时可用。
