...
4. 选择运行范围，然后点击 **Run now** 确认。确认之前不会运行任何内容。

每次手动运行覆盖以下三种范围之一：

- **Since a date**：覆盖从你选择的日期到现在期间更改的内容。默认为该自动化的上次运行时间。
- **Everything**：在整个站点上运行。通常比定向运行耗时更长，消耗的用量也明显更多。
- **Specific pull request(s)**：将运行限制为你在已连接仓库中选择的 pull request。

手动运行会在开始前保存自动化的当前配置，会计入你的积分使用量，并与按计划运行的记录一起出现在运行历史中。

使用 **Integration** 触发器的自动化无法手动运行。它们会在触发事件发生时运行。如果运行按钮不可用，将鼠标悬停在按钮上即可查看原因。
