...
要更改已激活自动化的设置，点击其卡片上的 <Icon icon="settings" /> 设置按钮以打开其配置页面。使用页面头部的开关可以启用或禁用自动化，然后点击 **Save** 应用你的更改。
...
打开自动化的配置页面，并在 **Tools** 中选择你要使用的集成。你可以直接从选择器连接尚未连接的共享集成。个人集成仅供其所有者在使用 Slack agent 时使用，不能添加到自动化。
...
1. 在控制台中打开 [Automations](https://app.mintlify.com/products/automations) 页面。
2. 点击自定义自动化卡片上的 <Icon icon="settings" /> 设置按钮以打开其配置页面。
...
手动运行会在开始前保存自动化的当前配置，会计入你的积分使用量，并与按计划运行的记录一起出现在运行历史中。

大多数自动化仅在配置了计划触发器时才能手动运行。如果运行按钮不可用，将鼠标悬停在按钮上即可查看需要更改的内容。**Translate content** 自动化始终可以手动运行。

<div id="scope-a-manual-run">
### 限定手动运行的范围
</div>

通过运行按钮启动的手动运行会审查自该自动化上次完成运行以来的所有更改。要以不同的范围运行自动化，请让[代理](/zh/editor/agent)启动运行。代理支持三种范围：

- **日期范围**：审查特定时间窗口内的更改，而不是默认窗口。例如：`run the changelog automation over the last two weeks`。
- **全部**：审查所有更改，不受日期限制。例如：`run the style guide automation across everything`。
- **单个拉取请求**：基于已连接源代码仓库中的一个拉取请求运行。例如：`run the update from code changes automation on PR #42 of acme/api`。对于使用**代码更改**触发器的自动化，该拉取请求必须位于已配置的源仓库之一中。

每次运行只使用一个范围。提供范围后，你也可以手动运行没有计划触发器的自动化。限定范围的运行会计入积分使用量，并像其他运行一样出现在运行历史中。
...
每个自动化都会保存历史运行日志，包括状态和所做更改的摘要。将更改追加到先前运行打开的 pull request 的运行，会在运行历史中显示为独立条目。
...
- **Modified**：该运行将其更改追加到了先前运行打开的 pull request 中。
- **Failed**：agent 无法完成运行。失败的运行不会计入每日运行限制。
- **No action needed**：agent 完成了运行，但未发现需要更新的内容。

你可以直接在列表中对运行执行操作。对于等待审查的运行，点击 **Accept** 合并更改。对于失败的运行，点击 **Re-trigger** 重新开始运行。

要查看某次运行的提示词、读取或更改的文件，以及它打开的任何 pull request，请打开该运行的操作菜单并选择 **View run details**。
...
1. 在控制台中打开 [Automations](https://app.mintlify.com/products/automations) 页面。
2. 在 **Runs** 标签页中，点击你想在编辑器中继续处理的运行上的 **Preview**。
