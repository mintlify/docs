...
你只能删除[自定义自动化](/zh/automations/create)。要停止预定义自动化，请改为禁用它。删除自动化是永久性的，会移除其配置和运行历史。
...
- **Failed**：agent 无法完成运行。失败的运行不会计入每日运行限制。若要重试，请[手动运行自动化](#run-an-automation-manually)或等待其下一次计划触发。
