...
- **代码变更（Code change）**：当已连接的源代码仓库中有 pull request 合并时运行。你必须至少指定一个源仓库。对于 GitHub 仓库，你还可以按路径过滤每个仓库，并排除特定的 pull request 作者。参见[过滤代码变更触发器](#filter-code-change-triggers)。
...
你可以手动运行使用计划、内容更新或代码变更触发器的自动化。由集成触发的自动化仅在其配置的事件发生时运行，因此其运行按钮不可用。**Translate content** 自动化始终可以手动运行。
