...
- **Slack 频道消息（Slack channel message）**：当有人在所选 Slack 频道中发布新的顶层消息时运行。此选项适用于自定义自动化。选择一个或多个 [Mintlify Slack 应用](/zh/agent/slack)已加入的频道。Mintlify 会跳过线程回复、来自机器人的消息，以及提及 Mintlify agent 的消息，并按 Slack 事件 ID 去重，因此单条消息最多只会触发每个匹配的自动化一次。当自动化执行操作时，会在触发它的线程中回复摘要，并附上它开启的任何 pull request 的链接。
