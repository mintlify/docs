...
对于自定义自动化和 **Update from code changes** 自动化，你可以启用已连接的[集成](/zh/automations/integrations)，以便 agent 在自动化运行时从 Slack、Notion 或 Linear 等工具拉取上下文。

要为自动化启用集成，请打开其设置，在 **Available integrations** 部分选择你希望 agent 使用的集成。只有已为你的组织连接的集成会出现在列表中。请在控制台设置中的[集成](https://app.mintlify.com/settings/organization/integrations)页面连接集成。更多信息请参见[集成](/zh/automations/integrations)。
...
如果某次运行显示 **Failed** 状态，请在 [Automation Runs](https://app.mintlify.com/products/automations) 页面打开它，查看 agent 的提示词、工具调用和错误输出。

常见原因：

- 缺少对上下文或源代码仓库的访问权限。请在受影响的仓库上重新安装 [Mintlify GitHub App](/zh/deploy/github)。
- 集成 token 已被撤销。请在 [Integrations](https://app.mintlify.com/settings/organization/integrations) 页面重新连接该集成。
- 项目仓库中的 `docs.json` 无效或存在校验错误。
- 计划或触发器在上一次运行完成前就再次触发。

在处理完根本原因后，点击该自动化旁边的 <Icon icon="play" /> 运行按钮进行重试。失败的运行不会计入你的每日运行限制，因此重试是安全的。
