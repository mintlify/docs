...
<Note>
  Draft changelog、Draft improvements from assistant conversations、Draft improvements from user feedback、Update from support feedback 以及自定义自动化不会分组或追加。每次运行都会以本次运行的更改单独打开一个 pull request。Update from support feedback 每次运行最多可打开三个 pull request，为其识别出的每个不同主题各打开一个。
</Note>
