...
<Update label="11 月 25 日" tags={["改进"]} rss={{ title: "Vale 版本升级", description: "现已支持 Vale 3.10+ 版本" }}>
  ...
  * 将后端依赖升级为 Vale 版本 3.11.2-r5，为 Vale CI 检查功能带来了原生 MDX 支持。
...
<Update label="11 月 21 日" tags={["改进"]} rss={{ title: "每周更新", description: "API 操作台增强、本地化更新以及 CLI 改进" }}>
  ...
  * 修复了在具有不同响应码的端点之间切换时，响应部分会消失的问题。现在在导航到新端点时，操作台会正确重置为第一个可用的响应码。
  * 修复了在 HTTP Basic 认证方案中重复显示 "Authorization" 头的问题。认证区域现在提供了更简洁的界面，并使用了正确的字段标签和说明。
  * 改进了对嵌套对象和数组中 `oneOf` schema 的处理。具有多种类型选项的复杂 API 架构现在会在合适的场景中正确渲染为下拉菜单，而不是选项卡。
  * 修复了在 OpenAPI 规范中定义在端点级别但缺失的路径参数。现在，定义在各个具体操作之上的参数会正确包含在主页面和操作台中。
  * 增强了响应示例生成逻辑，即使示例只指定了部分字段，也会包含所有必填属性。这样可以确保响应示例完整且准确。
  * 修复了对象数组中预填示例缺失的问题。带有示例数据的数组项现在会在操作台中按正确的索引自动预填。
  * 改进了 API 操作台中说明文本的内边距和间距，以提升可读性和视觉一致性。

  ## 本地化改进

  * 改进了所有受支持语言中上下文菜单选项和 AI 助手状态消息的翻译质量。

  ## CLI 改进

  * 为在 Node.js 20.17 以下版本上运行 `mint dev` 的用户新增了错误提示信息，并引导用户升级到 LTS 版本。
