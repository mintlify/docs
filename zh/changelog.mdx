...
<Update label="2026年9月11日" tags={["新版本","改进","错误修复"]} rss={{ title:"Recents 和 Starred 同步、automation 合并冲突解决等" }}>

<div id="editor-and-collaboration">
  ## 编辑器与协作
</div>

  - **随处可用的 Recents 和 Starred：** 编辑器的 Recents 和 Starred 部分现在会跨设备同步，并出现在 workspace 侧边栏中。两者都限定于跨 branch 的 workspace 页面，你还可以通过拖拽重新排序侧边栏各部分。
  - **移动和复制私密页面：** 从文档或侧边栏菜单将私密页面移动到 workspace 文件夹，从同一菜单复制私密页面，或离开他人邀请你加入的私密页面。当你将私密页面提升为公开页面时，其中的图片和视频会随之移动，内联引用也会被重写。详情请参见 [私密页面](/zh/editor/pages#private-pages)。
  - **评论面板：** 打开全文档级的评论面板，查看当前页面已打开和已解决的讨论。表情回应选择器现在会优先展示你的团队最常用的表情。
  - **侧边栏中的站点设置：** 站点配置页面现在直接从编辑器侧边栏打开，而不再通过弹窗，workspace 设置导航也做了相应的重新组织。
  - **页面图标作为标签页 favicon：** 编辑器标签页的 favicon 现在使用页面自身的图标，并且页面和导航图标的 frontmatter 支持 emoji 值。
  - **Card 图标字段：** `Card` 组件的属性面板现在直接内联展示 `icon` 和相关字段。
  - **编辑器打磨：** 输入时建议删除会自动合并，编辑器搜索结果只显示限定在你组织内的收件人列表，组织外的分享收件人会被跳过，即使页面未加入导航也可以使用实时预览，只读页面会显示 **View-only mode** 下拉菜单。

<div id="automations">
  ## Automations
</div>

  - **自动解决合并冲突：** 当 automation 的 pull request 出现合并冲突时，agent 可以为你解决冲突，既可以自动进行，也可以在你点击 pull request 上的 **Accept** 时进行。详情请参见 [解决合并冲突](/zh/automations/manage#resolve-merge-conflicts)。
  - **手动 pull request 运行：** 针对特定 pull request 手动触发的运行现在会在运行信息流中被明确标注，运行页面还会显示运行数量徽章、统一的结果卡片和更简洁的摘要。详情请参见 [管理 automations](/zh/automations/manage)。
  - **默认模型：** 新的 automation 运行现在默认使用 Opus 4.7。

<div id="dashboard">
  ## Dashboard
</div>

  - **Workspace 和组织图标：** 在常规设置中上传 workspace 图标，在组织设置中上传组织图标。详情请参见 [更改你的 workspace 图标](/zh/help-center/change-your-workspace-icon)。
  - **桌面版登录移交：** 通过在浏览器中确认一次性验证码即可登录 Mintlify 桌面版，然后可以在同一授权界面切换组织。
  - **IP 屏蔽错误页：** 被 IP 白名单拦截的请求现在会跳转到专门的错误页面，说明发生了什么。

<div id="git-and-integrations">
  ## Git 与集成
</div>

  - **编辑器中同步非 Markdown 文件：** 编辑器 branch 现在会同步非 MDX 的文本文件，因此配置文件和其他纯文本资源的更改都能通过编辑器双向流转。
  - **GitLab 编辑器 branch：** GitLab 推送会实时同步到编辑器 branch，branch 下拉菜单会显示贡献者，并且会检测强制推送。Git-only branch 现在会出现在编辑器的 branch 下拉菜单中，因此你可以切换到在编辑器外创建的 branch。
  - **可选启用的 branch 下拉菜单：** 编辑器的 branch 下拉菜单现在改为按用户可选启用；如果你想恢复它，请在编辑器设置中开启。

<div id="api-playground-and-openapi">
  ## API playground 与 OpenAPI
</div>

  - **OpenAPI Overlays 1.1.0：** 现在在更新和 schema-graph 工作流中会应用 OpenAPI Overlay 1.1.0 文档，`$ref` 前缀和数组条目在 base 路径和挂载点下都会得到正确处理。详情请参见 [OpenAPI 设置](/zh/api-playground/openapi-setup)。
  - **Playground 标签：** `oneOf` 变体下拉菜单使用可识别标题的标签，因此每个选项都更容易辨认。

<div id="assistant-and-ai">
  ## Assistant 与 AI
</div>

  - **免费的编辑器 agent、统一的 AI 计费：** 编辑器 agent 现在可以免费使用，Slack agent 按写入免费，assistant 回答按固定积分计费，automation 按每次写入固定 250 个积分计费。BYOK AI 用量消耗零积分。免费的 agent 事件会从用量历史中隐藏，用量页面也会公布新的统一积分计费方式。
  - **Assistant Playground 预设：** Assistant Playground 现在包含流式预设，让你无需逐个字段编辑即可对比不同配置。
  - **通用排序：** 搜索会将页面标题匹配的结果排到更靠前的位置，让精确标题查找优先出现。

<div id="bug-fixes">
  ## 错误修复
</div>

  - **预览部署：** GitHub 预览部署会固定到触发它的 commit，预览 URL 在重新构建之间保持稳定，预览评论会在每次重新构建后刷新。
  - **自定义域名和 404：** Cloudflare 不再服务的自定义主机名会返回 `not_found`，MCP 分析会将自定义域名解析到其规范子域名，Markdown 404 页面会包含恢复指引和发现头。
  - **Assistant 修复：** 响应流式传输期间会保留 assistant 草稿，代码块在流式传输期间保持高亮并跳过未变化的渲染，表格和紧凑代码块可以独立滚动，文件附件以横向卡片渲染，建议链接会在回答内渲染，发送和停止控件合并为一个按钮。
  - **编辑器修复：** 创建 branch 不再遗留卡住的骨架屏，pull request 链接重定向不再对 branch 名称进行双重编码，`saving` 行与服务器保持一致，调整大小不再进入循环，frontmatter 只在文档连接后才加载。
  - **发布：** 发布复选框现在按每行独立生效，部分发布会将选中的待处理操作应用到基线导航树上，无内容的创建操作在发布后会被清理，不再阻塞后续发布。
  - **Dashboard 修复：** 代码块的光标位置在提升和输入规则两种情况下都得到修复，移除连接时会重置 SSO 配置，配置 SSO 时只能选择已验证的域名，页面头恢复了 **Add description** 入口。
  - **认证和账户：** 密码重置改为异步并做了速率限制，dev 组织的速率限制提高，新增的新账户标记让 dashboard 能触发正确的注册事件。
  - **OpenAPI：** 删除 OpenAPI 规范现在会移除其生成的资源，OpenAPI 候选检测时的内存占用降低，API 参考页面会将 schema graph 裁剪到页面的目标。
  - **杂项：** 组织 logo 存储在平台 bucket 中，重新加载空正文时会保留私密页面的头部，缓存绕过路径在部署 base 路径下能正确匹配，用于搜索 Mintlify 邮件的 Gmail 搜索快捷方式再次可用。
