...
<Update label="May 29, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Mintlify MCP、GitLab 工作流、预览分享等更多更新" }}>

<div id="new-features">
  ## 新功能
</div>

  - **Mintlify MCP 服务器**：将 Claude、Cursor 或任意 MCP 客户端连接到你的 Mintlify 项目，以草拟修改、调整导航结构、更新 `docs.json` 并发起 pull request。每次修改都在分支上进行，可像其他提交一样进行审核。详见 [Mintlify MCP](https://www.mintlify.com/docs/ai/mintlify-mcp)。
  - **GitLab 工作流**：[工作流](https://www.mintlify.com/docs/workflows/enable) 现已支持 GitLab 仓库，与 GitHub 并列。你可以通过 OAuth 连接 GitLab.com 或[自托管的 GitLab 实例](https://www.mintlify.com/docs/deploy/gitlab-self-hosted)，并对具有 Maintainer 角色的 merge request 启用 automerge。
  - **分享预览**：新增 [Share a preview](https://www.mintlify.com/docs/editor/share-a-preview) 页面，介绍预览部署 URL 和编辑器链接，让评审者可在内容上线前查看你的修改。你也可以在 Add-ons 页面将预览访问限制为你的 Mintlify 组织成员。
  - **下载 API spec**：将 `"download-spec"` 添加到 `contextual.options`，即可让访问者在任意 API 参考页面一键下载你的 OpenAPI spec。单个 spec 以文件形式下载；多个 spec 会打包为 `api-specs.zip`。详见 [Let visitors download your spec](https://www.mintlify.com/docs/api-playground/openapi-setup#let-visitors-download-your-spec)。
  - **OpenAPI callbacks**：API 参考页面现在会自动渲染操作级别的 OpenAPI `callbacks`，显示为请求体与响应之间的可折叠区域。无需任何配置。详见 [Callbacks](https://www.mintlify.com/docs/api-playground/openapi-setup#callbacks)。
  - **Agent 发现端点**：Mintlify 现在会在 `/.well-known/agent-card.json` 提供 agent card（A2A 0.2.0），便于 agent 在无会话状态下发现你的 skills；同时新增 MCP 发现 URL `/.well-known/mcp.json`、`/.well-known/mcp/server-card.json` 和 `/.well-known/mcp/server-cards.json`。所有端点均自动提供，无需配置。详见 [skill.md](https://www.mintlify.com/docs/ai/skillmd#agent-card) 与 [Model Context Protocol](https://www.mintlify.com/docs/ai/model-context-protocol)。
  - **robots.txt 中的 Content-Signal 指令**：自动生成的 `robots.txt` 现在包含 Content Signals（`ai-train=yes`、`search=yes`、`ai-input=yes`），让 AI 爬虫了解你的文档可用于训练、搜索和回答生成。你可以通过自定义 `robots.txt` 覆盖默认值。详见 [Content-Signal directives](https://www.mintlify.com/docs/optimize/seo#content-signal-directives)。

<div id="updates">
  ## 更新
</div>

  - **编辑器发布**：默认 commit message 现在会列出你创建、更新、移动或删除的文件，取代由 AI 生成的提交信息。**Commit message instructions** 发布设置已被移除。详见 [Branching and publishing](https://www.mintlify.com/docs/editor/branching-and-publishing)。
  - **编辑器教程**：新增 [How to use the editor](https://www.mintlify.com/docs/editor/tutorial) 教程，带你完成创建分支、编辑页面、分享预览与发布的完整流程。
  - **Markdown 导出鉴权**：Markdown 导出文档新增表格，说明 `.md` URL 在无鉴权、部分鉴权和完全鉴权下的行为。详见 [Markdown export](https://www.mintlify.com/docs/ai/markdown-export#authentication)。
  - **Dashboard URL**：所有 Mintlify dashboard 链接现在均使用 `app.mintlify.com`，取代 `dashboard.mintlify.com`。现有链接将继续重定向。
...
<div id="workflows">
  ...
  - 新增 **Improve content from assistant conversations** 工作流，可分析用户提问趋势并建议文档更新以填补缺口。详见 [Workflows](https://www.mintlify.com/docs/workflows/enable#improve-docs-from-assistant-chats)。
...
使用 [dashboard](https://dashboard.mintlify.com/products/workflows) 中的预置模板更快地创建 [workflows](/workflows)。可选择的模板包括：changelog 生成、API 文档同步、功能文档草稿、断链检测、SEO 审计等。模板包含优化的 prompt 和触发器配置，帮助你快速上手。
...
<Update label="February 27, 2026" tags={["New releases", "Improvements"]} rss={{ title: "使用工作流自动化文档任务，并在编辑器中通过评论和建议协作" }}>
  ...
  Workflows 目前处于测试阶段，现已在 [Enterprise 套餐](https://mintlify.com/pricing) 中提供，很快也将支持 Pro 套餐。
