...
<Update label="2026年9月4日" tags={["新版本","改进","错误修复"]} rss={{ title:"协作源码模式、评论表情回应、mint test 全面开放等" }}>

<div id="editor-and-collaboration">
  ## 编辑器与协作
</div>

  - **协作源码模式：** 源码模式现已支持实时协作。协作者的光标会在源码面板中显示标签，frontmatter 的编辑会在可视模式和源码模式之间实时同步。参见[可视模式与源码模式](/zh/editor#visual-and-source-mode)。
  - **评论表情回应：** 用表情回应评论即可确认，无需回复。将鼠标悬停在回应上可查看回应者，回应他人的评论会通知评论作者。参见[评论](/zh/editor/collaborate#comments)。
  - **选择性发布：** 在发布菜单中勾选单个更改，只发布部分待发布内容，而不必一次发布全部。参见[分支与发布](/zh/editor/publish)。
  - **共享更新：** 当有人与你共享页面时，Mintlify 会通知你；共享对话框现在支持按邮箱地址搜索收件人。
  - **编辑器细节优化：** 搜索结果显示页面标题并在侧边栏中定位文件，侧边栏默认只列出 Markdown 页面，表情建议菜单的搜索相关性更好，代理面板在站点设置中也保持可用。

<div id="cli">
  ## CLI
</div>

  - **`mint test` 全面开放：** 通过 CLI 为文档中的代码块生成测试，现已向所有用户开放。运行记录会保存，方便回看以往报告。参见 [`mint test`](/zh/cli/commands#mint-test)。

<div id="automations">
  ## 自动化
</div>

  - **手动运行的范围选择：** 手动触发自动化时，可选择审查自某一日期以来的更改、整个站点或特定的 pull request。参见[手动运行自动化](/zh/automations/manage#run-an-automation-manually)。
  - **失败邮件：** 自动化运行失败时 Mintlify 会向你发送邮件，之后暂停发送失败邮件，直到有运行再次成功。

<div id="analytics">
  ## 分析
</div>

  - **页面筛选：** 使用 dashboard 中新的页面下拉菜单，将分析数据筛选到特定页面。参见[分析概览](/zh/analytics)。

<div id="api-playground">
  ## API playground
</div>

  - **可选字段开关：** 较长的请求表单会先列出必填字段，并将可选字段折叠在开关后面，让重要的输入项更容易找到。
  - **默认折叠字段：** 在操作上的 `x-mint: playground` 下设置 `expand: false`，即可默认折叠嵌套的对象字段。参见[折叠 playground 字段](/zh/api-playground/openapi-setup#collapse-playground-fields)。

<div id="authoring">
  ## 写作
</div>

  - **`<MDX>` 改进：** `<MDX>` 块内的标题会并入页面目录，`## Title {#id}` 锚点在任意缩进下都有效，`<MDX>` 内的代码块与页面根级代码块行为一致。参见 [MDX](/zh/components/mdx)。
  - **组件标题支持 Markdown：** 组件标题（例如手风琴标题）支持行内 Markdown 格式，如代码和加粗。
...
<div id="bug-fixes">
  ...
  - **大型仓库：** 文件列表被 GitHub 截断的超大仓库现在会同步所有文件。
  - **助手引用：** 助手会修复丢失路径首段的引用链接，使引用打开正确的页面。
  - **需认证的预览：** 需要认证的文档的预览部署会提供你更改后的文件。
  - **翻译：** 翻译页面链接到带语言前缀的内部路径。
  - **OpenAPI 清理：** 删除 OpenAPI 规范会同时从站点中删除其生成的资源。
  - **自定义字体：** 设置自定义正文字重时，加粗文本仍保持加粗。
  - **代码反馈：** 代码片段反馈弹层显示在目录面板之上。
  - **已关闭的横幅：** 已关闭的横幅在页面加载时不再闪烁。
  - **编辑器修复：** 引用式链接和 `Link` 组件正确渲染，带 `colspan` 或 class 的 HTML 表格保存后保持原样，编辑页面不再重新格式化未改动的内容，切换分支不再造成重定向循环。
