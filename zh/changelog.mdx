---
title: "产品更新"
description: "随时了解 Mintlify 的产品更新、新功能、改进和错误修复。"
rss: true
noindex: true
---

<Update label="March 6, 2026" tags={["New releases", "Improvements"]} rss={{ title: "从公共 GitHub 仓库自动生成文档、AI 助手多模态支持及更多改进" }}>

  <div id="auto-generate-documentation-from-public-repositories">
  ## 从公共仓库自动生成文档
  </div>

  从任何公共 GitHub 仓库[自动生成完整的文档站点](https://mintlify.com/explore)。在任意公共仓库 URL 中将 `github.com` 替换为 `mintlify.com`，即可即时预览生成的文档。如果你是仓库所有者，可以认领该文档站点，将其添加到你的 Mintlify 组织并进行自定义。

  <div id="assistant-improvements">
  ## AI 助手改进
  </div>

  [AI 助手](/zh/ai/assistant)现在支持多模态输入。可以在消息中附加文件以提供额外上下文。直接在聊天中拖放或粘贴图片、PDF 和代码文件。

  AI 助手现在可以生成[针对特定页面的示例问题](/zh/ai/assistant#add-sample-questions)。

  AI 助手现在还会根据用户所在页面构建上下文，并索引完整的 OpenAPI 规范。

  <div id="default-vale-configuration-updates">
  ## 默认 Vale 配置更新
  </div>

  [默认 Vale 配置](/zh/deploy/ci#configuration)现在将 MDX 解析为 MD，以避免 JSX 带来的脆弱性问题。此前，Vale 会对 MDX 组件中的任何 JSX 返回误报错误。现在，Vale 将 MDX 解析为 MD，并通过 tokenIgnores 和 blockIgnores 配置来忽略 Vale 应跳过的 MDX 和 JSX 元素，从而避免这些错误。

  <div id="improvements">
  ## 功能改进
  </div>

  - Workflows 现在每天最多支持 50 次运行，从原来的 20 次提升。
  - 通过在 `docs.json` 中配置 `api.url`，API 操作台现在支持[完整的基础 URL](/zh/organize/settings#param-api)。
  - 控制台布局已更新，改进了导航并可通过侧边栏访问编辑器和设置页面。
  - 编辑器搜索栏经过重新设计，提供更好的文件查找和导航体验。
  - 编辑器中的列表支持嵌套拖放。
  - 代码块支持 Twoslash 以显示内联 TypeScript 类型信息。
  - 部署历史现在区分排队中和进行中的状态。
  ...
  - 修复了受认证保护文档的 OAuth 和 JWT 重定向问题。
  - 修复了在编辑器中粘贴文本时光标跳动的问题。
  - 修复了取消 AI 助手消息的键盘快捷键。
  - 修复了包含下划线的 branch 名称在编辑器中无法正常工作的问题。
  - 修复了提示框组件中链接边框颜色的问题。
  - 修复了移动设备上输入框的缩放行为。

</Update>

<Update label="February 27, 2026" tags={["New releases", "Improvements"]} rss={{ title: "使用工作流自动化文档任务，在编辑器中通过评论和建议协作" }}>
  ## Workflows

  使用 [workflows](/zh/agent/workflows) 自动化文档相关任务。你可以配置定时或事件触发的自动化流程，由 agent 来运行以更新你的文档。

  Workflows 目前处于测试阶段，现已在 [Enterprise 套餐](https://mintlify.com/pricing) 中提供，很快也将支持 Pro 套餐。

  ## 评论与建议

  在 Web 编辑器中通过 [评论](/zh/editor/comments) 和 [建议](/zh/editor/suggestions) 直接协作。选中文本即可留下反馈、提及队友或提出修改建议。
  ...
  * Model Context Protocol (MCP) 现在可以与[部分受保护文档](/zh/ai/model-context-protocol)一起工作。
...
<Update label="February 20, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Agent 配置文件与自定义前端" }}>
  ...
  在你的存储库中使用 `.mintlify/Assistant.md` 和 `.mintlify/AGENTS.md` 文件来自定义 agent 和 AI 助手的行为。更多信息请参见 [Customize assistant behavior](/zh/ai/assistant#customize-assistant-behavior) 和 [Customize agent behavior](/zh/agent/customize)。
...
<Update label="February 6, 2026" tags={["New releases", "Improvements"]} rss={{ title: "自助式 SSO、查看者角色、人类/AI 流量切换和斜杠命令菜单重设计" }}>
  ## 自助式 SSO 配置

  企业客户现在可以直接在控制台中配置 SAML SSO (Okta 或 Microsoft Entra) ，包括即时 (JIT) 开通、身份提供商切换和移除连接。

  ## 查看者角色

  新增查看者 (Viewer) 角色，对你的控制台仅有只读访问权限。查看者可以在编辑器中查看内容，但不能编辑内容、邀请成员或修改配置。
  ...
  编辑器的斜杠命令菜单现在支持输入即搜索、改进的键盘导航以及其他 UX 改进。

  ## 错误修复

  * 修复了控制台表格中的单元格内容溢出问题。
  * 现在通过编辑器中的配置面板重命名文件可以正确保存更改。
  * MDX 解析错误 (例如未闭合的 `<Card>` 标签) 现在会在你修复时动态更新，而不是在刷新后仍然错误地保留。
  * 编辑器中的拖拽手柄现在会与不同类型的内容 (如标题或段落) 正确对齐，尺寸更加统一且可点击区域更好。
</Update>
...
<Update label="January 30, 2026" tags={["New releases", "Improvements"]} rss={{ title: "简化技能安装、焕新的 Analytics 页面、编辑器改进等" }}>
  ...
  * 稳定性改进
  ...
  在单一页面中管理所有 [AI 助手功能](/zh/ai/assistant)，布局更简洁、用户体验更佳。
...
<Update label="January 23, 2026" tags={["New releases", "Improvements"]} rss={{ title: "skills.md、审计日志与 Markdown 导出" }}>
  ## `skill.md` 自动生成

  文档站点现在会基于你的 API 参考、指南和功能自动生成一个 `skill.md` 文件，为 AI Agent 提供一份完整的技能文件，包含如何帮助用户使用你的产品的上下文信息。`skill.md` 以更结构化的能力和更广泛的适用性，取代了原先基于 `install.md` 的方案。如果你在文档存储库根目录下有自定义的 `skills.md` 文件，它会替代自动生成的版本。更多信息请参见：[skill.md](/zh/ai/skillmd)。

  ## 认证事件的审计日志

  事件现在会记录在[审计日志](/zh/dashboard/audit-logs)中，以提升可见性并满足合规需求。

  ## 面向 Agent 的 Markdown 导出

  当请求中携带 `Accept: text/markdown` 头时，文档页面现在会返回原始 Markdown，从而让 AI Agent 更好地读取和利用内容。

  ## 功能改进

  * 将完全认证、部分认证和个性化体验合并为统一的认证流程。更多信息请参见：[认证设置](/zh/deploy/authentication-setup)。
  * 隐藏页面现在会自动添加 `noindex` meta 标签，以便更好地控制 SEO (搜索引擎优化) 。
  * 为图片标题新增 Markdown 格式支持。
</Update>
...
<Update label="December 12, 2025" tags={["New releases", "Improvements"]} rss={{  title: "Agent 建议与问答机器人" }}>
  ...
  你可以将由 AI 助手驱动的机器人添加到你的 [Slack 工作区](/zh/ai/slack-bot) 和 [Discord 服务器](/zh/ai/discord)中。机器人会基于你的文档实时回答问题，为你的社区提供准确、最新的答复。
...
<Update label="December 8, 2025" tags={["New releases"]} rss={{ title: "最近修改时间", description: "通过自动时间戳显示页面的最近更新时间" }}>
  ...
  你现在可以在文档中的所有页面上显示页面的“最近修改”时间戳。通过在 `docs.json` 设置中启用 [`timestamp`](/zh/organize/settings#param-timestamp) 标志即可开启此功能。
