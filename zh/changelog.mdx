...
<div id="improvements">
  ...
  - [助手](/assistant)现已在 `mint dev` 本地预览中可用。
...
<Update label="March 6, 2026" tags={["New releases", "Improvements"]} rss={{ title: "从公开 GitHub 仓库自动生成文档、助手多模态功能，以及更多改进" }}>
  ...
  [助手](/assistant) 现在支持多模态输入。可向消息附加文件以提供更多上下文。你可以直接将图片、PDF 和代码文件拖放或粘贴到聊天中。

  助手现在还可以生成页面特定的[示例问题](/assistant/configure#add-sample-questions)。
...
<Update label="February 20, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Agent 配置文件与自定义前端" }}>
  ...
  在你的存储库中使用 `.mintlify/Assistant.md` 和 `.mintlify/AGENTS.md` 文件来自定义 agent 和 AI 助手的行为。更多信息请参见 [Customize assistant behavior](/assistant/customize) 和 [Customize agent behavior](/zh/agent/customize)。
...
<Update label="January 30, 2026" tags={["New releases", "Improvements"]} rss={{ title: "简化技能安装、焕新的 Analytics 页面、编辑器改进等" }}>
  ...
  * 稳定性改进
  ...
  在单一页面中管理所有 [AI 助手功能](/assistant)，布局更简洁、用户体验更佳。
