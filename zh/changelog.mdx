...
<Update label="February 13, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Prompt 组件、Sequoia 主题、Tabler 图标以及编辑器改进" }}>
  ...
  [Sequoia 主题](/zh/customize/themes#sequoia) 在侧边栏中提供了重新设计的目录，并集成了 [上下文菜单](/zh/ai/contextual-menu)。
  ...
  导航 groups 现在支持 [`root`](/zh/organize/navigation#root-pages) 属性，为分组设置着陆页。点击分组标题将跳转到 root 页面，而不是展开该分组。
...
<Update label="January 23, 2026" tags={["New releases", "Improvements"]} rss={{ title: "skills.md、审计日志和 Markdown 导出" }}>
  ...
  文档站点现在会基于你的 API 引用、指南和功能自动生成一个 `skill.md` 文件，为 AI 代理提供一份包含如何帮助用户使用你产品所需上下文的完整技能文件。`skill.md` 替代了之前的 `install.md` 方案，具备更结构化的能力定义和更广泛的适用性。如果你在文档存储库根目录下提供了自定义的 `skills.md` 文件，它会替换自动生成的版本。更多信息参见 [skill.md](/zh/ai/skillmd)。

  ## 认证事件审计日志

  事件现在会记录在 [审计日志](/zh/dashboard/audit-logs) 中，以满足可见性和合规性需求。

  ## 面向 AI 代理的 Markdown 导出

  当发送 `Accept: text/markdown` 头部时，文档页面现在会返回原始 Markdown，从而让 AI 代理能够更好地读取和利用内容。

  ## 改进

  - 将完整认证、部分认证和个性化体验合并为统一的认证体验。更多信息参见 [Authentication setup](/zh/deploy/authentication-setup)。
