---
title: "产品更新"
description: "新版本与改进内容"
rss: true
noindex: true
---

<Update label="12 月 1 日" tags={["新版本"]} rss={{ title: ".mintignore 支持", description: "从已发布的文档中排除文件和目录" }}>
  ## .mintignore 支持

  * 新增对 `.mintignore` 文件的支持，可以排除特定文件和目录，使其不会被处理或发布到你的文档站点。
  * 使用与 `.gitignore` 相同的模式语法，便于按你熟悉的方式进行配置。
  * 被排除的文件不会出现在已发布文档中，不会被搜索索引，也无法被访问者访问。

  在 [.mintignore 文档](/zh/organize/mintignore) 中了解更多。
</Update>

<Update label="11 月 25 日" tags={["改进"]} rss={{ title: "Vale 版本升级", description: "现已支持 Vale 3.10+ 版本" }}>
  ## Vale 版本升级

  * 升级后端依赖以使用 Vale 3.11.2-r5 版本，为 Vale CI 检查功能带来原生 MDX 支持。
</Update>

<Update label="11 月 21 日" tags={["改进"]} rss={{ title: "每周更新", description: "API 操作台增强、本地化更新以及 CLI 改进" }}>
  ## API 操作台改进

  * 修复了在切换具有不同响应码的端点时响应区域会消失的问题。现在在导航到新端点时，操作台会正确重置为第一个可用的响应码。
  * 修复了 HTTP Basic 认证方案中显示重复 &quot;Authorization&quot; 请求头的问题。认证区域现在展示更简洁的界面，并具有正确的字段标签和说明。
  * 改进了对嵌套对象和数组中 `oneOf` schema 的处理。包含多种类型选项的复杂 API schema 现在会在合适的场景下正确渲染为下拉菜单，而不是选项卡。
  * 修复了在 OpenAPI 规范中定义在端点级别但缺失的路径参数。现在在各个操作之上的参数会正确地包含在主页面和操作台中。
  * 增强了响应示例的生成逻辑，即便示例中只指定了部分字段，也会包含所有必填属性。这确保响应示例完整且准确。
  * 修复了对象数组中预填示例缺失的问题。带有示例数据的数组项现在会在操作台中按正确的索引得到正确填充。
  * 优化了整个 API 操作台中的说明文字留白与间距，提高了可读性和视觉一致性。

  ## 本地化改进

  * 优化了所有支持语言中上下文菜单选项和 AI 助手状态消息的翻译。
...
<Update label="8 月 23 日" tags={["Improvements"]} rss={{ title: "每周更新", description: "图片布局修复、认证增强、移动端导航改进以及 Web 编辑器稳定性提升" }}>
  ## 图片处理改进
