...
<Update label="July 16, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Visibility 组件、相关页面、按页面上下文菜单、草稿等" }}>

<div id="new-features">
  ## 新功能
</div>

  - **相关页面：** 在文档底部自动展示相关页面，帮助读者发现更多相关内容。
  - **草稿：** 无需创建分支即可按页面预览未发布的更改。全新的公告对话框和 dashboard 开关帮助团队上手新工作流。
  - **按页面覆盖上下文菜单：** 在 frontmatter 中按页面配置[上下文菜单](/zh/ai/contextual-menu)，以覆盖站点级选项。
  ...
  - **通用搜索 API：** 全新 API 可用于跨文档查询内容，并支持按部署选择退出。
  - **S3 键前缀：** S3 集成现在支持键前缀，让您可以将上传范围限定到存储桶中的特定路径。
  - **Pylon 支持：** [Pylon](/zh/automations/integrations) 现已作为已连接的集成上线，用于支持驱动的[自动化](/zh/automations)。

<div id="editor">
  ## 编辑器
</div>

  - **实时预览中的审阅请求卡片：** 审阅者可直接在实时预览中看到审阅请求卡片，并通过共享的更改列表一目了然地查看草稿更改。
  - **锁定部署分支：** 当检测到分支保护时，编辑器现在会提示锁定部署分支，并提供新开关来阻止对 `main` 的直接编辑。
  - **[编辑器代理](/zh/editor)中更好的撤销/重做：** 代理对内容和导航树所做的更改现在可以干净地撤销和重做。
  - **可折叠标题状态按用户持久化：** 您展开和折叠章节的偏好会在多个编辑器会话之间保留。
  - **缓存内容只读模式：** 可视化编辑器在显示缓存（未同步）内容时会切换为只读，以避免写入冲突。
  - **隐藏页面提示：** 编辑器中新增横幅，提示从导航中隐藏的页面，便于查找和管理。

<div id="automations-and-slack-agent">
  ## 自动化与 Slack 代理
</div>

  - **代理配置移至 Automations：** 代理设置现位于 [Automations](/zh/automations) 下。旧的代理页面 URL 会自动重定向。
  - **Slack 代理上手引导：** 启用代理后，组织成员会收到 Slack 私信，并创建带有上手引导消息的 `#mintlify` 频道。当频道长时间无消息时，代理还会发送后续提醒。
...
<div id="assistant">
  ...
  - **无头 Assistant 客户端：** 全新的框架无关 [Assistant](/zh/assistant) 核心与浏览器加载器，让您可以通过无头客户端 API 和直接挂载生命周期在自定义 UI 中嵌入助手。
  - **Widget dashboard：** 通过专属的 dashboard 页面和 playground 配置并预览 Assistant widget。
  - **按终端用户组过滤 MCP 技能资源：** 根据终端用户组成员资格限制通过 [MCP 服务器](/zh/ai/model-context-protocol)公开哪些技能资源。
  - **面向已认证 MCP 的分组自定义技能：** 在已认证的 MCP 部署上将自定义技能限定到特定的终端用户组。
  - **管理员 MCP 保存自动合并：** 配置后，管理员 MCP 编辑可以自动合并到 `main`。

<div id="analytics">
  ## Analytics
</div>

  - **Analytics v7 改进：** 合并了 Overview 与 Traffic 板块，统一了 CSV 导出菜单，在各板块间同步日期范围选择器，并新增 Feedback 板块。
  - **Assistant 聊天历史标签：** 从 [analytics](/zh/optimize/analytics) v7 的助手卡片深入查看单次助手对话，并在主题抽屉中提供反馈与分页。
  - **搜索细分与排名：** 新增搜索细分和排名卡片，并增加零结果搜索端点以及独立查询与点击率指标。
  - **CSV 导出：** 将 MCP 搜索、代理访客和零结果搜索报告导出为 CSV。
  - **版本下拉跟踪：** 站点上的版本切换现在会作为 `docs.version.change` 事件被跟踪，用于分析。
...
<div id="bug-fixes">
  ...
  - 缓存的 MDX 在解析失败后不再存储回退编译结果，后续编辑可以干净地恢复。
  - MCP 服务器 "copy primary URL" 按钮现在会显示确认提示。
  - AsyncAPI playground 现在会正确地将基于标头的认证作为 `Sec-WebSocket-Protocol` 子协议发送。
  - Analytics v7 中的聊天历史深链接现在可以正确打开。
  - Analytics v7 中的排名页面浏览量会汇总人类和代理流量，并且零结果差值颜色会反映正确的方向。
  - Assistant 在响应流式传输过程中不再抛出最大更新深度错误。
  ...
  - 修复了导航侧边栏树溢出、删除部署按钮溢出以及顶栏按钮内边距问题。
  - 引用式导航的重新加载明显更快。
