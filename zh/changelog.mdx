...
<Update label="December 8, 2025" tags={["New releases"]} rss={{ title: "最近修改时间", description: "使用自动时间戳显示页面的最近修改时间" }}>
  ## 最近修改时间

  现在，你可以在文档中的所有页面上显示“最近修改”时间戳。通过在 `docs.json` 配置中启用 [`timestamp`](/zh/organize/settings#param-timestamp) 标志即可开启此功能。

  启用后，每个页面都会自动显示其最近的更新时间，帮助读者判断内容是否是最新的。
</Update>
...
<Update label="November 21, 2025" tags={["Improvements"]} rss={{ title: "Weekly Updates", description: "API playground enhancements, localization updates, and CLI improvements" }}>
  ...
  * 修复了在不同响应码的端点之间切换时响应区域会消失的问题。现在在导航到新端点时，操作台会正确重置为第一个可用的响应码。
  * 修复了 HTTP Basic 认证方案中重复显示 &quot;Authorization&quot; 头的问题。认证区域现在提供更简洁的界面，并带有正确的字段标签和说明。
  * 改进了对嵌套对象和数组中 `oneOf` 模式的处理。包含多种类型选项的复杂 API 模式现在会在合适的场景下正确渲染为下拉菜单，而不是选项卡。
  * 修复了在 OpenAPI 规范中定义在端点级别但缺失的路径参数。现在，定义在各个具体操作之上的参数会正确出现在主页面和操作台中。
  * 增强了响应示例生成逻辑，即使示例只指定了部分字段，也会包含所有必填属性，从而确保响应示例完整且准确。
  * 修复了对象数组中预填示例缺失的问题。带有示例数据的数组项现在会在操作台中按照正确的索引进行填充。
  * 改进了整个 API 操作台中的说明文本留白和间距，使可读性和视觉一致性更好。

  ## 本地化改进

  * 增强了所有支持语言中情境菜单选项和 AI 助手状态消息的翻译质量。

  ## CLI 改进

  * 为在 Node.js 20.17 以下版本上运行 `mint dev` 的用户新增错误消息，并引导用户升级到 LTS 版本。
</Update>
