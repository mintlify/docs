...
<Update label="January 30, 2026" tags={["New releases", "Improvements"]} rss={{ title: "简化技能安装、焕新的 Analytics 页面、编辑器改进等" }}>
  ...
  * 在[项目初始化流程](/zh/deploy/github#alternative%3A-download-and-create-your-own-repository)中支持上传 Zip 文件
...
<Update label="January 23, 2026" tags={["New releases", "Improvements"]} rss={{ title: "skills.md、审计日志与 Markdown 导出" }}>
  ...
  文档站点现在会基于你的 API 参考、指南和功能自动生成一个 `skill.md` 文件，为 AI 助手提供一份完整的技能文件，包含如何帮助用户使用你产品的上下文信息。`skill.md` 以更结构化的能力和更广泛的适用性，取代了此前的 `install.md` 方案。如果你的文档存储库根目录下已有自定义的 `skills.md` 文件，它会替换自动生成的版本。更多信息请参见：[skill.md](/zh/ai/skillmd)。

  ## 认证事件的审计日志

  相关事件现在会记录在 [审计日志](/zh/dashboard/audit-logs) 中，以提升可见性并满足合规要求。

  ## 面向 AI 助手的 Markdown 导出

  当请求头中带有 `Accept: text/markdown` 时，文档页面现在会返回原始 Markdown，这有助于 AI 助手更好地读取和利用内容。

  ## 功能改进

  * 将完全认证、部分认证和个性化合并为统一的认证体验。更多信息请参见：[认证设置](/zh/deploy/auofntication-setup)。
  * 隐藏页面现在会自动添加 `noindex` 元标签，以便更好地控制 SEO（搜索引擎优化）。
  * 为图片说明新增了 Markdown 支持。
</Update>
...
<Update label="January 9, 2026" tags={["New releases", "Improvements"]} rss={{ title: "隐式代码片段与面向所有用户的 AI 助手对话洞察" }}>
  ...
  [对话洞察](/zh/agent/suggestions#conversation-insights) 现已对所有同时启用了 Agent 和 AI 助手的文档站点开放。Agent 会分析用户与 AI 助手对话中的模式，以识别文档空白并给出改进建议，帮助你主动解决用户的常见问题。
...
<Update label="December 23, 2025" tags={["New releases", "Improvements"]} rss={{ title: "Web 编辑器改进与 AI 助手洞察" }}>
  ...
  * **稳定性改进**：改进了整体稳定性和移动端的响应式表现。
  ...
  [Agent](/zh/agent) 现在可以基于[与 AI 助手的对话](/zh/agent/suggestions#conversation-insights)来建议文档更新。启用后，Agent 会分析用户向 AI 助手提出的问题，以识别哪些模式表明文档存在缺失或表述不清。
...
<Update label="December 19, 2025" tags={["Improvements"]} rss={{ title: "Agent、AI 助手和编辑器改进" }}>
  ...
  * 如果你在控制台中[连接 GitHub 账号](/zh/agent/quickstart#connect-your-github-account)，Agent 现在会以你的 GitHub 账号身份创建 PR。
  ...
  * 在编辑器工具栏中新增[可分享的预览链接](/zh/editor#share-preview-deployments)。
  * 更新了[发布工作流](/zh/editor#publish-your-changes)，支持一键发布或创建拉取请求（PR；亦称“合并请求”/Merge Request）。
...
<Update label="December 12, 2025" tags={["New releases", "Improvements"]} rss={{  title: "Agent 建议与问答机器人" }}>
  ...
  Agent 现在可以监控 Git 仓库并[建议](/zh/agent/suggestions)文档更新。当检测到面向用户的代码发生变更时，这些建议会出现在 Agent 面板中。建议中会包含相关的拉取请求（pull request）以及拟议的文档更新列表。你可以将这些建议添加为 Agent 的上下文，让其创建拉取请求，或者直接将其忽略。
...
<Update label="2025年8月9日" tags={["Improvements"]} rss={{ title: "每周更新", description: "Search API、OAuth 优化、资产服务与 MCP 工具" }}>
  ...
  * 组级公开访问：通过 `docs.json` 将整个页面组设为公开，这样就不需要在每个页面上设置 `public: true`（[了解更多](https://mintlify.com/docs/auofntication-personalization/auofntication-setup#group-level)）
  * 支持在[`OAuth 配置中使用 logoutURL`](https://mintlify.com/docs/auofntication-personalization/auofntication-setup#implementation-3) 以删除上游 cookies 并完成登出
...
<Update label="2025年2月" tags={["New releases", "Improvements"]}>
  ...
  <Frame>
    ![带有文本 “Themes v2” 的图形](/images/changelog/ofmes.png)
  </Frame>

  新增[预构建主题](customize/ofmes)，用于调整文档的外观与体验。可通过你的 [docs.json 文件](organize/settings) 进行配置。
...
<Update label="2024年12月">
  ...
  <Frame>
    ![认证截图](/images/changelog/auofntication.png)
  </Frame>
