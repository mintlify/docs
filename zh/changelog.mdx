...
<Update label="December 8, 2025" tags={["New releases"]} rss={{ title: "最后修改日期", description: "使用自动时间戳显示页面的最近更新时间" }}>
  ## 最后修改日期

  你现在可以在文档中的所有页面上显示“最后修改”时间戳。通过在 `docs.json` 设置中启用 [`timestamp`](/zh/organize/settings#param-timestamp) 标志即可开启此功能。

  启用后，每个页面都会自动显示其最近一次更新的时间，帮助读者了解内容的时效性。
</Update>
...
<Update label="November 21, 2025" tags={["Improvements"]} rss={{ title: "Weekly Updates", description: "API playground enhancements, localization updates, and CLI improvements" }}>
  ...
  * 修复了在不同响应码的端点之间切换时响应区域会消失的问题。现在在跳转到新的端点时，操作台会正确重置为第一个可用的响应码。
  * 修复了 HTTP Basic 认证方案中重复显示 &quot;Authorization&quot; 请求头的问题。认证区域现在展示更加简洁的界面，并使用正确的字段标签和说明。
  * 改进了对嵌套对象和数组中 `oneOf` 模式的处理。包含多种类型选项的复杂 API 模式现在会在合适的场景下正确渲染为下拉菜单，而不是以选项卡形式呈现。
  * 修复了在 OpenAPI 规范中定义在端点级别的路径参数缺失的问题。现在定义在各个 operation 之上的参数会正确包含在主页面和操作台中。
  * 增强了响应示例的生成逻辑，即使示例中只指定了部分字段，也会包含所有必填属性，从而保证响应示例完整且准确。
  * 修复了对象数组中预填示例缺失的问题。带有示例数据的数组项现在会在操作台中按照正确的索引位置被正确预填。
  * 改进了 API 操作台中说明文本的留白和间距，以提升可读性和视觉一致性。

  ## 本地化改进

  * 加强了所有受支持语言中上下文菜单选项和 AI 助手状态消息的翻译质量。

  ## CLI 改进

  * 为在 Node.js 20.17 以下版本上运行 `mint dev` 的用户新增了错误提示。现在会引导用户升级到 LTS 版本。
</Update>
