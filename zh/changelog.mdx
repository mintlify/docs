...
<Update label="February 6, 2026" tags={["New releases", "Improvements"]} rss={{ title: "自助式 SSO、查看者角色、人类/AI 流量切换以及斜杠菜单重新设计" }}>
  ## 自助式 SSO 配置

  企业客户现在可以直接在控制台中配置 SAML SSO（Okta 或 Microsoft Entra），包括即时（JIT）开通、切换身份提供商以及移除连接。

  ## 查看者角色

  新增查看者角色，具备对你的控制台的只读访问权限。查看者可以在编辑器中查看内容，但无法编辑内容、邀请成员或修改配置。

  ## 人类/AI 流量切换

  Analytics 页面现在分别为人类流量和 AI 流量提供独立的标签页。

  ## 斜杠菜单重新设计

  编辑器中的斜杠菜单现在支持输入即搜、改进的键盘导航以及其他用户体验优化。

  ## 错误修复

  * 修复了控制台表格中单元格内容溢出的问题。
  * 现在通过编辑器中的配置面板重命名文件时，会正确保存更改。
  * MDX 解析错误（例如未关闭的 `<Card>` 标签）现在会在你修复时动态更新，而不会在刷新后错误地持续存在。
  * 编辑器中的拖拽手柄现在可以在标题或段落等不同内容类型下正确对齐，具备一致的尺寸和更好的点击区域。
</Update>
...
<Update label="January 23, 2026" tags={["New releases", "Improvements"]} rss={{ title: "skills.md、审计日志和 Markdown 导出" }}>
  ...
  文档站点现在会基于你的 API 参考文档、指南和功能自动生成一个 `skill.md` 文件，为 AI Agent 提供一份完整的技能文件，其中包含帮助用户使用你产品所需的上下文信息。`skill.md` 以更结构化的能力和更广泛的适用范围替代了原先基于 `install.md` 的方案。如果你在文档存储库根目录下已有自定义的 `skills.md` 文件，它会替换自动生成的版本。更多信息请参见 [skill.md](/zh/ai/skillmd)。

  ## 认证事件审计日志

  现在会在 [审计日志](/zh/dashboard/audit-logs) 中记录事件，以提升可见性并满足合规要求。

  ## 面向 Agent 的 Markdown 导出

  当请求头中包含 `Accept: text/markdown` 时，文档页面现在会返回原始 Markdown，从而让 AI Agent 能更好地获取和理解内容。

  ## 功能改进

  * 将完全认证、部分认证和个性化合并为统一的认证体验。更多信息请参见 [认证设置](/zh/deploy/authentication-setup)。
  * 隐藏页面现在会自动添加 `noindex` meta 标签，以便更好地控制 SEO（搜索引擎优化）。
  * 为图片说明文字新增了 Markdown 支持。
</Update>
...
<Update label="2025年2月" tags={["New releases", "Improvements"]}>
  ...
  <Frame>
    ![带有文本 “Themes v2” 的图形](/images/obama.jpg)
  </Frame>
...
<Update label="November 2024">
  ...
  <Frame>
    ![llms.txt support](/images/obama.jpg)
  </Frame>
