...
<Update label="July 24, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"GraphQL 与 SDK 参考、编辑器评审流程、助手小部件等" }}>
...
<div id="editor">
  ...
  - **编辑器代理读取您的代码仓库：** 授予[编辑器代理](/zh/editor/agent)对已连接代码仓库的只读访问权限，使其能够基于您的源代码进行文档编辑。
  - **页面顶级评论：** 直接在页面本身而不仅仅是所选文本上发起评论线程。
  - **在评论中提及自己：** 在[编辑器评论](/zh/editor/comments)中使用 `@` 提及自己的账号，以留下备忘提醒。
  - **新的键盘快捷键：** `Option+S` 可对所选文本打开[建议](/zh/editor/suggestions)，`Option+C` 可打开[评论](/zh/editor/comments)。
  - **打开页面时光标聚焦顶部：** 打开页面时,光标现在会定位到文档顶部。
  - **加载更快：** 编辑器不再等待 git 文件树完成渲染。
  - **无 GitHub 权限也可查看：** 没有仓库写入权限的用户现在可以以只读模式打开编辑器,而不再被拦截。

<div id="assistant-and-widget">
  ## 助手与小部件
</div>

  - **简化的 [Assistant Widget](/zh/guides/assistant-embed)：** 更简洁的配置、框架中立的加载器、可配置的初始主题、改进的 Markdown 渲染，以及焕新的编辑器、触发器和 playground 界面。
  - **预填联系表单：** 当助手转接到支持时,现在会根据对话内容预填联系表单中的消息,用户无需再重复说明问题。
  - **ChatGPT MCP 连接器：** 将 Mintlify 文档添加为 [ChatGPT 连接器](/zh/ai/mintlify-mcp)，让 ChatGPT 能够搜索并引用您的文档。
  - **自动化中的 Claude：** 自动化现已支持 Claude 作为模型提供方。

<div id="automations-and-slack-agent">
  ## 自动化与 Slack 代理
</div>

  - **自动化分析 dashboard：** 面向[自动化](/zh/automations)的全新分析视图，包含每个工作流的运行历史、结果和趋势。
  - **Composio 集成：** 自动化的第三方集成现在由 Composio 提供支持，dashboard 和 Slack 代理共用同一套连接流程。
  - **集成触发器：** 通过 [Composio 触发器](/zh/automations/integrations)，从第三方事件（例如 Slack 消息或工单更新）触发自动化。
  - **用户范围的集成：** 在自己的账号下连接第三方工具，并可在自动化选择器中与组织级连接一同选用。
  - **一次性定时工作流：** 将自动化安排在未来某个时间运行一次，运行结束后自动删除。
  - **筛选后的 changelog URL：** 分享按特定标签筛选后的 [changelog](/zh/changelog) 链接。
...
<div id="cli">
  ...
  - **`mint format`：** 使用 Mintlify 内置的编辑器转换器格式化 MDX 文件，让文档结构保持一致。请参阅 [CLI 命令](/zh/cli/commands)。
  - **`mint deslop`：** 检测听起来像 AI 生成的行文，并在发布前给出人性化改写建议。
  - **`mint add-domain`：** 直接从 CLI 配置[自定义域名](/zh/customize/custom-domain)，无需离开终端。
  - **更友好的 `--help` 输出：** 所有命令的 `--help` 格式统一，并在 CLI 被 AI 代理调用时提供更友好的提示。

<div id="analytics">
  ## Analytics
</div>

  - **反馈改版：** [analytics 反馈](/zh/optimize/feedback)部分现在会展示每页评分，并可打开详细反馈抽屉查看底层评论。
  - **人类与代理页面排名：** 页面排名现在将人类和代理流量拆分为独立的列，方便您查看每个页面的真实读者构成。
  - **Analytics 流式配置：** 在[流式传输 analytics](/zh/optimize/analytics) 到自有数据仓库时，可选择要包含的部署。

<div id="deployments-and-search">
  ## 部署与搜索
</div>

  - **预览部署上的搜索：** 预览部署现在也提供文档搜索，方便您在合并前验证搜索结果。
  - **深链 PR 预览：** GitHub PR 预览链接现在会直接打开您所修改的页面。
  - **GHES 仓库选择：** 自托管的 GitHub Enterprise Server 仓库现在可以在 [git 设置](/zh/deploy/ghes)中选择。
  - **默认忽略 `.mintlify`：** 部署时默认忽略 `.mintlify` 目录，但 `skills` 和 `specs` 除外。

<div id="bug-fixes">
  ## 错误修复
</div>

  - 搜索域名路径的大小写在整个链路中得以保留。
  - 模糊搜索现在只会回退到接近拼写错误的匹配，而不再是宽泛的模糊命中。
  - 卡住的搜索索引任务由新的看门狗机制恢复，搜索 AST 上传在 S3 出现瞬时故障时会重试，而不会静默丢弃页面。
  - 对于跨上下文交叉列出的页面，搜索会发出针对每个上下文的版本和产品标签。
  - 预览构建现在会清晰显示分支已删除的情况，而不是显示误导性的构建错误。
  - 编辑器现在可以正确往返处理任务列表复选框、表格转义、LaTeX 数学公式、callout 的 `type` 属性、视频源、图片标题，以及表格单元格内包含竖线的内联代码。
  - 编辑器允许在 `Columns` 中放置块级内容，嵌套的块不会被静默删除。
  - PDF 导出现在始终以浅色模式渲染，无论读者的主题设置为何。
  - `.md` 页面的内部链接和 `Link` 请求头会遵循您的 `basePath`。
  - 受身份验证保护的文档现在会在搜索代理中正确刷新过期的终端用户 JWT，并在允许列表内的部署中为缺失页面返回真正的 404。
  - Assistant Widget 会在提交前等待 hCaptcha 准备就绪，并在重新加载后保留历史响应操作。
  - 子路径域名设置在 DNS 配置期间会保留其待处理状态，并在不适用时隐藏 DNS 迁移步骤。
  - `luma` 和 `sequoia` 主题在居中页面模式下能正确居中内容，`maple` 主题则会让 API 侧边面板保持在标签导航之下。
  - dashboard 中重名的页面现在会追加 slug 而不再冲突，产品重定向也会保留身份验证片段。
  - 密码认证在没有相应授权时会以只读方式渲染，而不会导致设置页面崩溃。
...
<div id="editor-improvements">
  ...
  - **草稿：** 无需创建分支或使用任何 Git 命令，即可处理未发布的更改。有关更多信息，请参阅[草稿](/zh/editor/drafts)。
