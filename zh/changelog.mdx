...
<Update label="2026年9月8日" tags={["新版本"]} rss={{ title:"基于结果的积分定价" }}>

<div id="outcome-based-pricing">
  ## 基于结果的定价
</div>

  - **每个操作的固定积分价格：** 每个 AI 操作都有固定的积分价格，只有当操作产生结果时才需要付费。assistant 回答一个问题花费 25 个积分。更新文档的自动化运行花费 250 个积分。不产生任何结果的操作不花费积分，包括 assistant 无法回答的消息，以及未发现需要更新的内容或失败的自动化运行。
  - **免费的 agent 对话：** 在 Pro 和 Enterprise 计划中，与编辑器 agent 以及 Slack 中的 agent 的对话不消耗积分。
  - 无论 agent 读取多少内容或操作运行多长时间，操作的价格都相同，因此你可以仅根据用量规模来预估消耗。你的计划、其包含的积分以及每个积分的价格保持不变。详情请参见 [积分定价](/zh/credits)。
