...
<Update label="January 23, 2026" tags={["New releases", "Improvements"]} rss={{ title: "skills.md、审计日志和 Markdown 导出" }}>
  ...
  文档站点现在会基于你的 API 参考、指南和功能自动生成一个 `skill.md` 文件，为 AI Agent 提供一个完整的技能文件，其中包含关于如何帮助用户使用你产品的上下文信息。`skill.md` 以更结构化、更易采用的方式取代了原先的 `install.md` 方案。如果你的文档存储库根目录下已有自定义的 `skills.md` 文件，它将替换自动生成的版本。更多信息请参见 [skill.md](/zh/ai/skillmd)。
  ...
  当请求头中包含 `Accept: text/markdown` 时，文档页面现在会返回原始 Markdown，这有助于 AI Agent 更好地读取和处理内容。

  ## 功能改进

  * 将认证、部分认证和个性化体验合并为统一的认证体验。更多信息请参见 [认证设置](/zh/deploy/authentication-setup)。
