...
<div id="deployment-and-git">
  ...
  - **即时回滚：** 从 dashboard 的活动历史中恢复符合条件的部署。目前尚不支持使用身份验证或 AsyncAPI 规范的部署。
  - **编辑器自动提交：** 在编辑器设置中开启自动提交，边输入边将您的工作保存到当前分支。部署分支上的自动提交由同一设置控制，新分支上的首次自动提交会自动打开一个 pull request。
  - **Bitbucket 支持：** Enterprise 客户可以将 Bitbucket 仓库连接为 Git 提供商，并从中部署文档。Slack agent 和 automations 尚不支持 Bitbucket 仓库。
