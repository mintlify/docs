...
<Update label="March 6, 2026" tags={["New releases", "Improvements"]} rss={{ title: "从公开 GitHub 存储库自动生成文档、AI 助手多模态功能，以及更多改进" }}>
  ## 从公开存储库自动生成文档

  从任何公开 GitHub 存储库 [自动生成完整的文档站点](https://mintlify.com/explore)。将任意公开存储库 URL 中的 `github.com` 替换为 `mintlify.com`，即可立即预览生成的文档。如果你是该存储库的所有者，可以认领该文档站点，将其添加到你的 Mintlify 组织中并进行自定义。

  ## AI 助手改进

  [AI 助手](/zh/ai/assistant) 现在支持多模态输入。可向消息附加文件以提供更多上下文。你可以直接将图片、PDF 和代码文件拖放或粘贴到聊天中。

  AI 助手现在还可以生成页面特定的[示例问题](/zh/ai/assistant#add-sample-questions)。

  AI 助手现在还会根据用户当前所在页面构建上下文，并为整个 OpenAPI 规范建立索引。
  ...
  * 修复了受保护文档中的 OAuth 和 JWT 重定向问题。
  * 修复了在编辑器中粘贴文本时光标跳动的问题。
  * 修复了用于取消 AI 助手消息的键盘快捷键。
  * 修复了带下划线的 branch 名称在编辑器中无法使用的问题。
