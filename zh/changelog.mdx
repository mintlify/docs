...
<Update label="10月24日" tags={["Improvements"]} rss={{ title: "每周更新", description: "控制台增强、API 操作台改进、认证修复和性能优化" }}>
  ## 洞察改进

  - 优化了洞察页面，修复了“今天”和“昨天”的日期选择器
  - 增强了反馈表，改进了搜索功能
  - 为分类表新增加载状态
  ...
  - 在控制台中新增助手位置配置
  - 改进了助手回复，修复了上下文获取与工具消息的问题

  ## API 操作台与 OpenAPI 增强

  - 重构了 API 操作台的端点字段，采用新的架构图以提升性能
  - 在助手侧边栏打开时，改进了操作台弹窗中的 API 示例展示
  - 修复了紧邻 `$refs` 与 `x-` 属性的其他属性在 OpenAPI 解析中丢失的问题
  - 修复了 API 操作台弹窗标题样式

  ## Web 编辑器与部署增强

  - 恢复了手动创建预览按钮，便于创建预览部署
  - 在控制台中新增用于获取 branch 保护数据的提取器
  - 在控制台中新增用于跟踪 PR 状态的提取器
  ...
  - 在 Web 编辑器中实现了完整的 Git 同步，以更好地同步存储库
  - 缓存了 MCP 响应以提升性能
  - 改进了站点地图以规范化 `/index` 路径

  ## Bug 修复与可靠性

  - 修复了 API 操作台中认证预填值未正确填充的问题
  - 修复了来自 `seo.metatags` 的自定义 meta 标签在客户端水合后仍能保留
  - 使用 `break-all` 样式修复了页面标题文本溢出
  - 移除了页面尺寸选项的全宽限制
  - 修复了接受组织邀请的按钮尺寸
  - 修复了在非 macOS 电脑上快捷键显示为不带加号的“Ctrl+I”的问题

</Update>

<Update label="10月17日" tags={["Improvements"]} rss={{ title: "每周更新", description: "AI 助手改进、导航增强、API 操作台更新和问题修复" }}>
  ## 助手与 AI 改进

  - 升级了助手提示词，以获得更高准确度和更具上下文感知的回复
  - 新增置信度评分，提升助手回复的决策质量
  - 增强助手搜索，使用并行文档查询以更快返回结果
  - 修复了助手消失的问题，并提升了其在小页面上的可见性

  ## 导航与 UI 增强

  - 为导航组件添加了稳定的滚动条槽，确保布局一致
  - 使“本页内容”部分可点击，可快速滚动到页面顶部
  - 修复了所有主题中的助手栏定位
  - 改进了助手栏以避免与内容重叠
  - 修复了 Almond 主题下宽页面的最大宽度
  - 调整了下拉箭头图标尺寸，使导航分组外观一致

  ## API 操作台与 OpenAPI 改进

  - 为新的 API 参考页面设计添加了功能开关支持
  - 改进了 OpenAPI 架构图工具，正确处理组合关系
  - 在操作数据中添加了端点类型，以改进 API 文档
  - 修复了当枚举与表格以 Markdown 编写时 API 操作台的说明渲染
  - 调整了 API 操作台中说明的渲染位置以获得更佳布局

  ## Bug 修复与可靠性

  - 修复了切换语言时会跳转到首页的问题，现保持在同一页面
  - 修复了断链 CLI 在 CI/CD 集成中以代码 1 退出
  - 修复了本地字体路径，现可正确添加前导斜杠
  - 修复了 `docs.json` 中重定向处理忽略前导斜杠的问题
  - 修复了规范 URL 对 `/index` 的剥离以提升 SEO
  - 修复了折叠面板内有序列表的位置
  - 修复了 `<span>` 标签样式，使其行为类似 `<p>` 标签
  - 修复了列表中首个 `<p>` 标签的块级显示
  - 修复了 OG 图片在索引页显示分区名称
  - 修复了图标路径以包含 `BASE_PATH`，以支持相对路径
  - 移除了助手背景模糊以提升性能
  
</Update>

<Update label="10月11日" tags={["Improvements"]} rss={{ title: "每周更新", description: "API 操作台改进、洞察增强、Web 编辑器更新以及错误修复" }}>
  ## AI 助手与 Agent AI 功能

  * 为 agent 新增 list_pull_requests 和 list_commits 工具，可一次性为某个日期范围内的 PR 或多个 PR 生成文档
  * 将 agent 和助手升级至 Claude Sonnet 4.5
  * 改进助手搜索，可并行查询文档，加快回复速度
  * 修正会话长度统计，排除工具调用
  ...
  * 去除编辑器中代码块背景，使外观更简洁
  * 修复 branch 创建流程，正确跳转到新创建的 branch
  * 新增部署历史分页
  * 为预览部署表格新增游标分页
  * 修复预览部署仅返回预览环境部署
  * 优化控制台 UI/UX，改进间距、输入处理和布局
  * 改进控制台助手聊天中的链接处理
  * 修复 Firefox 中聊天助手的描边/轮廓显示问题

  ## API 操作台增强

  * 改进 API 操作台模态框的渲染
  * 修复 API 操作台中可展开项与锚点链接不兼容的问题
  * 新增在 API 操作台中合并示例与用户输入的 body 的支持
...
<Update label="10月3日" tags={["New releases", "Improvements"]} rss={{ title: "每周更新", description: "产品导航、无障碍 CLI、API 操作台增强，以及大幅的无障碍改进" }}>
  ## 新功能

  * **产品导航**：使用产品切换器导航，为多个产品组织文档
  * **无障碍命令行界面（CLI）命令**：运行 `mint a11y` 测试文档的无障碍问题
  * **API 操作台中的视频响应**：API 操作台现已支持显示视频响应类型
  * **API 操作台预填**：新增选项，可预填 API 操作台示例以更快测试
  * **未认证反馈**：对使用部分认证的站点，用户现在可在未认证状态下提交反馈
  ...
  * 修复浅色模式颜色回退与主题相关样式问题
  * 修复 API 操作台中可展开项与锚点链接不兼容的问题
  * 修复 `.md` 链接对 ChatGPT 与其他 LLM agent 的可访问性
  * 修复按百分比宽度与高度设置的图片尺寸处理
  * 修复带语言标签与自定义主题的代码块渲染问题
  * 修复折叠面板链接的格式与样式
  * 在助手配置为空时修复页脚间距
  * 修复本地存储库图片的卡片 icon 渲染
  * 修复 API 操作台中的音频文件处理（防止 UTF-8 解码）
  * 改进 Web 编辑器中的 PR 发布状态管理
</Update>
...
<Update label="September 26" tags={["Improvements"]} rss={{ title: "每周更新", description: "语言支持扩展、UI 改进和基础设施增强" }}>
  ...
  * 通过跳过高开销的 Imgix 自动格式化来增强 GIF 处理，减少处理时间和带宽占用
  * 通过对动画内容的专项处理优化图片服务性能
...
<Update label="September 19" tags={["Improvements"]} rss={{ title: "每周更新", description: "Web 编辑器与认证改进，以及 Bug 修复" }}>
  ## Web 编辑器与控制台登录改进

  * 持续推进 Web 编辑器的 app router 迁移，移除阻塞并提升性能
  * 增强登录错误处理，提供明确的邮箱校验信息
  * 修复邮箱地址验证中的空白字符处理

  ## 认证改进

  * 在登录回调中新增对已验证重定向参数的支持，以降低登录阻力
  * 改进 SSO 登录流程，优化重定向处理
...
<Update label="September 13" tags={["New releases", "Improvements"]} rss={{ title: "每周更新", description: "AI 404 建议、AI 助手网页搜索接入外部来源、安全增强与可靠性修复" }}>
  ## 重大发布

  * 重大增强：在 404 页面提供 AI 推荐页面，[当用户访问死链 → AI 代理读取路径 → 语义推荐相似页面](https://x.com/mintlify/status/1966625627773059495)
  * 重大发布：AI 助手的网页搜索现可包含外部来源\
    注：如需为你的网站启用此功能，请联系我们。

  ## AI 助手 与 MCP

  * 修复因时间窗口未正确滑动导致 AI 助手被误限流的问题
  * 修复助手工具调用未正确处理空 `text` 块的问题
  * 修复将 MCP 服务器名称与工具调用拼接后有时超过 MCP 客户端强制的 60 字符限制的问题
  * 修复助手菜单高度远超视口并无限滚动的问题
  * 修复控制台中助手花费数值可能显示超过两位小数的问题

  ## Web 编辑器与部署

  * 增强编辑器安全性，仅具备已连接 Git 托管存储库“写入权限”的用户可进行更改
  * 修复包含 `=` 的 branch 名称无法进行预览部署的问题
  * 修复过长的 branch 名称在创建预览部署时导致模态溢出的问题
  * 体验优化：在注册邀请中，email query 参数将预填输入框
  * 修复通过上下文菜单复制页面在 Safari 上不可用的问题

  ## API 操作台 与 导航

  * 多个 API 操作台响应码在聚焦时现显示于受控样式的选择菜单，而非系统默认选择菜单
  * 你现在可以在 docs.json 的导航 groups 上使用[``expanded 字段使其默认展开``](https://mintlify.com/docs/navigation#default-expanded-state)

  ## SEO 与 UI

  * 通过与各站点的文档站相同的 URL 提供 favicon，修复其在搜索引擎中未显示的问题
  * 修复 YouTube 嵌入在加载时闪烁的问题
  * 修复展开反馈菜单以包含文字回复时导致目录布局位移的问题
  * 修复在 Maple 主题关闭通知横幅后文字会溢出至顶栏之上的问题
  * 增强 Maple 与 Willow 主题，在侧边栏新增登录/登出按钮以便更易访问

  ## Analytics 与导出

  * 修复 AI 助手分析视图与导出的可靠性问题
  * AI 助手分析导出现于后台执行并通过电子邮件发送，以获得更可靠的体验
</Update>
