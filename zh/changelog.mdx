...
<Update label="January 9, 2026" tags={["New releases", "Improvements"]} rss={{ title: "隐式代码片段与面向所有用户的 AI 助手对话洞察" }}>
  ...
  [对话洞察](/zh/ai/suggestions#conversation-insights) 现已对所有同时启用了 Agent 和 AI 助手的文档站点开放。Agent 会分析用户与 AI 助手对话中的模式，以识别文档空白并给出改进建议，帮助你主动解决用户的常见问题。
...
<Update label="December 23, 2025" tags={["New releases", "Improvements"]} rss={{ title: "Web 编辑器改进与 AI 助手洞察" }}>
  ...
  * **稳定性改进**：改进了整体稳定性和移动端的响应式表现。
  ...
  [Agent](/zh/ai/agent) 现在可以基于[与 AI 助手的对话](/zh/ai/suggestions#conversation-insights)来建议文档更新。启用后，Agent 会分析用户向 AI 助手提出的问题，以识别哪些模式表明文档存在缺失或表述不清。
...
<Update label="December 19, 2025" tags={["Improvements"]} rss={{ title: "Agent、AI 助手和编辑器改进" }}>
  ...
  * 新增支持从 `AGENTS.md` 文件中读取[自定义说明](/zh/ai/agent#customize-agent-behavior)。
  * 如果你在控制台中[连接 GitHub 账号](/zh/ai/agent#connect-your-github-account)，Agent 现在会以你的 GitHub 账号身份打开 PR。

  ## AI 助手改进

  * 新增复制对话按钮，便于分享或保存与 AI 助手的对话。
  * 改进了对流式响应卡顿/中断场景的处理，使 AI 助手的回复更加可靠。
  * 改进了 AI 助手在移动设备上的展示效果。

  ## Web 编辑器改进

  * 在编辑器工具栏中新增[可分享的预览链接](/zh/editor#share-preview-deployments)。
  * 更新了[发布工作流](/zh/editor#publish-your-changes)，支持一键发布或创建拉取请求。
  * 使用 Web 编辑器时不再需要通过 GitHub 登录。

  ## 新手引导（Onboarding）改进

  * 在注册过程中，你可以在 Mintlify 托管的文档存储库和你自己的存储库之间进行选择。
  * 你可以在不连接 GitHub 账号的情况下创建文档站点。
  ...
  * API 操作台中的 UI 元素（包括 “Try it” 和 “Send” 按钮）现已在所有受支持的语言中完成本地化。
</Update>

<Update label="December 12, 2025" tags={["New releases", "Improvements"]} rss={{  title: "Agent 建议与问答机器人" }}>
  ## Agent 建议

  Agent 现在可以监控 Git 仓库并[建议](/zh/ai/suggestions)文档更新。当检测到面向用户的代码发生变更时，这些建议会出现在 Agent 面板中。建议中会包含相关的拉取请求（pull request）以及拟议的文档更新列表。你可以将这些建议添加为 Agent 的上下文，让其创建拉取请求，或者直接将其忽略。
...
<Update label="December 8, 2025" tags={["New releases"]} rss={{ title: "Last modified dates", description: "Show when pages were last updated with automatic timestamps" }}>
  ...
  现在你可以在文档中的所有页面上显示“最后修改”时间戳。通过在 `docs.json` 配置中设置 [`timestamp`](/zh/organize/settings#param-timestamp) 开关即可启用此功能。

  启用后，每个页面都会自动显示其最近一次更新的时间，帮助读者判断内容是否为最新。
</Update>
...
<Update label="2025 年 11 月 21 日" tags={["Improvements"]} rss={{ title: "每周更新", description: "API playground 增强、本地化更新以及 CLI 改进" }}>
  ...
  * 修复了在切换具有不同响应码的端点时响应区域会消失的问题。现在在导航到新的端点时，playground 会正确重置到第一个可用的响应码。
  * 修复了 HTTP Basic 认证方案中重复显示 &quot;Authorization&quot; 头的问题。认证区域现在展示了更简洁的界面，并带有正确的字段标签和说明。
  * 改进了在嵌套对象和数组中对 `oneOf` 模式的处理。具有多种类型选项的复杂 API 模式现在会在合适的场景下正确渲染为下拉菜单，而不是选项卡。
  * 修复了在 OpenAPI 规范中定义在端点级别但未展示的路径参数。现在在各个 operation 级别之上定义的参数会正确包含在主页面和 playground 中。
  * 增强了响应示例生成逻辑，即使示例只指定了部分字段，也会包含所有必填属性。这可以确保响应示例完整且准确。
  * 修复了对象数组中缺失预填充示例数据的问题。带有示例数据的数组项现在会在 playground 中按正确索引自动填充。
  * 改进了整个 API playground 中说明文本的内边距和间距，以提升可读性和视觉一致性。

  ## 本地化改进

  * 改进了所有支持语言中的上下文菜单选项和 AI 助手状态消息的翻译。

  ## CLI 改进

  * 为在 Node.js 版本低于 20.17 时运行 `mint dev` 的用户添加了错误消息。用户会被引导升级到一个 LTS 版本。
</Update>
