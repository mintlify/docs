...
<Update label="2026年8月5日" tags={["新版本"]} rss={{ title:"推出 Mintlify Index" }}>

<div id="mintlify-index">
  ## Mintlify Index
</div>

  [Mintlify Index](/zh/search-index) 为编程代理提供一个检索层，用于访问发布者维护的技术文档和网络内容。

  - **公共 MCP 服务器：** [连接](/zh/search-index/connect)编程代理，无需 API 密钥。`context` 工具会为实现任务返回带有来源引用的资料。
  - **访问控制 REST API：** 搜索排序后的来源，在令牌预算内组装上下文，并通过独立的 API 端点检索选定的内容。需要 Index API 密钥。
...
<div id="new-releases">
  ...
  - **Google Drive 代理集成：** 代理现在可以从 Google Drive 拉取上下文，与 Slack、Notion、Linear、Jira 等[代理集成](/zh/automations/integrations)并列。
...
<div id="new-releases">
  ...
  - **Workflows 现已更名为 Automations：** 此前被称为 workflows 的功能在仪表盘和产品中现已统一更名为 [Automations](/zh/automations)，为计划运行和触发运行采用统一名称。
...
<div id="agent-onboarding">
  ...
  - **在入门过程中连接集成：** 作为设置的一部分，添加第三方[代理集成](/zh/automations/integrations)——包括 Confluence、Notion 和 Plain。
...
<div id="billing">
  ...
  - **更高的自助积分档位：** 自助积分档位现在最高可达 52,500。详情见[积分](/zh/credits)。
...
<div id="asyncapi-nested-groups">
  ...
  `asyncapi` 现支持[嵌套分组](/zh/api-playground/asyncapi-setup)，因此你可以在更广泛的 API 分组的小节内生成频道页面，或在同一父级下合并多个 AsyncAPI 规范。
...
<div id="improvements">
  ...
  - 编辑器现在可以解析拆分配置文件中的 [`$ref` 引用](/zh/organize/settings#split-configuration-with-ref)。
...
使用 [`$ref` 引用](/zh/organize/settings#split-configuration-with-ref)将大型配置拆分为更小的文件。每个 `$ref` 指向一个单独的 JSON 文件，在构建时解析。

<div id="data-active-css-selectors">
  ## `data-active` CSS 选择器
</div>

侧边栏项目、目录条目和导航下拉菜单现在暴露了一个稳定的 [`data-active` 属性](/zh/customize/custom-scripts)，用于自定义 CSS 定位。使用 `.toc-item[data-active]` 或 `#sidebar-content li[data-active]` 等选择器来设置活动状态的样式，无需依赖内部类名。

<div id="agent-skills-discovery-endpoint">
  ## Agent-skills 发现端点
</div>

文档站点现在提供 `/.well-known/agent-skills/` 发现端点，遵循 [agent-skills 0.2.0 规范](https://schemas.agentskills.io/discovery/0.2.0/schema.json)。AI agent 可以通过编程方式发现并获取你的 [skill 文件](/zh/ai/skillmd#skills-discovery-endpoints)，并进行内容完整性验证。
...
<Update label="February 20, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Agent 配置文件与自定义前端" }}>
  ...
  在你的存储库中使用 `.mintlify/Assistant.md` 和 `.mintlify/AGENTS.md` 文件来自定义 agent 和 AI 助手的行为。更多信息请参见 [自定义助手行为](/zh/assistant/customize) 和 [自定义代理行为](/zh/agent/customize)。
...
<Update label="January 30, 2026" tags={["New releases", "Improvements"]} rss={{ title: "简化技能安装、焕新的 Analytics 页面、编辑器改进等" }}>
  ...
  * 稳定性改进
  ...
  AI 助手现在可以按参数来[筛选](/zh/ai/model-context-protocol#search-parameters) MCP (Model Context Protocol) 搜索结果。
