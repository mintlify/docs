...
<Update label="December 3" tags={["New releases"]} rss={{ title: "Assistant query bucketing", description: "Automatically group similar assistant queries to identify common user questions" }}>
  ## AI 助手查询分组
...
<Update label="November 21" tags={["Improvements"]} rss={{ title: "Weekly Updates", description: "API playground enhancements, localization updates, and CLI improvements" }}>
  ...
  * 修复了在不同响应码的端点之间切换时，响应区域会消失的问题。现在在导航到新端点时，操作台会正确重置到第一个可用的响应码。
  * 修复了在 HTTP Basic 认证方案中显示双重 &quot;Authorization&quot; 请求头的问题。认证区域现在提供了更整洁的界面，并展示了正确的字段标签和说明。
  * 改进了在嵌套对象和数组中的 `oneOf` 模式处理。具有多种类型选项的复杂 API 模式现在会在合适的情况下正确渲染为下拉菜单，而不是选项卡。
  * 修复了在 OpenAPI 规范中定义在端点级别的路径参数缺失的问题。现在在单个操作级别之上定义的参数会正确包含在主页面和操作台中。
  * 增强了响应示例的生成逻辑，即使示例中只指定了部分字段，也会包含所有必填属性，从而确保响应示例完整且准确。
  * 修复了对象数组中预填示例缺失的问题。带有示例数据的数组项现在会在操作台中正确填充，并具有正确的索引。
  * 改进了 API 操作台中说明文本的内边距和间距，以提升可读性和视觉一致性。

  ## 本地化改进

  * 增强了上下文菜单选项和 AI 助手状态消息在所有支持语言中的翻译质量。

  ## CLI 改进

  * 为在 Node.js 20.17 以下版本上运行 `mint dev` 的用户新增错误提示，并引导用户升级到 LTS 版本。
</Update>
