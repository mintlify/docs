...
<Update label="July 17, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"FileTree 列表语法、焕新的 Assistant 小组件等" }}>

<div id="new-features">
  ## 新功能
</div>

  - **FileTree 列表语法：** 在 `<FileTree>` 块内使用熟悉的 Markdown 列表来创建[文件树](/zh/components/tree)。末尾的斜杠和嵌套列表会自动转换为文件夹，你可以省略显式的 `Tree.Folder` 和 `Tree.File` 标签。

<div id="improvements">
  ## 改进
</div>

  - **焕新的 Assistant 小组件：** 可嵌入的 [Assistant 小组件](/zh/guides/assistant-embed)现在与产品内的 Assistant 对话界面一致，并且会话状态与实时上下文保持同步。
  - **更精简的构建日志：** 部署构建日志更加精简,让你能更快地找到关键信息。
  - **事件横幅位置：** 状态事件横幅现在显示在 [dashboard](https://app.mintlify.com) 的顶部，可见性更强。

<div id="bug-fixes">
  ## 错误修复
</div>

  - 修复了 OAuth 重定向 URL，使已认证文档登录使用规范的显示域名，并在注销后将你保留在面向浏览器的主机上。
  - 修复了一个缓存问题：当上游返回 5xx 时，可能会提供过期的 `/login` 404。
  - 当指标为零且无变化时，Analytics 不再显示 delta 标签，并且 `cta_click` 事件在所有主题中都一致地发出。
  - Assistant 小组件的分区重新排序现在能正确保存。
  - Slack 应用安装不再请求未批准的作用域。
