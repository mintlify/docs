...
<Update label="2025 年 11 月 21 日" tags={["Improvements"]} rss={{ title: "每周更新", description: "API playground 增强、本地化更新以及 CLI 改进" }}>
  ...
  * 修复了在切换具有不同响应码的端点时响应区域会消失的问题。现在在跳转到新端点时，playground 会正确重置为第一个可用的响应码。
  * 修复了 HTTP Basic 认证方案中重复显示 &quot;Authorization&quot; 头的问题。认证区域现在使用更简洁的界面，并提供正确的字段标签和说明。
  * 改进了在嵌套对象和数组中对 `oneOf` schema 的处理。包含多种类型选项的复杂 API schema 现在会在合适的场景下正确渲染为下拉菜单，而不是使用选项卡。
  * 修复了 OpenAPI 规范中定义在端点级别却未正确显示的路径参数问题。现在，定义在各个 operation 之上的参数会正确出现在主页面和 playground 中。
  * 增强了响应示例生成逻辑，即便示例中只指定了部分字段，也会补全所有必填属性，从而确保响应示例完整且准确。
  * 修复了对象数组中缺失预填示例数据的问题。带有示例数据的数组项现在会在 playground 中按正确索引位置进行预填。
  * 改进了 API playground 中说明文字的内边距和间距，以提升可读性和视觉一致性。
