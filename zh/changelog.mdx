...
<Update label="2026年8月14日" tags={["新版本","改进","错误修复"]} rss={{ title:"即时回滚、编辑器自动提交、GitHub 组件等" }}>

<div id="new-features">
  ## 新功能
</div>

  - **即时回滚：** 从 dashboard 的活动历史中回滚到任意先前的部署。回滚立即生效，部署完成后发布状态会随之刷新。
  - **编辑器自动提交：** 在编辑器设置中开启自动提交，边输入边将您的工作保存到当前分支。部署分支上的自动提交由同一设置控制，新分支上的首次自动提交会自动打开一个 pull request。
  ...
  - **编辑器 agent 评论与建议：** 编辑器 agent 现在可以在您的文档中与其编辑一起留下评论和内联建议。
  - **批量成员邀请：** 从 dashboard 通过一次请求即可邀请多位队友加入您的组织。
  - **Stripe 与扩展的 Notion 集成：** 将 [Stripe](/zh/automations/integrations) 作为只读集成连接，并在 Slack 和 automations 中使用更多 Notion 读取工具。
  - **Zendesk 与 Pylon 支持平台：** 连接 [Zendesk 与 Pylon](/zh/automations/integrations)，让 agent 能够使用这些工具中的上下文。

<div id="improvements">
  ## 改进
</div>

  - **重新设计的版本历史：** 版本历史抽屉现在是按日期分组的时间线，更易于浏览。
  - **私密页面：** 私密页面拥有自己的版本历史、agent 注释以及显示真实页面标题的通知邮件。您也可以将私密页面拖到共享文件夹上以将其提升为共享页面。
  - **编辑器分支创建：** 分支和 pull request 名称会随您的输入自动生成；从受保护分支创建分支时可以带上或保留待处理的更改；分支创建速度也明显加快。您还可以在编辑器中编辑分支的 pull request 标题。
  - **分支中的协作者：** 分支下拉列表会显示每个分支上的协作者，头像会在 Git 和编辑器身份之间去重，pull request 作者也会包含在贡献者列表中。对编辑器分支的外部推送会实时同步到编辑器中。
  - **Personal 与 Shared 分区：** 编辑器侧边栏被重新组织为 **Personal** 和 **Shared** 分区，让私密草稿与共享内容更易于区分。
  - **实时预览：** agent 生成的新页面会被自动跟随，正在生成的页面会显示待处理的脉冲状态，实时预览不再按间隔轮询分支 diff。
  - **Assistant Widget：** 该 widget 现已正式发布。安装在初始设置中被优先处理，初始配置加载更快。
  - **入门引导：** agent 辅助的入门引导流程会流式展示设置进度，一旦就绪就展示完成 UI，并将入门会话纳入 agent 历史。
...
<div id="improvements">
  ...
  - **个性化功能移至 add-ons：** [个性化](/zh/create/personalization)设置现在位于 dashboard 的 add-ons 下，且不再需要自定义基础路径。
