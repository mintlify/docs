...
<Update label="2026年8月7日" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Bitbucket、编辑器版本历史、更深入的分析等" }}>

<div id="new-features">
  ## 新功能
</div>

  - **Bitbucket 支持：** 在 GitHub 和 GitLab 之外，现在还可以从 dashboard 连接 Bitbucket 仓库以同步您的文档。详情请参阅 [CI](/zh/deploy/ci)。
  - **编辑器版本历史：** 浏览编辑器的历史版本、与当前草稿进行对比，并可从页面直接恢复到之前的版本。
  - **复制页面：** 通过编辑器中的页面选项菜单复制页面。
  - **已认证的实时预览：** 在编辑器内直接预览需要认证访问的页面草稿，无需退出登录。
  - **OpenAPI 3.1 文件上传：** API playground 现在可以识别上传到 `FileInput` 字段的 OpenAPI 3.1 文件，且文件输入支持拖放。详情请参阅 [OpenAPI 设置](/zh/api-playground/openapi-setup)。
  - **预填 API playground 服务器变量：** 新的客户端 API 允许自定义脚本在 API playground 中预填服务器变量。
  - **多个 JWT 登录 URL：** 可为每个部署配置多个 [JWT 认证](/zh/deploy/authentication-setup)登录 URL，并可为下游登录流程传递额外的 `loginOptions` 元数据。
  - **Pylon 集成：** 将 [Pylon](/zh/automations/integrations) 作为只读集成连接，使代理可以在 Slack 和自动化中使用 Pylon 上下文。
  - **新增集成 allowlist 条目：** [LaunchDarkly](/zh/automations/integrations)、[PostHog](/zh/automations/integrations) 和 [Salesforce Service Cloud](/zh/automations/integrations) 现已作为代理集成提供，并配有各自的正式 logo。
  - **用户流量分析：** 分析中新增的用户流量路径卡片展示了访客在您的文档中所走的路径，每一步都会显示流失情况，并为长尾路径提供 "Other" 分组。
  - **漏斗分析：** 新的转化漏斗卡片涵盖用户漏斗、搜索漏斗和助手转移漏斗，包含 "took an action" 阶段。
  - **交互总览：** 新的交互总览卡片将 CTA 点击、搜索和助手活动汇总在同一处展示。

<div id="improvements">
  ## 改进
</div>

  - **Impressions 更名为 User engagements：** [analytics](/zh/optimize/analytics) 中的 Impressions 部分现更名为 User engagements，并在 CTA 点击率之外新增了 actions 线和 action-rate 列。
  - **编辑器评论重新设计：** 编辑器中的评论采用了全新的布局，标准键盘快捷键现在也可在评论、建议和源码模式下使用。
  - **建议模式徽标：** 编辑器顶部栏新增的徽标可以更清晰地显示您当前所处的编写模式。
  - **分支下拉菜单：** 编辑器的分支下拉菜单采用了全新的行设计，支持模糊搜索，并为您当前所在的分支提供实时标签。
  - **嵌套的私密页面：** 创建嵌套的私密页面，并可在移动节点时级联应用具备授权感知的权限。
  - **私密页面 @ 提及：** 在编辑器代理编辑器中使用 `@` 按标题引用私密页面。
  - **代理编写的编辑：** 编辑器可将由代理创建的评论和建议内联渲染，发送操作现在也采用乐观更新。
  - **Git 网关重新设计：** dashboard 中的 Git 连接网关采用了全新的设计。

<div id="bug-fixes">
  ## 错误修复
</div>

  - **过长标题：** 标题中过长的不可断开 token 不再溢出容器。
  - **分页标题：** 分页标题的截断在所有主题中都能一致地工作。
  - **Sitemap：** Sitemap URL 不再重复叠加自定义基础路径。
  - **API playground 无障碍：** 端点复制控件现在可被屏幕阅读器访问，下拉项的圆角保持一致。
  - **焦点轮廓：** 焦点轮廓不再在容器边缘被裁剪，导航栏下拉菜单的轮廓仅在键盘聚焦时出现。
  - **侧边栏项：** 纯文本侧边栏项现在以完整宽度渲染。
  - **预览握手：** 预览认证握手会重定向回您起始的确切主机（包括挂载路径），因此深层链接可以正确解析。
  - **集成 logo：** Salesforce Service Cloud 和 OneDrive 现在会显示各自真正的 logo。
  - **MDX 注释：** 内联的 MDX 注释会从渲染输出中剥离。
  - **静态导出链接：** 编译时 `.html` 链接模式现在可正确解析带版本和带尾部斜杠的 slug。
...
<div id="new-features">
  ...
  - **展示次数分析：** [analytics](/zh/optimize/analytics) 中新增的 Impressions 部分通过图表和排序表展示 CTA 点击率，让您可以了解哪些行动号召能够带来互动。
  ...
  - **Top searches 中的搜索源选择器：** 在单个视图中按来源（包括 MCP 搜索）筛选 [Top searches](/zh/optimize/analytics#search)。
...
<div id="analytics-and-data-streaming">
  ...
  - **Analytics 数据仓库流式传输：** Enterprise 计划可将 analytics 事件近实时传输到 Amazon S3，并控制要包含的事件类别和部署。请参阅 [Analytics 流式传输](/zh/optimize/analytics#stream-analytics-events)。
