---
title: "产品更新"
description: "新版本与功能改进"
rss: true
noindex: true
---

<Update label="11 月 21 日" tags={["改进"]} rss={{ title: "每周更新", description: "API playground 增强、本地化更新以及 CLI 改进" }}>
  ## API playground 改进

  * 修复了在具有不同响应码的端点之间切换时，响应区域会消失的问题。现在在导航到新端点时，playground 会正确重置为第一个可用的响应码。
  * 修复了 HTTP Basic 认证方案中重复显示 "Authorization" 请求头的问题。认证区域现在提供了更简洁的界面，以及更清晰的字段标签和描述。
  * 改进了对嵌套对象和数组中 `oneOf` schema 的处理。具有多种类型选项的复杂 API schema 现在会在合适的场景下正确渲染为下拉菜单，而不是选项卡。
  * 修复了在 OpenAPI 规范中定义在端点级别的路径参数缺失的问题。现在位于各个操作之上的参数会正确包含在主页面和 playground 中。
  * 增强了响应示例的生成逻辑，即使示例只指定了部分字段，也会包含所有必需属性。这样可以确保响应示例更加完整且准确。
  * 修复了对象数组中缺失预填示例的问题。带有示例数据的数组项现在会在 playground 中根据正确的索引进行预填。
  * 改进了整个 API playground 中描述文本的留白与间距，以提升可读性和视觉一致性。

  ## 本地化改进

  * 改进了所有支持语言中的上下文菜单选项翻译以及助手状态消息。

  ## CLI 改进

  * 针对在 Node.js 20.17 以下版本上运行 `mint dev` 的用户新增了错误提示信息，并引导用户升级到 LTS 版本。
</Update>

<Update label="11 月 15 日" tags={["New releases", "Improvements"]} rss={{ title: "每周更新", description: "Badge 组件、API playground 改进、自定义语言支持和性能增强" }}>
  ## 新功能
