...
<Update label="February 6, 2026" tags={["New releases", "Improvements"]} rss={{ title: "自助配置 SSO、查看者角色、人类/AI 流量切换以及斜杠命令菜单改版" }}>
  ## 自助配置 SSO

  企业客户现在可以直接在控制台中自行配置 SAML SSO（Okta 或 Microsoft Entra），包括即时（JIT）开通账号、切换身份提供商以及移除连接。

  ## 查看者角色

  新增查看者角色，可对你的控制台进行只读访问。查看者可以在编辑器中查看内容，但无法编辑内容、邀请成员或修改配置。

  ## 人类/AI 流量切换

  Analytics 页面现在分别展示人类流量和 AI 流量标签。

  ## 斜杠命令菜单改版

  编辑器中的斜杠命令菜单现在支持输入即搜索、更佳的键盘导航以及其他用户体验改进。

  ## 错误修复

  * 修复了控制台表格中单元格内容溢出的问题。
  * 现在通过编辑器中的配置面板重命名文件可以正确保存变更。
  * MDX 解析错误（例如未闭合的 `<Card>` 标签）现在会在你修复时动态更新，而不会在刷新后错误地持续存在。
  * 编辑器中的拖拽手柄现在会与标题、段落等不同内容类型正确对齐，尺寸更一致、可点击区域更合理。
</Update>
...
<Update label="January 23, 2026" tags={["New releases", "Improvements"]} rss={{ title: "skills.md、审计日志和 Markdown 导出" }}>
  ...
  文档站点现在会基于你的 API 参考、指南和功能自动生成一个 `skill.md` 文件，为 AI Agent 提供一个涵盖全面的技能文件，其中包含如何帮助用户使用你产品的上下文信息。`skill.md` 以更结构化的能力模型和更广泛的适用性取代了原先的 `install.md` 方案。如果你在文档存储库根目录中已有自定义的 `skills.md` 文件，它会替换自动生成的版本。更多信息请参见：[skill.md](/zh/ai/skillmd)。
  ...
  * 将完全认证、部分认证和个性化体验合并为统一的认证体验。更多信息请参见：[认证设置](/zh/deploy/authentication-setup)。
  * 隐藏页面现在会自动添加 `noindex` 元标签，以便更好地控制 SEO（搜索引擎优化）。
  * 为图片标题添加了 Markdown 支持。
</Update>
