...
<Update label="July 10, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"搜索筛选、自定义子路径托管、网络访问控制等" }}>

<div id="new-releases">
  ## 新版本发布
</div>

  - **搜索筛选：** 为文档搜索[添加筛选器](/zh/optimize/search)，读者可以按您在 dashboard 中配置的产品、类别或其他分面来缩小结果范围。
  - **自定义子路径托管：** 直接从 dashboard 将文档托管在自定义的基础路径（例如 `example.com/docs`）下，部署托管设置中提供了内嵌的基础路径输入框。请参阅[子路径托管](/zh/deploy/docs-subpath)。
  ...
  - **API 密钥有效期与作用域：** 创建带有过期时间以及只读或读写作用域的外部 API 密钥，提升凭证管理的安全性。
  - **由客户支持触发的文档自动化：** 全新的[自动化](/zh/automations)可根据客户支持对话起草文档更新，让反复出现的问题转化为文档改进。
  - **细粒度 PDF 导出：** 可针对特定页面或语言[导出 PDF](/zh/optimize/pdf-exports)，而不必导出整个站点。
  - **助手中的澄清性问题：** 当请求含糊不清时，[助手](/zh/assistant)现在可以提出一个有针对性的澄清性问题，并在聊天中以可选选项卡片的形式呈现。
  - **`/call` 页面：** 配置了电话联系方式的站点现在会暴露一个 `/call` 路由，将访问者转发到拨号器，并使用正确的号码和暂停格式。
  - **合并的 Identity & access 页面：** SSO、SCIM 和网络访问设置现在集中在 dashboard 中的同一个 [Identity & access](/zh/dashboard/sso) 页面上。
  - **文档页面的 Schema.org JSON-LD：** 文档页面现在会包含自动生成的 schema.org 结构化数据，帮助搜索引擎更好地理解您的内容。
  - **Pro 计划焕新：** Pro 计划和升级体验已经过重新设计，在更改额度层级前会显示按比例计算的费用预览。

<div id="improvements">
  ## 改进
</div>

  - **editor 中的评论：** 行内评论回归 [editor](/zh/editor)，删除建议的颜色对比更清晰，工具栏间距也更加精致。
  - **助手联系表单可选关闭：** 如果您不希望助手将对话移交给支持团队，可以在[助手偏转设置](/zh/optimize/analytics)中关闭聊天内的联系表单。
  - **editor 更快的首次渲染：** editor 会在浏览器中缓存导航树，并从本地副本流式加载文档，打开 editor 的速度明显加快。
  - **移动端抽屉重新设计：** 移动端侧边栏和移动端 AI 助手现在使用原生抽屉组件，开合更流畅，触控体验也更好。
  - **API playground：** 将 JSON 请求体粘贴到 [playground](/zh/api-playground/overview) 中即可自动填充请求体字段。
  - **发布下拉菜单虚拟化：** editor 的发布列表会对较长的变更集进行虚拟化处理，在大规模发布时依然保持流畅。
  ...
  - **助手重试：** 重试助手响应现在会重新生成答案，而不会追加重复的对话轮次。
  - **自定义域名 UX 文案：** dashboard 中自定义域名和托管模式设置的文案与内嵌说明更加清晰。
...
<div id="bug-fixes">
  ...
  - [助手](/zh/assistant)现在会显示流式错误，而不是静默失败，也不会在导航后仍显示上一个会话的过时内容。
  - editor 中的三方合并不再在文本块内部重复内容。
  - editor 中大型页面的可视化模式差异对比不再卡死。
  - 终端用户认证的退出登录现在会执行整页跳转，以确保状态被可靠清除。
  - MCP 发现在启用认证的部署上会正确报告 OAuth2 认证方式。
  - 计费页面现在会呈现当前部署对应的计划，而不是组织默认计划。
  - PDF 导出链接已规范化，某些部署上的目录不再渲染为空。
  - 搜索不再通过标题匹配将被降权的页面提升排名。
  - 重置时，默认搜索筛选器会被正确清除。
  - 断链自动化的文案已澄清，仅检查内部链接。
  - `/call` 页面会渲染站点图标，并使用两个逗号作为拨号暂停来格式化电话号码。
  - 在不再适用时，从额度购买 CTA 中移除了首次额度折扣的 UI。
  - 修复了客户端等宽字体替换时的闪烁问题。
