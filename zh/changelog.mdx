...
<Update label="July 17, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"tree 组件的 Markdown 语法、Assistant 小组件焕新等" }}>

<div id="new-features">
  ## 新功能
</div>

  - **tree 组件的 Markdown 语法：** 在 [`Tree`](/zh/components/tree) 中使用标准 Markdown 列表来编写文件和文件夹结构。末尾的 `/` 表示文件夹，嵌套列表会成为嵌套文件夹，无需再使用 `Tree.Folder` 或 `Tree.File` 标签。
  - **焕新的 Assistant 小组件：** 可嵌入的 [Assistant 小组件](/zh/assistant)现在与产品内的 Assistant 共享相同的对话界面和能力，包括实时会话上下文。
  - **audit log 流式传输的 S3 键前缀：** 在将[audit logs](/zh/dashboard/audit-logs) 流式传输到 Amazon S3 时，可以配置自定义键前缀，将对象组织到特定路径下。

<div id="improvements">
  ## 改进
</div>

  - **事件横幅置于 dashboard 顶部：** 活动事件通知现在显示在 dashboard 和编辑器的顶部，方便在任意页面查看。
  - **更简短的构建日志：** 部署构建日志更简洁，更易于查看。
  - **更快的编辑器导航更新：** [编辑器](/zh/editor)中由 gitsync 驱动的导航更新会被批处理并增量应用，从而降低大型站点的延迟。
  - **编辑器代理会加载未链接的文件：** 编辑器代理现在会加载尚未纳入导航的文件，让您在完成链接之前就能处理新页面。

<div id="bug-fixes">
  ## Bug 修复
</div>

  - 当值为 0 且没有变化时，analytics 的增量标签不再显示。
  - CTA 点击事件现在会在所有主题的 [analytics](/zh/optimize/analytics) 中一致地上报。
  - OAuth 重定向现在使用规范展示域名，登出后仍会保留在面向浏览器的主机上。
  - 当上游返回 5xx 错误时，`/login` 页面不再被缓存为 404。
  - 修复了 [Assistant 小组件](/zh/assistant)中章节重新排序的问题。
  - 恢复了 [Tile](/zh/components/tiles) 组件的缩放豁免，使 tile 以预期大小显示。
  - 编辑器中文件变更评论 API 调用现在会在瞬时失败时重试。
