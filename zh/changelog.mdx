...
<Update label="July 3, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"助手电话呼入、未受保护部署分支的自动提交等" }}>

<div id="new">
  ## 新版本发布
</div>

  - **助手电话呼入工具：** 当[助手](/zh/assistant)无法从您的内容中回答问题时，现在可以主动提供电话呼入。它会以点按即拨的链接形式展示您站点的呼入号码和按站点分配的分机号——在移动端会自动预拨分机号。
  - **未受保护部署分支的自动提交：** 当您的部署分支未受保护时，[编辑器](/zh/editor)现在会在草稿与发布流程中将保存的更改直接自动提交到该分支，让快速修复无需手动发布即可上线。受保护分支仍然需要草稿和显式发布。
  - **仅草稿部署选择器：** 在 `main` 上的草稿与发布流程中，手动的 **Publish** 按钮已被移除，部署下拉菜单现在只列出草稿，与上面的自动提交行为保持一致。
  - **`<Meter />` 组件：** 一个新的 [Meter](/zh/components) 原语组件，用于在您的文档中渲染进度和使用量指示器。
  - **AsyncAPI 遵循 `api.params.expanded`：** 在 `docs.json` 中设置 [`api.params.expanded: "all"`](/zh/organize/settings-reference) 现在也会在 [AsyncAPI](/zh/api-playground/asyncapi-setup) 页面上展开参数和消息绑定，与 OpenAPI 的行为一致。
  - **AI 预览中的会话已发布卡片：** 当 AI 预览编辑器发布一个会话后，聊天中会显示一张摘要卡片，让您可以直接跳转到相关变更。
  - **预览沙箱按需恢复：** 已停止的 AI 预览沙箱现在会在您恢复会话时自动重新启动，并且消息支持文件附件，让您可以直接在聊天中分享截图或规范文档。
  - **联系表单线索字段：** 助手[联系表单](/zh/assistant)会捕获可选的线索字段（公司、使用场景等），并将它们包含在发送给您团队的通知邮件中。

<div id="improvements">
  ## 改进
</div>

  - **Slack 代理——原生 Slack 读取工具：** [Slack 代理](/zh/agent/slack)现在通过原生 bot-token 工具读取 Slack，具有更好的话题分页能力和更清晰的错误消息。内部推理过程不会出现在面向用户的回复中。
  - **Slack 代理——识别 GitLab 上下文：** Slack 代理会正确识别由 GitLab 支持的部署（而不是默认为 GitHub），并在权限配置错误时展示真实的 GitLab API 错误。
  - **可折叠的已回答问题和子代理任务列表：** 助手聊天现在会折叠已解决的问题，并在子代理批处理运行时显示紧凑的任务列表。
  - **更精简的 AI 预览聊天顶栏：** 预览编辑器聊天采用了重新设计的精简顶栏，撰写框在内容滚动时会固定在顶栏下方。
  - **聊天在加载时固定到底部：** AI 预览聊天现在打开时会滚动到最新消息，而不是顶部。
  - **更快的 CLI 预构建：** [`mint`](/zh/cli) 拥有更快的冷启动速度和改进的预构建 UI。
  - **仪表盘字体：** 仪表盘现在已切换到正确的 Inter 字体，并在整个 UI 中收紧了间距。

<div id="bug-fixes">
  ## 错误修复
</div>

  - 在仪表盘中切换 `/docs` 基础路径现在会排队进行重建，让路由立即更新。
  - 已创建页面的 href 会去除 `.mdx` 和 `.md` 扩展名，让 AI 编辑器可以按路径解析新创建的页面。
  - 同时包含 `nullable: true` 和 `allOf` 的 OpenAPI 规范（在 Marshmallow 生成的规范中很常见）在 [API playground](/zh/api-playground/openapi-setup) 中渲染时会保留 null 变体。
  - 在多仓库部署中，RSS 行级溯源现在使用复合 git 服务，让"最近更改"订阅正确地归属提交。
  - AI 预览会话不再卡在"initializing"状态，会等待托管仓库的种子提交后再启动。
