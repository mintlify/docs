...
<Update label="March 14, 2026" tags={["New releases"]} rss={{ title: "工作流运行历史仪表板" }}>

  ## 工作流运行历史

  控制台中的[工作流](https://dashboard.mintlify.com/products/workflows)页面现在默认显示**运行历史**标签，提供所有工作流运行记录的综合视图。按工作流筛选或在待处理拉取请求和所有历史运行之间切换。

  每行运行记录显示更丰富的状态信息，包括"需要审查"、"PR 已合并"和"PR 已关闭"状态，以及操作按钮用于查看拉取请求、在编辑器中打开更改或直接从控制台关闭拉取请求。

  之前的工作流卡片列表在**工作流**标签下可用。详情请参阅[监控工作流运行](/zh/agent/workflows#monitor-workflow-runs)。
...
<Update label="March 6, 2026" tags={["New releases", "Improvements"]} rss={{ title: "从公开 GitHub 仓库自动生成文档、助手多模态功能，以及更多改进" }}>
  ...
  * 现在可在 API 操作台中支持[完整 base URL](/zh/organize/settings#param-api)，方法是在 `docs.json` 中配置 `api.url`。
