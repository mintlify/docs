...
<Update label="8月16日" tags={["Improvements"]} rss={{ title: "每周更新", description: "性能优化、LLM 集成、Web 编辑器优化，以及 API 操作台增强" }}>
  ...
  - 为 API 操作台新增 Ruby 支持
  - 新增功能：现在你可以[仅用 docs.json 指定 API 页面](/api-playground/openapi-setup#auto-populate-api-pages)，无需创建任何新的 mdx 文件。
  - 在文档导航中支持来自 OpenAPI 规范的[`webhook pages`](/api-playground/openapi-setup#webhooks)
  ...
  - 新增 [Hightouch 分析集成](/integrations/analytics/hightouch#hightouch)
  ...
  - 为[面包屑](/organize/navigation#breadcrumbs)添加了 CSS 选择器，便于自定义 CSS 定向
...
<Update label="July 2025" tags={["New releases", "Improvements"]} rss={{ title: "七月产品更新", description: "Slack 应用集成、托管 MCP 服务器、Cursor 集成，以及性能改进" }}>
  ...
  在我们的[Slack 应用指南](/ai/slack-app)中了解更多。

  ## 托管 MCP 服务器

  通过 Mintlify 直接部署托管的 Model Context Protocol (MCP) 服务器，以集成到 Claude、Cursor 等 AI 工具。详情见我们的 [MCP 指南](/ai/model-context-protocol)。

  通过上下文菜单，帮助用户在文档任意页面快速将你的 MCP 服务器连接到 Cursor 或 VS Code。更多信息参见[上下文菜单](/ai/contextual-menu)。
...
<Update label="June 2025" tags={["New releases", "Improvements"]} rss={{ title: "六月产品更新", description: "AI 助手更新与可订阅的更新日志" }}>
  ...
  在我们的全新[更新日志指南](/create/changelogs)中了解更多
</Update>
...
<Update label="2025 年 3 月" tags={["New releases", "Improvements"]}>
  ...
  生成 MCP 服务器，让 AI 应用能够与你的文档或 API 交互。书面内容会自动生成为 MCP 服务器，你也可以一键从 OpenAPI 规范生成 MCP 服务器。查看[MCP 入门文档。](/ai/model-context-protocol)
...
<Update label="2025年1月" tags={["Improvements"]}>
  ...
  我们对 [API 操作台](/api-playground/) 的设计与性能进行了全面升级。更新包括：
...
<Update label="November 2024">
  ...
  我们改进了与 GitLab 的同步支持，例如启用自动更新和预览部署。查看我们的[GitLab 文档](/deploy/gitlab)以开始使用。
...
<Update label="2024 年 10 月">
  ...
  推出全新的 [Update 组件](/components/update)，让你更轻松地向用户展示并告知更新（就像这条一样）。
...
<Update label="2024年9月">
  ...
  在卡片上添加一个 `img` 属性即可在卡片顶部显示图片。了解更多内容请见[这里](/components/cards#image-card)。
  ...
  - **API 页面疑难解答**：API 页面可能较为复杂，因此我们列出了常见问题，帮助你快速排查——[阅读文档](/api-playground/troubleshooting)
</Update>
