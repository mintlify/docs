...
<Update label="December 19, 2025" tags={["Improvements"]} rss={{ title: "Agent、AI 助手与编辑器改进" }}>
  ## Agent 功能改进

  * 新增支持从 `AGENTS.md` 文件读取[自定义指令](/zh/ai/agent#customize-agent-behavior)。
  * 如果你在控制台[连接你的账号](/zh/ai/agent#connect-your-github-account)，Agent 现在会以你的 GitHub 账号身份打开 PR。

  ## AI 助手功能改进

  * 新增复制对话按钮，便于分享或保存与 AI 助手的对话。
  * 改进了流式响应卡顿情况的处理逻辑，使 AI 助手回复更稳定可靠。
  * 改进了 AI 助手在移动设备上的展示效果。

  ## Web 编辑器功能改进

  * 在编辑器工具栏中新增[可分享的预览链接](/zh/editor#share-preview-deployments)。
  * 更新了[发布流程](/zh/editor#publish-your-changes)，支持一键发布或创建拉取请求（PR）。
  * 在 Web 编辑器中进行编辑时不再需要使用 GitHub 登录。

  ## 上手流程改进

  * 在注册过程中，你可以选择使用由 Mintlify 托管的文档存储库或你自己的存储库。
  * 在不关联 GitHub 账号的情况下创建文档站点。
  ...
  * 包括 &quot;Try it&quot; 和 &quot;Send&quot; 按钮在内的 API 操作台 UI 元素，现在在所有受支持语言中均已本地化。
</Update>
...
<Update label="December 8, 2025" tags={["New releases"]} rss={{ title: "Last modified dates", description: "Show when pages were last updated with automatic timestamps" }}>
  ...
  现在，你可以在文档中的所有页面上显示“最近修改”时间戳。通过在 `docs.json` 配置中设置 [`timestamp`](/zh/organize/settings#param-timestamp) 开关即可启用此功能。

  启用后，每个页面都会自动显示其最近一次更新的时间，帮助读者了解内容的最新程度。
</Update>
...
<Update label="2025 年 11 月 21 日" tags={["Improvements"]} rss={{ title: "每周更新", description: "API playground 增强、本地化更新和 CLI 改进" }}>
  ## API playground 改进

  * 修复了在不同响应码的端点之间切换时响应部分会消失的问题。现在在导航到新端点时，playground 会正确重置为第一个可用的响应码。
  * 修复了 HTTP Basic 认证方案中重复显示 “Authorization” 请求头的问题。认证区域现在提供了更简洁的界面，并带有正确的字段标签和说明。
  * 改进了在嵌套对象和数组中对 `oneOf` schema 的处理。对于具有多种类型选项的复杂 API schema，现在会在合适的场景下以下拉菜单而不是选项卡的形式正确渲染。
  * 修复了 OpenAPI 规范中定义在端点级别但未正确展示的路径参数问题。现在，定义在各个 operation 之上的参数会正确包含在主页面和 playground 中。
  * 增强了响应示例生成逻辑，即使示例只指定了部分字段，也会包含所有必填属性。这可以确保响应示例完整且准确。
  * 修复了对象数组中缺失预填示例的问题。带有示例数据的数组项现在会在 playground 中按正确索引位置正确填充。
  * 改进了整个 API playground 中描述文本的内边距与间距，以提升可读性和视觉一致性。

  ## 本地化改进

  * 改进了所有支持语言中上下文菜单选项和 AI 助手状态消息的翻译质量。

  ## CLI 改进

  * 为在 Node.js 版本低于 20.17 时运行 `mint dev` 的用户新增了错误提示，并引导用户升级到 LTS 版本。
</Update>
