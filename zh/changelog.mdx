...
<div id="new-releases">
  ...
  - **AI 预览编辑器：** 从一条提示词启动新的文档站点，并在交互式预览中与 AI 代理一起迭代。代理现在可以提出澄清性问题、在断线重连后恢复流式响应，并可直接从入门流程中启动。您还可以提供源 URL、代码仓库和上传的文件，作为首次生成的上下文。生成过程因工作进程故障中断后，现在会自动恢复。如果重试次数耗尽，Mintlify 会恢复初始部署。
