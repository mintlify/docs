...
<Update label="2026年8月21日" tags={["新版本","改进","错误修复"]} rss={{ title:"Branches v2、编辑器 wiki 模式、批量邀请等" }}>

<div id="editor-and-collaboration">
  ## 编辑器与协作
</div>

  - **Branches v2 与版本历史：** 焕然一新的分支体验，包括全新的欢迎界面、重命名和删除控件、按用户设置的分支名、按最近使用排序的分支下拉列表，以及重新设计的版本历史。详情请参见 [Branching and publishing](/zh/editor/branching-and-publishing)。
  - **Wiki 模式：** 在编辑器设置中启用全新的 wiki 模式，即可将更改自动提交到您的部署分支，跳过发布步骤。详情请参见 [Editor settings](/zh/editor/settings)。
  - **编辑器顶栏与侧边栏改版：** 顶栏新增面包屑行并整合了页面控件，新增 **Breadcrumbs** 偏好设置、粘性侧边栏分区标题，以及用于设置和 **Ask agent** 的专用底栏。Agent 面板现在停靠在左侧。
  - **移动端导航改版：** 编辑器的移动端导航针对小屏幕进行了重新设计。
  - **页面访问请求：** 直接在编辑器中请求对私密或受保护页面的编辑权限,并在新的审批弹窗中处理收到的请求。
  - **可丢弃的自动保存更改：** 当您不再需要保留时,可以丢弃分支上自动保存的更改。
  - **默认草稿 PR：** 从编辑器打开的 pull request 现在默认为草稿。对草稿 PR 请求 review 也不再报错。
  - **在创建分支时携带待处理更改：** 新的弹窗会在您创建分支时提示是否移动您的待处理更改。
  - **评论中可点击的 @提及：** 编辑器评论中的提及现在会链接到被提及的用户。
  - **评论中的头像：** 评论卡片会显示作者的头像。
  - **实时预览：** 可同时运行多个本地实时预览。Agent 面板停靠在左侧,预览在 onboarding 后续步骤中保持可见。

<div id="automations-and-integrations">
  ## Automations 与集成
</div>

  - **Slack 消息触发器：** 从已连接的 Slack 频道中的消息触发 automations。详情请参见 [Automations](/zh/automations)。
  - **重新设计的 automations 落地页：** 焕然一新的落地页,每个 automation 卡片上都会显示运行次数和创建者。

<div id="docs-and-delivery">
  ## 文档与交付
</div>

  - **`noCopy` 代码块：** 在围栏代码块上添加 `noCopy` meta 标志,或对于像 ASCII 图这样的无高亮块使用裸的 `no-copy` 围栏,以隐藏复制按钮。详情请参见 [Code blocks](/zh/create/code)。
  - **高亮的搜索片段：** 搜索结果可以显示来自匹配页面的高亮内容片段。请在 dashboard 搜索设置中开启 **Show content snippets**。
  - **分组的 `llms.txt` 索引：** 分组的 `llms.txt` 索引按 URL 段提供,这样代理可以拉取您文档的某个作用域切片。
  - **Assistant widget 用量：** Assistant [widget](/zh/assistant/widget) 现在是付费附加功能,并且 widget 分析会记录每次对话开始时所在的页面。
  - **批量成员邀请：** 通过 CSV 上传从 dashboard 批量邀请队友,发送前会有审阅步骤。
  - **自托管部署中的分析：** Dashboard 分析选项卡现已在自托管部署上可用。
  - **Starter 计划：** Hobby 计划已更名为 Starter。价格和权益保持不变。

<div id="bug-fixes">
  ## 错误修复
</div>

  - **API playground：** OpenAPI 3.1 示例和枚举值现在会出现在生成的响应示例中。端点弹窗中的 Markdown 列表不再渲染为双层项目符号。缩进在 JSX 组件内部的自定义标题锚点能正确解析。
  - **GraphQL：** 输入类型页面会在 GraphQL API reference 站点上生成并提供。
  - **Steps 组件：** 在 `Step` 内部复制粘贴不再创建嵌套的步骤。
  - **Tabs：** 点击时 Tabs 不再跳动。
  - **锚点图标：** 锚点链接图标在深色悬停时保持白色。
  - **Assistant widget 加载：** Widget 在加载时保持可交互。
  - **实时预览时效性：** 导航更新会传播到已打开的实时预览。
  - **设置侧边栏搜索：** 恢复了设置侧边栏中的搜索功能。
  - **编辑器分享菜单：** 编辑器分享和页面操作已整合到一个菜单中。
  - **Discord 机器人：** Discord assistant 在遇到过期的交互时不再崩溃。
  - **编辑器分支名称：** 自动生成的分支名称使用更简洁的 `Draft <date>` 格式。
