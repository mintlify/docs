...
<Update label="March 19, 2026" tags={["Improvements"]} rss={{ title: "自动生成功能增强：外部 URL、自定义指令和多个存储库" }}>

  ## 自动生成入门流程增强

  [自动生成流程](https://mintlify.com/explore)现在支持额外的来源选项用于生成文档：

  - **多个存储库** — 在主存储库之外添加最多 5 个额外的公开 GitHub 存储库，以生成覆盖整个项目的文档。
  - **外部 URL** — 提供最多 5 个外部 URL 供 Mintlify 抓取并作为来源材料。可用于整合现有文档站点、Wiki 或其他网页的内容。
  - **自定义指令** — 通过自由文本形式的指令来指导文档的生成方式。例如，指定要强调哪些功能或排除哪些主题。
