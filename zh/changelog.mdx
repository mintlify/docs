...
<Update label="10月31日" tags={["Improvements"]} rss={{ title: "每周更新", description: "API 操作台增强、表格改进、性能优化和错误修复" }}>
  ## AI 助手改进

  * **AI 助手的入门提问上线！你可以在控制台的 [Assistant --> Settings](https://dashboard.mintlify.com/mintlify/mintlify/products/assistant/settings) 中添加自定义问题**
  * 针对辱骂性关键词和 JSON 查询启用默认垃圾信息防护，提升了助手洞察的质量。

  ## API 操作台改进

  * API 操作台现在会自动为参数填充默认值，节省测试端点的时间。
  * 修复了 `allOf`、`oneOf` 和嵌套对象 schema 造成的渲染问题或字段显示错误；复杂 API schema 现在能正确渲染，所有字段可见且结构清晰。
  * Enum 与可为空字段类型现在在操作台中显示准确的标签，更清楚地表明可接受的取值以及字段是否为可选。
  * 在构建包含嵌套对象的 API 请求时，操作台现在会为每个字段显示恰当的描述性标签，而非通用占位符，更便于理解需要提供哪些数据。
  * Enum 常量表与 API 字段展示的格式与对齐度得到改进，更易于扫描可用选项并理解字段要求。
  * API 操作台的模态框现在会在顶部显示富文本的 MDX 格式说明，为包含格式化文本、链接和代码示例的端点提供更好的上下文。

  ## Analytics 变更

  * 延迟加载分析：PostHog 和 LogRocket 现在在文档内容之后加载，在不影响分析数据的情况下改善首屏加载时间和 Core Web Vitals（核心网页指标）得分。
  * 条件会话录制：可基于用户属性或页面上下文有条件地启用 PostHog 会话录制，以更好地控制录制时机。

  ## 视觉改进

  * 表格现在具有更好的对齐、更一致的间距与更高的可读性。表头与其内容列正确对齐。

  ## 错误修复

  * 修复了链接溢出导致移动设备出现横向滚动的问题。链接现在会在其容器内正确换行。
  * 改进了 LaTeX/数学块样式，提升可读性与格式一致性。
  * 为视频嵌入添加了 `playsInline` 和 autoplay 属性，确保在 iOS 设备上正常工作。
  * 更新了 Claude 图标的圆角以保持视觉一致性。
  * 修复了 `/llms.txt` 路由，使 LLM 代理能够正确访问文档。
  * 防止在复杂 OpenAPI 规范中出现无限 schema 展开，避免浏览器崩溃。
  * 移除了 Palm 主题中的 x-overflow，它会在 API 操作台里截断文本。
  * 增大了正文字号，以提升与浏览器阅读模式的兼容性。
  * 修复了 Google 登录按钮样式，提升视觉一致性。

</Update>

<Update label="10月24日" tags={["Improvements"]} rss={{ title: "每周更新", description: "控制台增强、API 操作台改进、认证修复与性能优化" }}>
  ## 洞察改进

  * 优化洞察页面，修复了“今天”和“昨天”的日期选择器
  * 增强反馈表格，改进搜索功能
  * 为分类表新增加载状态
  ...
  * 在控制台中新增助手位置配置
  * 通过修复上下文获取与工具消息，改进了助手回复

  ## API 操作台与 OpenAPI 增强

  * 重构了 API 操作台端点字段以使用新的 schema 图，提高性能
  * 即使助手侧边栏打开，操作台模态框中的 API 示例也能更好地展示
  * 修复了紧邻 `$refs` 与 `x-` 属性的 props 在 OpenAPI 解析中丢失的问题
  * 修复了 API 操作台模态框的标题样式

  ## Web 编辑器与部署增强

  * 恢复了“手动创建预览”按钮，便于创建预览部署
  * 在控制台中新增用于获取 branch 保护数据的 fetcher
  * 在控制台中新增用于跟踪 PR 状态的 fetcher
  * 改进编辑器状态管理，使状态可在浏览器会话间持久化
  * 修复了编辑器发布按钮的禁用态
  * 移除了编辑器中的代码块背景，使界面更清爽
  ...
  * 移除了页面尺寸选项的全宽限制
  * 修复了“接受组织邀请”按钮的尺寸
  * 修复了在非 macOS 电脑上键盘快捷键显示为“Ctrl+I”且没有加号的问题
