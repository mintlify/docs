...
<Update label="February 6, 2026" tags={["New releases", "Improvements"]} rss={{ title: "自助式 SSO、查看者角色、人类/AI 流量切换与斜杠命令菜单改版" }}>
  ## 自助式 SSO 配置

  企业客户现在可以直接在控制台中配置 SAML SSO（Okta 或 Microsoft Entra），包括即时（JIT）开通、身份提供商切换以及连接移除。

  ## 查看者角色

  新增只读访问权限的查看者角色，可访问你的控制台。查看者可以在编辑器中查看内容，但不能编辑内容、邀请成员或修改配置。

  ## 人类/AI 流量切换

  Analytics 页面现在将人类流量和 AI 流量分为不同的标签页展示。

  ## 斜杠命令菜单改版

  编辑器中的斜杠命令菜单现在支持输入即搜索、改进的键盘导航，以及其他 UX 改进。

  ## 错误修复

  * 修复了控制台表格中的单元格溢出问题。
  * 通过编辑器中的配置面板重命名文件现在会正确保存更改。
  * 诸如未闭合的 `<Card>` 标签等 MDX 解析错误，现在会在你修复时动态更新，而不会在刷新后错误地持续存在。
  * 编辑器中的拖拽手柄现在能与不同内容类型（如标题或段落）正确对齐，尺寸更一致且可点击区域更合理。
</Update>
...
<Update label="January 23, 2026" tags={["New releases", "Improvements"]} rss={{ title: "skills.md、审计日志与 Markdown 导出" }}>
  ...
  文档站点现在会基于你的 API 参考、指南和功能自动生成一个 `skill.md` 文件，为 AI 代理提供一份完整的技能文件，包含如何帮助用户使用你产品的上下文信息。`skill.md` 以更结构化的能力和更广泛的适用性替代了 `install.md` 的方案。如果你的文档存储库根目录下已有自定义的 `skills.md` 文件，它会覆盖自动生成的版本。更多信息请参见 [skill.md](/zh/ai/skillmd)。

  ## 认证事件审计日志

  现在会将相关事件记录到 [审计日志](/zh/dashboard/audit-logs) 中，以提升可见性和合规性。

  ## 面向代理的 Markdown 导出

  当发送 `Accept: text/markdown` 请求头时，文档页面现在会返回原始 Markdown 内容，这有助于 AI 代理更好地读取和利用内容。

  ## 功能改进

  * 将完全认证、部分认证和个性化合并为统一的认证体验。更多信息请参见 [Authentication setup](/zh/deploy/authentication-setup)。
  * 隐藏页面现在会自动添加 `noindex` 元标签，以便更好地控制 SEO（搜索引擎优化）。
  * 为图片说明文字新增了 Markdown 格式支持。
</Update>
