...
* 查看文档、已连接仓库和附件中的图片，以描述图片、编写替代文本，或检查截图是否与内容一致。
...
默认情况下，agent 会为每次更改打开一个拉取请求，以便你的团队在更改发布前进行审核。如果你的项目由 Mintlify 托管，agent 会将更改直接提交到你的部署 branch，使其立即发布。

对于连接到你自有 Git 提供商的项目，请在控制台的 [Automations](https://app.mintlify.com/products/automations) 页面点击 **Configure Slack**，然后在 Slack 设置中设置 **Review process**：
...
Agent 只能访问你通过 Mintlify GitHub 应用连接的仓库。你可以在 [GitHub 应用设置](https://github.com/apps/mintlify/installations/new) 中，或对于 GitLab 在 [Git Settings](https://app.mintlify.com/settings/project/git-settings) 中，配置 agent 可访问的仓库。
