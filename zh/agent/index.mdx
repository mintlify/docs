...
当 agent 在编辑器的 agent 面板中工作时，你可以与每条消息进行交互：

* **复制**：将消息复制到剪贴板以便复用内容。助手消息在粘贴到编辑器时会保留代码块的格式。
* **反馈**：用点赞或点踩对助手回复进行评分，帮助提升 agent 质量。
* **时间戳**：将鼠标悬停在消息上以查看其发送时间。

反馈和复制操作仅在消息完成后可用。按钮会在回复完成流式输出后出现。
