...
1. 在控制台中打开 [Agent](https://app.mintlify.com/products/agent) 页面。
2. 在你希望 agent 处理的仓库上安装 Mintlify GitHub App。
3. 通过 agent 面板、Slack，或在已连接的仓库中打开一个 pull request，发送你的第一个请求。
4. 审核 agent 打开的 pull request，并在准备就绪时合并它。

<div id="how-the-agent-stays-safe">
  ## agent 如何保持安全
</div>

- 仅打开 pull request。agent 永远不会直接向你的部署 branch 进行 commit。
- 只访问你通过 Mintlify GitHub App 连接的仓库。
- 将非文档仓库视为只读，仅用于获取上下文，绝不向其写入。
- 在任何更改进入你的正式站点之前，都需要人工审核并合并。
