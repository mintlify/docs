...
添加你希望 Agent 遵循的任何指令。Agent 会将这些指令追加到它的 system prompt（系统提示词）中，因此这些指令会应用于所有任务，无论你是在 Slack 上，还是通过 API 使用 Agent。

<Note>
  编辑器的[内置 Agent](/zh/editor/agent) 不会读取 `AGENTS.md`。若要为编辑器 Agent 提供持久的指导，请改为在编辑器设置中配置 [AI 指令](/zh/editor/settings#ai-instructions)。
</Note>
