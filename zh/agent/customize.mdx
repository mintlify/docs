...
在文档根目录下的 `.mintlify/` 目录中创建一个 `AGENTS.md` 文件，用来自定义 Agent 的行为（`Agents.md` 也同样有效）。

<Warning>
  将你的 `AGENTS.md` 文件放在 `.mintlify/` 目录内（`.mintlify/AGENTS.md`），而不是放在项目根目录下。放在项目根目录下的文件会在你的文档网站上公开访问，这意味着你的自定义指令将对任何人可见。`.mintlify/` 目录不会被公开提供服务。
</Warning>
