...
| **Automerge** | 否 | 默认为关闭，此时会打开一个拉取请求以供审查。如果启用，则会打开一个拉取请求并自动将其合并。 |
| **Notify** | 否 | 通知配置。工作流完成时发送 Slack 消息。 |

对于 GitHub 存储库，你必须在 context 或 trigger 字段中列出的每个存储库上安装 Mintlify GitHub App。请在 Mintlify 控制台的 [GitHub app](https://dashboard.mintlify.com/settings/organization/github-app) 页面中添加新的存储库。
...
当更改被推送到指定存储库或特定分支时运行工作流。这包括拉取请求合并和直接推送到该分支的操作。

以 `owner/repo` 格式指定每个存储库。可选择指定要监控的分支。如果你未指定分支，则工作流会在更改推送到存储库默认分支时触发。

一个工作流可以监控最多 10 个存储库或分支上的推送。
...
在控制台的 [Workflows](https://dashboard.mintlify.com/products/workflows) 页面中编辑、删除、禁用或触发工作流。
...
2. 点击 **View workflows** 查看所有工作流。
3. 点击你要禁用或启用的工作流。
4. 点击 **Active** 开关将其关闭以禁用工作流，或打开以启用工作流。
5. 点击 **Save changes**。
...
这些示例提示词展示了常见的工作流模式。在控制台中创建工作流，然后将提示词粘贴到指令字段中。
...
**触发器：** 在 push 到 `your-org/your-product`（main 分支）时
**Context 存储库：** `your-org/your-docs`
...
**触发器：** 在 push 到 `your-org/your-docs`（main 分支）时
...
**触发器：** Cron 计划 `0 9 * * 1`（每周一 9:00 AM UTC）
**Context 存储库：** `your-org/your-product`
