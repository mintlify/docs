...
<Note>
  工作流会自动忽略由其自身合并的拉取请求所引起的 push 事件。这可以防止工作流在无限循环中重新触发自身。
</Note>
...
默认情况下，代理会为每次工作流运行创建一个拉取请求 (PR；亦称"合并请求"/Merge Request) ，以便你在更改上线之前进行审查。将 `automerge: true` 设为自动以 squash 方式合并该拉取请求 (PR；亦称"合并请求"/Merge Request) ，无需手动批准。这样一来，你既可以在存储库的拉取请求历史记录中保留更改记录，又能自动完成合并步骤。
