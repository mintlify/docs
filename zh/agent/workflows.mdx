...
对于 GitLab 存储库，请在仪表板设置中通过 [GitLab OAuth](https://dashboard.mintlify.com/settings/organization/gitlab-oauth) 连接你的项目。或者，你也可以使用带有访问令牌和 webhook 的 [GitLab 集成](/zh/deploy/gitlab)。
...
<Note>
  ...
  对于 GitLab 存储库，自动合并使用你的 [GitLab OAuth](https://dashboard.mintlify.com/settings/organization/gitlab-oauth) 连接或在 [GitLab 集成](/zh/deploy/gitlab)中配置的访问令牌的凭证。请确保该令牌拥有足够的权限以合并目标分支上的合并请求。
</Note>
