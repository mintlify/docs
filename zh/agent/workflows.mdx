...
对于 GitHub 存储库，你必须在每个作为上下文存储库或推送触发器的存储库上安装 Mintlify GitHub App。请在 Mintlify 仪表板的 [GitHub app](https://dashboard.mintlify.com/settings/organization/github-app) 页面中添加新的存储库。
...
从控制台的 [Workflows](https://dashboard.mintlify.com/products/workflows) 页面编辑或删除工作流。
...
**触发器：** 当 `your-org/your-product` 的 `main` 分支收到推送时触发。
**上下文：** `your-org/your-docs`
...
**触发器：** 当 `your-org/your-docs` 的 `main` 分支收到推送时触发。
...
**触发器：** 按 cron 计划 `0 9 * * 3` 运行（每周三 9:00 UTC）。
...
**触发器：** 按 cron 计划 `0 9 * * 1` 运行（每周一 9:00 UTC）。
