...
Agent 在一个隔离的沙盒环境中运行，其互联网访问受到限制。编写提示词时，请仅引用 Agent 可用的工具。

- 标准 shell 工具（`grep`、`sed`、`awk`、`curl` 等）
- `git` 和 GitHub CLI (`gh`)
- Mintlify CLI (`mint`)
- Node.js 和 Bun

Agent 无法在运行时安装其他软件包或工具。包注册表和其他外部服务无法从沙盒中访问。

指示 Agent 运行不可用工具的提示词可能会产生意外结果或执行失败。
