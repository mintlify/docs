...
| `notify` | 否 | 通知配置。在工作流完成时发送 Slack 消息。 |
...
当工作流完成或失败时发送 Slack 消息。通知包括工作流状态、拉取请求链接以及更改摘要。
...
| 字段 | 说明 |
|-------|-------------|
| `channels` | 要通知的频道名称（带或不带 `#` 前缀）。 |
| `channel_ids` | 要通知的频道 ID。 |
| `users` | 要发送私信的用户名、显示名称或真实姓名。 |
| `user_ids` | 要发送私信的 Slack 用户 ID。 |

<Note>
Slack 通知需要在你的组织中安装 Slack。请在控制台的 [Slack 集成](https://dashboard.mintlify.com/settings/organization/integrations/slack) 页面连接 Slack。
</Note>
...
每周运行以生成更新日志，并在工作流完成时通过 Slack 通知你的团队。此示例展示了如何将定时工作流与 Slack 通知结合使用。
