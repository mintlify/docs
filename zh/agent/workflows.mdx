...
对于 GitLab 存储库，请在 Mintlify 仪表板的 [GitLab OAuth](https://dashboard.mintlify.com/settings/organization/gitlab-oauth) 页面通过 OAuth 连接你的 GitLab 项目。你也可以使用 [GitLab 集成](/zh/deploy/gitlab)配合访问令牌和 webhook。
...
<Note>
  ...
  对于 GitLab 存储库，自动合并使用你的 [GitLab OAuth](https://dashboard.mintlify.com/settings/organization/gitlab-oauth) 连接的凭据或在你的 [GitLab 集成](/zh/deploy/gitlab)中配置的访问令牌。请确保相关账户或令牌拥有足够的权限以合并目标分支上的合并请求。
</Note>
