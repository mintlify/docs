...
对于 GitHub 存储库，你必须在上下文或触发器字段中列出的每个存储库上安装 Mintlify GitHub App。请在 Mintlify 控制台的 [Git settings](https://dashboard.mintlify.com/settings/deployment/git-settings) 页面中添加新的存储库。

对于 GitLab 存储库，你必须在 Mintlify 控制台的 [Git settings](https://dashboard.mintlify.com/settings/deployment/git-settings) 页面上连接你的 GitLab 账户。然后添加你想要与工作流一起使用的存储库。你必须在每个项目上至少拥有 Maintainer 角色才能连接 GitLab OAuth。
...
<Note>
  ...
  对于 GitLab 存储库，自动合并使用在 Mintlify 控制台的 [Git settings](https://dashboard.mintlify.com/settings/deployment/git-settings) 页面上配置的 GitLab OAuth 连接。你必须在每个项目上至少拥有 Maintainer 角色才能连接 GitLab OAuth。
</Note>
...
要在工作流中使用 GitLab 存储库，请通过 [Git settings](https://dashboard.mintlify.com/settings/deployment/git-settings) 页面上的 GitLab OAuth 集成连接每个项目。你必须连接工作流涉及的所有存储库，包括你的文档存储库以及任何触发或上下文存储库。
