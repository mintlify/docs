...
如果你已安装 [Mintlify CLI](/zh/installation)，请在你的文档存储库中运行以下命令，以便通过 CLI 以交互方式创建工作流文件。
...
<Note>
  自动合并要求 Mintlify GitHub 应用对所有针对你部署分支的规则集都拥有绕过权限，包括组织级和仓库级的规则集。请参阅[配置自动合并](/zh/guides/configure-automerge)了解设置说明。
</Note>
