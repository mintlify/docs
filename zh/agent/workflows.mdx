...
对于 GitHub 存储库，你必须在上下文或推送触发器字段中列出的每个存储库上安装 Mintlify GitHub App。请在 Mintlify 仪表板的 [GitHub app](https://dashboard.mintlify.com/settings/organization/github-app) 页面中添加新的存储库。
...
* 以 `owner/repo` 格式指定存储库。
* 可选地指定要监控的 branch。如果你未指定 branch，则工作流会在更改推送到存储库默认 branch 时触发。
...
在工作流运行时为代理程序提供对其他存储库的只读访问权限。当你的提示词需要查看文档存储库之外的代码或内容时，这会很有用。每个工作流最多可以指定五个上下文存储库。

<div id="auto-merge-changes">
  ### 自动合并更改
</div>

默认情况下，代理会为每次工作流运行创建一个拉取请求 (PR；亦称"合并请求"/Merge Request) ，以便你在更改上线之前进行审查。启用自动合并功能可自动合并该拉取请求，无需手动批准。这样一来，你既可以在存储库的拉取请求历史记录中保留更改记录，又能自动完成合并步骤。
...
在仪表板的 [Workflows](https://dashboard.mintlify.com/products/workflows) 页面中编辑或删除工作流。
...
**触发器：** 推送到 `your-org/your-product`（main 分支）

**上下文存储库：** `your-org/your-docs`
...
**触发器：** 推送到 `your-org/your-docs`（main 分支）
...
**触发器：** Cron 计划（`0 9 * * 3` — 每周三 9:00 AM UTC）
...
**触发器：** Cron 计划（`0 9 * * 1` — 每周一 9:00 AM UTC）
...
**上下文存储库：** `your-org/your-product`

**自动合并：** 关闭

**Slack 通知：** 通知 `#documentation` 频道和 `tech-writer` 用户
