...
控制台中的[工作流](https://dashboard.mintlify.com/products/workflows)页面有两个标签：**运行历史**和**工作流**。

<div id="run-history">
  ### 运行历史
</div>

**运行历史**标签是默认视图。它显示所有工作流的综合运行列表，包含以下列：

- **时间**：运行开始和完成的时间。
- **工作流**：工作流名称和触发事件（如合并的拉取请求）。
- **状态**：运行的当前状态。可能的状态包括：
  - **排队中**：运行等待开始。
  - **运行中**：运行正在进行。
  - **需要审查**：运行完成并创建了需要审查的拉取请求。
  - **PR 已合并**：运行创建的拉取请求已合并。
  - **PR 已关闭**：运行创建的拉取请求已关闭但未合并。
  - **无需操作**：运行完成但未创建拉取请求。
  - **失败**：运行遇到错误。
- **摘要**：运行执行内容的描述，附带查看拉取请求、在编辑器中打开更改或关闭拉取请求的操作按钮。

默认情况下，运行历史仅显示包含待处理拉取请求的运行，以便你专注于需要操作的项目。切换**显示所有运行**以查看所有历史运行，包括已合并和已关闭的拉取请求。

使用工作流筛选下拉菜单可显示特定工作流的运行记录。

<div id="workflows-tab">
  ### 工作流标签
</div>

**工作流**标签列出你的活跃工作流。点击工作流即可查看或编辑其配置。
