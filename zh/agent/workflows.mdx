...
如果工作流运行因组织的 Mintlify 额度耗尽而失败，Mintlify 会自动禁用该项目中所有处于活动状态的工作流，以避免后续运行继续失败。组织的所有成员都会收到一封邮件，告知工作流已被暂停，并附上前往结算页面的链接。

要恢复自动化：

1. 在控制台的 [Billing](https://dashboard.mintlify.com/settings/organization/plans) 页面添加额度或升级套餐。
2. 打开 [Workflows](https://dashboard.mintlify.com/products/workflows?tab=workflows) 页面，重新启用你希望运行的每个工作流。

重新启用工作流会清除因结算原因被禁用的状态。计划型工作流会从你重新启用的时间起重新计算下次运行时间。
