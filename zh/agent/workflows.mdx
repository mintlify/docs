...
4. 在 GitHub 中，审核并合并将工作流文件添加到你存储库的拉取请求（PR；亦称“合并请求”/Merge Request）。
...
| `automerge` | 否 | 默认为 `false`，此时会打开一个拉取请求供审核。如果为 `true`，则会打开一个拉取请求并自动合并它。 |
...
默认情况下，智能代理会在每次工作流运行时创建一个拉取请求（PR），以便你在更改上线之前进行审查。将 `automerge` 设置为 `true`，拉取请求将在创建后自动合并。

```yaml
automerge: true
```

启用 `automerge` 后，工作流仍会创建一个拉取请求，运行任何已配置的检查，然后合并它。这样你可以在仓库的拉取请求历史中保留更改记录，同时自动化合并步骤。
