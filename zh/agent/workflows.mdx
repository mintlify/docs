...
<Info>
  工作流目前处于测试阶段。工作流功能、可用性和定价可能会发生变化。
</Info>
...
Hobby 方案的所有工作流每月限制为 100 次运行。滚动 30 天窗口内的非失败运行会计入此限制。[升级您的方案](https://mintlify.com/pricing)以取消每月上限。
...
对于 GitLab 存储库，你必须在 Mintlify 仪表板的 [GitLab OAuth](https://dashboard.mintlify.com/settings/organization/gitlab-oauth) 页面上连接你的 GitLab 账户。然后添加你想要与工作流一起使用的存储库。你必须在每个项目上至少拥有 Maintainer 角色才能连接 GitLab OAuth。
...
<Note>
  ...
  对于 GitLab 存储库，自动合并使用在 Mintlify 仪表板的 [GitLab OAuth](https://dashboard.mintlify.com/settings/organization/gitlab-oauth) 页面上配置的 GitLab OAuth 连接。你必须在每个项目上至少拥有 Maintainer 角色才能连接 GitLab OAuth。
</Note>
...
你可以按需运行基于 cron 的工作流，无需等待下一个计划时间。手动触发的运行会计入每日和每月运行限制。
