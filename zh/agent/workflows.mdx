...
例如：`@mintlify Update These quickstart page to include information about inviting collaborators`，然后检出该 branch，并使用你偏好的方式进行额外修改。
...
例如：`@mintlify This PR adds a new authentication method. Update These docs to include These new auth flow: [PR link]`.
...
例如：`@mintlify Review These API rate limiting section. We changed limits last month`.
...
例如：`@mintlify Users are getting confused by step 3 in These setup guide. What might be making it unclear?`.
