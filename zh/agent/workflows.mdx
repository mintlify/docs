...
对于 GitHub 存储库，你必须在列为上下文或触发存储库的每个存储库上安装 Mintlify GitHub App。请在 Mintlify 仪表板的 [GitHub app](https://dashboard.mintlify.com/settings/organization/github-app) 页面中添加新的存储库。
...
- **Repository**：以 `owner/repo` 格式指定的存储库。
...
使用上下文存储库在工作流运行时为代理程序提供对其他存储库的只读访问权限。当你的提示词需要查看文档存储库之外的代码或内容时，这会很有用。每个工作流最多可以指定五个上下文存储库。

<div id="auto-merge-changes">
  ### 自动合并更改
</div>

默认情况下，代理会为每次工作流运行创建一个拉取请求 (PR；亦称"合并请求"/Merge Request) ，以便你在更改上线之前进行审查。启用自动合并以自动合并该拉取请求 (PR；亦称"合并请求"/Merge Request) ，无需手动批准。这样一来，你既可以在存储库的拉取请求历史记录中保留更改记录，又能自动完成合并步骤。
