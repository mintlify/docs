...
你从 Slack 连接的集成的作用域为你个人的 Slack 用户，每位团队成员都需要设置各自的连接，agent 会使用发送消息者的连接。如果某位 Slack 用户没有连接某项服务，agent 会回退到从控制台中 [Integrations](https://dashboard.mintlify.com/settings/organization/integrations) 页面安装的组织级集成。

如需安装、移除或重新授权某个组织级集成，请前往 **Settings** > **Workspace** > **Integrations**。连接某项服务可让 agent 在整个工作区中使用它，断开某项服务可撤销其访问权限。列表中只显示支持托管认证的 toolkit，因此显示出的每个集成都可以直接从控制台连接。
