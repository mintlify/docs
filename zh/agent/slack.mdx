...
首次安装 Mintlify Slack 应用时，agent 会向你发送一条欢迎私信。如果你的组织已启用 agent，该消息会包含针对常用集成的一键 **Connect** 按钮：
...
1. 在 Slack 中打开 agent 的私信。
2. 点击你要添加的服务对应的 **Connect** 按钮。
3. 在弹出的模态框中，点击授权链接，登录第三方服务，并批准所请求的权限。
4. 返回 Slack。Agent 会确认连接成功，并立即可以将该集成用作后续请求的上下文。

按钮只会显示在你尚未连接的服务上。一旦某个连接变为活动状态，对应的按钮就不会出现在后续的提示中。

### 让 agent 连接任意服务

对于不在一键列表中的服务，让 agent 设置该集成：
