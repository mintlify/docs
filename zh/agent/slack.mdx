...
你从 Slack 连接的集成的作用域为你个人的 Slack 用户，每位团队成员都需要设置各自的连接，agent 会使用发送消息者的连接。如果某位 Slack 用户没有连接某项服务，agent 会回退到在控制台 **Settings > Workspace > [Integrations](https://dashboard.mintlify.com/settings/organization/integrations)** 页面安装的组织级集成。

如需移除或重新授权某个组织级集成，请在控制台中进入 **Settings > Workspace > [Integrations](https://dashboard.mintlify.com/settings/organization/integrations)**。
