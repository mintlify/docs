...
若要从你的 Slack 工作区中移除 agent：

1. 在 Slack 中，进入 **Settings & administration > Manage apps**，并打开 `mintlify` 应用。
2. 点击 **Remove app** 将其从工作区卸载。

移除后，agent 将不再响应提及和私信。现有的线程历史记录仍会保留在 Slack 中，但 agent 无法再访问它们。若要重新连接，请从你的 Mintlify dashboard 重新安装该应用。
...
- 向智能体发送私信，在私密环境中使用它来更新文档。应用安装完成后，即可立即使用私信功能。
- 在频道中提及 `@mintlify`，在公开场景中与团队协作使用它。agent 只会在被邀请加入的频道中响应——在频道中运行 `/invite @mintlify` 即可将其添加进来。
