...
如需移除或重新授权某个组织级集成，请使用 [Agent](https://dashboard.mintlify.com/products/agent) 页面的 **Agent settings** 部分。
...
如果你的文档位于 Mintlify 托管的仓库中（当你尚未连接自己的 GitHub 或 GitLab 账户时的默认选项），Slack 智能体无需任何额外设置即可使用：

- **无需安装 GitHub 应用**：智能体使用 Mintlify 对托管仓库的托管访问权限，因此你无需安装或授权单独的 Git 集成。
- **更改直接发布**：智能体不会创建拉取请求，而是直接将更改提交到你的部署分支。一旦部署完成，你的线上站点就会更新。
- **状态更新中省略仓库详情**：由于没有可引用的外部仓库，智能体在 Slack 中的进度消息会聚焦于文件和编辑内容，而不是仓库路径。
- **完成链接**：当智能体完成工作时，它会发布一个指向你线上站点的链接，方便你查看已发布的更改。

要从 Mintlify 托管的仓库迁移到你自己的 GitHub 或 GitLab 仓库，请按照 [Git Settings](https://dashboard.mintlify.com/settings/deployment/git-settings) 页面上的迁移指南操作。迁移完成后，智能体将切换到上述标准的拉取请求工作流。
