...
若要将 agent 从你的 Slack 工作区中移除，请在 dashboard 中打开 [Automations](https://app.mintlify.com/products/automations) 页面，然后点击 **Disconnect Slack**。此操作会撤销已存储的 bot token，并停止 agent 在 Slack 中的响应。Slack Workspace Owner 也可以从 Slack 的 **Manage apps** 页面卸载 `mintlify` 应用，效果相同。

之后重新连接会恢复 agent，且无需重新创建你的频道或线程历史。
