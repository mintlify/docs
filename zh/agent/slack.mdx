...
1. 在私信中，或在提及 `@mintlify` 的线程中，让 agent 设置该集成。例如：
   ...
   - "设置 Linear 集成。"
2. Agent 会回复一个授权链接。
3. 点击该链接，登录第三方服务，并批准所请求的权限。授权完成后，浏览器会显示一个确认页面，提示连接已完成。
...
Mintlify 会让你已连接服务的状态与 Composio 保持同步。在以下情况下，连接会自动被标记为已断开：

- 第三方提供商凭据过期或被撤销。
- 你从 Composio 控制台中删除了该连接。
- 提供商将该连接标记为非活动。

如果某个连接不再有效，下次你让 agent 使用该服务时，它会回复一个新的授权链接。点击该链接，重新授权该服务，agent 就会从你上次中断的位置继续。
