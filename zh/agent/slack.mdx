...
- 在线程中继续对话，对修改内容持续迭代。智能体会将线程中先前分享的文件和图片作为后续请求的上下文进行复用，因此你无需重新上传。
...
- **包含附件**：在消息中上传图片、图表、代码文件或其他文档。智能体会自动处理这些附件，并将其内容纳入你的文档中。在线程中，智能体还可以访问先前消息中分享的图片和文件，因此你可以在后续请求中引用它们，而无需重新上传。
