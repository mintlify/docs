---
title: "Mintlify agent 使用场景和示例"
sidebarTitle: "使用场景"
description: "了解如何使用 Mintlify agent 在 Slack、拉取请求和 API 工作流中自动更新文档、生成代码示例、处理反馈、沉淀团队知识，并在产品发布后持续维护内容准确性。覆盖从 Slack 线程迭代提示词，到合并功能变更、生成发布说明和通过 API 创建任务的常见场景，帮助团队选择适合的文档维护方式。"
keywords: ["agent 自动化", "文档自动化", "自动化示例"]
---
...
agent 可以查看文档、已连接仓库和附件中的 JPEG、PNG、GIF 和 WebP 图片。你可以让它描述图片、编写替代文本，或检查截图是否仍与周围内容一致。

例如：`@mintlify Add descriptive alt text to every image on the quickstart page`。

在需要[身份验证](/zh/deploy/authentication-setup)的部署中，agent 只会查看它已读取的已发布页面中出现的图片。
...
参见[自动化概览](/zh/automations/index)了解更多信息。
