...
从你的[控制台](https://dashboard.mintlify.com/editor)打开网页编辑器，在浏览器中直接管理文档。
...
<Steps>
  ...
  <Step title="发布你的更改">
    可将更改直接发布到线上文档站点，或先创建拉取请求（PR；亦称“合并请求”/Merge Request）以在合并前进行审核。
  </Step>
</Steps>
...
网页编辑器提供两种模式，以满足不同的编辑偏好与需求：可视化模式和 Markdown 模式。

使用编辑器工具栏中的模式开关在两种模式之间切换。

<Frame>
  <img src="/images/editor/mode-toggle-light.png" alt="在网页编辑器中高亮显示的模式切换图标。" className="block dark:hidden" />

  <img src="/images/editor/mode-toggle-dark.png" alt="在网页编辑器中高亮显示的模式切换图标。" className="hidden dark:block" />
</Frame>
...
<Frame>
  <img src="/images/editor/files-menu-light.png" className="block dark:hidden rounded-2xl border border-gray-100 shadow-lg" style={{ width:"400px",height:"auto" }} alt="网页编辑器中的文件菜单。" />

  <img src="/images/editor/files-menu-dark.png" className="hidden dark:block rounded-2xl border border-white/10 shadow-lg" style={{ width:"400px",height:"auto" }} alt="深色模式下的网页编辑器文件菜单。" />
</Frame>

文件名会自动添加 `.mdx` 扩展名。要更改文件类型，点击该扩展名，并从下拉菜单中选择所需的类型。

<Frame>
  <img src="/images/editor/file-type-light.png" className="block dark:hidden" style={{ width:"auto",height:"400px" }} alt="网页编辑器中的文件扩展名菜单。" />

  <img src="/images/editor/file-type-dark.png" className="hidden dark:block" style={{ width:"auto",height:"400px" }} alt="深色模式下的网页编辑器文件扩展名菜单。" />
</Frame>
...
<Frame>
  <img src="/images/editor/rename-light.png" className="block dark:hidden" alt="在网页编辑器中重命名页面。" />

  <img src="/images/editor/rename-dark.png" className="hidden dark:block" alt="在暗色模式的网页编辑器中重命名页面。" />
</Frame>
...
<Frame>
  <img src="/images/editor/delete-page-light.png" className="block dark:hidden" alt="在 Web 编辑器中删除页面。" />

  <img src="/images/editor/delete-page-dark.png" className="hidden dark:block" alt="在深色模式下的 Web 编辑器中删除页面。" />
</Frame>
...
编辑器的发布流程取决于你当前所在的 branch 以及存储库的 branch 保护规则。

| Branch 类型 | Branch 保护 | 发布时会发生的情况 |
| :---------- | :----------------- | :------------------------------ |
| 部署用分支 | 无 | 更改会被合并并立即生效 |
| 部署用分支 | 必须通过拉取请求（PR） | 会创建一个用于审阅的拉取请求（PR） |
| 其他分支 | 无 | 更改会合并到部署用分支并立即生效 |
| 其他分支 | 必须通过拉取请求（PR） | 会创建一个用于审阅的拉取请求（PR） |

<Tip>
  在你的 Git 提供商中配置 branch 保护规则，要求在合并前必须通过拉取请求（PR）审查。更多信息请参阅 GitHub 文档中的 [About protected branches](https://docs.github.com/en/repositories/configuring-branches-and-merges-in-your-repository/managing-protected-branches/about-protected-branches) 或 GitLab 文档中的 [Protected branches](https://docs.gitlab.com/user/project/repository/branches/protected/)。
</Tip>

<Frame>
  <img src="/images/editor/publish-flow-light.png" alt="Mintlify Web 编辑器中被强调的“发布”按钮" className="block dark:hidden" />

  <img src="/images/editor/publish-flow-dark.png" alt="Mintlify Web 编辑器中被强调的“发布”按钮" className="hidden dark:block" />
</Frame>

<Tip>
  要使用你的 GitHub 账户对提交进行签名，你必须在控制台的 [My profile](https://dashboard.mintlify.com/settings/account) 页面授权你的 GitHub 账户。如果你没有授权 GitHub 账户，Mintlify GitHub 应用会对在 Web 编辑器中完成的提交进行签名。
</Tip>
...
<Steps>
  ...
  <Step title="创建并分享">
    ...
    <Frame>
      <img src="/images/editor/pull-request-light.png" alt="在 Mintlify Web 编辑器中突出的“发布拉取请求”按钮。" className="block dark:hidden" />

      <img src="/images/editor/pull-request-dark.png" alt="在 Mintlify Web 编辑器中突出的“发布拉取请求”按钮。" className="hidden dark:block" />
    </Frame>
  </Step>
</Steps>
...
2. **留下反馈**：添加评论或请求修改。
3. **进一步修改**：可在 Web 编辑器中继续修改。保存更改时，编辑器会将其推送到你的拉取请求（PR）。
...
将预览部署与你的团队共享，在发布更改之前收集反馈。

1. 点击编辑器工具栏中的 **Share** 按钮。
2. 点击 **Preview** 按钮，在新的浏览器标签页中打开预览部署。
   <Frame>
     <img src="/images/editor/share-preview-light.png" alt="在编辑器工具栏中打开的分享预览面板。" className="block dark:hidden" />

     <img src="/images/editor/share-preview-dark.png" alt="在编辑器工具栏中打开的分享预览面板。" className="hidden dark:block" />
   </Frame>
3. 将预览 URL 分享给需要审核你更改的任何人。

当你进行更改时，预览部署会自动更新。更改可能需要短时间才会显示。

预览 URL 默认对公众开放访问。任何拥有该链接的人都可以查看你的预览部署。要将访问限制为你组织中已通过认证的成员，请在你的[控制台](https://dashboard.mintlify.com/products/addons)中启用预览认证。

<div id="keyboard-shortcuts">
  ## 键盘快捷键
</div>

Web 编辑器支持复制、粘贴、撤销、全选等常见快捷键，以及以下快捷键：
