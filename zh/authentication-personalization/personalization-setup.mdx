...
请参阅下方的[用户数据格式](zh/#user-data-format)部分，获取详细示例和实现指南。
  ...
  <Tab title="JWT">
    ...
    <Steps>
      ...
      <Step title="将 Mintlify 个性化集成到你的登录流程中。">
        ...
        - 生成一个包含已登录用户信息并符合 `User` 格式的 JWT。更多信息参见上方的 [User data format](zh/#user-data-format) 部分。
  ...
  <Tab title="OAuth 2.0">
    ...
    <Steps>
      <Step title="Create user info API endpoint.">
        ...
        - 按 `User` 格式返回用户数据。详见上方的[用户数据格式](zh/#user-data-format)。
        - 定义访问所需的作用域（scope）。
      </Step>
  ...
  <Tab title="Shared session">
    ...
    <Steps>
      <Step title="创建用户信息 API 端点。">
        ...
        - 使用你现有的会话认证来识别用户
        - 以 `User` 格式返回用户数据（参见上面的[用户数据格式](zh/#user-data-format)）
        - 如果 API 域与文档域**不完全匹配**：
