    ...
    * 能创建可由 OAuth 访问令牌访问的 API 端点（可选，用于启用个性化功能）。
    ...
    <Steps>
      ...
      <Step title="创建用户信息端点（可选）。">
        ...
        * 接受 OAuth 访问令牌进行认证。
        * 以 `User` 格式返回用户数据。参见 [User data format](/zh/zh/authentication-personalization/personalization-setup#user-data-format) 了解更多信息。
    ...
    * 能创建重定向 URL 的后端服务。
    ...
    <Steps>
      ...
      <Step title="将 Mintlify 认证集成到你的登录流程中。">
        ...
        * 生成一个包含已认证用户信息、符合 `User` 格式的 JWT。更多信息参见 [User data format](/zh/zh/authentication-personalization/personalization-setup#user-data-format)。
