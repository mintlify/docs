...
使用分组将侧边栏导航划分为多个部分。分组可以相互嵌套、添加标签，并配合图标进行展示。
...
在 `navigation` 对象中，`groups` 是一个数组，每个条目都是一个对象，且需要包含 `group` 字段和 `pages` 字段。`icon`、`tag` 和 `expanded` 字段为可选。
...
在分组上设置 `expanded: true`，即可使其在导航侧边栏中默认展开。这有助于突出重要部分，或提升关键内容的可发现性。
...
选项卡为你的文档划分具有独立 URL 路径的不同部分。它们会在文档顶部生成水平导航栏，方便用户在各部分之间切换。
...
在 `navigation` 对象中，`tabs` 是一个数组，每个元素都是一个对象，需包含 `tab` 字段，并可包含其他导航字段，例如 groups、pages、icons，或指向外部页面的链接。
...
菜单为一个 Tab 添加下拉导航项。使用菜单可帮助用户在某个 Tab 内直接跳转到特定页面。
...
锚点会在侧边栏顶部添加常驻的导航项。你可以用锚点来为内容分区、提供快速访问外部资源，或创建醒目的行动号召。
...
在 `navigation` 对象中，`anchors` 是一个数组，其中每个条目都是一个对象，必须包含 `anchor` 字段，并且可以包含其他导航字段，例如 groups、pages、icons，或指向外部页面的链接。
...
全局锚点特别适用于链接到不属于文档但应便于用户访问的资源，例如博客或支持门户。
...
在 `navigation` 对象中，`dropdowns` 是一个数组，其中每个条目都是一个对象，必须包含 `dropdown` 字段，并且可以包含其他导航字段，例如 groups、pages、icon，或指向外部页面的链接。
...
将 OpenAPI 规范直接集成到导航结构中，自动生成 API 文档。你可以创建独立的 API 区域，或将端点页面置于其他导航组件内。

可在导航层级的任意层级设置默认 OpenAPI 规范。子元素将继承该规范，除非它们定义了自己的规范。
...
在 `navigation` 对象中，`versions` 是一个数组；其中每个条目都是一个对象，必须包含 `version` 字段，并可包含其他任意导航字段。
...
在 `navigation` 对象中，`languages` 是一个数组，每个条目为一个对象，必须包含 `language` 字段，并可包含其他任意导航字段。
...
你可以任意组合使用锚点、标签页和下拉菜单。这些组件可以灵活相互嵌套，以构建所需的导航结构。
...
面包屑导航会在页面顶部显示完整的导航路径。部分主题默认启用面包屑导航，另一些则不会。你可以在 `docs.json` 中通过 `styling` 属性控制是否启用面包屑导航。
...
* 设为 `false`：不进行跳转，选择时只展开或折叠该分组。
* 保持未设置：使用主题的默认行为。

<CodeGroup>
  ```json 强制导航
  "interaction": {
    "drilldown": true  // 当用户展开下拉项时强制跳转到第一页
  }
  ```

  ```json 阻止导航
  "interaction": {
    "drilldown": false // 不进行跳转，只展开或折叠该分组
  }
  ```
</CodeGroup>