---
title: "React"
description: "使用 React 组件构建交互式、可复用的元素"
icon: "react"
---
...
你可以从 `snippets` 文件夹导入组件。与常规 React 不同，你不能从任意 `MDX` 文件中导入组件。可复用组件必须从 `snippets` 文件夹内的文件进行引用。了解更多：[可复用代码片段](/zh/zh/reusable-snippets)。

<div id="example">
  ### 示例
</div>

本示例声明了一个使用多个 React Hook 的 `ColorGenerator` 组件，并在一个 `MDX` 文件中引用它。

在 `snippets` 文件夹中创建 `color-generator.jsx` 文件：

```mdx /snippets/color-generator.jsx [可展开]
