...
[React 组件](https://react.dev) 是在文档中创建交互且可复用元素的强大方式。
...
此示例先声明一个 `Counter` 组件，然后通过 `<Counter />` 进行使用。
...
你可以从 `snippets` 文件夹导入组件。与常规 React 不同，你不能从每个 `MDX` 文件中导入组件。可复用组件必须从 `snippets` 文件夹内的文件进行引用。了解更多：[可复用 Snippets](/zh/reusable-snippets)。

<div id="example">
  ### 示例
</div>

下面的示例声明了一个使用多个 React Hook 的 `ColorGenerator` 组件，并在一个 `MDX` 文件中引用它。
...
<AccordionGroup>
  <Accordion title="客户端渲染的影响">
    React Hook 组件在客户端渲染，这会带来如下影响：

    * **SEO**：搜索引擎可能无法完整索引动态内容。
    * **初始加载**：在组件渲染之前，访客可能会看到短暂的加载占位或闪烁。
    * **可访问性**：确保动态内容变更能被屏幕阅读器宣布。
  </Accordion>

  <Accordion title="性能最佳实践">
    * **优化依赖数组**：在 `useEffect` 的依赖数组中仅包含必要的依赖项。
    * **缓存复杂计算**：对开销较大的操作使用 `useMemo` 或 `useCallback`。
    * **减少重复渲染**：将大型组件拆分为更小组件，避免级联式重渲染。
    * **按需加载**：考虑对复杂组件进行懒加载，以缩短页面的初始加载时间。
  </Accordion>
</AccordionGroup>
