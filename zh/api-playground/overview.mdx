...
<ResponseField name="url" type="&quot;full&quot;">
    端点头部中基础 URL 的显示模式。当设置为 `full` 时，每个端点页面会显示完整的基础 URL。默认情况下，仅显示相对端点路径。
</ResponseField>
...
<ResponseField name="examples" type="object">
  ...
  <Expandable title="examples" defaultOpen="True">
    ...
    <ResponseField name="autogenerate" type="boolean">
      是否根据 API 规范为端点生成代码示例。默认为 `true`。当设置为 `false` 时，只有手写的代码示例（来自 OpenAPI 规范中的 `x-codeSamples` 或 MDX 中的 `<RequestExample>` 组件）会显示在 API playground 中。
    </ResponseField>
  </Expandable>
</ResponseField>
