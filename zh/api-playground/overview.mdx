...
<ResponseField name="playground" type="object">
  ...
  <Expandable title="playground" defaultOpen="True">
    ...
    <ResponseField name="proxy" type="boolean" defaultOpen="True">
      ...
      当设为 `true` 时，playground 请求会通过 Mintlify 的服务器进行路由。当设为 `false` 时，playground 会直接从浏览器向你的 API 发送请求。当你的 API 接受浏览器直接请求且不需要 Mintlify 代理流量时，请设为 `false`。例如，当你的 API 需要特定的无法通过代理转发的请求头，或者当你需要请求直接来自用户浏览器以进行身份验证时。
    </ResponseField>
  </Expandable>
</ResponseField>
