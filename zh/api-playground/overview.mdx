...
<ResponseField name="playground" type="object">
  ...
  <Expandable title="playground" defaultOpen="True">
    ...
    <ResponseField name="credentials" type="boolean">
      在代理禁用时，是否在跨域请求中包含 Cookie 和认证头。默认为 `false`。

      当设置为 `true` 且 `proxy` 为 `false` 时，playground 会在请求中包含凭据（Cookie、授权头或 TLS 客户端证书）。当你的 API 依赖基于 Cookie 的认证或会话管理，且需要浏览器直接发送这些凭据时，此选项非常有用。当 `proxy` 为 `true` 时此选项无效。
    </ResponseField>
  </Expandable>
</ResponseField>
