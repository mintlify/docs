...
<ResponseField name="playground" type="object">
  ...
  <Expandable title="playground" defaultOpen="True">
    ...
    <ResponseField name="credentials" type="boolean">
      当代理被禁用时，是否在跨域请求中包含 cookies 和认证头。默认为 `false`。

      当设置为 `true` 且 `proxy` 为 `false` 时，playground 会在请求中包含凭证（cookies、授权头或 TLS 客户端证书）。当你的 API 依赖于基于 cookie 的身份验证或会话令牌，并且需要浏览器自动附加它们时，此选项非常有用。当 `proxy` 为 `true` 时，此选项不生效。
    </ResponseField>
  </Expandable>
</ResponseField>
