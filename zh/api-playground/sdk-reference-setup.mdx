...
在 group 上声明 `sdk`，可将生成的页面限定在现有 tab 的某个子部分中。嵌套的 group 和页面会继承其上方最近的 tab 或 group 的 `sdk` 配置。声明了自己的 `sdk` 的 group 会完全替换继承的配置；两份配置不会合并。
...
包含 `sdk` 的 group 也可以列出 `pages`。手动编写的页面会显示在生成的参考分组之前。

<Note>
  你可以在 [tab](/zh/organize/navigation#tabs) 或 [group](/zh/organize/navigation#groups) 上声明 `sdk`。包含 `sdk` 的 tab 可以包含 `groups`，但不能包含其他导航结构，例如 `pages`、`versions` 或 `languages`，也不能包含 `openapi`、`asyncapi` 或 `graphql` 属性。包含 `sdk` 的 group 可以包含 `pages` 和嵌套的 group，但不能包含 `graphql` 属性。
</Note>
...
添加多个 tab 或 group 即可为多个库生成文档，例如为同一 SDK 的稳定版和 beta 版使用并列的 group。为每个库使用唯一的 `directory`，以避免路由冲突。
...
Mintlify 会将生成的导航组添加到 tab 中已有的任何 `groups` 之后；如果 `sdk` 声明在 group 上，则添加到该 group 现有 `pages` 之后。这些组因格式而异，可能表示模块、包、命名空间或符号类型。
...
请像其他页面一样，将该页面列入 `docs.json` 的导航中。Mintlify 只会组合出现在导航中的页面。

当声明了 `sdk` 的 tab 或 group 下至少存在一个带有有效 `sdk` frontmatter 的页面时，Mintlify 会跳过该 tab 或 group 的批量生成，只渲染你编写的页面。如果你希望该库的其余部分自动填充，请将该页面移出该 tab 或 group。
...
字符串形式会被解析为 `[source] kind name`。省略 `source` 时，会从 tab 或 group 的 `sdk` 配置继承。字符串形式没有 `format` 字段，始终依赖继承，因此只能用于位于带有 `sdk` 的 tab 或 group 下的页面；其他情况请使用对象形式。对于 `method` 和 `property` 目标，需要使用父级限定名称，例如 `method Auth0Client.loginWithRedirect`。

如果页面的 frontmatter 省略了 `title` 或 `description`，Mintlify 会使用为该符号生成的标题和描述。
