...
在 group 上声明 `sdk`，可将生成的页面限定在现有 tab 的某个子部分中。Group 会从其父级继承 `sdk` 配置，因此嵌套的 group 可以覆盖或扩展 tab 级别的构建产物。
...
<Note>
  你可以在 [tab](/zh/organize/navigation#tabs) 或 [group](/zh/organize/navigation#groups) 上声明 `sdk`。包含 `sdk` 的 tab 或 group 可以包含嵌套的 `groups`，但不能包含其他导航结构，例如 `pages`、`versions` 或 `languages`。此外，它也不能包含 `openapi`、`asyncapi` 或 `graphql` 属性。
</Note>
...
当带有 `sdk` 的 tab 或 group 下至少存在一个手动编写的 SDK 页面时，Mintlify 会跳过该所属项的批量生成。如果你希望该库的其余部分自动填充，请将该页面移出该所属项。
...
字符串形式会被解析为 `[source] kind name`。省略 `source` 时，会从 tab 或 group 的 `sdk` 配置继承 `source`（以及 `format`）。对于 `method` 和 `property` 目标，需要使用父级限定名称，例如 `method Auth0Client.loginWithRedirect`。
...
<ParamField path="format" type="string">
  覆盖继承的 `format`。仅在页面不属于带有 `sdk` 的 tab 或 group 时才需要。
</ParamField>

<ParamField path="source" type="string">
  覆盖继承的 `source`。仅在页面不属于带有 `sdk` 的 tab 或 group 时才需要。
</ParamField>
