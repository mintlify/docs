...
在 group 上添加 `sdk`，即可只在 tab 的某一部分生成页面，而不是占用整个 tab。Group 和页面会继承其上方 tab 或 group 的 `sdk` 设置。如果嵌套的 group 定义了自己的 `sdk`，Mintlify 会使用该设置，而不是继承的设置。
...
字符串形式遵循 `[source] kind name` 的模式。如果省略 `source`，页面会从 tab 或 group 的 `sdk` 配置继承。字符串形式始终继承 `format`，因此只能用于带有 `sdk` 的 tab 或 group 下的页面。其他情况请使用对象形式。对于 method 和 property，请包含父级名称，例如 `method Auth0Client.loginWithRedirect`。
