...
运行 [`mint dev`](/zh/cli/commands#mint-dev) 可在本地预览 SDK 参考页面。CLI 会在启动时解析构建产物，并在你更改 `docs.json` 或该构建产物文件时重新构建页面。

如果页面未出现，或构建报错：

- 确认 `format` 与你的工具生成的构建产物匹配。
- 确认 `source` 能正确解析到你文档仓库中相应的文件或目录，或解析到一个可访问的 HTTPS URL。
- 对于目录格式，确认构建产物中包含预期的工具输出（例如 `docfx metadata` 生成的 YAML，或 Sphinx `json` builder 的输出）。
- 对于远程源，确认下载不超过 50 MB / 解压后 200 MB 的限制。

运行 [`mint validate`](/zh/cli/commands#mint-validate) 可与文档构建的其余部分一起显示配置错误。
