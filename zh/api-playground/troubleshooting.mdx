...
<AccordionGroup>
  ...
  <Accordion title="来自 API 操作台的请求无法正常工作">
    ...
    或者，如果你的反向代理禁止接受 `POST` 请求，你可以在 `docs.json` 中通过 `api.playground.proxy` 设置，让 Mintlify 直接将请求发送到你的后端，具体参见[设置文档](/zh/organize/settings#param-proxy)。使用此配置时，你必须在服务器上配置 CORS，因为请求是直接来自用户的浏览器，而不是通过你的代理。
  </Accordion>
