---
title: "故障排查"
description: "API 操作台常见问题的故障排查指南：逐一解决 OpenAPI 页面全部空白、规范校验失败、openapi 字段拼写错误、端点页面缺失与身份验证异常等配置问题，并提供使用 Swagger 校验器验证文档、运行 mint dev 在本地调试定位错误的实用排查方法。"
...
<AccordionGroup>
  ...
  <Accordion title="我的构建报错 &quot;Failed to fetch OpenAPI file for anchor or tab&quot;">
    此错误表示 Mintlify 在构建期间无法从 `docs.json` 中 `openapi` 字段指向的 URL 下载 OpenAPI 文档。常见原因：

    - 主机不可达或仅在私有网络中可解析。
    - URL 需要身份验证（token、会话 cookie 或 IP 允许列表）。
    - 证书无效，或域名存在 DNS 问题。
    - 源站返回了瞬时 `5xx` 或超时。
    - 构建运行时，规范正在重新发布，URL 返回了部分或空响应。

    构建从公网下载 URL 且不带凭据，请在你的网络之外的机器上用 `curl` 确认能下载到该文档。要修复该错误，请改用以下方式之一：

    - **将规范提交到文档仓库。** 当源 URL 需要认证时，推荐使用此方式。将 `openapi` 字段指向仓库相对路径（例如 `"openapi": "openapi.json"`），并在修改 API 的同一次提交中更新该文件。
    - **通过稳定的公共 HTTPS URL 提供规范。** 将其托管在不需要认证的 CDN 或对象存储桶上，配备有效的 TLS 证书，并在每次请求都返回完整文档。

    有关完整的诊断流程（包括由 CI 在触发构建后才发布规范导致的间歇性失败），请参见[构建报错 "Failed to fetch OpenAPI file for anchor or tab"](/zh/help-center/openapi-url-fetch-fails-during-build)。
  </Accordion>
