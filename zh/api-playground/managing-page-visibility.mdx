...
您可以控制哪些 OpenAPI 操作会发布为文档页面，以及它们在导航中的可见性。这对于仅供内部使用的端点、已废弃的操作、测试版功能，或应可通过直接 URL 访问但不应通过站点导航被发现的端点非常有用。

如果您的页面是由 OpenAPI 文档自动生成的，您可以使用 `x-hidden` 和 `x-excluded` 扩展来管理页面可见性。
