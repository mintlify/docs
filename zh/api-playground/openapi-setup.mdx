...
方法和路径必须与您的 OpenAPI 规范一致。末尾斜杠的差异是可以容忍的——`/users` 和 `/users/` 会解析到同一个端点。如果您的规范将两者定义为不同的操作，精确匹配优先。若您有多个 OpenAPI 规范，请在引用中包含对应规范文件的路径。您也可以在 `docs.json` 中引用外部 OpenAPI URL。
