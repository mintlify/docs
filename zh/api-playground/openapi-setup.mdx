...
使用 `x-mint: mcp` 控制哪些 API 操作通过你的 [MCP 服务器](/ai/model-context-protocol)作为工具暴露。默认情况下，操作不会作为 MCP 工具暴露。添加 `x-mint: mcp` 并设置 `enabled: true` 来启用特定操作。

<ParamField body="enabled" type="boolean">
  是否将此操作作为 MCP 工具暴露。默认为 `false`。
</ParamField>

<ParamField body="name" type="string">
  MCP 工具的自定义名称。如未设置，工具名称将从操作中派生。
</ParamField>

<ParamField body="description" type="string">
  MCP 工具的自定义描述。如未设置，工具描述将从操作的摘要或描述中派生。
</ParamField>
...
<Note>
  已弃用的 `x-mcp` 扩展仍然受支持，但 `x-mint: mcp` 优先级更高。请在新配置中使用 `x-mint: mcp`。
</Note>
