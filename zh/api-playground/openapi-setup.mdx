...
使用 `x-mint: mcp` 控制各个 API 端点如何在你的 [MCP 服务器](/ai/model-context-protocol)中作为工具暴露。默认情况下，所有端点都可作为 MCP 工具使用。使用此扩展可以禁用特定端点，或自定义 AI 应用连接到你的 MCP 服务器时看到的工具名称和描述。

`x-mint: mcp` 扩展接受以下属性：

| 属性 | 类型 | 描述 |
| :--- | :--- | :--- |
| `enabled` | boolean | 端点是否作为 MCP 工具可用。默认为 `true`。 |
| `name` | string | MCP 工具的自定义名称。覆盖默认生成的名称。 |
| `description` | string | MCP 工具的自定义描述。覆盖默认生成的描述。 |
...
要从 MCP 服务器隐藏某个端点，同时保持其在文档中可见，请将 `enabled` 设置为 `false`：
...
<Note>
  `x-mint: mcp` 扩展取代了已弃用的 `x-mcp` 扩展。如果两者同时存在于同一操作上，`x-mint: mcp` 优先。
</Note>
