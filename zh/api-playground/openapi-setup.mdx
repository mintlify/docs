...
要用 OpenAPI 为你的端点撰写文档，你需要一个有效的 OpenAPI 文档，采用 JSON 或 YAML 格式，并遵循 [OpenAPI 3.0+ 规范](https://swagger.io/specification/)。

你可以引用多个 OpenAPI 规范来为 API 创建页面。可以在导航中为每个规范组织版块，或从不同规范中引用特定端点。

<Note>
  Mintlify 仅在单个 OpenAPI 文档内支持 `$ref` 的**内部引用**。不支持外部引用。
</Note>
...
在某个导航元素中仅添加一个 `openapi` 字段（不包含其他页面），即可生成专用的 API 区段。规范中的所有端点都会被纳入：
...
要在文档的不同章节中组织多个 OpenAPI 规范，请在导航层级中将每个规范分配到不同的分组。每个分组都可以引用其各自的 OpenAPI 规范。
...
<Note>
  `directory` 字段是可选项，用于指定生成的 API 页面在文档仓库中的存放位置。若未指定，默认为仓库的 `api-reference` 目录。
</Note>
...
通过包含文件路径，无需设置默认的 OpenAPI 规范即可引用特定端点。你也可以在同一文档部分中引用来自多个 OpenAPI 规范的端点。
...
* 在同一导航部分中混合使用来自多个 OpenAPI 规范的端点
* 只包含不同规范中的特定端点，而非全部端点
* 将 API 端点与其他类型的文档页面一并集成
...
如果你在多个文件中有同名的架构，也可以指定 OpenAPI 文件：
