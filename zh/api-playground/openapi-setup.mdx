---
title: "OpenAPI 设置"
description: "从 OpenAPI 规范文件生成交互式 API 文档。"
...
通过 `x-mint: mcp` 控制哪些 API 操作作为工具暴露给你的 [MCP 服务器](/ai/model-context-protocol)。默认情况下，所有操作都会包含在内。使用此扩展可以禁用特定操作或自定义它们在 AI 工具中的显示方式。
...
| 属性 | 类型 | 说明 |
| :---------- | :------ | :---------- |
| `enabled` | boolean | 是否将此操作暴露为 MCP 工具。默认为 `true`。 |
| `name` | string | MCP 工具的自定义名称。覆盖默认生成的名称。 |
| `description` | string | MCP 工具的自定义描述。覆盖操作的 `description`。 |

要从 MCP 服务器中排除某个端点，将 `enabled` 设置为 `false`：
...
<Note>
  `x-mint: mcp` 扩展取代了旧版 `x-mcp` 扩展。如果两者同时存在于一个操作上，`x-mint: mcp` 优先。
</Note>
