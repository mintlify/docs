---
title: "OpenAPI 交互式 API 文档配置完整指南：规范文件、请求构建器、身份验证、端点页面和文件上传"
description: "通过 OpenAPI 规范文件生成交互式 API 文档，自动创建端点页面、请求构建器和导航结构。了解如何配置 API 服务器、身份验证、请求参数、响应示例及文件上传，让团队快速发布清晰、可测试且易于维护的 API 参考。适用于希望在一个地方展示并测试 API 的团队。"
...
当请求正文 schema 标识文件时，Mintlify 会在 API playground 中渲染文件输入框。对于接受图像、文档、归档文件或其他二进制数据的端点，请使用文件输入。

对于 OpenAPI 3.1，将 `type` 设置为 `string`，并添加 `contentMediaType`。

当 API 需要 base64 编码的文件时，添加 `contentEncoding: base64` 或 `contentEncoding: base64url`。

对于 OpenAPI 3.0，则使用 `format: binary` 或 `format: base64`。
...
API playground 支持常见的图像、音频、视频和二进制应用媒体类型。用户可以将文件拖到上传字段，或点击字段选择文件。
