---
title: "手动创建 API 文档页面"
description: "了解如何在 Mintlify 中使用 MDX 文件手动创建 API 参考文档页面：在 docs.json 中配置基础 URL 与认证方式，通过 frontmatter 的 api 字段定义端点、路径参数与请求方法，适用于小型 API、原型开发或需要完全控制布局的自定义文档场景。"
...
<Steps>
  ...
  <Step title="添加参数与响应">
    使用[参数和响应字段](/zh/components/fields)为端点的参数和返回值编写文档。若要在右侧边栏中显示请求和响应代码示例，请将其包装在 [`<RequestExample>` 和 `<ResponseExample>`](/zh/components/examples) 组件中。
