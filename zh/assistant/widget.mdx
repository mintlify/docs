...
Mintlify 会为小组件收集内部使用分析。默认情况下,每个事件都会包含嵌入页面的当前路径,便于你查看哪些页面驱动了助手的使用。路径长度上限为 500 个字符,并且不包含查询字符串或片段。查询参数、片段、完整 URL 和来源(referrer)始终不会被收集。

如果你的站点在路径段中包含令牌、个人数据或其他私有值,请在初始化时禁用路径捕获:
...
在运行时更改 `analytics.capturePathname` 需要先调用 `destroy()`,再调用新的 `init()`。
