---
title: "嵌入并配置 Mintlify AI 助手小组件"
sidebarTitle: "小组件"
description: "了解如何安装和配置 Mintlify AI 助手小组件，将基于你文档内容训练的对话助手嵌入网站、Web 应用、产品仪表板或支持门户，并控制触发器、主题、访问方式和页面显示效果。本文还介绍安装代码、生命周期方法、筛选条件、事件钩子和内容安全策略配置，并将其应用于不同页面和用户流程。"
...
<Warning>
  模块脚本会延迟加载并按文档顺序执行。当你使用 HTML 安装小组件时，请将托管加载器保持在初始化代码块之前，否则小组件将无法挂载。
</Warning>
...
例如，在两个页面脚本中使用同一个生成的 nonce，并将其传入 `init()`：
...
你的策略必须在 `script-src` 中允许 `https://widget.mintlify.com`，在 `style-src` 中允许生成的 nonce，并在 `connect-src` 中允许所需的小组件和 API 来源。
