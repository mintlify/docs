...
浏览器端唯一必需的选项是公共小组件 ID。启用状态、允许的来源、附件、机器人防护以及[分流联系表单](/zh/assistant/configure#set-deflection-emails)均可在仪表板中管理。可在浏览器配置中设置针对该嵌入的起始问题和支持邮箱。
...
1. 前往你部署的 [Widget](https://app.mintlify.com/settings/project/widget) 页面。
2. 启用小组件。
3. 添加嵌入小组件的允许来源。
4. 复制小组件 ID。
...
<Info>
  请将生成代码中的 `YOUR_WIDGET_ID` 替换为你仪表板 [Widget](https://app.mintlify.com/settings/project/widget) 页面中的小组件 ID。
</Info>
