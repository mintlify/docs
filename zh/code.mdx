...
你可以添加行内代码片段或代码块。代码块支持用于语法高亮、标题、行高亮、icon 等的元选项，另有更多功能。

<div id="inline-code">
  ### 行内代码
</div>

要将某个 `word` 或 `phrase` 标记为代码，请用反引号（`）包裹。
...
将代码用三个反引号包裹，以使用[fenced code blocks](https://www.markdownguide.org/extended-syntax/#fenced-code-blocks)。代码块可复制；如果你启用了 AI 助手，用户可以让 AI 解释代码。

为语法高亮和启用元选项指定编程语言。然后在语言标识之后添加任意元选项，例如 title 或 icon。
...
我们使用 [Shiki](https://shiki.style/) 实现语法高亮，支持其提供的全部语言。完整的[语言列表](https://shiki.style/languages)请参见 Shiki 文档。

在 `docs.json` 中通过 `styling.codeblocks` 全局自定义代码块主题。你可以设置 `system` 或 `dark` 等简单主题，或为浅色与深色模式配置自定义的 [Shiki 主题](https://shiki.style/themes)。详见[设置](/zh/settings#styling)以获取完整配置选项。
...
使用 `icon` 属性为你的代码块添加图标。有关所有可用选项，请参见 [图标](/zh/components/icons)。
...
在代码块中使用 `highlight` 指定要高亮的行号或行范围，以突出显示特定行。
...
* `// [!code ++:3]`：将当前行以及后续两行标记为新增。
* `// [!code --:5]`：将当前行以及后续四行标记为删除。
