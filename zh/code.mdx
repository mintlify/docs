...
在 `docs.json` 的 `styling.codeblocks` 中可全局自定义代码块主题。可设置如 `system` 或 `dark` 的简单主题，或为明暗模式配置自定义的 [Shiki 主题](https://shiki.style/themes)。详细配置选项见 [Settings](/zh/zh/settings#styling)。
...
使用 `icon` 属性为代码块添加图标。参见 [Icons](/zh/zh/components/icons) 查看所有可用选项。
