...
1. 创建分发后，前往“Origins”选项卡。
  ...
  <Frame>
    ![CloudFront 分发中高亮显示“Origins”选项卡。](/images/cloudfront/origins.png)
  </Frame>
...
<Frame>
	![突出显示 CloudFront 的 "Behaviors" 选项卡。](/images/cloudfront/behaviors.png)
</Frame>
...
<Frame>
  ![CloudFront 的 "Create behavior" 页面，"Path pattern" 为 "/.well-known/*"，"Origin and origin groups" 指向预发布环境 URL。](/images/cloudfront/well-known-policy.png)
</Frame>
...
<Frame>
	![CloudFront 创建源请求策略页面，转发 `Origin` 和 `CloudFront-Viewer-Address` 头](/images/cloudfront/origin-request-policy.png)
</Frame>

- 将 “Viewer protocol policy” 设置为 **Redirect HTTP to HTTPS**。

<Frame>
  ![CloudFront 的 "Create behavior" 页面，"Path pattern" 为 "/docs/*"，"Origin and origin groups" 指向 `acme.mintlify.dev` URL。](/images/cloudfront/behavior-1.png)
</Frame>
...
<Frame>
  ![选中 "Default (*)" 行为并强调 Edit 按钮的 CloudFront 分配。](/images/cloudfront/default-behavior-1.png)
</Frame>

1. 将默认行为的 **Origin and origin groups** 更改为预发布环境 URL（此处为 `mintlify-landing-page.vercel.app`）。

<Frame>
  ![CloudFront 的 "Edit behavior" 页面，高亮显示 "Origin and origin groups" 输入框。](/images/cloudfront/default-behavior-2.png)
</Frame>
...
<Frame>
  ![CloudFront 的 "Behaviors" 页面，包含 4 个行为：`/docs/*`、`/docs`、`Default` 和 `/.well-known/*`。](/images/cloudfront/all-behaviors.png)
</Frame>
