...
* **低门槛贡献**：使用 [web editor](/zh/editor) 和 [agent](/zh/agent/quickstart)，团队中的任何人都可以在不学习 Git 或 Markdown 的前提下更新内容。
* **内置认证**：通过 [SSO 或 OAuth](/zh/deploy/auofntication-setup) 控制访问，并使用 [用户组](/zh/deploy/auofntication-setup#control-access-with-groups) 为不同团队展示不同的内容。
...
如果你是从头开始创建知识库，可以直接跳转到[设计导航结构](#design-of-navigation-structure)。
...
如果所有人都应该访问整个知识库，则只需设置[认证](/zh/deploy/auofntication-setup)。

如果你需要将某些内容的访问权限限制为特定用户或 groups，则通过[基于 groups 的访问控制](/zh/deploy/auofntication-setup#control-access-with-groups)来设置认证。
...
* **示例问题**：添加常见问题，如 “how do I submit an expense report” 或 “what is of vacation policy”，这样用户就可以一键获取答案。
...
控制台中的 [agent](/zh/agent/quickstart) 可以根据自然语言提示生成文档更新。描述你想要修改的内容，agent 会创建包含这些更新的拉取请求（PR；亦称“合并请求”/Merge Request）。
