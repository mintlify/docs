...
如果你与自己的翻译供应商或区域翻译人员合作，可以使用 GitHub Actions 或类似的 CI/CD 工具将他们的工作流程与你的 Mintlify 文档集成。

1. **导出源内容**：提取需要翻译的 MDX 文件。
2. **发送给翻译人员**：将文件提供给你的翻译供应商。
3. **接收翻译**：收回已翻译的 MDX 文件。
4. **导入并部署**：将翻译后的文件添加到语言目录并更新导航。

以下 GitHub Actions 工作流程会在 PR 合并到 main 时自动导出已更改的英文内容以供翻译。
...
以下 GitHub Actions 工作流程会在通过 PR 添加翻译内容时进行验证和导入。
...
- **保留 frontmatter**：确保翻译人员保持 YAML frontmatter 完整，仅翻译 `title` 和 `description` 的值。
- **保护代码块**：为你的供应商标记代码块为"请勿翻译"。
- **使用翻译记忆库**：提供术语表，列出应保留英文或有特定翻译的技术术语。
- **自动化验证**：在合并翻译之前，使用 CI 检查验证 MDX 语法和 frontmatter。
- **版本控制**：跟踪每个翻译的源版本，以识别过时的内容。
