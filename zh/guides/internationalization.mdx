...
title: "如何设置多语言文档"
sidebarTitle: "国际化"
description: "设置多语言文档，支持基于语言区域的路由、语言切换导航和翻译内容，以覆盖全球用户。本文结合实践建议和操作步骤，帮助你在 Mintlify 项目中完成相关工作并持续维护文档。这些说明适合开发者和文档维护者查阅。你可以结合页面示例完成相关操作。相关做法也便于团队后续维护和协作。"
...
<Expandable title="支持的语言代码">
  ...
  * `ca` - 加泰罗尼亚语
  * `cs` - 捷克语
  * `zh` 或 `zh-Hans` 或 `zh-CN` - 中文 (简体)
  * `zh-Hant` 或 `zh-TW` - 中文 (繁体)
  * `de` - 德语
  * `en` - 英语
  * `es` - 西班牙语
  * `fi` - 芬兰语
...
如需自动化翻译解决方案，[设置自动化](/zh/automations)，让代理按计划运行或在仓库推送时触发。
...
<Accordion title="增加语言会影响站点性能吗？">
Mintlify 将每种语言版本作为一组独立的静态页面提供，因此访问 `/zh/quickstart` 的读者只会加载该页面的中文内容，而不是所有翻译。增加语言不会改变最终用户的页面大小或运行时性能。

不过，增加语言会扩大你的存储库以及构建的工作集。为保持写作和 CI 的高效：

* 将翻译后的图片存放在按语言划分的目录中，避免将未使用的语言的内容拉入其他语言中。
* 在较大的存储库中，将 `mint broken-links` 及其他 CI 检查限定在已更改的文件，而不是整个目录树。
</Accordion>
