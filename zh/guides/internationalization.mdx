...
如果你与自己的翻译供应商或区域译员合作，可以使用 GitHub Actions 或类似的 CI/CD 工具，将他们的工作流程集成到 Mintlify 文档中。
...
该 GitHub Actions 工作流会在通过 PR 添加翻译内容时进行验证并导入。
  ...
  validate:
    ...
    steps:
      - name: Checkout repository
        uses: actions/checkout@v4
        with:
          fetch-depth: 0  # 获取完整历史以确保 origin/main 可用
      ...
      - name: Validate frontmatter
        ...
        run: |
          ...
          while IFS= read -r file; do
            ...
            # 检查 frontmatter 是否有效（第 1-2 行必须是 ---）
