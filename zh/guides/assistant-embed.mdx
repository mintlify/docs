...
一个可复用的小部件，可将[AI 助手](/assistant)直接嵌入到你的应用中。该小部件提供：
...
你必须在第一次响应时捕获并持久化这两个值。在每条后续消息中，在请求体中同时包含 `threadId` 和 `threadKey`。如果你发送 `threadId` 但没有对应的 `threadKey`，服务器将返回 `404` 错误。

<Warning>
  你必须立即存储 `X-Thread-Key` 头。服务器仅在创建新线程时返回它。你无法在之后检索它。
</Warning>
...
export function AssistantWidget({ domain, docsURL }) {
  ...
  useEffect(() => {
    // 从 localStorage 中检索保存的线程 ID 和密钥
