...
Cursor 规则为你的文档提供持久的 context，确保生成的建议更一致，并符合你的标准与风格。

* **项目规则** 存储在你的文档存储库中，并与团队共享。
* **用户规则** 适用于你的个人 Cursor 环境。

我们建议为你的文档创建项目规则，以便所有贡献者都能遵循相同的规则。

在你的文档存储库的 `.cursor/rules` 目录中创建规则文件。完整的设置说明请参见 [Cursor 规则文档](https://docs.cursor.com/context/rules)。
...
此规则为 Cursor 提供 context，便于正确格式化 Mintlify 组件并遵循技术写作最佳实践。
...
将此规则（包含你所做的任何修改）作为一个 `.mdc` 文件添加到文档仓库的 `.cursor/rules` 目录中。
...
<Frame caption="Analytics 控制台提供实时洞察">
<img src="/images/analytics.png" alt="带有图表的 Analytics 控制台" />
</Frame>
