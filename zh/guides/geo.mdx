...
你的 `robots.txt` 文件控制哪些机器人可以爬取你的站点。如果它阻止了 AI 用户代理，ChatGPT、Claude 和 Perplexity 等工具将无法读取你的文档，也无法在回答中引用它。

Mintlify 自动生成的 `robots.txt` 默认允许所有爬虫。如果你使用[自定义 robots.txt 文件](/zh/optimize/seo#custom-sitemaps-and-robotstxt-files)，请确保它不会阻止 AI 代理。最常见的 AI 用户代理包括：
...
要阻止特定爬虫同时允许 AI 代理，请仅针对你想限制的机器人：
...
运行 [`mint score`](/zh/cli/commands#mint-score) 检查你的站点的 `robots.txt` 是否允许 AI 代理。当没有 AI 用户代理被阻止时，`robotsTxtAllowsAI` 检查会通过。
