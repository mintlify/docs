...
在页面中添加[Update 组件](/zh/components/update)，即可为文档创建更新日志。

查看 [Mintlify 更新日志](/zh/changelog) 作为示例：你可以在每次更新中加入链接、图片、文本，以及新功能演示。
...
<Steps>
  <Step title="为更新日志创建页面">
    ...
    2. 将该更新日志页面添加到 `docs.json` 的 navigation 配置中。
  </Step>

  <Step title="在更新日志中添加 Update 组件">
    为每条更新记录添加一个 `Update`。

    包含相关信息，例如功能发布、缺陷修复或其他公告。
  </Step>
</Steps>
...
<Frame>
  <img src="/images/changelog-filters-light.png" alt="浅色模式下的更新日志，已选择 Peppermint 标签筛选器。" className="block dark:hidden" />

  <img src="/images/changelog-filters-dark.png" alt="深色模式下的更新日志，已选择 Peppermint 标签筛选器。" className="hidden dark:block" />
</Frame>

<Tip>
  当使用 `custom`、`center` 或 `wide` 页面模式时，目录和更新日志筛选器将被隐藏。了解更多[页面模式](/zh/pages#page-mode)。
</Tip>
...
* 像 [Readybot](https://readybot.io) 或 [RSS Feeds to Discord Bot](https://rss.app/en/bots/rssfeeds-discord-bot) 这样的 Discord 机器人
...
<Frame>
  <img src="/images/changelog-rss-button-light.png" alt="浅色模式的更新日志页面，已启用 RSS 订阅按钮。" className="block dark:hidden" />

  <img src="/images/changelog-rss-button-dark.png" alt="深色模式的更新日志页面，已启用 RSS 订阅按钮。" className="hidden dark:block" />
</Frame>
