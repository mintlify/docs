...
sidebarTitle: "CSP 配置"
description: "在实施严格安全策略的反向代理、防火墙和网络环境下的域名白名单与响应头配置"
---

Content Security Policy（CSP）是一项安全标准，通过限制网页可加载的资源来帮助防止跨站脚本（XSS）攻击。Mintlify 提供的默认 CSP 可保护大多数站点。若你的文档托管在会覆盖默认 CSP 的反向代理或防火墙之后，可能需要自行配置 CSP 响应头，确保各项功能正常运行。
...
- `frame-src`: 控制可嵌入到 frame 或 iframe 中的 URL
- `default-src`: 未显式设置时，作为其他指令的后备
...
| 域名 | 用途 | CSP 指令 | 必填 |
|:-------|:--------|:--------------|:-------|
| `d4tuoctqmanu0.cloudfront.net` | KaTeX CSS、字体 | `style-src`、`font-src` | 必填 |
| `*.mintlify.dev` | 文档内容 | `connect-src`、`frame-src` | 必填 |
| `*.mintlify.com` | 控制台、API、Analytics 代理 | `connect-src` | 必填 |
| `leaves.mintlify.com` | AI 助手 API | `connect-src` | 必填 |
| `d3gk2c5xim1je2.cloudfront.net` | 图标、图片、标志 | `img-src` | 必填 |
| `d1ctpt7j8wusba.cloudfront.net` | Mint 版本与发布文件 | `connect-src` | 必填 |
| `mintcdn.com` | 图片、网站图标 | `img-src`、`connect-src` | 必填 |
| `*.mintcdn.com` | 图片、网站图标 | `img-src`、`connect-src` | 必填 |
| `api.mintlifytrieve.com` | 搜索 API | `connect-src` | 必填 |
| `cdn.jsdelivr.net` | 用于 OG 图片的表情符号资源 | `script-src`、`img-src` | 必填 |
...
<Note>
  仅包含你实际使用的服务的 domain。删除任何你未在文档中配置的 Analytics 相关 domain。
</Note>
