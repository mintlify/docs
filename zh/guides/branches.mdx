...
<Tabs>
  <Tab title="使用 Web 编辑器">
    ...
    下拉菜单只会列出在 Web 编辑器中创建的 branch，以及有指向部署用分支的开放拉取请求（PR）或合并请求（MR）的 branch。未开启拉取请求、直接推送到 GitHub、GitLab 或 Bitbucket 的 branch 仍然存在于你的存储库中，但不会出现在下拉菜单中。要在编辑器中打开这类 branch，请在你的 Git 提供商中为其开启一个拉取请求，或在编辑器中打开任意 branch，然后将浏览器 URL 中的 branch 名称替换为你想要的 branch 名称。
