---
title: "链接"
description: "创建内部链接、引用 API 端点，并使用相对路径和锚点目标维护文档中的链接完整性。"
...
每个 `View` 组件会自动根据其 `title` 属性生成锚点 ID。你可以使用 URL hash 直接链接到特定视图或视图内的元素：
...
当 URL hash 与视图 ID 或视图内的标题、手风琴、Tab 匹配时，对应的视图会自动激活，页面会滚动到目标位置。当用户通过下拉菜单切换视图时，URL hash 也会随之更新。
...
此视图可通过 `#js-guide` 访问，而非自动生成的 `#javascript`。

详情请参阅[视图](/zh/components/view)。
