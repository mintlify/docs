...
当同一页面中有多个标题生成相同的 ID 时，Mintlify 会为每个重复项依次追加 `-2`、`-3` 等后缀。该计数器在整个页面中生效，包括嵌套在选项卡等组件中的标题。如果你希望获得稳定、可预测的链接，请使用[自定义锚点 ID](#custom-anchor-ids)。完整的 slug 生成规则请参见[自动锚点 ID](/zh/create/text#automatic-anchor-ids)。
...
没有重定向时，已收藏和已分享的链接会变成 404 错误。每次移动或重命名页面时，都应在 `docs.json` 中设置重定向。重定向的添加成本很低，可以防止从外部来源（博客文章、Stack Overflow 回答、内部 wiki）链接到你文档的用户获得糟糕的体验。
</Accordion>
