...
当你在页面上使用 `View` 组件时，每个视图会根据其标题自动获得一个 URL 友好的 ID。使用 hash 直接链接到特定视图：

```text
https://你的文档地址/页面路径#python
```

你还可以链接到视图内的标题或其他锚点元素。正确的视图会自动激活，页面会滚动到目标位置：

```text
https://你的文档地址/页面路径#安装-sdk
```

当用户通过下拉菜单切换视图时，URL hash 会更新以匹配所选视图。详情请参阅[视图](/zh/components/view#deep-links)。
