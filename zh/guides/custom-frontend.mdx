...
当你需要完全掌控文档的设计、布局或行为时，这种无头方式会非常有用。例如，为了匹配现有的设计系统，或将文档嵌入到更大的网站中。

`@mintlify/astro` 集成会在构建时读取你的 `docs.json` 配置和 MDX 内容，然后将所有内容处理成 Astro 可渲染的格式。你可以在此基础上使用自己的布局、组件和样式。

通过无头自定义前端，你仍然可以无需导入它们就使用 Mintlify 组件，并且仍可使用发布、搜索和 AI 基础设施。
