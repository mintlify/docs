...
description: "结合 Mintlify 的无头模式与 Astro 集成，在借助 Mintlify 的内容与 AI 基础设施的同时，用完全自定义的前端渲染你的文档。"
keywords: ["Astro", "headless", "自托管", "自定义", "无头文档", "无头 CMS"]
---

Mintlify 的无头模式可以让你在由 Mintlify 管理文档内容的同时，完全掌控文档的外观和行为。你可以不使用 Mintlify 托管的前端，而是使用 [Astro](https://astro.build) 构建自己的自定义前端，并按任意方式渲染你的 MDX 内容和 `docs.json` 配置。

当你需要对文档的设计、布局或交互行为拥有完全控制时，这种无头方案会非常有用。例如，希望对齐现有的设计系统，或将文档嵌入到更大型的网站中。

`@mintlify/astro` 集成会在构建时读取你的 `docs.json` 配置和 MDX 内容，然后将所有内容处理为 Astro 可以渲染的格式。你可以在此基础上使用自己的布局、组件和样式。

使用无头自定义前端时，你仍然可以在无需显式导入的情况下使用 Mintlify 组件，并继续使用发布、搜索和 AI 基础设施。

本指南将引导你使用起始模板创建一个无头 Mintlify 项目，并在本地运行起来。
