---
title: "使用 Devin Desktop 编写文档"
sidebarTitle: "Devin Desktop"
description: "了解如何使用工作区规则和记忆配置 Devin Desktop（原 Windsurf）的 Cascade AI 编写 Mintlify 文档：在 .devin/rules 目录创建团队共享的规则，提供持久的项目 context，使建议符合你的样式指南、组件用法和 MDX 标准。"
keywords: ["Devin Desktop", "Windsurf", "Cascade", "workspace rules", "AI 文档", "Devin 文档"]
---

通过工作区规则和记忆，将 Devin Desktop 打造成一位熟悉你的样式指南、组件和项目 context 的文档专家。

<Info>
Cognition 收购了 Windsurf，并将其更名为 [Devin Desktop](https://devin.ai/desktop)。Cascade AI 助手和工作区规则系统保持不变。
</Info>
...
你可以调整 Devin Desktop 的 Cascade AI 助手，让它使用 Mintlify 组件按照你的标准撰写文档。工作区规则与记忆会为你的项目提供持久的 context，从而让 Cascade 的建议更加一致。

* **工作区规则** 保存在你的文档存储库中，因此可供整个团队共享。
* **记忆** 提供会随时间累积的个人 context。

我们建议为共享的文档规范设置工作区规则。你可以在工作中逐步积累记忆，但由于记忆不共享，团队成员之间的体验是不一致的。

在你的文档存储库的 `.devin/rules` 目录中创建工作区规则。为了向后兼容，Devin Desktop 仍支持 `.windsurf/rules` 中的规则。更多信息请参阅 Devin Desktop 文档中的 [Memories & Rules](https://docs.devin.ai/desktop/cascade/memories)。
