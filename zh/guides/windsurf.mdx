...
通过工作区规则和记忆，将 Windsurf 打造成了解你风格指南、组件和项目 context 的文档专家。
...
Windsurf 的 Cascade AI 助手可以配合 Mintlify 组件，按照你的规范撰写文档。Workspace 规则和记忆为你的项目提供持久的 context，确保 Cascade 提供的建议更加一致。

* **Workspace 规则** 存储在你的文档存储库中，并与团队共享。
* **Memories（记忆）** 提供会随时间累积的个人 context。

我们建议为共享的文档规范设置 Workspace 规则。你可以在工作中逐步积累记忆，但由于它们不共享，团队成员之间的体验将不一致。

在文档存储库的 `.windsurf/rules` 目录中创建 Workspace 规则。更多信息请参阅 Windsurf 文档中的 [Memories & Rules](https://docs.windsurf.com/windsurf/cascade/memories)。
...
此规则为 Cascade 提供关于 Mintlify 组件以及通用技术写作最佳实践的 context。

你可以直接使用此示例规则，或根据你的文档需求进行自定义：

* **写作标准**：更新语言规范以符合你的风格指南。
* **组件模式**：添加与你的项目相关的组件，或修改现有示例。
...
设置好规则后，你可以使用 Cascade 来协助处理各类文档任务。更多信息请参阅 Windsurf 文档中的 [Cascade](https://docs.windsurf.com/windsurf/cascade)。
