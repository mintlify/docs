...
通过工作区规则和记忆，将 Windsurf 打造成一名理解你的风格指南、组件和项目 context 的文档专家。
...
Windsurf 的 Cascade AI 助手可以结合 Mintlify 组件，按你的标准撰写文档。工作区规则和记忆会为你的项目提供持久的 context，确保 Cascade 给出更一致的建议。

* **工作区规则** 存储在你的文档存储库中，并与团队共享。
* **记忆** 提供会随时间累积的个人 context。

我们建议为共享的文档规范设置工作区规则。你可以在工作过程中逐步积累记忆，但由于它们不共享，因此团队成员之间的结果可能不一致。

在你的文档存储库的 `.windsurf/rules` 目录中创建工作区规则。有关更多信息，请参阅 Windsurf 文档中的 [Memories & Rules](https://docs.windsurf.com/windsurf/cascade/memories)。
...
此规则为 Cascade 提供有关 Mintlify 组件以及通用技术写作最佳实践的上下文信息。

你可以直接使用此示例规则，或根据你的文档进行自定义：

* **写作规范**：更新语言指南以符合你的风格指南。
* **组件模式**：添加项目专属组件或修改现有示例。
* **代码示例**：将通用示例替换为与你的产品相关的真实 API 调用与响应。
* **风格与语气偏好**：调整术语、格式和其他规则。

将此规则保存为 `.md` 文件，并放在你的文档仓库的 `.windsurf/rules` 目录中。
...
完成规则配置后，你可以使用 Cascade 来协助处理各类文档任务。更多信息请参阅 Windsurf 文档中的 [Cascade](https://docs.windsurf.com/windsurf/cascade)。
