...
description: "迁移到具备灵活导航的自动化 OpenAPI 生成。"
---

如果你目前为 API 端点使用独立的 `MDX` 页面，可以迁移为基于 OpenAPI 规范自动生成页面，同时保留单个页面的自定义能力。这有助于减少维护的文件数量，并提升 API 文档的一致性。

你可以在 OpenAPI 规范中为每个端点定义 metadata 和 content，并在导航中按需组织这些端点。
