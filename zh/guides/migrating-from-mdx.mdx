...
description: "从单个 MDX 端点页面迁移到带有灵活导航的自动化 OpenAPI 生成"
---

如果你目前为 API 端点使用单独的 `MDX` 页面，你可以迁移为基于 OpenAPI 规范自动生成页面，同时保留单页的自定义灵活性。这样可以减少需要维护的文件数量，并提升 API 文档的一致性。

你可以在 OpenAPI 规范中为每个端点定义 metadata 和 content，并在导航中按需组织端点的位置。
