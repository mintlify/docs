...
<AccordionGroup>
  ...
  <Accordion title="冲突">
    ...
    要在 Web 编辑器中解决冲突，请在你的 Git 提供商中打开对应的拉取请求，并在冲突视图中选择要保留的版本。在本地，编辑发生冲突的文件，删除冲突标记，然后暂存并提交这次修复。
  </Accordion>
  ...
  <Accordion title="Rebase">
    将一个分支上的提交依次重放到另一个分支的末端，从而重写历史，使工作看起来像是从更新的基础上开始的。Rebase 会产生一条不含合并提交的线性历史。关于何时在编辑器中使用 rebase，请参见[默认合并方式](/zh/editor/settings#default-merge-method)。
  </Accordion>
  ...
  <Accordion title="Squash">
    在合并时将一个分支上的所有提交合并为单个提交。Squash 可以让部署用分支的历史保持紧凑，但会丢失功能分支中每条单独的提交信息。关于何时在编辑器中使用 squash，请参见[默认合并方式](/zh/editor/settings#default-merge-method)。
  </Accordion>
