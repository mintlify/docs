...
Custom 模式页面没有默认排版样式。像 `<h2>` 和 `<p>` 这样的原生 HTML 元素会以浏览器默认样式渲染，因此请使用 Tailwind 类显式地为它们设置样式。
...
<Note>
  Web 编辑器的实时预览不会为页面专属的 Tailwind 类生成 CSS，因此自定义布局在编辑时可能看起来没有样式。请使用 `mint dev`、预览部署或生产部署来检查样式效果。参见 [Tailwind 类在编辑器实时预览中不生效](/zh/help-center/tailwind-classes-not-applying-in-editor-preview)。
</Note>
