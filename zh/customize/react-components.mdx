...
要在 MDX 文件中使用 React 组件，组件文件必须放在某个 snippet 文件夹中。默认情况下，这是 `/snippets/` 文件夹。你可以在 `docs.json` 中[配置额外的目录](/zh/create/reusable-snippets#configure-snippet-folders)用于存放片段。了解更多关于[可复用片段](/zh/create/reusable-snippets)的内容。
