---
title: "React"
description: "使用自定义 React 组件在 Mintlify 文档中构建交互式可复用元素。本文介绍在 MDX 中内联定义带 useState 状态的组件、从 snippets 文件夹导入组件的方法，以及库依赖、客户端渲染等注意事项，并附计数器示例，组件无需额外构建步骤。适合构建计算器、选择器等交互元素。"
