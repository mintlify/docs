...
即使自定义域名已经上线，你的部署所使用的默认 `<subdomain>.mintlify.site` URL 目前仍无法禁用或重定向。两个 URL 会提供相同的内容。请设置[规范 URL](#set-a-canonical-url)，以便搜索引擎将自定义域名作为主域名进行索引。

<div id="renaming-a-project">
  ### 重命名项目
</div>

在 dashboard 中重命名项目仅会更改显示名称，不会更改部署 URL 或默认的 `<subdomain>.mintlify.site` 主机名。

如果你重命名项目后，在默认 URL 上看到 `Disallowed origin host` 错误，说明该部署的主机名与 dashboard 期望的名称不再匹配。若要更改 URL，请删除该部署，并使用期望的名称重新创建。部署 URL 无法就地重命名。

<div id="releasing-a-domain">
  ### 释放域名
</div>

一个自定义域名同一时间只能绑定到一个 Mintlify 部署。如果你的域名仍绑定在先前的部署或组织上（例如迁移到新的 workspace 后），新的部署在旧部署释放该域名之前无法认领它。

若要释放你所控制的域名：

1. 联系[支持团队](mailto:support@mintlify.com)并请求释放该域名。
2. 支持团队会提供一条带有唯一值的验证 `TXT` 记录。
3. 在你的 DNS 提供商处添加该记录：
...
4. 记录生效后回复支持团队。支持团队会验证该记录，并将域名从先前的部署解绑。随后你可以按照[添加自定义域名](#add-your-custom-domain)将其添加到新的部署。
