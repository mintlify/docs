...
你可以使用 `docs.json` 中的 `integrations.customScripts` 字段在每个页面上加载外部脚本。每个条目需要一个 HTTPS `src` URL，并接受一个可选的 `strategy` 来控制脚本的加载时机。

| 策略 | 描述 |
|---|---|
| `beforeInteractive` | 在页面变为可交互之前将脚本注入到 `<head>` 中。适用于必须在任何页面内容加载之前运行的脚本。 |
| `afterInteractive` | 在页面变为可交互之后加载脚本。这是默认行为。 |
| `lazyOnload` | 延迟加载直到浏览器空闲。 |
...
最多可以添加 20 个外部脚本。`src` 必须是 HTTPS URL，且不能指向 `.svg`、`.exe` 或 `.zip` 文件。
