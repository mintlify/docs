...
通过 `docs.json` 中的 `integrations.customScripts` 配置，在文档的每个页面上从 URL 加载外部脚本。这对于添加托管在 CDN 上的第三方分析或追踪脚本非常有用。

<Note>
  外部脚本需要为你的项目启用 custom scripts 权限。如需访问权限，请联系支持团队。
</Note>
...
在 `docs.json` 的 `integrations` 中添加 `customScripts` 数组：
...
<ResponseField name="src" type="string" required>
  要加载的脚本的 HTTPS URL。必须指向有效的 JavaScript 文件。
</ResponseField>

<ResponseField name="strategy" type="string" default="afterInteractive">
  控制脚本的加载时机。接受 `afterInteractive`（页面可交互后加载）或 `lazyOnload`（空闲时加载）。
</ResponseField>

最多可以添加 20 个外部脚本。

<Warning>
  只加载来自可信来源的脚本。外部脚本可以完全访问页面，可能影响性能和安全性。
</Warning>
