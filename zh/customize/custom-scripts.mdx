...
你可以通过在 `docs.json` 文件的 `integrations` 对象中添加 `customScripts` 来从 URL 加载外部 JavaScript。每个条目需要一个 HTTPS `src` URL，并可选择 `strategy` 来控制脚本的加载时机。
...
<ResponseField name="src" type="string" required>
  要加载的脚本的 HTTPS URL。不允许指向 `.svg`、`.exe` 或 `.zip` 文件的 URL。
</ResponseField>

<ResponseField name="strategy" type="string">
  控制脚本的加载时机。选项：
  - `afterInteractive` — 在页面变为可交互后加载脚本。这是默认值。
  - `lazyOnload` — 在所有页面内容加载完成后的空闲时间加载脚本。
</ResponseField>
