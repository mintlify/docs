---
title: "自定义脚本"
description: "为你的文档站点添加自定义 JavaScript 和 CSS，用于分析、小组件、样式覆盖、第三方集成和 API Playground 服务器变量。"
...
当这些值不再适用时调用 `window.mintlify.api.playground.clearServerVariables()`，例如注销后。清除后，playground 会回退到其他已配置的值。
