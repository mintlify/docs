...
<AccordionGroup>
  ...
  <Accordion title="选择器">
    ...
    **数据属性**
    ...
    - TableOfContentsItem（最深层）: `toc-item[data-active-deepest]` — 仅出现在当前可见的确切标题上，与 `data-active` 不同，后者也会设置在其父级标题上
...
使用 `docs.json` 中的 `customScripts` 选项从外部 URL 加载第三方脚本。这对于添加分析、聊天小组件或其他提供托管脚本的服务非常有用。

每个条目需要一个 `src` URL（必须是 HTTPS）并接受可选的 `strategy` 来控制脚本的加载时机：

| 策略 | 描述 |
|---|---|
| `beforeInteractive` | 在页面变为可交互之前加载。用于必须在任何用户交互之前运行的脚本，例如 Cookie 同意或关键分析。 |
| `afterInteractive` | 在页面变为可交互之后加载。未指定策略时的默认行为。 |
| `lazyOnload` | 在页面加载后的空闲时间加载。用于低优先级脚本，如聊天小组件或非关键追踪。 |
...
<Note>
  最多可以添加 20 个自定义脚本。`src` URL 必须使用 HTTPS，且不得指向不允许的文件类型，如 `.svg`、`.exe` 或 `.zip`。
</Note>
