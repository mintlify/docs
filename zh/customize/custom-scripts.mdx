...
<AccordionGroup>
  ...
  <Accordion title="选择器">
    ...
    **数据属性**
    ...
    - TableOfContentsItem（最深层级）: `toc-item[data-active-deepest]`

    目录元素上的 `data-active` 属性会设置在当前可见的章节及其所有父级章节上。使用 `data-active-deepest` 可以仅选择最深层级的活动章节。

    目录元素还暴露了 `data-depth` 属性，表示其嵌套层级，从 `0` 开始表示顶级标题。
  </Accordion>
</AccordionGroup>
