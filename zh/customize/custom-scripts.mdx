...
<AccordionGroup>
  ...
  <Accordion title="选择器">
    ...
    **数据属性**
    ...
    - TableOfContentsItem（最深层）: `toc-item[data-active-deepest]`

    使用 `data-active-deepest` 定位目录中最具体的激活章节。当你滚动到 `h2` 内的 `h3` 时，两个目录项都有 `data-active`，但只有 `h3` 项有 `data-active-deepest`。
