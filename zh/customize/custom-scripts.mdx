...
有三个组件不接受 `className`：[Banner](/zh/components/banner)、[MDX](/zh/components/mdx) 和 [Visibility](/zh/components/visibility)。
...
<Note>
  Web 编辑器的实时预览不会为页面特有的 Tailwind 类生成 CSS，因此已设置样式的页面在编辑时可能看起来没有样式。请参阅 [Tailwind 类在编辑器实时预览中不生效](/zh/help-center/tailwind-classes-not-applying-in-editor-preview)。
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="顶栏导航">
    - `nav-logo`—导航栏中的 logo 图片。要覆盖其默认的尺寸工具类，请选中 `img.nav-logo` 并使用 `!important`。
  ...
  <Accordion title="数据属性">
    ...
    - `[data-component-part="card-content-container"]`—卡片内容的内部包装器。
    - `[data-component-part="card-icon"]`—卡片内的图标。请使用它来设置卡片图标的样式，因为 `icon` 元素选择器无法匹配卡片内的图标。
    - `[data-component-part="card-title"]`—卡片内的标题文本。
    - `[data-component-part="card-content"]`—卡片内的描述内容。
    - `[data-component-part="card-image"]`—显示在卡片顶部的图片。
    - `[data-component-part="card-cta"]`—卡片内的号召性用语标签。
