...
在文档站点的每个页面上加载第三方外部脚本。这对于添加分析、聊天小组件、同意管理器或其他提供托管脚本的服务非常有用。

你可以在[控制面板](https://dashboard.mintlify.com)的 **Settings > Add-ons > Custom scripts** 中管理外部脚本。为每个脚本提供 HTTPS URL 并选择加载策略：
...
<Note>
  最多可以添加 20 个自定义脚本。每个 URL 必须使用 HTTPS，且不得指向不允许的文件类型，如 `.svg`、`.exe` 或 `.zip`。
</Note>
