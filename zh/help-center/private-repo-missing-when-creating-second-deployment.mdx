---
title: "创建第二个项目时私有仓库未出现"
description: "重新授权 Mintlify 以获得私有仓库访问权限，使现有的私有 GitHub 仓库出现在新项目的引导仓库选择器中。"
keywords: ["第二个项目", "同一仓库多个项目", "第二个部署", "同一仓库多个部署", "私有仓库未出现", "onboarding 仓库选择器", "授权 GitHub", "private=true", "重新授权 GitHub"]
---

当你从同一个已经支撑现有项目的私有 GitHub 仓库创建第二个 Mintlify 项目时，引导流程中的仓库选择器可能不会列出该私有仓库。这是因为你最初的 GitHub 授权只授予了 `public_repo` 权限，而重新进入引导流程会复用已有授权，而不会再次请求私有仓库访问权限。

你可以从同一个仓库创建多个独立的项目。Mintlify 并不要求每个项目使用一个新仓库。如果需要将多个仓库合并到一个站点，请参考 [Multi-repo 设置](/zh/deploy/multi-repo)。
