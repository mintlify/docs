---
title: "agent 被 Git 提供商 IP 允许列表阻止"
description: "通过将项目路由到 Mintlify 静态 IP，解决因 GitHub 或 GitLab IP 允许列表导致的 agent 会话失败。"
...
1. 发送邮件至 <a href="mailto:support@mintlify.com">support@mintlify.com</a>，请求将你的项目路由到 Mintlify 静态 IP。请附上你的项目子域名。
2. Mintlify 支持团队确认更改后，在 GitHub 或 GitLab 组织设置的 IP 允许列表中添加 `54.242.90.151`。
3. 重新运行 agent。

只有你项目的 git 流量会通过静态 IP 路由。其他项目不受影响。
