---
title: "GitHub"
description: "将你的 GitHub 仓库连接到 Mintlify，实现自动部署、拉取请求预览构建和持续文档同步。"
...
<Tip>
  **你是否需要安装 GitHub 应用？**
  ...
  在控制台的 [Git Settings](https://app.mintlify.com/settings/deployment/git-settings) 页面查看你的存储库。
</Tip>
...
如果你在入门引导过程中跳过了连接自己的 Git 存储库，你的文档目前位于 Mintlify 组织拥有的私有存储库中。若要将其迁移到你自己的账号或组织，请前往控制台中的 [Git Settings](https://app.mintlify.com/settings/deployment/git-settings)。设置向导将引导你完成整个过程，提供两种选项：
...
<AccordionGroup>
  <Accordion title="一键克隆（推荐）">
    ...
    2. 点击 **Clone**。
    3. 在提示时授权 GitHub。
    4. 选择要创建存储库的 GitHub 组织。
    5. 确认克隆。Mintlify 会将你的文档文件复制到一个新的存储库中。
    6. 可选择安装 Mintlify GitHub 应用以启用自动部署。
  </Accordion>
  <Accordion title="手动设置">
    <Warning>
      此过程会永久删除你在 Mintlify 托管的存储库中的内容。
...
在控制台的 [Git settings](https://app.mintlify.com/settings/deployment/git-settings) 页面安装 Mintlify GitHub 应用。当 GitHub 询问要授予哪些存储库的访问权限时，请选择 **Only select repositories**，然后选择你的文档存储库。
...
安装 GitHub 应用时，你可以授予对所有或特定存储库的访问权限。请仅为你的文档存储库，以及任何你希望作为 agent 或工作流上下文提供的存储库授予访问权限。你可以随时在 [GitHub 应用设置](https://github.com/apps/mintlify/installations/new) 中修改此选择。

<div id="configure-docs-source">
  ## 配置文档来源
</div>

在控制台的 [Git Settings](https://app.mintlify.com/settings/deployment/git-settings) 部分，您可以更改用于构建文档的组织、存储库或 branch。
...
<AccordionGroup>
  <Accordion title="验证 GitHub 应用安装状态">
    ...
    1. 在控制台中前往 [Git settings](https://app.mintlify.com/settings/deployment/git-settings) 页面。
    1. 确认你的存储库出现在已启用的应用安装列表中。
  </Accordion>
  <Accordion title="检查部署用分支">
    确保你正在推送到正确的 branch。

    1. 前往 [Git 设置](https://app.mintlify.com/settings/deployment/git-settings)
...
<Steps>
  ...
  <Step title="重新安装 Mintlify 应用">
    1. 在你的 Mintlify 控制台中，前往 [Git Settings](https://app.mintlify.com/settings/deployment/git-settings) 并安装 GitHub 应用。
    2. 在控制台的 [My Profile](https://app.mintlify.com/settings/account) 中授权你的账户。
  </Step>
</Steps>
