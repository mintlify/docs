...
如果你在入门引导过程中跳过了连接自己的 Git 存储库，你的文档目前位于 Mintlify 组织拥有的私有存储库中。若要将其迁移到你自己的账号或组织，请前往控制台中的 [Git Settings](https://dashboard.mintlify.com/settings/deployment/git-settings)。设置向导会引导你完成此过程，提供两种选项：

<AccordionGroup>
  <Accordion title="一键克隆（推荐）">
    克隆选项会自动在你的 GitHub 账号中创建一份文档副本。

    1. 选择 **GitHub** 作为提供商，然后选择 **Clone**。
    2. 在出现提示时授权 GitHub。
    3. 选择你想要创建存储库的 GitHub 组织。
    4. 确认克隆。Mintlify 会将你的文档文件复制到一个新存储库中。
    5. 可选择安装 Mintlify GitHub 应用以实现自动部署。
  </Accordion>
  <Accordion title="手动设置">
    <Warning>
      此过程会永久删除你在 Mintlify 托管存储库中的内容。

      在完成手动设置流程之前，请先从设置向导中下载你的文档。
    </Warning>
    
    如果你更喜欢手动设置存储库：

    1. 将你的文档下载为 zip 文件，以便备份你的文件。
    2. 选择 **GitHub** 作为提供商，然后选择 **Continue setup**。
    3. 在出现提示时授权 GitHub。
    4. 选择你的组织、存储库和分支。
    5. 如果你的文档不在存储库根目录，可选择指定子目录。
    6. 保存你的设置。

  </Accordion>
</AccordionGroup>

完成任一选项后，请按照[安装 GitHub 应用](#install-the-github-app)中的步骤安装 GitHub 应用。
