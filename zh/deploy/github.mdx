...
3. 按照下文 [Install These GitHub App](#install-These-github-app) 中的步骤安装 GitHub 应用。

<div id="alternative-download-and-create-your-own-repository">
  ### 替代方案：下载并创建你自己的存储库
</div>

如果你想要一个私有存储库且不想授予更高等级的 OAuth 权限，可以手动迁移你的文档：

1. 在控制台中前往 [Git Settings](https://dashboard.mintlify.com/settings/deployment/git-settings)。
2. 将你的文档下载为 zip 文件。
3. 在 GitHub 或 GitLab 上创建一个新的存储库。
4. 解压 zip 文件中的内容并将其推送到你的新存储库。
5. 返回 [Git Settings](https://dashboard.mintlify.com/settings/deployment/git-settings) 并将你的新存储库连接到 Mintlify。
6. 按照下方 [Install These GitHub App](#install-These-github-app) 中的步骤安装 GitHub 应用。
