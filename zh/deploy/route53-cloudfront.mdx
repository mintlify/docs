...
3. 在 Origin domain 中输入 `[SUBDOMAIN].mintlify.cc`，其中 `[SUBDOMAIN]` 是你项目的唯一子域。

<Frame>
    ![CloudFront “Create distribution” 页面显示 “acme.mintlify.cc” 作为 Origin domain。](/images/cloudfront/origin-name.png)
  </Frame>
...
- **如果用户访问你的自定义子路径**，跳转到 `[SUBDOMAIN].mintlify.cc`。
...
创建一个行为，将 **Path pattern** 设置为你选择的子路径，例如 `/docs`，并将 **Origin and origin groups** 指向 `.mintlify.cc` 的 URL（在我们的示例中为 `acme.mintlify.cc`）。
...
<Frame>
  ![CloudFront 的 "Create behavior" 页面，其中 "Path pattern" 设置为 "/docs/*"，并且 "Origin and origin groups" 指向 `acme.mintlify.cc` 的 URL。](/images/cloudfront/behavior-1.png)
</Frame>
...
创建一个行为，在 **Path pattern** 中填写你选择的子路径并在后面添加 `/*`，例如 `/docs/*`，并将 **Origin and origin groups** 指向相同的 `.mintlify.cc` URL。
