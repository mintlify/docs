...
在创建 Route 53 记录之前，请先将你的域名添加到该分配。在分配的 “General” 标签页中，将你的域名（例如 `yoursite.com`）添加为 **Alternate domain name (CNAME)**，并附加一个覆盖该域名的 AWS Certificate Manager (ACM) 证书。如果没有备用域名，CloudFront 会拒绝通过你的域名到达的请求。
