...
sidebarTitle: "AWS"
description: "通过组合 Route 53 DNS 路由、CloudFront 分发和缓存行为，将你的 Mintlify 文档部署到 AWS 子路径。"
...
将流量路由到以下路径，并将缓存策略（Cache Policy）设置为 **CachingOptimized**：
...
确认每个行为都指向正确的源站：文档、静态资源、API 和 `/.well-known/*` 路径指向 `.mintlify.site` URL，`Default (*)` 指向你的预发布环境 URL。
...
首先，将你的域名添加到 CloudFront 分配。如果没有备用域名，CloudFront 会拒绝通过你的域名到达的请求。

1. 打开分配的“General”标签页，然后在“Settings”部分选择 **Edit**。
2. 将你的域名（例如 `yoursite.com`）添加为 **Alternate domain name (CNAME)**。
3. 在“Custom SSL certificate”中，附加一个覆盖该域名的 AWS Certificate Manager (ACM) 证书。

<Warning>
  CloudFront 仅接受在 US East (N. Virginia) `us-east-1` 区域颁发的 ACM 证书。在其他区域颁发的证书不会出现在“Custom SSL certificate”菜单中。
</Warning>
...
<Note>
  本节你也可以参考 AWS 的官方指南：[将 Amazon Route 53 配置为将流量路由到 CloudFront 分配](https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/routing-to-cloudfront-distribution.html#routing-to-cloudfront-distribution-config)，其中涵盖了这些前提条件。
</Note>
