...
如果你的文档站点在几秒钟后出现 500 错误，或者页面之间导航速度缓慢，可能是 Cloudflare 的防火墙拦截了对 Mintlify 资源的请求。

<div id="symptoms">
  ### 症状
</div>

- 文档页面最初可以加载，但在 30-60 秒后崩溃并显示 500 错误。
- 页面之间的客户端导航缓慢或失效。
- 浏览器控制台中出现对 `/mintlify-assets/*` 路径请求的 403 错误。
- Cloudflare 安全挑战消息提示"malformed data"或"suspicious URL patterns"。
...
创建一条 Cloudflare 防火墙规则，将 Mintlify 资源从安全检查中豁免。
...
2. 让页面闲置 2-3 分钟。
3. 在页面之间导航。
4. 检查浏览器控制台是否有任何 403 错误。

如果问题仍然存在，请核对你的规则配置：

- 确保主机名与你的文档域名完全一致。
- 确认 URI Path 使用 `starts with`（而不是 `contains`）。
- 不要在 path 值中包含通配符（`*`）。
- 确认你已启用并部署了该规则。
...
- 对 `/mintlify-assets/*` 使用 `contains` 运算符。`*` 会被视为字面字符，而不是通配符。
- 对 URI Path 使用 `equals`。这样只会匹配确切路径 `/mintlify-assets/`，而不匹配子路径。
- 忘记跳过 Bot Fight Mode。请在 skip 操作中显式包含它。
- 主机名设置错误。它必须与你实际的文档域名一致。
...
1. 检查 Cloudflare 的 **Security > Events** 日志中被拦截的请求。
2. 确认你的 Cloudflare Worker（如果使用自定义子路径）将 `Host` 头设置为你的 `<subdomain>.mintlify.site` 目标，而不是直接透传原始请求的 `Host` 头。
3. 临时将 Security Level 设置为 "Essentially Off"，以确认 Cloudflare 是否为问题根源。
4. 检查任何可能覆盖防火墙例外的自定义 Page Rules。
