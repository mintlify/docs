...
title: "内容安全策略（CSP）配置"
sidebarTitle: "CSP 配置"
description: "配置 CSP 标头，在严格安全策略（如反向代理、防火墙和受控网络）下允许 Mintlify 资源，同时保持整体安全性。"
---

内容安全策略（CSP）是一项安全标准，通过限制网页可加载的资源来帮助防止跨站脚本（XSS）攻击。Mintlify 提供了默认的 CSP，能保护大多数站点。若你的文档托管在会覆盖默认 CSP 的反向代理或防火墙之后，可能需要配置 CSP 标头，才能确保功能正常运行。
