...
title: "内容安全策略（CSP）配置"
sidebarTitle: "CSP 配置"
description: "内容安全策略（CSP）通过控制页面可加载的资源帮助防止 XSS 攻击。本文列出 script-src、connect-src 等各项 CSP 指令的作用、需要加入允许列表的域名及其用途，帮助你在反向代理或防火墙后托管 Mintlify 文档时正确配置 CSP 标头并保持功能正常。"
