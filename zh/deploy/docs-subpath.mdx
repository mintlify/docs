...
如果你的域名不承载其他内容，Mintlify 可以无需代理即可在你的子路径下提供文档服务。在控制台的 [Custom domain setup](https://app.mintlify.com/settings/project/custom-domain) 页面上，启用 **Host at** 开关，输入你的基础路径并添加你的域名。然后按照控制台中显示的 DNS 记录配置，将你的域名指向 Mintlify。有关设置说明，请参见 [自定义域名](/zh/customize/custom-domain)。
...
<Note>
  当由你自己的代理为子路径提供服务时，请勿添加 [Custom domain setup](https://app.mintlify.com/settings/deployment/custom-domain) 页面上显示的 `CNAME` 记录。该记录会将该域名的所有流量路由到 Mintlify，并绕过你的代理。如果你的代理运行在 Cloudflare 上，验证 `TXT` 记录也会保持未验证状态。这是预期行为，不会影响你的文档。参见 [_cf-custom-hostname TXT 记录一直未验证](/zh/help-center/cf-custom-hostname-txt-record-stuck-unverified)。
</Note>
