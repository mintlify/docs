...
<Note>
  当由你自己的代理为子路径提供服务时，请勿添加 [Custom domain setup](https://app.mintlify.com/settings/deployment/custom-domain) 页面上显示的 DNS 记录。由于你域名的流量永远不会路由到 Mintlify，验证 `TXT` 记录会一直保持未验证状态。这是预期行为，不会影响你的文档。参见 [_cf-custom-hostname TXT 记录一直未验证](/zh/help-center/cf-custom-hostname-txt-record-stuck-unverified)。
</Note>
