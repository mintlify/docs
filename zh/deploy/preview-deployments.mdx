...
4. 输入要预览的 branch 名称。
5. 可选：启用 **Password protection** 并输入密码。访问者必须输入此密码才能查看预览。
...
默认情况下，任何持有 URL 的人都可以访问预览部署。有两种方式可以限制访问：

### 组织认证

要求所有预览查看者必须是你 Mintlify 组织中的已认证成员。
...
为单个手动预览设置密码以限制访问。启用后，访问者必须输入密码才能查看预览内容。你可以在[创建手动预览](#manual-previews)时设置密码。
