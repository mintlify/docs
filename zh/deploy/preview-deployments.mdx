...
[控制台](https://app.mintlify.com/) **Previews** 标签中显示的 URL 是权威预览 URL。当你分享预览供他人审阅，或当客户反馈某个具体部署存在问题时，请使用该 URL。

Mintlify GitHub 机器人有时会在 pull request 时间线中发布 `.mintlify.app` 链接，而控制台为同一预览显示的是 `.mintlify.site` 链接。两个主机指向同一个部署，但某个预览在同一时间只会由其中一个主机提供服务，因此另一个主机在相同路径上可能返回 `404`。如果某个预览 URL 返回 `404`，请从控制台打开预览获取当前 URL，而不要手动修改主机。
...
* **`ERR_TOO_MANY_REDIRECTS` 或"危险网站"警告**：这些症状通常意味着预览是在错误的主机上加载的。请从[控制台](https://app.mintlify.com/) **Previews** 标签打开预览，以获取权威 URL（参见[预览 URL 格式](#preview-url-format)），并直接加载该 URL，而不是使用发布在 pull request 评论中的链接。