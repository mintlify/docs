...
4. 输入要预览的 branch 名称。
5. 可选：启用 **Password protection** 并输入密码以限制预览的访问权限。
...
默认情况下，任何持有 URL 的人都可以访问预览部署。你可以通过组织级认证或单独的预览密码保护来限制访问权限。

### 组织认证

要求所有预览查看者必须是 Mintlify 组织中的已认证成员。
...
你可以为单个手动预览部署设置密码。启用后，访问者必须输入密码才能查看预览内容。

要添加密码保护，请在[创建手动预览](#manual-previews)时启用 **Password protection** 开关并输入密码。密码长度至少为 8 个字符。
