...
该集成需要你的 GHES 实例与 Mintlify 之间存在双向流量。请配置你的防火墙以允许下面两种连接。

#### 出站：从你的 GHES 实例到 Mintlify

你的 GHES 实例通过 HTTPS（端口 443）将 webhook 事件发送到 Mintlify 的 `leaves.mintlify.com`。

* 允许对 `leaves.mintlify.com` 的出站 HTTPS（端口 443）流量。请按主机名放行此流量。Mintlify 不会为此端点发布静态 IP 地址列表，其底层 IP 地址也可能发生变化。
* 确保你的 GHES 实例能够通过 DNS 解析 `leaves.mintlify.com`。如果你的服务器使用内部 DNS 解析器，请确认它可以解析公共主机名。

#### 入站：从 Mintlify 到你的 GHES 实例

Mintlify 通过 HTTPS（端口 443）连接到你的 GHES 实例，以调用 GitHub API、上报 check run 与部署状态，以及读取仓库内容。这些连接来自 Mintlify 的静态出站 IP 地址：`54.242.90.151`。

如果防火墙或 IP 允许列表限制了对你的 GHES 实例的访问，请允许来自 `54.242.90.151` 的入站 HTTPS（端口 443）流量。

<Note>
  [Agent](/zh/agent) 的运行会在独立的沙箱基础设施中执行 clone 与 push 操作，该基础设施默认使用动态 IP 地址。如果你的 GHES 实例启用了 IP 允许列表，请参见[修复被 Git 提供方 IP 允许列表阻止的 agent 运行](/zh/help-center/agent-blocked-by-git-provider-ip-allow-list)。
</Note>
...
* Installation owner：拥有该应用安装的 GitHub 组织名称
* 私钥：`.pem` 文件的全部内容（请通过安全文件传输方式传递）
* Webhook 密钥：（来自步骤 5）
...
* 将你的 GHES 环境与现有的 Mintlify 项目集成。
* 将你的 GHES 环境与我们为你准备的新 Mintlify 项目集成。

当你的 GHES 环境与某个 Mintlify 项目完成集成后，你就可以为 GitHub App 启用 webhook 了。
