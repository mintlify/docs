...
<Tabs>
  ...
  <Tab title="OAuth 2.0">
    ...
    * 具备创建可通过 OAuth 访问令牌访问的 API 端点的能力（可选，用于启用个性化功能）。
    ...
    <Steps>
      <Step title="配置你的 OAuth 设置。">
        ...
        * **Logout URL**：你的 OAuth 提供商的原生登出 URL。用户登出时，Mintlify 会使用 `GET` 请求将用户重定向到该 URL。Mintlify 不会追加 query 参数，因此请在 URL 中直接包含所需参数（例如 `returnTo`）。配置一个页面，在成功登出后将用户重定向至该页面。
      ...
      <Step title="创建你的用户信息端点（可选）。">
        ...
        * 按 `User` 格式返回用户数据。更多信息参见 [User data format](/zh/deploy/personalization-setup#user-data-format)。
        ...
        Mintlify 会携带 OAuth 访问令牌调用此端点以获取用户信息。不会发送任何额外的 query 参数。
    ...
    * **Logout URL**：`https://auth.foo.com/logout?returnTo=https%3A%2F%2Ffoo.com%2Fdocs`
    ...
    <Note>
      使用用户信息响应中的 `expiresAt` 字段控制会话时长。该字段是一个 Unix 时间戳（自 Unix 纪元以来的秒数），用于指示会话应何时过期。更多详情参见 [用户数据格式](/zh/deploy/personalization-setup#user-data-format)。
    </Note>
