...
<Tabs>
  ...
  <Tab title="JWT（JSON Web Token）">
    ...
    * 一个可以创建重定向 URL 的后端服务。
    ...
    如果你的组织有多个身份提供商或按地区区分的登录流程，可以为使用 JWT 认证的站点配置 2 到 10 个具名登录 URL。当配置了多个 URL 时，未认证的访客会看到一个使用你站点品牌样式的登录选择页面，每个选项都会重定向到你所配置的 URL。

    要配置多个登录 URL，请在更新 JWT 认证设置时提供一个 `{ name, url }` 键值对列表。每个 `name` 会显示在选择页面上，长度必须在 1 到 100 个字符之间。每个 `url` 必须是有效的 URL。当只配置一个登录 URL 时，访客仍会像以前一样直接重定向到该 URL。
    ...
    多个登录 URL 仅适用于站点范围的 JWT 认证。JWT [个性化](/zh/create/personalization) 只接受单个 `loginUrl`。
