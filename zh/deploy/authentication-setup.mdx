...
<Tabs>
  ...
  <Tab title="OAuth 2.0">
    ...
    * `groupsDelimiter`：可选的分隔符，长度为 1 至 4 个字符。Mintlify 仅使用它来拆分字符串类型的声明值。
    ...
    例如，当 `"groups": "general,clienta_eur"` 且 `groupsDelimiter` 设置为 `","` 时，Mintlify 会将 `general` 和 `clienta_eur` 作为两个独立的用户组。Mintlify 会删除每个用户组两侧的空格，并忽略空片段。不设置 `groupsDelimiter` 时，整个字符串会被视为一个用户组。数组类型的声明始终将每个字符串元素视为一个用户组，不会进行拆分。
