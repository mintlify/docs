...
启用认证后，部分功能的行为会有所不同，或可能不可用。Mintlify 不支持在需要认证的站点上公开提供任意文件。所有托管文件（包括 `skill.md`）都受与你的文档页面相同的认证要求约束。

| 功能                                                       | 公开   | 完全认证 (所有页面受保护)                 | 部分认证 (部分页面公开)                  |
| :------------------------------------------------------- | :--- | :----------------------------- | :----------------------------- |
| [llms.txt](/zh/ai/llmstxt)                  | 完全支持 | 需要通过认证后才能访问。会排除分组中的页面以及 OpenAPI 或 AsyncAPI 规范 | 可公开访问。仅包含公开页面，并排除 OpenAPI 或 AsyncAPI 规范 |
| [llms-full.txt](/zh/ai/llmstxt#llms-full-txt) | 完全支持 | 不支持 | 可公开访问。仅包含公开页面的内容 |
