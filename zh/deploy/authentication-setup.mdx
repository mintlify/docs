---
title: "认证设置"
description: "为你的站点配置用户认证，以控制对页面和 API 参考的访问权限。将页面设为仅对你的 Mintlify 组织可见，或使用密码、OAuth 或 JWT 认证。"
...
在将 URL 分享给用户之前，请先验证认证是否按预期工作：

- **Preview deployments**：[Preview deployments](/zh/deploy/preview-deployments) 使用与生产站点相同的认证设置。推送一个 branch 并打开预览 URL，即可确认登录流程能端到端正常工作。
- **使用真实账号测试**：使用测试用户或每个应有访问权限的用户组中的成员登录，确认受限页面按预期显示或隐藏。
- **本地开发**：`mint dev` 在提供服务时不会强制执行认证，方便你迭代内容。请始终针对预览或生产部署（而不是本地服务器）来确认访问规则。
