    ...
    * 能够创建可通过 OAuth 访问令牌访问的 API 端点（可选，用于启用基于用户组的访问控制）。
    ...
    <Steps>
      <Step title="配置你的 OAuth 设置。">
        ...
        * **Logout URL**（可选）：你的 OAuth 提供方自带的登出 URL。用户登出时，Mintlify 会使用 `GET` 请求将用户重定向到该 URL。Mintlify 不会追加 query 参数，因此请将任何参数（例如 `returnTo`）直接包含在 URL 中。配置一个页面，用于在成功登出后重定向用户。
      ...
      <Step title="创建用户信息端点（可选）。">
        ...
        * 以 `User` 格式返回用户数据。更多信息参见 [User data format](#user-data-format)。
        ...
        将此端点 URL 填入你[认证设置](https://dashboard.mintlify.com/settings/deployment/authentication)中的 **Info API URL** 字段。
      </Step>
    </Steps>
