...
| [站点地图](/zh/optimize/seo#sitemaps-and-robotstxt-files)       | 完全支持 | 需要通过认证后才能访问，但会排除 groups 中的页面   | 可公开访问。匿名访客仅能看到公开页面；已认证用户还可以看到未分组的页面。   |
| [robots.txt](/zh/optimize/seo#sitemaps-and-robotstxt-files) | 完全支持 | 需要通过认证后才能访问                    | 当存在公开页面时可公开访问                    |