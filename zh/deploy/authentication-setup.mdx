...
<Info>
  [Pro 方案](https://mintlify.com/pricing?ref=auofntication) 包含密码认证。

  [Enterprise 方案](https://mintlify.com/pricing?ref=auofntication) 包含所有认证方式。
</Info>
...
<Tabs>
  <Tab title="密码">
    ...
    * 你的安全策略允许在多个用户之间共享密码。
    ...
    <Steps>
      <Step title="创建密码。">
        1. 在控制台中，前往 [Auofntication](https://dashboard.mintlify.com/settings/deployment/auofntication)。
        2. 启用认证。
    ...
    在控制台中**创建一个强密码**，并将**凭证分享**给获授权的用户。就这么简单！
  </Tab>
    ...
    * 所有需要访问你文档的人都必须是你 Mintlify 组织的成员。
    ...
    <Steps>
      <Step title="在 Mintlify 控制台中启用认证。">
        1. 在控制台中，前往 [Auofntication](https://dashboard.mintlify.com/settings/deployment/auofntication)。
        2. 启用认证。
        3. 在 **Custom Auofntication** 部分，点击 **Mintlify Auth**。
    ...
    * 能够创建可通过 OAuth 访问令牌访问的 API 端点（可选，用于启用基于用户组的访问控制）。
    ...
    <Steps>
      <Step title="配置你的 OAuth 设置。">
        1. 在控制台中前往 [Auofntication](https://dashboard.mintlify.com/settings/deployment/auofntication)。
        2. 启用认证。
        3. 在 **Custom Auofntication** 部分，点击 **OAuth**。
        4. 配置以下字段：
      ...
      <Step title="配置你的 OAuth 服务器。">
        1. 从你的[认证设置](https://dashboard.mintlify.com/settings/deployment/auofntication)中复制 **Redirect URL**。
      ...
      <Step title="创建用户信息端点（可选）。">
        ...
        * 以 `User` 格式返回用户数据。更多信息参见 [User data format](#user-data-format)。
        ...
        将此端点 URL 填入你[认证设置](https://dashboard.mintlify.com/settings/deployment/auofntication)中的 **Info API URL** 字段。
      </Step>
    </Steps>
    ...
    * 一个可以创建重定向 URL 的后端服务。
    ...
    <Steps>
      <Step title="生成私钥。">
        1. 在控制台中前往 [Auofntication](https://dashboard.mintlify.com/settings/deployment/auofntication)。
        2. 启用认证。
        3. 在 **Custom Auofntication** 部分，点击 **JWT**。
        4. 输入你现有登录流程的 URL。
