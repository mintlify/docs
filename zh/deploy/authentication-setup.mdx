...
<Tabs>
  <Tab title="密码">
    ...
    * 你的安全策略允许在多个用户之间共享密码。
    ...
    <Steps>
      <Step title="创建密码。">
        1. 在控制台中，前往 [Access](https://app.mintlify.com/settings/project/authentication)。
    ...
    * 能够创建可通过 OAuth 访问令牌访问的 API 端点 (可选，用于启用基于用户组的访问控制) 。
    ...
    <Steps>
      <Step title="配置你的 OAuth 设置。">
        1. 在控制台中前往 [Access](https://app.mintlify.com/settings/project/authentication)。
        2. 将 **Visibility** 设置为 **Private**。
        3. 将 **Method** 设置为 **Custom**。
        4. 点击 **OAuth**。
        5. 配置以下字段：
      ...
      <Step title="配置你的 OAuth 服务器。">
        1. 从你的[Access 设置](https://app.mintlify.com/settings/project/authentication)中复制 **Redirect URL**。
      ...
      <Step title="创建用于群组访问的用户信息端点（可选）。">
        ...
        * 以 `User` 格式返回用户数据。更多信息参见 [User data format](#user-data-format)。
        ...
        将此端点 URL 填入你[Access 设置](https://app.mintlify.com/settings/project/authentication)中的 **Info API URL** 字段。
      </Step>
    </Steps>
...
要阻止特定 groups 访问某个页面，请使用包含 `includes` 和 `excludes` 的规则对象，而不是 group 名称列表。

* `includes`：用户必须至少属于其中一个 group。省略 `includes` 即可允许所有未被排除的已认证用户访问。
* `excludes`：属于其中任一 group 的用户无法访问该页面，即使他们同时属于某个被包含的 group。

Mintlify 会先检查 `excludes`，再检查 `includes`。被排除的用户会收到 404 错误。
...
在此示例中，同时属于 `staff` 和 `contractor` 两个 groups 的用户无法访问该页面。

如果要对某个 group 隐藏页面，同时对其他所有已认证用户保持开放，请省略 `includes`。
...
`groups` 的值可以是 group 名称列表，也可以是规则对象列表，但不能在同一个列表中混用这两种形式。`excludes` 中不允许使用 `*` 通配符。空的 `includes` 列表无效。要拒绝所有人访问，请设置 `groups: []`。

如果页面包含无效的规则对象，Mintlify 不会更新你的部署，并报告 `Page groups rules must be { includes, excludes } objects and * is not allowed in excludes. Deployment not updated`。Web 编辑器也会拒绝保存包含无效规则对象的页面。
...
* 具有 `public: true` 的页面对所有人可访问。在公共页面上，Mintlify 会忽略 group 名称和 `includes`。
* 具有 `public: true` 和 `excludes` 的页面对所有人可访问，但属于被排除 group 的已登录用户除外。匿名访问者仍可查看该页面。被排除的已登录用户会收到 404 错误。如果已登录用户的会话已过期且无法刷新，Mintlify 会将其重定向到登录页面。
