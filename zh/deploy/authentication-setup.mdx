...
启用认证后，部分功能的行为会有所不同，或可能不可用。除部分认证站点上的 `llms.txt` 和 `llms-full.txt` 外，Mintlify 不支持在需要认证的站点上公开提供任意文件。所有其他托管文件，包括 `skill.md`，都受与你的文档页面相同的认证要求约束。

| 功能                                                       | 公开   | 完全认证 (所有页面受保护)                                       | 部分认证 (部分页面公开)                              |
| :------------------------------------------------------- | :--- | :--------------------------------------------------- | :------------------------------------------- |
| [llms.txt](/zh/ai/llmstxt)                                  | 完全支持 | 需要通过认证后才能访问，因此 AI 工具可能无法访问该文件。会排除归属于分组的页面，并省略 OpenAPI 和 AsyncAPI 规范 | 公开可访问，但仅包含公开页面，并省略 OpenAPI 和 AsyncAPI 规范    |
| [llms-full.txt](/zh/ai/llmstxt#llms-fulltxt)                | 完全支持 | 不支持                                                  | 公开可访问，但仅包含公开页面                              |
