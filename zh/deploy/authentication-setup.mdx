...
仪表盘的站点可见性开关只提供 **Public** 或 **Private** 两个选项。部分认证并不是一种独立的模式。要设置它，先将站点可见性切换为 **Private**，让所有页面默认都需要认证。然后使用 `public: true` 将你希望对所有人开放的页面或导航分组标记出来。有关 frontmatter 和 `docs.json` 的语法，请参见[将页面设为公开](#make-pages-public)。
