...
首次打开 [Git Settings](https://dashboard.mintlify.com/settings/deployment/git-settings) 时，设置向导会引导你完成 GitLab 存储库的连接。
...
<Steps>
  ...
  <Step title="下载你的内容">
    <Tip>
      如果你已有包含文档的 GitLab 存储库，可以跳过下载，直接点击 **Continue setup**。
    </Tip>
  ...
  <Step title="连接你的存储库">
    ...
    - **GitLab instance URL**：如果使用 `gitlab.com` 则留空，或输入你的自托管实例 URL（例如 `https://gitlab.your-domain.com`）。你的实例必须可公开访问，以便 Mintlify 能够连接。
...
要在初始设置后修改 GitLab 连接设置，请前往 [Git Settings](https://dashboard.mintlify.com/settings/deployment/git-settings)，直接更新你的项目 ID、访问令牌、分支或实例 URL。
