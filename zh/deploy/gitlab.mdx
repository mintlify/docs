...
<Steps>
  ...
  <Step title="设置连接">
    ...
    1. 输入你的项目 ID 和访问令牌。
    2. 选择你的部署分支。
    3. 如果你的文档不在仓库根目录，请启用 **Set up as monorepo** 并输入包含 `docs.json` 的目录路径。
    4. 如果你使用的是自托管 GitLab 实例，请启用 **Set up as self-hosted** 并输入你的 GitLab 主机 URL（例如 `https://gitlab.example.com`）。
    5. 点击 **保存更改**。
    <Frame>
