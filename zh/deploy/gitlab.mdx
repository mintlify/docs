...
<Steps>
  ...
  <Step title="设置连接">
    ...
    1. 输入你的项目 ID 和访问令牌。
    2. 选择要部署的分支。
    3. 如果你的 GitLab 实例是自托管的，请启用 **Set up as self-hosted** 并输入你的主机 URL（例如 `https://gitlab.example.com`）。详情请参阅[自托管 GitLab](#自托管-gitlab)。
    4. 如果你的文档在子目录中，请启用 **Set up as monorepo** 并输入包含 `docs.json` 文件的目录路径（例如 `apps/docs`）。
    5. 点击 **保存更改**。
    <Frame>
...
如果你运行的是自托管 GitLab 实例而非 `gitlab.com`，你可以在设置过程中提供你的实例主机 URL 来将其连接到 Mintlify。

<Steps>
  <Step title="启用自托管模式">
    在 [Mintlify 控制台](https://dashboard.mintlify.com/settings/deployment/git-settings)中，在 GitLab 配置面板中启用 **Set up as self-hosted**。
  </Step>
  <Step title="输入主机 URL">
    在 **Host URL** 字段中输入你的 GitLab 实例的基础 URL。例如：
  ...
  <Step title="完成设置">
    按照[设置连接](#set-up-the-connection)中的说明输入你的项目 ID 和访问令牌，然后点击 **保存更改**。
  </Step>
</Steps>

<Note>
  使用自托管 GitLab 实例时，请确保你的实例可以从互联网访问，以便 Mintlify 能够与其通信。Webhook URL 保持不变，仍为 `https://leaves.mintlify.com/gitlab-webhook`。
</Note>