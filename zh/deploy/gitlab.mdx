...
如果你正在运行自托管的 GitLab 实例，可以通过提供实例的主机 URL 将其连接到 Mintlify。

<Steps>
  <Step title="启用自托管设置">
    在 [Mintlify 控制台](https://dashboard.mintlify.com/settings/deployment/git-settings) 的 GitLab 配置面板中，开启 **Set up as self-hosted** 选项。
  </Step>
  <Step title="输入主机 URL">
    在 **Host URL** 字段中输入你的自托管 GitLab 实例的 URL（例如 `https://gitlab.example.com`）。
  </Step>
  <Step title="完成设置">
    按照[上方的连接设置](#set-up-the-connection)中的说明，填写项目 ID、访问令牌和部署分支，然后点击 **保存更改**。
  </Step>
</Steps>

<Note>
  自托管 GitLab 实例的访问令牌和 webhook 配置步骤与标准 GitLab 相同。请确保你的实例可以从互联网访问，以便 Mintlify 能够连接。
</Note>
