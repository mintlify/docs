...
Mintlify 通过访问令牌和 webhook 在 GitLab 与 Mintlify 之间进行身份验证并同步变更。
...
<Steps>
  ...
  <Step title="设置连接">
    ...
    1. 输入你的项目 ID 和访问令牌。
    2. 如果你使用 monorepo 且文档不在仓库根目录下，请启用 **Set up as monorepo** 开关并输入文档目录的相对路径。
    3. 如果你使用自托管的 GitLab 实例，请启用 **Set up as self-hosted** 开关并输入你的 GitLab 实例的主机 URL（例如 `https://gitlab.your-domain.com`）。你的实例必须可公开访问，Mintlify 才能连接。
    4. 选择用于部署文档的分支。
    5. 点击 **保存更改**。
    <Frame>
