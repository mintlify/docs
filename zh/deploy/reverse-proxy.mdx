...
Mintlify 根据你的子路径需求支持两种反向代理配置。两种方式都指向 `<your-subdomain>.mintlify.site` 主机名。
...
- **自定义子路径**：当你需要 `/docs` 以外的子路径（例如 `/help` 或 `/resources`）时使用。这种方式需要额外的路由规则。
...
<Warning>
  当你启用 **Host at `/docs`** 时，你的文档规范 URL 将变为 `<your-subdomain>.mintlify.site`。请将代理指向该主机名，以便缓存失效和更新能够到达你的站点。
</Warning>
...
**原因**：你的反向代理指向的主机名与 Mintlify 规范子域名不一致，因此缓存失效无法到达你的站点。
