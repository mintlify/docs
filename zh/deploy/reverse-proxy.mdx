...
Mintlify 支持两种反向代理配置，可根据你的子路径需求进行选择。

- **在 `/docs` 下托管**：使用 `mintlify.dev` 作为代理目标。在控制台的 [自定义 domain 设置](https://dashboard.mintlify.com/settings/deployment/custom-domain) 页面中开启 **在 `/docs` 下托管** 开关。这是一种更简单的配置，涉及的路由更少。
...
当你希望在 domain 的 `/docs` 路径下托管文档时，使用此配置。
...
<Warning>
  启用 **Host at `/docs`** 会停止对 `mintlify.app` 的缓存失效。你必须将代理指向 `mintlify.dev`，更新才会生效。
</Warning>
...
`/.well-known/skills/*` 和 `/skill.md` 路由是可选的。仅当你希望在类似 `your-domain.com/skills.md` 这样的根路径提供 AI 技能文件，而不是在 `your-domain.com/docs/skills.md` 这样的文档子路径下时，才需要包含它们。
...
**现象**：你发布了文档更新，但这些更改没有在你的网站上显示出来。

**原因**：你在控制台中启用了 **Host at `/docs`**，但反向代理指向的是 `mintlify.app` 而不是 `mintlify.dev`。

**解决方法**：将反向代理配置更新为指向 `<your-subdomain>.mintlify.dev`，而不是 `<your-subdomain>.mintlify.app`。
