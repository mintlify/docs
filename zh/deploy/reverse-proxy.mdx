...
- **在 `/docs` 下托管**：使用 `mintlify.dev` 作为反向代理目标。在控制台的 [自定义 domain 设置](https://dashboard.mintlify.com/settings/deployment/custom-domain) 页面中启用 **在 `/docs` 下托管** 开关选项。这是一种更简单、路由更少的配置。
- **自定义子路径**：使用 `mintlify.app` 作为反向代理目标。这种方式支持任意子路径，并且需要额外的路由规则。

<div id="host-at-docs-subpath">
  ## 在 `/docs` 子路径下托管
</div>

当你想在自己的 domain 的 `/docs` 路径下提供文档内容时，请使用此配置。
...
<Warning>
  启用 **Host at `/docs`** 后，将不再对 `mintlify.app` 执行缓存失效处理。你必须代理到 `mintlify.dev`，更新才会生效。
</Warning>
...
`/.well-known/skills/*` 和 `/skill.md` 路由是可选的。只有当你希望在根路径（例如 `your-domain.com/skills.md`）而不是文档子路径（例如 `your-domain.com/docs/skills.md`）下提供 AI skills 文件时，才需要包含它们。
...
- 停止转发 `Host` 头
- 将 `Origin` 头设置为你的 Mintlify 子域（`/docs` 子路径使用 `mintlify.dev`，其他子路径使用 `mintlify.app`）
