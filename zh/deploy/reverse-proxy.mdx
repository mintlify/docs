...
- **在 `/docs` 下托管**：使用 `mintlify.dev` 作为代理目标。在控制台的 [自定义 domain 设置](https://dashboard.mintlify.com/settings/deployment/custom-domain) 页面中启用 **在 `/docs` 下托管** 开关。这是一种路由更少、配置更简单的方案。
...
当你希望在 domain 的 `/docs` 路径下访问文档时，请使用此配置。

在配置反向代理之前：

1. 前往控制台中的 [自定义 domain 设置](https://dashboard.mintlify.com/settings/deployment/custom-domain)。
2. 启用 **Host at `/docs`** 开关。
3. 输入你的 domain 并选择 **Add domain**。

<Warning>
  启用 **Host at `/docs`** 会停止在 `mintlify.app` 上的缓存失效机制。你必须将流量代理到 `mintlify.dev` 才能使更新生效。
</Warning>
...
`/.well-known/skills/*` 和 `/skill.md` 这两个路由是可选的。只有当你希望在站点根路径（如 `your-domain.com/skills.md`）而不是在文档子路径下（如 `your-domain.com/docs/skills.md`）提供 AI 技能文件时，才需要包含它们。
...
**症状**：你发布了文档更新，但网站上没有显示这些更改。

**原因**：你在控制台中启用了 **Host at `/docs`**，但你的反向代理指向的是 `mintlify.app` 而不是 `mintlify.dev`。

**解决方法**：更新你的反向代理配置，使其指向 `<your-subdomain>.mintlify.dev`，而不是 `<your-subdomain>.mintlify.app`。
...
- 停止转发 `Host` 头
- 将 `Origin` 头设置为你的 Mintlify 子域（对于 `/docs` 子路径使用 `mintlify.dev`，对于其他子路径使用 `mintlify.app`）
