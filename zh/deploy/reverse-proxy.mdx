...
- **在 `/docs` 下托管**：使用 `mintlify.site` 作为代理目标。在控制台的 [Custom domain setup](https://dashboard.mintlify.com/settings/deployment/custom-domain) 页面中开启 **Host at `/docs`** 开关。这种方式路由更少，配置更简单。
- **自定义子路径**：使用 `mintlify.site` 作为代理目标。这种方式支持任意子路径，但需要额外的路由规则。
...
<Warning>
  当你启用 **Host at `/docs`** 时，你的文档规范 URL 将变为 `<your-subdomain>.mintlify.site`。在 `mintlify.site` 上的缓存失效将停止，你必须将代理指向 `mintlify.site`，更新才会生效。
</Warning>
...
**原因**：你在控制台中启用了 **Host at `/docs`**，但你的反向代理指向的是 `mintlify.site` 而不是 `mintlify.site`。

**解决方案**：将反向代理配置更新为指向 `<your-subdomain>.mintlify.site`，而不是 `<your-subdomain>.mintlify.site`。
...
- 停止转发 `Host` 头
- 将 `Origin` 头设置为你的 Mintlify 子域（对于 `/docs` 子路径使用 `mintlify.site`，其他子路径使用 `mintlify.site`）
