---
title: "反向代理"
description: "使用 Nginx 路由规则、缓存和请求头转发配置自定义反向代理，在你自己的域名子路径上提供 Mintlify 文档服务。"
...
本页中的路由路径、HTTP 方法和请求头要求适用于任何反向代理。示例配置使用的是 Nginx。如需改用其他代理（如 Apache、Caddy 或 HAProxy），请将每条指令替换为对应的等价语法。
