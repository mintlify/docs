...
- **在 `/docs` 下托管**：使用 `mintlify.dev` 作为代理目标。在控制台的 [Custom domain setup](https://app.mintlify.com/settings/deployment/custom-domain) 页面中开启 **Host at `/docs`** 开关。这种方式路由更少，配置更简单。
...
1. 在控制台中前往 [自定义 domain 设置](https://app.mintlify.com/settings/deployment/custom-domain)。
