...
Mintlify 支持两种反向代理配置，具体取决于你的子路径需求。

- **在 `/docs` 下托管**：使用 `mintlify.dev` 作为代理目标。在控制台的 [自定义 domain 设置](https://dashboard.mintlify.com/settings/deployment/custom-domain) 页面中启用 **在 `/docs` 下托管** 开关。此配置更简单，路由更少。
- **自定义子路径**：使用 `mintlify.app` 作为代理目标。此方式支持任意子路径，但需要额外的路由规则。

<div id="host-at-docs-subpath">
  ## 在 `/docs` 子路径下托管
</div>

当你希望在你的 domain 的 `/docs` 路径下提供文档时，请使用此配置。
...
`/.well-known/skills/*` 和 `/skill.md` 路由是可选的。只有在你希望在类似 `your-domain.com/skills.md` 这样的根路径下提供 AI 技能文件，而不是在类似 `your-domain.com/docs/skills.md` 这样的文档子路径下提供时，才需要配置它们。
