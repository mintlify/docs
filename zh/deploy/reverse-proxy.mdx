...
Mintlify 支持两种反向代理配置方式，具体取决于你的子路径需求。

- **在 `/docs` 下托管**：使用 `mintlify.dev` 作为代理目标。在控制台的 [自定义 domain 设置](https://dashboard.mintlify.com/settings/deployment/custom-domain) 页面中启用 **Host at `/docs`** 开关。这种配置更简单，所需路由规则更少。
- **自定义子路径**：使用 `mintlify.app` 作为代理目标。这种方式支持任意子路径，并且需要额外的路由规则。

<div id="host-at-docs-subpath">
  ## 在 `/docs` 子路径托管
</div>

当你希望在 domain 的 `/docs` 路径下提供文档时，请使用此配置。

在配置反向代理之前：

1. 在控制台中进入 [自定义 domain 设置](https://dashboard.mintlify.com/settings/deployment/custom-domain)。
2. 启用 **Host at `/docs`** 开关。
3. 输入你的 domain 并选择 **Add domain**。

<Warning>
  启用 **Host at `/docs`** 会停止 `mintlify.app` 上的缓存失效机制。你必须将请求代理到 `mintlify.dev`，更新才会生效。
</Warning>
...
`/.well-known/skills/*` 和 `/skill.md` 路由是可选的。仅当你希望在根路径（例如 `your-domain.com/skills.md`）而不是在文档子路径下（例如 `your-domain.com/docs/skills.md`）提供 AI 技能文件时，才需要包含它们。
...
当你需要使用除 `/docs` 之外的其他子路径（例如 `/help` 或 `/resources`）时，请使用以下路由配置。
...
**症状**：你发布了文档更新，但这些更改没有在站点上显示。

**原因**：你在控制台中启用了 **Host at `/docs`**，但你的反向代理指向的是 `mintlify.app` 而不是 `mintlify.dev`。

**解决方案**：更新你的反向代理配置，使其指向 `<your-subdomain>.mintlify.dev`，而不是 `<your-subdomain>.mintlify.app`。
