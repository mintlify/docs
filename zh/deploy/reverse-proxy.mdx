...
Mintlify 提供两种反向代理配置，以适应不同的子路径需求。

- **在 `/docs` 下托管**：使用 `mintlify.dev` 作为代理目标。在控制台的 [自定义 domain 设置](https://dashboard.mintlify.com/settings/deployment/custom-domain) 页面中启用 **在 `/docs` 下托管** 选项。此配置更简单，路由更少。
...
当你希望通过你的 domain 的 `/docs` 路径提供文档时，使用此配置。
...
<Warning>
  启用 **Host at `/docs`** 会停止对 `mintlify.app` 的缓存失效处理。你必须将请求代理到 `mintlify.dev`，更新才会生效。
</Warning>
...
`/.well-known/skills/*` 和 `/skill.md` 路由是可选的。只有在你希望在站点根路径（例如 `your-domain.com/skills.md`）而不是文档子路径下（例如 `your-domain.com/docs/skills.md`）提供 AI skills 文件时，才需要包含它们。
...
如果你需要使用除 `/docs` 之外的其他子路径（例如 `/help` 或 `/resources`），请使用以下路由配置。
