...
<Info>
  [Pro 和 Enterprise 方案](https://mintlify.com/pricing?ref=docs-ci)包含针对 GitHub 存储库的 CI 检查。
</Info>
...
<Tip>
  Mintlify GitHub 应用只需要访问存放文档内容的存储库。我们建议仅授予该存储库的访问权限。
</Tip>

## 配置

前往控制台的 [常规](https://app.mintlify.com/settings/project/general) 页面，配置在项目中启用的 CI 检查。开启你希望运行的检查。
...
要查看拉取请求中失效链接的详细结果，请点击 **Checks** 选项卡并选择 Mintlify 失效链接检查。结果会列出拉取请求中发现的包含失效链接的文件。
...
如果在你的项目的内容根目录中有一个 `.vale.ini` 文件，那么 Vale CI 检查会使用该配置文件以及你在 `StylesPath` 中指定的任何配置文件。

如果你没有 Vale 配置文件，则会自动加载默认配置。

<Note>
  默认配置在 Mintlify 的构建环境中运行，其中 `StylesPath = /app/styles` 指向内部目录。如果你创建自己的 `.vale.ini` 文件，请使用你存储库中的相对路径，例如 `StylesPath = styles`。

  出于安全原因，你不能在配置文件中使用绝对路径或包含 `..` 的路径。
</Note>
...
<Note>
  出于安全原因，你不能自动下载不来自 [Vale package registry](https://vale.sh/explorer) 的软件包。
</Note>
