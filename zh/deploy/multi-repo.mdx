---
title: "多仓库部署"
description: "将多个 Git 仓库中的文件合并到一个 Mintlify 站点，并为每个源仓库分配独立的 URL 路径。本文说明部署流程、配置要点和故障处理方法，帮助你稳定发布并维护文档站点。这些说明适合开发者和文档维护者查阅。你可以结合页面示例完成相关操作。相关做法也便于团队后续维护和协作。"
...
Mintlify 支持在 `anchors`、`tabs`、`groups`、`pages`、`products` 以及 `navigation.global` 下的数组中使用 `sourceRef`。其他导航结构（包括标签页内的 `menu` 条目）不支持 `sourceRef`。
