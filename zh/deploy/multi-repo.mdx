...
`sourceRef` 条目在多仓库部署期间解析，此时 Mintlify 可以访问每一个源。使用 `mint dev` 进行本地预览时只能看到单个仓库，因此无法内联所引用的导航。

当你在一个 `docs.json` 中包含 `sourceRef` 条目的仓库中运行 `mint dev` 时，Mintlify 会从预览中剥离这些条目，并记录一条警告。导航的其余部分会正常渲染，但来自其他源的分区在你部署之前将不会显示。
