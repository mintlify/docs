...
在基础源 `docs.json` 的某个导航数组中添加 `sourceRef` 条目。其值必须与部署中另一个仓库源的 URL 路径匹配。
...
在此示例中，位于 `/docs` 的基础源展示了一个 `Guides` 标签页，并内联了配置在 `/api` 的第二个源的 `tabs`。
...
- `sourceRef` 的值必须与同一部署中某个仓库源的 URL 路径匹配。
