...
Mintlify 会通过 HTTPS（端口 443）连接你的 GitLab 实例，用于交换 OAuth 令牌、调用 GitLab API、克隆存储库和推送提交。这些连接来自 Mintlify 的静态出口 IP 地址：`54.242.90.151`。

如果防火墙或 IP 允许列表限制了对你的 GitLab 实例的访问，请允许来自 `54.242.90.151` 的入站 HTTPS（端口 443）连接。
