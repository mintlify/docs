...
<Note>
  本指南仅涉及为自动化提供支持的 **OAuth** 集成。你必须使用访问令牌单独配置部署端连接（用于内容同步和预览），请参阅 [GitLab 指南](/zh/deploy/gitlab)。OAuth 集成依赖于部署端连接。
</Note>
...
- 你的 Mintlify 组织需要 **Pro 或更高套餐**。
- 你的 Mintlify 组织已启用自托管 GitLab 功能。如果你在 [Git 设置](https://app.mintlify.com/settings/project/git-settings)控制台页面中未看到 **Self-hosted GitLab** 部分，请联系支持团队。

## 网络要求

该集成需要你的 GitLab 实例与 Mintlify 之间存在双向流量。

### 入站：从 Mintlify 到你的 GitLab 实例

Mintlify 通过 HTTPS（端口 443）连接到你的 GitLab 实例，以交换 OAuth 令牌并调用 GitLab API。这些连接来自 Mintlify 的静态出站 IP 地址：`54.242.90.151`。

如果防火墙或 IP 允许列表限制了对你的 GitLab 实例的访问，请允许来自 `54.242.90.151` 的入站 HTTPS（端口 443）流量。

<Note>
  自动化运行会在独立的沙箱基础设施中执行 clone 与 push 操作，该基础设施默认使用动态 IP 地址。如果你的 GitLab 实例启用了 IP 允许列表，请参见[修复被 Git 提供方 IP 允许列表阻止的 agent 运行](/zh/help-center/agent-blocked-by-git-provider-ip-allow-list)。
</Note>

### 出站：从你的 GitLab 实例到 Mintlify

你的 GitLab 实例通过 HTTPS（端口 443）将 webhook 事件发送到 Mintlify 的 `app.mintlify.com`。请按主机名放行此流量。Mintlify 不会为此端点发布静态 IP 地址列表。
...
<Steps>
  ...
  <Step title="Register the instance in Mintlify">
    ...
    - **GitLab instance URL**：你的 GitLab 实例的公网 URL，例如 `https://gitlab.your-company.com`。Mintlify 在交换令牌和调用 GitLab API 时会通过此 URL 访问你的实例。URL 必须使用 `https://` 并解析为公网地址。保存实例时，Mintlify 会拒绝 `http://` URL 以及解析为私有、回环或链路本地地址的主机。Mintlify 还会拒绝包含用户名或密码，或使用默认 HTTPS 端口 443 以外端口的 URL。
