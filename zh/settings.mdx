...
`docs.json` 文件可以将一组 Markdown 文件构建为一个可导航、可自定义的文档站点。这个必需的配置文件控制样式、导航、integrations 等等。可以把它看作你的文档蓝图。
...
开始时，你只需指定 `theme`、`name`、`colors.primary` 和 `navigation`。其他字段可选，可随着文档需求的增长再添加。

为获得最佳编辑体验，请在 `docs.json` 文件顶部包含模式引用（schema reference）。这将在大多数代码编辑器中启用自动补全、验证和有用的工具提示：
...
<ResponseField name="logo" type="string or object">
  ...
  <Expandable title="Logo">
    ...
    <ResponseField name="href" type="string (uri)">
      点击徽标时跳转的 URL。若未提供，将默认链接到你的首页。示例：`https://mintlify.com`
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="favicon" type="string or object">
  ...
  可为单个文件，或分别提供浅色与深色模式文件。示例：`/favicon.png`
...
<ResponseField name="styling" type="object">
  ...
  <Expandable title="Styling">
    ...
    <ResponseField name="codeblocks" type="&#x22;system&#x22; | &#x22;dark&#x22; | object">
      ...
      **自定义主题配置：**
      使用对象指定 [Shiki 主题](https://shiki.style/themes)。可以为两种模式共用一个主题，或分别为浅色和深色指定不同主题。
...
<ResponseField name="fonts" type="object">
  ...
  <Expandable title="Fonts">
    ...
    <ResponseField name="source" type="string (uri)">
      字体来源的 URL，例如 https://mintlify-assets.b-cdn.net/fonts/Hubot-Sans.woff2。指定 Google 字体的 `family` 名称时，[Google Fonts](https://fonts.google.com) 会自动加载，因此无需提供 source URL。
    </ResponseField>
...
<ResponseField name="appearance" type="object">
  ...
  <Expandable title="Appearance">
    <ResponseField name="default" type="&#x22;system&#x22; | &#x22;light&#x22; | &#x22;dark&#x22;">
      默认主题模式。选择 `system` 以匹配用户的操作系统设置，或选择 `light` 或 `dark` 强制使用特定模式。默认值为 `system`。
    </ResponseField>
...
<ResponseField name="background" type="object">
  ...
  <Expandable title="Background">
    <ResponseField name="image" type="string or object">
      站点的背景图片。可为单个文件，或分别为浅色和深色模式提供文件。

      <Expandable title="Image">
        <ResponseField name="light" type="string" required>
          浅色模式下背景图片的路径。需包含文件扩展名。示例：`/background.png`。
        </ResponseField>

        <ResponseField name="dark" type="string" required>
          深色模式下背景图片的路径。需包含文件扩展名。示例：`/background-dark.png`。
        </ResponseField>
      </Expandable>
    </ResponseField>
...
<ResponseField name="navigation 导航" type="object" required>
  您内容的导航结构。
  ...
  <Expandable title="导航">
    ...
    <ResponseField name="版本" type="array of object">
      为包含多个[版本](zh/navigation#versions)的网站提供版本切换器。
    </ResponseField>

    <ResponseField name="Tabs" type="array of object">
      顶级导航[Tabs](zh/navigation#tabs)。
    </ResponseField>
    ...
    <ResponseField name="groups" type="array of object">
      用于将内容组织成各个部分的[groups](zh/navigation#groups)。
    </ResponseField>
...
<ResponseField name="interaction" type="object">
  ...
  <Expandable title="交互">
    <ResponseField name="drilldown" type="boolean">
      控制选择导航分组时的自动跳转行为。设为 `true` 可在展开导航分组时强制跳转至第一个页面；设为 `false` 则不跳转，仅展开或折叠该分组；不设置则采用主题的默认行为。
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="footer" type="object">
  ...
  <Expandable title="页脚">
    <ResponseField name="socials" type="object">
      在页脚中展示的社交媒体账号。每个 key 为平台名称，每个 value 为你的个人资料 URL。例如：
...
<ResponseField name="seo" type="object">
  ...
  <Expandable title="Seo">
    <ResponseField name="metatags" type="object">
      添加到每个页面的 Meta 标签。必须是有效的 key-value 对。可参阅[常用 Meta 标签参考](/zh/settings/seo#common-meta-tags-reference)了解可选项。
    </ResponseField>

    <ResponseField name="indexing" type="&#x22;navigable&#x22; | &#x22;all&#x22;">
      指定搜索引擎应索引的页面。选择 `navigable` 仅索引位于你的 `docs.json` navigation 中的页面；选择 `all` 则索引所有页面。默认值为 `navigable`。
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="integrations" type="object">
  ...
  <Expandable title="集成">
    ...
    <ResponseField name="Intercom 客服系统" type="object">
      Intercom 集成。
...
如果你的文档项目使用的是已废弃的 `mint.json` 文件，请按以下步骤升级到 `docs.json`。
...
<Steps>
  ...
  <Step title="创建 docs.json 文件">
    ...
    该命令会基于现有的 `mint.json` 生成一个 `docs.json` 文件。请审阅生成的文件，确认所有设置均正确。
  </Step>
