---
title: "全局设置"
description: "通过 `docs.json` 文件配置站点范围的设置"
icon: "settings-2"
...
入门时，你只需指定 `theme`、`name`、`colors.primary` 和 `navigation`。其他字段都是可选的，可随着文档需求增长再逐步添加。
...
<ResponseField name="colors" type="object" required>
  ...
  <Expandable title="Colors">
    ...
    <ResponseField name="dark" type="string matching ^#([a-fA-F0-9]{6}|[a-fA-F0-9]{3})$">
      在浅色与深色模式下用于按钮与悬停状态的颜色，具体效果因主题而异。
...
<ResponseField name="logo" type="string or object">
  ...
  <Expandable title="Logo">
    ...
    <ResponseField name="href" type="string (uri)">
      点击 Logo 时跳转的 URL。若未提供，Logo 将链接到您的主页。示例：`https://mintlify.com`
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="thumbnails" type="object">
  ...
  <Expandable title="Thumbnails">
    <ResponseField name="appearance" type="&#x22;light&#x22; | &#x22;dark&#x22;">
      缩略图的视觉主题。若未指定，缩略图将使用由 `colors` 字段定义的站点配色方案。
    </ResponseField>
...
<ResponseField name="styling" type="object">
  ...
  <Expandable title="Styling">
    <ResponseField name="eyebrows" type="&#x22;section&#x22; | &#x22;breadcrumbs&#x22;">
      页面“眉”的样式。选择 `section` 显示分区名称，或选择 `breadcrumbs` 显示完整导航路径。默认值为 `section`。
    </ResponseField>
...
<ResponseField name="icons" type="object">
  ...
  <Expandable title="Icons">
    <ResponseField name="library" type="&#x22;fontawesome&#x22; | &#x22;lucide&#x22;" required>
      ...
      <Note>
        无论库设置如何，你都可以为任意单个图标指定外部托管图标的 URL、项目内图标文件的路径，或与 JSX 兼容的 SVG 代码。
      </Note>
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="fonts" type="object">
  ...
  <Expandable title="Fonts">
    ...
    <ResponseField name="source" type="string (uri)">
      字体源的 URL，例如 https://mintlify-assets.b-cdn.net/fonts/Hubot-Sans.woff2。指定 Google 字体的 `family` 名称时，[Google Fonts](https://fonts.google.com) 会自动加载，因此无需提供来源 URL。
    </ResponseField>
...
<ResponseField name="appearance" type="object">
  ...
  <Expandable title="Appearance">
    <ResponseField name="default" type="&#x22;system&#x22; | &#x22;light&#x22; | &#x22;dark&#x22;">
      默认主题模式。选择 `system` 以匹配用户的操作系统设置，或选择 `light`、`dark` 以强制使用特定模式。默认值为 `system`。
    </ResponseField>
...
<ResponseField name="background" type="object">
  ...
  <Expandable title="Background">
    <ResponseField name="image" type="string or object">
      站点的背景图像。可以是单个文件，也可以分别为浅色和深色模式提供不同文件。

      <Expandable title="Image">
        <ResponseField name="light" type="string" required>
          浅色模式下背景图像的路径。需包含文件扩展名。示例：`/background.png`。
        </ResponseField>

        <ResponseField name="dark" type="string" required>
          深色模式下背景图像的路径。需包含文件扩展名。示例：`/background-dark.png`。
        </ResponseField>
      </Expandable>
    </ResponseField>
    ...
    <ResponseField name="primary" type="object">
      ...
      <Expandable title="Primary">
        <ResponseField name="type" type="&#x22;button&#x22; | &#x22;github&#x22;" required>
          按钮样式。选择 `button` 为带文本的标准按钮，或选择 `github` 为带图标的 GitHub 仓库链接。
        </ResponseField>
        ...
        <ResponseField name="href" type="string (uri)" required>
          按钮链接目标。必须是有效路径或外部 URL。若 `type` 为 `github`，必须是 GitHub 仓库的 URL。
        </ResponseField>
      </Expandable>
    </ResponseField>
  </Expandable>
...
<ResponseField name="导航" type="object" required>
  ...
  <Expandable title="导航">
    ...
    <ResponseField name="版本" type="array of object">
      用于含有多个[版本](zh/navigation#versions)的网站的版本切换器。
    </ResponseField>
    ...
    <ResponseField name="分组" type="array of object">
      用于将内容组织成章节的 [Groups](zh/navigation#groups)。
    </ResponseField>

    <ResponseField name="页面" type="array of string or object">
      构成你文档的单个[页面](zh/navigation#pages)。
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="interaction" type="object">
  ...
  <Expandable title="Interaction">
    <ResponseField name="drilldown" type="boolean">
      控制选择导航分组时的自动导航行为。设置为 `true` 时，展开导航分组将自动跳转到该分组的第一个页面；设置为 `false` 时，将不会跳转，仅展开或折叠分组；不设置则采用主题的默认行为。
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="footer" type="object">
  ...
  <Expandable title="Footer">
    <ResponseField name="socials" type="object">
      在页脚中展示的社交媒体账号。每个键为平台名称，每个值为你的账号 URL。例如：
...
<ResponseField name="api" type="object">
  ...
  <Expandable title="Api">
    <ResponseField name="openapi" type="string or array or object">
      用于生成 API 文档的 OpenAPI规范 文件。可以是单个 URL/路径，或由多个 URL/路径构成的数组。

      <Expandable title="Openapi">
        <ResponseField name="source" type="string">
          你的 OpenAPI规范 文件的 URL 或路径。
    ...
    <ResponseField name="asyncapi" type="string or array or object">
      用于生成 API 文档的 AsyncAPI规范 文件。可以是单个 URL/路径，或由多个 URL/路径构成的数组。

      <Expandable title="Asyncapi">
        <ResponseField name="source" type="string">
          你的 AsyncAPI规范 文件的 URL 或路径。
...
<ResponseField name="seo" type="object">
  ...
  <Expandable title="Seo">
    <ResponseField name="metatags" type="object">
      将添加到每个页面的 Meta 标签。必须为有效的键值对。参见[常用 Meta 标签参考](/zh/settings/seo#common-meta-tags-reference)以查看可选项。
    </ResponseField>
...
<ResponseField name="集成" type="object">
  ...
  <Expandable title="集成">
    ...
    <ResponseField name="考拉" type="object">
      Koala 集成。
...
<Tabs>
  ...
  <Tab title="交互式API示例">
    ```json title="文档配置文件" wrap lines highlight={43-61, 72-79}
    {
      "$schema": "https://mintlify.com/docs.json",
      "theme": "maple",
      "name": "Example Co.",
