...
要启用 JIT 配置，你必须先启用 SSO。前往 dashboard 中的 [Security](https://app.mintlify.com/settings/organization/access) 页面，完成 SSO 设置，然后启用 JIT 配置。
...
只有在 SSO 连接处于可用状态后，**Verified domains** 部分才会显示。添加域名前，请先[配置 SSO](#configure-sso)。

1. 前往 dashboard 中的 [Security](https://app.mintlify.com/settings/organization/access) 页面。
2. 在 **SSO** 部分的 **Verified domains** 下，输入域名（例如 `example.com`），然后点击 **Add**。
...
要要求组织内的所有人都通过身份提供商登录，请开启 **Require SSO**。当 **Require SSO** 开启时，Mintlify 会拒绝密码、magic link 和 Google OAuth 登录。
...
1. 前往 dashboard 中的 [Security](https://app.mintlify.com/settings/organization/access) 页面。
2. 在 **SSO** 部分的 **Break-glass emails** 下，输入应保留非 SSO 访问权限的成员的邮箱地址，然后点击 **Add**。
