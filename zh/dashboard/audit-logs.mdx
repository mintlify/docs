---
title: "Mintlify 组织审计日志完整使用指南：查看部署、配置、权限、集成和分析变更，并导出或流式传输日志"
description: "了解如何在 Mintlify 中查看、筛选、导出和流式传输组织审计日志，跟踪部署、配置、权限、集成、分析、自动化和工作流变更，并将日志发送到 Amazon S3 或 Datadog 以满足安全、合规和长期保留需求。使用这些记录调查问题、掌握组织活动并支持合规审查。"
...
| Integration      | 第三方集成以及凭证或 webhook 令牌的更改，包括 GitHub、GitLab、Bitbucket、Slack、Discord、Linear、Notion、MCP 和 Composio。 |
| Analytics        | 分析数据导出。 |
| Analytics streaming | 流式传输目标的更改，以及包含的事件或部署的更改。 |
| Billing          | 订阅更新、附加组件购买和发票查看。          |
| Quota            | 超额策略和告警配置更改。                    |
| User             | 个人用户通知设置。                             |
| Automations      | 自动化配置和存储库管理。                 |
| Workflow         | 工作流创建、更新、删除、开关切换、运行以及连接服务的更改。 |
| Audit log        | 审计日志的查看、导出、流式传输目标和类别配置更改。                              |
