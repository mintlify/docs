...
Mintlify 的 SCIM 端点可与任何符合 SCIM 2.0 标准、并使用 bearer token 进行身份验证的身份提供商配合使用。下面的分步说明以 Okta 为例。对于其他提供商，请[联系我们](mailto:support@mintlify.com)以获取设置指导。
...
<Steps>
  <Step title="打开 SCIM 设置">
    在 Mintlify 控制台中，前往 [Security](https://app.mintlify.com/settings/organization/access) 页面的 **SCIM** 部分。
  </Step>
  <Step title="开启 SCIM">
...
1. 在 Mintlify 控制台中，前往 [Security](https://app.mintlify.com/settings/organization/access) 页面的 **SCIM** 部分。
...
1. 前往 [Security](https://app.mintlify.com/settings/organization/access) 页面的 **SCIM** 部分。
2. 点击 **Rotate token**。
