---
title: "CLI 安装"
sidebarTitle: "安装"
description: "安装 CLI，在本地预览并开发文档"
icon: "terminal"
---

<img className="block dark:hidden my-0 pointer-events-none" src="/images/installation/local-development-light.png" />

<img className="hidden dark:block my-0 pointer-events-none" src="/images/installation/local-development-dark.png" />

<div id="installing-the-cli">
  ## 安装命令行工具
</div>

<Info>
  **先决条件**：请先安装 [Node.js](https://nodejs.org/en)。
</Info>

<Steps>
  <Step title="安装命令行工具">
    运行以下命令安装 [CLI](https://www.npmjs.com/package/mint)：

    <CodeGroup>
      ```bash npm
      npm i -g mint
      ```

      ```bash pnpm
      pnpm add -g mint
      ```
    </CodeGroup>
  </Step>

  <Step title="本地预览">
    进入文档目录（`docs.json` 所在位置），执行以下命令：

    ```bash
    mint dev
    ```

    随后可在 `http://localhost:3000` 访问本地预览。
  </Step>
</Steps>

或者，如果你不想全局安装命令行工具，可以运行一次性脚本：

```bash
npx mint dev
```

<div id="updates">
  ## 更新
</div>

如果你的本地预览与线上生产环境显示不一致，请更新本地命令行工具（CLI）：

```bash
mint update
```

如果你的本地环境中没有 `mint update` 命令，请重新安装最新版的 CLI：

<CodeGroup>
  ```bash npm
  npm i -g mint@latest
  ```

  ```bash pnpm
  pnpm add -g mint@latest
  ```
</CodeGroup>

<div id="custom-ports">
  ## 自定义端口
</div>

默认情况下，CLI 使用 3000 端口。你可以使用 `--port` 标志来自定义端口。例如，要在 3333 端口运行 CLI，请使用以下命令：

```bash
mint dev --port 3333
```

如果你尝试使用已被占用的端口运行，它会改用下一个可用端口：

```mdx
端口 3000 已被占用。尝试改用 3001。
```

<div id="previewing-as-a-specific-group">
  ## 以特定分组进行预览
</div>

如果你使用部分认证来限制文档访问，可以通过 `--group [groupname]` 标志以特定认证分组的身份进行预览。

例如，如果你有一个名为 `admin` 的分组，可以使用以下命令以该分组成员的身份进行预览：

```bash
mint dev --group admin
```

<div id="additional-commands">
  ## 其他命令
</div>

虽然 `mint dev` 是最常用的命令，但你还可以使用其他命令来管理文档。

<div id="finding-broken-links">
  ### 查找断链
</div>

CLI 可以帮助你验证文档中的引用链接。要定位任何断链，请使用以下命令：

```bash
mint 断链检查
```

<div id="checking-openapi-spec">
  ### 检查 OpenAPI 规范
</div>

你可以使用命令行工具，通过以下命令检查 OpenAPI 文件是否有错误：

```bash
mint openapi-check <OpenAPI规范文件名或URL>
```

你可以传入文件名（如 `./openapi.yaml`）或 URL（如 `https://petstore3.swagger.io/api/v3/openapi.json`）。

<div id="renaming-files">
  ### 重命名文件
</div>

你可以使用以下命令重命名文件，并更新对这些文件的所有引用：

```bash
mint rename <oldFilename> <newFilename>
```

<div id="migrating-mdx-endpoint-pages">
  ### 迁移 MDX 端点页面
</div>

你可以使用以下命令，将 MDX 端点页面迁移为基于你的 OpenAPI规范 自动生成的页面：

```bash
mint migrate-mdx
```

此命令会将单独的 MDX 端点页面转换为在你的 `docs.json` 中定义的自动生成页面，将 MDX 内容移至 OpenAPI规范 中的 `x-mint` 扩展，并更新你的导航。详见[从 MDX 迁移](/zh/api-playground/migrating-from-mdx)了解详细信息。

<div id="formatting">
  ## 格式化
</div>

在本地开发时，我们建议在你的 IDE 中安装扩展来识别并格式化 `MDX` 文件。

如果你使用 Cursor、Windsurf 或 VS Code，推荐安装用于语法高亮的 [MDX VS Code 扩展](https://marketplace.visualstudio.com/items?itemName=unifiedjs.vscode-mdx)，以及用于代码格式化的 [Prettier](https://marketplace.visualstudio.com/items?itemName=esbenp.prettier-vscode)。

如果你使用 JetBrains，推荐安装用于语法高亮的 [MDX IntelliJ IDEA 插件](https://plugins.jetbrains.com/plugin/14944-mdx)，并配置 [Prettier](https://prettier.io/docs/webstorm) 进行代码格式化。

<div id="troubleshooting">
  ## 故障排查
</div>

<AccordionGroup>
  <Accordion title="错误：无法在 darwin-arm64 运行时加载“sharp”模块">
    这可能是由于 Node 版本过旧所致。请尝试以下操作：

    1. 卸载当前已安装的 Mint CLI：`npm uninstall -g mint`
    2. 升级到最新的 Node.js
    3. 重新安装 Mint CLI：`npm install -g mint`
  </Accordion>

  <Accordion title="问题：遇到未知错误">
    **解决方案**：前往用户主目录并删除 `~/.mintlify` 文件夹。然后再次运行 `mint dev`。
  </Accordion>

  <Accordion title="错误：permission denied">
    原因是缺少全局安装 Node 包所需的权限。

    **解决方案**：尝试运行 `sudo npm i -g mint`。系统会提示你输入密码，即用于解锁电脑的那一个。
  </Accordion>

  <Accordion title="本地预览与线上文档显示不一致">
    这很可能是因为 CLI 版本过旧。

    **解决方案：** 运行 `mint update` 获取最新更新。
  </Accordion>

  <Accordion title="mintlify 与 mint 包">
    如果你在使用 CLI 包时遇到问题，首先运行 `npm ls -g`。该命令会显示机器上全局安装的包。

    如果你不使用 npm，或在 -g 列表中未看到它，试试运行 `which mint` 定位安装路径。

    如果同时安装了名为 `mint` 和 `mintlify` 的包，请卸载 `mintlify`。

    1. 卸载旧包：

    ```bash
      npm uninstall -g mintlify
    ```

    2. 清理 npm 缓存：

    ```bash
      npm cache clean --force
    ```

    3. 重新安装新包：

    ```bash
    npm i -g mint
    ```
  </Accordion>
</AccordionGroup>
