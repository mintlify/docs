...
使用 `mint scrape` 命令从外部文档站点或 OpenAPI 规范抓取内容。这在你将文档从其他平台迁移到 Mintlify 时非常有用。
...
使用 `--filter`（或 `-f`）标志将抓取范围限制为匹配特定路径前缀的 URL：
...
| 标志 | 描述 |
| --- | --- |
| `--outDir` | 写入生成文件的目录。默认为 `./docs`。 |
| `--overwrite` | 覆盖现有文件。 |
| `--no-writeFiles` | 预览输出而不写入文件。 |
...
参见 [全局设置](/zh/organize/settings) 了解更多关于 `docs.json` 的信息。
...
此命令将引导你在 `.mintlify/workflows/` 目录下创建工作流文件。它会提示你输入工作流名称、触发类型（定时任务或推送）、上下文存储库、自动合并设置，并打开默认编辑器来编写代理指令。

<Tip>
  命令行界面（CLI）会自动检测非交互式环境。在非交互式环境中，它会输出使用说明，而不会在等待输入时卡住。
</Tip>

有关工作流文件和配置选项的更多信息，请参阅 [Workflows](/zh/agent/workflows)。
