...
默认情况下，浏览器会自动打开。若要阻止浏览器自动打开，请使用 `--no-open` 标志：
...
该命令会忽略与 [.mintignore](/zh/organize/mintignore) 模式相匹配的文件。指向被忽略文件的链接会被报告为损坏链接。
...
若要检查通过 HTTP 提供的本地托管 OpenAPI 文件，请使用 `--local-schema` 标志：
...
<Note>
  生产部署仅支持 HTTPS URL。`--local-schema` 标志仅用于本地开发。
</Note>
