...
将你的文档导出为独立的静态站点，用于离线或隔离环境部署：

```bash
mint export
```

此命令会生成一个 ZIP 压缩包，其中包含整个文档站点的静态 HTML 文件，以及用于本地提供服务的辅助脚本。导出的压缩包包含一个基于 Node.js 的静态服务器和适用于 macOS/Linux 和 Windows 的启动脚本。

默认情况下，输出文件名为 `export.zip`。使用 `--output` 标志来指定自定义输出路径：
...
| 标志 | 说明 |
| --- | --- |
| `--output` | 输出 ZIP 文件路径。默认为 `export.zip`。 |
| `--groups` | 为导出模拟用户组（在导出受组限制的内容时很有用）。 |
| `--disable-openapi` | 在导出过程中禁用 OpenAPI 文件生成。 |

<Note>
  首次运行时需要网络连接以下载客户端应用程序。后续运行将使用 `~/.mintlify/` 中缓存的客户端。
</Note>
