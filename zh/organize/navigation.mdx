...
将分组的 `mode` 设置为 `"tree"`，即可在其根页面上显示导航树。导航树会列出子页面和嵌套分组，让用户清晰了解该部分的内容结构。
...
`mode` 属性会递归继承。当你在父级元素（如选项卡、锚点或顶级导航）上设置 `mode` 时，所有后代分组都会继承相同的模式。任何后代都可以通过设置自己的 `mode` 值来覆盖继承的模式。

<Note>
  树形导航要求分组必须有 `root` 页面。没有 `root` 页面的分组不受树形模式影响。
</Note>
