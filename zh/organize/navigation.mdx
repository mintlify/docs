---
title: "导航"
description: "在 docs.json 中使用分组、页面、下拉菜单和标签页配置文档站点导航，构建侧边栏结构。"
...
在 `navigation` 对象中，`groups` 是一个数组，其中每一项都是一个对象，每个对象都必须包含 `group` 字段和 `pages` 字段。`icon`、`tag`、`root`、`expanded` 和 `mode` 字段是可选的。
...
使用 `mode` 属性在分组的根页面上显示导航树。当 `mode` 设置为 `"tree"` 时，根页面会自动呈现子页面和嵌套分组的列表。

树形导航需要分组上有一个 [`root`](#root-page) 页面。根页面会为每个子页面显示卡片，为每个嵌套分组显示区域，包含标题、描述和图标。
...
- `"tree"`：在分组的根页面上呈现导航树，显示子页面和嵌套分组。
- `"auto"` 或省略：标准行为，不显示导航树。

#### 模式继承

`mode` 属性从父级继承到子级。您可以在任何导航层级设置 `mode`——包括根 `navigation` 对象、选项卡、锚点、下拉菜单、产品、版本和语言——所有后代分组都会继承该模式。
...
在这个示例中，“指南”分组从根导航继承了 `"tree"` 模式，而“API”分组将其覆盖为 `"auto"`。
