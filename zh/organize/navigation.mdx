...
在分组上将 `mode` 属性设置为 `"tree"`，可在其根页面上显示导航树。导航树会列出子页面和嵌套分组，为读者提供该部分的可浏览概览。
...
<Note>
  树状导航要求分组具有 `root` 页面。导航树将在该根页面上渲染。
</Note>

#### 模式继承

`mode` 属性可递归继承。在父元素（如顶级 `navigation` 对象、选项卡或锚点）上设置该属性，会将其应用到所有后代分组。任何后代都可以通过设置自己的 `mode` 来覆盖继承的模式。
