...
<ResponseField name="groups" type="string[]">
  将页面限制为特定组中的用户。用户必须属于至少一个列出的组才能访问该页面。必须先设置[身份验证](/zh/deploy/authentication-setup)。详情参见[使用组控制访问权限](/zh/deploy/authentication-setup#control-access-with-groups)。
</ResponseField>
