...
每个页面都以 frontmatter 开始，它是位于文件开头、由 `---` 包裹的 YAML metadata。该 metadata 决定页面的显示与行为。
...
<ResponseField name="iconType" type="string">
  [Font Awesome](https://fontawesome.com/icons) 的 icon 样式。仅在使用 Font Awesome 图标时生效。
...
自定义模式提供极简布局，除顶部导航栏外移除所有元素。它是一块空白画布，适合创建登录页或其他希望尽可能精简导航元素的独特布局。所有主题均支持自定义模式。
