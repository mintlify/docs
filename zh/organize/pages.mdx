...
<Warning>
  在自定义模式页面上使用 `style` 属性可能会在页面加载时导致布局偏移。请改用 [Tailwind CSS 或自定义 CSS](/zh/customize/custom-scripts) 来避免此问题。
</Warning>
...
当本设置为 `true` 时，即使全局设置为 `false`，仍会显示时间戳。当本设置为 `false` 时，即使全局设置为 `true`，时间戳也会被隐藏。
