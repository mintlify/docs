...
<ResponseField name="logo.href" type="string (uri)">
  点击 logo 时重定向到的 URL。如果未提供，对于[国际化文档](/zh/guides/internationalization)，logo 链接到当前所选区域设置的第一页，对于单语言站点则链接到首页。可以是绝对 URL（例如 `https://yoursite.com`），也可以是以 `/` 开头的相对路径（例如 `/quickstart`）。
</ResponseField>
