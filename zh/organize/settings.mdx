---
title: "全局设置"
description: "在 docs.json 中配置全站设置，包括主题、颜色、导航、API 操作台和 integrations。"
...
<ResponseField name="theme" required>
  ...
  以下之一：`mint`、`maple`、`palm`、`willow`、`linden`、`almond`、`aspen`、`sequoia`、`luma`。

  更多信息请参见 [Themes](/zh/customize/themes)。
</ResponseField>
...
<ResponseField name="colors" type="object" required>
  文档中使用的颜色。不同主题对颜色的应用方式有所不同。如果你只提供一个主色，它将应用于所有颜色元素。

  <Expandable title="Colors">
    <ResponseField name="primary" type="string matching ^#([a-fA-F0-9]{6}|[a-fA-F0-9]{3})$" required>
      你的文档主色。通常在 light 模式下用于强调，具体会因主题而略有不同。
    ...
    <ResponseField name="dark" type="string matching ^#([a-fA-F0-9]{6}|[a-fA-F0-9]{3})$">
      在 light 模式和 dark 模式下用于按钮和悬停状态的颜色，具体会因主题而略有不同。
...
<ResponseField name="description" type="string">
  你的站点描述，用于 SEO 和 AI 索引。
</ResponseField>

<ResponseField name="logo" type="string or object">
  为 light 模式和 dark 模式设置你的 logo。

  <Expandable title="Logo">
    <ResponseField name="light" type="string" required>
      指向 light 模式下 logo 文件的路径。请包含文件扩展名。示例：`/logo.png`
    </ResponseField>

    <ResponseField name="dark" type="string" required>
      指向 dark 模式下 logo 文件的路径。请包含文件扩展名。示例：`/logo-dark.png`
    </ResponseField>

    <ResponseField name="href" type="string (uri)">
      点击 logo 时跳转到的 URL。如果未提供，对于[internationalized docs](/zh/guides/internationalization)，logo 会链接到当前所选区域设置的第一页；对于单语言站点，则会链接到你的主页。示例：`https://mintlify.com`
    </ResponseField>
  </Expandable>
</ResponseField>

<ResponseField name="favicon" type="string or object">
  favicon 文件的路径，包括文件扩展名。会自动调整为合适的 favicon 尺寸。可以是单个文件，也可以分别为 light 模式和 dark 模式提供单独的文件。示例：`/favicon.png`

  <Expandable title="Favicon">
    <ResponseField name="light" type="string" required>
      light 模式下 favicon 文件的路径。请包含文件扩展名。示例：`/favicon.png`
    </ResponseField>

    <ResponseField name="dark" type="string" required>
      dark 模式下 favicon 文件的路径。请包含文件扩展名。示例：`/favicon-dark.png`
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="thumbnails" type="object">
  ...
  <Expandable title="Thumbnails">
    <ResponseField name="appearance" type="&#x22;light&#x22; | &#x22;dark&#x22;">
      缩略图的视觉主题。如果未指定，缩略图将使用你的站点在 `colors` 字段中定义的配色方案。
    </ResponseField>
    ...
    <ResponseField name="fonts" type="object">
      ...
      <Expandable title="Fonts">
        <ResponseField name="family" type="string" required>
          字体家族名称，例如 “Open Sans” 或 “Playfair Display”。支持 [Google Fonts](https://fonts.google.com) 字体家族名称。
        </ResponseField>
      </Expandable>
    </ResponseField>
  </Expandable>
...
<ResponseField name="variables" type="object">
  可在整个文档中使用的全局变量。Mintlify 会在构建时使用 `{{variableName}}` 语法替换变量。

  <Expandable title="Variables">
    <ResponseField name="variableName" type="string">
      一个 key-value 对，其中 key 是变量名，value 是替换文本。你必须定义内容中引用的所有变量，否则构建将失败。变量名可以包含字母数字字符、连字符和句点。Mintlify 会对值进行安全过滤以防止 XSS 攻击。
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="导航" type="object" required>
  ...
  <Expandable title="导航">
    ...
    <ResponseField name="语言" type="array of object">
      [multi-language](/zh/organize/navigation#languages) 站点的语言切换器。除导航结构外，每个语言条目还可以包含特定于该语言的 `banner`、`footer` 和 `navbar` 配置。
      ...
      <Expandable title="Languages">
        ...
        <ResponseField name="banner" type="object">
          特定于该语言的横幅配置。可用选项请参见 [banner](#param-banner-1)。
        </ResponseField>

        <ResponseField name="footer" type="object">
          特定于该语言的页脚配置。可用选项请参见 [footer](#param-footer-1)。
        </ResponseField>

        <ResponseField name="navbar" type="object">
          特定于该语言的导航栏配置。可用选项请参见 [navbar](#param-navbar-1)。
        </ResponseField>
    ...
    <ResponseField name="tabs" type="array of object">
      顶级导航 [tabs](/zh/organize/navigation#tabs)。
    </ResponseField>

    <ResponseField name="锚点" type="array of object">
      侧边栏 [anchors](/zh/organize/navigation#anchors)。
    </ResponseField>

    <ResponseField name="下拉菜单" type="array of object">
      用于分组相关内容的 [Dropdowns](/zh/organize/navigation#dropdowns)。
    </ResponseField>

    <ResponseField name="产品" type="array of object">
      具有多个 [products](/zh/organize/navigation#products) 的站点的产品切换器。
    </ResponseField>

    <ResponseField name="groups" type="array of object">
      用于将内容组织到各个分区中的 [Groups](/zh/organize/navigation#groups)。
    </ResponseField>

    <ResponseField name="页面" type="array of string or object">
      构成你的文档的各个 [pages](/zh/organize/navigation#pages)。
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="interaction" type="object">
  ...
  <Expandable title="Interaction">
    <ResponseField name="drilldown" type="boolean">
      控制选择导航分组时的自动导航行为。设置为 `true` 时，在展开导航分组时会强制导航到第一个页面。设置为 `false` 时将阻止导航，仅展开或折叠该分组。不设置时将使用主题的默认行为。
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="metadata" type="object">
  ...
  <Expandable title="Metadata">
    <ResponseField name="timestamp" type="boolean">
      在所有页面上启用最后修改日期。启用后，所有页面都会显示内容的最后修改日期。默认为 `false`。

      你可以在单个页面上使用 `timestamp` frontmatter 字段覆盖此设置。更多信息请参见 [Pages](/zh/organize/pages#last-modified-timestamp)。
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="footer" type="object">
  ...
  <Expandable title="Footer">
    <ResponseField name="socials" type="object">
      要在页脚中显示的社交媒体资料。每个键都是平台名称，每个值都是你的资料 URL。例如：
...
<ResponseField name="banner" type="object">
  ...
  <Expandable title="Banner">
    <ResponseField name="content" type="string" required>
      横幅中显示的文本内容。支持基本 MDX 格式，包括链接、粗体和斜体文本。不支持自定义组件。例如：
...
<ResponseField name="redirects" type="array of object">
  ...
  <Expandable title="Redirects">
    ...
    <ResponseField name="permanent" type="boolean">
      如果为 `true`，则使用永久重定向 (308)。如果为 `false`，则使用临时重定向 (307)。默认为 `true`。
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="contextual" type="object">
  ...
  <Expandable title="Contextual">
    <ResponseField name="options" type="array of &#x22;assistant&#x22; | &#x22;copy&#x22; | &#x22;view&#x22; | &#x22;chatgpt&#x22; | &#x22;claude&#x22; | &#x22;perplexity&#x22; | &#x22;grok&#x22; | &#x22;aistudio&#x22; | &#x22;devin&#x22; | &#x22;windsurf&#x22; | &#x22;mcp&#x22; | &#x22;add-mcp&#x22; | &#x22;cursor&#x22; | &#x22;vscode&#x22; | object" required>
      ...
      * `grok`: 将当前页面内容发送到 Grok。
      * `aistudio`: 将当前页面内容发送到 Google AI Studio。
      * `devin`: 将当前页面内容发送到 Devin。
      * `windsurf`: 使用当前页面内容打开 Windsurf Cascade。
    ...
    <ResponseField name="display" type="&#x22;header&#x22; | &#x22;toc&#x22;">
      在何处显示这些上下文选项。选择 `header` 以在页面顶部的上下文菜单中显示，或选择 `toc` 以在目录侧边栏中显示。默认为 `header`。
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="api" type="object">
  ...
  <Expandable title="api">
    <ResponseField name="openapi" type="string or array or object">
      用于生成 API 文档的 OpenAPI 规范文件。可以是单个 URL/路径，也可以是由多个 URL/路径组成的数组。
    ...
    <ResponseField name="playground" type="object">
      ...
      <Expandable title="Playground">
        <ResponseField name="display" type="&#x22;interactive&#x22; | &#x22;simple&#x22; | &#x22;none&#x22; | &#x22;auth&#x22;">
          API 演练场的显示模式。默认为 `interactive`。使用 `auth` 时，仅向已通过身份验证的用户显示演练场。
        </ResponseField>
    ...
    <ResponseField name="url" type="&#x22;full&#x22;">
      端点标题中基础 URL 的显示模式。设置为 `full` 后，会在每个端点页面始终显示完整的基础 URL。默认情况下，仅当有多个基础 URL 可供选择时才会显示基础 URL。
    </ResponseField>

    <ResponseField name="examples" type="object">
      自动生成的 API examples 的配置。

      <Expandable title="Examples">
        <ResponseField name="languages" type="array of string">
          自动生成的 API 代码示例所使用的示例语言。有关可用语言及别名的完整列表，请参阅[支持的语言](/zh/api-playground/overview#all-supported-languages)。
        </ResponseField>
        ...
        <ResponseField name="prefill" type="boolean">
          是否使用 schema 示例数据预填 API 演练场。启用后，演练场会自动使用 OpenAPI 规范中的示例值填充请求字段。默认为 `false`。
        </ResponseField>

        <ResponseField name="autogenerate" type="boolean">
          是否根据 API 规范为端点生成代码示例。默认为 `true`。当设置为 `false` 时，API 演练场中只会显示手动编写的代码示例 (来自 OpenAPI 规范中的 `x-codeSamples` 或 MDX 中的 `<RequestExample>` 组件) 。
        </ResponseField>
      </Expandable>
    </ResponseField>
    ...
    <ResponseField name="spec" type="object">
      ...
      <Expandable title="Spec">
        <ResponseField name="download" type="boolean">
          是否在 API 参考页面上为 OpenAPI spec 显示下载按钮。默认为 `false`。
        </ResponseField>
      </Expandable>
    </ResponseField>
    ...
    <ResponseField name="mdx" type="object">
      ...
      <Expandable title="Mdx">
        ...
        <ResponseField name="server" type="string or array">
          会被添加到页面级 `api` frontmatter 字段中的相对路径前面的基础 URL。当 frontmatter 中包含完整 URL 时，不会使用此配置。
        </ResponseField>
      </Expandable>
    </ResponseField>
  </Expandable>
