...
<ResponseField name="variables" type="object">
  可在整个文档中使用的全局变量。变量会在构建时使用 `{{variableName}}` 语法进行替换。

  <Expandable title="Variables">
    <ResponseField name="variableName" type="string">
      一个 key-value 对，其中 key 是变量名，value 是替换文本。文档中引用的所有变量都必须已定义，否则构建将失败。变量名可以包含字母数字字符、连字符和句点。值会经过安全过滤以防止 XSS 攻击。
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="导航" type="object" required>
  ...
  <Expandable title="导航">
    ...
    <ResponseField name="版本" type="array of object">
      ...
      <Expandable title="Versions">
        <ResponseField name="default" type="boolean">
          设置为 `true` 可将此版本设为默认版本。如果省略或设为 `false`，则数组中的第一个版本为默认版本。
        </ResponseField>
