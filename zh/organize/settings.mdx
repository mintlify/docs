    ...
    <ResponseField name="links" type="array of object">
      ...
      <Expandable title="链接">
        <ResponseField name="type" type="&#x22;github&#x22; | &#x22;discord&#x22;">
          可选的链接类型。省略则使用标准文本链接。设置为 `github` 时，将链接到 GitHub 存储库并显示该存储库的 Star 数；设置为 `discord` 时，将链接到 Discord 服务器并显示该服务器的在线用户数。
        </ResponseField>

        <ResponseField name="label" type="string">
          链接文本。当未设置 `type` 时为必填；对于 `github` 和 `discord` 类型为可选。省略时，链接文本会根据 API 数据自动填充（GitHub 为存储库名称，Discord 为服务器名称）。
        </ResponseField>
    ...
    <ResponseField name="primary" type="object">
      ...
      <Expandable title="主按钮">
        <ResponseField name="type" type="&#x22;button&#x22; | &#x22;github&#x22; | &#x22;discord&#x22;" required>
          按钮样式。选择 `button` 以使用带文本标签的标准按钮，选择 `github` 以链接到带实时 Star 数的 GitHub 存储库，或选择 `discord` 以使用带实时在线用户数的 Discord 服务器邀请链接。
        </ResponseField>

        <ResponseField name="label" type="string">
          按钮文本。当 `type` 为 `button` 时为必填；对于 `github` 和 `discord` 类型为可选。省略时，按钮文本会根据 API 数据自动填充（GitHub 为存储库名称，Discord 为服务器名称）。
        </ResponseField>
