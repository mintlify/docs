    ...
    <ResponseField name="links" type="array of object">
      ...
      <Expandable title="链接">
        <ResponseField name="type" type="&#x22;github&#x22; | &#x22;discord&#x22;">
          可选的链接类型。省略则为标准文本链接。设置为 `github` 时，将链接到一个 GitHub 存储库并显示该存储库的 star 数。设置为 `discord` 时，将链接到一个 Discord 服务器并显示该服务器的在线用户数。
        </ResponseField>

        <ResponseField name="label" type="string">
          链接文本。当未设置 `type` 时为必填。对于 `github` 和 `discord` 类型为可选。省略时，label 会根据 API 数据自动填充（GitHub 为存储库名称，Discord 为服务器名称）。
        </ResponseField>

        <ResponseField name="href" type="string (uri)" required>
          链接目标地址。必须是有效的外部 URL。对于 `github` 类型，必须是 GitHub 存储库的 URL。对于 `discord` 类型，必须是 Discord 邀请 URL。
        </ResponseField>
    ...
    <ResponseField name="primary" type="object">
      ...
      <Expandable title="主按钮">
        <ResponseField name="type" type="&#x22;button&#x22; | &#x22;github&#x22; | &#x22;discord&#x22;" required>
          按钮样式。选择 `button` 以使用带文本 label 的标准按钮；选择 `github` 以链接到带有实时 star 数的 GitHub 存储库；选择 `discord` 以链接到带有实时在线用户数的 Discord 服务器邀请。
        </ResponseField>

        <ResponseField name="label" type="string">
          按钮文本。当 `type` 为 `button` 时为必填。对于 `github` 和 `discord` 类型为可选——省略时，label 会根据 API 数据自动填充（GitHub 为存储库名称，Discord 为服务器名称）。
        </ResponseField>

        <ResponseField name="href" type="string (uri)" required>
          按钮目标地址。必须是外部 URL。如果 `type` 为 `github`，则必须是 GitHub 存储库的 URL。如果 `type` 为 `discord`，则必须是 Discord 邀请 URL。
        </ResponseField>
      </Expandable>
    </ResponseField>
  </Expandable>
