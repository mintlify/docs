---
title: "隐藏页面"
description: "将文档页面从侧边栏导航中隐藏，同时仍可通过直接 URL、站内搜索或 AI 助手访问，适用于内部参考、AI 上下文等特殊场景。本文说明配置字段、使用场景和维护要点，帮助你组织内容并管理文档站点。这些说明适合开发者和文档维护者查阅。你可以结合页面示例完成相关操作。"
...
在页面 frontmatter 层级，`searchable` 仅接受 `false`。使用它可以让外部搜索引擎仍然可以索引该页面，同时将其从站内搜索和 AI 助手 context 中排除。示例请参阅[将页面从搜索中排除](/zh/optimize/search#exclude-a-page-from-search)。
