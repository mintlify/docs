...
若要将站点上所有隐藏页面纳入搜索、站点地图和 AI 上下文，请在你的 `docs.json` 中添加 `seo` 属性：
...
若只想包含某个隐藏标签页或分组下的页面，请在 `docs.json` 中为该标签页或分组设置 `searchable: true`。当你为了控制导航布局而隐藏标签页或分组，但仍希望其下的页面可被发现时，请使用此选项。
...
设置 `searchable: true` 后，其下的页面仍会出现在：
...
而该标签页或分组本身仍会在渲染的导航中保持隐藏。

页面自身的 `hidden: true` frontmatter 始终优先生效。若要重新排除某个子分组，请在该分组上设置 `hidden: true`，且不要设置 `searchable: true`。
