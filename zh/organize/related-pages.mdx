...
<Warning>
  **相关主题** 只会在生产部署中呈现。无论是自动建议还是 `related` frontmatter 中的手动列表，都会在[分支预览](/zh/deploy/preview-deployments)中被跳过，因此即使已配置，该部分也不会出现在预览 URL 中。Mintlify 不会为预览构建搜索索引（自动模式依赖此索引），且该 add-on 仅适用于生产环境。合并后请在生产环境中验证你的配置。
</Warning>
