...
对于启用了[终端用户认证](/zh/deploy/authentication-setup)的部署，自动推荐会在读者登录后加载，并使用该读者自己的已认证搜索结果。相关页面仅包含该读者有权限查看的页面。若要在已认证的部署中对推荐页面拥有更多控制，请使用 `related` frontmatter 字段来展示经过整理的推荐。
