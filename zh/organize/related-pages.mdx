...
如果在 **Automatic** 模式下,某个页面的 **相关主题** 部分为空:

- 搜索索引可能尚未就绪。自动推荐会在每次部署后重建,因此请在发布新内容后等待几分钟再重新加载页面。
- 你的文档中可能没有足够多的相似内容供 Mintlify 推荐。自动推荐仅从同一页面的语言和版本中抽取,因此文档规模较小或某个语言环境内容较少时,可能返回不到匹配结果。
- 已启用终端用户认证。请改用 `related` frontmatter 字段来整理推荐。
- 页面使用了 `related: false`,该部分被有意隐藏。
