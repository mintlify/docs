...
1. 在仪表盘中前往 [General](https://app.mintlify.com/settings/project/general) 页面。
...
对于启用了[终端用户认证](/zh/deploy/authentication-setup)的项目，Mintlify 会在读者登录后加载自动推荐，并使用该读者自己的已认证搜索结果。相关页面仅包含该读者有权限查看的页面。若要在已认证的项目中获得更多控制，请使用 `related` frontmatter 字段来精选推荐。
