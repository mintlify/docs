---
title: "相关页面"
description: "使用自动推荐或在页面 frontmatter 中手动整理的链接，在每个页面底部添加“相关主题”版块。"
...
1. 在仪表盘中前往 [General](https://app.mintlify.com/settings/deployment/general) 页面。
2. 在“相关主题”部分，启用 **显示相关主题**。
...
对于启用了[终端用户认证](/zh/deploy/authentication-setup)的部署，Mintlify 会在读者登录后加载自动推荐，并使用该读者自己的已认证搜索结果。相关页面仅包含该读者有权限查看的页面。若要在已认证的部署中获得更多控制，请使用 `related` frontmatter 字段来精选推荐。
