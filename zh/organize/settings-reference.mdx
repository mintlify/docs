...
你的 `docs.json` 文件位于文档仓库的根目录。基本设置说明请参见[全局设置](/zh/organize/settings)。

<Tip>
  在 `docs.json` 顶部添加 `"$schema": "https://mintlify.com/docs.json"`，可在 VS Code 等编辑器中启用自动补全和校验。要在本地校验 `docs.json`，请在项目根目录运行 [`mint dev`](/zh/cli/commands#mint-dev) 或 [`mint validate`](/zh/cli/commands#mint-validate)—— schema 错误会在启动时显示。
</Tip>
