...
本页跟踪最新发布的 `docs.json` schema。在你的文件中通过 `"$schema": "https://mintlify.com/docs.json"` 引用它，即可在编辑器中获得校验和自动补全。
...
| `markdown.schema` | boolean | 否 | `true` |
| `markdown.instructions` | string 或字符串数组 | 否 | 无 |
| `seo.indexing` | `"navigable"` \| `"all"` | 否 | `"navigable"` |
| `seo.metatags` | object | 否 | 无 |
| `seo.organization` | object | 否 | 无 |
...
参见[使用 `$ref` 拆分配置](/zh/organize/settings#split-configuration-with-%24ref)了解更多示例。
...
**支持的语言代码：** `ar`、`ca`、`cn`、`cs`、`de`、`en`、`es`、`fr`、`fr-CA`、`he`、`hi`、`hu`、`id`、`it`、`ja`、`ja-JP`、`jp`、`ko`、`lv`、`nl`、`no`、`pl`、`pt`、`pt-BR`、`ro`、`ru`、`sv`、`tr`、`uk`、`uz`、`vi`、`zh`、`zh-CN`、`zh-Hans`、`zh-Hant`、`zh-TW`
...
<Note>
  变量替换仅适用于 MDX 页面和 snippets。它不适用于 OpenAPI 规范文件（`.yaml`、`.yml`、`.json`）或其他静态资源。
</Note>
...
在所有页面上显示最后修改日期。对于由 GitHub 或 GitLab 支持的项目，日期反映最后一次修改页面源文件的 git 提交时间；当无法获取 git 提交日期时，将回退为最近一次部署的时间戳。
...
**类型：** string | string 或 object 数组 | 带有 `source` (string)、`directory` (string) 和 `overlays` (string 数组) 的 object
...
Mintlify 提供给 AI 工具和代理的 Markdown 相关设置。请参阅 [Markdown 导出](/zh/ai/markdown-export)。

**类型：** object

#### `markdown.schema`

是否在 API 参考页面的 Markdown 导出中包含完整的 OpenAPI 或 AsyncAPI 规范。
...
追加到每个页面生成的 Markdown 以及你的 `llms.txt` 和 `llms-full.txt` 文件的自定义代理指令。可提供单个字符串或字符串数组，Mintlify 会以换行符将它们连接起来。请参阅 [自定义代理指令](/zh/ai/markdown-export#custom-agent-instructions)。
...
用作每个页面结构化数据 (JSON-LD) 中发布者实体的组织。接受 `id`、`name`、`legalName`、`url`、`logo` 和 `sameAs`。参见 [SEO 与搜索](/zh/organize/settings-seo#seo)。
