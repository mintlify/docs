...
| `integrations.customScripts` | array of objects（最多 20 个） | 否 | 在每个页面上加载的外部脚本。 |
| `integrations.customScripts[].src` | string (uri) | 是 | 脚本的 HTTPS URL。不能指向不允许的文件类型。 |
| `integrations.customScripts[].strategy` | `"beforeInteractive"` \| `"afterInteractive"` \| `"lazyOnload"` | 否 | 何时加载脚本。 |
