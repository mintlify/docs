...
| `integrations.customScripts` | array of object | 否 | 要加载的自定义第三方脚本。最多 20 个。 |
| `integrations.customScripts[].src` | string (HTTPS uri) | 是 | 脚本的 HTTPS URL。不能以 `.svg`、`.exe` 或 `.zip` 结尾。 |
| `integrations.customScripts[].strategy` | `"afterInteractive"` \| `"lazyOnload"` | 否 | 脚本加载时机。默认值：`afterInteractive`。 |
