...
| `integrations.customScripts` | array of object | 否 | 在每个页面上加载的外部 JavaScript 文件。最多 20 个。 |
| `integrations.customScripts[].src` | string (HTTPS URL) | 是 | 脚本 URL。必须为 HTTPS。`.svg`、`.exe`、`.zip` 扩展名被阻止。 |
| `integrations.customScripts[].strategy` | `"afterInteractive"` \| `"lazyOnload"` | 否 | 脚本加载时机。默认：`afterInteractive`。 |
