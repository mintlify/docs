...
| `integrations.customScripts` | array | 否 | 在每个页面上加载的外部脚本列表。最多 20 个条目。 |
| `integrations.customScripts[].src` | string (HTTPS URL) | 是 | 脚本的 URL。必须是 HTTPS URL。 |
| `integrations.customScripts[].strategy` | string | 否 | 加载策略。`beforeInteractive`、`afterInteractive` 或 `lazyOnload` 之一。使用 `beforeInteractive` 在页面变为可交互之前将脚本注入到 `<head>` 中。 |
