...
<ResponseField name="integrations.customScripts" type="array of objects">
  在每个页面上加载外部 JavaScript 文件。最多可添加 20 个自定义脚本。

  <Expandable title="integrations.customScripts">
    <ResponseField name="src" type="string (uri)" required>
      要加载的脚本的 HTTPS URL。不能指向不允许的文件类型（例如 `.svg`、`.exe`、`.zip`）。
    </ResponseField>
    <ResponseField name="strategy" type="string">
      何时加载脚本。可选值：
      - `beforeInteractive` — 在页面变为可交互之前加载。用于必须尽早运行的脚本，例如 cookie 同意或标签管理器。
      - `afterInteractive` — 在页面变为可交互之后加载。这是默认行为。
      - `lazyOnload` — 在空闲时加载。用于不需要立即运行的低优先级脚本，如分析工具。
    </ResponseField>
  </Expandable>
</ResponseField>
