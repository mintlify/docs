...
<ResponseField name="navbar.links" type="object 数组">
  ...
  <Expandable title="navbar.links">
    ...
    <ResponseField name="href" type="string (uri)" required>
      链接目标。可以是绝对 URL，也可以是以 `/` 开头的相对路径，例如 `/quickstart`。对于 `github`，必须是 GitHub 仓库 URL。对于 `discord`，必须是 Discord 邀请 URL。
    </ResponseField>
    <IconsOptional />
  </Expandable>
</ResponseField>
...
<ResponseField name="navbar.primary" type="object">
  ...
  <Expandable title="navbar.primary">
    ...
    <ResponseField name="href" type="string (uri)" required>
      按钮目标。可以是绝对 URL，也可以是以 `/` 开头的相对路径，例如 `/contact`。对于 `github`，必须是 GitHub 仓库 URL。对于 `discord`，必须是 Discord 邀请 URL。
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="contextual.options" type="array" required>
  ...
  - `"devin-desktop"`—以当前页面为上下文打开 Devin Desktop
