...
Si no hay cambios pendientes o ya hay una solicitud de extracción abierta para la branch actual, el botón **Publish** no se puede hacer clic.

<Note>
  Tu sitio de documentación en producción se actualiza después de que Mintlify construya y despliegue los cambios publicados. Esto suele tardar entre 30 segundos y unos pocos minutos. Consulta el estado de despliegue de tus cambios en tu [panel de control](https://dashboard.mintlify.com).
</Note>
...
Los conflictos ocurren cuando intentas fusionar branches con cambios incompatibles en los mismos archivos.

- Tú y otra persona del equipo editan las mismas líneas de un archivo en diferentes branches.
- Mueves, renombras o eliminas archivos en una branch y los modificas de manera diferente en otra branch.
