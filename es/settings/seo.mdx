---
title: "SEO"
description: "Mejores prácticas de SEO y configuración de metaetiquetas para una mejor visibilidad en buscadores"
icon: "search"
---
...
Puedes personalizar completamente las metaetiquetas de tu sitio añadiendo el campo `metatags` a tu `docs.json` o al frontmatter de una página.

<div id="global-meta-tags">
  ## Etiquetas meta globales
</div>

Puedes establecer etiquetas meta predeterminadas agregando el campo `metadata` a tu `docs.json`.
...
<Warning>
  Las etiquetas meta con dos puntos deben escribirse entre comillas cuando las incluyas en la metadata de la página.
</Warning>
...
A continuación encontrarás una lista completa de metaetiquetas que puedes añadir a tu `docs.json`. Estas metaetiquetas ayudan a mejorar el SEO de tu sitio, la difusión en redes sociales y la compatibilidad con los navegadores.

Puedes ver una vista previa de cómo se mostrarán tus metaetiquetas en distintas plataformas usando [metatags.io](https://metatags.io/).
...
De forma predeterminada, solo se incluyen las páginas que estén en tu `docs.json`. Para incluir enlaces ocultos, añade `seo.indexing` a tu `docs.json`:
...
Para añadir un archivo personalizado `sitemap.xml` o `robots.txt`, crea `sitemap.xml` o `robots.txt` en la raíz de tu proyecto. Añadir un archivo personalizado sustituirá el archivo generado automáticamente con el mismo nombre. Si eliminas el archivo personalizado, se volverá a usar el archivo predeterminado.
