...
<Info>
  Esta es la documentación del Shared Session**Personalization** Handshake. El Shared Session Handshake no está disponible para Authentication.
</Info>
...
<Steps>
  <Step title="Create your Info API">
    Crea un punto final de API que utilice la autenticación de sesión para identificar usuarios y responda con una carga útil JSON siguiendo el formato[User](../sending-data).

    Si el dominio de la API no*coincide exactamente* con el dominio de la documentación:

    * Agrega el dominio de la documentación a tu encabezado API`Access-Control-Allow-Origin` (no debe ser`*`)
...
Tengo un panel en`dash.foo.com`, que utiliza autenticación de sesión basada en cookies. Mis rutas de API del panel están alojadas en`dash.foo.com/api`. Quiero configurar la autenticación para mi documentación alojada en`docs.foo.com`.
...
Tengo un panel en`dash.foo.com`, que utiliza autenticación de sesión basada en cookies. Mis rutas de API del panel están alojadas en`dash.foo.com/api`. Quiero configurar la autenticación para mis documentos alojados en `foo.com/docs`.
...
Luego voy a la configuración del panel y ingreso `https://dash.foo.com/api/docs/user-info` para el campo URL de la API.
...
Luego voy a la configuración del panel y ingreso `https://foo.com/api/docs/user-info` para el campo URL de la API.
