...
También puedes referenciar variables de fragmento dentro de bloques de código. Usa la misma sintaxis `{variableName}` en el cuerpo de un bloque de código delimitado y el valor que pases como propiedad reemplazará el marcador al renderizar la página. Los marcadores que no coincidan con una propiedad se mantienen sin cambios.

1. Añade un bloque de código a tu fragmento que referencie una variable.
...
2. Importa el fragmento y pasa el valor como propiedad.

    ```mdx destination-file.mdx
    ---
    title: "Una página de ejemplo"
    description: "Esta es una página de ejemplo que importa un fragmento con una variable en un bloque de código."
    ---
