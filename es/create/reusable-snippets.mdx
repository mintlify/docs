...
El uso de llaves en una instrucción `import` depende de cómo se exporta el fragmento. Importa el contenido renderizado de un archivo `.md` o `.mdx` como una exportación predeterminada sin llaves (`import MySnippet from "..."`). Usa llaves para exportaciones con nombre, como variables o componentes de React (`import { myName } from "..."` o `import { MyComponent } from "..."`).
