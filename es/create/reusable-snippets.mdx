...
El nombre que asignas en la instrucción `import` es la etiqueta JSX que usas para renderizar el fragmento. Debe ser un identificador de componente JSX válido: empieza con una letra mayúscula y usa PascalCase (por ejemplo, `MySnippet`, no `mySnippet` ni `my-snippet`). Los nombres que empiezan con minúscula se tratan como elementos HTML y no renderizan tu fragmento.
