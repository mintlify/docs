...
Un `Update` se muestra solo cuando cada una de sus etiquetas está incluida en la selección de filtros activos. Los `Update` sin etiquetas se ocultan cuando hay al menos un filtro activo. Para que un `Update` aparezca bajo varios filtros, incluye cada una de esas etiquetas en el `Update`.
