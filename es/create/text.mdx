...
De forma predeterminada, Mintlify genera los IDs de anclaje automáticamente a partir del texto del encabezado. Puedes sobreescribir el ID generado con uno personalizado usando la sintaxis `{#custom-id}`:
...
Los IDs personalizados se conservan exactamente como los escribes, incluidos los guiones bajos y las mayúsculas y minúsculas. Puedes usar camelCase, kebab-case, snake_case o cualquier combinación:
...
Estos encabezados son accesibles en `#quickStart`, `#setup_steps` y `#advanced-options`, respectivamente. Respeta la capitalización exacta al enlazar a un ID personalizado: los anclajes distinguen entre mayúsculas y minúsculas.

<Tip>
  Conservar la capitalización es útil cuando migras desde herramientas de documentación que usan anclajes camelCase o snake_case. Mantén tus IDs personalizados existentes para evitar romper enlaces entrantes.
</Tip>
