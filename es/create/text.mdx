...
De forma predeterminada, los IDs de anclaje se generan automáticamente a partir del texto del encabezado. Puedes sobreescribir el ID generado con uno personalizado usando la sintaxis `{#custom-id}`:
...
<Note>
  Los IDs de encabezado personalizados son compatibles con los niveles de encabezado 1 a 4 (`#` a `####`). Los niveles 5 y 6 no son compatibles.
</Note>
