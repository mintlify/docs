...
Para cambiar la configuración de una automatización activa, haz clic en el botón de configuración <Icon icon="settings" /> en su tarjeta para abrir su página de configuración. Usa el interruptor en el encabezado de la página para activar o desactivar la automatización y haz clic en **Save** para aplicar los cambios.
...
1. Abre la página [Automatizaciones](https://app.mintlify.com/products/automations) en tu dashboard.
2. Haz clic en el botón de configuración <Icon icon="settings" /> en la tarjeta de la automatización personalizada para abrir su página de configuración.
...
Las ejecuciones manuales guardan la configuración actual de la automatización antes de comenzar, cuentan para tu uso de créditos y aparecen en el historial de ejecuciones junto a las ejecuciones programadas.

La mayoría de las automatizaciones solo pueden ejecutarse manualmente cuando tienen un activador de programación. Si el botón de ejecución no está disponible, pasa el cursor sobre él para ver qué debes cambiar. La automatización **Translate content** siempre puede ejecutarse manualmente.

<div id="scope-a-manual-run">
### Delimitar el alcance de una ejecución manual
</div>

Una ejecución manual iniciada desde el botón de ejecución revisa todo lo que cambió desde la última ejecución completada de la automatización. Para ejecutar una automatización con un alcance diferente, pide al [agente](/es/editor/agent) que inicie la ejecución. El agente admite tres alcances:

- **Rango de fechas**: revisa los cambios de una ventana de tiempo específica en lugar de la predeterminada. Por ejemplo, `run the changelog automation over the last two weeks`.
- **Todo**: revisa todos los cambios sin restricción de fechas. Por ejemplo, `run the style guide automation across everything`.
- **Un único pull request**: se ejecuta a partir de un pull request de un repositorio de código fuente conectado. Por ejemplo, `run the update from code changes automation on PR #42 of acme/api`. Para automatizaciones con un activador de **Cambio de código**, el pull request debe estar en uno de los repositorios de origen configurados.

Una ejecución usa exactamente un alcance. Indicar un alcance también te permite ejecutar manualmente automatizaciones que no tienen un activador de programación. Las ejecuciones con alcance cuentan para el uso de créditos y aparecen en el historial de ejecuciones como cualquier otra ejecución.
...
Cada automatización mantiene un registro de las ejecuciones pasadas, incluido el estado y un resumen de los cambios realizados. Las ejecuciones que agregan cambios a una pull request abierta de una ejecución anterior aparecen como entradas propias en el historial de ejecuciones.
...
- **Modified**: La ejecución agregó sus cambios a una pull request abierta por una ejecución anterior.
...
Puedes actuar sobre las ejecuciones directamente desde la lista. Para las ejecuciones pendientes de revisión, haz clic en **Accept** para fusionar los cambios. Para las ejecuciones fallidas, haz clic en **Re-trigger** para iniciar la ejecución de nuevo.

Para ver el prompt de una ejecución, los archivos que leyó o modificó y cualquier pull request que haya abierto, abre el menú de acciones de la ejecución y selecciona **View run details**.
