...
Cuando seleccionas **Programación personalizada**, elige una de las siguientes opciones de programación:
...
Usa un valor preestablecido para una frecuencia recurrente común. Selecciona **Cron personalizado** cuando necesites una programación que no cubran los valores preestablecidos. El panel de configuración muestra una descripción en lenguaje natural para los valores preestablecidos. Muestra la entrada de cron solo cuando se selecciona **Cron personalizado**.

Las expresiones de cron personalizado utilizan cinco campos en este orden: `minute hour day month weekday`. Por ejemplo, la siguiente expresión ejecuta la automatización a las 9:00 UTC de lunes a viernes:

```text
0 9 * * 1-5
```

Mintlify valida la expresión antes de que la guardes. Una programación existente que no coincida con un valor preestablecido se abre con **Cron personalizado** seleccionado para que puedas revisar o editar la expresión.
