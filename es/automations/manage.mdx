...
4. Selecciona un alcance de ejecución y haz clic en **Run now**.

Las ejecuciones manuales guardan la configuración actual de la automatización antes de comenzar, cuentan para tu uso de créditos y aparecen en el historial de ejecuciones junto a las ejecuciones programadas. Después de iniciar una ejecución, haz clic en **View runs** en la confirmación para abrir el historial de ejecuciones filtrado por esa automatización.

Las automatizaciones con un activador de **Integración** no pueden ejecutarse manualmente. Se ejecutan cuando se dispara su activador.

<div id="scope-a-manual-run">
### Delimitar el alcance de una ejecución manual
</div>

Cada ejecución manual usa exactamente un alcance, que seleccionas al iniciar la ejecución:

- **Desde una fecha**: revisa el contenido que cambió desde la fecha seleccionada hasta ahora. De forma predeterminada, usa la última ejecución de la automatización.
- **Todo**: se ejecuta sobre todo tu sitio sin restricción de fechas. Esto suele tardar más y usar más créditos que una ejecución delimitada.
- **Pull requests específicos**: limita la ejecución a un pull request de un repositorio de código fuente conectado. Selecciona un repositorio y busca sus pull requests por título o número. Para automatizaciones con un activador de **Cambio de código**, el repositorio debe ser uno de los repositorios de origen configurados.

También puedes pedir al [agente](/es/editor/agent) que inicie una ejecución con alcance. Por ejemplo, `run the changelog automation over the last two weeks` o `run the update from code changes automation on PR #42 of acme/api`.
