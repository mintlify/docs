...
4. Selecciona un alcance de ejecución y haz clic en **Run now** para confirmar. Nada se ejecuta hasta que confirmes.

Cada ejecución manual cubre uno de tres alcances:

- **Since a date**: Cubre el contenido que cambió desde la fecha que elijas hasta ahora. De forma predeterminada, es la última ejecución de la automatización.
- **Everything**: Se ejecuta en todo tu sitio. Suele tardar más y consume bastante más de tu uso que una ejecución dirigida.
- **Specific pull request(s)**: Limita la ejecución a los pull requests que selecciones en un repositorio conectado.
...
Las automatizaciones con un activador de **Integración** no pueden ejecutarse manualmente. Se ejecutan cuando se produce su evento activador. Si el botón de ejecución no está disponible, pasa el cursor sobre él para ver por qué.
