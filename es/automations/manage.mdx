...
- **Mensaje en canal de Slack**: se ejecuta cuando alguien publica un nuevo mensaje de nivel superior en un canal de Slack seleccionado. Está disponible para automatizaciones personalizadas. Invita a la [aplicación Mintlify de Slack](/es/agent/slack) a cada canal que quieras seleccionar y, a continuación, elige los canales en el panel de configuración. Si la aplicación no es miembro de un canal seleccionado, el panel de configuración impide guardar hasta que la invites. Mintlify omite las respuestas en hilos, los mensajes de bots y los mensajes que mencionan al agente de Mintlify. Mintlify deduplica por ID de evento de Slack para que un mismo mensaje active cada automatización coincidente como máximo una vez. Cuando la automatización actúa, responde en el hilo desencadenante con un resumen y un enlace a cualquier pull request que abra. El detalle de la ejecución en el panel muestra el mensaje desencadenante con un enlace permanente a Slack.
...
Abre la configuración de la automatización y selecciona las integraciones en **Tools**. Puedes conectar una integración compartida desde el selector. Las integraciones personales solo están disponibles para su propietario al usar el agente de Slack y no pueden añadirse a automatizaciones.
