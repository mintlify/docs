...
description: "Restringe el acceso al dashboard de Mintlify a IP de confianza y controla la duración de las sesiones con tiempos de inactividad y duración máxima."
...
- **Dirección IP individual**: permite una única dirección, como `203.0.113.7` o `2001:db8::1`.
- **Rango CIDR**: permite un bloque de direcciones, como `10.0.0.0/8` o `2001:db8::/48`. Usa la notación CIDR para conceder acceso a toda una oficina o red VPN sin listar cada dirección de forma individual.
...
<Warning>
  Añade tu dirección IP actual a la lista de IP permitidas antes de activarla. Si tu dirección no está en la lista, puedes bloquearte el acceso al dashboard. La sección **Policies** muestra tu dirección IP actual para ayudarte a evitarlo.
</Warning>

1. En tu dashboard de Mintlify, ve a la página [Security](https://app.mintlify.com/settings/organization/access).
2. En la sección **Policies**, en **Allowed IP addresses**, introduce una dirección IP individual (por ejemplo, `203.0.113.7`) o un rango CIDR (por ejemplo, `10.0.0.0/8`).
3. Haz clic en **Add**.
...
1. En tu dashboard de Mintlify, ve a la página [Security](https://app.mintlify.com/settings/organization/access).
2. En la sección **Policies**, en **Session lifetimes**, activa **Maximum session lifetime** o **Idle session timeout**.
