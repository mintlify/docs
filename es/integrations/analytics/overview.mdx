...
<Note>
  Las integraciones de Analytics solo requieren claves de API públicas, que son accesibles para cualquier persona que visite tu sitio de documentación. Tus claves de API privadas, que se usan junto con las claves públicas en los proveedores de Analytics, deben permanecer privadas. No incluyas claves de API privadas en tu `docs.json`.
</Note>
