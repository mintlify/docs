...
<Note>
  Las integraciones de Analytics solo requieren claves públicas de API, que son accesibles para cualquier persona que visite tu sitio de documentación. Tus claves privadas de API, que se usan junto con las claves públicas en el lado de los proveedores de Analytics, deben mantenerse privadas. No incluyas claves privadas de API en tu `docs.json`.
</Note>
