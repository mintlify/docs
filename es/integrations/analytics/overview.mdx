...
Cuando agregas integraciones de Analytics a tu sitio de documentación, Mintlify registra interacciones de los usuarios como vistas de página, consultas de búsqueda, solicitudes en el área de pruebas de la API y envíos de comentarios. Después, Mintlify envía estos eventos a tus proveedores de Analytics. Puedes conectar cualquier cantidad de proveedores de Analytics compatibles agregando tus claves de API al archivo `docs.json`. Los eventos de Analytics se envían a tus proveedores tan pronto como los agregas a tu configuración, sin necesidad de configuración adicional.
...
Después de desplegar tu `docs.json` actualizado, carga una página de tu sitio en vivo y confirma que los eventos están llegando a la vista en tiempo real o de depuración de tu proveedor. Si los eventos no aparecen, comprueba que:

- Estás usando la clave pública o el ID del proveedor, no una clave privada ni de servidor.
- El nombre de la propiedad en `docs.json` coincide con el documentado por tu proveedor.
- El valor no está envuelto en comillas adicionales ni tiene espacios en blanco.
