...
Si el banner de Transcend no aparece o la analítica nunca se carga, comprueba lo siguiente:

- **Bundle ID**: Confirma que el `bundleId` de tu `docs.json` coincide con el deployment de tu dashboard de Transcend. Un error tipográfico o un bundle de un entorno diferente impide que Airgap se cargue.
- **Script de Airgap**: Abre las herramientas de desarrollo de tu navegador y revisa en la pestaña de red si hay una solicitud a `https://transcend-cdn.com/cm/<bundleId>/airgap.js`. Si la solicitud falla o devuelve un 404, el bundle ID no es válido o el deployment no está publicado en Transcend.
- **Nombre del propósito**: Si defines `analyticsPurpose`, confirma que coincide exactamente con el nombre del propósito en Transcend, incluidas mayúsculas y espacios. Un propósito que no coincide hace que Mintlify espere un consentimiento que Transcend nunca resuelve.
- **Errores de consola**: Busca errores de `airgap` o de Transcend en la consola del navegador. Suelen apuntar a un bundle mal configurado o a un script bloqueado.
- **Content Security Policy**: Si tu sitio define una CSP personalizada, asegúrate de que permite `transcend-cdn.com` para los scripts.
