...
<AccordionGroup>
  ...
  <Accordion title="Mi build falla con &quot;Failed to fetch OpenAPI file for anchor or tab&quot;">
    Este error significa que Mintlify no pudo descargar el documento de OpenAPI en la URL del campo `openapi` de tu `docs.json` durante el build. Las causas más comunes incluyen:

    - El host es inaccesible o solo se resuelve desde una red privada.
    - La URL requiere autenticación (un token, una cookie de sesión o una lista de IPs permitidas).
    - El certificado no es válido o el dominio tiene un problema de DNS.
    - El origen devolvió un 5xx transitorio o agotó el tiempo de espera.
    - La especificación se estaba republicando en el momento en que se ejecutó el build, por lo que la URL sirvió una respuesta parcial o vacía.

    Para acotar la causa, reproduce la descarga fuera de Mintlify desde una red pública:
    ...
    Luego valida el documento descargado localmente con el CLI de Mint:

    ```bash
    mint validate
    ```

    Si la URL no es accesible desde la internet pública, o solo es accesible con credenciales, cambia a uno de estos patrones:

    - **Incluye la especificación en tu repositorio de documentación.** Este es el patrón recomendado cuando la URL de origen está detrás de autenticación. Apunta el campo `openapi` a la ruta relativa del repositorio (por ejemplo, `"openapi": "openapi.json"`) y actualiza el archivo en el mismo commit que modifica tu API.
    - **Sirve la especificación desde una URL HTTPS pública y estable.** Alójala en un CDN o en un bucket de almacenamiento de objetos que no requiera autenticación, tenga un certificado TLS válido y devuelva el documento completo en cada solicitud.

    Si generas la especificación en CI, secuencia el pipeline para que la especificación esté completamente publicada en su URL pública antes de llamar al endpoint de la API [Trigger deployment](/es/api/update/trigger). Disparar el despliegue primero hace que el build descargue una especificación desactualizada o faltante.
  </Accordion>
  ...
  <Accordion title="Las solicitudes del área de pruebas de la API no funcionan">
    ...
    Como alternativa, si tu proxy inverso impide aceptar solicitudes `POST`, puedes configurar Mintlify para enviar solicitudes directamente a tu backend con el ajuste `api.playground.proxy` en el `docs.json`. Consulta la [documentación de configuración](/es/organize/settings-api) para obtener más detalles. Al usar esta configuración, deberás configurar CORS en tu servidor, ya que las solicitudes llegarán directamente desde los navegadores de los usuarios en lugar de pasar por tu proxy.
  </Accordion>
  ...
  <Accordion title="La descripción del campo Authorization es genérica o poco útil">
    Mintlify genera automáticamente una descripción para el campo `Authorization` en el área de pruebas de la API según el tipo de esquema de seguridad. Puedes sustituirla añadiendo una `description` al esquema de seguridad en tu especificación de OpenAPI.
      ...
      "components": {
        "securitySchemes": {
          "bearerAuth": {
            "type": "http",
            "scheme": "bearer",
            "description": "Tu API key desde el [dashboard](https://example.com/dashboard). Envíala como `Bearer <key>`."
          }
        }
      }
    }
    ...
    La `description` admite Markdown y reemplaza el texto generado automáticamente en cada endpoint que use el esquema de seguridad.
  </Accordion>
