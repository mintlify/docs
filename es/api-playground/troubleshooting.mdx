...
<AccordionGroup>
  ...
  <Accordion title="Una de mis páginas de OpenAPI está completamente en blanco">
    ...
    Mintlify tolera las diferencias de barra diagonal final — `/users/{id}` y `/users/{id}/` se resuelven al mismo endpoint. Sin embargo, si tu especificación define ambos como operaciones distintas, la coincidencia exacta tiene prioridad.
