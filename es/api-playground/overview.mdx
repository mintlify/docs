...
<ResponseField name="playground" type="object">
  ...
  <Expandable title="playground" defaultOpen="True">
    ...
    <ResponseField name="proxy" type="boolean" defaultOpen="True">
      ...
      Cuando es `true`, las solicitudes del playground se enrutan a través de los servidores de Mintlify. Cuando es `false`, el playground envía las solicitudes directamente desde el navegador a tu API. Establece `false` cuando tu API acepta solicitudes directas del navegador y no necesitas que Mintlify actúe como proxy del tráfico. Por ejemplo, cuando tu API requiere encabezados específicos que no pueden reenviarse a través del proxy o cuando necesitas que la solicitud se origine directamente desde el navegador del usuario para fines de autenticación.
    </ResponseField>
  </Expandable>
</ResponseField>
...
- **Audio**: Se renderizan con un reproductor de audio integrado (`audio/*`).
- **Video**: Se renderizan con un reproductor de video integrado (`video/*`). Cualquier respuesta con un tipo de contenido `video/*`, como `video/mp4` o `video/webm`, se muestra como un video reproducible directamente en el playground.
