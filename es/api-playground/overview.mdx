...
<ResponseField name="playground" type="object">
  ...
  <Expandable title="playground" defaultOpen="True">
    ...
    <ResponseField name="credentials" type="boolean">
      Indica si se deben incluir cookies y encabezados de autenticación en solicitudes de origen cruzado cuando el proxy está deshabilitado. El valor predeterminado es `false`.

      Cuando es `true` y `proxy` es `false`, el playground envía solicitudes con credenciales (cookies, encabezados de autorización o certificados de cliente TLS) incluidas. Esto es útil cuando tu API depende de autenticación basada en cookies o tokens de sesión y necesitas que el navegador los adjunte automáticamente. Esta opción no tiene efecto cuando `proxy` es `true`.
    </ResponseField>
  </Expandable>
</ResponseField>
