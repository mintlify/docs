...
description: "Genera documentación de API interactiva desde archivos OpenAPI con páginas de endpoints, constructores de solicitudes y navegación automáticos."
...
En una especificación de OpenAPI, las rutas como `/users/{id}` o `/` identifican los distintos endpoints de la API. La URL base indica dónde anexan esas rutas los clientes. Para obtener más información sobre cómo configurar el campo `servers`, consulta [API Server and Base Path](https://swagger.io/docs/specification/api-host-and-base-path/) en la documentación de OpenAPI.
...
La extensión `x-default` admite los tipos de esquema de seguridad `apiKey` y `http` bearer. El valor aparece como la entrada predeterminada en los campos de autenticación del playground. El prellenado para esquemas de seguridad se aplica de forma incondicional y no requiere configuración adicional.

También puedes usar `x-default` en otras propiedades de esquema en tu especificación de OpenAPI para establecer un valor predeterminado en el playground de la API sin afectar el campo `default` en la definición del esquema. A diferencia de los esquemas de seguridad, el prellenado para propiedades que no son de esquema de seguridad solo tiene efecto cuando [`api.examples.prefill`](/es/organize/settings-api) está establecido en `true` en tu [`docs.json`](/es/api-playground/overview#example-configuration).
...
Los overlays son archivos JSON o YAML independientes que describen cambios que se aplican a tu especificación de OpenAPI. Usa overlays para modificar una especificación sin editar su archivo de origen. Esto es útil cuando la especificación es generada por otra herramienta o mantenida por otro equipo. Los usos comunes incluyen renombrar rutas, establecer URLs de servidor, eliminar endpoints internos y mejorar las descripciones para la documentación.

Mintlify es compatible con las versiones 1.0 y 1.1 de la [OpenAPI Overlay Specification](https://spec.openapis.org/overlay/v1.1.0.html). Mintlify aplica los overlays después de analizar una especificación y antes de validarla. Las páginas de endpoints generadas, la navegación, las referencias `openapi` en el frontmatter y `mint validate` usan el documento transformado.

<div id="create-an-overlay-document">
  ### Crea un documento de overlay
</div>

Un documento de overlay tiene un campo de versión `overlay` de nivel superior, un objeto `info` con un `title` y una `version`, y un array `actions`. Cada acción selecciona nodos de la especificación con una expresión `target` de [JSONPath RFC 9535](https://www.rfc-editor.org/rfc/rfc9535) y aplica un modificador:
...
* `copy`: Copia el nodo seleccionado por otra expresión JSONPath en cada nodo seleccionado. El modificador `copy` requiere Overlay 1.1.
...
El campo opcional `extends` vincula un overlay a una especificación para el [descubrimiento automático](#auto-discover-overlays). Establece `extends` en una ruta relativa al archivo del overlay o en la URL exacta que tu `docs.json` usa para hacer referencia a una especificación alojada.

<div id="reference-overlays-in-your-docsjson">
  ### Haz referencia a los overlays en tu docs.json
</div>

Enumera los overlays de forma explícita usando la forma de objeto del campo `openapi`. La forma de objeto funciona en cualquier lugar donde se acepte el campo `openapi`, incluso dentro de arrays. Los overlays se aplican en el orden en que los enumeras.
...
Las rutas de los overlays deben apuntar a archivos dentro de tu repositorio de documentación. Las URLs de los overlays deben usar `https`. Si dos entradas `openapi` hacen referencia a la misma especificación con listas de `overlays` diferentes, la compilación falla.

<div id="auto-discover-overlays">
  ### Descubrimiento automático de overlays
</div>

Mintlify trata cualquier archivo JSON o YAML de tu repositorio con una clave `overlay` de nivel superior como un documento de overlay. Un overlay cuyo campo `extends` se resuelve en una especificación de tu documentación se aplica automáticamente a esa especificación. Los overlays descubiertos automáticamente se aplican en orden alfabético según sus rutas de archivo. Los overlays sin un campo `extends` nunca se aplican automáticamente.

Una lista `overlays` explícita para una especificación reemplaza el descubrimiento automático para esa especificación. Establece `"overlays": []` para deshabilitar todos los overlays de una especificación, incluidos los descubiertos automáticamente.

Los overlays explícitos y los descubiertos automáticamente fallan de forma distinta. Si un overlay de una lista `overlays` explícita no se puede cargar o aplicar, la especificación no pasa la validación y el despliegue reporta un error de especificación. Si falla un overlay descubierto automáticamente, Mintlify lo omite y publica la especificación sin él.

<div id="example-rename-a-path">
  ### Ejemplo: Renombrar una ruta
</div>

La especificación de Overlay no tiene una acción de movimiento. Para renombrar una ruta, crea la nueva ruta con `update`, copia el elemento de ruta existente en ella con `copy` y luego elimina la ruta antigua con `remove`.
...
Haz referencia a la especificación transformada en toda tu documentación. Por ejemplo, una referencia en el frontmatter de una página debe usar la ruta posterior al overlay: `openapi: "POST /credit/accounts"`.

Cuando ejecutas `mint dev`, editar o eliminar un archivo de overlay reconstruye las especificaciones afectadas. `mint validate` y `mint openapi-check` validan el documento transformado, por lo que los errores de validación hacen referencia a tu especificación después de aplicar los overlays.
...
Cuando está habilitada, al hacer clic en la opción se descarga directamente tu especificación de OpenAPI. Los despliegues con varias especificaciones las reciben empaquetadas como `api-specs.zip`. En los despliegues protegidos por `auth` o `userAuth`, solo los lectores autenticados pueden descargar la especificación.

<Warning>
  La especificación de OpenAPI descargada no está filtrada y no respeta los [grupos de autenticación](/es/deploy/authentication-setup). Cualquier lector autenticado que pueda abrir el menú contextual recibe la especificación completa, incluidos endpoints y esquemas que de otro modo estarían ocultos para su grupo. No habilites `download-spec` en un sitio autenticado si tu especificación de OpenAPI contiene endpoints o campos que consideras sensibles.
</Warning>
...
<div id="collapse-playground-fields">
  ### Contraer los campos del playground
</div>

Contrae los campos de tipo objeto en el playground de la API de forma predeterminada usando `x-mint: playground` con `expand: false` en cualquier operación. Las secciones de la solicitud como Authorization, Headers, Query, Path y Body siempre permanecen expandidas, y el objeto de nivel superior del body permanece expandido, pero los campos de tipo objeto anidados dentro de ellas comienzan contraídos para que los lectores expandan solo los campos con los que quieren interactuar. Si `expand` no está establecido, los campos de tipo objeto se expanden de forma predeterminada.
...
Anota los parámetros en la referencia de la API y en el playground con etiquetas de píldora personalizadas usando `x-mint.pre` y `x-mint.post` en cualquier esquema. Las píldoras definidas con `x-mint.pre` se renderizan antes del nombre del parámetro, y las píldoras definidas con `x-mint.post` se renderizan después, junto con las píldoras integradas de Mintlify como `required`, `read-only` y `write-only`.
...
El campo `openapi` acepta una ruta en tu repositorio de documentación o una URL a un documento OpenAPI alojado. Las especificaciones alojadas deben ser accesibles desde la Internet pública.
...
<Warning>
  **Especifica siempre la ruta del archivo de OpenAPI cuando tengas varias especificaciones de OpenAPI en tu repositorio.**

  Mintlify carga todas las especificaciones de OpenAPI de tu repositorio de documentación, incluso si no se referencian en `docs.json`. Cuando una página MDX usa `openapi: "MÉTODO /ruta"` sin una ruta de archivo, Mintlify busca una operación coincidente en todas las especificaciones cargadas. Si más de una especificación contiene el mismo método y ruta, gana la primera coincidencia según el orden alfabético del nombre del archivo, que puede no ser la especificación que esperas.

  Para evitar ambigüedades, haz una de las siguientes acciones:

  - Incluye la ruta del archivo de la especificación en el frontmatter: `openapi: "ruta/al/openapi-correcto.json POST /v1/endpoint"`.
  - Usa la [generación automática de páginas](#auto-populate-api-pages) referenciando el archivo de OpenAPI en `docs.json`, que siempre asigna los endpoints a la especificación correcta.
  - Elimina la ruta en conflicto de la otra especificación.
  - Elimina de tu repositorio los archivos de OpenAPI que ya no necesites.
</Warning>
