...
<Note>
  Los campos de tipo enum solo se prerrellenan en el playground de la API cuando defines explícitamente `example` o `default` en la propiedad. Mintlify no prerrellena automáticamente el primer valor del enum cuando ninguno está definido, por lo que el campo aparece vacío hasta que el usuario selecciona un valor.
</Note>
