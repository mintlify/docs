...
Controla cómo los endpoints individuales de tu API se exponen como herramientas en tu [servidor MCP](/ai/model-context-protocol) usando `x-mint: mcp`. De forma predeterminada, todos los endpoints están disponibles como herramientas MCP. Usa esta extensión para deshabilitar endpoints específicos, o para personalizar el nombre y la descripción de la herramienta que ven las aplicaciones de IA al conectarse a tu servidor MCP.

La extensión `x-mint: mcp` acepta las siguientes propiedades:

| Propiedad | Tipo | Descripción |
| :--- | :--- | :--- |
| `enabled` | boolean | Si el endpoint está disponible como herramienta MCP. Por defecto es `true`. |
| `name` | string | Nombre personalizado para la herramienta MCP. Reemplaza el nombre generado por defecto. |
| `description` | string | Descripción personalizada para la herramienta MCP. Reemplaza la descripción generada por defecto. |
...
Para ocultar un endpoint de tu servidor MCP manteniendo su visibilidad en la documentación, establece `enabled` en `false`:
...
<Note>
  La extensión `x-mint: mcp` reemplaza la extensión obsoleta `x-mcp`. Si ambas están presentes en una operación, `x-mint: mcp` tiene prioridad.
</Note>
