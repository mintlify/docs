---
title: "Configura OpenAPI para documentación de API interactiva"
...
Mintlify muestra un campo de archivo en el playground de la API cuando el esquema del cuerpo de tu solicitud identifica un archivo. Usa campos de archivo para endpoints que aceptan imágenes, documentos, archivos comprimidos u otros datos binarios.

Para OpenAPI 3.1, establece `type` en `string` y añade un `contentMediaType`.

Añade `contentEncoding: base64` o `contentEncoding: base64url` cuando tu API espera un archivo codificado en base64.

Para OpenAPI 3.0, usa `format: binary` o `format: base64` en su lugar.
...
El playground admite tipos de medios comunes de imágenes, audio, vídeo y aplicaciones binarias. Los usuarios pueden arrastrar un archivo al campo de carga o hacer clic en el campo para seleccionar uno.
...
Cuando está habilitada, al hacer clic en la opción se descarga directamente tu especificación de OpenAPI. Los despliegues con varias especificaciones las reciben empaquetadas como `api-specs.zip`. Mintlify oculta la opción en los despliegues protegidos por `auth` o `userAuth` para evitar filtrar el contenido de las especificaciones desde documentación autenticada.
