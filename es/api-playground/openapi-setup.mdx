...
<div id="use-a-localhost-specification">
  ### Usar una especificación en localhost
</div>

Durante el desarrollo local, puedes generar un documento de OpenAPI desde un servicio que se ejecute en tu equipo y hacer referencia a su URL de localhost:
...
Pasa `--local-schema` para permitir que la CLI obtenga la especificación mediante HTTP:

```bash
mint dev --local-schema
```

Para validar la misma configuración sin iniciar una vista previa, ejecuta `mint validate --local-schema`. Los despliegues de producción requieren que las URL de OpenAPI usen HTTPS.
