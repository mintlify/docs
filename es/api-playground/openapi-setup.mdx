...
Controla qué operaciones de la API se exponen como herramientas a través de tu [servidor MCP](/ai/model-context-protocol) usando `x-mint: mcp`. Por defecto, todas las operaciones se incluyen. Usa esta extensión para deshabilitar operaciones específicas o personalizar cómo aparecen para herramientas de IA.
      ...
      "get": {
        ...
        "x-mint": {
          "mcp": {
            "enabled": true,
            "name": "list_users",
            "description": "Search and list user accounts with optional filters"
          }
        }
      }
    }
...
El objeto `mcp` acepta las siguientes propiedades:

| Propiedad | Tipo | Descripción |
| :---------- | :------ | :---------- |
| `enabled` | boolean | Si se expone esta operación como herramienta MCP. Por defecto es `true`. |
| `name` | string | Nombre personalizado para la herramienta MCP. Sobrescribe el nombre generado por defecto. |
| `description` | string | Descripción personalizada para la herramienta MCP. Sobrescribe la `description` de la operación. |

Para excluir un endpoint de tu servidor MCP, establece `enabled` en `false`:
...
<Note>
  La extensión `x-mint: mcp` reemplaza la extensión heredada `x-mcp`. Si ambas están presentes en una operación, `x-mint: mcp` tiene prioridad.
</Note>
