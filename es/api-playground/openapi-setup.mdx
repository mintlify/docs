...
Controla qué operaciones de la API se exponen como herramientas a través de tu [servidor MCP](/ai/model-context-protocol) usando `x-mint: mcp`. Por defecto, las operaciones no se exponen como herramientas MCP. Agrega `x-mint: mcp` con `enabled: true` para habilitar operaciones específicas.

<ParamField body="enabled" type="boolean">
  Si se debe exponer esta operación como una herramienta MCP. El valor predeterminado es `false`.
</ParamField>

<ParamField body="name" type="string">
  Nombre personalizado para la herramienta MCP. Si no se establece, el nombre se deriva de la operación.
</ParamField>

<ParamField body="description" type="string">
  Descripción personalizada para la herramienta MCP. Si no se establece, la descripción se deriva del resumen o descripción de la operación.
</ParamField>
      ...
      "get": {
        ...
        "x-mint": {
          "mcp": {
            "enabled": true,
            "name": "list-users",
            "description": "Recuperar una lista paginada de todos los usuarios"
          }
        },
        "responses": {
          "200": {
...
<Note>
  La extensión obsoleta `x-mcp` aún es compatible, pero `x-mint: mcp` tiene prioridad. Usa `x-mint: mcp` para nuevas configuraciones.
</Note>
