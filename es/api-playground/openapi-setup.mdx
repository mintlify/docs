...
Controla qué endpoints de API se exponen como herramientas MCP (Model Context Protocol) usando `x-mint: mcp`. Esto permite que los agentes de IA descubran y usen directamente tus endpoints de API.
...
<ResponseField name="x-mint.mcp.enabled" type="boolean">
  Si se expone este endpoint como una herramienta MCP.
</ResponseField>

<ResponseField name="x-mint.mcp.name" type="string">
  Nombre personalizado para la herramienta MCP. Si no se especifica, se genera un nombre a partir de la operación.
</ResponseField>

<ResponseField name="x-mint.mcp.description" type="string">
  Descripción personalizada para la herramienta MCP. Si no se especifica, se utiliza el resumen o la descripción de la operación.
</ResponseField>
