...
Declara `sdk` en un grupo para acotar las páginas generadas a una subsección de una pestaña existente. Los grupos y páginas anidados heredan la configuración de `sdk` de la pestaña o grupo más cercano por encima de ellos. Un grupo que declara su propio `sdk` reemplaza por completo la configuración heredada; las dos configuraciones no se combinan.
...
Un grupo con `sdk` también puede incluir `pages`. Las páginas escritas manualmente aparecen antes que los grupos de referencia generados.

<Note>
  Puedes declarar `sdk` en una [pestaña](/es/organize/navigation#tabs) o en un [grupo](/es/organize/navigation#groups). Una pestaña con `sdk` puede incluir `groups`, pero no otras estructuras de navegación, como `pages`, `versions` o `languages`, y tampoco puede incluir una propiedad `openapi`, `asyncapi` o `graphql`. Un grupo con `sdk` puede incluir `pages` y grupos anidados, pero no puede incluir una propiedad `graphql`.
</Note>
...
Agrega varias pestañas o grupos para documentar varias bibliotecas, por ejemplo, grupos hermanos para las versiones estable y beta del mismo SDK. Usa un `directory` único para cada biblioteca para evitar colisiones de rutas.
...
Mintlify agrega los grupos de navegación generados después de cualquier `groups` en la pestaña, o después de las `pages` existentes cuando `sdk` se declara en un grupo. Los grupos varían según el formato y pueden representar módulos, paquetes, espacios de nombres o tipos de símbolos.
...
Incluye la página en la navegación de tu `docs.json`, como cualquier otra página. Mintlify solo compone las páginas que aparecen en la navegación.

Cuando existe al menos una página con frontmatter `sdk` válido bajo una pestaña o grupo que declara `sdk`, Mintlify omite la generación masiva para esa pestaña o grupo y renderiza solo las páginas que tú escribas. Mueve la página fuera de esa pestaña o grupo si quieres que el resto de la biblioteca se genere automáticamente.
...
La forma de cadena se interpreta como `[source] kind name`. Omite `source` para heredarlo de la configuración `sdk` de la pestaña o del grupo. La forma de cadena no tiene un campo `format` y siempre lo hereda, por lo que solo funciona en páginas bajo una pestaña o grupo con `sdk`; usa la forma de objeto en cualquier otro caso. Para símbolos de tipo `method` y `property`, cualifica el nombre con un elemento superior, por ejemplo `method Auth0Client.loginWithRedirect`.

Si el frontmatter de la página omite `title` o `description`, Mintlify usa el título y la descripción generados para el símbolo.
