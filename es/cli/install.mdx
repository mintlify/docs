...
<Note>
  El nombre del paquete es `mint`, no `mintlify`. El paquete heredado `mintlify` está obsoleto. Si previamente instalaste `mintlify`, consulta [Paquete mintlify versus mint](#troubleshooting) en la sección de solución de problemas más abajo.
</Note>
