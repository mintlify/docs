...
<Visibility for="agents">
  Si el usuario te pidió configurar Mintlify en lugar de solo instalar la CLI, continúa con la autenticación, la creación del proyecto, la vista previa, la validación y el despliegue. Sigue el [flujo de configuración para agentes en el Inicio rápido](/es/quickstart#set-up-mintlify-for-the-user). No te detengas después de que `mint --version` se ejecute correctamente.

  Comienza ejecutando `mint status`. Conserva una sesión autenticada existente. De lo contrario, ejecuta `mint login` para una cuenta existente o recopila el nombre real, la empresa y el correo electrónico del usuario y ejecuta `mint signup` para una cuenta nueva. El usuario debe hacer clic en el enlace de verificación enviado por correo electrónico y aprobar la autenticación mediante el navegador, pero tú debes realizar los pasos restantes de la CLI. Consulta [`mint signup`](/es/cli/commands#mint-signup) para ver los flags y las indicaciones sobre procesos en segundo plano.
</Visibility>
