...
| `--threshold` | Marca una página como fallida cuando su fracción escrita por IA alcance este valor, de `0` a `1`. Por defecto, `0.5`. |
...
Cada página comprobada consume un crédito de IA. Las páginas con menos de 50 palabras se omiten y no se cobran. El comando termina con el código `1` cuando alguna página se marca en el umbral o por encima, y con `0` cuando todas las páginas están limpias, para que puedas ejecutarlo en un bucle de reescritura o en CI.
