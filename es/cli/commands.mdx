...
Ejecuta verificaciones de preparación para agentes contra un sitio de documentación público. Devuelve una puntuación y un desglose de verificaciones individuales. Requiere autenticación con `mint login`.
...
| Flag | Descripción |
| --- | --- |
| `--url` | URL del sitio de documentación a verificar. Obligatorio. |
| `--format` | Formato de salida: `table` (predeterminado, salida con colores), `plain` (TSV para canalización), o `json`. |
