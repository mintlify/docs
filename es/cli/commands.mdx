---
title: "Comandos"
description: "Referencia completa de todos los comandos y flags de la CLI de Mintlify."
...
Abre una ventana del navegador para completar la autenticación. Si el navegador no se abre, la CLI muestra una URL para abrir manualmente y un campo para pegar el código de autorización. Después de un inicio de sesión exitoso, la CLI te solicita seleccionar un proyecto predeterminado cuando tu organización tiene varios proyectos, o lo establece automáticamente si solo hay uno. Las credenciales se guardan en `~/.config/mintlify/config.json`.
...
Muestra la cuenta y la organización asociadas a tu sesión actual.
...
| Flag | Descripción |
| --- | --- |
| `--subdomain` | Subdominio de la documentación. Por defecto usa el valor establecido con `mint config set subdomain`. |
...
Crea un nuevo proyecto de documentación a partir de la plantilla inicial de Mintlify.
