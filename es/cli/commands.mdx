...
Crear una cuenta de Mintlify desde la CLI.
...
Ejecútalo sin flags para registrarte de forma interactiva — la CLI te solicita tu nombre, apellido, nombre de la empresa y correo electrónico, y luego abre una ventana del navegador para verificar tu correo. Después de hacer clic en el enlace de verificación, la autenticación se completa automáticamente y tus credenciales se guardan en `~/.config/mintlify/config.json`. Una vez registrado, has iniciado sesión y puedes ejecutar `mint status` para confirmar la sesión.

Pasa los flags anteriores para omitir las indicaciones y registrarte de forma no interactiva, lo que resulta útil cuando un agente de codificación ejecuta el comando en tu nombre:

```bash
mint signup --firstName Ada --lastName Lovelace --company Analytical --email ada@example.com
```

Cuando el registro se complete, visita el [dashboard de Mintlify](https://dashboard.mintlify.com) para conectar tu repositorio de GitHub y terminar de configurar tu despliegue.
