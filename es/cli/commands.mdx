...
| `openApiSpec` | Cualquier especificación OpenAPI o Swagger servida en una ruta estándar es válida. Esta verificación pasa cuando no se sirve ninguna especificación y solo falla cuando un archivo de especificación servido no es un documento OpenAPI o Swagger válido. |
...
| `pagesHaveMarkdown` | Las páginas muestreadas del mapa del sitio tienen una contraparte Markdown correspondiente (`path.md` o `path/index.md`) que sirve contenido no HTML. Pasa con 90% o más de cobertura. |
