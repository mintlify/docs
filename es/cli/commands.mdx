...
Para instalar la CLI, consulta [Instalar la CLI](/es/cli/install).
...
| Flag | Descripción |
| --- | --- |
| `--telemetry`, `-t` | Habilitar o deshabilitar la telemetría de uso. |
...
| `--groups` | Lista separada por espacios de grupos de usuarios a simular para la previsualización. Por ejemplo, `--groups admin user`. |
...
Si tienes más de un proyecto, la CLI te solicita que selecciones uno predeterminado después de iniciar sesión. Puedes cambiar el proyecto predeterminado más tarde con `mint config set subdomain <subdomain>`.
...
Añade un [dominio personalizado](/es/customize/custom-domain) a tu proyecto desde la terminal. Requiere autenticación con `mint login`.
...
<Note>
  Si algunos registros `TXT` aún se están generando cuando el comando finaliza, revisa la página de [Configuración de dominio personalizado](https://app.mintlify.com/settings/project/custom-domain) en tu panel para ver los valores restantes.
</Note>
...
| --- | --- | --- |
| `subdomain` | Subdominio predeterminado de la documentación. | `mint dev`, `mint analytics`, `mint add-domain`, `mint score` |
| `dateFrom` | Fecha de inicio predeterminada para las consultas de analíticas. | `mint analytics` |
| `dateTo` | Fecha de fin predeterminada para las consultas de analíticas. | `mint analytics` |
...
El comando analiza los archivos `.mdx` y `.md` en busca de enlaces y excluye los archivos que coinciden con los patrones de [.mintignore](/es/organize/mintignore). No se verifican los enlaces dentro de archivos de especificación OpenAPI (`.yaml`, `.yml`, `.json`). Los enlaces que apuntan a archivos ignorados se reportan como rotos.
...
| Flag | Descripción |
| --- | --- |
| `--groups` | Lista separada por espacios de grupos de usuarios a simular para la validación. Por ejemplo, `--groups admin user`. |
...
`mint test` analiza tu contenido en busca de bloques de código, genera pruebas unitarias para validarlos y ejecuta las pruebas con un agente de codificación local.

<div id="prerequisites">
  ### Requisitos previos
</div>

- Autentícate con `mint login`.
- Instala el SDK del agente de codificación que quieras usar:
...
Solo las páginas incluidas en la navegación de tu `docs.json` aparecen para su selección.
...
El comando `mint test` escribe en dos ubicaciones de tu proyecto:

| Ruta | Contenido |
| --- | --- |
| `tests/mint-test/<run-id>/` | Los proyectos de prueba generados, un directorio por agente y página. |
| `.mintlify/test/` | Informes e historial de ejecuciones, incluido `runs/<run-id>.json` para cada ejecución. |

Agrega ambas rutas a tu `.gitignore` si no quieres confirmar los artefactos de prueba.

Cuando termina una ejecución, el comando imprime un resumen de resultados, por ejemplo `mint test passed: 8 passed, 0 failed, 0 agent errors`. El comando sale con el código `0` cuando todas las pruebas pasan y con `1` en caso contrario.
...
| `--groups` | Lista separada por espacios de grupos de usuarios para incluir páginas restringidas. Por ejemplo, `--groups admin user`. |
...
<Tip>
  Ejecutar `mint format` puede generar muchos cambios según el tamaño de tu proyecto. Para evitar conflictos de fusión y no introducir cambios en otro trabajo en curso, primero resuelve todas las ramas pendientes importantes. Luego crea una nueva rama para los cambios de formato y ejecuta `mint format` en esa rama.
</Tip>

Cuando termina, el comando muestra cuántos archivos MDX se reformatearon y cuántos no se pudieron analizar. Si algún archivo falla, el comando termina con el código `1` e imprime la ruta del archivo y el error, de modo que puedas ejecutarlo en CI para exigir un formato coherente. Consulta [Instalar en CI](/es/cli/install#install-in-ci) para ver un pipeline de ejemplo.
...
La CLI recopila telemetría de uso para ayudar a mejorar Mintlify. Los datos de telemetría incluyen el nombre del comando, la versión de la CLI, el sistema operativo y la arquitectura. Si has iniciado sesión, los eventos de telemetría también incluyen el correo electrónico de tu cuenta. El uso sin sesión iniciada permanece anónimo, y cerrar sesión elimina tu correo electrónico almacenado. Mintlify **no** recopila contenido del proyecto ni rutas de archivos.
