...
Formatea todos los archivos `.mdx` del directorio actual siguiendo el estilo canónico de Mintlify. El comando analiza cada archivo con el mismo convertidor que utiliza el editor web y luego lo reescribe in situ si la salida canónica difiere. Los archivos formateados coinciden con la salida que escribe el editor web al publicar, por lo que publicar desde el editor en un repositorio formateado produce un diff que contiene solo las líneas editadas.

```bash
mint format
```

Ejecuta el comando desde la raíz de tu proyecto de documentación. Recorre cada subdirectorio, omitiendo las rutas que coincidan con `.gitignore` y cualquier regla de exclusión de Mintlify. Los archivos que ya coincidan con la salida canónica se dejan intactos. Si el formateo eliminaría el frontmatter de un archivo, el comando deja ese archivo sin cambios y lo reporta como un fallo.

<Warning>
  `mint format` reescribe los archivos in situ. Confirma o guarda tus cambios antes de ejecutarlo para que puedas revisar el diff. La primera ejecución en un proyecto existente puede producir un diff único de gran tamaño al normalizar los archivos, incluyendo las comillas del frontmatter, el espaciado de listas y la sangría de comentarios. Después de esa ejecución, las ejecuciones posteriores dejan intactos los archivos ya formateados.
</Warning>

Cuando termina, el comando muestra cuántos archivos MDX se reformatearon y cuántos fallaron. Si algún archivo falla, el comando termina con el código `1` e imprime la ruta del archivo y el error, de modo que puedas ejecutarlo en CI para exigir un formato coherente.
