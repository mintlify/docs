...
<Steps>
  ...
  <Step title="Set up the connection">
    ...
    2. Selecciona la rama desde la cual desplegar.
    3. Si tu instancia de GitLab es autoalojada, activa **Set up as self-hosted** e ingresa la URL de tu host (por ejemplo, `https://gitlab.example.com`). Consulta [GitLab autoalojado](#gitlab-autoalojado) para más detalles.
    4. Si tus docs están en un subdirectorio, activa **Set up as monorepo** e ingresa la ruta al directorio que contiene tu archivo `docs.json` (por ejemplo, `apps/docs`).
...
<div id="gitlab-autoalojado">
  ## GitLab autoalojado
</div>

Si ejecutas una instancia de GitLab autoalojada en lugar de usar `gitlab.com`, puedes conectarla a Mintlify proporcionando la URL de tu instancia durante la configuración.

<Steps>
  <Step title="Activar el modo autoalojado">
    En el [Dashboard de Mintlify](https://dashboard.mintlify.com/settings/deployment/git-settings), activa **Set up as self-hosted** en el panel de configuración de GitLab.
  </Step>
  <Step title="Ingresar la URL del host">
    Ingresa la URL base de tu instancia de GitLab en el campo **Host URL**. Por ejemplo:
  ...
  <Step title="Completar la configuración">
    Ingresa tu Project ID y access token como se describe en [Configura la conexión](#set-up-the-connection), luego haz clic en **Guardar cambios**.
  </Step>
</Steps>

<Note>
  Al usar una instancia de GitLab autoalojada, asegúrate de que tu instancia sea accesible desde Internet para que Mintlify pueda comunicarse con ella. La URL del webhook sigue siendo `https://leaves.mintlify.com/gitlab-webhook`.
</Note>