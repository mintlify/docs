...
Si estás ejecutando una instancia de GitLab autoalojada, puedes conectarla a Mintlify proporcionando la URL del host de tu instancia.

<Steps>
  <Step title="Habilitar la configuración autoalojada">
    En el [Dashboard de Mintlify](https://dashboard.mintlify.com/settings/deployment/git-settings), activa la opción **Set up as self-hosted** en el panel de configuración de GitLab.
  </Step>
  <Step title="Ingresa la URL de tu host">
    Introduce la URL de tu instancia de GitLab autoalojada en el campo **Host URL** (por ejemplo, `https://gitlab.example.com`).
  </Step>
  <Step title="Completa la configuración">
    Rellena tu Project ID, access token y rama de despliegue como se describe en la [configuración de la conexión anterior](#set-up-the-connection), luego haz clic en **Guardar cambios**.
  </Step>
</Steps>

<Note>
  Los pasos de configuración del access token y del webhook son los mismos para instancias de GitLab autoalojadas. Asegúrate de que tu instancia sea accesible desde Internet para que Mintlify pueda conectarse.
</Note>
