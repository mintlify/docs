...
description: "Configura los encabezados de CSP para permitir recursos de Mintlify sin comprometer la seguridad en proxies inversos, firewalls y redes que aplican políticas estrictas."
---

Content Security Policy (CSP) es un estándar de seguridad que ayuda a prevenir ataques de cross-site scripting (XSS) al controlar qué recursos puede cargar una página web. Mintlify proporciona una CSP predeterminada que protege a la mayoría de los sitios. Si alojas tu documentación detrás de un proxy inverso o un firewall que sobrescribe la CSP predeterminada, puede que necesites configurar los encabezados de CSP para que las funciones se ejecuten correctamente.
