...
description: "Soluciona solicitudes bloqueadas y resuelve conflictos de reglas WAF de Cloudflare al servir documentación de Mintlify a través de Cloudflare Workers."
...
2. Verifica que tu Cloudflare Worker (si usas una subruta personalizada) establezca el encabezado `Host` a tu destino `<subdomain>.mintlify.site` en lugar de pasar el encabezado `Host` original de la solicitud
