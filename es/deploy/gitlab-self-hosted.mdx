...
description: "Conecta una instancia autoalojada de GitLab con Mintlify mediante OAuth para clonar repositorios, enviar commits y abrir merge requests automatizados."
...
- Una organización de Mintlify en el **plan Pro o superior**.
- Una organización de Mintlify que tenga habilitada la función de GitLab autoalojado. Contacta con soporte si no ves la sección **Self-hosted GitLab** en la página del dashboard [Git settings](https://app.mintlify.com/settings/project/git-settings).
...
Mintlify se conecta a tu instancia de GitLab por HTTPS (puerto 443) para intercambiar tokens de OAuth y llamar a la API de GitLab. Estas conexiones se originan desde la dirección IP estática de egreso de Mintlify: `54.242.90.151`.

Si un firewall o una lista de IPs permitidas restringe el acceso a tu instancia de GitLab, permite el tráfico entrante HTTPS (puerto 443) desde `54.242.90.151`.

<Note>
  Las ejecuciones de automatización clonan y hacen push desde una infraestructura sandbox independiente que, por defecto, usa direcciones IP dinámicas. Si tu instancia de GitLab aplica una lista de IPs permitidas, consulta [Solucionar ejecuciones del agente bloqueadas por una lista de IPs permitidas](/es/help-center/agent-blocked-by-git-provider-ip-allow-list).
</Note>

### Saliente: de tu instancia de GitLab a Mintlify

Tu instancia de GitLab envía eventos de webhook a Mintlify por HTTPS (puerto 443) a `app.mintlify.com`. Autoriza este tráfico por nombre de host. Mintlify no publica una lista estática de direcciones IP para este endpoint.
...
<Steps>
  ...
  <Step title="Authorize">
    Haz clic en **Authorize self-hosted GitLab**. GitLab te redirige a tu instancia, te pide iniciar sesión si es necesario y muestra una pantalla de consentimiento con los scopes solicitados.
...
Si necesitas cambiar el client secret de la aplicación registrada —por ejemplo, después de renovarlo en GitLab—, elimina la instancia guardada en Mintlify y vuelve a añadirla con los nuevos valores. Primero debes revocar las conexiones OAuth activas. De lo contrario, Mintlify bloquea la eliminación.
...
<div id="invalid_client-after-authorizing">
  ### La autorización falla con `invalid_client`
</div>
...
GitLab se negó a registrar el webhook porque su lista de permitidos de solicitudes salientes rechazó la URL que envió Mintlify (`https://app.mintlify.com/gitlab-oauth-webhook`). Las instancias autoalojadas rechazan las URLs "locales" a menos que el administrador las permita explícitamente.
...
- La aplicación tiene la marca **Trusted** en GitLab. Las aplicaciones marcadas como confiables omiten el consentimiento para todos los usuarios. Desmarca **Trusted** en la configuración de la aplicación si quieres que los usuarios vean y confirmen los scopes.
