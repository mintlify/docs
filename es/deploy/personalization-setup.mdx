...
Para todos los métodos de personalización, el cierre de sesión se maneja en el cliente. Cuando los usuarios hacen clic en el botón de cierre de sesión, Mintlify borra sus datos de sesión almacenados en el navegador. No necesitas implementar un endpoint de cierre de sesión para la personalización.

Si necesitas limitar cuánto tiempo conserva Mintlify los datos de personalización, establece el campo `expiresAt` en la respuesta con la información del usuario. Cuando la hora actual supere `expiresAt`, los datos almacenados del usuario se eliminarán automáticamente y el usuario deberá autenticarse de nuevo para acceder a contenido personalizado. Esto no revoca sesiones ni tokens en tu proveedor de identidad. Gestiona la revocación en el servidor en tu propio sistema, si es necesario.
...
<Tabs>
  ...
  <Tab title="OAuth 2.0">
    ...
    **Crea un endpoint de información de usuario** en `api.foo.com/docs/user-info`, que requiera un token de acceso de OAuth con el scope `provider.users.docs` y responda con los datos personalizados del usuario:
    ...
    <Note>
      Controla la duración de la sesión con el campo `expiresAt` en la respuesta de información de usuario. Esta es una marca de tiempo de Unix (segundos desde la época Unix) que indica cuándo debe expirar la sesión. Cuando la sesión expira, los datos almacenados del usuario se eliminan y el usuario debe iniciar sesión nuevamente.
    </Note>
    ...
    * Tu documentación está alojada en el mismo domain o subdomain que tu dashboard.
    ...
    <Steps>
      <Step title="Create user info API endpoint.">
        ...
        * Responda a solicitudes `GET`.
        * Use tus cookies de sesión existentes para identificar a los usuarios. Mintlify llama a este endpoint con credenciales incluidas, por lo que tus cookies de sesión se envían automáticamente. No se usa ningún encabezado `Authorization`.
