...
description: "Instala y configura la aplicación GitHub de Mintlify en tu instancia de GitHub Enterprise Server para despliegues automáticos de documentación y sincronización."
...
La integración requiere tráfico en ambas direcciones entre tu instancia de GHES y Mintlify. Configura tu firewall para permitir las dos conexiones siguientes.

#### Saliente: de tu instancia de GHES a Mintlify

Tu instancia de GHES envía eventos de webhook a Mintlify por HTTPS (puerto 443) a `leaves.mintlify.com`.

* Permite el tráfico saliente HTTPS (puerto 443) hacia `leaves.mintlify.com`. Autoriza este tráfico por nombre de host. Mintlify no publica una lista estática de direcciones IP para este endpoint, y sus direcciones IP subyacentes pueden cambiar.
* Asegúrate de que tu instancia de GHES pueda resolver `leaves.mintlify.com` mediante DNS. Si tus servidores usan un resolutor DNS interno, confirma que resuelva nombres de host públicos.

#### Entrante: de Mintlify a tu instancia de GHES

Mintlify se conecta a tu instancia de GHES por HTTPS (puerto 443) para llamar a la API de GitHub, informar sobre estados de check runs y despliegues, y leer contenido del repositorio. Estas conexiones se originan desde la dirección IP estática de egreso de Mintlify: `54.242.90.151`.

Si un firewall o una lista de IPs permitidas restringe el acceso a tu instancia de GHES, permite el tráfico entrante HTTPS (puerto 443) desde `54.242.90.151`.

<Note>
  Las ejecuciones del [agente](/es/agent) clonan y hacen push desde una infraestructura sandbox independiente que, por defecto, usa direcciones IP dinámicas. Si tu instancia de GHES aplica una lista de IPs permitidas, consulta [Solucionar ejecuciones del agente bloqueadas por una lista de IPs permitidas](/es/help-center/agent-blocked-by-git-provider-ip-allow-list).
</Note>
...
<Steps>
  <Step title="Configurar permisos del repositorio">
    Establece los siguientes permisos para la aplicación. La aplicación no necesita permisos de organización, cuenta ni Enterprise:
...
<Steps>
  <Step title="Crear la app">
    ...
    GitHub te redirige a la página de configuración de la app.
  </Step>
...
* Installation owner: El nombre de la organización de GitHub propietaria de la instalación de la aplicación
* Clave privada: Todo el contenido del archivo `.pem` (compártelo mediante transferencia segura de archivos)
...
* Repository names: Nombres de los repositorios específicos donde has instalado la aplicación
...
* Integrar tu entorno GHES con un proyecto existente de Mintlify.
* Integrar tu entorno GHES con un nuevo proyecto de Mintlify que aprovisionamos para ti.

Después de integrar tu entorno GHES con un proyecto de Mintlify, podrás habilitar los webhooks de tu GitHub App.
...
- Comprueba que el secreto del webhook coincida con el que configuraste.
...
- Asegúrate de que nuestro servidor reciba y reconozca los webhooks con un código de respuesta 200.
- Otorgaste los permisos necesarios durante la instalación.
</Accordion>
...
- Avísanos de cualquier cambio de permisos para que podamos aconsejarte sobre cualquier paso adicional que puedas necesitar.
</Accordion>
