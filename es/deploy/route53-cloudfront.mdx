...
sidebarTitle: "AWS"
description: "Despliega tu documentación de Mintlify en una subruta de AWS combinando el enrutamiento DNS de Route 53, una distribución de CloudFront y políticas de caché."
...
Crea comportamientos para las rutas de verificación de domain de Vercel con un **Path pattern** de `/.well-known/*` y establece **Origin and origin groups** en la URL de tu documentación.
...
Si sigues los pasos anteriores, tu distribución tiene seis comportamientos:
...
Confirma que cada comportamiento apunte al origen correcto: la URL `.mintlify.site` para las rutas de la documentación, los assets, la API y `/.well-known/*`, y tu URL de staging para `Default (*)`.
