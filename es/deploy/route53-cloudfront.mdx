...
3. En Origin domain, ingresa `[SUBDOMAIN].mintlify.cc`, donde `[SUBDOMAIN]` es el subdomain único de tu proyecto.

<Frame>
    ![Página «Create distribution» de CloudFront mostrando «acme.mintlify.cc» como Origin domain.](/images/cloudfront/origin-name.png)
  </Frame>
...
- **Si un usuario llega a tu subruta personalizada**, redirigir a `[SUBDOMAIN].mintlify.cc`.
...
Crea un comportamiento con un **Path pattern** de la subruta que elijas, por ejemplo `/docs`, con **Origin and origin groups** apuntando a la URL `.mintlify.cc` (en nuestro caso `acme.mintlify.cc`).
...
<Frame>
  ![Página "Create behavior" de CloudFront con un "Path pattern" de "/docs/*" y "Origin and origin groups" apuntando a la URL `acme.mintlify.cc`.](/images/cloudfront/behavior-1.png)
</Frame>
...
Crea un comportamiento con un **Path pattern** que sea la subruta que elijas seguida de `/*`, por ejemplo `/docs/*`, y con **Origin and origin groups** apuntando a la misma URL `.mintlify.cc`.
