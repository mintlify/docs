...
<Tabs>
  ...
  <Tab title="OAuth 2.0">
    ...
    * Posibilidad de crear un endpoint de API accesible mediante tokens de acceso de OAuth (opcional, para habilitar funciones de personalización).
    ...
    <Steps>
      <Step title="Configura los ajustes de OAuth.">
        ...
        * **Logout URL**: La URL de cierre de sesión nativa de tu proveedor de OAuth. Mintlify redirige a los usuarios a esta URL con una petición `GET` cuando cierran sesión. Mintlify no agrega parámetros de query, así que incluye cualquier parámetro (por ejemplo, `returnTo`) directamente en la URL. Configura una página a la cual redirigir a los usuarios tras un cierre de sesión exitoso.
      ...
      <Step title="Crea tu endpoint de información de usuario (opcional).">
        ...
        * Devuelva datos de usuario en el formato `User`. Consulta [Formato de datos de usuario](/es/deploy/personalization-setup#user-data-format) para más información.
        ...
        Mintlify llama a este endpoint con el token de acceso de OAuth para obtener la información del usuario. No se envían parámetros de query adicionales.
    ...
    **Crea un endpoint de información de usuario** en `api.foo.com/docs/user-info`, que requiere un token de acceso de OAuth con el ámbito `provider.users.docs` y devuelve:
    ...
    <Note>
      Controla la duración de la sesión con el campo `expiresAt` en la respuesta de información de usuario. Se trata de una marca de tiempo Unix (segundos desde la época Unix) que indica cuándo debe expirar la sesión. Consulta [Formato de datos de usuario](/es/deploy/personalization-setup#user-data-format) para más detalles.
    </Note>
