...
El interruptor de visibilidad del sitio del panel solo ofrece **Public** o **Private**. La autenticación parcial no es un modo aparte. Para configurarla, cambia la visibilidad del sitio a **Private** para que todas las páginas requieran autenticación de forma predeterminada. Luego marca las páginas o grupos de navegación que quieras dejar abiertos a todos con `public: true`. Consulta [Hacer páginas públicas](#make-pages-public) para la sintaxis del frontmatter y de `docs.json`.
