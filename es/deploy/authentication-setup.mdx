    ...
    * Tus requisitos de seguridad permiten compartir contraseñas entre usuarios.
    ...
    <Steps>
      <Step title="Crea una contraseña.">
        1. En tu dashboard, ve a [Access](https://app.mintlify.com/settings/project/authentication).
        2. Establece **Visibility** en **Private**.
        3. Establece **Method** en **Password**.
    ...
    * Todas las personas que necesiten acceder a tu sitio deben ser miembros de tu organización de Mintlify.
    ...
    <Steps>
      <Step title="Habilita la autenticación privada.">
        1. En tu dashboard, ve a [Access](https://app.mintlify.com/settings/project/authentication).
        2. Establece **Visibility** en **Private**.
        3. Establece **Method** en **Authenticated**.
        4. Haz clic en **Save**.
    ...
    * Capacidad para crear un endpoint de API accesible mediante tokens de acceso OAuth (opcional, para habilitar el control de acceso basado en grupos).
    ...
    <Steps>
      ...
      <Step title="Configura tu servidor OAuth.">
        1. Copia la **Redirect URL** de tus [ajustes de Access](https://app.mintlify.com/settings/project/authentication).
      ...
      <Step title="Crea tu endpoint de información de usuario para el acceso por grupos (opcional).">
        ...
        * Devuelva los datos de usuario en el formato `User`. Consulta [Formato de datos de usuario](#user-data-format) para obtener más información.
        ...
        Agrega la URL de este endpoint al campo **Info API URL** en tus [ajustes de Access](https://app.mintlify.com/settings/project/authentication).
      </Step>
    </Steps>
    ...
    Al configurar los claims del token de OAuth para tu proyecto, usa valores como estos:
...
Para bloquear groups específicos en una página, usa objetos de regla con `includes` y `excludes` en lugar de una lista de nombres de groups.

* `includes`: Los usuarios deben pertenecer al menos a uno de estos groups. Omite `includes` para permitir el acceso a todos los usuarios autenticados que no estén excluidos.
* `excludes`: Los usuarios de cualquiera de estos groups no pueden acceder a la página, aunque también pertenezcan a un group incluido.

Mintlify comprueba `excludes` antes que `includes`. Los usuarios excluidos reciben un error 404.

```mdx Example page for staff, excluding contractors highlight={3}
---
title: "Hoja de ruta interna"
groups: [{ includes: ["staff"], excludes: ["contractor"] }]
---
```

En este ejemplo, un usuario que pertenece a los groups `staff` y `contractor` no puede acceder a la página.

Para ocultar una página a un group y mantenerla abierta para todos los demás usuarios autenticados, omite `includes`.

```mdx Example page hidden from contractors highlight={3}
---
title: "Guía de compensación"
groups: [{ excludes: ["contractor"] }]
---
```

Un valor de `groups` es una lista de nombres de groups o una lista de objetos de regla. No puedes combinar ambas formas en una misma lista. El comodín `*` no está permitido en `excludes`. Una lista `includes` vacía no es válida. Para denegar el acceso a todos, establece `groups: []`.

Si una página tiene objetos de regla no válidos, Mintlify no actualiza tu implementación y muestra `Page groups rules must be { includes, excludes } objects and * is not allowed in excludes. Deployment not updated`. El editor web tampoco permite guardar una página con objetos de regla no válidos.
...
* Las páginas con `public: true` son accesibles para cualquier persona. Mintlify ignora los nombres de groups y `includes` en las páginas públicas.
* Las páginas con `public: true` y `excludes` son accesibles para cualquier persona, excepto para los usuarios con sesión iniciada que pertenezcan a un group excluido. Los visitantes anónimos pueden ver la página. Los usuarios excluidos con sesión iniciada reciben un error 404. Si la sesión de un usuario ha expirado y no se puede renovar, Mintlify lo redirige para que inicie sesión.
