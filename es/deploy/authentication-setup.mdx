...
description: "Configura la autenticación de usuarios para tu sitio con OAuth, JWT, contraseña o enlaces compartidos para controlar el acceso a páginas y referencias de API."
keywords: ['authentication', 'auth', 'OAuth', 'JWT', 'password']
---

<Info>
  La autenticación del dashboard de Mintlify está disponible en todos los planes.

  La autenticación por contraseña, OAuth y JWT requiere un [plan Enterprise](https://mintlify.com/pricing?ref=authentication).
</Info>

<Warning>
  La autenticación solo está disponible para sitios alojados en un dominio personalizado o subdominio de Mintlify (por ejemplo, `docs.ejemplo.com` o `ejemplo.mintlify.dev`). La autenticación **no es compatible** cuando se usa una [ruta base personalizada](/es/deploy/docs-subpath) (por ejemplo, `ejemplo.com/docs`).
</Warning>
