...
<Tabs>
  ...
  <Tab title="OAuth 2.0">
    ...
    * Capacidad para crear un endpoint de API accesible mediante tokens de acceso OAuth (opcional, para habilitar el control de acceso basado en grupos).
    ...
    Cuando se configuran los claims del token, Mintlify lee los grupos directamente del token devuelto por tu intercambio de tokens y no llama a la Info API URL, incluso si hay una configurada. Para configurar los claims del token en tu implementación, ponte en contacto con el soporte de Mintlify.

    Usa valores como estos:
