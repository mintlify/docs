...
Si tu sitio de documentación muestra errores 500 después de unos segundos o experimenta una navegación lenta, es posible que el firewall de Cloudflare esté bloqueando las solicitudes a los recursos de Mintlify.
...
- Errores 403 en la consola del navegador para solicitudes a rutas `/mintlify-assets/*`.
- Mensajes de desafío de seguridad de Cloudflare sobre "malformed data" o "suspicious URL patterns".

<div id="root-cause">
  ### Causa raíz
</div>

El Web Application Firewall (WAF) y el Bot Fight Mode de Cloudflare pueden marcar las solicitudes de recursos de Mintlify como sospechosas debido a:
...
5. Configura la regla con estos ajustes:
    ...
    **Cuando las solicitudes entrantes coincidan con:**
    - Campo: `Hostname`
    - Operador: `equals`
    - Valor: `docs.yourdomain.com` (reemplázalo con tu dominio de documentación real)
...
- Verifica que hayas activado y desplegado la regla.

<div id="common-mistakes">
  ### Errores comunes
</div>

- Usar el operador `contains` con `/mintlify-assets/*`. El `*` se trata como un carácter literal, no como un comodín.
- Usar `equals` para URI Path. Esto solo coincide con la ruta exacta `/mintlify-assets/` y no con las subrutas.
- Olvidar omitir el Bot Fight Mode. Inclúyelo explícitamente en la acción de skip.
- Establecer el hostname incorrecto. Debe coincidir con el dominio real de tu documentación.
...
1. Revisa el registro **Security > Events** de Cloudflare en busca de solicitudes bloqueadas.
2. Verifica que tu Cloudflare Worker (si usas una subruta personalizada) establezca el encabezado `Host` con el destino `<subdomain>.mintlify.site` en lugar de pasar el encabezado `Host` original de la solicitud.
3. Establece temporalmente el Security Level en "Essentially Off" para confirmar que Cloudflare es la causa.
