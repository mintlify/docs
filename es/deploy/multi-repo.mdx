...
Añade una entrada `sourceRef` a un arreglo de navegación en el `docs.json` de la fuente base. El valor debe coincidir con la ruta de URL de otra fuente de repositorio en la implementación.
...
En este ejemplo, la fuente base en `/docs` muestra una pestaña `Guides` e inserta en línea los `tabs` de una segunda fuente configurada en `/api`.
...
- Los valores de `sourceRef` deben coincidir con la ruta de URL de una fuente de repositorio en la misma implementación.
