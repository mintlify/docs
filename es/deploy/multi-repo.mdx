...
De forma predeterminada, Mintlify envuelve la navegación de cada repositorio en una sección de producto de nivel superior. Si quieres más control sobre dónde aparece la navegación de otra fuente, usa una entrada `sourceRef` en el `docs.json` de la fuente base para insertar en línea la navegación de otro repositorio en una ubicación específica.

Usa `sourceRef` cuando quieras:

- Mostrar varios repositorios bajo un mismo conjunto de `tabs` en lugar de secciones de producto separadas.
- Colocar la navegación de un repositorio dentro de un `group`, `tab` o anclaje global concreto.
- Combinar contenido de varios repositorios en una única estructura de navegación.

<div id="how-sourceref-works">
  ### Cómo funciona `sourceRef`
</div>

Añade una entrada `sourceRef` a un arreglo de navegación en el `docs.json` de la fuente base. El valor debe ser el identificador `owner/repo` de otra fuente de repositorio en la implementación.

```json
{ "sourceRef": "acme/api-docs" }
```

La comparación no distingue entre mayúsculas y minúsculas, y las barras iniciales o finales se ignoran. El valor debe tener exactamente dos segmentos (`owner/repo`); otros formatos hacen que la combinación falle.

Cuando Mintlify construye la navegación combinada, reemplaza cada entrada `sourceRef` por el arreglo de navegación correspondiente del `docs.json` del repositorio referenciado. La clave del arreglo padre determina qué arreglo lee Mintlify de la fuente referenciada. Por ejemplo, un `sourceRef` dentro de un arreglo `tabs` se reemplaza por el arreglo `tabs` del `docs.json` de la fuente referenciada.

`sourceRef` es compatible dentro de cualquier arreglo de navegación, incluidos `tabs`, `groups`, `pages`, `products` y los arreglos bajo `navigation.global`.

Mintlify antepone a cada ruta de la fuente referenciada la ruta de URL de esa fuente, de modo que las páginas se resuelven en la ubicación correcta sin configuración adicional.

<div id="example">
  ### Ejemplo
</div>

En este ejemplo, la fuente base (propiedad de `acme` en el repositorio `docs`) se sirve en `/docs` y muestra una pestaña `Guides`. El `sourceRef` inserta en línea los `tabs` de una segunda fuente (`acme/api-docs`) configurada en `/api`.

`docs.json` de la fuente base:
...
`docs.json` de la fuente referenciada en la ruta de URL `api`:
...
Después de que se construye la implementación, la navegación combinada queda así:
...
- Los valores de `sourceRef` deben usar el formato `owner/repo` y coincidir con una fuente de repositorio configurada en la misma implementación.
- La fuente referenciada debe definir un arreglo de navegación bajo la misma clave que el arreglo padre. Por ejemplo, un `sourceRef` dentro de `tabs` requiere que la fuente referenciada defina `navigation.tabs`.
- Dos fuentes de repositorio en la misma implementación no pueden compartir el mismo `owner/repo`. Si lo hacen, `sourceRef` no puede resolverse de forma inequívoca y la combinación falla.
- Las entradas `sourceRef` no pueden formar un ciclo. Una fuente no puede referenciarse a sí misma, y dos fuentes no pueden referenciarse entre sí.
...
Las entradas `sourceRef` se resuelven durante una implementación multi-repositorio, cuando Mintlify tiene acceso a todas las fuentes. La vista previa local con `mint dev` solo ve un único repositorio, por lo que no puede insertar en línea la navegación referenciada.

Cuando ejecutas `mint dev` en un repositorio cuyo `docs.json` contiene entradas `sourceRef`, Mintlify elimina esas entradas de la vista previa y registra una advertencia. El resto de la navegación se renderiza con normalidad, pero las secciones que provienen de otra fuente faltan hasta que hagas la implementación.
