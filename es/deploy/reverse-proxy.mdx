...
- **Hospedar en `/docs`**: Usa `mintlify.dev` como destino del proxy. Activa la opción **Host at `/docs`** en la página de [configuración de domain personalizado](https://dashboard.mintlify.com/settings/deployment/custom-domain) de tu dashboard. Esta es una configuración más sencilla con menos rutas.
...
Utiliza esta configuración cuando quieras servir la documentación en la ruta `/docs` de tu domain.

Antes de configurar tu proxy inverso:

1. Navega a [Configuración de domain personalizado](https://dashboard.mintlify.com/settings/deployment/custom-domain) en tu dashboard.
2. Activa la opción **Alojar en `/docs`**.
3. Ingresa tu domain y selecciona **Agregar domain**.

<Warning>
  Activar **Alojar en `/docs`** detiene la invalidación de caché en `mintlify.app`. Debes hacer proxy hacia `mintlify.dev` para que aparezcan las actualizaciones.
</Warning>
...
Redirige estas rutas a tu subdomain de Mintlify:
...
Las rutas `/.well-known/skills/*` y `/skill.md` son opcionales. Inclúyelas solo si quieres servir archivos de habilidades de IA en rutas raíz como `your-domain.com/skills.md` en lugar de bajo tu ruta `/docs`, como `your-domain.com/docs/skills.md`.
...
Cuando necesites una subruta diferente de `/docs` (como `/help` o `/resources`), usa la siguiente configuración de enrutamiento.
...
**Causa**: Tienes **Host at `/docs`** habilitado en tu dashboard, pero tu proxy inverso apunta a `mintlify.app` en lugar de `mintlify.dev`.
...
**Causa**: El proxy inverso está reenviando el encabezado `Host` o falta el encabezado `Origin`.
