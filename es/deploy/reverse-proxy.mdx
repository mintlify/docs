...
- **Alojar en `/docs`**: Usa `mintlify.cc` como destino del proxy. Activa la opción **Host at `/docs`** en la página [Custom domain setup](https://dashboard.mintlify.com/settings/deployment/custom-domain) de tu dashboard. Esta es una configuración más sencilla con menos rutas.
...
<Warning>
  Cuando activas **Host at `/docs`**, la URL canónica de tu documentación se convierte en `<your-subdomain>.mintlify.cc`. La invalidación de caché se detiene en `mintlify.app` y debes dirigir el proxy a `mintlify.cc` para que aparezcan las actualizaciones.
</Warning>
...
| Ruta                              | Destino                             | Caché    |
| --------------------------------- | ----------------------------------- | -------- |
| `/docs`                           | `<your-subdomain>.mintlify.cc/docs` | Sin caché |
| `/docs/*`                         | `<your-subdomain>.mintlify.cc/docs` | Sin caché |
| `/.well-known/vercel/*`           | `<your-subdomain>.mintlify.cc`      | Sin caché |
| `/.well-known/skills/*` (opcional)        | `<your-subdomain>.mintlify.cc/docs` | Sin caché |
| `/.well-known/agent-skills/*` (opcional)  | `<your-subdomain>.mintlify.cc/docs` | Sin caché |
| `/skill.md` (opcional)                    | `<your-subdomain>.mintlify.cc/docs` | Sin caché |
| `/llms.txt` (opcional)            | `<your-subdomain>.mintlify.cc/docs` | Sin caché |
| `/llms-full.txt` (opcional)       | `<your-subdomain>.mintlify.cc/docs` | Sin caché |
...
* **Origin**: Contiene el subdomain de destino `<your-subdomain>.mintlify.cc`
...
**Causa**: Tienes **Host at `/docs`** activado en tu dashboard, pero tu proxy inverso apunta a `mintlify.app` en lugar de `mintlify.cc`.

**Solución**: Actualiza la configuración de tu proxy inverso para que apunte a `<your-subdomain>.mintlify.cc` en lugar de `<your-subdomain>.mintlify.app`.
...
- Configura el encabezado `Origin` en tu subdominio de Mintlify (`mintlify.cc` para una subruta `/docs` o `mintlify.app` para una subruta diferente)
