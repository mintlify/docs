...
Mintlify admite dos configuraciones de proxy inverso en función de tus requisitos de subruta. Ambas apuntan al hostname `<your-subdomain>.mintlify.site`.
...
- **Subruta personalizada**: Úsala cuando necesites una subruta distinta de `/docs` (por ejemplo, `/help` o `/resources`). Este enfoque requiere reglas de enrutamiento adicionales.
...
<Warning>
  Cuando activas **Host at `/docs`**, la URL canónica de tu documentación se convierte en `<your-subdomain>.mintlify.site`. Dirige el proxy a ese hostname para que las invalidaciones de caché y las actualizaciones lleguen a tu sitio.
</Warning>
...
**Causa**: Tu proxy inverso apunta a un hostname distinto de tu subdominio canónico de Mintlify, por lo que las invalidaciones de caché no llegan a tu sitio.
