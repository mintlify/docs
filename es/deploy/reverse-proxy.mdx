...
- **Alojar en `/docs`**: Usa `mintlify.site` como destino del proxy. Activa la opción **Host at `/docs`** en la página [Custom domain setup](https://dashboard.mintlify.com/settings/deployment/custom-domain) de tu dashboard. Esta es una configuración más sencilla con menos rutas.
- **Subruta personalizada**: Usa `mintlify.site` como destino del proxy. Este enfoque admite cualquier subruta y requiere reglas de enrutamiento adicionales.
...
<Warning>
  Cuando activas **Host at `/docs`**, la URL canónica de tu documentación se convierte en `<your-subdomain>.mintlify.site`. La invalidación de caché se detiene en `mintlify.site` y debes dirigir el proxy a `mintlify.site` para que aparezcan las actualizaciones.
</Warning>
...
**Causa**: Tienes **Host at `/docs`** activado en tu dashboard, pero tu proxy inverso apunta a `mintlify.site` en lugar de `mintlify.site`.

**Solución**: Actualiza la configuración de tu proxy inverso para que apunte a `<your-subdomain>.mintlify.site` en lugar de `<your-subdomain>.mintlify.site`.
...
- Configura el encabezado `Origin` en tu subdominio de Mintlify (`mintlify.site` para una subruta `/docs` o `mintlify.site` para una subruta diferente)
