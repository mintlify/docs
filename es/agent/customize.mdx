...
Crea un archivo `AGENTS.md` en un directorio `.mintlify/` en la raíz de tu documentación para personalizar el comportamiento del agente (también se acepta `Agents.md`).

<Warning>
  Coloca tu archivo `AGENTS.md` dentro del directorio `.mintlify/` (`.mintlify/AGENTS.md`), no en la raíz de tu proyecto. Los archivos en la raíz de tu proyecto son accesibles públicamente en tu sitio de documentación, lo que significa que tus instrucciones personalizadas serían visibles para cualquiera. El directorio `.mintlify/` no se sirve públicamente.
</Warning>
