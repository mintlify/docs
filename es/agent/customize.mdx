...
description: "Personaliza el comportamiento del agente con un archivo `AGENTS.md` para controlar cómo gestiona las tareas de documentación y tus convenciones."
...
Añade todas las instrucciones que quieras que el agente siga. El agente agrega estas instrucciones a su prompt del sistema, por lo que se aplican a todas las tareas, tanto si usas el agente en Slack como mediante la API.

<Note>
  El [agente integrado](/es/editor/agent) del editor no lee `AGENTS.md`. Para dar al agente del editor una guía persistente, configura las [instrucciones de IA](/es/editor/settings#ai-instructions) en la configuración del editor.
</Note>
