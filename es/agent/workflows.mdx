...
<Note>
  Los flujos de trabajo omiten automáticamente los eventos de push causados por sus propias solicitudes de extracción fusionadas. Esto evita que un flujo de trabajo se reactive a sí mismo en un bucle infinito.
</Note>
...
De forma predeterminada, el agente abre una solicitud de extracción por cada ejecución del flujo de trabajo para que puedas revisar los cambios antes de que se publiquen. Establece `automerge: true` para fusionar automáticamente la solicitud de extracción mediante squash sin necesidad de aprobación manual. Esto te proporciona un registro de los cambios en el historial de solicitudes de extracción de tu repositorio, a la vez que automatiza el paso de fusión.
...
Se ejecuta semanalmente para comprobar si hay páginas con metadata faltante o deficiente y abrir una solicitud de extracción con mejoras. Este flujo de trabajo de ejemplo comprueba si faltan o son deficientes los frontmatter `title` y `description`. Edita el flujo de trabajo para comprobar otros problemas de metadata o contenido que consideres prioritarios para tu documentación.
...
Criterios de éxito: Todas las páginas tienen un título de menos de 60 caracteres y una descripción de entre 50 y 160 caracteres que resuma con precisión el contenido de la página en lenguaje sencillo.
