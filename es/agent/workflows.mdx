...
Envía mensajes de Slack cuando un flujo de trabajo se completa o falla. Las notificaciones incluyen el estado del flujo de trabajo, un enlace a la solicitud de extracción y un resumen de los cambios.
...
Puedes especificar los destinatarios de las notificaciones por nombre o por ID:

| Campo | Descripción |
|-------|-------------|
| `channels` | Nombres de canales a notificar (con o sin el prefijo `#`). |
| `channel_ids` | IDs de canales a notificar. |
| `users` | Nombres de usuario, nombres de visualización o nombres reales para enviar DM. |
| `user_ids` | IDs de usuario de Slack para enviar DM. |

<Note>
Las notificaciones de Slack requieren una instalación de Slack en tu organización. Conecta Slack en la página de [integración de Slack](https://dashboard.mintlify.com/settings/organization/integrations/slack) de tu Dashboard.
</Note>
...
Se ejecuta semanalmente para generar un registro de cambios y notificar a tu equipo en Slack cuando el flujo de trabajo se completa. Este ejemplo muestra cómo combinar flujos de trabajo programados con notificaciones de Slack.
