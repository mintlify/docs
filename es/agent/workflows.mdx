...
### Desactivación automática cuando se agotan los créditos

Si una ejecución de un flujo de trabajo falla porque tu organización se queda sin créditos de Mintlify, Mintlify desactiva automáticamente todos los flujos de trabajo activos del proyecto para evitar más ejecuciones fallidas. Todos los miembros de la organización reciben un correo electrónico que les notifica que los flujos de trabajo se han pausado y que incluye un enlace a la página de facturación.

Para reanudar la automatización:

1. Añade créditos o actualiza tu plan desde la página [Billing](https://dashboard.mintlify.com/settings/organization/plans) de tu dashboard.
2. Abre la página [Workflows](https://dashboard.mintlify.com/products/workflows?tab=workflows) y vuelve a habilitar cada flujo de trabajo que quieras ejecutar.

Al volver a habilitar un flujo de trabajo se borra el estado de desactivación por facturación. Los flujos de trabajo programados recalculan su próxima ejecución a partir del momento en que los vuelves a habilitar.
