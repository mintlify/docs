...
El agente se ejecuta en un sandbox aislado con acceso restringido a internet. Al escribir prompts, solo haz referencia a las herramientas disponibles para el agente.

- Utilidades de shell estándar (`grep`, `sed`, `awk`, `curl` y otras)
- `git` y la CLI de GitHub (`gh`)
- La CLI de Mintlify (`mint`)
- Node.js y Bun

El agente no puede instalar paquetes o herramientas adicionales en tiempo de ejecución. Los registros de paquetes y otros servicios externos no son accesibles desde el sandbox.

Los prompts que indican al agente ejecutar herramientas no disponibles pueden producir resultados inesperados o fallar.
