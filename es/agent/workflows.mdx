...
De forma predeterminada, el agente abre una solicitud de extracción por cada ejecución del flujo de trabajo para que puedas revisar los cambios antes de que se publiquen. Establece `automerge: true` para fusionar automáticamente la solicitud de extracción después de crearla.

```yaml
automerge: true
```

Cuando `automerge` está habilitado, el flujo de trabajo aún crea una solicitud de extracción, ejecuta cualquier verificación configurada y luego la fusiona. Esto te proporciona un registro de cambios en el historial de solicitudes de extracción de tu repositorio mientras automatiza el paso de fusión.
