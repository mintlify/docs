...
Para repositorios de GitLab, conecta tus proyectos a través de [GitLab OAuth](https://dashboard.mintlify.com/settings/organization/gitlab-oauth) en la configuración de tu dashboard. Alternativamente, puedes usar la [integración de GitLab](/es/deploy/gitlab) con un token de acceso y un webhook.
...
<Note>
  ...
  Para repositorios de GitLab, la función automerge utiliza las credenciales de tu conexión [GitLab OAuth](https://dashboard.mintlify.com/settings/organization/gitlab-oauth) o el token de acceso configurado en tu [integración de GitLab](/es/deploy/gitlab). Asegúrate de que el token tenga permisos suficientes para fusionar solicitudes de fusión en la rama de destino.
</Note>
