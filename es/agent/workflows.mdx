...
| **Automerge** | No | El valor predeterminado es desactivado, lo que abre una solicitud de extracción para revisión. Si se habilita, abre una solicitud de extracción y la fusiona automáticamente. |
...
El valor es una expresión cron estándar de 5 campos con el formato `minuto hora día-del-mes mes día-de-la-semana`. Utiliza una herramienta como [crontab.guru](https://crontab.guru) para crear y validar los horarios.
...
Los repositorios de contexto proporcionan al agente acceso de lectura a repositorios adicionales cuando se ejecuta el flujo de trabajo. Esto es útil cuando tu prompt requiere revisar código o contenido fuera de tu repositorio de documentación. Puedes especificar hasta cinco repositorios de contexto por flujo de trabajo.
...
4. Haz clic en el interruptor **Active** para deshabilitar o habilitar el flujo de trabajo.
...
**Disparador:** Programación cron `0 9 * * 3` (miércoles a las 9:00 AM UTC)
...
**Disparador:** Programación cron `0 9 * * 1` (lunes a las 9:00 AM UTC)
