...
Para repositorios de GitHub, debes tener instalada la aplicación de GitHub de Mintlify en cada repositorio enumerado en los campos de contexto o de disparador push. Agrega nuevos repositorios en la página de la [aplicación de GitHub](https://dashboard.mintlify.com/settings/organization/github-app) de tu dashboard de Mintlify.

Para repositorios de GitLab, cada proyecto debe estar conectado mediante OAuth en la [página de configuración OAuth de GitLab](https://dashboard.mintlify.com/settings/organization/gitlab-oauth). Consulta [Configuración de GitLab](#gitlab-setup).
...
* Especifica el repositorio en formato `owner/repo`.
* Opcionalmente, especifica una branch a supervisar. Si no especificas una branch, el flujo de trabajo se activa cuando se realizan pushes a la branch predeterminada del repositorio.
...
Concede al agente acceso de lectura a repositorios adicionales cuando se ejecuta el flujo de trabajo. Esto es útil cuando tu prompt requiere revisar código o contenido fuera de tu repositorio de documentación. Puedes especificar hasta cinco repositorios de contexto por flujo de trabajo.

<div id="auto-merge-changes">
  ### Combinación automática de cambios
</div>

De forma predeterminada, el agente abre una solicitud de extracción por cada ejecución del flujo de trabajo para que puedas revisar los cambios antes de que se publiquen. Habilita automerge para fusionar automáticamente la solicitud de extracción sin necesidad de aprobación manual. Esto te proporciona un registro de los cambios en el historial de solicitudes de extracción de tu repositorio, a la vez que automatiza el paso de fusión.
...
Edita o elimina flujos de trabajo desde la página [Workflows](https://dashboard.mintlify.com/products/workflows) de tu dashboard.
...
**Disparador:** Al hacer push a `your-org/your-product` (rama main)

**Repositorios de contexto:** `your-org/your-docs`

**Automerge:** Desactivado
...
**Disparador:** Al hacer push a `your-org/your-docs` (rama main)
...
**Disparador:** Programación cron (`0 9 * * 3` — cada miércoles a las 9:00 AM UTC)
...
**Disparador:** Programación cron (`0 9 * * 1` — cada lunes a las 9:00 AM UTC)
...
**Notificaciones de Slack:** Notificar al canal `#documentation` y al usuario `tech-writer`
