...
## Repositorios alojados por Mintlify

Si tu documentación vive en un repositorio alojado por Mintlify (la opción predeterminada cuando no has conectado tu propia cuenta de GitHub o GitLab), el agente de Slack funciona sin configuración adicional:

- **No se requiere instalar la app de GitHub**: El agente usa el acceso gestionado por Mintlify a tu repositorio alojado, por lo que no necesitas instalar ni autorizar una integración Git por separado.
- **Los cambios se publican directamente**: En lugar de abrir una solicitud de extracción, el agente confirma los cambios directamente en tu rama de despliegue. Tu sitio en producción se actualiza tan pronto como los cambios se despliegan.
- **Las actualizaciones de estado omiten los detalles del repositorio**: Como no hay un repositorio externo al que hacer referencia, los mensajes de progreso del agente en Slack se centran en los archivos y las ediciones en lugar de en una ruta de repositorio.
- **Enlace de finalización**: Cuando el agente termina, publica un enlace a tu sitio en producción para que puedas revisar el cambio publicado.

Para pasar de un repositorio alojado por Mintlify a tu propio repositorio de GitHub o GitLab, sigue la guía de migración en la página de [Git Settings](https://dashboard.mintlify.com/settings/deployment/git-settings). Después de migrar, el agente cambia al flujo estándar de solicitudes de extracción descrito anteriormente.
