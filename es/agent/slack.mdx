...
- Compartir enlaces a pull requests o merge requests con el agente para actualizar la documentación relacionada.
...
El agente lee el contexto de la solicitud o del hilo y crea un pull request (o un merge request, para los despliegues conectados a GitLab) en tu repositorio conectado con las actualizaciones.

<div id="github-and-gitlab-support">
  ### Compatibilidad con GitHub y GitLab
</div>

El agente de Slack funciona tanto con despliegues de GitHub como de GitLab:

- **Despliegues conectados a GitHub**: El agente abre pull requests a través de la [Aplicación de GitHub de Mintlify](/es/deploy/github). El mensaje de finalización enlaza con el pull request en GitHub.
- **Despliegues conectados a GitLab**: El agente abre merge requests a través de tu [integración con GitLab](/es/deploy/gitlab), utilizando el token de acceso del proyecto que configuraste para el despliegue. El mensaje de finalización enlaza con el merge request en GitLab (o en tu instancia autoalojada de GitLab).

No se requiere configuración adicional: el agente detecta automáticamente qué proveedor de Git utiliza tu despliegue y crea el tipo de solicitud de cambio correspondiente.

<div id="reference-other-repositories">
  ## Hacer referencia a otros repositorios
</div>

El agente puede leer el código fuente de cualquier repositorio de GitHub que tenga instalada la aplicación de GitHub de Mintlify, incluso si no es tu repositorio de documentación. Esto le permite construir un contexto más profundo para guiar las actualizaciones de la documentación.
...
Los repositorios clonados son de solo lectura. El agente los utiliza como contexto, pero siempre abre pull requests o merge requests contra tu repositorio de documentación.
...
- **Finalización**: Cuando el agente termina, publica un resumen con un enlace al pull request o merge request y un enlace para abrir los cambios en el editor web.
...
- **Administradores y editores**: Pueden hacer preguntas y aplicar cambios en la documentación. El agente tiene acceso a las herramientas de edición de archivos y de pull requests o merge requests.
...
- **Revisa cuidadosamente**: Siempre debes revisar los pull requests o merge requests que genere el agente antes de fusionarlos.