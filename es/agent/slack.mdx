...
description: "Añade el agente de Mintlify a Slack para hacer preguntas sobre tus documentos, crear actualizaciones desde conversaciones y capturar el conocimiento del equipo."
...
<Note>
  Cada espacio de trabajo de Slack solo puede conectarse a una organización de Mintlify a la vez. Si intentas instalar el agente en un espacio de trabajo que ya está conectado a otra organización, la instalación fallará con un error de "organización incorrecta". Desinstala primero la app de la otra organización o usa un espacio de trabajo de Slack diferente.
</Note>

<div id="reconnect-or-reinstall-the-agent">
  ### Volver a conectar o reinstalar el agente
</div>

Si necesitas volver a autorizar al agente—por ejemplo, para otorgar nuevos permisos tras una actualización de los permisos de Slack o para recuperarte de un token revocado—reinstala la app desde tu dashboard de Mintlify.

Cuando reinstalas el agente en un espacio de trabajo que ya está conectado a tu organización, Mintlify actualiza el token de bot almacenado en su lugar. Tus canales, hilos e integraciones por usuario existentes permanecen intactos.
...
Mintlify mantiene sincronizado el estado de tus servicios conectados con Composio. Una conexión se desconecta cuando:

- El proveedor externo caduca o revoca las credenciales.
- El proveedor marca la conexión como inactiva.
- Eliminas la conexión.

Si una conexión ya no está activa, la próxima vez que pidas al agente que use ese servicio responderá con un enlace de autorización. Haz clic en el enlace, vuelve a autorizar el servicio y el agente reanudará la conexión.
...
- Continuar las conversaciones en hilos para iterar sobre los cambios o hacer preguntas de seguimiento. El agente reutiliza los archivos e imágenes compartidos previamente en el hilo como contexto para solicitudes posteriores, por lo que no tienes que volver a subirlos.
...
<div id="ask-questions-about-your-docs">
  ## Hacer preguntas sobre tu documentación
</div>

Pídele al agente preguntas sobre tu producto o tu documentación y recibe respuestas en el hilo.
...
- "¿Qué páginas mencionan el límite de velocidad de la API?"
