...
<ResponseField name="boost" type="number">
  Multiplica la clasificación de la página en la búsqueda integrada por este factor. Usa valores mayores que `1` para priorizar la página y valores entre `0` y `1` para despriorizarla. Consulta [Búsqueda](/es/optimize/search#boost-search-ranking) para más detalles. `boost` no tiene efecto cuando `searchable: false`, ya que la página queda excluida de los resultados de la búsqueda integrada.
</ResponseField>
