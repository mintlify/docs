...
Todos los campos del frontmatter son opcionales. Si omites `title`, Mintlify genera uno a partir de la ruta del archivo. Puedes omitir por completo el bloque de frontmatter en las páginas que no necesiten metadatos personalizados.
...
<ResponseField name="noindex" type="boolean">
  Establécelo en `true` para excluir la página de la búsqueda del sitio, de los sitemaps, de la indexación en motores de búsqueda y del contexto del asistente de IA. La página sigue visible en la navegación. Consulta [Deshabilitar la indexación](/es/optimize/seo#disable-indexing) para más detalles. Todas las páginas con `hidden: true` en su frontmatter reciben `noindex: true` automáticamente.
</ResponseField>

<ResponseField name="searchable" type="boolean" default="true">
  Su valor predeterminado es `true`. Las páginas son consultables a menos que decidas excluirlas. Establécelo en `false` para excluir la página de los resultados de búsqueda del sitio de documentación y del contexto del asistente de IA. La página sigue indexable por los motores de búsqueda externos, listada en el sitemap y visible en la navegación. Consulta [Excluir una página de la búsqueda](/es/optimize/search#exclude-a-page-from-search) para más detalles.
</ResponseField>

<ResponseField name="boost" type="number">
  Multiplica la clasificación de la página en la búsqueda integrada por este factor. Usa valores mayores que `1` para priorizar la página y valores entre `0` y `1` para despriorizarla. Consulta [Búsqueda](/es/optimize/search#boost-search-ranking) para más detalles. `boost` no tiene efecto cuando `searchable: false`, ya que la página no aparece en los resultados de la búsqueda integrada.
</ResponseField>
...
<ResponseField name="related" type="array or boolean">
  Una lista de páginas relacionadas para mostrar en la sección **Temas relacionados** en la parte inferior de la página, o `false` para ocultar la sección por completo. Anula las sugerencias automáticas de la implementación. Requiere que el add-on [Páginas relacionadas](/es/organize/related-pages) esté habilitado. Consulta [Páginas relacionadas](/es/organize/related-pages) para más detalles.
</ResponseField>
...
<ResponseField name="contextual" type="object">
  Sobrescribe el [menú contextual](/es/ai/contextual-menu) del sitio para esta página. Acepta el mismo array `options` y valor `display` que el campo `contextual` en `docs.json`. Omite el campo para heredar la configuración global, o define `options: []` para desactivar el menú contextual en esta página.
</ResponseField>
...
El modo Asistente muestra la página como una experiencia de [asistente](/es/assistant) a pantalla completa. Una interfaz de chat reemplaza todo el contenido de la página. Usa este modo para crear una página específica donde los usuarios interactúen con el asistente, como una página de «Preguntar a la IA». Todos los temas son compatibles con el modo Asistente.

Para usar el modo Asistente, debes habilitar el [asistente](/es/assistant) en tu despliegue. Si no habilitas el asistente, el modo de página del asistente muestra un campo de búsqueda deshabilitado.

```yaml
---
title: "Preguntar a la IA"
description: "Haz preguntas sobre cómo empezar y utilizar el producto"
mode: "assistant"
---
```
...
Ayuda a los usuarios a descubrir una página específica en los resultados de búsqueda añadiendo `keywords` a tus metadatos. Estas palabras clave no aparecen en el contenido de la página. Si los usuarios buscan esas palabras clave, la página aparecerá en los resultados de búsqueda.
...
La fecha mostrada se determina en el siguiente orden:

1. El campo `lastUpdatedDate` del frontmatter, si está definido en la página.
2. En despliegues respaldados por GitHub o GitLab, la fecha del último commit de Git que modificó el archivo de origen de la página.
3. La marca de tiempo del despliegue más reciente.

Para mostrar una fecha específica en lugar de la del commit de Git, define el campo `lastUpdatedDate` del frontmatter. Usa este campo cuando tu historial de Git no refleje cuándo cambió el contenido, por ejemplo, con contenido importado o sincronizado desde otro sistema.
...
Usa una fecha con formato `YYYY-MM-DD` o una cadena de fecha y hora ISO 8601 completa. Si el valor falta, está vacío o no se puede analizar, Mintlify recurre a la fecha del commit de Git y luego a la marca de tiempo del despliegue. Mintlify también acepta `last_updated_at` como alias de `lastUpdatedDate`.
