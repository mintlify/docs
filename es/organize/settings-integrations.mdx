---
title: "Integraciones"
description: "Conecta plataformas de analítica, herramientas de chat y otros servicios de terceros a tu sitio de documentación en tu archivo `docs.json`."
...
<ResponseField name="integrations.customScripts" type="array of objects">
  Carga archivos JavaScript externos en cada página. Puedes agregar hasta 20 scripts personalizados.

  <Expandable title="integrations.customScripts">
    <ResponseField name="src" type="string (uri)" required>
      URL HTTPS del script a cargar. No debe apuntar a un tipo de archivo no permitido (por ejemplo, `.svg`, `.exe`, `.zip`).
    </ResponseField>
    <ResponseField name="strategy" type="string">
      Cuándo cargar el script. Uno de los siguientes valores:
      - `beforeInteractive` — Cargar antes de que la página sea interactiva. Úsalo para scripts que deben ejecutarse lo antes posible, como el consentimiento de cookies o los gestores de etiquetas.
      - `afterInteractive` — Cargar después de que la página sea interactiva. Este es el comportamiento predeterminado.
      - `lazyOnload` — Cargar durante el tiempo de inactividad. Úsalo para scripts de baja prioridad como herramientas de análisis que no necesitan ejecutarse de inmediato.
    </ResponseField>
  </Expandable>
</ResponseField>
