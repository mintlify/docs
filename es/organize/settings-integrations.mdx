...
<ResponseField name="integrations.customScripts" type="array of object">
  Archivos JavaScript externos para cargar en cada página. Puedes incluir hasta 20 scripts.

  <Expandable title="customScripts item">
    <ResponseField name="src" type="string (HTTPS URL)" required>
      La URL del script a cargar. Debe usar HTTPS. No se permiten URLs que apunten a archivos `.svg`, `.exe` o `.zip`.
    </ResponseField>
    <ResponseField name="strategy" type="string">
      Controla cuándo se carga el script. Opciones: `afterInteractive` (predeterminado), `lazyOnload`.
    </ResponseField>
  </Expandable>
</ResponseField>
