...
<ResponseField name="integrations.customScripts" type="array of object">
  Carga scripts de terceros personalizados en tu sitio de documentación. Máximo de 20 scripts.

  <Expandable title="integrations.customScripts">
    <ResponseField name="src" type="string (uri)" required>
      La URL HTTPS del script a cargar. No debe apuntar a un tipo de archivo no permitido (`.svg`, `.exe`, `.zip`).
    </ResponseField>
    <ResponseField name="strategy" type="string">
      Controla cuándo se carga el script. `afterInteractive` o `lazyOnload`. Predeterminado: `afterInteractive`.
    </ResponseField>
  </Expandable>
</ResponseField>
