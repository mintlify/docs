...
| `integrations.customScripts` | array | No | Lista de scripts externos para cargar en cada página. Máximo 20 entradas. |
| `integrations.customScripts[].src` | string (URL HTTPS) | Sí | URL del script. Debe ser una URL HTTPS. |
| `integrations.customScripts[].strategy` | string | No | Estrategia de carga. Una de `beforeInteractive`, `afterInteractive` o `lazyOnload`. Usa `beforeInteractive` para inyectar el script en el `<head>` antes de que la página sea interactiva. |
