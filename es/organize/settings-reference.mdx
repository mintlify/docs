...
Esta página refleja el esquema `docs.json` publicado más reciente. Referéncialo en tu archivo con `"$schema": "https://mintlify.com/docs.json"` para obtener validación y autocompletado en tu editor.
...
| `seo.organization` | object | No | Ninguno |
...
| `icon` | string | No | Nombre del icono, emoji, URL, ruta o SVG. |
...
Muestra una fecha de última modificación en todas las páginas. En proyectos respaldados por GitHub o GitLab, la fecha refleja el último commit de git que modificó el archivo de origen de cada página, y recurre a la marca de tiempo del despliegue más reciente cuando no hay una fecha de commit de git disponible.
...
Organización utilizada como entidad editora en los datos estructurados (JSON-LD) de cada página. Acepta `id`, `name`, `legalName`, `url`, `logo` y `sameAs`. Consulta [SEO y búsqueda](/es/organize/settings-seo#seo).
