...
| `integrations.customScripts` | array of objects | No | Scripts externos a cargar. Máx 20 elementos. Cada objeto tiene `src` (requerido, URL HTTPS; `.svg`, `.exe`, `.zip` no permitidos) y `strategy` (`"afterInteractive"` o `"lazyOnload"`). |
