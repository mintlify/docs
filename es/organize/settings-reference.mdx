...
| `integrations.customScripts` | array of object | No | Archivos JavaScript externos para cargar en cada página. Máx. 20. |
| `integrations.customScripts[].src` | string (URL HTTPS) | Sí | URL del script. Debe ser HTTPS. Las extensiones `.svg`, `.exe`, `.zip` están bloqueadas. |
| `integrations.customScripts[].strategy` | `"afterInteractive"` \| `"lazyOnload"` | No | Cuándo cargar el script. Predeterminado: `afterInteractive`. |
