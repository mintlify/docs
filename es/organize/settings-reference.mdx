...
**Tipo:** array of object—cada uno con: `tab` (string, required), `icon` (string), `iconType` (string), `hidden` (boolean), `searchable` (boolean), `href` (string uri, required)

Establece `searchable: true` junto con `hidden: true` para mantener las páginas descendientes localizables a través de la búsqueda, `sitemap.xml`, el asistente de IA y `llms.txt`, manteniendo la pestaña oculta en la navegación renderizada. Consulta [Páginas ocultas](/es/organize/hidden-pages#include-pages-under-specific-hidden-tabs-or-groups).
...
Grupos para organizar el contenido en secciones etiquetadas. Cada grupo puede incluir `group` (string), `hidden` (boolean), `searchable` (boolean), `pages` (array) y otras propiedades de navegación.

**Tipo:** array of object

Establece `searchable: true` junto con `hidden: true` para mantener las páginas descendientes localizables a través de la búsqueda, `sitemap.xml`, el asistente de IA y `llms.txt`, manteniendo el grupo oculto en la navegación renderizada. Consulta [Páginas ocultas](/es/organize/hidden-pages#include-pages-under-specific-hidden-tabs-or-groups).
