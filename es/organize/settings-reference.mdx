...
| `integrations.customScripts` | array of objects (máx. 20) | No | Scripts externos a cargar en cada página. |
| `integrations.customScripts[].src` | string (uri) | Sí | URL HTTPS del script. No debe apuntar a un tipo de archivo no permitido. |
| `integrations.customScripts[].strategy` | `"beforeInteractive"` \| `"afterInteractive"` \| `"lazyOnload"` | No | Cuándo cargar el script. |
