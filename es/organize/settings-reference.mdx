...
Tu archivo `docs.json` reside en la raíz de tu repositorio de documentación. Consulta [Configuración global](/es/organize/settings) para conocer los fundamentos de la configuración.

<Tip>
  Agrega `"$schema": "https://mintlify.com/docs.json"` al principio de tu `docs.json` para obtener autocompletado y validación en editores como VS Code. Para validar `docs.json` localmente, ejecuta [`mint dev`](/es/cli/commands#mint-dev) o [`mint validate`](/es/cli/commands#mint-validate) desde la raíz de tu proyecto: los errores de esquema aparecen al iniciar.
</Tip>
