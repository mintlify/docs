...
description: "Configura las especificaciones OpenAPI y AsyncAPI, el área de pruebas interactiva, los ejemplos de código y la autenticación en `docs.json`."
...
<ResponseField name="api.playground" type="object">
  ...
  <Expandable title="api.playground">
    ...
    <ResponseField name="credentials" type="boolean">
      Si se deben incluir cookies y encabezados de autenticación en solicitudes de origen cruzado cuando el proxy está deshabilitado. El valor predeterminado es `false`. Solo tiene efecto cuando `proxy` es `false`.
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="api.spec" type="object">
  ...
  <Expandable title="api.spec">
    <ResponseField name="download" type="boolean">
      Si se muestra un botón de descarga para la especificación OpenAPI en las páginas de referencia de API. El valor predeterminado es `false`.
    </ResponseField>
  </Expandable>
</ResponseField>
