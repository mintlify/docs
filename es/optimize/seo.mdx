...
Las etiquetas por carpeta prevalecen sobre `seo.metatags` para la misma clave. Cuando una página está en varias entradas, las rutas más largas prevalecen sobre las más cortas. El frontmatter de la página sigue prevaleciendo sobre ambas. `canonical` siempre proviene de `seo.metatags` y no puede definirse por carpeta. Consulta [`seo.paths`](/es/organize/settings-seo#seo) para ver el esquema completo.
...
El frontmatter también puede sobrescribir cualquier metaetiqueta personalizada declarada en tu `docs.json` bajo [`seo.metatags`](/es/organize/settings-seo#seo) o en una entrada coincidente de [`seo.paths`](/es/organize/settings-seo#seo). Proporciona un valor de cadena no vacío con la misma clave para reemplazar el valor global o el limitado a una carpeta en esa página. El frontmatter no puede introducir metaetiquetas que la configuración no declare, y los valores que no sean cadenas se ignoran.
...
Para ocultar todo tu despliegue de los motores de búsqueda manteniéndolo públicamente accesible, activa **Don't index project** en la página [General](https://app.mintlify.com/settings/deployment/general) de tu panel.
