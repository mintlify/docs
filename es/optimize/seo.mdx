---
title: "SEO"
description: "Configura meta tags, propiedades Open Graph, URLs canónicas y ajustes SEO por página para mejorar el posicionamiento de tu documentación."
...
Mintlify añade datos estructurados de [schema.org](https://schema.org) a cada página indexable mediante un script JSON-LD. Estos datos estructurados ayudan a los motores de búsqueda a mostrar resultados enriquecidos para tus páginas.

Cada página emite un `@graph` conectado de entidades con `@id`s estables:

- `Organization`: el editor de tu sitio. Se deriva del nombre del sitio, el logo y la URL del sitio, o se configura explícitamente con [`seo.organization`](/es/organize/settings-seo#seo).
- `WebSite`: tu sitio.
- `WebPage`: la página actual, incluidas su descripción y fechas de modificación.
- `BreadcrumbList`: la ubicación de la página en tu jerarquía de navegación, generada a partir de la navegación de `docs.json`.
- La entidad principal de contenido: `TechArticle` para páginas de documentación o `APIReference` para páginas generadas a partir de especificaciones de API (páginas con frontmatter `api`, `openapi` o `asyncapi`).

Mintlify genera los datos estructurados a partir del frontmatter de la página y de las configuraciones de `docs.json`, incluidos el título de la página, la descripción, las palabras clave, la URL canónica, la fecha de la última actualización, el nombre del sitio y el logo. Los datos estructurados no incluyen ningún campo sin un valor correspondiente. Las páginas con `noindex: true` no incluyen datos estructurados.

Para cambiar los datos estructurados, actualiza los campos correspondientes del frontmatter o las configuraciones de `docs.json`. Para controlar la entidad editora, incluidos un `@id` estable, el nombre legal, el logo canónico y los enlaces de perfil `sameAs`, define [`seo.organization`](/es/organize/settings-seo#seo) en tu `docs.json`.
...
<div id="folder-scoped-meta-tags">
  ### Meta tags con alcance a una carpeta
</div>

Para aplicar distintas meta tags a un subconjunto de páginas, como documentación versionada o un área específica de producto, agrega `seo.paths` a tu `docs.json`. Cada entrada apunta a una ruta de carpeta relativa a la raíz de la documentación y aplica sus etiquetas a todas las páginas dentro de esa carpeta.
...
Las etiquetas por carpeta prevalecen sobre `seo.metatags` para la misma clave, las rutas más largas prevalecen sobre las más cortas y el frontmatter de la página prevalece sobre ambas. Consulta [`seo.paths`](/es/organize/settings-seo#param-seo-paths) para ver las reglas de las rutas y sus excepciones.

<div id="verify-site-ownership">
  ### Verificar la propiedad del sitio
</div>

Para verificar tu sitio con servicios como Google Search Console, Bing Webmaster Tools u otros motores de búsqueda, agrega la meta etiqueta de verificación a `seo.metatags` en tu `docs.json`. Mintlify inyecta la etiqueta en cada página.
...
El frontmatter también puede sobrescribir cualquier metaetiqueta personalizada declarada en tu `docs.json` bajo [`seo.metatags`](/es/organize/settings-seo#param-seo-metatags) o en una entrada coincidente de [`seo.paths`](/es/organize/settings-seo#param-seo-paths). Asigna a la misma clave un valor de cadena no vacío para reemplazar el valor global o el de la carpeta en esa página. El frontmatter no puede añadir metaetiquetas personalizadas que tu configuración no declare, y Mintlify ignora los valores que no sean cadenas.
...
Por defecto, Mintlify solo indexa las páginas que incluyes en la navegación de tu `docs.json`. Excluye las [páginas ocultas](/es/organize/hidden-pages)—páginas que faltan en tu navegación, páginas bajo una pestaña o grupo oculto y páginas con `hidden: true` en su frontmatter—de:
...
Si tu alojamiento sirve todas las páginas con una barra final (por ejemplo, `/quickstart/` en lugar de `/quickstart`), habilita `seo.trailingSlash` en tu `docs.json` para que las URLs que Mintlify emite para SEO coincidan:
...
Cuando está habilitado, Mintlify añade una barra final a:

- Etiquetas canónicas y metaetiquetas `og:url`
- Entradas `<loc>` en el `sitemap.xml` generado automáticamente
- Enlaces internos relativos a la raíz entre páginas, conservando cualquier cadena de consulta o fragmento

Habilitar `trailingSlash` solo afecta cómo se emiten las URLs. No cambia las rutas de las páginas, no redirige URLs existentes ni se aplica a los valores explícitos de `canonical` por página que definas en el [frontmatter](/es/organize/pages). Deja `trailingSlash` sin definir o en `false` si tu sitio sirve URLs sin barra final.
...
Para ocultar todo tu proyecto de los motores de búsqueda manteniéndolo públicamente accesible, activa **Don't index project** en la página [General](https://app.mintlify.com/settings/project/general) de tu panel.
...
- Devuelve `404` para `llms.txt`, incluso en `/.well-known/llms.txt`.
- Elimina tus páginas de [Mintlify Index](/es/search-index), de modo que los agentes de código ya no puedan recuperar tu contenido allí.
...
Tu búsqueda integrada en el producto, el [asistente](/es/assistant/index) y el [servidor MCP](/es/ai/model-context-protocol) siguen funcionando. Para eliminar páginas de esas superficies, usa `noindex: true` o `searchable: false` en páginas individuales.

<Note>
  **Don't index project** no bloquea [`llms-full.txt`](/es/ai/llmstxt#llms-fulltxt), que sigue disponible en `/llms-full.txt`. La exportación en Markdown de cada página también conserva su referencia a `/llms.txt`, aunque ese archivo devuelva `404` mientras la opción está activada. Si tu `llms.txt` devuelve `404` pero `llms-full.txt` funciona, revisa esta opción. Consulta [Solucionar que llms.txt devuelva 404 mientras llms-full.txt funciona](/es/help-center/llms-txt-returns-404-but-llms-full-txt-works).
</Note>

Usa esta opción cuando quieras mantener un proyecto en línea para revisión interna, pruebas o uso compartido limitado, pero no quieras que aparezca en los resultados públicos de búsqueda ni que se utilice como datos de entrenamiento o de recuperación para herramientas de IA.
