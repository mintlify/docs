...
<Note>
  Para sitios de documentación totalmente autenticados, los sitemaps y los archivos `robots.txt` también requieren autenticarse para acceder a ellos, y los sitemaps excluyen las páginas que pertenecen a [grupos de usuarios](/es/deploy/authentication-setup#control-access-with-groups).

  Para sitios parcialmente autenticados que incluyen páginas públicas, los sitemaps y los archivos `robots.txt` son de acceso público. Los visitantes anónimos solo ven las páginas públicas en el sitemap. Los usuarios autenticados también ven las páginas sin grupo.
</Note>
