...
description: "Instala la CLI de Mintlify para previsualizar documentación en local, probar cambios en tiempo real y detectar errores de compilación antes de desplegar a producción."
...
Usa la [CLI](https://www.npmjs.com/package/mint) para obtener una vista previa de tu documentación de forma local mientras escribes y editas. Revisa los cambios en tiempo real antes de implementar, prueba la apariencia y características de tu sitio de documentación y detecta problemas como enlaces rotos o problemas de accesibilidad.
...
Por defecto, el navegador se abre automáticamente. Para evitar que el navegador se abra, usa la opción `--no-open`:
...
Identifica los enlaces rotos en tu documentación:

```bash
mint broken-links
```

El comando ignora los archivos que coinciden con los patrones definidos en [.mintignore](/es/organize/mintignore). El comando reporta como rotos los enlaces que apuntan a archivos ignorados.

De forma predeterminada, el comando solo verifica los enlaces internos. Usa opciones para ampliar el alcance:

| Opción | Descripción |
| --- | --- |
| `--check-anchors` | También verifica los enlaces con ancla (por ejemplo, `/page#section`) comparándolos con los slugs de encabezados. |
| `--check-external` | También verifica los enlaces externos para detectar URL rotas. |
| `--check-snippets` | También verifica los enlaces dentro de componentes `<Snippet>`. |
...
Verifica tu archivo de OpenAPI en busca de errores con el siguiente comando:
...
Para verificar un archivo de OpenAPI alojado localmente y servido a través de HTTP, usa la opción `--local-schema`:
...
<Note>
  Los despliegues de producción solo admiten URL HTTPS. La opción `--local-schema` es solo para desarrollo local.
</Note>
...
Usa `--force` para renombrar archivos y omitir errores:

```bash
mint rename <ruta/al/archivo-anterior> <ruta/al/archivo-nuevo> --force
```
...
Usa `--output` para establecer un nombre de archivo personalizado, `--groups` para incluir páginas restringidas por [grupos de usuarios](/es/deploy/authentication-setup#control-access-with-groups), y `--disable-openapi` para omitir el procesamiento de OpenAPI.

```bash
mint export --output customer-docs.zip --groups enterprise
```

Consulta [Exportación offline](/es/deploy/export) para más detalles sobre todas las opciones y la distribución del archivo exportado.

<div id="import-content">
  ### Importar contenido
</div>

Importa contenido desde un sitio de documentación externo o una especificación OpenAPI con los comandos `mint scrape`. Esto resulta útil cuando migras tu documentación desde otra plataforma a Mintlify.

**Importar un sitio completo:**

```bash
mint scrape <url>
```

Usa la opción `--filter` (o `-f`) para limitar la extracción a las URL que coincidan con un prefijo de ruta específico:

```bash
mint scrape <url> --filter=<path>
```

**Importar una sola página:**

```bash
mint scrape page <url>
```

**Generar páginas a partir de una especificación OpenAPI:**
...
| Opción | Descripción |
| --- | --- |
| `--outDir` | Directorio donde se escriben los archivos generados. El valor predeterminado es `./docs`. |
| `--overwrite` | Sobrescribe los archivos existentes. |
| `--no-writeFiles` | Muestra una vista previa de la salida sin escribir archivos. |

<div id="upgrade-configuration">
  ### Migrar la configuración
</div>

Convierte un archivo de configuración `mint.json` al formato actual de `docs.json`:

```bash
mint upgrade
```

Consulta [Configuración global](/es/organize/settings) para obtener más información sobre `docs.json`.
...
La CLI recopila datos de telemetría de uso anónimos para ayudar a mejorar Mintlify. Los datos de telemetría incluyen el nombre del comando, la versión de la CLI, el sistema operativo y la arquitectura. Mintlify **no** recopila información de identificación personal, contenido del proyecto ni rutas de archivos.

De forma predeterminada, la CLI recopila datos de telemetría. Puedes desactivarla en cualquier momento usando la opción `--telemetry`:
...
| `DO_NOT_TRACK` | `1` | Desactiva la telemetría usando el estándar [Console Do Not Track](https://consoledonottrack.com/). |
...
<AccordionGroup>
  ...
  <Accordion title="mintlify versus paquete mint">
  ...
  <Accordion title="La versión del cliente muestra 'none' después de la instalación">
    ...
    **Solución:** Pídele a tu administrador de TI que agregue `releases.mintlify.com` a la lista de permitidos de tu red para habilitar el desarrollo local con la CLI.
  </Accordion>
</AccordionGroup>