...
description: "Usa la CLI para previsualizar la documentación en local, probar cambios en tiempo real y detectar problemas antes de desplegar."
...
<div id="export-a-static-site">
  ### Exportar un sitio estático
</div>

Exporta tu documentación como un sitio estático autónomo para despliegue sin conexión o en entornos aislados:

```bash
mint export
```

Este comando genera un archivo ZIP que contiene todo tu sitio de documentación como archivos HTML estáticos, junto con scripts auxiliares para servir el sitio localmente. El archivo exportado incluye un servidor estático basado en Node.js y scripts de inicio para macOS/Linux y Windows.

Por defecto, el archivo de salida se llama `export.zip`. Usa la opción `--output` para especificar una ruta de salida personalizada:
...
| Opción | Descripción |
| --- | --- |
| `--output` | Ruta del archivo ZIP de salida. Por defecto: `export.zip`. |
| `--groups` | Simula grupos de usuarios para la exportación (útil al exportar contenido restringido por grupo). |
| `--disable-openapi` | Desactiva la generación de archivos OpenAPI durante la exportación. |

<Note>
  Se requiere conexión a Internet en la primera ejecución para descargar la aplicación cliente. Las ejecuciones posteriores usan el cliente almacenado en caché en `~/.mintlify/`.
</Note>
...
<Note>
  Solo las URL HTTPS son compatibles con los despliegues de producción. La opción `--local-schema` es solo para desarrollo local.
</Note>
