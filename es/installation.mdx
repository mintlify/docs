...
De forma predeterminada, el navegador se abre automáticamente. Para evitar que el navegador se abra, usa el flag `--no-open`:
...
Para verificar un archivo OpenAPI alojado localmente y servido a través de HTTP, usa el flag `--local-schema`:
...
<Note>
  Solo se admiten URLs HTTPS en despliegues de producción. El flag `--local-schema` es solo para desarrollo local.
</Note>
