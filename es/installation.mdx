...
De forma predeterminada, solo se comprueban los enlaces internos. Usa opciones para ampliar el alcance:

| Flag               | Descripción                                                                                                     |
| ------------------ | --------------------------------------------------------------------------------------------------------------- |
| `--check-anchors`  | También valida los enlaces con ancla (por ejemplo, `/page#section`) comparándolos con los slugs de encabezados. |
| `--check-external` | También comprueba los enlaces externos para detectar URL rotas.                                                 |
| `--check-snippets` | También comprueba los enlaces dentro de componentes `<Snippet>`.                                                |
...
Extrae contenido de un sitio de documentación externo o especificación OpenAPI usando los comandos `mint scrape`. Esto es útil cuando migras tu documentación desde otra plataforma a Mintlify.
...
Usa la bandera `--filter` (o `-f`) para limitar la extracción a URLs que coincidan con un prefijo de ruta específico:
...
**Generar páginas desde una especificación OpenAPI:**
...
| Bandera | Descripción |
| --- | --- |
| `--outDir` | Directorio donde escribir los archivos generados. Por defecto es `./docs`. |
| `--overwrite` | Sobrescribir archivos existentes. |
| `--no-writeFiles` | Previsualizar la salida sin escribir archivos. |

<div id="upgrade-configuration">
  ### Actualizar configuración
</div>

Convierte un archivo de configuración `mint.json` al formato actual `docs.json`:

```bash
mint upgrade
```

Consulta [Configuración global](/es/organize/settings) para más información sobre `docs.json`.

<div id="create-a-workflow">
  ### Crear un workflow
</div>

Crea archivos de workflow de forma interactiva con el siguiente comando:

```bash
mint workflow
```

Este comando te guía en la creación de un archivo de workflow en `.mintlify/workflows/`. Te solicita el nombre del workflow, el tipo de disparador (cron o push), los repositorios de contexto, la configuración de automerge, y abre tu editor predeterminado para escribir las instrucciones del agente.

<Tip>
  La CLI detecta automáticamente los entornos no interactivos. En entornos no interactivos, muestra instrucciones de uso en lugar de quedarse bloqueada esperando entradas.
</Tip>

Consulta [Workflows](/es/agent/workflows) para más información sobre archivos de workflow y opciones de configuración.

<div id="check-version">
  ### Verificar versión
</div>

Muestra la versión actual del CLI y del cliente:
