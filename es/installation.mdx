...
Por defecto, solo se comprueban los enlaces internos. Utiliza las siguientes opciones para ampliar el alcance:

| Opción | Descripción |
| --- | --- |
| `--check-anchors` | Valida también los enlaces de ancla (por ejemplo, `/page#section`) contra los slugs de encabezados. |
| `--check-external` | Comprueba también los enlaces externos en busca de URLs rotas. |
| `--check-snippets` | Comprueba también los enlaces dentro de los componentes `<Snippet>`. |
