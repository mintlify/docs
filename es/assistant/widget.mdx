...
<div id="configure-analytics-privacy">
  ## Configurar la privacidad de la analítica
</div>

Mintlify captura analíticas de uso internas del widget. De forma predeterminada, cada evento incluye la ruta actual de la página en la que se incrusta el widget para que puedas ver qué páginas generan uso del asistente. La ruta se limita a 500 caracteres y se envía sin su cadena de consulta ni su fragmento. Nunca se recopilan parámetros de consulta, fragmentos, URLs completas ni referrers.

Si tu sitio incluye tokens, datos personales u otros valores privados en los segmentos de la ruta, desactiva la captura de la ruta en la inicialización:

```js
await window.MintlifyAssistant.init({
  id: "YOUR_WIDGET_ID",
  analytics: {
    capturePathname: false,
  },
});
```

Cambiar `analytics.capturePathname` en tiempo de ejecución requiere llamar a `destroy()` seguido de un nuevo `init()`.
