...
keywords: ["MCP", "acceso de escritura", "IA", "edición", "Claude", "ChatGPT", "Cursor", "rama", "pull request"]
---
...
El Admin MCP es un servicio de Mintlify alojado en `https://mcp.mintlify.com`. Todos los clientes se conectan al mismo punto de conexión y se autentican con tu cuenta de Mintlify.
...
| **Punto de conexión** | `https://mcp.mintlify.com` | `/mcp` en el dominio de tu sitio | `https://index.mintlify.com` |
...
Consulta la [referencia de Mintlify Index MCP](/es/search-index/mcp) para conocer sus entradas de herramientas y límites de uso.
...
- **`save`**: Abre una pull request o hace commit en la rama de la sesión. Fusiona automáticamente la PR cuando la implementación está configurada para enviar los cambios del agente a la rama principal y la rama de despliegue no está protegida.
