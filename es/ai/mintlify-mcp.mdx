...
<Steps>
  ...
  <Step title="Hacer checkout de una rama">
    La primera llamada requerida es `checkout {subdomain}`. Crea una nueva rama `admin-mcp/<slug>-<sha>` a partir de la rama de despliegue de esa implementación (o se adjunta a una rama existente que indiques) y devuelve un `editorUrl` que puedes abrir para seguir el progreso en el editor del panel.
...
- **Mintlify aloja tu implementación.** Para los sitios alojados por Mintlify, los cambios del MCP siempre se envían directamente, a menos que la protección de rama aún requiera una pull request.
...
El modo de código gestiona las operaciones a nivel de implementación que no tienen una herramienta dedicada, como administrar workflows, la configuración de la implementación, los miembros, la facturación, las integraciones, las analíticas y el uso compartido de [páginas privadas](/es/editor/pages#private-pages). Las herramientas del modo de código no requieren un `checkout`.
