...
Mintlify aloja un directorio de skills en `/.well-known/skills/` que los agentes pueden usar para descubrir y obtener tus archivos de skill de forma programática.

<div id="skills-index">
  ### Índice de skills
</div>

`GET /.well-known/skills/index.json` devuelve un manifiesto JSON con todas las skills disponibles:
...
`GET /.well-known/skills/{name}/skill.md` devuelve el archivo `skill.md` de una skill específica identificada por su nombre slugificado en el índice.
