...
`GET /.well-known/agent-card.json` devuelve un documento JSON conforme al esquema 0.2.0 de tarjeta de agente A2A. Cada entrada del arreglo `skills` corresponde a una skill de tus endpoints de descubrimiento de skills.

Los agentes compatibles con A2A obtienen `/.well-known/agent-card.json` para descubrir tu sitio por su nombre y descripción, siguen `documentationUrl` para recuperar contenido legible por humanos, e iteran sobre `skills` para obtener cada archivo `skill.md`.
