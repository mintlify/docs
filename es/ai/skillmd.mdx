...
`GET /.well-known/skills/index.json` devuelve un manifiesto JSON que lista todos los skills disponibles:
...
```json
{
  "skills": [
    {
      "name": "my-product",
      "description": "Una breve descripción de lo que hace tu skill.",
      "files": ["SKILL.md"]
    }
  ]
}
...
El campo `name` es un slug compatible con URL derivado del campo `name` en el frontmatter de tu `skill.md`.

### Archivos de skills individuales

`GET /.well-known/skills/{name}/skill.md` devuelve el archivo `skill.md` para un skill específico identificado por su nombre slugificado del índice.
