...
Las URL de la tarjeta (`url`, `documentationUrl`, `provider.url` y cada URL de skill) se construyen a partir del dominio personalizado configurado cuando está disponible, de modo que la tarjeta publicada siempre anuncia el dominio canónico en lugar del subdominio `*.mintlify.app`.
