---
title: "skill.md"
description: "Haz que tu documentación esté lista para agentes con archivos skill.md generados automáticamente que describen las capacidades de tu producto a la IA."
...
<Note>
  Si utilizas un [proxy inverso](/es/deploy/reverse-proxy), configúralo para reenviar las rutas `/skill.md`, `/.well-known/skills/*` y `/.well-known/agent-skills/*` a tu subdominio de Mintlify. Consulta la [configuración de enrutamiento](/es/deploy/reverse-proxy#routing-configuration) para conocer las rutas de destino y las políticas de caché.
</Note>
...
Mintlify resuelve los enlaces simbólicos durante el despliegue, por lo que descubre y sirve los archivos de skill como si estuvieran directamente dentro de `.mintlify/skills/`. Esto funciona tanto con enlaces simbólicos de directorios como de skills individuales.
...
| `groups`        | array  | Restringe la skill a grupos específicos de usuarios autenticados. Consulta [Restringir skills a grupos de usuarios](#restrict-skills-to-user-groups). |
...
<div id="restrict-skills-to-user-groups">
  ### Restringir skills a grupos de usuarios
</div>

Si tu documentación usa [autenticación](/es/deploy/authentication-setup), puedes restringir una skill a grupos específicos de usuarios añadiendo un arreglo `groups` a su frontmatter. El filtrado por grupos funciona igual que la [visibilidad de páginas](/es/create/personalization#page-visibility): la skill solo se expone a los usuarios autenticados que pertenecen al menos a uno de los grupos indicados.

```md SKILL.md
---
name: admin-tools
description: Operaciones internas para administradores.
groups: ["admin"]
---
```

Las skills restringidas por grupos:

- Se ocultan de los [endpoints públicos de descubrimiento](#skills-discovery-endpoints) y del archivo público `/skill.md`.
- Se exponen como [recursos MCP](/es/ai/model-context-protocol#mcp-resources) solo a los usuarios autenticados cuyos grupos coinciden. Los usuarios que no coinciden y los clientes máquina a máquina sin usuario final solo ven las skills sin grupos.

Las skills sin el campo `groups` permanecen visibles para todos.

<div id="validate-a-custom-skill-file">
  ### Validar un archivo de skill personalizado
</div>

Antes de desplegar, verifica que tus archivos de skill personalizados sean válidos:

- **Comprueba el frontmatter.** Cada archivo de skill personalizado debe comenzar con frontmatter YAML que incluya al menos un `name` y una `description`. Consulta [Campos de frontmatter](#frontmatter-fields) para ver la lista completa de campos admitidos.
- **Previsualiza en local.** Ejecuta [`mint dev`](/es/organize/settings) para servir tu sitio localmente y abre `http://localhost:3000/skill.md` para comprobar que se carga tu archivo personalizado en lugar del generado.
- **Verifica después de desplegar.** Cuando se despliegue tu sitio, añade `/skill.md` a la URL de tu sitio para ver el archivo de skill servido. Si tienes varias skills, obtén `/.well-known/agent-skills/index.json` para confirmar que cada skill personalizada aparece en el manifiesto de descubrimiento.
- **Valida contra la especificación.** Compara tu archivo con la [especificación de agentskills.io](https://agentskills.io/specification) para confirmar que las secciones obligatorias están presentes y con el formato correcto.

Los cambios en los archivos de skill personalizados se publican con tu siguiente despliegue. Como las skills generadas pueden tardar hasta 24 horas en actualizarse, las skills personalizadas son la forma más rápida de controlar lo que ven los agentes.
