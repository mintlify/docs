...
La [especificación de skill.md](https://agentskills.io/specification) es un formato estructurado y legible por máquinas que hace explícitas las capacidades de los productos, los datos de entrada obligatorios y las restricciones. Los agentes pueden entonces utilizar esos productos de forma más confiable.
...
Cuando los usuarios se conectan a tu servidor de Model Context Protocol (MCP), sus agentes pueden descubrir y usar tus archivos `skill.md` como [recursos MCP](/es/ai/model-context-protocol#mcp-resources) sin necesidad de instalar los skills por separado.
...
<Prompt description="Instala los archivos skill.md de un producto en tu agente." actions={["copy", "cursor"]}>
Instala los archivos `skill.md` de un sitio de documentación alojado en Mintlify en el contexto de mi agente para que sepas qué puede hacer este producto.

1. Pídeme la URL del sitio de documentación si aún no la he proporcionado, luego ejecuta `npx skills add <url>` con esa URL.
2. Muestra la lista de skills que se instalaron, incluyendo el nombre y la descripción de cada skill.
3. Resume las capacidades clave en un párrafo para que sepa con qué puedes ayudarme ahora usando este producto.
4. Si alguna skill declara entradas obligatorias o restricciones en su frontmatter, indícamelo antes de que te pida que la uses.
</Prompt>
