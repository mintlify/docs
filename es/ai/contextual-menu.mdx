...
| **Descargar especificación de API**                | `download-spec` | Descarga la especificación OpenAPI de tu proyecto. Si hay varias especificaciones configuradas, las descarga como un archivo zip. Solo aparece en páginas de referencia de API. |
...
<ResponseField name="href" type="string | object" required>
  ...
  <Expandable title="href object">
    ...
    <ResponseField name="query" type="object[]">
      Un array de objetos de parámetros de búsqueda que se añaden a la URL base.
...
Se aplican los mismos campos y reglas de validación que en `docs.json`, incluidas las [opciones personalizadas](#add-custom-options) y la configuración `display`. Si la sobrescritura de página no es válida, Mintlify utiliza la configuración global de `contextual`.
