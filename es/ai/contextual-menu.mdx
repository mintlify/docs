...
## Sobreescribir en páginas individuales
...
Se aplican los mismos campos y reglas de validación que en `docs.json`, incluidas las [opciones personalizadas](#adding-custom-options) y la configuración `display`. Si la sobreescritura de página no es válida, Mintlify utiliza la configuración global de `contextual`.
