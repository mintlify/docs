...
- `/mcp`: No requiere autenticación. Devuelve solo el contenido público. Comparte este endpoint con los usuarios que necesitan acceder al contenido público.
- `/authed/mcp`: Siempre requiere autenticación. Devuelve contenido según los permisos de cada usuario basados en sus [grupos de usuarios](/es/deploy/authentication-setup). Comparte este endpoint con los usuarios que necesitan acceder al contenido protegido.

El endpoint `/authed/mcp` utiliza su propio flujo de OAuth en `/authed/mcp/oauth/*`. Los dominios de redirección configurados para tu servidor MCP se aplican tanto a `/mcp` como a `/authed/mcp`.
