...
Cuando una aplicación de IA se conecta a tu servidor MCP de documentación, puede buscar directamente en tu documentación en respuesta al mensaje de un usuario, en lugar de depender de la información de sus datos de entrenamiento o realizar una búsqueda genérica en la web. Tu servidor MCP proporciona acceso a todo el contenido indexado de tu sitio de documentación.

* Las aplicaciones de IA pueden buscar proactivamente en tu documentación mientras generan una respuesta, incluso si no se les pide explícitamente que busquen en tu documentación.
...
* Cada búsqueda, también conocida como llamada a herramienta, ocurre durante el proceso de generación, por lo que la aplicación de IA busca información actualizada de tu documentación para generar su respuesta.
...
La herramienta de búsqueda de MCP admite parámetros de filtrado opcionales que las aplicaciones de IA utilizan para acotar los resultados de búsqueda.
...
* Para documentación pública, tu servidor MCP está disponible para cualquier persona. Busca en todas las páginas públicas indexadas.
* Para documentación con autenticación parcial, donde algunas páginas son públicas y otras están protegidas, debes habilitar tu servidor MCP antes de que esté disponible para los usuarios. Los usuarios no autenticados pueden buscar contenido público. Los usuarios que se autentican pueden buscar todo el contenido al que tienen permiso de acceso según sus [grupos de usuarios](/es/deploy/authentication-setup).
* Para documentación donde todas las páginas requieren autenticación, debes habilitar tu servidor MCP antes de que esté disponible para los usuarios. Los usuarios deben autenticarse antes de conectarse a tu servidor MCP. Tu servidor MCP busca únicamente el contenido al que cada usuario tiene acceso según sus [grupos de usuarios](/es/deploy/authentication-setup).
...
Si tu documentación requiere autenticación, tu servidor MCP exige que los usuarios se autentiquen antes de conectarse. Cuando un usuario añade la URL de tu servidor MCP a su herramienta de IA, debe iniciar sesión con sus credenciales existentes. Después de autenticarse, una redirección lo envía de vuelta a su herramienta. El servidor MCP solo devuelve el contenido al que cada usuario tiene permiso de acceso según sus [grupos de usuarios](/es/deploy/authentication-setup).

Si tu documentación tiene autenticación parcial con páginas públicas y protegidas, los usuarios pueden conectarse a tu servidor MCP sin autenticarse para acceder al contenido público. Los usuarios que se autentican obtienen acceso al contenido protegido disponible para ellos.

De forma predeterminada, tu servidor MCP solo está disponible para herramientas en localhost. Para permitir que herramientas basadas en web se conecten, agrega los dominios de redirección de las herramientas de IA. Un dominio de redirección es el nombre de host que una herramienta de IA utiliza después de que un usuario complete la autenticación, como `claude.ai` o `app.cursor.ai`. Las herramientas de IA de tus usuarios no podrán completar la autenticación a menos que su dominio de redirección esté en esta lista.

<Steps>
  <Step title="Habilita MCP autenticado en tu dashboard">
    1. Navega a la [página del servidor MCP](https://dashboard.mintlify.com/products/mcp) en tu dashboard.
    2. Haz clic en el interruptor **Enable MCP Server**.
  </Step>
  <Step title="Agrega dominios de redirección">
    Agrega los dominios de redirección de las herramientas de IA a las que deseas otorgar acceso a tus usuarios. Las herramientas de IA de tus usuarios no podrán completar la autenticación a menos que su dominio de redirección esté en esta lista. Los dominios de redirección comunes incluyen `claude.ai` y `vscode.dev/redirect`.

    Las direcciones de loopback (`localhost`, `127.0.0.1`) siempre son de confianza y no necesitan ser agregadas.
  </Step>
</Steps>
...
| :---------------------------------- | :------------------------- | :-------------------------------------------------------------------------------- |
| Por usuario (dirección IP)          | 5.000 solicitudes por hora | Limita la frecuencia con la que un solo usuario puede buscar en tu documentación. |
...
Para documentación que requiere autenticación, tu servidor MCP solo indexa las páginas en [grupos de usuarios públicos](/es/deploy/authentication-setup#make-pages-public). Para documentación con autenticación parcial, tu servidor MCP indexa las páginas públicas para usuarios no autenticados y cualquier página en grupos de usuarios públicos.
