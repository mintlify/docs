...
La herramienta de búsqueda de Model Context Protocol (MCP) admite parámetros de filtrado opcionales que las aplicaciones de IA usan para acotar los resultados de búsqueda.
...
Las aplicaciones de IA determinan cuándo aplicar estos filtros en función del contexto de la búsqueda del usuario. Por ejemplo, si un usuario pregunta por una versión específica de la API, la aplicación de IA puede aplicar automáticamente el filtro adecuado para ofrecer resultados más relevantes.
...
De forma predeterminada, tu servidor MCP solo está disponible para herramientas en localhost. Para permitir que se conecten herramientas basadas en la web y clientes de escritorio, configura las URIs de redirección OAuth permitidas. Una URI de redirección es la dirección que usa una herramienta de IA después de que un usuario completa la autenticación. Las herramientas de IA de tus usuarios no pueden completar la autenticación a menos que su URI de redirección esté permitida.
...
<Steps>
  ...
  <Step title="Habilitar clientes predefinidos">
    El dashboard incluye interruptores predefinidos para herramientas de IA comunes como Claude Desktop y Cursor. Activa el interruptor de cada cliente que quieras permitir. Cada interruptor agrega automáticamente la URI de redirección apropiada para ese cliente.
  </Step>

  <Step title="Agregar dominios personalizados">
    Para herramientas de IA que no están listadas como clientes predefinidos, agrega sus dominios de redirección en la sección **Custom Domains**. Las herramientas de IA de tus usuarios no pueden completar la autenticación a menos que su dominio de redirección esté en esta lista.
