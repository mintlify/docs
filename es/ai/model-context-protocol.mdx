...
Si la autenticación está habilitada, configura tu servidor Search MCP alojado desde **Settings → Security & access → MCP** en tu dashboard. El servidor Search MCP autenticado está disponible en la ruta `/authed/mcp` de la URL de tu sitio.
...
Tu servidor MCP alojado se habilita automáticamente cuando tu sitio tiene [autenticación](/es/deploy/authentication-setup) configurada. Para permitir que las herramientas de IA completen la autenticación, agrega sus dominios de redirección en tu dashboard.
...
Cuando se supera un límite, el servidor MCP devuelve una respuesta HTTP `429 Too Many Requests`. Los clientes MCP deberían aplicar backoff y reintentar tras un breve retraso; un backoff exponencial que empiece en unos pocos segundos es un valor predeterminado seguro. Los límites se restablecen en una ventana horaria móvil, por lo que la mayoría de las llamadas se recuperan automáticamente en el plazo de una hora.
