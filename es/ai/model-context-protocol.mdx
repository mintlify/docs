...
La herramienta de búsqueda de Model Context Protocol (MCP) admite parámetros de filtrado opcionales que las aplicaciones de IA pueden usar para acotar los resultados de búsqueda. Los parámetros disponibles se adaptan dinámicamente según la configuración de tu documentación.

**Siempre disponibles:**
* **`apiReferenceOnly`**: Solo devuelve documentación de referencia de API.
* **`codeOnly`**: Solo devuelve fragmentos de código.

**Disponibles cuando están configurados:**
* **`version`**: Filtra los resultados a una versión específica de la documentación (por ejemplo, `'v0.7'`). Solo disponible cuando tu documentación usa [versiones](/es/organize/versions). Solo devuelve contenido etiquetado con la versión especificada o contenido disponible en todas las versiones.
* **`language`**: Filtra los resultados a un código de idioma específico (por ejemplo, `'en'`, `'zh'` o `'es'`). Solo disponible cuando tu documentación usa [idiomas](/es/organize/languages). Solo devuelve contenido en el idioma especificado o contenido disponible en todos los idiomas.

El esquema de la herramienta de búsqueda de MCP incluye dinámicamente solo los parámetros aplicables a tu implementación. Las aplicaciones de IA determinan cuándo aplicar estos filtros según el contexto de la búsqueda del usuario.
