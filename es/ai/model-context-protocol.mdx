...
<Tip>
  ¿Quieres permitir que los agentes editen tu contenido en lugar de solo leerlo? Consulta el [servidor MCP de Mintlify](/ai/mintlify-mcp) para acceder a un servidor MCP autenticado que expone herramientas de branching, edición de páginas, navegación y `docs.json` a agentes de confianza.
</Tip>
...
Tu servidor MCP proporciona una herramienta de búsqueda que los agentes pueden usar:
...
Los agentes llaman a la herramienta de búsqueda según el contexto de la conversación para recuperar el contenido de la documentación relevante para la solicitud del usuario.
...
* **`version`**: Filtra los resultados a una versión específica de la documentación. Por ejemplo, `'v0.7'`. Solo está disponible cuando tu documentación tiene varias versiones. Solo devuelve contenido etiquetado con la versión especificada o contenido disponible en todas las versiones.
* **`language`**: Filtra los resultados a un código de idioma específico. Por ejemplo, `'en'`, `'zh'` o `'es'`. Solo está disponible cuando tu documentación tiene varios idiomas. Solo devuelve contenido en el idioma especificado o contenido disponible en todos los idiomas.
