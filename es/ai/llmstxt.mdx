...
<Note>
  La autenticación afecta cómo se sirven `llms.txt` y `llms-full.txt`:

  - **Sitios totalmente autenticados**: ambos archivos requieren autenticación para poder verse. Los LLM y las herramientas de IA que no puedan autenticarse en tu sitio no pueden acceder a estos archivos.
  - **Sitios parcialmente autenticados**: ambos archivos son accesibles públicamente sin autenticación, pero su contenido se limita a tus páginas públicas. Se excluyen las páginas que pertenecen a [grupos de usuarios](/es/deploy/authentication-setup#control-access-with-groups).
