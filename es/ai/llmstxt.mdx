...
<div id="when-to-use-each-file">
  ### Cuándo usar cada archivo
</div>

Usa `llms.txt` y `llms-full.txt` con distintos propósitos:

- **`llms.txt`** es un índice ligero de tu documentación. Las herramientas de IA lo utilizan para descubrir qué páginas existen y obtener solo las relevantes para la consulta de un usuario. Es preferible para agentes que pueden seguir enlaces y quieren minimizar el uso de contexto.
- **`llms-full.txt`** es el texto completo de cada página concatenado en un único archivo. Es preferible cuando quieres cargar toda tu documentación en el contexto de una herramienta de IA en una sola solicitud, como para evaluación, análisis sin conexión o herramientas que no siguen enlaces.
