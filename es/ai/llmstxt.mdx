...
<Note>
  El archivo `llms.txt` generado automáticamente está limitado a 100.000 caracteres. A medida que un archivo se acerca a este límite, Mintlify primero acorta las descripciones de las páginas y luego elimina entradas, añadiendo una nota que indica qué omitió. Este límite no se aplica a `llms-full.txt`. Para listar todas las páginas sin truncamiento, añade un [archivo `llms.txt` personalizado](#custom-files) en la raíz de tu proyecto.
</Note>
