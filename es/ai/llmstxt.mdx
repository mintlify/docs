...
<Note>
  El comportamiento de `llms.txt` y `llms-full.txt` depende de tu configuración de autenticación:

  - **Sitios públicos**: Ambos archivos son de acceso público e incluyen todas las páginas.
  - **Sitios parcialmente autenticados** (sitios con al menos una página `public: true`): Ambos archivos son de acceso público, pero solo listan e incluyen contenido de páginas públicas. Se excluyen las especificaciones de OpenAPI y AsyncAPI.
  - **Sitios totalmente autenticados** (sin páginas públicas): `llms.txt` solo se sirve a usuarios autenticados y excluye las páginas restringidas a [grupos de usuarios](/es/deploy/authentication-setup#control-access-with-groups), así como las especificaciones de OpenAPI y AsyncAPI. `llms-full.txt` no está disponible.
