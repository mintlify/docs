...
Los archivos de índice `llms.txt` generados automáticamente no pueden superar los 100.000 caracteres. Cuando un índice supera este límite, Mintlify mantiene el archivo `llms.txt` principal como un directorio y traslada grupos de enlaces de páginas a archivos Markdown generados bajo `/_llms/`. Este límite no se aplica a `llms-full.txt`.

Por ejemplo, un sitio grande podría enlazar al siguiente índice generado:

```mdx
- [API reference](https://docs.example.com/_llms/api-reference.md)
```

Los agentes deben seguir estos enlaces de índice de forma recursiva hasta llegar a los enlaces de las páginas de documentación. Los archivos generados forman parte de `llms.txt` y no necesitan existir en tu repositorio. Son independientes de `llms-full.txt`.
