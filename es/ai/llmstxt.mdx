...
<Warning>
  Si tu sitio requiere [autenticación](/es/deploy/authentication-setup), `llms.txt` y `llms-full.txt` se sirven detrás de la autenticación. La mayoría de las herramientas de IA y los LLM no pueden autenticarse en tu sitio, por lo que no pueden acceder a estos archivos. Esto significa que herramientas de IA como ChatGPT, Claude y Cursor no podrán descubrir ni indexar tu documentación.

  No hay forma de hacer públicos `llms.txt` o `llms-full.txt` en un sitio autenticado. Si necesitas que las herramientas de IA indexen tu documentación, considera usar [autenticación parcial](/es/deploy/authentication-setup#make-pages-public) para mantener algunas páginas públicas en lugar de autenticar completamente tu sitio.

  Los archivos también excluyen las páginas que pertenecen a [grupos de usuarios](/es/deploy/authentication-setup#control-access-with-groups).
</Warning>
