...
<Note>
  Cómo afecta la autenticación a `llms.txt` y `llms-full.txt`:

  - **Sitios públicos**: Ambos archivos son de acceso público e incluyen todas las páginas.
  - **Sitios parcialmente autenticados** (algunas páginas marcadas con `public: true`): Ambos archivos son de acceso público e incluyen solo las páginas públicas. Las páginas que pertenecen a [grupos de usuarios](/es/deploy/authentication-setup#control-access-with-groups) o que requieren autenticación se excluyen.
  - **Sitios totalmente autenticados** (todas las páginas protegidas): Ambos archivos devuelven `404` y no se sirven. Esto evita filtrar los títulos o las descripciones de las páginas a rastreadores no autenticados.
