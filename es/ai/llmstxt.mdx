...
<Note>
  La forma en que se sirven `llms.txt` y `llms-full.txt` depende de tu configuración de autenticación:

  - **Sitios públicos**: Ambos archivos están disponibles públicamente e incluyen todas las páginas.
  - **Sitios parcialmente autenticados** (algunas páginas marcadas como [`public`](/es/deploy/authentication-setup#make-pages-public)): Ambos archivos son accesibles públicamente sin autenticación, pero solo incluyen las páginas públicas. Se omiten las especificaciones OpenAPI y AsyncAPI.
  - **Sitios totalmente autenticados** (sin páginas públicas): `llms.txt` solo se sirve tras la autenticación y excluye cualquier página que pertenezca a un [grupo de usuarios](/es/deploy/authentication-setup#control-access-with-groups). `llms-full.txt` devuelve un 404 y no está disponible. En `llms.txt` se omiten las especificaciones OpenAPI y AsyncAPI.

  Los LLM y las herramientas de IA que no puedan autenticarse en tu sitio no podrán acceder al contenido protegido.
