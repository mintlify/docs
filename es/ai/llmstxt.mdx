...
Los enlaces a las páginas en el archivo `llms.txt` incluyen una extensión `.md` para que las herramientas de IA puedan obtener directamente la versión Markdown de cada página.
...
Mintlify añade encabezados HTTP a cada respuesta de página para que las herramientas de IA puedan descubrir automáticamente tus archivos `llms.txt`:
...
- `X-Llms-Txt: /llms.txt` — un encabezado de conveniencia para herramientas que verifican el soporte de `llms.txt`.
