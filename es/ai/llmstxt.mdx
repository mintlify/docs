...
<Note>
  Si tu sitio está totalmente autenticado, `llms.txt` y `llms-full.txt` también requieren autenticación para poder verse. Los LLM y las herramientas de IA que no puedan autenticarse en tu sitio no pueden acceder a estos archivos. Si tu sitio está parcialmente autenticado y tiene al menos una página pública, `llms.txt` y `llms-full.txt` son accesibles públicamente. Los archivos excluyen las páginas que pertenecen a [grupos de usuarios](/es/deploy/authentication-setup#control-access-with-groups).
