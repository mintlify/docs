...
<Note>
  El comportamiento de `llms.txt` y `llms-full.txt` depende de cómo esté configurado tu sitio:

  - **Sitios públicos**: Ambos archivos son de acceso público y listan todas las páginas de tu documentación.
  - **Sitios parcialmente autenticados** (algunas páginas son públicas): Ambos archivos son de acceso público y solo listan las páginas públicas.
  - **Sitios totalmente autenticados**: `llms.txt` devuelve 404. Los usuarios autenticados pueden obtener `llms-full.txt` para usarlo como contexto, y el archivo omite cualquier página restringida a [grupos de usuarios](/es/deploy/authentication-setup#control-access-with-groups) específicos u oculta de la búsqueda.

  En los sitios con autenticación, los enlaces a especificaciones de OpenAPI y AsyncAPI se omiten de `llms.txt`.
