...
* **Enlaces a especificaciones de API** hacia tus especificaciones OpenAPI y AsyncAPI, si tu documentación las incluye.
...
El archivo `llms-full.txt` reúne todo tu sitio de documentación en un único archivo como contexto para herramientas de IA e indexación de LLM.
...
Para agregar un archivo personalizado `llms.txt` o `llms-full.txt`, crea un archivo `llms.txt` o `llms-full.txt` en la raíz de tu proyecto. Agregar un archivo personalizado reemplaza el archivo generado automáticamente con el mismo nombre. Si eliminas un archivo personalizado, Mintlify restaura el archivo generado automáticamente.
