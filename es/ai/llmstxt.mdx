...
Mintlify aloja automáticamente un archivo `llms-full.txt` en la raíz de tu proyecto. Visualiza tu `llms-full.txt` agregando `/llms-full.txt` a la URL de tu sitio de documentación. Mintlify también aloja el archivo en `/.well-known/llms-full.txt` para compatibilidad con herramientas que siguen la convención `.well-known`.
