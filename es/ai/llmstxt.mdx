...
<Note>
  Si tu sitio requiere autenticación para acceder a tu documentación, tus archivos `llms.txt` y `llms-full.txt` no están disponibles para los LLM y las herramientas de IA. Los LLM y las herramientas de IA solo pueden indexar contenido que sea accesible públicamente.
</Note>
...
* **Enlaces a especificaciones API** a tus especificaciones OpenAPI y AsyncAPI (si están configuradas).
...
Si tu proyecto incluye especificaciones [OpenAPI](/es/api-playground/openapi-setup) o [AsyncAPI](/es/api-playground/asyncapi-setup), Mintlify incluye automáticamente enlaces a estas especificaciones en tu archivo `llms.txt`. Esto permite a los LLM acceder a tu esquema API completo para respuestas más precisas sobre tus endpoints, parámetros y tipos de datos.

Los archivos de especificaciones se deduplicar por nombre de archivo, priorizando la ruta más corta cuando varios archivos comparten el mismo nombre en diferentes directorios.
