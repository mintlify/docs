...
La API REST (Representational State Transfer) de Mintlify te permite interactuar de forma programática con tu documentación, lanzar actualizaciones, integrar experiencias de chat impulsadas por IA y exportar datos de Analytics.
...
* [Start static export job](/es/api/static-export/start-job): Inicia una tarea de exportación estática para una implementación.
* [Get static export job status](/es/api/static-export/get-job-status): Consulta el estado y el progreso de una tarea de exportación estática.
* [Generate export bundle](/es/api/static-export/generate-bundle): Empaqueta una tarea de exportación estática completada y devuelve un enlace de descarga.
* [Trigger automation](/es/api/automations/trigger): Ejecuta una automatización programada bajo demanda.
* [Detect AI-sounding prose](/es/api/admin/deslop): Analiza una página en busca de prosa generada por IA y obtén sugerencias de reescritura hechas por humanos.
...
* **Exportaciones estáticas**: Genera y descarga un paquete estático autónomo para alojarlo por tu cuenta con [Start static export job](/es/api/static-export/start-job), [Get static export job status](/es/api/static-export/get-job-status) y [Generate export bundle](/es/api/static-export/generate-bundle).
...
Puedes generar API keys en la [página de API keys](https://app.mintlify.com/settings/organization/api-keys) de tu dashboard. Las claves de API de administrador e Index pertenecen a una organización. Puedes usar las mismas claves en múltiples implementaciones dentro de la misma organización. Las claves de API del Assistant pertenecen al despliegue donde las creas.
...
| ----------------- | ----------- | ----------------------------------------------------------------------------- |
| Admin API key     | `mint_`     | Actualizaciones, tareas de agente, exportaciones de Analytics y exportaciones estáticas. Solo servidor. |
...
Usa la clave de la API del Assistant para autenticar solicitudes a los endpoints [Create assistant message](/es/api/assistant/create-assistant-message-v2), [Search documentation](/es/api/assistant/search) y [Get page content](/es/api/assistant/get-page-content).
...
Configura la lista de permitidos al crear la key en la [página de API keys](https://app.mintlify.com/settings/organization/api-keys) de tu dashboard. La lista de permitidos es fija durante la vida de la key; para cambiarla, elimina la key y crea una nueva. Si no configuras una lista de permitidos, la key acepta solicitudes desde cualquier dirección IP.
...
Puedes restringir opcionalmente una clave de la API de administrador a los scopes `read` o `write`. Los scopes se aplican solo a las claves de la API de administrador; las claves de la API del Assistant no se ven afectadas.

Configura los scopes al crear la key en la [página de API keys](https://app.mintlify.com/settings/organization/api-keys) de tu dashboard. Los scopes son fijos durante la vida de la key; para cambiarlos, elimina la key y crea una nueva. Si no configuras scopes, la key puede llamar a todos los endpoints de administrador (las keys existentes siguen funcionando).
...
Configura la expiración en la [página de API keys](https://app.mintlify.com/settings/organization/api-keys) de tu dashboard. La expiración es fija durante la vida de la key; para cambiarla, elimina la key y crea una nueva. Si no configuras una expiración, la key nunca expira.
