...
<Note>
  La exportación estática no está disponible para deployments con autenticación habilitada. Si tu deployment usa autenticación parcial o completa, [Iniciar job de exportación estática](/es/api/static-export/start-job) devuelve una respuesta `403`. Contacta con [support@mintlify.com](mailto:support@mintlify.com) si necesitas exportar un deployment autenticado.
</Note>

<div id="page-urls-in-static-bundles">
  ## URLs en paquetes estáticos
</div>

Los paquetes de exportación estática usan URLs de página explícitas con la extensión `.html`.
La exportación convierte `/guides/getting-started` en `/guides/getting-started.html`.
La exportación convierte la página de inicio en `/index.html`.
Este formato coincide con los archivos del paquete y permite alojarlos en S3 o en un hosting estático sencillo, sin reescrituras para URLs sin extensión.
Conserva los nombres de archivo `.html` al subirlos. La API de exportación estática habilita este formato automáticamente, por lo que no necesitas cambiar la configuración de tu documentación.
Los despliegues con CloudFront conservan la navegación del lado del cliente y la precarga.
Los servidores que solo sirven archivos permiten la navegación de página completa mediante los enlaces `.html` generados.
Los sitios de producción alojados en la nube siguen sin cambios.

La navegación interna integrada también utiliza destinos `.html`. Esto incluye los selectores de versión e idioma, los resultados de búsqueda, las tarjetas, las citas del asistente y los enlaces del playground de API. Las cadenas de consulta se conservan, por lo que los enlaces del playground de API siguen funcionando.

<Note>
  Las URL canónicas y las del sitemap siguen sin extensión. Las URL personalizadas de navegación del lado del cliente también pueden seguir sin extensión. Los despliegues con CloudFront las resuelven, pero los servidores que solo sirven archivos quizá necesiten reescrituras.
</Note>
