...
# Evitar ejecuciones de flujo de trabajo simultáneas para evitar condiciones de carrera
concurrency:
  ...
  export:
    ...
    steps:
      ...
      - name: Get changed MDX files
        id: changed
        run: |
          # Comprobar si existe el commit padre (maneja el push inicial)
  ...
  validate:
    ...
    steps:
      - name: Checkout repository
        uses: actions/checkout@v4
        with:
          fetch-depth: 0  # Historial completo para asegurar que origin/main esté disponible
      ...
      - name: Get changed translation files
        id: changed-files
        run: |
          # Obtener todos los archivos MDX modificados en directorios de traducción
      ...
      - name: Validate frontmatter
        ...
        run: |
          ...
          while IFS= read -r file; do
            ...
            # Comprobar que el frontmatter sea válido (líneas 1-2 deben ser ---)
      ...
      - name: Validate file integrity
        ...
        run: |
          ...
          while IFS= read -r file; do
            # Comprobar que el archivo sea legible y no esté vacío
            if [ ! -r "$file" ] || [ ! -s "$file" ]; then
              echo "✗ File integrity issue: $file (not readable or empty)"
              integrity_passed=false
            fi
            
            # Comprobación básica: el archivo debe tener contenido después del frontmatter
      ...
      - name: Final validation check
        id: final-check
        # Solo ejecutar esta comprobación si realmente teníamos archivos MDX para validar
        if: steps.changed-files.outputs.count > 0
        run: |
