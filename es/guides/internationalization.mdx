...
Si trabajas con tus propios proveedores de traducción o traductores regionales, puedes integrar su flujo de trabajo con tu documentación de Mintlify usando GitHub Actions u otras herramientas de CI/CD similares.

1. **Exportar contenido fuente**: Extrae los archivos MDX que necesitan traducción.
2. **Enviar a traductores**: Proporciona los archivos a tu proveedor de traducción.
3. **Recibir traducciones**: Recibe los archivos MDX traducidos.
4. **Importar y desplegar**: Añade los archivos traducidos a los directorios de idioma y actualiza la navegación.

Este flujo de trabajo de GitHub Actions exporta automáticamente el contenido en inglés modificado para su traducción cuando los PRs se fusionan en main.
...
Este flujo de trabajo de GitHub Actions valida e importa el contenido traducido cuando se añade mediante PR.
...
**Mejores prácticas para flujos de trabajo de traducción externa**

- **Conservar el frontmatter**: Asegúrate de que los traductores mantengan el frontmatter YAML intacto, traduciendo solo los valores de `title` y `description`.
- **Proteger los bloques de código**: Marca los bloques de código como "no traducir" para tus proveedores.
- **Usar memoria de traducción**: Proporciona glosarios con términos técnicos que deben permanecer en inglés o tener traducciones específicas.
- **Automatizar la validación**: Usa comprobaciones de CI para verificar la sintaxis MDX y el frontmatter antes de fusionar las traducciones.
- **Control de versiones**: Rastrea la versión fuente de cada traducción para identificar contenido desactualizado.
