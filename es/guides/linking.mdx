---
title: "Enlaces"
description: "Crea enlaces internos, referencia endpoints de API y mantén la integridad de los enlaces en tu documentación con rutas relativas y anclas."
keywords: ["enlaces internos", "referencias cruzadas", "enlaces de anclaje", "enlaces rotos", "enlaces profundos", "custom heading IDs"]
---
...
Enlaza a otras páginas de tu documentación usando rutas relativas a la raíz. Las rutas relativas a la raíz comienzan en la raíz de tu directorio de documentación y funcionan de manera consistente sin importar dónde se encuentren las páginas que enlazan en tu directorio.
...
<div id="how-anchor-links-are-generated">
  ### Cómo genera Mintlify los enlaces de anclaje
</div>

Mintlify genera automáticamente los enlaces de anclaje a partir del texto del encabezado.
...
Puedes sobreescribir el anclaje generado automáticamente para cualquier encabezado añadiendo `{#custom-id}` al texto del encabezado:

```mdx
## Configuration options {#config}
```

Este encabezado es accesible en `#config` en lugar de `#configuration-options`. Los IDs personalizados son útiles para mantener los enlaces de anclaje estables cuando actualizas el texto del encabezado. Consulta [Formatear texto](/es/create/text#custom-heading-ids) para más detalles.

<div id="deep-linking">
  ## Enlaces profundos
</div>

Los enlaces profundos apuntan a estados o ubicaciones específicos dentro de una página, no solo a la página en sí. Usa enlaces profundos para dirigir a los usuarios directamente a un acordeón abierto o a una vista del API playground.

<div id="accordion-deep-links">
  ### Enlaces profundos a acordeones
</div>

Cuando un usuario abre un acordeón, el hash de la URL se actualiza para reflejar el estado abierto. Al visitar una URL con ese hash, el acordeón se abre automáticamente y la página se desplaza hasta él.

Por defecto, el hash deriva del `title` del acordeón. Usa la propiedad `id` para establecer un hash personalizado:
...
Este acordeón es accesible en `#install` en lugar del `#installation-steps` generado automáticamente.

Consulta [Acordeones](/es/components/accordions) para obtener más información.

<div id="api-playground-deep-links">
  ### Enlaces profundos al API playground
</div>

Para abrir el API playground mediante un enlace, añade `?playground=open` a la URL de cualquier página de endpoint:

```text
https://your-docs-url/endpoint-path?playground=open
```

La URL se actualiza a medida que los usuarios abren o cierran el playground. Usa enlaces profundos al playground para compartir un enlace directo al playground interactivo de un endpoint en conversaciones de soporte o flujos de incorporación.

Consulta [API playground](/es/api-playground/overview#parameter-anchor-links) para obtener información sobre los enlaces de ancla de parámetros.

<div id="view-deep-links">
  ### Enlaces profundos a vistas
</div>

Cada componente `View` genera automáticamente un ID de anclaje a partir de su propiedad `title`. Puedes enlazar directamente a una vista específica o a un elemento dentro de una vista usando un hash en la URL:

```text
https://your-docs-url/page-path#javascript
https://your-docs-url/page-path#heading-inside-view
```

Cuando el hash de la URL coincide con un ID de vista o con un encabezado, acordeón o pestaña dentro de una vista, la vista correspondiente se activa automáticamente y la página se desplaza hasta el objetivo. El hash de la URL también se actualiza cuando los usuarios cambian de vista usando el menú desplegable.

Usa la propiedad `id` para establecer un hash personalizado:
...
Esta vista es accesible en `#js-guide` en lugar del `#javascript` generado automáticamente.

Consulta [Vista](/es/components/view) para obtener más información.
