...
Cuando usas componentes `View` en una página, cada vista obtiene automáticamente un ID compatible con URL basado en su título. Enlaza directamente a una vista específica usando el hash:

```text
https://tu-url-de-docs/ruta-de-pagina#python
```

También puedes enlazar a encabezados u otros elementos anclados dentro de una vista. La vista correcta se activa automáticamente y la página se desplaza hasta el objetivo:

```text
https://tu-url-de-docs/ruta-de-pagina#instalar-el-sdk
```

Cuando un usuario cambia de vista usando el menú desplegable, el hash de la URL se actualiza para coincidir con la vista seleccionada. Consulta [Vista](/es/components/view#deep-links) para más detalles.
