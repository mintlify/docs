...
Si actualmente usas páginas individuales en `MDX` para tus endpoints de API, puedes migrar a la generación automática de páginas desde tu especificación de OpenAPI, sin perder la personalización de cada página. Esto puede ayudarte a reducir la cantidad de archivos que necesitas mantener y a mejorar la coherencia de tu documentación de API.
