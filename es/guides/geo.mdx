...
Tu archivo `robots.txt` controla qué bots pueden rastrear tu sitio. Si bloquea los agentes de usuario de IA, herramientas como ChatGPT, Claude y Perplexity no pueden leer tu documentación ni citarla en las respuestas.

El `robots.txt` autogenerado por Mintlify permite todos los rastreadores de forma predeterminada. Si utilizas un [archivo robots.txt personalizado](/es/optimize/seo#custom-sitemaps-and-robotstxt-files), asegúrate de que no bloquee a los agentes de IA. Los agentes de usuario de IA más comunes incluyen:
...
Un `robots.txt` que bloquea todos los rastreadores también bloquea a los agentes de IA:

```txt Malo — bloquea todos los agentes de IA
User-agent: *
Disallow: /
```

Para bloquear rastreadores específicos y permitir agentes de IA, dirige el bloqueo solo a los bots que deseas restringir:

```txt Bueno — permite agentes de IA
User-agent: BadBot
Disallow: /
...
Ejecuta [`mint score`](/es/cli/commands#mint-score) para comprobar si el `robots.txt` de tu sitio permite agentes de IA. La verificación `robotsTxtAllowsAI` pasa cuando ningún agente de usuario de IA está bloqueado.
