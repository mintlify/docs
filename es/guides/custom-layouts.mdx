...
Las páginas en modo custom no tienen tipografía predeterminada. Los elementos HTML nativos como `<h2>` y `<p>` se renderizan con los estilos predeterminados del navegador, así que aplícales estilos explícitamente con clases de Tailwind.
...
<Note>
  La vista previa en vivo del editor web no genera el CSS de las clases de Tailwind específicas de la página, por lo que los diseños personalizados pueden verse sin estilos mientras editas. Comprueba tus estilos con `mint dev`, un despliegue de vista previa o un despliegue de producción. Consulta [Las clases de Tailwind no se aplican en la vista previa en vivo del editor](/es/help-center/tailwind-classes-not-applying-in-editor-preview).
</Note>
