...
<Note>
  La vista previa en vivo del editor web no genera CSS para las clases de Tailwind específicas de una página, por lo que los diseños personalizados pueden verse sin estilo mientras los editas. Verifica tus estilos con `mint dev`, una implementación de vista previa o una implementación de producción. Consulta [Las clases de Tailwind no se aplican en la vista previa en vivo del editor](/es/help-center/tailwind-classes-not-applying-in-editor-preview).
</Note>
...
Usa un archivo CSS personalizado cuando necesites animaciones con keyframes, selectores complejos o estilos que se apliquen a muchas páginas. Para valores puntuales, usa un [valor arbitrario](/es/customize/custom-scripts#arbitrary-values-and-variants) de Tailwind, como `w-[350px]`, en su lugar.
...
- **Aplica estilos directamente a los componentes.** Los componentes aceptan una propiedad `className`, así que puedes ajustar una sola instancia sin un elemento contenedor ni una regla CSS que la anule. Consulta [Estiliza componentes con className](/es/customize/custom-scripts#style-components-with-classname).
