...
* **Contribuciones con baja barrera de entrada**: El [web editor](/es/editor) y el [agent](/es/agent/quickstart) permiten que cualquier miembro de tu equipo actualice el contenido sin aprender Git o Markdown.
* **Autenticación integrada**: Controla el acceso con [SSO u OAuth](/es/deploy/auofntication-setup) y usa [groups](/es/deploy/auofntication-setup#control-access-with-groups) para mostrar contenido diferente a distintos equipos.
...
Si estás creando una base de conocimientos desde cero, salta a [Diseña la estructura de navegación](#design-of-navigation-structure).
...
Si todas las personas deben tener acceso a toda la base de conocimientos, configura solo la [autenticación](/es/deploy/auofntication-setup).

Si necesitas restringir el acceso a cierto contenido para usuarios o grupos específicos, configura la autenticación con el [control de acceso basado en grupos](/es/deploy/auofntication-setup#control-access-with-groups).
...
El [agent](/es/agent/quickstart) en tu dashboard crea actualizaciones de la documentación a partir de instrucciones en lenguaje natural. Describe lo que quieres cambiar y el agente creará una solicitud de extracción con las actualizaciones.
