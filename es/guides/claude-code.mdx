...
Para confirmar tu configuración, hazle a Claude Code una pregunta como `Summarize the standards from CLAUDE.md and list the Mintlify components you can use`. Una sesión que funcione correctamente cita tu `CLAUDE.md` y hace referencia a componentes de Mintlify (por ejemplo, `Card`, `Steps` o `Accordion`) del skill cargado.

<div id="template-for-claudemd">
...
Ejecuta Claude Code automáticamente cuando haya cambios en el código para mantener la documentación actualizada. Puedes activar revisiones de documentación en solicitudes de extracción o actualizar ejemplos cuando cambien las API.
...
Haz commit del archivo CLAUDE.md en tu repositorio de documentación. Cualquier persona que clone el repositorio y ejecute Claude Code usa automáticamente la configuración de tu proyecto. Esto hace que los estándares de documentación sean consistentes entre los colaboradores sin necesidad de que cada persona configure su propio contexto.
</Accordion>
</AccordionGroup>