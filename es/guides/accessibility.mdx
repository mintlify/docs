...
Cada página debe tener un solo encabezado H1, que proviene de la propiedad `title:` en el frontmatter de la página. Usa los demás encabezados en orden, sin saltarte niveles. Por ejemplo, no pases de H2 a H4.
...
El texto alternativo hace que las imágenes sean accesibles para las personas que usan lectores de pantalla y aparece cuando las imágenes no se cargan. Las imágenes en tu documentación deben incluir texto alternativo que describa la imagen y deje claro por qué la incluiste. Incluso con texto alternativo, no debes depender únicamente de las imágenes para transmitir información. Asegúrate de que tu contenido describa lo que comunica la imagen.
...
- **Evita redundancias**: No empieces con "Imagen de" porque los lectores de pantalla ya saben que el texto alternativo pertenece a una imagen. Sin embargo, incluye descripciones como "Captura de pantalla de" o "Diagrama de" si ese contexto es importante para la imagen.
...
Para conocer los flags que limitan el análisis a un solo tipo de comprobación, consulta la [referencia de `mint a11y`](/es/cli/commands#mint-a11y).
...
Los lectores de pantalla leen el **texto alternativo** y aparece cuando las imágenes no se cargan. Describe lo que muestra la imagen y por qué es relevante. El texto alternativo es un requisito para la accesibilidad.
