...
<div id="automerge-setup-banner">
  ## Banner de configuración de automerge
</div>

Si un flujo de trabajo intenta hacer automerge y la fusión falla porque a la GitHub App le faltan permisos de bypass (u otro problema de configuración impide la fusión), la página **Workflows** del panel muestra un banner debajo del conmutador de pestañas.

El banner enlaza a la configuración de ramas de tu repositorio para que puedas revisar tus rulesets y agregar la GitHub App de Mintlify a la lista de bypass. Cuando la configuración sea correcta y el siguiente intento de automerge tenga éxito, el banner desaparece automáticamente.

Puedes cerrar el banner con el botón de cierre. Volverá a aparecer la próxima vez que falle un intento de automerge para que no se te escapen nuevas regresiones de configuración.
