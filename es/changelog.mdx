...
<Update label="31 de agosto - 13 de septiembre" tags={["New releases", "Improvements"]} rss={{ title: "Actualizaciones semanales", description: "Sugerencias de IA en 404, búsqueda web del asistente con fuentes externas, mejora de seguridad y correcciones de confiabilidad" }}>
  ## Lanzamientos importantes

  * **Mejora importante**: páginas sugeridas por IA en errores 404, [cuando alguien cae en un enlace roto → el agente de IA lee la ruta → sugiere páginas semánticamente similares](https://x.com/mintlify/status/1966625627773059495)
  ...
  * Se corrigió un error por el cual al asistente se le aplicaba limitación de tasa de forma incorrecta porque la ventana de tiempo no se deslizaba correctamente
  * Se corrigió un error en las llamadas a herramientas del asistente para manejar correctamente bloques `text` vacíos
  * Se corrigió un error donde los nombres de servidor MCP concatenados con las llamadas a herramientas a veces superaban el límite de 60 caracteres que imponen los clientes MCP
  ...
  * Mejora de usabilidad: el parámetro de consulta de correo electrónico ahora completa previamente el campo en invitaciones de registro
  ...
  * Múltiples códigos de respuesta del área de pruebas de API ahora se muestran en un menú select con estilo y controlado en lugar del select predeterminado del sistema cuando está enfocado
  ...
  * Se corrigió un error por el cual los favicons no aparecían en los motores de búsqueda al servirlos desde la misma URL que el sitio de documentación correspondiente
  * Se corrigió un error donde las incrustaciones de YouTube parpadeaban al cargar
  * Se corrigió un error donde expandir el menú de comentarios para incluir respuestas escritas provocaba un cambio de diseño que afectaba la tabla de contenido
  * Se corrigió un error donde el texto se desbordaba por encima de la barra superior en el tema Maple al descartar la barra de notificaciones
  ...
  * Se corrigieron problemas de confiabilidad en la vista y exportaciones de analíticas del asistente
...
<Update label="August 3 - August 9" tags={["Improvements"]} rss={{ title: "Weekly Updates", description: "Search API, OAuth polish, asset serving, and MCP tooling" }}>
  ...
  * Acceso público a nivel de grupo: haz que grupos completos de páginas sean públicos vía `docs.json` para no tener que poner `public: true` en cada página ([más información](https://mintlify.com/docs/authentication-personalization/authentication-setup#group-level))
  ...
  * Ante errores de OAuth, los usuarios se redirigen a tu `logoutURL` especificado para reiniciar el flujo de autenticación
  * Corregido un parpadeo de error 500 durante los flujos de OAuth/JWT antes del callback
  * Eliminación automática de `https://` en las URL de la configuración de autenticación OAuth/JWT para evitar configuraciones incorrectas
  ...
  * Se corrigió un problema por el que el menú de acciones (con opciones como copiar y pulgar arriba) de mensajes antiguos desaparecía cuando llegaban nuevos mensajes en streaming
  ...
  * El campo de email en el inicio de sesión del panel de Mintlify ahora tiene enfoque automático para que puedas empezar a escribir de inmediato (mejora de calidad de vida)
  ...
  * Se agregó lógica de reintento para las exportaciones a PDF para mejorar la confiabilidad
  * Se corrigió que el aviso de consentimiento de cookies reapareciera después de la aceptación o el rechazo; ahora se respeta la primera selección
...
<Update label="February 2025" tags={["New releases", "Improvements"]}>
  ...
  Hemos introducido un nuevo esquema `docs.json` en sustitución de `mint.json`, para admitir un mejor versionado multinivel, una comprensión visual más sencilla y una terminología más consistente. Para más información sobre los cambios, [consulta nuestro blog](https://mintlify.com/blog/refactoring-mint-json-into-docs-json).
  ...
  1. Elimina el archivo `mint.json` antiguo y publica tus cambios

  ## Comprobaciones de CI

  Analiza automáticamente tu documentación para encontrar enlaces rotos, detectar errores ortográficos y gramaticales o aplicar estilos de redacción con tu propia configuración de Vale. Obtén más información en nuestra [documentación](es/settings/ci).

  ## Compatibilidad .md para LLM

  Todas las páginas de documentación ahora están disponibles automáticamente como archivos Markdown sin formato: solo agrega `.md` a la URL. Esto facilita que los LLM ingieran páginas individuales de tu documentación.
  ...
  * [Guide to Technical Writing:](https://mintlify.com/guides/introduction) Mejores prácticas para escribir documentación técnica, incluida la investigación de audiencia, tipos de contenido y consejos de redacción.
  * [Componente de menú desplegable](es/navigation#dropdowns): Organiza la navegación con un desplegable, además de pestañas y anclas.
...
<Update label="January 2025" tags={["Improvements"]}>
  ...
  * Diseño anidado más intuitivo, p. ej., al agregar o eliminar elementos
  * Respuestas más rápidas

  ## Mejoras de calidad

  * Posibilidad de requerir autenticación para acceder a implementaciones de vista previa
</Update>
...
<Update label="December 2024">
  ...
  Haz privada la documentación configurando autenticación mediante JWT, OAuth o una contraseña universal. Con esta privacidad, puedes crear una base de conocimiento interna o evitar que los competidores vean tu documentación.
</Update>
