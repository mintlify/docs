...
<AccordionGroup>
  ...
  <Accordion title="Selectores">
    ...
    **Atributos de datos**
    ...
    Algunos elementos exponen un atributo `data-active` cuando están en estado activo.
    ...
    - TableOfContentsItem (más profundo): `toc-item[data-active-deepest]`

    El atributo `data-active` en los elementos de la tabla de contenido se establece en la sección actualmente visible y en todas sus secciones padre. Usa `data-active-deepest` para seleccionar solo la sección activa más profunda.

    Los elementos de la tabla de contenido también exponen un atributo `data-depth` que indica su nivel de anidamiento, comenzando desde `0` para los encabezados de nivel superior.
  </Accordion>
</AccordionGroup>
