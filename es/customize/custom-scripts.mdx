...
Carga scripts externos desde una URL en cada página de tu documentación usando la configuración `integrations.customScripts` en `docs.json`. Es útil para agregar scripts de análisis o seguimiento de terceros alojados en un CDN.

<Note>
  Los scripts externos requieren que la opción de custom scripts esté habilitada para tu proyecto. Contacta con soporte si necesitas acceso.
</Note>
...
Agrega el array `customScripts` a `integrations` en tu `docs.json`:
...
Cada objeto de script acepta las siguientes propiedades:

<ResponseField name="src" type="string" required>
  La URL HTTPS del script a cargar. Debe apuntar a un archivo JavaScript válido.
</ResponseField>

<ResponseField name="strategy" type="string" default="afterInteractive">
  Controla cuándo se carga el script. Acepta `afterInteractive` (carga después de que la página sea interactiva) o `lazyOnload` (carga durante el tiempo de inactividad).
</ResponseField>

Puedes agregar hasta 20 scripts externos.

<Warning>
  Solo carga scripts de fuentes confiables. Los scripts externos tienen acceso completo a la página y pueden afectar el rendimiento y la seguridad.
</Warning>
