...
Puedes cargar scripts externos en cada página usando el campo `integrations.customScripts` en tu `docs.json`. Cada entrada requiere una URL HTTPS en `src` y acepta una `strategy` opcional para controlar cuándo se carga el script.

| Estrategia | Descripción |
|---|---|
| `beforeInteractive` | Inyecta el script en el `<head>` antes de que la página sea interactiva. Úsalo para scripts que deben ejecutarse antes de que se cargue cualquier contenido de la página. |
| `afterInteractive` | Carga el script después de que la página sea interactiva. Este es el comportamiento predeterminado. |
| `lazyOnload` | Difiere la carga hasta que el navegador esté inactivo. |
...
Puedes agregar hasta 20 scripts externos. El `src` debe ser una URL HTTPS y no puede apuntar a archivos `.svg`, `.exe` o `.zip`.
