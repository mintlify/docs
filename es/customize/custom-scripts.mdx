...
Puedes cargar JavaScript externo desde una URL añadiendo `customScripts` al objeto `integrations` en tu archivo `docs.json`. Cada entrada requiere una URL HTTPS `src` y acepta una opción `strategy` para controlar cuándo se carga el script.
...
<ResponseField name="src" type="string" required>
  La URL HTTPS del script a cargar. No se permiten URLs que apunten a archivos `.svg`, `.exe` o `.zip`.
</ResponseField>

<ResponseField name="strategy" type="string">
  Controla cuándo se carga el script. Opciones:
  - `afterInteractive` — carga el script después de que la página sea interactiva. Este es el valor predeterminado.
  - `lazyOnload` — carga el script durante el tiempo de inactividad, después de que todo el contenido de la página se haya cargado.
</ResponseField>

Puedes incluir hasta 20 scripts personalizados.
