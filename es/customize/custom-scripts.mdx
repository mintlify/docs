...
<div id="style-components-with-classname">
  ### Estiliza componentes con `className`
</div>
...
Tres componentes no aceptan `className`: [Banner](/es/components/banner), [MDX](/es/components/mdx) y [Visibility](/es/components/visibility).

<div id="arbitrary-values-and-variants">
  ### Valores arbitrarios y variantes
</div>
...
Las variantes funcionan como en cualquier proyecto de Tailwind, incluidos los prefijos responsivos (`sm:`, `md:`, `lg:`), las variantes de estado (`hover:`, `focus:`), `dark:`, las variantes de atributos de datos (`data-[state=open]:`), los modificadores de opacidad (`bg-black/50`) y el modificador de importancia `!`.
...
<Note>
  La vista previa en vivo del editor web no genera CSS para las clases de Tailwind específicas de una página, por lo que una página con estilos puede verse sin estilo mientras la editas. Consulta [Las clases de Tailwind no se aplican en la vista previa en vivo del editor](/es/help-center/tailwind-classes-not-applying-in-editor-preview).
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Atributos de datos">
    ...
    - `[data-component-part="card-content-container"]`—Contenedor interno alrededor del contenido de una tarjeta.
    - `[data-component-part="card-icon"]`—Icono dentro de una tarjeta. Úsalo para dar estilo a los iconos de las tarjetas, ya que el selector de elemento `icon` no coincide con los iconos dentro de las tarjetas.
    - `[data-component-part="card-title"]`—Texto del título dentro de una tarjeta.
    - `[data-component-part="card-content"]`—Contenido de la descripción dentro de una tarjeta.
    - `[data-component-part="card-image"]`—Imagen mostrada en la parte superior de una tarjeta.
    - `[data-component-part="card-cta"]`—Etiqueta de llamada a la acción dentro de una tarjeta.

    Partes del panel del asistente:
