...
description: "Agrega JavaScript y CSS personalizados a tu documentación para analíticas, widgets, estilos, integraciones de terceros y variables de servidor del API Playground."
...
Usa `window.mintlify.api.playground.setServerVariables` para rellenar previamente [las variables de servidor de OpenAPI](/es/api-playground/openapi-setup) desde JavaScript personalizado. Úsalo cuando los valores estén disponibles después de cargar la página, por ejemplo, cuando se inicialice un SDK de autenticación o cambie un inquilino. El método actualiza los API Playgrounds abiertos y se aplica a los que abras después.
...
Llama a `window.mintlify.api.playground.clearServerVariables()` cuando los valores ya no se apliquen, por ejemplo, después de cerrar sesión. Después de borrarlos, el playground vuelve a sus otros valores configurados.
