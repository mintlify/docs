...
Carga scripts de terceros desde URLs externas usando la opción `customScripts` en `docs.json`. Es útil para agregar análisis, widgets de chat u otros servicios que proporcionan un script alojado.

Cada entrada requiere una URL `src` (debe ser HTTPS) y acepta una opción `strategy` para controlar cuándo se carga el script:
...
```json docs.json
{
  "integrations": {
    "customScripts": [
      ...
      {
        "src": "https://example.com/consent.js",
...
<Note>
  Puedes agregar hasta 20 scripts personalizados. La URL `src` debe usar HTTPS y no debe apuntar a tipos de archivo no permitidos como `.svg`, `.exe` o `.zip`.
</Note>
