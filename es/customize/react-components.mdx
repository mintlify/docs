...
Puedes importar componentes desde tu carpeta `snippets`. A diferencia de React, no puedes importar componentes desde cualquier archivo `MDX`. Los componentes reutilizables deben hacer referencia a archivos dentro de la carpeta `snippets`. Obtén más información sobre [fragmentos reutilizables](/es/create/reusable-snippets).
