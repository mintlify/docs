...
<div id="managing-a-custom-domain">
  ## Gestionar un dominio personalizado
</div>

<div id="default-subdomain-remains-active">
  ### El subdominio predeterminado permanece activo
</div>

La URL predeterminada de tu implementación `<subdomain>.mintlify.site` no puede desactivarse ni redirigirse por ahora, incluso después de que un dominio personalizado esté activo. Ambas URL sirven el mismo contenido. Establece una [URL canónica](#set-a-canonical-url) para que los motores de búsqueda indexen el dominio personalizado como el principal.

<div id="renaming-a-project">
  ### Renombrar un proyecto
</div>

Renombrar un proyecto en tu dashboard cambia únicamente el nombre visible. No cambia la URL de la implementación ni el nombre de host predeterminado `<subdomain>.mintlify.site`.

Si renombras un proyecto y luego ves un error `Disallowed origin host` en tu URL predeterminada, el nombre de host de la implementación ya no coincide con lo que tu dashboard espera. Para cambiar la URL, elimina la implementación y vuelve a crearla con el nombre deseado. No hay un renombrado en el sitio para la URL de la implementación.

<div id="releasing-a-domain">
  ### Liberar un dominio
</div>

Un dominio personalizado solo puede estar asociado a una implementación de Mintlify a la vez. Si tu dominio sigue asociado a una implementación u organización anterior (por ejemplo, tras trasladarte a un nuevo workspace), la nueva implementación no puede reclamarlo hasta que la anterior lo libere.

Para liberar un dominio que controlas:

1. Contacta con [soporte](mailto:support@mintlify.com) y solicita la liberación del dominio.
2. Soporte proporciona un registro `TXT` de verificación con un valor único.
3. Agrega el registro en tu proveedor de DNS:

   ```text
   TXT | <tu-dominio> | <valor proporcionado por soporte>
   ```

4. Responde a soporte una vez que el registro esté activo. Soporte verifica el registro y desvincula el dominio de la implementación anterior. Luego puedes añadirlo a tu nueva implementación siguiendo [Agrega tu dominio personalizado](#add-your-custom-domain).
