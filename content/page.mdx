...
Want an icon for your sidebar item like the ones in [components](/content/components/accordion)? You can set an `icon` attribute in the metadata! All icons from [Font Awesome](https://fontawesome.com/icons) are available for us. You can also set an icon type (optional). If not set, the icon type will be regular.
