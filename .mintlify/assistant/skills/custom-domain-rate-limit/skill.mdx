...
Once the cooldown clears, the `_acme-challenge` TXT value appears automatically. The customer can then add it to their DNS and complete validation as normal.

If the customer has already been retrying repeatedly, the cooldown can extend up to 7 days. The best path forward is still to leave the domain entry in place and wait. If the value hasn't appeared after a week, direct the customer to contact support.
