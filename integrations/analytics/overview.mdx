...
2. **Trigger an event.** Visit your live site and interact with it. View a page, run a search, or make an API playground request to generate one of the [tracked events](#tracked-events).
...
If events still aren't arriving, double-check that you're using the correct key type for each provider (see the note in [Setup](#setup)) and that your `docs.json` changes have deployed successfully.
