...
If the Transcend banner does not appear or analytics never load, check the following:

- **Bundle ID**: Confirm the `bundleId` in your `docs.json` matches the deployment in your Transcend dashboard. A typo or a bundle from a different environment prevents Airgap from loading.
- **Airgap script**: Open your browser's developer tools and check the network tab for a request to `https://transcend-cdn.com/cm/<bundleId>/airgap.js`. If the request fails or returns a 404, the bundle ID is invalid or the deployment is not published in Transcend.
- **Purpose name**: If you set `analyticsPurpose`, confirm it exactly matches the purpose name in Transcend, including capitalization and spacing. A mismatched purpose means Mintlify waits on consent that Transcend never resolves.
- **Console errors**: Look for errors from `airgap` or Transcend in the browser console. These usually point to a misconfigured bundle or a blocked script.
- **Content Security Policy**: If your site sets a custom CSP, make sure it allows `transcend-cdn.com` for scripts.
