...
<ResponseField name="playground" type="object">
    ...
    <ResponseField name="credentials" type="boolean">
        Whether to include cookies and authentication headers for cross-origin requests when the proxy is disabled. Defaults to `false`.

        When `true` and `proxy` is `false`, the playground sends requests with credentials (cookies, authorization headers, or TLS client certificates) included. This is useful when your API relies on cookie-based authentication or session tokens and you need the browser to attach them automatically. This option has no effect when `proxy` is `true`.
    </ResponseField>
    </Expandable>
</ResponseField>
