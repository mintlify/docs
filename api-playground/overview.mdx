...
<ResponseField name="playground" type="object">
    ...
    <ResponseField name="credentials" type="boolean">
        Whether to include cookies and authentication credentials in cross-origin API requests. Defaults to `false`.

        This option only takes effect when `proxy` is set to `false`. When enabled, the browser includes cookies, authorization headers, and TLS client certificates in requests sent directly from the playground to your API.

        <Note>Your API server must include the appropriate [CORS headers](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Access-Control-Allow-Credentials) (e.g., `Access-Control-Allow-Credentials: true`) for credentialed requests to succeed.</Note>
    </ResponseField>
    </Expandable>
</ResponseField>
...
### Direct browser requests with credentials

When your API requires cookies or other browser credentials (for example, session-based authentication), disable the proxy and enable the `credentials` option:
...
This configures the playground to send requests directly from the user's browser with cookies and authentication credentials included. Your API server must support [CORS with credentials](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS#requests_with_credentials) for this to work.
