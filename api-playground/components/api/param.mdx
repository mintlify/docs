...
All `ParamField` components are added as interactive elements in the API playground. See the [demo](/api-playground) for an example.
...
| `default`                         | string  | Default value used by the server if the request does not provide a value    |
| `initialValue`                    | any     | Value that will be used to initialize the playground                        |
