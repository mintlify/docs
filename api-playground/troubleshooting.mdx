...
<AccordionGroup>
  ...
  <Accordion title="Some OpenAPI operations appear in navigation but others don't">
    ...
    3. **Missing tags**: Operations without proper tags may not be grouped correctly in navigation.
    4. **Deprecated operations**: Operations marked as deprecated may be filtered out depending on your configuration.
  ...
  <Accordion title="API responses show CORS errors in browser">
    CORS (Cross-Origin Resource Sharing) errors occur when your API server doesn't allow requests from the documentation domain.

    **For development:**
    - Ensure your API server includes the Mintlify domain in `Access-Control-Allow-Origin` headers
    - For local development, include `http://localhost:3000` in your CORS configuration

    **For production:**
    - Add your documentation domain (e.g. `https://docs.acme.com`) to your API's CORS whitelist
    - Include common headers: `Content-Type`, `Authorization`, `X-Requested-With`
    - Allow necessary HTTP methods: `GET`, `POST`, `PUT`, `DELETE`, `PATCH`

    **Example Express.js CORS configuration:**
    ```javascript
    app.use(cors({
      origin: ['https://docs.acme.com', 'http://localhost:3000'],
      credentials: true,
      optionsSuccessStatus: 200
    }));
    ```
  </Accordion>
  <Accordion title="Authentication not working in API playground">
    If API calls fail with 401/403 errors even with authentication configured:

    1. **Check authentication method**: Verify your `api.auth.method` in `docs.json` matches your API's requirements
    2. **Header format**: For bearer tokens, ensure your API expects `Authorization: Bearer <token>` format
    3. **API key location**: Confirm whether your API expects keys in headers, query parameters, or request body
    4. **Environment-specific endpoints**: Some APIs use different authentication endpoints for staging vs production
