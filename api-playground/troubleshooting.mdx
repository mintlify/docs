...
<AccordionGroup>
  ...
  <Accordion title="One of my OpenAPI pages is completely blank">
    ...
    <Note>
      Trailing slash mismatches are handled automatically. For example, `GET /users/{id}/` in
      your page metadata correctly resolves to `/users/{id}` in your OpenAPI spec, and vice versa.
    </Note>
