...
<AccordionGroup>
  ...
  <Accordion title="One of my OpenAPI pages is completely blank">
    ...
    Mintlify tolerates trailing slash differences, so `/users/{id}` and `/users/{id}/` resolve
    to the same endpoint. However, if your spec defines both `/users/{id}` and `/users/{id}/` as
    distinct operations, exact matching takes precedence.
