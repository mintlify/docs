...
<AccordionGroup>
  ...
  <Accordion title="My build fails with &quot;Failed to fetch OpenAPI file for anchor or tab&quot;">
    ...
    If you generate the spec in CI, sequence the pipeline so the spec is fully published to its public URL before you call the [Trigger deployment](/api/update/trigger) API endpoint. Triggering the deployment first causes the build to fetch a stale or missing spec.
  </Accordion>
