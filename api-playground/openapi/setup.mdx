...
**1) In the repo**: The most common method of setting up OpenAPI with Mintlify is to add your OpenAPI documents directly to your docs repo. The format will either be in `.yaml` or `.json`. 

**2) On the web**: If you have a publicly-hosted OpenAPI document that serves as the single-source-of-truth for your API, you can use that in your docs through the document URL (ex: https://mintlify.com/specs/v2/openapi.json)


## Autogenerating the API playground

You can either autogenerate the API playground with our [tabs](/settings/navigation#tabs) and [anchors](/settings/navigation#anchors) setup or create MDX files for the OpenAPI endpoints.

### Auto-populate API pages with tabs or anchors
