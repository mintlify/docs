...
Bearer token authentication uses JSON Web Tokens (JWT) for secure, stateless authentication. The token is included in the `Authorization: Bearer <token>` header.
...
<Tip>
Users can paste their JWT token in the input field. The token is stored securely in the browser and included in all API requests.
</Tip>
