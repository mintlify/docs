...
description: "Generate SDK reference pages in your Mintlify docs from TypeDoc, DocFX, Javadoc, Sphinx, or phpDocumentor artifacts you already build."
...
Run [`mint dev`](/cli/commands#mint-dev) to preview SDK reference pages locally. The CLI parses the artifact on startup and rebuilds pages when you change `docs.json` or the artifact file.

If pages do not appear or the build reports errors:

- Confirm the `format` matches the artifact your tool produced.
- Confirm `source` resolves to the correct file or directory relative to your docs repository, or to a reachable HTTPS URL.
- For directory formats, confirm the artifact contains the expected tool output (for example, `docfx metadata` YAML, Sphinx `json` builder output).
- For remote sources, confirm the download stays within the 50 MB / 200 MB extracted limits.

Run [`mint validate`](/cli/commands#mint-validate) to surface configuration errors alongside the rest of your docs build.
