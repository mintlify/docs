...
Use the `sdk` navigation property to generate reference pages for your SDK libraries from the documentation tools you already run. Mintlify reads each tool's build artifact. It creates a page for every class, interface, module, and function, with navigation groups, cross-page links, and search indexing included.
...
Add an `sdk` property to a tab in your `docs.json`. Mintlify parses the artifact. It creates navigation groups and pages for the library.
