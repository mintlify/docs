...
Declare `sdk` on a [group](/organize/navigation#groups) to scope generated pages to a subsection of an existing tab. Groups inherit `sdk` config from their parent, so nested groups can override or extend the tab-level artifact.
...
<Note>
  You can declare `sdk` on a [tab](/organize/navigation#tabs) or a [group](/organize/navigation#groups). A tab or group with `sdk` may include nested `groups`, but no other navigation structures, such as `pages`, `versions`, or `languages`. It also cannot include an `openapi`, `asyncapi`, or `graphql` property.
</Note>
...
When at least one hand-authored SDK page exists under a tab or group with `sdk`, Mintlify skips bulk generation for that owner. Move a page out from under that owner if you want the rest of the library to auto-populate.
...
The string form parses as `[source] kind name`. Omit `source` to inherit it (and `format`) from the tab or group `sdk` config. For `method` and `property` targets, qualify the name with a parent, such as `method Auth0Client.loginWithRedirect`.
...
<ParamField path="format" type="string">
  Overrides the inherited `format`. Only needed when the page is not under a tab or group with `sdk`.
</ParamField>

<ParamField path="source" type="string">
  Overrides the inherited `source`. Only needed when the page is not under a tab or group with `sdk`.
</ParamField>
