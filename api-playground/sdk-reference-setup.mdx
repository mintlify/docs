...
<Note>
  You can declare `sdk` on a [tab](/organize/navigation#tabs) or a [group](/organize/navigation#groups). A tab with `sdk` may include `groups`, but no other navigation structures, such as `pages`, `versions`, or `languages`, and cannot include an `openapi`, `asyncapi`, or `graphql` property. A group with `sdk` may include `pages` and nested groups, but cannot include a `graphql` property.
</Note>
