...
The `x-default` extension is supported on `apiKey`, `http` (bearer and basic), and `oauth2` security scheme types. The value appears as the default input in the playground's authentication fields.
