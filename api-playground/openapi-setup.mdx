...
Add custom labels that appear before or after a parameter name in your endpoint pages by setting `x-mint: pre` or `x-mint: post` on a schema or property. Each is an array of strings that renders as pills around the parameter name, alongside auto-derived pills like `read-only` and `write-only`.
...
To surface an arbitrary OpenAPI spec field as a post label on every parameter field, list its key under [`api.params.post`](/organize/settings#api-params-post) in your `docs.json`. Mintlify looks up each configured key on every schema and renders its value as a pill:

- String values render verbatim.
- Boolean `true` renders the key name.
...
```json docs.json
{
  "api": {
    "params": {
      "post": ["x-stability", "x-internal"]
    }
  }
}
```

```json openapi.json {7-8}
{
  "components": {
    "schemas": {
      "User": {
...
In this example, the `email` parameter renders `beta` and `x-internal` as post pills.
