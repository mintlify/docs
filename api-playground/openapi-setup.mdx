...
Use `x-default` on any schema property to pre-fill request fields in the API playground without affecting the `default` field in the schema definition. This works on strings, numbers, booleans, arrays, and objects.
...
When `x-default` and `default` are both present on a property, `x-default` takes priority in the playground while `default` remains unchanged in the schema definition.
