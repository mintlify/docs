...
For OpenAPI 3.1, set `type` to `string` and add a `contentMediaType`. Add `contentEncoding: base64` or `contentEncoding: base64url` when your API expects a base64-encoded file. For OpenAPI 3.0, use `format: binary` or `format: base64` instead.
