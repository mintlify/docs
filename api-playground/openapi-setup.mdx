...
Control which API operations are exposed as tools on your [MCP server](/ai/model-context-protocol) using `x-mint: mcp`. When your MCP server generates tools from your OpenAPI specification, this extension lets you opt individual operations in or out and customize how they appear to AI applications.

<ResponseField name="enabled" type="boolean">
  Whether this operation is available as an MCP tool. Set to `false` to exclude an operation from your MCP server.
</ResponseField>

<ResponseField name="name" type="string">
  Custom name for the MCP tool. Overrides the default name derived from the operation.
</ResponseField>

<ResponseField name="description" type="string">
  Custom description for the MCP tool. Overrides the operation's `description` field. Use this to provide AI-optimized descriptions that help AI applications understand when and how to use the tool.
</ResponseField>
      ...
      "get": {
        ...
        "x-mint": {
          "mcp": {
            "enabled": true,
            "name": "list-users",
            "description": "Search and retrieve user accounts. Use this when the user asks about team members, account holders, or needs to look up a specific person."
          }
        }
      }
    }
...
To exclude a specific operation from your MCP server while keeping it in your API documentation, set `enabled` to `false`:
...
<Note>
  The `x-mint: mcp` extension is set on individual operations. You can also use the standalone `x-mcp` extension directly on an operation for backward compatibility, but `x-mint: mcp` takes precedence when both are present.
</Note>
