...
You can also use `x-default` on other schema properties in your OpenAPI specification to set a default value in the API playground without affecting the `default` field in the schema definition. Unlike security schemes, prefill for non-security-scheme properties only takes effect when [`api.examples.prefill`](/organize/settings-api) is set to `true` in your [`docs.json`](/api-playground/overview#example-configuration).

<Note>
  Prefill from `x-default` on array-typed schema properties is not currently supported in the API playground, even when `api.examples.prefill` is enabled.
</Note>
