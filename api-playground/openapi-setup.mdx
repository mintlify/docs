...
Control which API operations are exposed as tools through your [MCP server](/ai/model-context-protocol) using `x-mint: mcp`. By default, all operations are included. Use this extension to disable specific operations or customize how they appear to AI tools.
...
The `mcp` object accepts the following properties:

| Property | Type | Description |
| :---------- | :------ | :---------- |
| `enabled` | boolean | Whether to expose this operation as an MCP tool. Defaults to `true`. |
| `name` | string | Custom name for the MCP tool. Overrides the default generated name. |
| `description` | string | Custom description for the MCP tool. Overrides the operation's `description`. |
...
<Note>
  The `x-mint: mcp` extension replaces the legacy `x-mcp` extension. If both are present on an operation, `x-mint: mcp` takes precedence.
</Note>
