...
Control which API endpoints are exposed as MCP (Model Context Protocol) tools using `x-mint: mcp`. This lets AI agents discover and use your API endpoints directly.
...
<ResponseField name="x-mint.mcp.enabled" type="boolean">
  Whether to expose this endpoint as an MCP tool.
</ResponseField>

<ResponseField name="x-mint.mcp.name" type="string">
  Custom name for the MCP tool. If not specified, a name is generated from the operation.
</ResponseField>

<ResponseField name="x-mint.mcp.description" type="string">
  Custom description for the MCP tool. If not specified, the operation summary or description is used.
</ResponseField>
