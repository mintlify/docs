...
Control which API operations are exposed as tools through your [MCP server](/ai/model-context-protocol) using `x-mint: mcp`. By default, operations are not exposed as MCP tools. Add `x-mint: mcp` with `enabled: true` to opt in specific operations.

<ParamField body="enabled" type="boolean">
  Whether to expose this operation as an MCP tool. Defaults to `false`.
</ParamField>

<ParamField body="name" type="string">
  Custom name for the MCP tool. If not set, the tool name is derived from the operation.
</ParamField>

<ParamField body="description" type="string">
  Custom description for the MCP tool. If not set, the tool description is derived from the operation summary or description.
</ParamField>
...
<Note>
  The deprecated `x-mcp` extension is still supported but `x-mint: mcp` takes precedence. Use `x-mint: mcp` for new configurations.
</Note>
