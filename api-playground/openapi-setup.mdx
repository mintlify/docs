...
By default, Mintlify uses the tag `name` from your OpenAPI specification as the navigation group heading. Use the `x-group` extension on a tag definition to set a custom display name for the navigation group without changing the tag name or URL structure.

This is useful when your tag names are machine-friendly identifiers (like `CRMActions`) but you want human-readable group headings in the sidebar (like `CRM Actions`).
  ...
  "tags": [
    {
      "name": "CRMActions",
      "x-group": "CRM Actions",
      "description": "Endpoints for managing CRM actions"
    }
  ],
  "paths": {
    "/actions": {
...
In this example, the sidebar displays "CRM Actions" as the group heading, while the URL path remains based on the original tag name `CRMActions`.

If `x-group` is not set, Mintlify uses the tag name as the group heading.
