...
Control which API endpoints are exposed as tools in your [MCP server](/ai/model-context-protocol) using `x-mint: mcp`. By default, your MCP server provides a search tool over your documentation content. Use `x-mint: mcp` to configure individual endpoints for MCP tool exposure.

<Note>
  `x-mcp` at the operation level is deprecated. Use `x-mint: mcp` instead. The [Mint CLI migration command](/guides/migrating-from-mdx) automatically converts `x-mcp` to `x-mint: mcp`.
</Note>
...
| :--- | :--- | :--- |
| `enabled` | boolean | Whether this endpoint is exposed as an MCP tool. |
| `name` | string | Override the tool name for this endpoint in MCP clients. |
| `description` | string | Override the tool description for this endpoint in MCP clients. |
      ...
      "get": {
        ...
        "x-mint": {
          "mcp": {
            "enabled": true,
            "name": "list_users",
            "description": "Search and retrieve user records with filtering options"
          }
        }
      }
    }
...
To disable MCP exposure for a specific endpoint, set `enabled` to `false`:
