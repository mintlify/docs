...
keywords: ["OpenAPI", "API specification", "Swagger", "MCP", "x-mint", "x-mcp"]
---
...
Control which API endpoints are exposed as tools on your [MCP server](/ai/model-context-protocol) by adding `x-mint: mcp` to any operation. By default, all endpoints in your OpenAPI specification are available as MCP tools. Use this extension to disable specific endpoints or customize the tool name and description that AI applications see.

<ResponseField name="mcp.enabled" type="boolean">
  Whether to expose this endpoint as an MCP tool. Set to `false` to exclude the endpoint from your MCP server. Defaults to `true`.
</ResponseField>

<ResponseField name="mcp.name" type="string">
  Custom name for the MCP tool. If not set, Mintlify generates a name from the operation.
</ResponseField>

<ResponseField name="mcp.description" type="string">
  Custom description for the MCP tool. If not set, Mintlify uses the operation's `summary` or `description`.
</ResponseField>
...
To exclude an endpoint from your MCP server, set `enabled` to `false`:
...
<Note>
  You can also use `x-mcp` as a top-level operation extension instead of nesting under `x-mint`. If both are present, `x-mint.mcp` takes precedence.
</Note>
