...
Overlays are separate JSON or YAML files that describe changes to apply to your OpenAPI specification. Use overlays to modify a specification without editing its source file. This is useful when the specification is generated by another tool or maintained by another team. Common uses include renaming paths, setting server URLs, removing internal endpoints, and improving descriptions for documentation.

Mintlify supports [OpenAPI Overlay Specification](https://spec.openapis.org/overlay/v1.1.0.html) versions 1.0 and 1.1. Mintlify applies overlays after parsing a specification and before validating it. Generated endpoint pages, navigation, `openapi` frontmatter references, and `mint validate` all use the transformed document.

### Create an overlay document

An overlay document has a top-level `overlay` version field, an `info` object with a `title` and `version`, and an `actions` array. Each action selects nodes in the specification with a `target` [RFC 9535 JSONPath](https://www.rfc-editor.org/rfc/rfc9535) expression and applies one modifier:
...
- `copy`: Copies the node selected by another JSONPath expression into each targeted node. The `copy` modifier requires Overlay 1.1.
...
The optional `extends` field links an overlay to a specification for [auto-discovery](#auto-discover-overlays). Set `extends` to a path relative to the overlay file or to the exact URL that your `docs.json` uses to reference a hosted specification.

### Reference overlays in your docs.json

List overlays explicitly using the object form of the `openapi` field. The object form works anywhere the `openapi` field is accepted, including inside arrays. Overlays apply in the order you list them.
...
Overlay paths must point to files inside your docs repository. Overlay URLs must use `https`. If two `openapi` entries reference the same specification with different `overlays` lists, the build fails.

### Auto-discover overlays

Mintlify treats any JSON or YAML file in your repository with a top-level `overlay` key as an overlay document. An overlay whose `extends` field resolves to a specification in your docs applies to that specification automatically. Auto-discovered overlays apply in alphabetical order of their file paths. Overlays without an `extends` field never apply automatically.

An explicit `overlays` list for a specification replaces auto-discovery for that specification. Set `"overlays": []` to disable all overlays for a specification, including auto-discovered ones.

Explicit and auto-discovered overlays fail differently. If an overlay in an explicit `overlays` list fails to load or apply, the specification fails validation and the deployment reports a spec error. If an auto-discovered overlay fails, Mintlify skips it and publishes the specification without it.

### Example: Rename a path
...
Reference the transformed specification everywhere in your docs. For example, a page frontmatter reference must use the post-overlay path: `openapi: "POST /credit/accounts"`.

When you run `mint dev`, editing or deleting an overlay file rebuilds the affected specifications. `mint validate` and `mint openapi-check` validate the transformed document, so validation errors reference your specification after overlays apply.
