...
You can create API pages from a single OpenAPI document or use multiple OpenAPI documents. There are two ways to work with multiple specifications:

1. **Navigation hierarchy**: Assign different OpenAPI specs to different groups in your navigation structure (each group references its own spec)
2. **Individual endpoints**: Reference specific endpoints from different specs inline in your pages array

<Warning>
  You cannot point multiple OpenAPI specifications to a single group. Each group can only reference one OpenAPI document.
</Warning>

### `$ref` support and limitations
...
- Local component references: `#/components/schemas/User`
- Nested references within properties, `oneOf`, `anyOf`, `allOf`, and `items`
- Chained references (references pointing to other references, up to 5 levels deep)
- Sibling properties alongside `$ref` in the same object

**Not supported:**
- External URL references: `https://example.com/schemas/User.json`
- File path references: `./schemas/User.json` or `../common/User.json`

If you need to use schemas or components from multiple files, you must either:
- Consolidate them into a single OpenAPI document, or
- Reference each OpenAPI file separately in your navigation structure (see examples below)
...
You can use multiple OpenAPI specifications by assigning each one to a different group in your navigation hierarchy. Each group can have its own OpenAPI spec:
...
This pattern allows you to organize different API specifications into separate sections of your documentation.
...
#### Individual endpoints from different specs

Reference specific endpoints from different OpenAPI specifications by including the file path with each endpoint:
...
- Mix endpoints from multiple OpenAPI specifications in the same navigation section
- Include only specific endpoints from a spec rather than all endpoints
- Integrate API endpoints alongside regular documentation pages
