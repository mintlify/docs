---
title: "OpenAPI setup guide for interactive API documentation"
...
Mintlify renders a file input in the API playground when your request body schema identifies a file. Use file inputs for endpoints that accept images, documents, archives, or other binary data.

For OpenAPI 3.1, set `type` to `string` and add `contentMediaType`.

Add `contentEncoding: base64` or `contentEncoding: base64url` when your API expects a base64-encoded file.

For OpenAPI 3.0, use `format: binary` or `format: base64` instead.
...
The playground supports common image, audio, video, and binary application media types. Users can drag a file onto the upload field or click the field to select one.
...
When enabled, clicking the option downloads your OpenAPI spec directly. Deployments with multiple specs receive them bundled as `api-specs.zip`. Mintlify hides the option for deployments behind `auth` or `userAuth` to prevent leaking spec contents from authenticated docs.
