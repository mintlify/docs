...
When enabled, selecting the option downloads your OpenAPI spec directly. Deployments with multiple specs receive them bundled as `api-specs.zip`. Mintlify hides the option for deployments behind `auth` or `userAuth` to prevent leaking spec contents from authenticated docs.
