...
Control which endpoints are exposed as [MCP tools](/ai/model-context-protocol) using `x-mint: mcp`. By default, all endpoints are available as MCP tools. Use this extension to exclude specific endpoints or customize their tool names and descriptions in MCP clients.

<Note>
  The `x-mint: mcp` extension replaces the legacy `x-mcp` extension. If both are present, `x-mint: mcp` takes precedence.
</Note>
...
You can also customize the name and description that MCP clients display for an endpoint:
    ...
    "/users": {
      "get": {
        ...
        "x-mint": {
          "mcp": {
            "name": "search_users",
            "description": "Search and retrieve a filtered list of users from the system."
          }
        }
      }
    }
...
<ResponseField name="enabled" type="boolean" default="true">
  Set to `false` to exclude this endpoint from your MCP server. Useful for internal or administrative endpoints that should not be available to AI tools.
</ResponseField>

<ResponseField name="name" type="string">
  Custom name for the MCP tool. Overrides the default name generated from the operation ID or endpoint path.
</ResponseField>

<ResponseField name="description" type="string">
  Custom description for the MCP tool. Overrides the default description generated from the operation's `summary` or `description` fields.
</ResponseField>
