...
Control which API endpoints are exposed as tools on your [hosted MCP server](/ai/model-context-protocol) using `x-mint: mcp`. When your documentation includes an OpenAPI specification, Mintlify can automatically expose individual endpoints as MCP tools that AI applications can call.

Set `enabled` to `true` to expose an endpoint as an MCP tool. You can also customize the tool `name` and `description` that AI applications see when they connect to your MCP server.
      ...
      "get": {
        ...
        "x-mint": {
          "mcp": {
            "enabled": true,
            "name": "list-users",
            "description": "Search and retrieve a list of users with optional filters"
          }
        }
      }
    }
...
|---|---|---|
| `enabled` | boolean | Whether to expose the endpoint as an MCP tool. |
| `name` | string | Custom name for the MCP tool. Defaults to the operation ID or a generated name from the method and path. |
| `description` | string | Custom description for the MCP tool. Defaults to the operation's summary or description. |

<Note>
You can also use the standalone `x-mcp` extension directly on an operation instead of nesting it under `x-mint`. If both `x-mint.mcp` and `x-mcp` are present, `x-mint.mcp` takes precedence.
</Note>
</div>
