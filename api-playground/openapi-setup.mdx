...
<Note>
  Enum fields are only prefilled in the API playground when you set an explicit `example` or `default` on the property. Mintlify does not automatically prefill the first enum value when neither is set, so the field appears empty until the user picks a value.
</Note>
