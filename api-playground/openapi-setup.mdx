...
When Mintlify auto-generates navigation groups from your OpenAPI tags, it uses the tag name as the group label. To display a different name in the sidebar, add the `x-group` extension to a tag definition. The `x-group` value becomes the group label while the original tag name is still used for URL paths.

This is useful when your tag names follow a code-friendly convention (like `CRMActions` or `EmailVerification`) but you want human-readable group names in the sidebar (like "CRM Actions" or "Email Verification").
...
In this example, the sidebar displays the group as "CRM Actions" while the generated page URL uses the original tag name `CRMActions` for the folder path.
