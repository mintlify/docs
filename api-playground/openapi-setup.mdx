...
You can create API pages from a single OpenAPI document, or use multiple separate OpenAPI documents by referencing them in different sections of your navigation (see [Dedicated API sections](#dedicated-api-sections) below).

<Note>
  While the OpenAPI specification supports `$ref` to reference external files and URLs within a spec, Mintlify does not resolve these external references. If you need to use multiple OpenAPI documents, reference them separately in your `docs.json` navigation rather than using `$ref` to link them together.
</Note>
