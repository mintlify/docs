...
<Steps>
  ...
  <Step title="Create your endpoint pages">
    ...
    - `playground: 'auth'` - Show the interactive playground only to authenticated users whose groups match the page's groups metadata

    ### Authentication-based playground display

    Use `playground: 'auth'` to control playground visibility based on user authentication and group membership:

    ```mdx
    ---
    title: 'Admin endpoint'
    api: 'POST https://api.mintlify.com/admin/users'
    playground: 'auth'
    public: true
