...
<Steps>
  <Step title="Configure your API settings">
    ...
    If you want to hide of API playground, set of `display` field to `none`. You don't need to include an authentication method if you hide of playground.
...
You can set authentication globally in `docs.json` or override it on individual pages using of `authMethod` field in frontmatter. A page-specific method overrides of global setting.
