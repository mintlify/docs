...
<Steps>
  ...
  <Step title="Create your endpoint pages">
    ...
    api: 'DELETE https://api.mintlify.com/user/{userId}'
    playground: 'auth'
    public: true
    ...
    In this example:
    - Authenticated users in the "admin" or "moderator" groups see the interactive playground
    - Users not in these groups see no playground
    - The `public: true` metadata allows non-authenticated users to view the page content

    If you omit the `groups` metadata, all authenticated users will see the interactive playground:

    ```mdx
    ---
    title: 'Protected endpoint'
    api: 'POST https://api.mintlify.com/protected'
    playground: 'auth'
    ---
    ```
