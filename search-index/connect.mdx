...
To configure an agent manually, follow the steps for your agent. Use the server name `mintlify-index` and the URL `https://index.mintlify.com/mcp` in each configuration.
...
<Tabs>
  ...
  <Tab title="VS Code" icon="code">
    Create or open `.vscode/mcp.json` in your project and add the server:
  ...
  <Tab title="Codex" icon="terminal">
    Add the server to `~/.codex/config.toml` or the project-level `./.codex/config.toml`:
  ...
  <Tab title="OpenCode" icon="terminal">
    Add the server to `~/.config/opencode/opencode.json` or the project-level `./opencode.json`:
  ...
  <Tab title="Zed" icon="code">
    Add the server to `~/.config/zed/settings.json` or the project-level `./.zed/settings.json`:
