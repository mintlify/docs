...
description: "Get better results from Mintie bot with clear, focused prompts."
keywords: ["prompts", "Mintie bot prompts", "effective prompting", "Mintie bot usage"]
---

Think of Mintie bot as a helpful assistant that needs your guidance to complete tasks. Give it clear instructions and context. More focused tasks are easier to complete, so break down complex projects into smaller steps.
...
If you have multiple documentation sites, include the `subdomain` parameter in your message to specify which documentation set Mintie bot should work on.
...
Use the format `@mintlify subdomain=<your-domain-name> <your-prompt>` to prompt Mintie bot to work on a specific documentation set.

- `@mintlify subdomain=public-docs Add a new section to the quickstart about inviting collaborators based on this PR`: Prompts Mintie bot to update the quickstart only on the `public-docs` site.
- `@mintlify subdomain=customer-docs Update the auth docs for the new authentication method`: Prompts Mintie bot to update the auth docs only on the `customer-docs` site.
