...
Context repositories can be one of the following:

- A repository connected through the Mintlify GitHub App.
- A project connected through the GitLab OAuth integration.
- A public GitHub repository in `owner/repo` format. Public repositories don't require the Mintlify GitHub App to be installed, but they must be accessible without authentication. Mintlify verifies that the repository is public when you save the workflow.

Use public GitHub repositories as context to reference open-source projects, SDKs, or upstream dependencies that the agent should consult when generating documentation updates.
