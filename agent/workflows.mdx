...
3. Pick a workflow template to start with pre-filled settings, or choose **Custom workflow** to build one from scratch.
4. Configure the workflow name, trigger type, and schedule.
5. Write the agent prompt with instructions for what to update.
6. Under **Settings**, optionally toggle on **Additional context repos**, **Auto-merge**, or **Slack notifications**.
