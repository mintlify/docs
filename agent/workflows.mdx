...
For GitLab repositories, you must have the [GitLab integration](/deploy/gitlab) configured, either through OAuth or with a valid access token and webhook.
...
<Note>
  ...
  For GitLab repositories, automerge uses the access token from your [GitLab integration](/deploy/gitlab). If you connected with OAuth, Mintlify uses the OAuth token automatically. If you connected with an access token, ensure the token has sufficient permissions to merge merge requests on the target branch.
</Note>
