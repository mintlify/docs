...
- **Connected repositories**: Repositories from your connected GitHub or GitLab accounts. For GitHub, the Mintlify GitHub App must be installed on the repository. For GitLab, the project must be connected through GitLab OAuth.
- **Public GitHub repositories**: Any public repository on GitHub.com. Public context repositories don't require the Mintlify GitHub App to be installed. Enter the repository as `owner/repo` or paste a full GitHub URL. Mintlify validates that the repository is public before adding it. Public GitLab repositories are not supported.

<Warning>
  Public repositories can contain prompt injection or poisoned context. Only add repositories you trust as workflow context.
</Warning>
