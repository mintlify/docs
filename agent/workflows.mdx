...
For GitLab repositories, connect your GitLab account via OAuth on the [GitLab OAuth](https://dashboard.mintlify.com/settings/organization/gitlab-oauth) page of your Mintlify dashboard. After connecting, expand a connection to select the projects the agent can access.
...
<Note>
  ...
  For GitLab repositories, automerge uses the OAuth connection configured on the [GitLab OAuth](https://dashboard.mintlify.com/settings/organization/gitlab-oauth) page. Ensure the connected GitLab account has sufficient permissions to merge merge requests on the target branch.
</Note>
