...
The [Workflows](https://dashboard.mintlify.com/products/workflows) page in the dashboard has two tabs: **Run history** and **Workflows**.

### Run history

The **Run history** tab is the default view. It shows a consolidated list of runs across all your workflows with the following columns:

- **Time**: When the run started and completed.
- **Workflow**: The workflow name and the triggering event, such as a merged pull request.
- **Status**: The current state of the run. Possible statuses are:
  - **Queued**: The run is waiting to start.
  - **Running**: The run is in progress.
  - **Review needed**: The run completed and opened a pull request that needs review.
  - **PR merged**: The pull request from the run was merged.
  - **PR closed**: The pull request from the run was closed without merging.
  - **No action needed**: The run completed without opening a pull request.
  - **Failed**: The run encountered an error.
- **Summary**: A description of what the run did, with action buttons to view the pull request, open the changes in the editor, or close the pull request.

By default, the run history shows only runs with open pull requests so you can focus on actionable items. Toggle **Show all runs** to see all historical runs including merged and closed pull requests.

Use the workflow filter dropdown to show runs for a specific workflow.

### Workflows tab

The **Workflows** tab lists your active workflows. Click a workflow to view or edit its configuration.
