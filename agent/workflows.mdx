...
Use `context` to give the agent read access to additional repositories when the workflow runs. This is useful when your prompt requires reviewing code or content outside your documentation repository. You can specify up to 5 context repositories per workflow.
...
Workflows created in the dashboard can be edited and deleted from the dashboard. Workflows created from files in your repository can only be modified by editing the file and pushing the change — they cannot be edited or deleted from the dashboard.

### Trigger a workflow manually

You can run a cron-based workflow on demand without waiting for the next scheduled time. Open the workflow in the [dashboard](https://dashboard.mintlify.com/products/workflows) and click **Run now**. Manually triggered runs count toward the daily run limit.

## Workflow runs

Each time a workflow executes, it creates a run that you can monitor on the [Workflows](https://dashboard.mintlify.com/products/workflows) page. The runs view shows all runs across your workflows with details about their status, trigger source, and outcome.
...
| **Running** | The agent is actively working on the workflow prompt. |
| **Completed** | The agent finished. Check the run for a pull request link and summary. |
| **Failed** | The run encountered an error. Failed runs do not count toward the daily run limit. |

### Run details

Each completed run includes:

- **Summary** — A description of what the agent did, generated by the agent.
- **Pull request link** — A link to the pull request the agent opened, if any.
- **PR status** — Whether the pull request is open, merged, or closed.
- **Trigger source** — Whether the run was triggered by a cron schedule, a manual trigger, or a merge event.
- **Usage** — Token usage and cost for the run.

### Close a pull request

You can close a workflow-generated pull request directly from the dashboard without visiting GitHub. In the runs view, find the run with the pull request you want to close and click **Close PR**.
...
Runs weekly to check for pages with missing or weak metadata and open a pull request with improvements. This example workflow checks for missing or weak `title` and `description` frontmatter. Edit the workflow to check for other metadata or content issues that you prioritize for your documentation.
