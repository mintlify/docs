...
By default, the agent opens a pull request for each workflow run so you can review changes before they go live. Set `automerge: true` to automatically merge the pull request after it's created.

```yaml
automerge: true
```

When `automerge` is enabled, the workflow still creates a pull request, runs any configured checks, and then merges it. This gives you a record of changes in your repository's pull request history while automating the merge step.
