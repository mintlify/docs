...
For GitLab repositories, you must connect GitLab via OAuth so that workflows can pull code, push commits, and create merge requests. See [Connect GitLab via OAuth](/deploy/gitlab#connect-gitlab-via-oauth) for setup instructions.
...
<Note>
  ...
  For GitLab repositories, automerge uses the OAuth connection configured in your [GitLab OAuth settings](/deploy/gitlab#connect-gitlab-via-oauth). Ensure the authorizing user has sufficient permissions to merge merge requests on the target branch.
</Note>
