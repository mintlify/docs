...
Send Slack messages when a workflow opens or merges a pull request. Notifications include the workflow name, a link to the pull request, and a summary of the changes.

<Note>
Slack notifications require the Mintlify Slack app in your organization's Slack workspace. Install the Slack app from your [dashboard](https://dashboard.mintlify.com/products/workflows).
</Note>

You can configure Slack notifications globally for all workflows or per workflow.

#### Global notifications

Configure a default set of Slack channels that receive notifications for every workflow in your organization. Use this when you want a single team channel to track all workflow activity without configuring each workflow individually.

1. Open the [Workflows](https://dashboard.mintlify.com/products/workflows) page in your dashboard.
2. Click **Notifications** in the top right.
3. If you haven't connected Slack, click **Connect Slack** to start the OAuth flow.
4. Search for and select the channels you want to receive workflow notifications.
5. Click **Save changes**.

Selected channels appear as badges in the modal and can be removed individually. When no channels are configured, a banner appears on the Workflows page prompting you to set them up. Dismiss the banner if you prefer not to use Slack notifications.

#### Per-workflow notifications

When creating or editing a workflow in the dashboard, you can override or supplement the global channel list under the **Notify on completion** toggle:
