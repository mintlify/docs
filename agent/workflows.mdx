...
For GitLab repositories, any repository connected to your Mintlify organization through a [GitLab deployment](/deploy/gitlab) is automatically available as a trigger or context repository in workflows.
...
Run a workflow when changes push to a specific repository or branch. This includes both pull request merges (or merge request merges on GitLab) and direct pushes to the branch.
...
- `repo`: The repository in `owner/repo` format. This works with both GitHub and GitLab repositories.
