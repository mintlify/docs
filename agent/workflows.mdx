...
Hobby plans are limited to 100 workflow runs per month across all workflows. Non-failed runs within a rolling 30-day window count toward this limit. [Upgrade your plan](https://mintlify.com/pricing) to remove the monthly cap.
...
You can run cron-based workflows on demand without waiting for the next scheduled time. Manually triggered runs count toward the daily and monthly run limits.
