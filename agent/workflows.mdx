...
| `notify` | No | Notification configuration. See [Slack notifications](#slack-notifications). |
...
Configure workflows to send Slack notifications when they complete or fail. This helps your team stay informed about workflow results without checking the dashboard.

To use Slack notifications, you must first [connect Slack to your Mintlify organization](/ai/slack-bot).
...
The `notify.slack` field accepts the following options:

| Field | Description |
|-------|-------------|
| `channels` | List of Slack channel names to notify. Omit the `#` prefix. |
| `channel_ids` | List of Slack channel IDs to notify. Use this if channel names are ambiguous. |
| `users` | List of Slack usernames to notify via direct message. Omit the `@` prefix. |
| `user_ids` | List of Slack user IDs to notify via direct message. |

You can combine these options to notify both channels and individual users.

Notifications include:
- Workflow status (completed or failed)
- Link to the pull request
- Summary of the changes (for successful runs)
...
### Auto-merge with Slack notifications

Runs when you push changes to your product repository to update documentation automatically and notify your team via Slack. This example uses `automerge: true` to merge changes without manual review, which is useful for low-risk updates like changelog entries.
...
Review the diff from the last merged PR in `your-org/your-product`. If the changes include new features, bug fixes, or deprecations, add a changelog entry.

Format entries with a date header, feature name, and brief description of what changed and how it affects users.

Success criteria: Users can read the changelog to understand what changed in the latest release.

## Important

- Only add entries for user-facing changes.
- Match the format and tone of existing changelog entries.
```
