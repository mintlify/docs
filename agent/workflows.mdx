...
| `notify` | No | Notification configuration. See [Notifications](#notifications). |
...
<Note>
  Slack notifications require a Slack integration. Connect Slack on the [Slack integration](https://dashboard.mintlify.com/settings/organization/integrations/slack) page in your dashboard.
</Note>

Send Slack notifications when a workflow completes or fails. Notifications include the workflow name, status, a link to the pull request, and a summary of changes.
...
You can specify recipients by name or by ID. Using IDs is faster because it skips the lookup step.

| Field | Description |
|-------|-------------|
| `channels` | List of Slack channel names (with or without `#`). |
| `channel_ids` | List of Slack channel IDs. |
| `users` | List of Slack usernames, display names, or real names. |
| `user_ids` | List of Slack user IDs. |

Notifications show the workflow status and whether the pull request was auto-merged or needs review.
...
### Auto-merge with team notifications

Runs when changes push to your product repository and automatically merges documentation updates. The workflow notifies your team in Slack when it completes so they can review the changes.

```markdown .mintlify/workflows/sync-api-docs.md
---
name: Sync API docs
on:
  push:
...
Review the diff from the last merged PR in `your-org/your-product` for changes to API endpoints, parameters, or response shapes.

Update the corresponding API reference pages to reflect the changes. Include updated parameter descriptions and examples where affected.

Success criteria: API reference pages accurately reflect the current state of the API.

## Important

- Only update API reference pages. Do not modify other documentation.
- If no API changes were introduced, do nothing.
```
