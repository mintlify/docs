...
The [Workflows](https://dashboard.mintlify.com/products/workflows) page in your dashboard has two tabs: **Run history** and **Workflows**.

### Run history

The **Run history** tab shows a unified view of all workflow runs across your workflows. Each row displays the run time, workflow name, status, and a summary of the result.

By default, the list shows only runs with open pull requests. Toggle **Show all runs** to see every run, including completed and closed runs. You can also filter by a specific workflow using the workflow dropdown.

The run history refreshes automatically every 30 seconds.

#### Run statuses

Each run displays a status based on its current state and the state of any associated pull request:

| Status | Description |
|--------|-------------|
| **Queued** | The run is waiting to start. |
| **Running** | The agent is actively working on this run. |
| **Review needed** | The run completed and a pull request is open for review. |
| **PR merged** | The pull request from this run was merged. |
| **PR closed** | The pull request from this run was closed without merging. |
| **Failed** | The run encountered an error. |
| **No action needed** | The run completed without opening a pull request. |

#### Manage pull requests

From the run history, you can take action on runs directly without leaving the dashboard:

- **View PR** opens the associated pull request on GitHub.
- **View in editor** opens the run's branch in the Mintlify web editor so you can review or edit changes before merging.
- **Close PR** closes the pull request on GitHub directly from the dashboard.

### Workflows tab

The **Workflows** tab displays your configured workflows as cards. Click a workflow card to view its run history filtered to that workflow. Click the gear icon on a card to edit the workflow's configuration.
