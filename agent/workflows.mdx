...
You can disable a workflow to temporarily stop it from running without deleting it. Disabled workflows keep their configuration and run history, but do not execute on their schedule or in response to push events. You cannot manually trigger a disabled workflow.
...
When you re-enable a cron-based workflow, its next run time is recalculated from the current time. Re-enabling a workflow counts toward your plan's workflow limit.
