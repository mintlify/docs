...
| `notify` | No | Notification configuration. Currently supports Slack notifications. |
...
Send notifications to Slack when a workflow completes or fails. You can notify channels, users, or both.

<Note>
  Slack notifications require a non-knowledgebase Slack installation on your organization. Connect Slack on the [Slack settings](https://dashboard.mintlify.com/settings/organization/slack) page.
</Note>
...
The `notify.slack` field supports the following properties:

| Property | Description |
|----------|-------------|
| `channels` | Channel names to notify (e.g., `#docs-updates` or `docs-updates`). |
| `channel_ids` | Slack channel IDs to notify directly. |
| `users` | Usernames to notify via DM (e.g., `@jane` or `jane`). Matches against username, display name, or real name. |
| `user_ids` | Slack user IDs to notify via DM directly. |

Notifications include the workflow name, status, a link to the pull request (if applicable), and a summary of changes or error message.
