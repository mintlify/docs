---
title: "Workflows"
description: "Automate documentation maintenance with scheduled or event-triggered Mintie bot tasks."
...
Workflows run Mintie bot automatically on a schedule or on a push to a repository. Each workflow defines a prompt for Mintie bot and a trigger for when to run it.

When a workflow runs, Mintie bot clones any specified repositories as context, follows the prompt, and either opens a pull request or pushes changes directly to your deployment branch.
...
3. Configure the trigger, prompt, and other settings, then click **Create workflow**.
...
Workflow files use YAML frontmatter to configure the workflow, followed by a Markdown prompt for Mintie bot.
...
Use `context` to give Mintie bot read access to additional repositories when the workflow runs. This is useful when your prompt requires reviewing code or content outside your documentation repository.
...
<Note>
Slack notifications require a Slack installation on your organization. Connect Slack on the [Slack integration](https://dashboard.mintlify.com/settings/organization/integrations/slack) page of your dashboard.
</Note>

## Prompts

Effective prompts focus on one task and have a specific outcome in mind. Workflows always have some variance because of the nondeterministic nature of AI, but you can improve the consistency of workflow outputs by following these best practices.

- Describe the outcome you want Mintie bot to achieve.
- Include success criteria.
- Specify the context you want Mintie bot to use.
- Split complex tasks into steps or multiple workflows.

### Mintie bot environment

Mintie bot runs in an isolated sandbox with restricted internet access. When writing prompts, only reference tools available to Mintie bot.
...
Mintie bot cannot install additional packages or tools at runtime. Package registries and other external services are not reachable from the sandbox.

Prompts that instruct Mintie bot to run unavailable tools may produce unexpected results or fail.
...
<Tip>
  If you use Mintie bot suggestions in your dashboard, this workflow replicates that behavior.
