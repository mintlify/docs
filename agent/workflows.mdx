...
View the status and history of all workflow runs on the [Workflows](https://dashboard.mintlify.com/products/workflows) page. Each run shows whether its pull request is open, merged, or closed so you can track outcomes without switching to GitHub.

To close a workflow's pull request directly from the dashboard, click the run and select **Close PR**.
...
Slack notifications update automatically when the workflow's pull request is merged or closed on GitHub, so your team can track the full lifecycle of each workflow run without leaving Slack.
