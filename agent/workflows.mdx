...
For GitLab repositories, connect your GitLab account through [OAuth](/deploy/gitlab#connect-with-oauth) in the Mintlify dashboard. Alternatively, you can configure the [GitLab integration](/deploy/gitlab#connect-with-access-tokens) with an access token and webhook.
