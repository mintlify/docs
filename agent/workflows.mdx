...
For GitHub repositories, you must have the Mintlify GitHub App installed on every repository listed in the context or push trigger fields. Add new repositories on the [GitHub app](https://dashboard.mintlify.com/settings/organization/github-app) page of your Mintlify dashboard.
...
The schedule uses a standard 5-field cron expression in `minute hour day-of-month month day-of-week` format. Use a tool like [crontab.guru](https://crontab.guru) to build and validate schedules.
...
- Specify the repository in `owner/repo` format.
- Optionally specify a branch to watch. If you don't specify a branch, the workflow triggers on pushes to the repository's default branch.

A workflow can watch for pushes to multiple repositories or branches.

### Context repositories

Give the agent read access to additional repositories when the workflow runs. This is useful when your prompt requires reviewing code or content outside your documentation repository. You can specify up to five context repositories per workflow.

### Auto-merge changes

By default, the agent opens a pull request for each workflow run so you can review changes before they go live. Enable automerge to automatically merge the pull request without requiring manual approval. This gives you a record of changes in your repository's pull request history while automating the merge step.
...
**Context repositories:** `your-org/your-docs`
...
**Trigger:** Cron schedule (`0 9 * * 3` — every Wednesday at 9:00 AM UTC)
...
**Trigger:** Cron schedule (`0 9 * * 1` — every Monday at 9:00 AM UTC)
...
**Context repositories:** `your-org/your-product`

**Automerge:** Off

**Slack notifications:** Notify `#documentation` channel and `tech-writer` user
