...
<Note>
  File-based workflows (`.mintlify/workflows/*.md`) are deprecated. Existing file-based workflows continue to run, but create and manage all workflows from the dashboard.
</Note>
...
Specify each repository in `owner/repo` format. Optionally specify a branch to watch — if you don't, the workflow triggers on pushes to the repository's default branch.
...
<Note>
  ...
  For GitLab repositories, automerge uses the access token configured in your [GitLab integration](/deploy/gitlab). Ensure the token has sufficient permissions to merge merge requests on the target branch.
</Note>
