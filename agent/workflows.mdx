...
There are two types of context repositories:

- **Connected repositories**: Repositories in your organization connected through the Mintlify GitHub App or GitLab OAuth. Use connected repositories for private source code, internal tooling, or other repositories your team owns.
- **Public GitHub repos**: Any public GitHub repository, even if it isn't connected to your organization. Enter the repository URL in `owner/repo` format or as a full GitHub URL (for example, `https://github.com/owner/repo`). Mintlify verifies that the repository exists and is public before adding it. Use public repos to reference open-source libraries, SDKs, or external documentation that informs your docs.

<Warning>
  Public repositories can contain prompt injection or poisoned context. Only add public repositories that you trust as workflow context.
</Warning>

Public GitLab repositories are not supported as context.
