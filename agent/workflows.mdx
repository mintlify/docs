...
You can have up to 10 active workflows. Each workflow can run up to 50 times a day. Runs that fail do not count toward this limit.
...
4. In GitHub, review and merge the pull request that adds the workflow file to your repository.
...
The agent runs in an isolated sandbox with restricted internet access. When writing prompts, only reference tools the agent has available:
...
Prompts that instruct the agent to run unavailable tools fail.
