...
Workflows run the agent automatically when content changes, on a schedule, or when source code changes. Each workflow defines a prompt for the agent and a trigger for when to run it. Workflows support both GitHub and GitLab repositories.
...
<Tip>
Use **Content update** workflows for tasks that should run whenever your documentation is updated, like translation lag checks or style audits.

Use **Custom schedule** workflows for recurring tasks like publishing changelogs or checking for broken links.

Use **Code change** workflows for reactive maintenance tasks like updating API references or drafting documentation for new features when source code merges.
</Tip>
...
Each workflow defines exactly one trigger. When you create a workflow from a template, the dashboard preselects the recommended trigger for that template. For custom workflows, choose the trigger kind that best matches the task.

| Trigger | When it runs |
|---------|--------------|
| **Content update** | When your documentation is updated and a new deployment is published. |
| **Custom schedule** | On a recurring cron schedule. |
| **Code change** | When changes are pushed to a connected source repository. |

#### Content update

Run a workflow whenever your documentation deploys. This is useful for tasks that should react to documentation changes themselves, like style audits or translation lag checks.

Content update triggers don't require any additional configuration. The workflow runs after each successful deployment of your docs.
...
**Trigger:** Code change — push to `your-org/your-product` (main branch)
...
**Trigger:** Content update (runs after each docs deployment)
...
**Trigger:** Custom schedule `0 9 * * 3` (Wednesdays at 9:00 AM UTC)
...
**Trigger:** Custom schedule `0 9 * * 1` (Mondays at 9:00 AM UTC)
