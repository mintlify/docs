...
For private repositories, the Mintlify GitHub App must be installed on the repository (or the GitLab project must be connected through GitLab OAuth). You can also reference **public GitHub repositories** as context without installing the GitHub App by marking them as public in the workflow configuration. Mintlify verifies that the repository exists and is public when the workflow is created.
