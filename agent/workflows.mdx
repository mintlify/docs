...
For GitLab repositories, connect your GitLab projects via OAuth on the [GitLab OAuth](https://dashboard.mintlify.com/settings/organization/gitlab-oauth) page of your Mintlify dashboard. Alternatively, you can use the [GitLab integration](/deploy/gitlab) with an access token and webhook.
...
<Note>
  ...
  For GitLab repositories, automerge uses the credentials from your [GitLab OAuth](https://dashboard.mintlify.com/settings/organization/gitlab-oauth) connection or the access token configured in your [GitLab integration](/deploy/gitlab). Ensure the account or token has sufficient permissions to merge merge requests on the target branch.
</Note>
