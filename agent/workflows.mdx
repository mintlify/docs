...
<Note>
  Workflows automatically skip push events caused by their own merged pull requests. This prevents a workflow from re-triggering itself in an infinite loop.
</Note>
...
By default, the agent opens a pull request for each workflow run so you can review changes before they go live. Set `automerge: true` to automatically squash-merge the pull request without requiring manual approval. This gives you a record of changes in your repository's pull request history while automating the merge step.
