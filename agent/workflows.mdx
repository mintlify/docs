...
- **Connected repos**: Private or public repositories connected to your organization through the Mintlify GitHub App or GitLab OAuth. The agent has full read access to these repositories.
- **Public GitHub repos**: Any public GitHub repository, even if it isn't connected to your organization. Add a public repo by entering its URL or `owner/repo` slug. Mintlify validates that the repository exists and is public before adding it.

<Warning>
  Public repositories can contain prompt injection or poisoned context. Only add public repositories you trust as workflow context.
</Warning>
