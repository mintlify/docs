...
| **Type** | No | A built-in workflow template that supplies a maintained prompt for a common task. See [Workflow types](#workflow-types). |
...
Workflow types are built-in templates for common documentation tasks. When you choose a type, Mintlify supplies a maintained prompt that is tuned for that task, so you do not need to write one from scratch. Choose **Custom** to write your own prompt instead.

| Type | Use case |
|------|----------|
| **Changelog** | Summarize recent merged changes from a product repository into a changelog entry. |
| **Source code agent** | Inspect a product repository on push and draft docs updates for new features, APIs, or behavior changes. |
| **Translations** | Compare English pages against translated subdirectories and flag pages that are out of sync. |
| **Writing style** | Review recent changes against a style guide and open a pull request that resolves style violations. |
| **Typo check** | Scan for typos and grammar issues and open a pull request with corrections. |
| **Broken link detection** | Find and fix broken internal and external links in your docs. |
| **SEO metadata audit** | Audit `title` and `description` frontmatter across pages and propose improvements. |

Each type can be used at most once per deployment. To customize a built-in type, add extra instructions in the prompt field when you create the workflow. Your instructions are appended to the built-in template, so you can layer project-specific context (such as style guide paths or repositories to audit) without rewriting the base prompt.

To combine multiple tasks or write fully custom behavior, create a **Custom** workflow and supply your own prompt. See [Example prompts](#example-prompts).
...
Each workflow run (instance) appears on the workflow's detail page with a link to the pull request or merge request it opened. From the dashboard you can act on that pull request without leaving Mintlify:

- **Merge**: merge the pull request directly. For GitHub, this uses the Mintlify GitHub App and honors the same bypass rules as [auto-merge](#auto-merge-changes). For GitLab, this uses your GitLab OAuth connection and requires at least the Maintainer role on the project.
- **Close**: close the pull request without merging. The run stays in the history so you can revisit the changes later.

Both actions are available for GitHub and GitLab workflow runs. Once a pull request is merged or closed, the buttons update to reflect its state.
