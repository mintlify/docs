...
Comments on pull requests from forks are not supported because the agent cannot push to fork branches. For changes to fork pull requests, share the pull request link with the agent from another surface (such as Slack or the dashboard) so it can open a follow-up pull request against your repository instead.
