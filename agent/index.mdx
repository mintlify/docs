...
You can send prompts to the agent from:

- **Slack**: Mention `@Mintlify` in any channel the agent is in, or DM the app directly. See [Add the agent to Slack](/agent/slack).
- **The dashboard**: Open the agent panel from the [Mintlify dashboard](https://app.mintlify.com) to start a new conversation.
- **Automations**: Trigger the agent on a schedule or from an event. See [Automations](/automations/index).
