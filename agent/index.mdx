...
## Reference plugins and pages with @ mentions

When you prompt the agent from the [Agent](https://dashboard.mintlify.com/products/agent) page in the dashboard, type `@` in the composer to open a picker that lets you reference connected integrations and specific pages directly in your message.

- **Plugins**: Mention `@Slack`, `@Linear`, `@Notion`, or `@GitHub` to scope a request to a connected integration. Plugins that are not yet connected appear with a **Connect** indicator so you can set them up before referencing them.
- **Pages**: Mention any page from your documentation by title or filename to point the agent at the exact content you want it to read or edit.

Mentions are inserted as chips in the composer, so you can review and remove them before sending. For example:

```
Update @Authentication to match the new flow described in @Linear issue ENG-1234.
```

The agent uses each mention as structured context when researching, planning, and drafting changes.
