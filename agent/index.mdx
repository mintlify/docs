...
As the agent works in the editor's agent panel, you can interact with each message:

- **Copy**: Copy a message to your clipboard to reuse the content. Assistant messages preserve code block formatting when pasted into the editor.
- **Feedback**: Rate assistant responses with thumbs up or thumbs down to help improve agent quality.
- **Timestamp**: Hover over a message to see when it was sent.

Feedback and copy actions are available on completed messages. Buttons appear after a response finishes streaming.
