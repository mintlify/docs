...
<Tip>
  If you install the Slack app but close the dashboard before finishing the connection, the agent sends you a direct message in Slack with a **Connect to Mintlify** button. Click the button to return to the dashboard and finish linking your workspace. The prompt is only sent to the person who installed the app and is sent once per workspace.
</Tip>
