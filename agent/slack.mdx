...
Question-only requests run on a read-only fast path. The agent skips checking out your repository when it doesn't need to edit files, so answers come back sooner.
...
When the agent appends commits to a pull request, it leaves a comment describing what changed and links to any code pull requests that were used as context. It also rotates the assigned reviewer so the update reaches someone new.

## Manage automations

The Slack agent and [Automations](/automations) are the same system. Ask the agent to create, configure, run, or troubleshoot any automation. It can perform every action available to you on the [Automations](https://app.mintlify.com/products/automations) page of your dashboard.
...
- "Run the code change automation on server PR #7003."
...
## Use the agent outside of channels and DMs

The Slack agent is also available in other places you already work:

- **Slack sidepanel**: Chat with `@mintlify` or the `@kb` knowledge base agent in the Slack side panel while you keep working in a channel.
- **Linear issues**: Mention `@mintlify` on a Linear issue to open a docs pull request based on the issue.
- **GitHub pull requests**: Mention `@mintlify` on a pull request in a connected code repository to open a matching docs pull request.

Each of these behaves like the Slack agent: same permissions, same tools, and the same review process for any changes it makes.
