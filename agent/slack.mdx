...
The agent's capabilities in Slack match the [role](/dashboard/roles) of the linked Mintlify account:
...
- **Viewers**: Can ask questions only. The agent runs in read-only mode, so the file-editing, commit, and pull request tools are unavailable. Third-party integrations are filtered to read-only actions. If a viewer asks the agent to make a change, it explains the limitation and suggests asking an editor or admin teammate.

To change a teammate's permissions, update their role from the [Members](https://dashboard.mintlify.com/settings/organization/members) page in your dashboard.
