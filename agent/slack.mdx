...
- **With attachments**: Upload images, diagrams, code files, or other documents with your message. The agent automatically processes and includes them in your documentation. The agent also picks up files and images posted earlier in the same thread since its last reply, so you can share a screenshot first and then mention `@mintlify` in a follow-up message to act on it.
