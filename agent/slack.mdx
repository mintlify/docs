...
Ask the agent to create, configure, or run [automations](/automations) instead of setting them up on the [Automations](https://app.mintlify.com/products/automations) page of your dashboard. The agent checks your existing automations first and extends or re-enables one that already covers your request instead of creating a duplicate.
...
You can also ask the agent about automation activity, such as recent runs, failures, or which automations use the most credits.

- "Which automations burned the most credits this week?"
- "Why did the last run of the code change automation fail?"

<Note>
  The agent's ability to manage automations from Slack is still evolving. Review any automation the agent creates or changes on the [Automations](https://app.mintlify.com/products/automations) page to confirm the trigger, instructions, and context match what you asked for.
</Note>
