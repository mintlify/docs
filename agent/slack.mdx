...
1. **Trigger**: You mention `@mintlify` in a channel, send the agent a direct message, or reply in an existing thread where the agent is active.
2. **Context gathering**: The agent reads the current message, the rest of the thread (including files, images, and links shared earlier), and your connected documentation. If your request references another repository, integration, or workflow, the agent pulls that context in on demand.
3. **Plan and work**: The agent breaks the request into tasks, then searches, reads, and edits files to complete them. Progress streams back to the thread as a live-updating status message.
4. **Human checkpoints**: The agent pauses to ask for clarification or to approve any third-party action that writes data, such as creating a ticket or sending an email.
5. **Delivery**: For questions, the agent replies in the thread. For content changes, it opens a pull request against your documentation repository and posts links to the PR and the web editor.

A few properties are worth calling out:

* **Threads are the unit of memory.** The agent treats the whole thread as its context window, so follow-ups don't need to restate what was already shared. Starting a new top-level message (not a reply) starts a fresh conversation with no prior context.
* **Identity and permissions are per user.** The agent acts as the Slack user who triggered it, using that person's Mintlify [role](/dashboard/roles) and their personal integration connections. Two people asking the same thing in the same thread can see different results if their permissions or connections differ.
* **Visibility matches Slack.** Anyone in a channel can see the agent's public replies. Approval prompts and other user-specific messages are sent as ephemeral messages that only the requester can see.
* **The agent never pushes to your deploy branch.** Content changes always land on a new branch as a pull request for you to review before merging.
