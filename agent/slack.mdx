...
The agent can read source code from any repository where the Mintlify GitHub App is installed, even if it isn't your documentation repository. Use this to ground documentation updates in code that lives in a separate repo.
...
Cloned repositories are read-only. The agent uses them for context, but pull requests are always opened against your documentation repository.

If the agent reports that the GitHub App is not installed on the referenced owner, install the [Mintlify GitHub App](https://github.com/apps/mintlify/installations/new) on that organization or user account and try again.
