...
- **Permission requests**: Before the agent runs a third-party action that writes data, such as sending an email or creating a ticket, it asks you to approve or deny the action. The approval card is sent as an ephemeral message visible only to you, the person who triggered the request, so other thread participants are not interrupted. Click **Approve** to let the agent proceed or **Deny** to cancel the action.
