...
Each time you mention `@mintlify` or DM the agent, it runs as a scoped conversation tied to your Slack account and your Mintlify organization.

* **Conversation scope**: Each Slack thread is a separate conversation. The agent has access to every message, file, image, and reaction in the thread it is replying in, but does not carry context across unrelated threads. Start a new thread when you want a clean slate.
* **Identity and permissions**: The agent acts on behalf of the Slack user who mentioned it. It looks up that user's linked Mintlify account and applies their [role](/dashboard/roles) to every action—editors and admins can open pull requests, viewers get read-only tools.
* **Docs context**: The agent reads your published documentation through the same index that powers the in-product assistant, so it can search and cite pages without cloning your docs repo on every request.
* **Code context**: When a request references another repository connected through the [GitHub app](/deploy/github) or [GitLab integration](/deploy/gitlab), the agent shallow-clones it on demand into a sandbox and inspects the relevant files before drafting changes. Clones are read-only and discarded after the run.
* **Writes go through pull requests**: The agent never pushes to your deploy branch. File edits land on a new branch in your docs repo and are surfaced as a pull request that you review and merge.
* **Third-party actions**: Connected integrations (Linear, Notion, Jira, and so on) are gated per user. Read actions run automatically; write actions prompt you for explicit approval in the thread before they run.
* **Sandbox**: All file reads, edits, and shell commands run in an isolated per-conversation sandbox with no outbound network access beyond the APIs the agent is explicitly wired to.
