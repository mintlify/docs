...
- Attach files and images directly to your messages. The agent reads their content, so it can use them as context for an answer or include them in your docs.
...
Pure questions automatically run on a faster, read-only path, regardless of your role. If your request also includes an edit, the agent switches to the full editing flow.
...
- **With an image**: Attach a screenshot, diagram, or mockup to your question. The agent reads the image and uses it as context for its answer, even if you don't reference it directly in your docs.
...
- "Based on this screenshot of our design system, does the button styling in our components docs match?"
