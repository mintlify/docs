...
Integrations that you connect from Slack apply to individual Slack users, so each teammate sets up their own connections and the agent uses the connections of whoever sent the message. If a Slack user has not connected a service, the agent falls back to the organization-level integrations installed from the [Integrations](https://dashboard.mintlify.com/settings/organization/integrations) page in the dashboard.

To install, remove, or reauthorize an organization-level integration, go to **Settings** > **Workspace** > **Integrations**. Connect a service to make it available to the agent across the entire workspace, or disconnect a service to revoke its access. Only toolkits that support managed authentication appear in the list, so every shown integration can be connected directly from the dashboard.
