...
If your organization has multiple deployments, the agent asks you to choose which
deployment to work with the first time you send a request. Select a deployment
from the dropdown and the agent processes your request against that deployment's
repository.

After selecting a deployment, the agent asks if you want to set it as your
default. Choosing **Yes** saves the deployment as your preferred default so future
requests use it automatically. Choosing **No thanks** skips this step—the agent
will ask again after 24 hours.
