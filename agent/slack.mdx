...
### How the agent ships changes

The agent never pushes directly to your deploy branch. Every documentation change follows the same path so you can review before anything goes live:

* **Feature branch**: The agent commits to a new branch named like `mintlify/<session-id>`, scoped to the current Slack thread. Follow-up requests in the same thread push additional commits to that branch so the pull request stays in sync.
* **Pull request**: When the branch has at least one commit, the agent opens a pull request against your deploy branch (for example, `main`) with a summary of what changed and why.
* **Review and merge**: You review, request changes, or merge the pull request in GitHub or GitLab. Merging triggers your normal deploy flow.
* **Validation**: Before pushing, the agent runs `mint validate` and `mint broken-links` in the sandbox and fixes any errors it introduced.

If you ask the agent to "push to main" or otherwise bypass review, it still opens a pull request instead. Branch protection on your deploy branch is respected—if a push is blocked, the agent surfaces the error rather than working around it.
