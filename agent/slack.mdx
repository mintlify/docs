...
The Slack agent is the same agent that runs in the Mintlify web editor, exposed as a Slack bot. Each request flows through the same steps:

- **Trigger**: The agent responds to direct messages and to `@mintlify` mentions in channels it has been added to. Inside an existing thread, you can reply without re-mentioning the agent—it stays subscribed to the thread until the conversation ends.
- **Context gathering**: The agent reads your message, any attachments, and the surrounding thread history. It also draws on your connected documentation repository, any additional repositories connected through the [GitHub app](/deploy/github) or [GitLab integration](/deploy/gitlab), and any per-user [integrations](/automations/integrations) you have authorized, such as Notion, Linear, or Google Drive.
- **Identity and permissions**: Every request runs as the Mintlify user linked to the Slack account that triggered it. The agent's tools are gated by that user's [role](/dashboard/roles), so admins and editors can open pull requests while viewers get read-only access. Third-party actions use each user's own OAuth tokens.
- **Planning and tools**: For non-trivial work, the agent breaks the request into tasks and picks tools one step at a time—searching docs, reading files, editing MDX, cloning a referenced repo, or calling a connected integration. It streams progress back into the thread as it goes and can be interrupted with a follow-up message.
- **Output**: Questions are answered inline in the thread. Edits are committed to a new branch in your connected repository and opened as a pull request; the agent never pushes directly to your deploy branch. Changes go live only after you merge the PR through your normal review flow.
- **Thread isolation**: Each Slack thread and DM is a separate conversation with its own history and working branch. Starting a new thread gives the agent a clean slate.
