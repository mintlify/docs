...
### One-click connect from the welcome DM

When you first install the Mintlify Slack app, the agent sends you a welcome direct message. If your organization has the agent enabled, the message includes one-click **Connect** buttons for the most common integrations:
...
1. Open the agent's direct message in Slack.
2. Click the **Connect** button for the service you want to add.
3. In the modal, click the authorization link, sign in to the third-party service, and approve the requested permissions.
...
The buttons only appear for services you have not connected yet. Once a connection becomes active, the corresponding button no longer shows up on subsequent prompts.

### Ask the agent to connect any service

For services that are not in the one-click list, ask the agent to set up the integration:
