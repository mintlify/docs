...
The agent can read source code from any repository connected to your Mintlify project, even if it isn't your documentation repository. This lets the agent build deeper context to guide documentation updates.

- For GitHub, the agent can clone any repository where the Mintlify GitHub App is installed.
- For GitLab, the agent can clone any project that the connected GitLab account has access to.
...
<Note>
  For GitLab-connected deployments, the agent stays bound to the deployment you started the thread on. Use a new thread to switch deployments instead of switching mid-conversation.
</Note>
