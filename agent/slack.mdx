...
## Mintlify-hosted repositories

If your documentation lives in a Mintlify-hosted repository (the default when you haven't connected your own GitHub or GitLab account), the Slack agent works without any additional setup:

- **No GitHub App install required**: The agent uses Mintlify's managed access to your hosted repository, so you don't need to install or authorize a separate Git integration.
- **Changes publish directly**: Instead of opening a pull request, the agent commits straight to your deploy branch. Your live site updates as soon as the changes deploy.
- **Status updates omit repo details**: Because there is no external repository to reference, the agent's progress messages in Slack focus on the files and edits rather than a repo path.
- **Completion link**: When the agent finishes, it posts a link to your live site so you can review the published change.

To move from a Mintlify-hosted repository to your own GitHub or GitLab repository, follow the migration guide on the [Git Settings](https://dashboard.mintlify.com/settings/deployment/git-settings) page. After you migrate, the agent switches to the standard pull request workflow described above.
