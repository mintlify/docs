...
The connection is scoped to your organization, so you only need to set up each integration once. To remove or reauthorize an integration later, use the **Agent settings** section of the [Agent](https://dashboard.mintlify.com/products/agent) page.

<Note>
  Slack, Linear, and Notion are first-class integrations with their own setup flows. Use the dedicated install buttons in the dashboard for those services instead of the Slack-driven flow.
</Note>
