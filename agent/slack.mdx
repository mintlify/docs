...
Mintlify keeps the status of your connected services in sync with Composio. A connection is automatically marked as disconnected when:

- The third-party provider expires or revokes the credentials.
- You delete the connection from the Composio dashboard.
- The provider marks the connection as inactive.

If a connection is no longer active, the next time you ask the agent to use that service it replies with a fresh authorization link. Click the link, reauthorize the service, and the agent resumes where you left off.
