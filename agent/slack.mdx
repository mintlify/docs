...
4. Confirm the connection in your Mintlify dashboard to bind the Slack workspace to your organization.
...
<Note>
  Each Slack workspace can be connected to only one Mintlify organization at a time. If you try to install the agent into a workspace that is already connected to a different organization, the install fails with a "wrong organization" error. Uninstall the app from the other organization first, or use a different Slack workspace.
</Note>

### Reconnect or reinstall the agent

If you need to reauthorize the agent—for example, to grant new permissions after a Slack scope update or to recover from a revoked token—reinstall the app from the dashboard.

1. Go to the [Agent](https://dashboard.mintlify.com/products/agent) page in your dashboard.
2. Click **Install Slack app** again.
3. Approve the Slack consent screen for the same workspace.
