...
How the agent applies updates depends on where your docs are hosted:

- **Mintlify-hosted repositories**: The agent commits changes directly to your deploy branch, so updates go live as soon as the next deploy finishes. There is no pull request to review. When the agent finishes, it posts a summary of the changes in the thread.
- **Your own GitHub or GitLab repository**: The agent opens a pull request in your connected repository so you can review the diff and merge when ready. When the agent finishes, it posts a link to the pull request.
...
- **Completion**: When the agent finishes, it posts a summary of the changes. For repositories you own, the summary includes a link to the pull request and a link to open the changes in the web editor. For Mintlify-hosted repositories, the agent commits directly to your deploy branch and the summary describes what changed.
...
- **Review carefully**: Always review pull requests that the agent creates before merging them. If your docs are on a Mintlify-hosted repository, the agent commits directly to your deploy branch, so review the agent's summary in Slack and check your live site after the next deploy.
