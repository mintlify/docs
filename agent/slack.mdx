...
## Reference code from other repositories

The agent can read from any repository where the Mintlify GitHub App is installed, not just your documentation repository. This is useful when you want to update docs based on changes in a product or service repository.

- Ask "what repositories do you have access to?" to see the full list.
- Reference a file using `owner/repo:path/to/file.ts` (for example, "Update my docs based on the change in `acme/api:src/routes/billing.ts`").

The agent clones referenced repositories for read-only inspection. Pull requests are still opened against your documentation repository only. To grant access to additional repositories, install the Mintlify GitHub App on them from the [GitHub App settings](https://github.com/apps/mintlify/installations/new).
