...
Integrations that you connect from Slack apply to individual Slack users, so each teammate sets up their own connections and the agent uses the connections of whoever sent the message. If a Slack user has not connected a service, the agent falls back to the organization-level integrations installed from the [Integrations](https://dashboard.mintlify.com/settings/organization/integrations) page in **Settings > Workspace > Integrations**.

To remove or reauthorize an organization-level integration, go to **Settings > Workspace > [Integrations](https://dashboard.mintlify.com/settings/organization/integrations)** in your dashboard.
