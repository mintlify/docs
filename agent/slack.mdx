...
Integrations that you connect from Slack apply to individual Slack users, so each teammate sets up their own connections and the agent uses the connections of whoever sent the message. If a Slack user has not connected a service, the agent falls back to the organization-level integrations installed from the [Integrations](https://app.mintlify.com/settings/organization/integrations) settings page in the dashboard.

To remove or reauthorize an organization-level integration, see [Manage integrations](/agent/integrations).
