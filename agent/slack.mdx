...
To remove the agent from your Slack workspace, open the [Automations](https://app.mintlify.com/products/automations) page in your dashboard and click **Disconnect Slack**. This revokes the stored bot token and stops the agent from responding in Slack. A Slack Workspace Owner can also uninstall the `mintlify` app from Slack's **Manage apps** page, which has the same effect.

Reconnecting later restores the agent without recreating your channel or thread history.
