...
Review the exact scopes during the OAuth consent screen before authorizing the connection. The consent screen lists every permission the agent will have, and you can revoke the connection at any time from the dashboard or from the connected app's own integrations settings.
...
3. Click **Disconnect**, then **Connect** again, and complete the OAuth flow.
