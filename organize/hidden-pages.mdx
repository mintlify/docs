...
- **`hidden: true` → automatically applies `noindex`**: By default, hidden pages are excluded from search engines, sitemaps, and AI context. When `seo.indexing` is set to `"all"` in `docs.json`, hidden pages no longer receive `noindex` automatically, so they can be indexed by search engines. An explicit `noindex: true` in a page's frontmatter is always respected, regardless of `seo.indexing`.
