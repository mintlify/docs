...
<Note>
  By default, search engines cannot index hidden pages. When you set `hidden: true`, the page is automatically excluded from search engine indexing, sitemaps, and AI context. To override this behavior and include hidden pages in indexing, set `"indexing": "all"` in the `seo` property of your `docs.json`. See [Search, SEO, and AI indexing](#search-seo-and-ai-indexing) for details.

  Note that setting `noindex: true` does **not** hide a page from navigation. See [Disable indexing](/optimize/seo#disable-indexing) for more information.
</Note>
