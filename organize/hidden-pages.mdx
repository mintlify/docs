...
Versions, anchors, dropdowns, languages, and products also accept the `hidden` property. Setting `hidden: true` on one of these elements removes it from the rendered navigation and excludes every page beneath it from site search, sitemaps, search engine indexing, and AI context.
...
<Warning>
  If you set `hidden: true` on your only version, or on the version that contains all of your pages, your site search index becomes empty and searches return no results. Unlike tabs and groups, versions do not support `searchable: true`. To restore hidden versions to search and indexing, remove `hidden: true` from the version or set `seo.indexing: "all"` in your `docs.json`. See [Fix site search returning no results](/help-center/site-search-returns-no-results).
</Warning>
...
| `hidden: true` (version or other top-level navigation element) | Hidden | Excluded | Excluded | Excluded | Excluded | Excluded |
