...
## Keep hidden pages discoverable with `searchable`

By default, hiding a tab, group, anchor, or dropdown also removes its descendant pages from search, the sitemap, `llms.txt`, the AI assistant, and search engine indexing. To hide a navigation element from the sidebar while keeping its pages discoverable, add `searchable: true` alongside `hidden: true`.

Use `searchable: true` when you want a section to be reachable through search, SEO, or AI tools, but not surfaced in the rendered navigation. Common cases include legacy products that should still rank in search results, reference material linked only from other pages, and content that you want the AI assistant to use as context.
...
- Descendant pages appear in site search and the sitemap.
- Pages are included in `llms.txt` and the AI assistant's context.
- Rendered HTML does not include `<meta name="robots" content="noindex">`, so search engines can index the pages.
- The tab, group, anchor, or dropdown still does not appear in the sidebar navigation.

You can set `searchable: true` on any navigation element that supports `hidden`, including tabs, groups, anchors, and dropdowns. Pages opted in through `searchable` are still reachable by direct URL.

<Note>
  `searchable: true` only has an effect when `hidden: true` is also set. On a visible element it does nothing.
</Note>
