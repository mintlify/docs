    ...
    <ResponseField name="sidebar" type="object">
      ...
      <Expandable title="Sidebar">
        <ResponseField name="items" type='"container" | "card" | "border" | "undecorated"'>
          Visual style for sidebar navigation items. Choose `container` for grouped items with a subtle background, `card` for elevated card-style items, `border` for items with a subtle border, or `undecorated` for minimal styling. The default varies by theme.
        </ResponseField>
      </Expandable>
    </ResponseField>
    <ResponseField name="topbar" type="object">
      Topbar styling configuration.

      <Expandable title="Topbar">
        <ResponseField name="style" type='"gradient" | "default"'>
          Visual style for the navigation bar. Choose `gradient` for a gradient background effect or `default` for standard styling. Defaults to `default`.
        </ResponseField>
      </Expandable>
    </ResponseField>
