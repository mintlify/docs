    ...
    <ResponseField name="links" type="array of object">
      ...
      <Expandable title="Links">
        <ResponseField name="type" type='"github" | "discord"'>
          Special link type that displays live stats. When set to `github`, displays the repository's star count. When set to `discord`, displays the number of online users in the Discord server.
        </ResponseField>
        <ResponseField name="label" type="string">
          Text for the link. Required for standard links. Optional for `github` and `discord` types—when omitted, the label is automatically populated from API data (repository name for GitHub, server name for Discord).
        </ResponseField>
