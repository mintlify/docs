...
If your project uses the deprecated `mint.json` file, the CLI automatically upgrades it to `docs.json` when you run commands like `mint dev`, `mint broken-links`, or `mint rename`.
...
<Steps>
  ...
  <Step title="Run any CLI command">
    In your docs repository, run any CLI command such as:

    ```bash
    mint dev
    ```

    If a `mint.json` file is detected without a corresponding `docs.json`, the CLI automatically creates a `docs.json` file from your existing configuration. Review the generated file to ensure all settings are correct.
  </Step>
