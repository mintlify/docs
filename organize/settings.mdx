...
The `$ref` value must be a relative path to a JSON file in your repository. Mintlify resolves all references recursively before validating the configuration, so the referenced file's contents replace the `$ref` object entirely. Referenced files can contain their own `$ref` values for further nesting.
