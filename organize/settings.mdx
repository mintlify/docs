...
<ResponseField name="variables" type="object">
  Global variables that can be referenced throughout your documentation pages and snippets using `{{variableName}}` syntax. Define variables as key-value pairs where the key is the variable name and the value is the text to substitute.

  Variables are replaced at runtime, allowing you to maintain consistent values across your documentation and update them in one place.
  ...
  ```mdx
  Welcome to {{companyName}}! Our API is currently on {{apiVersion}}.
  
  For support, contact us at {{supportEmail}}.
  ```

  Variables work in both documentation pages and snippets. When you update a variable value in `docs.json`, all references throughout your documentation are automatically updated.

  <Note>
    Global variables defined in `docs.json` are separate from snippet-specific variables. Global variables apply site-wide, while snippet variables are scoped to individual snippet files.
  </Note>
</ResponseField>
