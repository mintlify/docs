...
<ResponseField name="variables" type="object">
  Global variables that can be referenced throughout your documentation pages and snippets. Define variables as key-value pairs in `docs.json` and reference them using `{{variableName}}` syntax in your content.

  Variables are useful for maintaining consistent values across your documentation, such as product names, version numbers, API endpoints, or any other text that appears in multiple places.

  <Expandable title="Variables">
    Define variables as key-value pairs where the key is the variable name and the value is the text to insert:
    ...
    Reference variables in your MDX content using double curly braces:

    ```mdx example.mdx
    Welcome to {{productName}}! Our API is currently on {{apiVersion}}.
    ...
    - MDX page content
    - Snippet files
    - Any text content in your documentation

    <Note>
      Global variables defined in `docs.json` are different from snippet variables. Global variables are available everywhere in your documentation, while snippet variables are scoped to individual snippet files.
    </Note>

    When you update a variable value in `docs.json`, all pages using that variable are automatically recompiled to reflect the new value.
  </Expandable>
</ResponseField>
