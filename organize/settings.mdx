...
This section provides complete, working examples of `docs.json` configurations for different use cases. Each example includes comments explaining the structure and purpose of each section.
...
{
  "$schema": "https://mintlify.com/docs.json",
  // Basic branding and appearance
  "theme": "mint",
  "name": "Simple Docs",
  "description": "Clear and simple documentation for your product",
  "colors": {
    "primary": "#0D9373"
  },
  ...
  // Simple navigation structure with groups
  "navigation": {
    "groups": [
      {
        "group": "Getting started",
...
A configuration optimized for API documentation with OpenAPI integration and interactive playground.
...
```json
{
  ...
  // Navigation with tabs separating guides from API reference
  "navigation": {
    "tabs": [
      {
        "tab": "Documentation",
      ...
      },
      {
        ...
        "groups": [
          {
            "group": "Endpoints",
            // OpenAPI integration - automatically generates pages for all endpoints
            "openapi": "/openapi.json",
            "pages": [
              "api-reference/overview",
              // Explicitly list endpoints you want to include
              "GET /users",
              "POST /users",
              "GET /users/{id}",
              "PUT /users/{id}",
  ...
  // Navbar with link to API status page
  "navbar": {
    "links": [
      {
        "label": "API Status",
        "href": "https://status.yourapi.com"
      }
    ],
    "primary": {
      "type": "button",
      "label": "Get API Key",
      "href": "https://dashboard.yourapi.com/signup"
    }
  },
  ...
  "footer": {
    "socials": {
      "github": "https://github.com/yourcompany/api",
...
A configuration for documentation available in multiple languages with a language switcher.
...
```json
{
  ...
  // Language-based navigation structure
  "navigation": {
    "languages": [
      {
        // English version
  ...
  "navigation": {
    ...
    "languages": [
      // ... language configurations as shown above
    ]
  },
...
```json
{
  ...
  "theme": "palm",
  "name": "Versioned Docs",
  "description": "Documentation for all versions of our product",
  "colors": {
    "primary": "#DC2626",
    "light": "#FCA5A5",
    "dark": "#991B1B"
        ...
        "tabs": [
          {
            ...
            "groups": [
              ...
              },
              {
                ...
                "tag": "NEW",
                "pages": [
                  "v3/features/async-operations",
                  "v3/features/batch-processing",
                  "v3/features/webhooks-v2"
                ]
              },
              {
                "group": "Core concepts",
                "pages": [
                  "v3/concepts/authentication",
                  "v3/concepts/rate-limiting",
                  "v3/concepts/error-handling"
                ]
              }
            ]
          },
        ...
        "tabs": [
          {
            ...
            "groups": [
              ...
              },
              {
                "group": "Core concepts",
                "pages": [
                  "v2/concepts/authentication",
                  "v2/concepts/rate-limiting"
                ]
              }
            ]
          },
  ...
  "banner": {
    "content": "📢 v3.0 is now available! [View migration guide](/v3/migration-from-v2)",
    "dismissible": true
  },
    ...
    "links": [
      {
        ...
        "items": [
          {
            "label": "Changelog",
            "href": "https://yoursite.com/changelog"
          },
          {
            "label": "Release Notes",
            "href": "https://yoursite.com/releases"
          }
        ]
      }
    ]
...
JSON is strict about syntax. Common mistakes include:

<AccordionGroup>
  <Accordion title="Trailing commas">
    **Problem:** JSON does not allow trailing commas after the last item in an object or array.
  ...
  <Accordion title="Missing commas">
    **Problem:** Commas are required between items in objects and arrays.
  ...
  <Accordion title="Unquoted keys or values">
    **Problem:** All keys and string values must be in double quotes.

    ```json
    // ❌ Invalid - unquoted key and single quotes
    {
      name: 'My Docs',
      theme: "mint"
    }
    ...
    ```json
    // ✅ Valid - double quotes for keys and values
    {
      "name": "My Docs",
      "theme": "mint"
    }
  ...
  <Accordion title="Comments in JSON">
    **Problem:** Standard JSON does not support comments. While some tools may allow them, they can cause validation errors.

    ```json
    // ❌ May cause issues - comments in JSON
    {
      "name": "My Docs", // This is my site name
      "theme": "mint"
    }
    ```
    ...
    <Note>
      The examples in this documentation use comments for clarity, but you should remove them from your actual `docs.json` file if you encounter validation errors.
    </Note>
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Page file not found">
    **Problem:** Referenced page files don't exist or paths are incorrect.

    **Error message:** `Page not found: guides/quickstart`

    **Solution:**
    - Verify the file exists at the specified path
    - Check that the path is relative to your docs root
    - Ensure the file has a `.mdx` extension (don't include the extension in `docs.json`)
    - Check for typos in the filename or path

    ```json
    // If your file is at guides/quickstart.mdx
    {
      "navigation": {
        "pages": [
          "guides/quickstart"  // ✅ Correct - no extension
        ]
      }
    }
    ```
  ...
  <Accordion title="Duplicate page paths">
    **Problem:** The same page is referenced multiple times in navigation.

    **Error message:** `Duplicate page path: introduction`

    **Solution:** Each page can only appear once in your navigation structure. If you need the same content in multiple places, create separate files or use redirects.

    ```json
    // ❌ Invalid - duplicate page
    {
      "navigation": {
        "groups": [
          {
  ...
  <Accordion title="Invalid navigation structure">
    **Problem:** Navigation elements are nested incorrectly or missing required fields.

    **Solution:** Ensure each navigation element has its required fields:
    - Groups require `group` and `pages`
    - Tabs require `tab` and either `pages`, `groups`, or `href`
    - Anchors require `anchor` and either `pages`, `groups`, or `href`
    ...
    // ❌ Invalid - missing required fields
    {
      "navigation": {
        "groups": [
          {
            "pages": ["introduction"]  // Missing "group" field
          }
        ]
      }
    }
  ...
  <Accordion title="OpenAPI endpoint not found">
    **Problem:** Referenced OpenAPI endpoint doesn't exist in the specification.

    **Error message:** `OpenAPI endpoint not found: GET /invalid`

    **Solution:**
    - Verify the endpoint exists in your OpenAPI specification file
    - Check the HTTP method and path match exactly (case-sensitive)
    - Ensure your OpenAPI file is valid and accessible
        ...
        "groups": [
          {
            ...
            "openapi": "/openapi.json",
            "pages": [
              "GET /users",      // ✅ Must match OpenAPI spec exactly
              "POST /users",
              "GET /users/{id}"  // ✅ Include path parameters
            ]
          }
        ]
      }
...
<AccordionGroup>
  <Accordion title="Invalid color format">
    **Problem:** Color values must be valid hex codes starting with `#`.

    **Error message:** `Invalid color format: blue`

    **Solution:** Use hex color codes with 3 or 6 characters after the `#`.

    ```json
    // ❌ Invalid - not a hex code
    {
      "colors": {
        "primary": "blue"
      }
    ...
    {
      "colors": {
        "primary": "#0000FF",  // 6-character hex
        "light": "#00F"        // 3-character hex
      }
    }
    ```
  </Accordion>
  ...
  <Accordion title="Invalid theme name">
    **Problem:** Theme must be one of the supported values.

    **Error message:** `Invalid theme: custom`

    **Solution:** Use one of the supported themes: `mint`, `maple`, `palm`, `willow`, `linden`, `almond`, `aspen`.
  ...
  <Accordion title="Invalid icon name">
    **Problem:** Icon name doesn't exist in the configured icon library.

    **Solution:**
    - For Font Awesome (default): Use [Font Awesome icon names](https://fontawesome.com/icons)
    - For Lucide: Use [Lucide icon names](https://lucide.dev/icons)
    - Alternatively, use a URL to a custom icon or inline SVG

    ```json
    {
      "icons": {
        "library": "lucide"  // Set your icon library
      },
      "navigation": {
        "groups": [
          {
            "group": "Getting started",
            "icon": "rocket",  // ✅ Valid Lucide icon
            "pages": ["introduction"]
          }
        ]
      }
...
To validate your `docs.json` structure:

<Steps>
  <Step title="Use the schema reference">
    Include the schema reference at the top of your file for automatic validation in most code editors:
  ...
  <Step title="Use a JSON validator">
    Validate your JSON syntax using online tools:
    - [JSONLint](https://jsonlint.com/)
    - [JSON Formatter](https://jsonformatter.curiousconcept.com/)
    
    These tools will identify syntax errors like missing commas, unmatched brackets, or invalid characters.
  </Step>

  <Step title="Test locally with the CLI">
    Use the Mintlify CLI to preview your documentation and catch configuration errors:

    ```bash
    mint dev
    ```

    The CLI will report any validation errors or warnings in your `docs.json` configuration.
  </Step>

  <Step title="Check deployment logs">
    After pushing changes, check your deployment logs for any configuration errors. Mintlify will report specific issues with your `docs.json` during the build process.
  </Step>
</Steps>
...
<CardGroup cols={2}>
  <Card title="VS Code" icon="code">
    Install the [JSON Language Features](https://marketplace.visualstudio.com/items?itemName=ZainChen.json) extension for enhanced JSON validation and autocomplete.
  </Card>
  <Card title="JSONLint" icon="check" href="https://jsonlint.com/">
    Online JSON validator that provides clear error messages for syntax issues.
  </Card>
  <Card title="Mintlify CLI" icon="terminal">
    Run `mint dev` to validate your configuration and preview your docs locally.
  </Card>
  <Card title="Schema validation" icon="file-check">
    Use the `$schema` field to enable automatic validation in most modern editors.
  </Card>
</CardGroup>
