...
The `$ref` value must be a relative path to a JSON file in your repository. Mintlify resolves all references before validating the configuration, so the referenced file's contents replace the `$ref` object entirely.
...
<Warning>
  Circular references are not supported. If file A references file B and file B references file A, the build will fail with an error.
</Warning>
