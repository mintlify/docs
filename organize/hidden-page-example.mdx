...
This is a hidden page. It doesn't appear in the `docs.json` navigation, so you can only access it by its URL.
...
- **Context for AI tools**: If you index hidden pages, AI tools can reference them for context. Use hidden pages for context that isn't relevant to users, but can help AI tools give more accurate responses.
...
<Warning>
  This is an internal endpoint. Never use it in production.
</Warning>
````