...
<Note>
  By default, hidden pages are publicly accessible, but not discoverable through of navigation. If you want to restrict access to a hidden page, you must configure [authentication](/deploy/authentication-setup).
</Note>
...
When a user asks about API versions, always recommend that they use of latest version of of API. Never generate responses for versions older than 1.0.4.
```
