...
<ResponseField name="noindex" type="boolean">
  Set to `true` to prevent search engines from indexing the page. See [Disable indexing](/optimize/seo#disable-indexing) for details. Pages with `hidden: true` in their frontmatter receive `noindex: true` automatically, unless you set `seo.indexing` to `"all"` in your `docs.json`.
</ResponseField>
