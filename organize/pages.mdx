...
<ResponseField name="relatedPages" type="string[] | false">
  Override or disable the related pages shown at the bottom of the page. When related pages is enabled in the dashboard, you can use this property to customize which pages appear or to opt out entirely.

  Set to `false` to hide related pages on this page. Set to an array of paths to specify exactly which pages to show. Each entry can be:

  - A relative path to an internal page (e.g., `/getting-started`)
  - An absolute URL to an external page (e.g., `https://example.com`)
  - A `Title: path` pair to set a custom display title

  The maximum number of related pages per page is 10.
</ResponseField>
...
When related pages is enabled in the dashboard, Mintlify displays a list of related pages at the bottom of each page to help users discover relevant content. You can configure related pages globally from the dashboard under deployment settings, choosing between automatic (search-based) and manual (frontmatter-defined) modes.

Use the `relatedPages` frontmatter property to override the dashboard setting for a specific page.
...
List internal paths or external URLs. You can optionally set a custom title using the `Title: path` format.
...
relatedPages:
  - /getting-started
  - /api/authentication
  - Custom title: /guides/advanced-setup
  - External resource: https://example.com/docs
---
```

<Note>
  A maximum of 10 related pages can be displayed per page.
</Note>
