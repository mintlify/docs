...
<ResponseField name="hideFooterPagination" type="boolean">
  Set to `true` to hide the previous/next page links at the bottom of the page. Use this for standalone pages that don't fit into a sequential reading order.
</ResponseField>

<ResponseField name="hideApiMarker" type="boolean">
  Set to `true` to hide the API method badge (such as GET or POST) next to the page title in the sidebar navigation. Only applies to API pages.
</ResponseField>
