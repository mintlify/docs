...
Assistant mode renders the page as a full-screen [assistant](/assistant) experience. The top bar and navigation stay in place, and the chat interface replaces the page content. Use this mode to create a dedicated page that you link to directly, such as an "Ask AI" entry in your navigation. Every theme supports assistant mode.

Enable the [assistant](/assistant) on your deployment to use assistant mode. If the assistant isn't enabled, the page shows a disabled search field followed by the page's MDX content.
