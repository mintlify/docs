...
<ResponseField name="boost" type="boolean">
  Set to `true` to boost the page's search ranking. Boosted pages have their search relevance multiplied by 3x, making them more likely to surface for relevant queries. Use this for high-priority content that you want to prioritize in search results. You can also boost entire navigation groups or pages from `docs.json`. See [Boost search ranking](/organize/navigation#boost-search-ranking) for details.
</ResponseField>
