...
Custom mode provides a minimalist layout and removes all elements except for the top navbar. The sidebar, table of contents, and footer are hidden. Treat custom mode as a blank canvas to build landing pages or unique layouts with minimal navigation. All themes support custom mode.
