...
Use a `YYYY-MM-DD` date or a full ISO 8601 datetime string. If the value is missing, empty, or unparseable, Mintlify falls back to the Git commit date and then the deployment timestamp. Mintlify also accepts `last_updated_at` as an alias for `lastUpdatedDate`.
