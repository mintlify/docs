...
<ResponseField name="relatedPages" type="string[] | false">
  Override or disable the related pages section for this page.

  When related pages is enabled for your deployment in the Mintlify dashboard, you can use this field to:

  - Disable related pages on a specific page by setting `relatedPages: false`
  - Manually specify related pages with an array of paths or URLs

  Each entry can be a page path, an external URL, or a mapping of a custom title to a path:

  ```yaml
  ---
  relatedPages:
    - /guides/quickstart
    - Custom title: /api-reference/overview
    - https://example.com/external-resource
  ---
  ```

  Manual entries override automatic suggestions. The maximum number of related pages displayed is determined by the limit set in your deployment settings (up to 10).
</ResponseField>
