...
For deployments backed by GitHub or GitLab, the displayed date is the date of the last git commit that modified the page's source file. If a git commit date isn't available, such as on deployments that aren't connected to GitHub or GitLab, the date falls back to the most recent deployment timestamp.
