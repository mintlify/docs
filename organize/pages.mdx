...
<ResponseField name="boost" type="number">
  Search ranking multiplier for the page. For example, `boost: 3` makes the page 3x more likely to appear in search results. If the page is in a navigation group with `boost` set, the page-level value overrides the group-level value.
</ResponseField>
