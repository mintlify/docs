...
<ResponseField name="hidden" type="boolean">
  Set to `true` to remove the page from the sidebar navigation. Readers can still access the page via its URL, but search engines do not index it. To make a page visible again, remove this field entirely. **Do not** set to `false` as it results in undefined behavior. See [Hidden pages](/organize/hidden-pages) for details.
</ResponseField>
