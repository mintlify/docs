...
<ResponseField name="boost" type="number">
  A search ranking multiplier for the page. Values greater than `1` boost the page in search results; values less than `1` reduce its ranking. For example, `boost: 3` makes the page rank as if it were 3x more relevant. Use this for high-priority pages such as quickstarts or landing pages. See [Search ranking boost](/organize/navigation#search-ranking-boost) for details.
</ResponseField>
