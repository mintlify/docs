...
<ResponseField name="boost" type="boolean">
  Set to `true` to boost the page's ranking in search results. Use this to prioritize important pages over related content. Pages inherit the `boost` value set on any parent navigation group unless overridden in their own frontmatter.
</ResponseField>
