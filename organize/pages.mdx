...
<ResponseField name="boost" type="number">
  Multiply the page's in-product search ranking by this factor. Use values greater than `1` to prioritize the page and values between `0` and `1` to de-prioritize it. See [Search](/optimize/search#boost-search-ranking) for details. `boost` has no effect when `searchable: false`, since the page is excluded from in-product search results.
</ResponseField>
