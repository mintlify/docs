...
<ResponseField name="hidden" type="boolean">
  When `true`, removes the page from the sidebar navigation. The page remains accessible via its URL, but is not indexed by search engines. See [Hidden pages](/organize/hidden-pages) for details.
</ResponseField>

<ResponseField name="noindex" type="boolean">
  When `true`, prevents search engines from indexing the page. See [Disable indexing](/optimize/seo#disable-indexing) for details. Pages with `hidden: true` in their frontmatter are automatically treated as having `noindex: true`.
</ResponseField>
