...
- When `$ref` resolves to an object, sibling keys are merged on top and take precedence.
- When `$ref` resolves to a non-object value such as an array, sibling keys are ignored.
