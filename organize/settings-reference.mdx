...
**Type:** array of object—each with: `tab` (string, required), `icon` (string), `iconType` (string), `hidden` (boolean), `searchable` (boolean), `href` (string uri, required)

Set `searchable: true` alongside `hidden: true` to keep descendant pages discoverable through search, `sitemap.xml`, AI assistant, and `llms.txt` while still hiding the tab from the rendered navigation. See [Hidden pages](/organize/hidden-pages#include-pages-under-specific-hidden-tabs-or-groups).
...
Groups for organizing content into labeled sections. Each group can include `group` (string), `hidden` (boolean), `searchable` (boolean), `pages` (array), and other navigation properties.

**Type:** array of object

Set `searchable: true` alongside `hidden: true` to keep descendant pages discoverable through search, `sitemap.xml`, AI assistant, and `llms.txt` while still hiding the group from the rendered navigation. See [Hidden pages](/organize/hidden-pages#include-pages-under-specific-hidden-tabs-or-groups).
