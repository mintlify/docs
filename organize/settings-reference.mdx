...
Visual style of the banner. `info` uses your primary brand color, `warning` uses an amber background, and `critical` uses a red background.
...
Override the banner background color with a custom hex color. Accepts a string or a `{ light, dark }` object for theme-aware colors. Takes precedence over `banner.type`.
