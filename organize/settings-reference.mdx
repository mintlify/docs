...
| `integrations.customScripts` | array of objects | No | External scripts to load. Max 20 items. Each object has `src` (required, HTTPS URL; `.svg`, `.exe`, `.zip` not allowed) and `strategy` (`"afterInteractive"` or `"lazyOnload"`). |
