...
| `banner.color` | string or object | No | None |
...
Visual style of the banner. `info` uses your primary brand color. `warning` uses an amber background for cautionary notices. `critical` uses a red background for urgent notices.
...
Override the banner background color with a custom hex color. Accepts a string hex code or a `{ light, dark }` object for theme-aware colors. Takes precedence over `banner.type` when set.
...
OpenAPI spec field keys to render as post labels on every parameter field. For each listed key, Mintlify looks up that field on the schema and displays its value as a pill after the parameter name:

- String values are shown verbatim.
- Boolean `true` is shown as the key name.
- Arrays render one pill per string or number element.
- Objects and `false` or empty values are skipped.
