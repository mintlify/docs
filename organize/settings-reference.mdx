...
| `integrations.customScripts` | array of object | No | External JavaScript files to load on every page. Max 20. |
| `integrations.customScripts[].src` | string (HTTPS URL) | Yes | Script URL. Must be HTTPS. `.svg`, `.exe`, `.zip` extensions are blocked. |
| `integrations.customScripts[].strategy` | `"afterInteractive"` \| `"lazyOnload"` | No | When to load the script. Default: `afterInteractive`. |
