...
Your `docs.json` file lives at the root of your docs repository. See [Global settings](/organize/settings) for setup basics.

<Tip>
  Add `"$schema": "https://mintlify.com/docs.json"` to the top of your `docs.json` for autocomplete and validation in editors like VS Code. To validate `docs.json` locally, run [`mint dev`](/cli/commands#mint-dev) or [`mint validate`](/cli/commands#mint-validate) from your project root—schema errors surface at startup.
</Tip>
