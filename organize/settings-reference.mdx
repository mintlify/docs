...
Display a last-modified date on all pages. For deployments backed by GitHub or GitLab, the date reflects the last git commit that touched each page's source file, and falls back to the most recent deployment timestamp when a git commit date isn't available.
