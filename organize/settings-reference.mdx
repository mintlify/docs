...
| `integrations.customScripts` | array | No | List of external scripts to load on every page. Maximum 20 entries. |
| `integrations.customScripts[].src` | string (HTTPS URL) | Yes | URL of the script. Must be an HTTPS URL. |
| `integrations.customScripts[].strategy` | string | No | Loading strategy. One of `beforeInteractive`, `afterInteractive`, or `lazyOnload`. Use `beforeInteractive` to inject the script into the `<head>` before the page becomes interactive. |
