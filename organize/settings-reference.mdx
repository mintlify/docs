...
Navigation mode inherited by all descendant groups. Set to `"tree"` to render a navigation tree on group root pages showing child pages and nested groups. Can be overridden at the group, tab, or anchor level. Defaults to `"auto"`.
