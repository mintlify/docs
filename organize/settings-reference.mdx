...
| `integrations.customScripts` | array of objects | No | None |
...
| `integrations.customScripts` | array of objects (max 20) | No | External scripts to load on every page. Each object requires `src` (HTTPS URL) and accepts `strategy` (`beforeInteractive`, `afterInteractive`, or `lazyOnload`). |
