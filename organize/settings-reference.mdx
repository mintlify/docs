...
| `integrations.customScripts` | array of objects (max 20) | No | External scripts to load on every page. |
| `integrations.customScripts[].src` | string (uri) | Yes | HTTPS URL of the script. Must not point to a disallowed file type. |
| `integrations.customScripts[].strategy` | `"beforeInteractive"` \| `"afterInteractive"` \| `"lazyOnload"` | No | When to load the script. |
