...
description: "Complete reference for all `docs.json` configuration properties, with types, defaults, and descriptions."
...
Whether your documentation is publicly accessible by default.
...
**Type:** array of object—each with: `dropdown` (string, required), `icon` (string), `iconType` (string), `color` (object with `light`/`dark` hex strings), `description` (string), `hidden` (boolean), `href` (string uri, required)
...
**Type:** array of object—each with: `product` (string, required), `name` (string), `description` (string), `icon` (string), `iconType` (string), `color` (object with `light`/`dark` hex strings), `hidden` (boolean)
...
**Type:** array of object—each with: `version` (string, required), `default` (boolean), `hidden` (boolean), `tag` (string)
...
**Type:** array of object—each with: `group` (string, required), `pages` (array), `public` (boolean), `icon` (string), `hidden` (boolean), `root` (string—page path), `tag` (string), `expanded` (boolean)
...
| `integrations.customScripts` | array of object | No | None |
| `integrations.koala.publicApiKey` | string | Yes | Koala public API key. |
| `integrations.customScripts` | array of object | No | Custom script tags. Each with `src` (HTTPS URL, required) and `strategy` (`"beforeInteractive"`, `"afterInteractive"`, `"lazyOnload"`). Max 20. |
