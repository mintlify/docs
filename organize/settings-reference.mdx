...
| `integrations.customScripts` | array (max 20) | No | External scripts to load by URL. See [Custom scripts](/customize/custom-scripts#loading-external-scripts). |
| `integrations.customScripts[].src` | string (uri) | Yes | HTTPS URL of the script. `.svg`, `.exe`, and `.zip` URLs are not allowed. |
| `integrations.customScripts[].strategy` | `"afterInteractive"` \| `"lazyOnload"` | No | When to load the script. |
