...
Default [directory listing layout](/organize/navigation#directory-listings) for group root pages. Inherits recursively through the navigation tree; any descendant node can override.
...
Groups for organizing content into labeled sections. Each group object supports `group` (string, required), `pages` (array, required), `icon` (string), `tag` (string), `root` (string), `expanded` (boolean), and `directory` (`"none"` | `"accordion"` | `"card"`).
