...
| `integrations.customScripts` | array | No | Array of external scripts to load. Maximum 20 entries. |
| `integrations.customScripts[].src` | string (uri) | Yes | HTTPS URL of the script to load. Must not point to disallowed file types (e.g. `.svg`, `.exe`, `.zip`). |
| `integrations.customScripts[].strategy` | string | No | Loading strategy: `beforeInteractive`, `afterInteractive`, or `lazyOnload`. Default: `afterInteractive`. |
