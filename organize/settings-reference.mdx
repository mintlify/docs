...
| `integrations.customScripts` | array of object | No | External scripts to load on every page. Maximum 20 scripts. |
| `integrations.customScripts[].src` | string (HTTPS URL) | Yes | URL of the script to load. Must be HTTPS. |
| `integrations.customScripts[].strategy` | `"beforeInteractive"` \| `"afterInteractive"` \| `"lazyOnload"` | No | When to load the script. Default: `afterInteractive`. |
