...
| `integrations.customScripts` | array | No | Array of external scripts to load on every page (max 20). See [external scripts](/customize/custom-scripts#external-scripts). |
| `integrations.customScripts[].src` | string (uri) | Yes | HTTPS URL of the script. Must not point to a disallowed file type. |
| `integrations.customScripts[].strategy` | string | No | Load strategy: `beforeInteractive`, `afterInteractive`, or `lazyOnload`. Default: `afterInteractive`. |
