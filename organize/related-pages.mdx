...
<Warning>
  **Related topics** only render on your production deployment. Both automatic suggestions and manual `related` frontmatter lists are skipped on [branch previews](/deploy/preview-deployments), so the section will not appear on preview URLs even when configured. Mintlify does not build a search index for previews, which the automatic mode depends on, and the add-on is scoped to production. Verify your configuration on production after merging.
</Warning>
