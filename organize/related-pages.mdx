...
For deployments with [end-user authentication](/deploy/authentication-setup), automatic suggestions load after a reader signs in, using that reader's own authenticated search results. Related pages only include pages the reader has permission to view. For more control over suggested pages in an authenticated deployment, use the `related` frontmatter field to show curated suggestions.
