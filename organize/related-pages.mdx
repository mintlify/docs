...
For deployments with [end-user authentication](/deploy/authentication-setup), Mintlify loads automatic suggestions after a reader signs in, using that reader's own authenticated search results. Related pages only include pages the reader has permission to view. For more editorial control in an authenticated deployment, use the `related` frontmatter field to curate suggestions.
