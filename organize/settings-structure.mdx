...
<ResponseField name="navigation.mode" type='"auto" | "tree"'>
  Sets the default navigation mode for all groups. When set to `"tree"`, groups with a `root` page display a navigation tree on the root page. Inherited by all descendant groups unless overridden. Defaults to `"auto"`.
</ResponseField>
