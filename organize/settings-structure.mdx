...
<ResponseField name="banner.type" type="string">
  The visual style of the banner. One of `info`, `warning`, or `critical`. `info` uses your primary brand color, `warning` uses an amber background for cautionary notices, and `critical` uses a red background for urgent notices. Defaults to `info`.
</ResponseField>

<ResponseField name="banner.color" type="string or object">
  Override the banner background color with a custom hex color. Accepts a string or a `{ light, dark }` object for theme-aware colors. Takes precedence over `banner.type`. Banner text is white, so choose a dark enough background for legibility.
</ResponseField>
