...
<ResponseField name="navigation.directory" type="string" default="none">
  Directory layout for group root pages. Set to `"accordion"` or `"card"` to automatically display a listing of child pages on group root pages. Set to `"none"` to disable. Inherits recursively; descendants can override. See [Directory listings](/organize/navigation#directory-listings).
</ResponseField>
