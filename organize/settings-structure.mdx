...
<ResponseField name="navigation.mode" type='"auto" | "tree"'>
  Navigation mode inherited by all descendant groups. Set to `"tree"` to display a [navigation tree](/organize/navigation#tree-navigation) on group root pages. Defaults to `"auto"`. Can be overridden on individual groups, tabs, or anchors.
</ResponseField>
