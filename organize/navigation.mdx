...
Use the `align` property to control where a tab appears in the navigation bar. By default, tabs align to the start (left). Set `align` to `"end"` to push a tab to the right side of the navigation bar.
...
In the `navigation` object, `products` is an array where each entry is an object that requires a `product` field and can contain other navigation fields such as groups, pages, icons, or links to external pages. Use the optional `name` field to set a display name that differs from the `product` identifier.
