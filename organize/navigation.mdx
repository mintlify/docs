...
Use tags to highlight navigation items with status indicators or labels. Tags appear as small badges next to navigation group names, helping users identify new features, beta functionality, deprecated content, or other important states.
...
Add the `tag` field to any navigation group to display a label. Tags work with groups, tabs, anchors, dropdowns, and products.
...
Use consistent tag labels to communicate status and maturity across your documentation.
...
```json New features
{
  ...
  "icon": "brain",
  "tag": "NEW",
  "pages": ["ai/overview", "ai/embeddings", "ai/chat"]
}
```
...
```json Preview features
{
  ...
  "icon": "eye",
  "tag": "PREVIEW",
  "pages": ["preview/features", "preview/feedback"]
}
```
...
Tags work consistently across all navigation components. Use them wherever you need to highlight status or draw attention.
  ...
  "navigation": {
    "tabs": [
      ...
      },
      {
        ...
        "icon": "code",
        "tag": "DEPRECATED",
        "pages": ["api/v1/overview", "api/v1/endpoints"]
      }
    ]
  }
}
...
#### Tags in nested navigation

Tags appear on both parent and nested groups, allowing you to mark specific sections within larger navigation hierarchies.
          ...
          },
          {
            ...
            "pages": [
              "api/graphql/overview",
              {
                "group": "Mutations",
                "tag": "NEW",
                "pages": ["api/graphql/mutations/create", "api/graphql/mutations/update"]
              }
            ]
          }
        ]
...
- **Be consistent**: Use the same tag labels across your documentation. Stick to common patterns like `NEW`, `BETA`, `DEPRECATED`, `PREVIEW`, or `UPDATED`.
- **Keep tags short**: Tags should be concise, typically one word. Long tags can disrupt navigation layout.
- **Use sparingly**: Too many tags reduce their effectiveness. Reserve tags for truly important status indicators.
- **Update regularly**: Remove `NEW` tags after features mature. Update `BETA` to stable or remove the tag when features launch.
- **Combine with icons**: Pair tags with appropriate icons to reinforce meaning. For example, use a flask icon with `BETA` or a warning icon with `DEPRECATED`.
- **Document tag meanings**: If you use custom tags, explain their meaning in your documentation or onboarding materials.

#### Tag styling

Tags inherit your documentation's color scheme and automatically adapt to light and dark modes. The tag appearance is consistent across all themes and navigation elements, ensuring a cohesive visual experience.

Tags are designed to be noticeable without being distracting. They use subtle styling that complements your navigation structure while clearly communicating status information to users.
