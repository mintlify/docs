...
Use the pattern that matches how you want visitors to move between top-level sections of your docs.

| Pattern | Use when | Example |
| --- | --- | --- |
| [Groups](#groups) | Your docs fit in one linear sidebar and you don't need a second axis of navigation. | A single-product SDK reference. |
| [Tabs](#tabs) | You have parallel sections that share the same audience, like guides, API reference, and changelog. | "Guides", "API reference", and "SDKs" tabs for one product. |
| [Anchors](#anchors) | You want persistent links to a small number of top-level destinations (external or internal) visible at all times. | Community, status page, or a "Home" link alongside your docs. |
| [Dropdowns](#dropdowns) | You have several sections but not enough surface area to justify tabs, or you want them tucked into a menu. | Switching between "Product docs", "API reference", and "Partner docs". |
| [Products](#products) | You maintain distinct product docs under one deployment and each product has its own full site. | A platform with separate docs for "Payments", "Identity", and "Analytics". |
| [Versions](#versions) | You publish multiple versions of the same docs in parallel. | Maintaining `v1` and `v2` API docs simultaneously. |
| [Languages](#languages) | You publish translated versions of the same docs. | English, Spanish, and Japanese docs for one product. |

Tabs, dropdowns, anchors, versions, languages, and products can all appear together—pick the one that best represents your top-level hierarchy and nest the rest inside groups.
