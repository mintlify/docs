...
In the `navigation` object, `pages` is an array. Each entry must reference the path to a [page file](/organize/pages).
...
If you publish in multiple languages, you can opt in to auto-routing visitors to their preferred language from the root of your docs. When enabled, Mintlify matches the visitor's browser `Accept-Language` header against your configured languages and redirects eligible visits to that language's entry page.

To enable auto-routing, go to your deployment's settings in the [dashboard](https://dashboard.mintlify.com) and toggle **Auto-route to preferred language**. Changes take effect without a redeploy.

How the match works:

- Only root visits are auto-routed. Deep links, markdown, RSC, prefetch, and AI agent traffic pass through unchanged.
- Quality weights in the `Accept-Language` header are honored, so a visitor's most preferred matchable language wins.
- Regional and legacy aliases fall back sensibly: `fr-CA` matches `fr`, `pt-PT` matches `pt-BR`, and the legacy Mintlify codes `cn`, `jp`, and `ja-jp` are recognized.
- Simplified and Traditional Chinese are never substituted for each other. A `zh-TW` visitor is not routed to a `zh` or `cn` locale, and vice versa. If only one script is configured, the visitor falls through to their next preferred language.
- Your configured default language wins whenever it is the visitor's most preferred matchable language.
- If a visitor uses the docs language switcher, their choice is remembered in the `mintlify_preferred_locale` cookie and always takes precedence over the header on later root visits.
...
Mixing incompatible child types at the same level fails schema validation. Run [`mint validate`](/cli/commands#mint-validate) locally before publishing to catch these errors.
