...
In the `navigation` object, `groups` is an array where each entry is an object that requires a `group` field and a `pages` field. The `icon`, `tag`, `root`, `expanded`, and `boost` fields are optional.
...
Use the `boost` property to multiply the search ranking of pages in a group. The value is a number that acts as a multiplier. For example, `boost: 3` makes pages in the group 3x more likely to appear in search results.

The `boost` value applies to all pages in the group, including pages in nested groups. Individual pages can override the group-level `boost` by setting [`boost` in their frontmatter](/organize/pages#page-metadata).
