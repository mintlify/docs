...
Add a tag to any navigation group by including the `tag` field with your desired label text.
...
Tags work best when used consistently across your documentation. Here are recommended patterns:

<AccordionGroup>
  <Accordion title="NEW - Recently added content">
    Use `NEW` to highlight recently released features, endpoints, or documentation sections. Remove the tag after the content has been available for a few weeks or once it's no longer considered new.
  ...
  <Accordion title="BETA - Preview or experimental features">
    Use `BETA` to indicate features that are available but may change, have limited support, or are still being refined based on user feedback.
    ...
    ```json
    {
      ...
      "tag": "BETA",
      "icon": "robot",
      "pages": ["ai/quickstart", "ai/configuration"]
    }
    ```
  </Accordion>

  <Accordion title="DEPRECATED - Legacy content">
    Use `DEPRECATED` to mark outdated features or APIs that will be removed in future versions. Include migration guides to help users transition to newer alternatives.
    ...
    ```json
    {
      ...
      "pages": ["v1/overview", "v1/migration-guide"]
    }
    ```
  </Accordion>

  <Accordion title="UPDATED - Recently changed content">
    Use `UPDATED` to highlight documentation that has been significantly revised or features that have received major improvements.
  ...
  <Accordion title="PREVIEW - Early access features">
    Use `PREVIEW` for features available to select users or requiring special access. This differs from `BETA` by indicating more limited availability.
    ...
    ```json
    {
      ...
      "tag": "PREVIEW",
      "icon": "chart-line",
      "pages": ["analytics/custom-reports"]
    }
    ```
  </Accordion>
</AccordionGroup>
...
Tags can be applied to groups within any navigation structure, including tabs, anchors, and nested groups.
...
```json Tabs containing groups with tags
{
  "navigation": {
    "tabs": [
      {
...
```json Anchors containing groups with tags
{
  "navigation": {
    "anchors": [
      {
...
<CardGroup cols={2}>
  <Card title="Keep tags short" icon="text-size">
    Use concise labels (3-12 characters) that are easy to scan. Long tags can disrupt the visual flow of your navigation.
  </Card>
  <Card title="Use consistent terminology" icon="list-check">
    Establish a standard set of tags and use them consistently across your documentation. Avoid creating similar tags like "NEW" and "RECENT".
  </Card>
  <Card title="Limit tag usage" icon="filter">
    Too many tags reduce their effectiveness. Reserve tags for truly important items that need extra visibility.
  </Card>
  <Card title="Remove outdated tags" icon="clock-rotate-left">
    Regularly review and remove tags that are no longer relevant. A "NEW" tag loses meaning if it stays for months.
  </Card>
  <Card title="Combine with icons" icon="icons">
    Pair tags with appropriate icons to create stronger visual cues and improve navigation scannability.
  </Card>
  <Card title="Consider your audience" icon="users">
    Choose tag labels that resonate with your users. Technical audiences might prefer "ALPHA" while general users might prefer "PREVIEW".
  </Card>
</CardGroup>

<Note>
  Tags are purely visual indicators in the navigation sidebar. They do not affect page functionality, search results, or SEO. For page-level metadata and status indicators, use frontmatter fields in your MDX files.
</Note>
