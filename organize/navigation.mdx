...
Use the `boost` property to increase the search relevance of specific pages. Boosted pages have their search ranking multiplied by 3x, making them more likely to surface in search results for relevant queries. Use this to prioritize high-value content such as getting started guides, key concepts, or flagship product pages.

You can apply `boost` at three levels:

- **Individual page** in `docs.json` navigation
- **Navigation group** in `docs.json` (boost applies to all descendant pages)
- **Page frontmatter** (see [Pages](/organize/pages#page-metadata))

Nested elements inherit the `boost` value from their parent. A child element can override an inherited value by setting `boost` explicitly.

```json Example boosted group
{
  "navigation": {
    "groups": [
      {
...
```yaml Example boosted page frontmatter
---
title: "Quickstart"
boost: true
---
