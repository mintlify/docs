...
Each language can have its own banner configuration. When a user views documentation in a specific language, they'll see that language's banner if configured. If no language-specific banner exists, the global banner (defined at the root level) will be displayed as a fallback.
...
Banner dismissals are tracked per language. When a user dismisses a banner in one language, switching to another language will show that language's banner (if not previously dismissed). For more information about banner configuration, see the [Banner component documentation](/components/banner).
