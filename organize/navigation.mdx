...
Page entries support anchor links (`#` links) to deep-link to a specific section on a page. Append a `#` followed by the heading ID to the page path.
    ...
    "groups": [
      {
        ...
        "pages": [
          "guides/overview",
          "guides/configuration#authentication",
          "guides/configuration#rate-limits"
        ]
      }
    ]
  }
...
When a user clicks a navigation entry with an anchor link, they navigate directly to that section on the page. This is useful when a single page covers multiple topics and you want each topic to have its own sidebar entry.
