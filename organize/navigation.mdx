...
Use the `boost` property on a group to bias search ranking for the pages it contains. The value is a numeric multiplier applied at index time:

- `boost: 3`: Pages in the group rank as if they were 3x more relevant.
- `boost: 0.5`: Pages in the group rank at half their normal relevance.
- Omitted or `1`: No change to ranking.

Pages inherit the closest ancestor's `boost` value, so a value set on a top-level group applies to every group and page nested inside it. A descendant can override the inherited value by setting its own `boost`.

Use boosts to surface high-value content (quickstarts, popular guides, current product lines) and to deprioritize legacy or archived sections.
  ...
  "navigation": {
    "groups": [
      ...
      },
      {
        "group": "Legacy",
        "boost": 0.25,
        "pages": ["v1/migration", "v1/deprecated-endpoints"]
      }
    ]
  }
}
...
You can also boost a single page from its frontmatter using the [`boost`](/organize/pages#page-metadata) field. Page-level `boost` overrides any value inherited from a parent group.
