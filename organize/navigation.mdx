...
When a group has a root page, you can automatically generate a directory listing on that page. The directory listing displays the group's child pages and nested groups, giving users an overview of the section's content.
...
- `"accordion"` — displays child pages in a compact list layout.
- `"card"` — displays child pages as horizontal cards.
- `"none"` — disables directory listings (default).

The `directory` value inherits through the navigation hierarchy. Setting it on a parent element applies to all descendant groups unless overridden.

```json Enable accordion directory for a group
{
  "navigation": {
    "groups": [
      {
...
```json Enable card directory for all groups
{
  "navigation": {
    "directory": "card",
    "groups": [
...
<Note>
  Directory listings only appear on group root pages. Groups without a `root` property are not affected by the `directory` setting.
</Note>
