...
By default, tabs appear on the left side of the navigation bar. Set `align` to `"end"` to move a tab to the right side. Use this to separate primary navigation tabs from secondary actions like external links or support pages.
...
Each product supports the following optional properties:

- `name` — A display name that overrides the `product` identifier in the UI.
- `description` — A short description displayed in the product switcher.
- `icon` — An icon displayed next to the product name.
- `color` — A custom color for the product with `light` and `dark` hex values.
...
Each dropdown supports the following optional properties:

- `description` — A short description displayed below the dropdown name.
- `icon` — An icon displayed next to the dropdown name.
- `color` — A custom color for the dropdown with `light` and `dark` hex values.
