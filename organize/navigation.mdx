...
You can link to a specific section of a page by appending a `#` fragment to the page path. When a user clicks the navigation entry, they go directly to that section.
...
This works anywhere a page path is accepted in `docs.json`, including inside groups, tabs, and anchors. Anchor links are useful when you want a navigation entry to point to a subsection of a page instead of the top.
