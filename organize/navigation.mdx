...
Set the `align` property on a tab to control its position in the navigation bar. Tabs with `align` set to `"end"` are pushed to the right side of the tab bar, while tabs default to `"start"` (left-aligned).

Use end-aligned tabs for secondary navigation like changelogs, status pages, or external links that you want visually separated from your primary content tabs.
