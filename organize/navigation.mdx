...
You can use custom icons for products in several ways:

- **Icon library**: Use [Font Awesome](https://fontawesome.com/icons) or [Lucide](https://lucide.dev/icons) icon names
- **Custom SVG**: Provide JSX-compatible SVG code
- **External URL**: Link to an externally hosted icon
- **Local file**: Reference an icon file in your project
...
{
  "product": "Core API",
  "icon": "<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='currentColor'><path d='M12 2L2 7l10 5 10-5-10-5z'/></svg>"
}
```
...
To use custom SVG icons, convert your SVG to JSX-compatible format using the [SVGR converter](https://react-svgr.com/playground/), then copy the complete `<svg>...</svg>` element from the JSX output.
