...
Set `mode` to `"tree"` on a group to display a navigation tree on its root page. The tree lists child pages and nested groups, giving users a clear overview of the section's contents.
...
The `mode` property inherits recursively. When you set `mode` on a parent element such as a tab, anchor, or the top-level navigation, all descendant groups inherit the same mode. Any descendant can override the inherited mode by setting its own `mode` value.
...
In this example, the `"Guides"` group inherits `"tree"` mode from the top-level navigation, while `"Blog"` overrides it with `"auto"`.

<Note>
  Tree navigation requires the group to have a `root` page. Groups without a `root` page are not affected by tree mode.
</Note>
