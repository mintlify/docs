...
<img
  ...
  alt="Navigation groups with tags showing NEW, BETA, and DEPRECATED labels."
/>
...
Add a tag to any navigation group using the `tag` field. Tags work with all navigation elements that support groups, including tabs, anchors, dropdowns, and products.
  ...
  "navigation": {
    "groups": [
      ...
      },
      {
        ...
        "icon": "sparkles",
        "tag": "NEW",
        "pages": ["ai/overview", "ai/chat", "ai/search"]
      },
      {
        "group": "Experimental APIs",
        "icon": "flask",
        "tag": "BETA",
        "pages": ["experimental/webhooks", "experimental/streaming"]
      },
      {
        "group": "Legacy endpoints",
        "icon": "archive",
        "tag": "DEPRECATED",
        "pages": ["legacy/v1", "legacy/v2"]
      }
    ]
  }
}
...
<AccordionGroup>
  <Accordion title="NEW" icon="sparkles">
    Highlight recently added features or documentation sections. Use for content added in the current or previous release.
    ...
    ```json
    {
      ...
      "icon": "chart-line",
      "tag": "NEW",
      "pages": ["analytics/overview", "analytics/reports"]
    }
    ```
  </Accordion>

  <Accordion title="BETA" icon="flask">
    Indicate features in beta testing or preview. Use for functionality that may change before general availability.
    ...
    ```json
    {
      ...
      "icon": "users",
      "tag": "BETA",
      "pages": ["collab/setup", "collab/permissions"]
    }
    ```
  </Accordion>

  <Accordion title="DEPRECATED" icon="triangle-exclamation">
    Mark outdated features or APIs scheduled for removal. Include migration guides in these sections.
    ...
    ```json
    {
      ...
      "icon": "key",
      "tag": "DEPRECATED",
      "pages": ["auth/legacy-oauth", "auth/migration-guide"]
    }
    ```
  </Accordion>

  <Accordion title="PREVIEW" icon="eye">
    Identify early-access features available to select users or plans.
    ...
    ```json
    {
      ...
      "tag": "PREVIEW",
      "pages": ["workflows/builder", "workflows/triggers"]
    }
    ```
  </Accordion>

  <Accordion title="UPDATED" icon="arrow-rotate-right">
    Call attention to recently updated documentation with significant changes.
    ...
    ```json
    {
      ...
      "icon": "shield",
      "tag": "UPDATED",
      "pages": ["security/authentication", "security/encryption"]
    }
    ```
  </Accordion>
</AccordionGroup>
...
Tags work consistently across all navigation components. Apply them wherever groups appear in your navigation structure.
        ...
        "pages": [
          {
            "group": "GraphQL API",
            "tag": "NEW",
            "pages": ["graphql/queries", "graphql/mutations"]
          },
          {
            "group": "REST API v1",
            "tag": "DEPRECATED",
            "pages": ["rest/v1/users", "rest/v1/posts"]
          }
        ]
      }
    ]
        ...
        "groups": [
          {
            ...
            "tag": "BETA",
            "pages": ["edge/overview", "edge/deployment"]
          }
        ]
      }
    ]
...
<CardGroup cols={2}>
  <Card title="Use sparingly" icon="filter">
    Limit tags to truly important status changes. Too many tags reduce their effectiveness and create visual clutter.
  </Card>
  
  <Card title="Keep labels short" icon="text-size">
    Use concise, uppercase labels (3-12 characters). Short tags are easier to scan and maintain consistent styling.
  </Card>
  
  <Card title="Maintain consistency" icon="check-double">
    Use the same tag labels across your documentation. Establish a standard set of tags and stick to them.
  </Card>
  
  <Card title="Update regularly" icon="calendar-check">
    Remove or update tags as features mature. A "NEW" tag should not remain indefinitely.
  </Card>
  
  <Card title="Combine with icons" icon="icons">
    Pair tags with relevant icons to reinforce meaning. Icons provide additional visual context.
  </Card>
  
  <Card title="Document tag meanings" icon="book">
    Explain what each tag means in your documentation, especially for terms like "PREVIEW" or "EXPERIMENTAL."
  </Card>
</CardGroup>

<Note>
  Tags inherit your theme's styling and automatically adapt to light and dark modes. Custom tag colors are not currently supported.
</Note>
