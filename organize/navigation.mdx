---
title: "Configure docs navigation"
...
Run [`mint dev`](/cli/commands#mint-dev) to preview navigation changes locally before deploying. The preview reloads whenever you save `docs.json`, so you can iterate on structure and see the sidebar, tabs, and dropdowns update in real time.

Run [`mint validate`](/cli/commands#mint-validate) to check your navigation for common errors, including page paths that do not exist and pages missing from the navigation. Mintlify also runs these checks on every deploy and reports errors in the deployment logs.
