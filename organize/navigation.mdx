...
In the `navigation` object, `languages` is an array where each entry is an object that requires a `language` field and can contain any other navigation fields, including language-specific banner configuration.

Each language can have its own banner configuration. When a user views documentation in a specific language, they see that language's banner. If no language-specific banner is configured, the global banner (defined at the root level of `docs.json`) displays instead. Banner dismissals are stored per-language, so dismissing a banner in one language doesn't affect other languages.
