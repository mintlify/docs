...
### Default expanded state hello hello 
...
Add a `menu` array to a product to create a multi-column dropdown for that product. Each menu item requires an `item` field and can contain groups, pages, icons, or external links. This is the same `menu` shape supported under [tabs](#menus), and is useful when a product has several distinct content areas that you want to surface from the product switcher.
