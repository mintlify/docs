...
Use the `boost` property to prioritize pages in a group in search results. Set `boost: true` on a group to boost the search ranking of every page it contains, including pages in nested groups. Individual pages can override the inherited value by setting `boost` in their [frontmatter](/organize/pages#page-metadata).
