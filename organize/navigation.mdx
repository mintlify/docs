...
Use the `directory` property to automatically render a directory of child pages on group root pages. When you set `directory` on a navigation node, any group with a `root` page beneath that node displays a listing of its children below the page content.
...
| `"accordion"` | Displays children in a collapsible list grouped by section. |
| `"card"` | Displays children in a horizontal card layout. |

The `directory` value inherits recursively through the navigation tree. Set it on a parent node and all descendant groups inherit the same value. Any descendant can override the inherited value, including setting `"none"` to opt out.

You can set `directory` at any level of the navigation hierarchy, including on the root `navigation` object, tabs, anchors, dropdowns, products, versions, languages, and individual groups.
