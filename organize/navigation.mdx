---
title: "Navigation"
description: "Configure your documentation site navigation with groups, pages, dropdowns, and tabs in docs.json to create a sidebar structure."
...
In the `navigation` object, `groups` is an array where each entry is an object that requires a `group` field and a `pages` field. The `icon`, `tag`, `root`, `expanded`, and `mode` fields are optional.
...
Use the `mode` property to display a navigation tree on a group's root page. When `mode` is set to `"tree"`, the root page automatically renders a listing of child pages and nested groups within that group.

Tree navigation requires a [`root`](#root-page) page on the group. The root page displays cards for each child page and section for each nested group, with titles, descriptions, and icons.
...
- `"tree"`: Renders a navigation tree on the group's root page showing child pages and nested groups.
- `"auto"` or omitted: Standard behavior with no navigation tree.

#### Mode inheritance

The `mode` property inherits from parent to child. You can set `mode` on any navigation level—including the root `navigation` object, tabs, anchors, dropdowns, products, versions, and languages—and all descendant groups inherit that mode.
...
In this example, the "Guides" group inherits `"tree"` mode from the root navigation, while the "API" group overrides it with `"auto"`.
...
<CardGroup cols={2}>
  ...
  <Card title="Hungarian (hu)" icon="/images/navigation/languages/hu.png" horizontal />
