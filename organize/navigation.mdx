...
In the `navigation` object, `tabs` is an array where each entry is an object that requires a `tab` field and can contain other navigation fields such as groups, pages, icons, or links to external pages. Use the optional `align` property to control tab positioning.
...
Use the `align` property to control the position of a tab in the navigation bar. By default, tabs align to the start (left). Set `align` to `"end"` to push a tab to the right side of the navigation bar.
...
In the `navigation` object, `products` is an array where each entry is an object that requires a `product` field and can contain other navigation fields such as groups, pages, icons, or links to external pages. Use the optional `name` property to set a display name that differs from the `product` identifier.
