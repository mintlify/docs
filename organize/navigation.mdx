...
Add tags to groups using the `tag` field. Tags work with groups, tabs, anchors, dropdowns, and products.

#### Basic tag usage

Add a tag to any navigation group by including the `tag` field:
  ...
  "navigation": {
    "groups": [
      ...
      },
      {
        ...
        "tag": "BETA",
        "icon": "flask",
        "pages": ["features/ai-assistant", "features/auto-complete"]
      },
      {
        "group": "Legacy API",
        "tag": "DEPRECATED",
...
Use tags to highlight specific tabs in your navigation:
  ...
  "navigation": {
    "tabs": [
      ...
      },
      {
        ...
        "tag": "BETA",
        "icon": "code",
        "pages": ["sdk/installation", "sdk/usage"]
      }
    ]
  }
}
...
Add tags to anchors to emphasize important sections:
...
Highlight product features or status:
...
Tags work on both parent and nested groups:
...
Use tags strategically to guide users to important content:

- **NEW**: Highlight recently added features, pages, or sections
- **BETA**: Indicate features in testing or early access
- **UPDATED**: Show recently refreshed or improved content
- **DEPRECATED**: Mark outdated features scheduled for removal
- **PREVIEW**: Denote experimental or preview features
- **v2.0** or **v3.0**: Indicate version-specific content

<Note>
  Keep tag text short and clear. Tags are most effective when they're concise and immediately recognizable. Avoid using more than one tag per navigation item.
</Note>

<Warning>
  Use tags sparingly. Too many tags reduce their effectiveness and can make navigation feel cluttered. Reserve tags for truly important items that need user attention.
</Warning>
