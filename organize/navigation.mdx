...
Add a `tag` field to any navigation group to display a label. Tags are text-only and automatically styled to match your documentation theme.
  ...
  "navigation": {
    "groups": [
      ...
      },
      {
        ...
        "tag": "BETA",
        "icon": "flask",
        "pages": ["beta/feature-one", "beta/feature-two"]
      },
      {
        "group": "Legacy API",
        "tag": "DEPRECATED",
...
Use these common tag patterns to communicate status and availability:
...
```json Feature status
{
  "groups": [
    ...
    },
    {
      "group": "Experimental",
      "tag": "BETA",
      "pages": ["experimental/feature-flags"]
    },
    {
      "group": "Preview",
      "tag": "PREVIEW",
      "pages": ["preview/next-gen-api"]
    }
  ]
}
```

```json Lifecycle indicators
{
  "groups": [
    {
      "group": "Current API",
  ...
  "groups": [
    {
      ...
      "tag": "ENTERPRISE",
      "pages": ["enterprise/sso", "enterprise/audit-logs"]
    },
    {
      "group": "Pro features",
      "tag": "PRO",
      "pages": ["pro/advanced-analytics"]
    },
    {
      "group": "Premium add-ons",
      "tag": "PREMIUM",
      "pages": ["premium/white-label"]
    }
  ]
}
```

```json Development stages
{
  "groups": [
    {
      "group": "In development",
      "tag": "WIP",
      "pages": ["wip/upcoming-features"]
    },
    {
      "group": "Coming soon",
      "tag": "SOON",
      "pages": ["roadmap/q1-features"]
    },
    {
      "group": "Updated",
      "tag": "UPDATED",
...
Follow these guidelines when using tags in your navigation:

- **Keep tags short**: Use 3-10 characters for optimal display. Long tags may be truncated on smaller screens.
- **Use uppercase**: Tags are typically displayed in uppercase for consistency and visibility.
- **Be consistent**: Use the same tag text for similar states across your documentation (e.g., always use "BETA" rather than mixing "BETA" and "Beta").
- **Limit tag usage**: Only tag groups that need special attention. Too many tags reduce their effectiveness.
- **Pair with icons**: Combine tags with relevant icons to reinforce meaning (e.g., `"icon": "flask"` with `"tag": "BETA"`).
- **Update regularly**: Remove or update tags as features mature (e.g., remove "NEW" after a feature has been available for several releases).
        ...
        "groups": [
          {
            "group": "New guides",
            "tag": "NEW",
            "pages": ["guides/webhooks", "guides/rate-limits"]
          },
          {
            "group": "Core concepts",
            "pages": ["concepts/authentication", "concepts/errors"]
          }
        ]
      }
    ]
        ...
        "groups": [
          {
            ...
            "tag": "EXPERIMENTAL",
            "pages": ["platform/experimental/ai"]
          }
        ]
      }
    ]
...
Tags inherit styling from your documentation theme and automatically adapt to light and dark modes. The tag appearance is determined by your theme's design system and cannot be customized with custom colors or styles.

To emphasize tagged groups further, combine tags with icons that reinforce the tag's meaning:
  ...
  "groups": [
    {
      ...
      "tag": "BETA",
      "icon": "flask",
      "pages": ["experimental/feature"]
    },
    {
      "group": "Deprecated",
      "tag": "DEPRECATED", 
      "icon": "triangle-exclamation",
      "pages": ["deprecated/old-api"]
    },
    {
      "group": "New releases",
      "tag": "NEW",
