...
<ResponseField name="logo.href" type="string (uri)">
  The URL to redirect to when clicking the logo. If not provided, the logo links to the first page of the currently selected locale for [internationalized docs](/guides/internationalization), or to your homepage for single-language sites. Accepts an absolute URL, such as `https://yoursite.com`, or a relative path that starts with `/`, such as `/quickstart`.
</ResponseField>
