...
<Note>
  `.mintignore` controls what gets processed and published, not what triggers a deployment. Changes to files or directories matched by `.mintignore` still trigger a build and a preview deployment for the pull request. The excluded files are then dropped during processing and don't appear on the resulting site.
</Note>
