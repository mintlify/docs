...
<ResponseField name="integrations.customScripts" type="object[]">
  Load external JavaScript from third-party HTTPS URLs on every page. You can add up to 20 scripts. See [Custom scripts](/customize/custom-scripts#external-scripts) for usage details.

  <Expandable title="integrations.customScripts">
    <ResponseField name="src" type="string (uri)" required>
      The HTTPS URL of the script to load. Must start with `https://`. URLs ending in `.svg`, `.exe`, or `.zip` are not allowed.
    </ResponseField>
    <ResponseField name="strategy" type="string">
      Controls when the script loads. Set to `afterInteractive` to load after the page becomes interactive, or `lazyOnload` to defer loading until the browser is idle.
    </ResponseField>
  </Expandable>
</ResponseField>
