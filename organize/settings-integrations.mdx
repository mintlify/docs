...
<ResponseField name="integrations.customScripts" type="array">
  Load external JavaScript files by URL. Useful for third-party scripts not covered by a built-in integration. Maximum of 20 scripts. See [Custom scripts](/customize/custom-scripts#loading-external-scripts) for usage details.

  <Expandable title="integrations.customScripts[]">
    <ResponseField name="src" type="string (uri)" required>
      An HTTPS URL pointing to the script file. URLs ending in `.svg`, `.exe`, or `.zip` are not allowed.
    </ResponseField>
    <ResponseField name="strategy" type="string">
      Controls when the script loads. Accepts `afterInteractive` or `lazyOnload`.
    </ResponseField>
  </Expandable>
</ResponseField>
