...
<ResponseField name="integrations.customScripts" type="array of object">
  Load custom third-party scripts on your documentation site. Maximum of 20 scripts.

  <Expandable title="integrations.customScripts">
    <ResponseField name="src" type="string (uri)" required>
      The HTTPS URL of the script to load. Must not point to a disallowed file type (`.svg`, `.exe`, `.zip`).
    </ResponseField>
    <ResponseField name="strategy" type="string">
      Controls when the script loads. One of `afterInteractive` or `lazyOnload`. Defaults to `afterInteractive`.
    </ResponseField>
  </Expandable>
</ResponseField>
