...
<ResponseField name="integrations.customScripts" type="array of object">
  ...
  <Expandable title="customScripts item">
    <ResponseField name="src" type="string (HTTPS URL)" required>
      The URL of the script to load. Must use HTTPS. URLs pointing to `.svg`, `.exe`, or `.zip` files are not allowed.
    </ResponseField>
    <ResponseField name="strategy" type="string">
      Controls when the script loads. Options: `afterInteractive` (default), `lazyOnload`.
    </ResponseField>
  </Expandable>
</ResponseField>
