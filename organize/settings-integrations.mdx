...
<ResponseField name="integrations.koala" type="object">
  Koala revenue intelligence and intent signal tracking.

  <Expandable title="integrations.koala">
    <ResponseField name="publicApiKey" type="string (min 2)" required>
...
<ResponseField name="integrations.customScripts" type="array of object">
  Load external JavaScript from URLs on every page of your documentation. You can add up to 20 scripts.

  <Expandable title="customScripts item">
    ...
    <ResponseField name="strategy" type="&quot;beforeInteractive&quot; | &quot;afterInteractive&quot; | &quot;lazyOnload&quot;">
      When to load the script. Defaults to `afterInteractive`.
      - `beforeInteractive`: Load before the page becomes interactive. Use for critical scripts that must run immediately.
      - `afterInteractive`: Load after the page becomes interactive. Use for most analytics and tracking scripts.
      - `lazyOnload`: Load during idle time. Use for low-priority scripts that don't need to run immediately.
    </ResponseField>
  </Expandable>
</ResponseField>
