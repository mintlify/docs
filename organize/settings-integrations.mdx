...
<ResponseField name="integrations.customScripts" type="array of objects">
  Load external JavaScript files on every page. You can add up to 20 custom scripts.

  <Expandable title="integrations.customScripts">
    <ResponseField name="src" type="string (uri)" required>
      HTTPS URL of the script to load. Must not point to a disallowed file type (e.g. `.svg`, `.exe`, `.zip`).
    </ResponseField>
    <ResponseField name="strategy" type="string">
      When to load the script. One of:
      - `beforeInteractive` — Load before the page becomes interactive. Use for scripts that must run as early as possible, such as cookie consent or tag managers.
      - `afterInteractive` — Load after the page becomes interactive. This is the default behavior.
      - `lazyOnload` — Load during idle time. Use for low-priority scripts like analytics that don't need to run immediately.
    </ResponseField>
  </Expandable>
</ResponseField>
