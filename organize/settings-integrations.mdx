---
title: "Integrations"
description: "Connect analytics platforms, chat tools, and other third-party services to your documentation site in your `docs.json` file."
...
<ResponseField name="integrations.customScripts" type="array of objects">
  Load external JavaScript files on every page of your documentation site. Use this to add third-party scripts that are not covered by the built-in integrations. You can configure up to 20 custom scripts.

  Each item in the array is an object with the following properties:

  <Expandable title="integrations.customScripts item">
    <ResponseField name="src" type="string (HTTPS URL)" required>
      The HTTPS URL of the script to load. Must not point to a disallowed file type such as `.svg`, `.exe`, or `.zip`.
    </ResponseField>
    <ResponseField name="strategy" type="string">
      Controls when the script loads. Options:
      ...
      - `afterInteractive` — Loads after the page becomes interactive. This is the recommended default for most scripts.
      - `lazyOnload` — Loads during idle time after everything else has loaded. Use for low-priority scripts like background analytics.
    </ResponseField>
  </Expandable>
</ResponseField>
...
<Note>
  For scripts hosted in your repository rather than loaded from an external URL, see [Custom JavaScript](/customize/custom-scripts#custom-javascript).
</Note>
