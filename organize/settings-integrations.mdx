...
<ResponseField name="integrations.customScripts" type="array">
  Array of external scripts to load on every page (max 20). See [custom scripts](/customize/custom-scripts#external-scripts) for usage details.

  <Expandable title="integrations.customScripts[]">
    <ResponseField name="src" type="string (uri)" required>
      HTTPS URL of the script. Must not point to a disallowed file type (`.svg`, `.exe`, `.zip`).
    </ResponseField>
    <ResponseField name="strategy" type="string">
      Load strategy. One of `beforeInteractive`, `afterInteractive`, or `lazyOnload`. Defaults to `afterInteractive`.
    </ResponseField>
  </Expandable>
</ResponseField>
