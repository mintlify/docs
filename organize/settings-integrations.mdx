...
<ResponseField name="integrations.koala" type="object">
  Koala visitor intelligence integration.
...
<ResponseField name="integrations.customScripts" type="array of object">
  Custom script tags to load on your documentation site. Maximum 20 scripts.

  <Expandable title="integrations.customScripts item">
    <ResponseField name="src" type="string (HTTPS URL)" required>
      The URL of the script to load. Must be a valid HTTPS URL. URLs pointing to `.svg`, `.exe`, or `.zip` files are not allowed.
    </ResponseField>
    <ResponseField name="strategy" type="string">
      The loading strategy for the script. Options: `beforeInteractive`, `afterInteractive`, `lazyOnload`.
    </ResponseField>
  </Expandable>
</ResponseField>
