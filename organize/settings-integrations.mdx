...
<ResponseField name="integrations.customScripts" type="array of objects">
  Load external JavaScript files on every page of your documentation site. You can add up to 20 scripts. Each script must use an HTTPS URL.
  ...
  <Expandable title="integrations.customScripts[]">
    ...
    <ResponseField name="strategy" type="string">
      Controls when the script loads. Set to `afterInteractive` to load after the page becomes interactive or `lazyOnload` to defer loading until browser idle time. Defaults to `afterInteractive`.
    </ResponseField>
  </Expandable>
</ResponseField>
