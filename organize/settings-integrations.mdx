...
<ResponseField name="integrations.koala" type="object">
  [Koala](https://getkoala.com) intent-based analytics integration.
...
<ResponseField name="integrations.customScripts" type="array of object">
  External JavaScript files to load on every page. You can include up to 20 scripts.

  <Expandable title="integrations.customScripts">
    <ResponseField name="src" type="string (uri)" required>
      The HTTPS URL of the script to load. Must begin with `https://`. URLs pointing to `.svg`, `.exe`, or `.zip` files are not allowed.
    </ResponseField>
    <ResponseField name="strategy" type='"afterInteractive" | "lazyOnload"'>
      When to load the script. `afterInteractive` loads the script after the page becomes interactive. `lazyOnload` defers loading until the browser is idle. Defaults to `afterInteractive`.
    </ResponseField>
  </Expandable>
