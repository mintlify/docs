...
## API specification inclusion

For API reference pages backed by an OpenAPI or AsyncAPI specification, Mintlify includes the full specification for the endpoint in the Markdown export by default. This gives AI agents complete schema context for the endpoint they are looking at.

If you'd rather keep specifications out of per-page Markdown exports—for example, to reduce token usage when agents fetch many pages—set `markdown.schema` to `false` in your `docs.json`. Pages will then include a link to your [`llms.txt`](/ai/llmstxt) index in place of the inline specification.
