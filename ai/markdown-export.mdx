...
Use the [`Visibility`](/components/visibility) component to tailor content for each channel. Content wrapped in `<Visibility for="humans">` appears on the web page but is excluded from Markdown output. Content wrapped in `<Visibility for="agents">` is hidden on the web page but included in Markdown output.
