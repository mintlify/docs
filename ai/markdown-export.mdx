...
When you enable agent feedback from the [Add-ons](https://dashboard.mintlify.com/products/addons) page in your dashboard, Mintlify appends feedback instructions to the Markdown export of each page. These instructions tell AI agents how to submit feedback when they encounter incorrect, outdated, or confusing documentation.

Agents submit feedback by sending a `POST` request to `/feedback` on your documentation site with a JSON body:
...
Feedback submitted by agents appears in the [feedback dashboard](https://dashboard.mintlify.com/products/analytics/v2/feedback) alongside feedback from human users.
