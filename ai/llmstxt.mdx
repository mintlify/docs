...
<Note>
  If your site is fully authenticated, `llms.txt` and `llms-full.txt` also require authentication to view. LLMs and AI tools that cannot authenticate into your site cannot access these files. If your site is partially authenticated and has at least one public page, `llms.txt` and `llms-full.txt` are publicly accessible. The files exclude pages that belong to [user groups](/deploy/authentication-setup#control-access-with-groups).
