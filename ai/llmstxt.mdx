...
<Note>
  How authentication affects `llms.txt` and `llms-full.txt`:

  - **Public sites**: Both files are publicly accessible and include all pages.
  - **Partially authenticated sites** (some pages marked `public: true`): Both files are publicly accessible and include only public pages. Pages that belong to [user groups](/deploy/authentication-setup#control-access-with-groups) or require authentication are excluded.
  - **Fully authenticated sites** (all pages protected): Both files return a `404` and are not served. This prevents leaking page titles or descriptions to unauthenticated crawlers.
