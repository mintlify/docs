...
Each page's description comes from the `description` field in its frontmatter. Descriptions longer than 300 characters are truncated to the first paragraph and capped at 300 characters with an ellipsis. For API reference pages, the description also includes the specification information from the `openapi` or `api` frontmatter field. Pages without a `description` field appear in the `llms.txt` file without a description.
