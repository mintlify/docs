...
<Note>
  How `llms.txt` and `llms-full.txt` behave depends on how your site is configured:

  - **Public sites**: Both files are publicly accessible and list every page in your documentation.
  - **Partially authenticated sites** (some pages are public): Both files are publicly accessible and only list public pages.
  - **Fully authenticated sites**: `llms.txt` returns 404. Authenticated end users can fetch `llms-full.txt` to use as context, and the file omits any pages restricted to specific [user groups](/deploy/authentication-setup#control-access-with-groups) or hidden from search.

  OpenAPI and AsyncAPI specification links are omitted from `llms.txt` on sites with authentication.
