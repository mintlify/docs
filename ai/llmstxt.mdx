...
<Note>
  How `llms.txt` and `llms-full.txt` are served depends on your authentication setup:

  - **Public sites**: Both files are publicly available and include every page.
  - **Partially authenticated sites** (some pages marked [`public`](/deploy/authentication-setup#make-pages-public)): Both files are publicly accessible without authentication, but only include public pages. OpenAPI and AsyncAPI specifications are omitted.
  - **Fully authenticated sites** (no public pages): `llms.txt` is served only after authentication and excludes any page that belongs to a [user group](/deploy/authentication-setup#control-access-with-groups). `llms-full.txt` returns a 404 and is not available. OpenAPI and AsyncAPI specifications are omitted from `llms.txt`.

  LLMs and AI tools that cannot authenticate into your site cannot access protected content.
