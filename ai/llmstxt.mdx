...
<Note>
  Authentication affects how `llms.txt` and `llms-full.txt` are served:

  - **Fully authenticated sites**: Both files require authentication to view. LLMs and AI tools that cannot authenticate into your site cannot access these files.
  - **Partially authenticated sites**: Both files are publicly accessible without authentication, but their contents are limited to your public pages. Pages that belong to [user groups](/deploy/authentication-setup#control-access-with-groups) are excluded.
