...
- **API specification links** to your OpenAPI and AsyncAPI specs (if configured).
...
If your project includes [OpenAPI](/api-playground/openapi-setup) or [AsyncAPI](/api-playground/asyncapi-setup) specifications, Mintlify automatically includes links to these specs in your `llms.txt` file. This allows LLMs to access your full API schema for more accurate responses about your API endpoints, parameters, and data types.

Specification files are deduplicated by filename, with the shallowest path taking priority when multiple files share the same name across different directories.
