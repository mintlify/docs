...
<Note>
  How `llms.txt` and `llms-full.txt` behave depends on your authentication setup:

  - **Public sites**: Both files are publicly accessible and include all pages.
  - **Partially authenticated sites** (sites with at least one `public: true` page): Both files are publicly accessible, but only list and include content from public pages. OpenAPI and AsyncAPI specifications are excluded.
  - **Fully authenticated sites** (no public pages): `llms.txt` is served only to authenticated users and excludes any pages restricted to [user groups](/deploy/authentication-setup#control-access-with-groups), as well as OpenAPI and AsyncAPI specifications. `llms-full.txt` is not available.
