...
Use `llms.txt` and `llms-full.txt` for different purposes:

- **`llms.txt`** is a lightweight index of your documentation. AI tools use it to discover which pages exist and fetch only the ones relevant to a user's query. Prefer this for agents that can follow links and want to minimize context usage.
- **`llms-full.txt`** is the full text of every page concatenated into a single file. Prefer this when you want to load your entire documentation into an AI tool's context in one request, such as for evaluation, offline analysis, or tools that don't follow links.
