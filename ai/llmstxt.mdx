...
Page links in the `llms.txt` file include a `.md` extension so AI tools can fetch the Markdown version of each page directly.
...
Mintlify adds HTTP headers to every page response so AI tools can automatically discover your `llms.txt` files without prior knowledge of their location:
