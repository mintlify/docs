...
<Warning>
  If your site requires authentication, `llms.txt` and `llms-full.txt` also require authentication to view. LLMs and AI tools that cannot authenticate into your site cannot access these files. There is no way to make these files publicly accessible on an authenticated site. The files exclude pages that belong to [user groups](/deploy/authentication-setup#control-access-with-groups).
