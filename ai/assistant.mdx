...
Create an `Assistant.md` file in your repository to customize how the assistant responds to user questions (`ASSISTANT.md` is also accepted). The assistant reads this file and follows any instructions you provide.

Place the file at `.mintlify/Assistant.md` or `.mintlify/ASSISTANT.md` in your content directory. The assistant appends these instructions to its system prompt, so they apply to all conversations.
...
- **Answer guidelines**: What information to prioritize, how to structure responses, or when to recommend contacting support.
- **Code preferences**: Programming languages or frameworks to use in generated examples.
...
## Response style
- Keep responses concise and actionable. Users want quick answers.
- Use a friendly but professional tone.
- When explaining complex concepts, provide a brief overview first, then details.

## Product context
- Our API uses OAuth 2.0 for authentication. Always mention this when discussing auth.
- Rate limits are 1000 requests/hour for free tier, 10000 for pro tier.
- Our primary users are mobile app developers using React Native.

## Code examples
- Use JavaScript/TypeScript for code examples unless the user specifies another language.
- Always include error handling in API examples.
- Show both the request and expected response.

## Answer guidelines
- For billing questions, direct users to contact support@example.com.
- When discussing deprecated features, mention the replacement and migration path.
- If a question requires account-specific information, suggest contacting support rather than guessing.
```
