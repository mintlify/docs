...
Create an `Assistant.md` file in your repository to customize the assistant's behavior (`ASSISTANT.md` is also accepted). The assistant reads this file and appends the instructions to its system prompt, so the instructions apply to all assistant interactions.

Place the file at `.mintlify/Assistant.md` or `.mintlify/ASSISTANT.md` in your content directory. If you have a custom content directory configured, place the file in `.mintlify/` within that directory.

Add any instructions that you want the assistant to follow when answering questions. Consider adding instructions for:

- **Response style**: Tone, formality level, and how technical responses should be.
- **Domain-specific context**: Product terminology, architecture details, or user personas that inform better answers.
- **Answer guidelines**: What to emphasize, what to avoid, or how to structure responses.
- **Code preferences**: Programming languages or frameworks to prioritize in code examples.
...
## Response style
- Keep responses concise and actionable. Users prefer quick answers over comprehensive explanations.
- Use a friendly, conversational tone while remaining professional.
- When explaining technical concepts, provide a brief explanation followed by a practical example.

## Product context
- Our API uses webhook-based event notifications. Always mention webhook setup when discussing real-time updates.
- We support both REST and GraphQL APIs. Ask users which they prefer if the question could apply to either.
- Our authentication uses OAuth 2.0 with short-lived access tokens (1 hour) and long-lived refresh tokens.

## Code examples
- Prioritize Python and JavaScript examples since 80% of our users work in these languages.
- Always include error handling in code examples.
- Show environment variable usage for API keys rather than hardcoding credentials.

## Common issues
- If users ask about rate limits, mention our tiered rate limiting (100 req/min for free tier, 1000 req/min for pro).
- For questions about data retention, note that we keep logs for 30 days on all plans.
```
