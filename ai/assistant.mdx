...
You can view assistant usage and detailed analytics through your dashboard to understand user behavior and documentation effectiveness. The main Assistant page shows key metrics including current monthly spend, total questions, answered properly, and not answered. For more detailed analytics, visit the insights page. Export and analyze query data to help identify:
...
Manage the assistant from the [Assistant](https://dashboard.mintlify.com/products/assistant) page of your dashboard. Enable or disable the assistant, configure response handling, add starter questions, and manage your message allowance. Use the **General** and **Billing** tabs to access different configuration options.

### Enable or disable the assistant

On the **General** tab, toggle the assistant status to enable or disable the assistant for your documentation site.
...
On the **General** tab, in the response handling section, enable the assistant to redirect unanswered questions to your support team. Specify an email address that the assistant provides to users who ask questions that it cannot answer.
...
On the **General** tab, in the response handling section, configure domains that the assistant can search for additional context when answering questions.

- Domains must be publicly available.
- Domains that require JavaScript to load are not supported.

<Frame>
  <img src="/images/assistant/search-domains-light.png" alt="The assistant search domains panel in the dashboard. The assistant is configured to search the mintlify.com and mintlify.com/blog domains." className="block dark:hidden" />
  <img src="/images/assistant/search-domains-dark.png" alt="The assistant search domains panel in the dashboard. The assistant is configured to search the mintlify.com and mintlify.com/blog domains." className="hidden dark:block" />
</Frame>
...
Help your users start questions with the assistant by adding starter questions. On the **General** tab, in the search suggestions section, add up to three starter questions.
...
If you want to allow overages, enable them in the **Billing Controls** section on the **Billing** tab of the [Assistant](https://dashboard.mintlify.com/products/assistant) page of your dashboard.
...
You can explore queries from the insights page in your dashboard, but to get more powerful insights we recommend exporting a `CSV` file of your queries, responses, and sources to analyze with your preferred AI tool.
