...
The Mintlify MCP uses OAuth scopes to control which areas of your project a session can access. When you connect from an MCP client, the consent screen shows the scopes the client is requesting. When you generate credentials programmatically from the dashboard, you choose which scopes to grant.

Available scopes include:

- **Deployment**: Read or write deployment configuration like `docs.json`, navigation, and theme.
- **Members**: Read or write organization members.
- **Billing**: Read billing and plan information.
- **Integrations**: Write to connected integrations.
- **Analytics**: Read analytics for your project.

Grant only the scopes a client needs. You can change which scopes a credential carries by deleting it and generating a new one.
