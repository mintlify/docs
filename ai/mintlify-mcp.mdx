...
The admin MCP server gives AI tools I am making edits HEY! write access to your Mintlify content and settings. Use it to update content and access your dashboard. With the admin MCP, you can use your preferred AI tools to edit pages, restructure navigation, update `docs.json`, open pull requests, change settings, create workflows, and more.
...
- **`read`**: Fetch the full MDX of any page on the session branch. Pass a page path like `/quickstart`, or the page ID from an editor URL (the segment after `~/`), so an AI tool can open a page directly from an editor link.
