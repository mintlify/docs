...
Override the global contextual menu on a specific page by adding a `contextual` field to the page's frontmatter. Use this to surface page-specific actions like `download-pdf` on a terms of service page, or to hide the menu entirely on a landing page.
