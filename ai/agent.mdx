...
Access the agent through a resizable panel in your dashboard using the keyboard shortcut <kbd>⌘</kbd>+<kbd>I</kbd> (Mac) or <kbd>Ctrl</kbd>+<kbd>I</kbd> (Windows/Linux), or by clicking the agent button in the top navigation.

The agent panel opens on the right side of your dashboard (on desktop) or as a full-screen modal (on mobile) and includes three views:

- **Chat**: Send prompts and receive responses with proposed documentation changes. When the agent makes changes, you can view the pull request directly from the chat or open the changes in the web editor.
- **History**: View and continue previous conversations. Click any conversation to resume it in the Chat view.
- **Settings**: Configure GitHub App permissions to control which repositories the agent can access, and connect the agent to your Slack workspace.
...
You can resize the panel by dragging the left edge to adjust its width. The panel remembers your preferred size and which conversation you were viewing.
...
The agent can only access repositories that you connect through the Mintlify GitHub App. Configure which repositories the agent can access in the agent panel **Settings** view or in the [GitHub App settings](https://github.com/apps/mintlify/installations/new).
