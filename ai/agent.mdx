...
<Note>
  The GitHub App must be installed and configured to use the agent in pull requests. See [GitHub integration](/deploy/github) for setup instructions.
</Note>

When you mention `@mintlify` in a PR comment, the agent:
- Reads the PR context, including code changes and discussion
- Creates a new branch in your docs repository
- Generates documentation changes based on your request
...
Continue mentioning `@mintlify` in the same PR thread to iterate on the documentation changes. The agent adds additional commits to the existing documentation PR.
...
If you have multiple documentation sites, the agent automatically selects the appropriate deployment based on your message context. In most cases, you don't need to manually specify which documentation set to update.
