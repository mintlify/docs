...
Add context items to your prompts to help the agent understand what you're referring to. Context items appear as badges above the input field and are included with your message when you submit.

To add context:

1. Click **Apply suggestion** on an Autopilot PR suggestion to automatically add the PR as context.
2. The PR appears as a badge above the input field with the repository name and PR number.
3. Type your prompt in the input field.
4. Submit your message. The agent includes the PR context when processing your request.

You can remove context items by clicking the **×** button on any badge. Context items are automatically cleared after you submit a message.
...
You can also use Autopilot PR suggestions in the dashboard. When Autopilot identifies a PR that may need documentation updates, click **Apply suggestion** to add the PR as context and open the agent panel with a pre-filled prompt.
