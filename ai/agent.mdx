...
You can attach context items to your prompts to help the agent understand what you're referring to. Context items appear as badges above the input field and are included with your message when you submit.

To add context:

1. Click **Apply suggestion** on an Autopilot PR suggestion to automatically add the PR as context.
2. The PR appears as a badge (for example, `repo-name#123`) above the input field.
3. Type your prompt in the input field.
4. Press <kbd>Enter</kbd> or click the send button to submit your message with the attached context.

You can remove context items by clicking the **×** button on any badge. When you start a new chat, any pending context items are automatically cleared.
...
You can also use Autopilot PR suggestions in the dashboard. When the agent identifies a PR that may need documentation updates, click **Apply suggestion** to automatically add the PR as context and open the agent panel with a pre-filled prompt.
