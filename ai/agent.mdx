...
Access the agent through a resizable panel in your dashboard. Open the panel using the keyboard shortcut <kbd>⌘</kbd>+<kbd>I</kbd> (Mac) or <kbd>Ctrl</kbd>+<kbd>I</kbd> (Windows/Linux), or by clicking the agent button in the dashboard header.

On mobile devices, the agent opens as a full-screen modal for an optimized experience.
