...
The agent uses specialized tools to analyze your repositories:

- **list** and **read**: Navigate and read files in your documentation repository
- **read_navigation**: Understand your documentation structure
- **external_list** and **external_read**: Access files in connected code repositories
- **fetch_pull_request**: Retrieve pull request details and diffs

These tools enable the agent to make informed decisions about documentation updates based on actual code changes.
...
## Autopilot PR analysis

The autopilot feature automatically analyzes merged pull requests to determine if documentation updates are needed. When a PR is merged to a watched repository, the agent:

1. Fetches the PR details and changes
2. Analyzes the code changes using its agentic workflow
3. Determines if documentation updates are required
4. Creates a suggestion in your dashboard if updates are needed

You can also manually trigger PR analysis using the [check PR endpoint](/api-reference/autopilot/check-pr).

### How autopilot decides

The agent uses a `generate_suggestion` tool that evaluates whether a PR requires documentation updates based on:
...
The agent is conservative and only suggests updates when they would genuinely help users. Internal refactorings, test changes, and dependency updates typically don't trigger suggestions.
