...
Access the agent through a resizable panel in your dashboard. Open the panel by clicking the agent button in the top right corner or using the keyboard shortcut `⌘I` (Mac) or `Ctrl+I` (Windows/Linux).

The agent panel includes three views:

- **Chat**: Send prompts and view responses. The agent creates pull requests with proposed changes based on your requests.
- **History**: View and resume previous conversations with the agent.
- **Settings**: Configure integrations and connected repositories.

On mobile devices, the agent opens in a full-screen modal for an optimized experience.
...
1. Open the agent panel in your dashboard using `⌘I` (Mac) or `Ctrl+I` (Windows/Linux), or click the agent button.
