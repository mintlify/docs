...
The agent automatically selects the appropriate deployment when you have multiple documentation sites. You can still specify a deployment using the `subdomain` parameter if needed.
...
Mention `@mintlify` in comments on any GitHub pull request to create documentation updates based on your code changes. The agent works on pull requests in any repository where you've installed the GitHub App, not just your documentation repository.

When you mention `@mintlify` in a PR comment, the agent:
- Analyzes the pull request context and your request
- Creates a new pull request in your documentation repository with the proposed changes
- Links back to the original pull request where you mentioned it

The agent automatically selects the correct documentation deployment when you have multiple sites. You can still specify a deployment using the `subdomain` parameter if needed.

For example, comment on a feature PR: `@mintlify Update the API docs to reflect the new authentication endpoint added in this PR`.
...
## Specify a domain name (optional)

The agent automatically selects the appropriate documentation site when you have multiple deployments. In most cases, you don't need to specify which site to update.
