...
Access the agent from your dashboard using the keyboard shortcut <kbd>⌘</kbd>+<kbd>I</kbd> (macOS) or <kbd>Ctrl</kbd>+<kbd>I</kbd> (Windows/Linux). The agent opens in a resizable panel on the right side of your dashboard on desktop. On mobile devices, the agent opens in a full-screen modal.

<Note>
  Screenshots in this guide show the previous dedicated Agent page interface and will be updated to reflect the new panel UI.
</Note>
...
2. In the Slack integration section, click **Connect**.
