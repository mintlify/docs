...
Mention `@mintlify` in comments on any GitHub pull request to create documentation updates in your docs repository. The agent analyzes the PR context and creates a separate documentation PR with the requested changes.

<Note>
  The GitHub App must be installed and have access to both your code repository and documentation repository for this feature to work.
</Note>

When you mention `@mintlify` in a PR comment:
- The agent reads the PR context (code changes, description, and conversation)
- It creates a new pull request in your documentation repository with the requested changes
- The agent links back to the original PR in the documentation PR description

For example, comment on a feature PR: `@mintlify Update the API docs to include this new authentication method`. The agent creates a documentation PR with the updates and links it in a reply to your comment.

If you have multiple documentation sites, the agent automatically selects the correct deployment based on your request context. You can still specify a subdomain if needed using the `subdomain=<domain-name>` parameter.
...
If you have multiple documentation sites, the agent automatically selects the correct deployment based on your request context. In most cases, you don't need to manually specify which documentation site to update.

If you need to override the automatic selection, include the `subdomain` parameter in your message to specify which documentation set the agent should work on.
