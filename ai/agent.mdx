...
The agent is available as a resizable panel on the right side of your dashboard. Open the panel by pressing ⌘I (Mac) or Ctrl+I (Windows/Linux), or by clicking the agent button in the top navigation. On mobile devices, the agent opens as a full-screen modal.

The agent panel has three views:

- **Chat**: Send prompts to the agent and view responses. The agent creates pull requests based on your instructions and displays links to view the PR or open the changes in the editor.
- **History**: View and resume previous conversations with the agent. Click any conversation to load it in the Chat view.
- **Settings**: Configure the agent's integrations and repository access. Connect your Slack workspace and manage which repositories the agent can access.
...
2. Click the **Settings** icon in the panel header to open the Settings view.
...
The agent can only access repositories that you connect through the Mintlify GitHub App. Configure which repositories the agent can access by opening the agent panel, clicking the **Settings** icon, and managing your repository connections in the Settings view. You can also configure repositories in the [GitHub App settings](https://github.com/apps/mintlify/installations/new).
