...
Access the agent in a resizable panel on the right side of your dashboard using the keyboard shortcut <kbd>⌘</kbd>+<kbd>I</kbd> (Mac) or <kbd>Ctrl</kbd>+<kbd>I</kbd> (Windows/Linux), or by clicking the agent button in the top navigation. On mobile devices, the agent opens in a full-screen modal.
...
When the agent makes changes, you can view the pull request directly from the chat or open the changes in the web editor. The panel is resizable—drag the left edge to adjust its width to your preference.
