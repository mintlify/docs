...
The agent panel is a resizable interface that appears on the right side of your dashboard. Open it by pressing ⌘I (Mac) or Ctrl+I (Windows/Linux), or by clicking the agent button in the top navigation. On mobile devices, the agent opens as a full-screen modal.
...
- **Settings**: Configure the agent's integrations and repository access. Connect your Slack workspace and manage which repositories the agent can read from.
