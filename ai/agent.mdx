...
The panel is resizable on desktop—drag the left edge to adjust its width. On mobile devices, the agent opens in a full-screen modal.

When the agent makes changes, you can view the pull request directly from the chat or open the changes in the web editor using the quick action buttons.
