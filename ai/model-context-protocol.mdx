...
Your MCP server provides three tools that agents can use:

- **Search**: Searches across your documentation to find relevant content, returning snippets with titles and links. This tool lets agents discover information or find pages matching a query.
- **Get page**: Retrieves the full content of a specific documentation page by its path. This tool lets agents efficiently retrieve the full content of a page when they already know the page path, such as from search results.
- **Query docs filesystem**: Explores and interacts with your documentation's virtual filesystem. This tool lets agents navigate your documentation structure the same way the Mintlify assistant does.

Agents determine when to use each tool based on the context of the conversation. For example, an agent might first search your documentation to find relevant pages, then use the get page tool to retrieve the full content of the most relevant result.
