...
Your MCP server exposes two tools for AI applications: a search tool that queries your documentation and a get page tool that retrieves the full content of a specific page. Your users must connect your MCP server to their tools.
...
The search tool queries your documentation and returns relevant snippets with titles and links. AI applications use this tool to find information across your documentation. If the AI needs the full content of a specific page, it follows up with the get page tool using a path from the search results.

The search tool supports optional parameters that AI applications use to control and refine search results.

- **`query`** (required): A query to search the content with.
...
The get page tool retrieves the full content of a specific documentation page by its path. AI applications use this tool when they already know the page path, typically from search results, and need the complete content rather than a snippet.

- **`page`** (required): The page path to retrieve, such as `api-reference/create-customer`. Use page paths returned from search results.
