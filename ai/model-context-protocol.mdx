...
- **Query docs filesystem**: Runs shell commands against a virtualized read-only filesystem that contains your documentation source files. Use this to browse the file tree, search across files with `rg` or `grep`, or read specific pages. This replaces the previous "Get page" tool and gives AI applications more flexibility when exploring your documentation.

AI applications determine when to use each tool based on the context of the conversation. For example, an AI application might first search your documentation to find relevant pages, then use the query docs filesystem tool to read the full content of the most relevant result.
...
- **`command`**: A shell command to run against the virtualized documentation filesystem. Common commands include `tree / -L 2` to browse the file structure, `rg -il "keyword" /` to search across files, and `head -80 /path/file.mdx` to read a specific page.

### MCP resources

Your MCP server also exposes your [skill.md](/ai/skillmd) files as MCP resources. If you have custom skills defined in your project, AI applications can read them directly through the MCP connection without fetching them from a URL.

Each skill is available as a resource with the URI `mintlify://skills/<skill-slug>` and the `text/markdown` MIME type. AI applications can list and read these resources to understand your product's capabilities alongside search results.
