...
- **apiReferenceOnly**: When set to `true`, only returns API reference documentation.
- **codeOnly**: When set to `true`, only returns code snippets.

AI tools automatically determine when to apply these filters based on the context of the user's query. For example, if a user asks about a specific version or requests only code examples, the AI may apply the appropriate filters to provide more relevant results.
