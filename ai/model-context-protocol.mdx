...
* For documentation with partial authentication, where some pages are public and others require login, you must enable your MCP server before users can access it. The `/mcp` endpoint serves public content without authentication. The `/authed/mcp` endpoint requires authentication and returns content scoped to each user's [user groups](/deploy/authentication-setup).
