...
If you have [custom skill files](/ai/skillmd#custom-skill-files) defined in your project, your MCP server automatically exposes them as MCP resources. AI applications that support MCP resources can read your skill files directly from the MCP server, giving them instructions on how to use your product alongside the documentation content available through search and get page tools.

Each skill is available as a resource at `mintlify://skills/{skill-slug}` and returns the skill's content in Markdown format.

<Note>
  Skill resources only appear when you have custom skill files. The automatically generated `skill.md` is not included as an MCP resource.
</Note>
