...
By default, your MCP server is only available for localhost tools. To allow web-based tools to connect, add the redirect domains for the AI tools. A redirect domain is the hostname that an AI tool uses after a user completes authentication like `claude.ai` or `app.cursor.ai`. You can also specify a custom protocol for native apps that use custom URI schemes (for example, `myapp://auth.example.com`). Your users' AI tools cannot complete authentication unless their redirect domain is on this list.
...
<Steps>
  ...
  <Step title="Add redirect domains">
    ...
    By default, redirect domains only allow `https` connections. To allow a custom protocol, include the full protocol in the entry (for example, `myapp://auth.example.com`). This is useful for native desktop or mobile apps that use custom URI schemes for OAuth callbacks.
