...
**Always available:**
- **`apiReferenceOnly`**: Only return API reference docs.
- **`codeOnly`**: Only return code snippets.

**Available when configured:**
- **`version`**: Filter results to a specific documentation version (e.g., `'v0.7'`). Only available when your documentation uses [versions](/organize/versions). Only returns content tagged with the specified version or content available across all versions.
- **`language`**: Filter results to a specific language code (e.g., `'en'`, `'zh'`, or `'es'`). Only available when your documentation uses [languages](/organize/languages). Only returns content in the specified language or content available across all languages.

The MCP search tool schema dynamically includes only the parameters applicable to your deployment. AI applications determine when to apply these filters based on the context of the user's query.
