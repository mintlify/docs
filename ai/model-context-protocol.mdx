...
<Note>
  If your documentation site requires end-user authentication, the MCP server only returns search results from pages marked as public. MCP is not available for fully authenticated sites with no public pages.
</Note>
