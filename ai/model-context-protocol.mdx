...
If a limit is exceeded, the MCP server returns an HTTP `429 Too Many Requests` response. MCP clients should back off and retry after a short delay. Limits reset on a rolling hourly window, so most callers recover automatically within an hour.
