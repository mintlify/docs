...
<Tip>
  Looking to let agents *edit* your docs instead of read them? See the [Admin MCP server](/ai/admin-mcp) for an authenticated MCP server that exposes branching, page editing, navigation, and `docs.json` tools to trusted agents.
</Tip>
