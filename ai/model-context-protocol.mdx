...
Mintlify enables your hosted MCP server automatically when your site has [authentication](/deploy/authentication-setup) configured. To allow AI tools to complete authentication, add their redirect domains in your dashboard.

<Steps>
  <Step title="Open the hosted MCP settings">
    In your dashboard, go to **Settings → Security & access → MCP**.
  </Step>
  <Step title="Add allowed OAuth redirect URIs">
    Under **Allowed OAuth redirect URIs**, add the redirect domains for the AI tools you want to grant your users access to. Your users' AI tools cannot complete authentication unless their redirect domain is on this list. Common redirect domains include `claude.ai` and `vscode.dev/redirect`.
