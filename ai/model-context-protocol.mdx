...
Your MCP server exposes tools for AI applications to search your documentation and retrieve full page content. If your documentation includes [skill files](/ai/skillmd), your MCP server also exposes them as resources that AI applications can read directly. Your users must connect your MCP server to their tools.
...
If your documentation includes [skill files](/ai/skillmd), your MCP server exposes each skill as an MCP resource. Resources let AI applications read your skill files directly through the MCP protocol instead of fetching them over HTTP.

Each skill is registered as a resource with a `mintlify://skills/{slug}` URI and a `text/markdown` MIME type. AI applications can call `resources/list` to discover available skills and `resources/read` to retrieve the content of a specific skill.

If your documentation has no skill files, the MCP server does not advertise the resources capability.
