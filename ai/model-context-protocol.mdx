...
Mintlify hosts a public MCP discovery document at `/.well-known/mcp` so agents and agent-readiness scanners can find your MCP server without prior configuration. The endpoint does not require authentication and is safe to expose publicly.

`GET /.well-known/mcp` returns a JSON document describing your MCP server URL and authentication options:
...
| `servers`   | The available MCP servers. Includes a `public` server and, if authentication is enabled on your documentation, an `authenticated` server with OAuth 2.0. |
