...
If authentication is enabled, view and copy your search MCP server URL on the [MCP server page](https://app.mintlify.com/products/mcp) in your dashboard, under the **Hosted MCP** tab.

<Note>
  The **Hosted MCP** tab only appears when you enable [authentication](/deploy/authentication-setup) for your site. If your site doesn't require authentication, the MCP server page only shows the **Admin MCP** tab, but your search MCP server is still accessible at the `/mcp` path of your site URL.
</Note>
