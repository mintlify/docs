...
- **Search**: Searches across your documentation to find relevant information, code examples, API references, and guides. Returns contextual snippets with titles and direct links to documentation pages.
- **Get page**: Retrieves the full content of a specific documentation page by its path. Use this when the AI already knows the page path (for example, from search results) and needs the complete content rather than a snippet.

AI applications determine which tool to use based on the context of the conversation. Typically, the AI searches first to find relevant pages, then uses the get page tool to retrieve the full content of specific pages when needed.
...
- **`page`**: The page path to retrieve (for example, `'api-reference/create-customer'`). Use the page paths returned from search results.
