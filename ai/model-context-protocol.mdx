...
Agents determine when to use each tool based on the context of the conversation. For example, an agent might first search your site to find relevant pages, then use the query docs filesystem tool to read the full content of the most relevant results. Along the way, it can call submit feedback if it discovers a problem with the page.
...
<Steps>
  <Step title="Create a client credential">
    ...
    2. In the **Client credentials** section, select **Generate**.
    3. Enter a label for the credential to identify its purpose, then confirm.
    4. Copy the **client ID** and **client secret** from the **Secret generated** dialog. Mintlify only shows the client secret once. You cannot retrieve it later.
  </Step>
...
To connect the Mintlify MCP server to VS Code, select the **Install in VS Code** button. Or to manually connect the MCP server, create a `.vscode/mcp.json` file and add:
