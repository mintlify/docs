...
### API endpoint filtering with OpenAPI

If your documentation includes an OpenAPI specification, all API operations are exposed as MCP tools by default. You can control which operations are available and customize their tool names and descriptions using the [`x-mint: mcp` extension](/api-playground/openapi-setup#mcp) in your OpenAPI specification.
