...
The [MCP Trust Score](https://glama.ai/mcp/servers) evaluates MCP servers based on technical quality rather than vanity metrics like GitHub stars. Use this badge to help users identify well-maintained, high-quality MCP servers.

### Evaluation criteria

The Trust Score assesses servers based on:

- **Protocol feature implementation** - How completely the server implements MCP protocol features
- **Dependency health** - Security and maintenance status of dependencies
- **Code quality** - Technical quality indicators and best practices

### Adding the badge

Add the Trust Score badge to your MCP server's README:

```markdown
[![MCP Trust Score](https://glama.ai/mcp/servers/YOUR_SERVER_ID/badge)](https://glama.ai/mcp/servers/YOUR_SERVER_ID)
```

Replace `YOUR_SERVER_ID` with your server's identifier from the [Glama MCP catalog](https://glama.ai/mcp/servers).
