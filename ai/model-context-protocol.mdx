...
By default, your MCP server is only available for localhost tools. To allow web-based tools and desktop clients to connect, configure the allowed OAuth redirect URIs. A redirect URI is the address that an AI tool uses after a user completes authentication. Your users' AI tools cannot complete authentication unless their redirect URI is allowed.
...
<Steps>
  ...
  <Step title="Enable predefined clients">
    The dashboard includes predefined toggles for common AI tools like Claude Desktop and Cursor. Enable the toggle for each client you want to allow. Each toggle automatically adds the appropriate redirect URI for that client.
  </Step>
  <Step title="Add custom domains">
    For AI tools not listed as predefined clients, add their redirect domains in the **Custom Domains** section. Enter the hostname of the redirect domain (for example, `example.com`). Your users' AI tools cannot complete authentication unless their redirect domain is in this list.
