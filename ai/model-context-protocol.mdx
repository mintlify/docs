...
Your MCP server exposes tools for AI applications to search your documentation and explore your documentation files. Your users must connect your MCP server to their tools.

### How MCP servers work

When an AI application connects to your documentation MCP server, it can search your documentation and explore your documentation files directly in response to a user's prompt instead of relying on information from its training data or making a generic web search. Your MCP server provides access to all indexed content on your documentation site.
...
- **Query docs filesystem**: Runs a shell command against a virtualized filesystem containing your documentation files. Use this to read full page content, search across files with `rg`, explore the file structure with `tree`, or retrieve specific sections of a page. This tool replaces the previous get page tool and provides more flexible access to your documentation content.

AI applications determine when to use each tool based on the context of the conversation. For example, an AI application might first search your documentation to find relevant pages, then use the query docs filesystem tool to retrieve the full content of the most relevant result.
...
- **`command`**: A shell command to run against the virtualized documentation filesystem. For example, `rg -il "authentication" /` to search for files mentioning authentication, `tree / -L 2` to view the file structure, or `head -80 /api-reference/create-customer.mdx` to read the beginning of a page.

### MCP resources

If your documentation site has [custom skills](/ai/skillmd#custom-skills), your MCP server exposes them as MCP resources. AI applications can read these resources to understand what your product can do, what inputs it requires, and what constraints apply.

Each skill in your `.mintlify/skills/` directory or your root `skill.md` file becomes a resource at `mintlify://skills/<slug>`, where `<slug>` is derived from the skill name. AI applications can list and read these resources alongside using the search and query docs filesystem tools.

<Tip>
  MCP resources are read-only references that AI applications can access on demand. Unlike tools, which the AI calls to perform an action, resources provide static content that the AI can incorporate into its context.
</Tip>
