...
For server-to-server or automated integrations that don't involve an interactive user, you can generate client credentials. These use the OAuth `client_credentials` grant to authenticate without a browser-based login flow.

<Steps>
  <Step title="Generate credentials">
    1. Navigate to the [MCP server page](https://dashboard.mintlify.com/products/mcp) in your dashboard.
    2. In the **Client credentials** section, click **Generate credentials**.
    3. Enter a label to identify the credential and click **Generate**.
  </Step>
  <Step title="Save your secret">
    After generating, you receive a **client ID** and **client secret**. Copy both values immediately — the secret is only shown once and cannot be retrieved later.
  </Step>
</Steps>

<Warning>
  Store your client secret securely. If you lose it, delete the credential and generate a new one.
</Warning>
