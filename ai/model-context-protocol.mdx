...
<Steps>
  ...
  <Step title="Add redirect domains">
    ...
    By default, redirect domains use HTTPS. To allow a custom protocol, include the full URI
    scheme in the entry. For example, a native app that handles OAuth callbacks with a custom
    URL scheme can be allowed by adding `myapp://callback.example.com` as a redirect domain.
  </Step>
</Steps>
