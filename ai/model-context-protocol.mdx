...
<Note>
  MCP servers work on authenticated documentation sites that have public pages. The MCP server will only return search results from pages marked as public. MCP is not available for fully authenticated sites with no public pages.
</Note>
