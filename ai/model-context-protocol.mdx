...
<Steps>
  ...
  <Step title="Enable client connections">
    In the **Allowed OAuth redirect URIs** section, use the toggles to enable predefined AI tools like Claude Desktop and Cursor. Enabling a client automatically adds its redirect URI to your allowed list.
  </Step>
  <Step title="Add custom domains (optional)">
    If you need to support AI tools that are not in the predefined list, add their redirect domains under **Custom Domains**. Your users' AI tools cannot complete authentication unless their redirect domain is on this list.
