...
By default, your MCP server is only available for localhost tools. To allow web-based tools to connect, you must add their redirect domains. A redirect domain is the hostname that an AI tool uses after a user completes authentication. Your users' AI tools cannot complete authentication unless their redirect domain is on this list.
...
<Steps>
  ...
  <Step title="Enable predefined clients">
    In the **Allowed OAuth redirect URIs** section, toggle on the AI tools you want to allow. Predefined clients like Claude Desktop and Cursor can be enabled with a single toggle.
  </Step>
  <Step title="Add custom redirect domains">
    If your users connect with tools that are not in the predefined list, add the redirect domain under **Custom Domains**. For example, `vscode.dev/redirect` for VS Code web.
