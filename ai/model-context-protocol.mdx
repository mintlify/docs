...
If your documentation requires authentication, your MCP server requires users to authenticate before connecting. When a user adds your MCP server URL to their AI tool, they are redirected to log in with their existing credentials. After authenticating, they are redirected back to their tool automatically. The MCP server only returns content each user has permission to access based on their [user groups](/deploy/authentication-setup).
...
<Steps>
  ...
  <Step title="Add redirect domains">
    Add the redirect domains for the AI tools you want to grant your users access to. Your users' AI tools cannot complete authentication unless their redirect domain is on this list. Redirect URIs vary by tool. Commono redirects include `claude.ai` and `vscode.dev/redirect`.
