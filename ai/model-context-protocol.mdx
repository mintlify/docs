...
<Steps>
  ...
  <Step title="Add redirect domains">
    ...
    By default, redirect domains use `https://`. To allow a custom protocol scheme (for example, a native app callback like `myapp://`), include the full protocol in the domain entry (for example, `myapp://callback`). Dangerous protocols such as `javascript:`, `data:`, and `file:` are always blocked.
