...
- **Submit feedback**: Reports a documentation issue back to your team when an agent finds a page that is incorrect, outdated, confusing, or incomplete. The agent supplies the page path and a description of the problem, which are recorded as unhelpful feedback in your analytics dashboard.
...
When a limit is exceeded, the MCP server returns an HTTP `429 Too Many Requests` response. MCP clients should back off and retry after a short delay — an exponential backoff starting at a few seconds is a safe default. Limits reset on a rolling hourly window, so most callers recover automatically within an hour.
