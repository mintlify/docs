...
<Steps>
  ...
  <Step title="Add redirect domains">
    ...
    When you add a hostname without a protocol like `claude.ai`, it defaults to HTTPS. To allow a custom protocol, include the full URL with the protocol like `custom-app://callback.example.com`. This is useful for native or desktop AI tools that use custom URL schemes for OAuth callbacks.
