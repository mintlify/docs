...
<Note>
  You can only install the Slack app once per workspace. If you have multiple Mintlify <Snippet file="snippets/rainbow-text.mdx" />s, you can only connect one <Snippet file="snippets/rainbow-text.mdx" /> at a time to a workspace. You must disconnect the app from one <Snippet file="snippets/rainbow-text.mdx" /> before connecting it to another.
</Note>
