...
<Note>
  You can only install the Slack app once per workspace. If you have multiple Mintlify deployments, you can only connect one <Rainbow>deployment</Rainbow> at a time to a workspace. You must disconnect the app from one <Rainbow>deployment</Rainbow> before connecting it to another.
</Note>
