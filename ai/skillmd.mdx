...
<Note>
  If you use a [reverse proxy](/deploy/reverse-proxy), configure it to forward `/.well-known/skills/*` and `/.well-known/agent-skills/*` paths to your Mintlify subdomain.
</Note>
...
| `description` | A brief description from your `skill.md` frontmatter. |
