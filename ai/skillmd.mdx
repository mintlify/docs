...
### Use a symlink for the skills directory

If your skill files live elsewhere in your repository, you can make `.mintlify/skills` a Git symlink that points to another directory. Mintlify resolves symlinks automatically during skill file discovery.

This is useful when you want to keep skill definitions alongside related code in a monorepo or in a shared directory outside `.mintlify/`.

```bash
# Create the .mintlify directory if it doesn't exist
mkdir -p .mintlify

# Create a symlink from .mintlify/skills to your skills directory
ln -s ../path/to/skills .mintlify/skills
...
The symlink target must be a directory within the same repository. The target directory follows the same structure as `.mintlify/skills/` with each skill in its own subdirectory containing a `SKILL.md` file.
