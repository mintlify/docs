...
<Note>
  `skill.md` is consumed by third-party agents and LLMs (for example, ChatGPT, Claude, or custom agents connected through [MCP](/ai/model-context-protocol)). The Mintlify AI assistant that runs inside your docs site uses your documentation content directly and does not depend on `skill.md`.
</Note>
