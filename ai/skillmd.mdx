...
URLs in the card (`url`, `documentationUrl`, `provider.url`, and each skill URL) are built from your configured custom domain when one is set, so the published card always advertises the canonical domain instead of the `*.mintlify.app` subdomain.
