...
When you open a pull request in a monitored repository, a GitHub check named **Mintlify Autopilot** appears on the PR. This check indicates whether the agent has analyzed the PR for potential documentation updates. The check completes once the agent finishes its analysis, regardless of whether suggestions are created.
