...
Go to the [Audit log](https://app.mintlify.com/settings/organization/audit-logs) page of your dashboard to view audit logs. Mintlify retains audit log entries for two years. Export logs to CSV to keep records for longer.
...
Organization admins can add multiple destinations and choose which categories to send. All categories are streamed by default, and the category selection applies to every configured destination.
...
1. Open the **Audit log streaming** configuration.
2. Under **Streamed categories**, select the categories that you want to send. Clear a category to exclude its events from the stream.
