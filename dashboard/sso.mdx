...
## RBAC role mapping with SAML groups

You can automatically assign [roles](/dashboard/roles) to users based on their identity provider group membership. When a user signs in through SSO, Mintlify reads the `groups` attribute from the SAML assertion and maps those groups to dashboard roles.
...
The resulting SAML assertion should include an `AttributeStatement` like this:
...
Once configured, Mintlify maps the group names from the SAML assertion to roles in your organization. You can manage these mappings in the [Single Sign-On](https://dashboard.mintlify.com/settings/organization/sso) page of your dashboard.
