...
- View your current SSO provider configuration
...
JIT (Just-In-Time) provisioning allows users who are not already invited to Mintlify to be automatically created when they log in through your identity provider.

<Warning>
  Enabling JIT provisioning will enforce SSO for all logins to your organization. Users will only be able to authenticate through your configured identity provider.
</Warning>

To enable or disable JIT provisioning:

1. Navigate to `/organization/sso` in your dashboard
2. Locate the "Enable JIT provisioning" toggle
3. Toggle it on or off based on your needs

### Switching SSO providers

You can switch between supported SSO providers (Okta and Microsoft Entra) directly from the dashboard:

1. Navigate to `/organization/sso` in your dashboard
2. Click the settings icon on your SSO configuration card
3. Select a different provider from the dropdown menu
4. Enter the new provider's metadata URL
5. Click "Save changes"

The system will fetch the necessary configuration URLs (Single sign-on URL and Audience URI) for the new provider automatically.

### Disabling SSO

To remove SSO configuration from your organization:
...
This will disable SSO authentication for your organization and remove the JIT provisioning settings.
