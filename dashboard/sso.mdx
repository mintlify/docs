...
<Steps>
  ...
  <Step title="Save in Mintlify">
    Back in the Mintlify dashboard, paste the metadata URL and select **Save changes**.
  </Step>
</Steps>
...
1. Navigate to the [Single Sign-On](https://app.mintlify.com/settings/organization/sso) page in your dashboard.
2. In the **Verified domains** section, select **Add domain**.
3. Enter the domain (for example, `example.com`) and select **Add**.
4. Mintlify generates a verification token. Add it as a DNS TXT record on the domain.
5. Return to the dashboard and select **Verify**. The status changes from **Pending** to **Verified** once the record propagates.

To remove a domain, select the <Icon icon="trash" /> delete button beside it.
...
<Steps>
  <Step title="Create an application">
    ...
    2. Select **Add custom SAML app** in the **Add app** dropdown.
    <Frame>
