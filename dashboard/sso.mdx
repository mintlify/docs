...
With SAML group role mappings, you can automatically assign roles to users provisioned through SSO based on their identity provider groups. Instead of all new SSO users receiving the default editor role, you can map SAML groups to specific Mintlify roles (admin, editor, or viewer).

To configure SAML group role mappings:

1. Navigate to the [Single Sign-On](https://dashboard.mintlify.com/settings/organization/sso) page in your dashboard.
2. Ensure you have a SAML connection configured.
3. Under role mappings, assign Mintlify roles to your SAML groups.

<Note>
  When a user has roles assigned through SAML group mappings, those roles take precedence over the default editor role during provisioning.
</Note>
