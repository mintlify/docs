...
Each setting has its own enable toggle. Both settings accept values between **5 minutes and 14 days** (20,160 minutes). When a setting is disabled, Mintlify's default applies:

- **Idle session timeout**: 14 days.
- **Maximum session lifetime**: No cap. Sessions end only when the idle timeout is reached or the user signs out.
...
The allowlist applies to dashboard sessions only. API keys, MCP calls, and requests from your published documentation site are not affected.
...
<Warning>
  ...
  If you do lock yourself out, ask another admin on an allowed IP to sign in and remove or update the entry. If no admin can reach the dashboard, [contact support](mailto:support@mintlify.com) to restore access.
</Warning>
