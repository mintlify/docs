...
Use the `.mintlify.site` URL during the POC. Authentication works on a [custom domain](/customize/custom-domain) or your `.mintlify.site` subdomain, but does not work on a custom subpath such as `yourcompany.com/docs`. See [Authentication setup](/deploy/authentication-setup).
