...
| Identity or IT administrator | Connects your identity provider, if you test [authentication](/deploy/authentication-setup). Configures DNS if you also set up a custom domain. | 1 to 2 hours |
...
<Steps>
  ...
  <Step title="Install the GitHub App">
    ...
    <Accordion title="Request approval from an organization owner">
      ...
      To continue while approval is pending, skip the Git provider during onboarding. Mintlify creates a private repository that you can later move from [Git settings](https://app.mintlify.com/settings/project/git-settings). See [Clone to your own repository](/deploy/github#clone-to-your-own-repository).
    </Accordion>
  </Step>
...
Use the `.mintlify.site` URL during the POC. [Authentication](/deploy/authentication-setup) works on your `.mintlify.site` subdomain or a [custom domain](/customize/custom-domain), but does not work on a custom subpath such as `yourcompany.com/docs`.
...
<Steps>
  <Step title="Create a branch">
    Open the [web editor](https://app.mintlify.com/editor). Click the branch name in the editor toolbar, type a title in **Find a branch**, then click **New branch**. If you don't see the branch name, [show the branch selector](/editor/publish#show-the-branch-selector).
  ...
  <Step title="Share the preview">
    The editor commits your edits to the branch and opens a pull request. Mintlify builds a [preview deployment](/editor/review#preview-deployments): a temporary URL where your changes render exactly as they look when published. Copy the URL and send it to a reviewer. Make any requested changes on the same branch. The preview rebuilds each time the editor commits to the branch.
  </Step>
...
| Week | Focus |
|---|---|
| Week 1 | Connect the repository, invite your team, define success, and start the content migration. Start [authentication setup](/deploy/authentication-setup) if you plan to test it. |
