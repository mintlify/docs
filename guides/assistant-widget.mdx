              ...
              >
                ...
                >
                  {message.role === 'user' ? (
                    ...
                    <ReactMarkdown
                      className="prose prose-sm max-w-none text-left prose-p:my-2 prose-ul:my-2 prose-ol:my-2 prose-code:bg-gray-200 prose-code:px-1 prose-code:py-0.5 prose-code:rounded prose-pre:bg-gray-800 prose-pre:text-white"
                      components={{
                        p: ({ children }) => <p className="mb-2 last:mb-0 text-left">{children}</p>,
                        ul: ({ children }) => <ul className="mb-2 last:mb-0 ml-4 text-left">{children}</ul>,
                        ol: ({ children }) => <ol className="mb-2 last:mb-0 ml-4 text-left">{children}</ol>,
                        ...
                        <div className="mt-2 pt-2 border-t border-gray-300">
                          ...
                          <ul className="text-xs space-y-1">
                            {sources.map((source, idx) => (
                              <li key={idx}>
                                <a
                                  href={source.url || `https://${domain}.mintlify.app/${source.path}`}
                                  target="_blank"
