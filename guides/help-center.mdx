...
description: "Build a help center on Mintlify that helps customers find answers, reduces support ticket volume, and improves user satisfaction."
...
- **AI-powered search**: The [assistant](/ai/assistant) answers customer questions using your support content, so people can get answers quickly without knowing exactly where to look or specific terms to search for.
...
Use the `directory` property in `docs.json` to automatically render a listing of pages and subgroups on group root pages. This gives customers a visual overview of what's in each section of your help center.
...
| Value | Behavior |
| --- | --- |
| `"none"` | No directory listing. Default. |
| `"accordion"` | Collapsible list grouped by section. |
| `"card"` | Horizontal card layout. |

The value inherits through the navigation tree, so setting it once on a parent applies to all descendants. Any descendant can override it by setting `directory` to a different value.

See [Directory listings](/organize/navigation#directory-listings) for full details and examples.
...
Monitor how customers use your help center to improve content over time.
