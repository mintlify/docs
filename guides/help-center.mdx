...
Set `directory` to `"card"` for a horizontal card layout or `"accordion"` for a collapsible list. The value passes down through the navigation tree unless you specifically override it on a later level.
