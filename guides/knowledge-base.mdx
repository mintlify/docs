...
- **AI-powered search**: [Mintie assistant](/ai/assistant) answers questions using your knowledge base content, so people find answers without knowing exactly where to look.
- **Slack integrations**: Add Mintie assistant to [Slack](/ai/slack-bot) so your team can ask questions and use [Mintie bot](/agent/slack) to capture knowledge from conversations.
- **Low-barrier contributions**: The [web editor](/editor) and [Mintie bot](/agent/index) let anyone on your team update content without learning Git or Markdown.
...
## Set up Mintie assistant

Mintie assistant is automatically enabled for Pro and Enterprise plans. Mintie assistant lets your team ask questions and get answers with cited sources from your knowledge base.

Configure Mintie assistant from your [dashboard](https://dashboard.mintlify.com/products/assistant/settings):
...
- **Search sites**: Include additional sites Mintie assistant can search when answering questions.
- **Deflection email**: Set a support email for questions Mintie assistant can't answer.

### Add Mintie assistant to Slack

The [Slack bot](/ai/slack-bot) lets your team ask Mintie assistant questions without leaving Slack. Create a channel where the bot responds to every message or let people @mention the bot in any channel.
...
[Mintie bot](/agent/index) in your dashboard creates documentation updates from natural language prompts. Describe what you want to change, and Mintie bot creates a pull request with the updates.

For example, a team member could prompt "add a section to the expense policy page explaining how to submit receipts for meals over $50" and copy the existing expense policy page into the prompt. Mintie bot would draft the content and open a PR for review.

### Capture knowledge from Slack

Teams share valuable information in Slack that often never makes it into documentation. [Mintie bot in Slack](/agent/slack) can capture this knowledge and convert it into structured documentation.

When someone shares useful information in a Slack thread, any team member can mention `@mintlify` with instructions to document it. Mintie bot reads the conversation, extracts the relevant information, and creates a pull request. This is useful for capturing technical decisions, troubleshooting solutions, and process explanations while the context is still fresh.
...
Mintie bot uses the thread context to create documentation that captures the key information from the discussion.
