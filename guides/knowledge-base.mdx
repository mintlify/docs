...
- **Slack integrations**: Add the [agent](/agent/slack) to Slack to ask questions and capture knowledge from conversations.
...
Add the agent to your [Slack](/agent/slack) workspace so that your team can ask questions and update content without leaving Slack. Add the `@mintlify` bot to a channel to use it collaboratively, or send it a direct message to use it privately.
...
For example, you could prompt the agent with "add a section to the expense policy page explaining how to submit receipts for meals over $50." You could also copy the existing expense policy page into the prompt. The agent would draft the content and open a PR for review.
