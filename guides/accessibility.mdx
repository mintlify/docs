...
You can use flags to check for specific accessibility issues, which is useful for CI/CD workflows when you want to focus on particular problems:
...
You cannot use both flags simultaneously. The command will return an error if you attempt to skip both checks.
