...
<Tip>
  Once Claude Code is running, paste the following prompt to load the Mintlify skill, [search MCP](/ai/model-context-protocol), and [admin MCP](/ai/mintlify-mcp) into the session. This gives Claude full context on Mintlify components and your site's content.
</Tip>

<SkillMcpPrompt />

To confirm your setup, send a prompt to Claude Code like `List the Mintlify components you can use`.
