...
To confirm your setup, ask Claude Code a question like `Summarize the standards from CLAUDE.md and list the Mintlify components you can use`. A working session cites your `CLAUDE.md` and references Mintlify components (for example, `Card`, `Steps`, or `Accordion`) from the loaded skill.
