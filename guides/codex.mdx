...
3. (Optional) Add the `AGENTS.md` file below to your project.
4. Run `codex` to start a session.
...
<Tip>
  Once Codex is running, paste the following prompt to load the Mintlify [skill](/ai/skillmd), [search MCP](/ai/model-context-protocol), and [admin MCP](/ai/mintlify-mcp) into the session. This gives Codex full context on Mintlify components and your site's content.
</Tip>
...
```toml
[mcp_servers.my-docs]
url = "https://your-docs.mintlifysite.com/mcp"
```

Restart your `codex` session for the config change to take effect. To confirm the MCP server connected, ask Codex `Which MCP servers do you have access to?`. It should list the entry you added.
