...
Mintlify includes predefined automations for the most common content maintenance tasks. Enable any of them from the [Automations](https://app.mintlify.com/products/automations) page of your dashboard.
...
- **Update and merge changes**: Changes merge directly into your Mintlify project repository without review. Best for low-risk, high-frequency tasks like translation, grammar checks, and link fixes.
- **Modify and wait for review**: The agent opens a pull request and a human reviews it before merging. Best for tasks that change content meaning, like syncing with code changes or updating API references.
...
Scheduling cron jobs to run at specific times gives you more precise control over credit usage. For example, run an automation once a day or once a week to cap how many updates it can make per month. A schedule also batches several source changes into one update, so you don't pay for each change separately. Push-triggered automations can vary more from month to month because they depend on how often content or code changes.
...
2. To see more information about a run, click the <Icon icon="ellipsis" /> actions menu button and select **View run details** to see the prompt, summary, files, and pull request.
