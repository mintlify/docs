...
When a new thread is created, the server returns two values in the response headers:
...
<Warning>
  The `X-Thread-Key` header is only returned when a new thread is created. You must store it immediately — it cannot be retrieved again.
</Warning>
