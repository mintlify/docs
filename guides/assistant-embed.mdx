...
*`domain`: Your Mintlify project domain found at the end of your dashboard URL. For example, `domain-name` from `https://dashboard.mintlify.com/org-name/domain-name`)
*`docsURL`: The full URL to your documentation site. For example, `https://docs.yourcompany.com`. This prop converts relative links like `/quickstart` from the assistant to absolute URLs like `https://docs.yourcompany.com/quickstart`, which makes the links navigable.
...
This guide is a starting point. Here are some ways to begin customizing your assistant widget.
