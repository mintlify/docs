---
title: "Linking"
description: "Create internal links, reference API endpoints, and maintain link integrity across your documentation with relative paths and anchor targets."
...
Each `View` component automatically generates an anchor ID from its `title` prop. You can link directly to a specific view or to an element inside a view using a URL hash:
...
When a URL hash matches a view ID or a heading, accordion, or tab inside a view, the correct view activates automatically and the page scrolls to the target. The URL hash also updates when users switch views using the dropdown.

Use the `id` property to set a custom hash:
...
This view is reachable at `#js-guide` instead of the auto-generated `#javascript`.

See [View](/components/view) for more information.
