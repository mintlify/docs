...
When a page has multiple headings that generate the same ID, Mintlify appends `-2`, `-3`, and so on to each duplicate. The counter runs across the entire page, including headings nested inside components like tabs. Use a [custom anchor ID](#custom-anchor-ids) if you want a stable, predictable link. See [Automatic anchor IDs](/create/text#automatic-anchor-ids) for the full slugging rules.
