...
Mintlify preserves custom IDs exactly as you write them, so camelCase, kebab-case, and snake_case all work:
...
Anchor links are case-sensitive, so match the casing of the custom ID when linking to it. See [Format text](/create/text#custom-heading-ids) for more details.
