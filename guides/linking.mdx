...
This command validates that linked pages exist and that anchor links point to valid heading slugs. For example, if you link to `/quickstart#installation`, the command verifies both that the quickstart page exists and that it contains a heading that generates the `installation` anchor.
