...
keywords: ["internal links", "cross-references", "anchor links", "broken links", "deep linking", "accordion deep links", "playground deep links", "custom heading IDs"]
---
...
Deep links point to specific states or locations within a page, not just the page itself. Use deep links to send users directly to an open accordion, a specific heading anchor, or an API playground view.
...
By default, the hash is derived from the accordion's `title`. Use the `id` prop to set a custom hash:
...
See [Accordions](/components/accordions) for full documentation.

### Custom heading IDs

By default, anchor IDs are generated from heading text. Use the `{#custom-id}` syntax at the end of a heading to set an explicit ID:

```mdx
## Step 2: Source the plugins.sh file {#configure-credentials}
```

This heading links to `#configure-credentials` instead of the auto-generated `#step-2-source-the-pluginssh-file`.

Custom IDs are useful for shorter anchors and for keeping links stable after editing heading text. Spaces and special characters are slugified to hyphens. If two headings share the same ID, Mintlify appends a number to deduplicate them (for example, `#my-id`, `#my-id-2`).

<Note>
  Custom heading IDs work with heading levels 1 through 4 (`#` to `####`).
</Note>

### API playground deep links

Open the API playground automatically by appending `?playground=open` to any endpoint page URL:

```text
https://your-docs-url/endpoint-path?playground=open
```

The URL updates automatically as users open or close the playground. Use playground deep links to share a direct link to an endpoint's interactive playground in support conversations or onboarding flows.
...
- [Accordions](/components/accordions): Accordion component reference.
- [API playground](/api-playground/overview): API playground configuration and deep links.
