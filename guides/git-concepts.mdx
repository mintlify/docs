...
{/* vale Vale.Terms = YES */}
  ...
  <Accordion title="Conflict">
    ...
    To resolve a conflict in the web editor, open the affected pull request in your Git provider and pick the version to keep from the conflict view. Locally, edit the conflicted file, remove the conflict markers, then stage and commit the fix.
  </Accordion>
...
{/* vale Vale.Terms = YES */}
  ...
  <Accordion title="Rebase">
    Replay the commits from one branch onto the tip of another, rewriting history so it looks like the work started from the newer base. Rebase produces a linear history without a merge commit. See [Default merge method](/editor/settings#default-merge-method) for when to use rebase in the editor.
  </Accordion>
  ...
  <Accordion title="Squash">
    Combine all commits from a branch into a single commit when you merge. Squashing keeps your deployment branch history compact but loses individual commit messages from the feature branch. See [Default merge method](/editor/settings#default-merge-method) for when to use squash in the editor.
  </Accordion>
...
- **Make changes**: The editor tracks your changes and turns them into a commit when you save your work.
...
<Tabs>
  <Tab title="Using web editor">
    1. Open file in the [web editor](https://app.mintlify.com/editor).
    2. Make changes.
    3. Click **Publish**.
...
- **Use descriptive branch names**: Branch names should explain the purpose of the branch. Use informative names like `update-api-reference` instead of generic names like `temp` or `my-branch`.
