...
<Steps>
  <Step title="Use the web editor first.">
    The [web editor](/editor) handles Git operations automatically.
...
<AccordionGroup>
  ...
  <Accordion title="Repository">
    Your documentation's source code where all files and their history are stored. The web editor connects to your documentation repository to access and modify content.
  </Accordion>

  <Accordion title="Stage">
    Prepare specific changes to be included in your next commit. Staging lets you organize changes into logical commits.
...
<Tabs>
  <Tab title="Using web editor">
    <Note>
      To create pull requests from the web editor, you must have a branch protection rule enabled for your deployment branch that requires pull requests before changes can be merged. Without branch protection rules, changes on branches merge to your deployment branch when published.
    </Note>
...
- **Use descriptive branch names**: Branch names should explain the work being done. Use informative names like `update-api-reference` instead of generic names like `temp` or `my-branch`.
