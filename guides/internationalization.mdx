...
### Language-specific footer and navbar

Configure different footer and navbar content for each language in your documentation. Define these settings within the `navigation.languages` array.
  ...
  "navigation": {
    "languages": [
      ...
      },
      {
        ...
        "navbar": {
          "links": [{ "label": "Blog", "href": "https://example.com/es/blog" }],
          "primary": {
            "type": "button",
            "label": "Comenzar",
...
Language-specific footer and navbar configurations take priority over global configurations. If a language entry does not define its own `footer` or `navbar`, the global configuration defined at the top level of your `docs.json` is used as a fallback.

<Tip>
  Localize footer link labels, navbar labels, and button text to create a fully translated experience for your users.
</Tip>
