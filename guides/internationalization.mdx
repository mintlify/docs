...
By default, the first language in the `languages` array is used as the default. To specify a different default language, add the `default` property to the desired language object:
...
Setting `"default": true` makes that language the initial language shown to users when they visit your documentation.
