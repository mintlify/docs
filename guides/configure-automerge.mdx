...
<Note>
  The Mintlify GitHub App must have bypass permissions on **every** ruleset that targets your deploy branch — including rulesets defined at the organization level and the repository level. If any ruleset is missing the bypass entry, automerge will fail.
</Note>
...
<Steps>
  ...
  <Step title="Select the ruleset">
    Click a ruleset that targets your deploy branch (e.g. `main`).
  </Step>
