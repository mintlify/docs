...
## Automerge setup banner

If a workflow attempts automerge and the merge fails because the GitHub App is missing bypass permissions (or another setup issue prevents the merge), the **Workflows** page in your dashboard shows a banner under the tab switcher.

The banner links to your repository's branch settings so you can review your rulesets and add the Mintlify GitHub App to the bypass list. Once the setup is correct and the next automerge attempt succeeds, the banner is removed automatically.

You can dismiss the banner with the close button. It reappears the next time an automerge attempt fails so you don't miss new setup regressions.
