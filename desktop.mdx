...
To check now, click **Check for updates** next to the version in [Settings](#settings). If an update is available, the button changes to **Update to** followed by the new version number. Click it to install the update the same way as the tab bar button.
