...
keywords: ["fonctionnement de Mintlify"]
---
...
<div id="organizations-projects-and-sites">
  ## Organisations, projets et sites
</div>

Une **organisation** est l'espace de travail de votre équipe. Elle contient vos membres, les paramètres au niveau de l'organisation, et un ou plusieurs projets.

Un **projet** est un site de documentation dans votre organisation. Il connecte un référentiel, un répertoire de contenu et une branche de déploiement à un site publié. Une organisation peut avoir plusieurs projets pour des produits distincts ou différentes propriétés de documentation.

Un **site en ligne** est le résultat publié d'un projet. Mintlify fournit par défaut une URL `.mintlify.site`. Vous pouvez connecter un [domaine personnalisé](/fr/customize/custom-domain) à votre site. Les sites incluent votre contenu, votre navigation, la recherche et toutes les fonctionnalités que vous activez, comme l'assistant ou le playground d'API.

<Note>
  Les projets s'appelaient auparavant des déploiements. Certaines API et certains outils utilisent encore **déploiement** pour désigner un projet.
</Note>
...
| [Serveur MCP admin](/fr/ai/mintlify-mcp) | Agents | Lit et met à jour les projets via des outils authentifiés | Oui |
