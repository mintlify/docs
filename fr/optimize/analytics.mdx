---
title: "Analytique de documentation et parcours utilisateurs"
...
La section Parcours utilisateurs montre les chemins suivis par les visiteurs dans votre documentation après leur arrivée depuis une page sélectionnée. Utilisez-la pour voir quelles pages ils consultent ensuite. Vous pouvez également voir où ils quittent votre site.
...
Chaque colonne d'interaction affiche jusqu'à six pages nommées, classées selon les arrivées. Les autres sont regroupées dans le bloc **Other (N pages)**. Les sessions qui continuent sont ainsi conservées, et le taux d'abandon reflète les visiteurs qui ne poursuivent pas leur parcours. Le bloc apparaît en bas de sa colonne. La dernière colonne d'interaction n'affiche pas de taux d'abandon, car ces sessions n'ont pas de page suivante dans le parcours affiché.
...
Le tableau de bord affiche d'abord le graphique et le tableau des CTA, puis le parcours utilisateur et enfin le parcours de recherche.

Cette section affiche un graphique **Actions effectuées au fil du temps** avec des totaux distincts pour les actions et les clics sur le CTA. Les actions incluent les clics sur le menu contextuel, les copies de blocs de code, les appels du playground d'API et les réponses aux retours. Elles sont distinctes des clics sur le CTA. Le tableau des pages inclut les vues de page, les clics, le taux de CTA, les actions et le taux d'actions.

Le **parcours utilisateur** suit les sessions de recherche à travers cinq étapes : recherche ouverte, requête saisie, résultat sélectionné, page consultée et action effectuée. La dernière étape compte les sessions qui cliquent sur un CTA ou effectuent une action sur une page, comme utiliser les options du menu contextuel, copier des blocs de code, appeler le playground d'API ou envoyer un retour. Chaque étape affiche les sessions atteintes et abandonnées. L'action effectuée ne peut pas dépasser la page consultée.

Examinez vos analyses d'impressions pour :

- **Identifier les pages à forte intention** : les pages présentant un taux de clic du CTA élevé indiquent souvent des visiteurs prêts à passer à l'étape suivante. Il peut s'agir de s'inscrire à votre produit ou de contacter votre équipe commerciale.
- **Comprendre quel contenu compte pour vos utilisateurs** : examinez les pages à forte intention pour découvrir quel contenu incite les utilisateurs à s'engager plus profondément avec votre produit ou service. Veillez à ce que ces pages soient bien tenues à jour. Envisagez d'ajouter davantage de contenu connexe si nécessaire.
