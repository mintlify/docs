---
title: "Analytique Mintlify pour le trafic et les engagements"
...
<Info>
  Les engagements des utilisateurs sont en bêta et susceptibles d'évoluer.
</Info>

La section Engagements des utilisateurs montre comment les visiteurs interagissent avec votre documentation au-delà des pages vues. Utilisez-la pour comprendre quel contenu incite les visiteurs à cliquer, copier, appeler des API, répondre aux retours ou effectuer d'autres actions.

La vue d'ensemble affiche le nombre total d'interactions, le nombre moyen d'interactions par session et le taux de clic sur le CTA. Chaque indicateur inclut l'évolution par rapport à la période précédente.

Le graphique **Actions effectuées au fil du temps** compare les clics sur le CTA aux autres actions. Les clics sur le CTA correspondent aux clics sur votre [bouton d'appel à l'action de la barre de navigation](/fr/organize/settings-structure#navbar). Les actions comprennent les clics sur le menu contextuel, les copies de blocs de code, les appels du playground de l'API, les réponses aux retours et d'autres interactions similaires.

Le tableau par page affiche les pages vues, les clics, le taux de clic du CTA, les actions et le taux d'action. Utilisez ces indicateurs pour comparer les pages et identifier le contenu qui favorise un engagement plus approfondi.

**Chemins du parcours utilisateur** affiche les chemins courants depuis un point d'entrée jusqu'aux première, deuxième et troisième interactions d'un visiteur. Sélectionnez un point d'entrée pour explorer ses chemins.

**Entonnoir utilisateur** mesure deux parcours :

- **De la recherche à la réponse** : recherche ouverte, requête saisie, résultat cliqué, page consultée et action effectuée.
- **Conversation avec l'assistant** : conversation démarrée, réponse obtenue, question complémentaire posée et interaction avec la réponse.
