...
Les utilisateurs peuvent, s’ils le souhaitent, fournir leur adresse e-mail lors de l’envoi de feedback. Le champ e-mail apparaît lorsqu’un utilisateur sélectionne **Other** comme catégorie de feedback, ce qui lui permet de partager ses coordonnées pour un éventuel suivi.

Vous pouvez consulter les adresses e-mail envoyées avec le feedback dans le [dashboard de feedback](https://dashboard.mintlify.com/products/analytics/v2/feedback), aux côtés du contenu du feedback.
...
Mintlify détecte et filtre automatiquement les retours abusifs tels que le spam, les propos injurieux, les discours de haine et tout autre contenu non constructif. Les retours signalés sont exclus de vos indicateurs analytiques par défaut, garantissant ainsi que vos données reflètent fidèlement le sentiment réel des utilisateurs.

La détection des abus utilise une combinaison de correspondance par mots-clés et de classification par IA pour identifier les contenus abusifs. Les retours négatifs constructifs, les signalements de bugs et les demandes de fonctionnalités ne sont pas affectés par le filtrage des abus.
