---
title: "SEO"
description: "Configurez les balises meta, les propriétés Open Graph, les URLs canoniques et les paramètres SEO par page pour améliorer votre classement de recherche."
...
Mintlify ajoute des données structurées [schema.org](https://schema.org) à chaque page indexable sous la forme d'un script JSON-LD. Ces données structurées aident les moteurs de recherche à afficher des résultats enrichis pour vos pages.

Chaque page émet un `@graph` connecté d'entités avec des `@id`s stables :

- `Organization` : l'éditeur de votre site. Dérivée du nom du site, du logo et de l'URL du site, ou configurée explicitement avec [`seo.organization`](/fr/organize/settings-seo#seo).
- `WebSite` : votre site.
- `WebPage` : la page actuelle, y compris sa description et ses dates de modification.
- `BreadcrumbList` : l'emplacement de la page dans votre hiérarchie de navigation, généré à partir de la navigation de `docs.json`.
- L'entité de contenu principale : `TechArticle` pour les pages de documentation ou `APIReference` pour les pages générées à partir de spécifications d'API (pages avec un frontmatter `api`, `openapi` ou `asyncapi`).

Mintlify génère les données structurées à partir du frontmatter de la page et des configurations `docs.json`, notamment le titre de la page, la description, les mots-clés, l'URL canonique, la date de dernière mise à jour, le nom du site et le logo. Les données structurées n'incluent aucun champ sans valeur correspondante. Les pages avec `noindex: true` n'incluent pas de données structurées.

Pour modifier les données structurées, mettez à jour les champs correspondants du frontmatter ou les configurations `docs.json`. Pour contrôler l'entité éditrice, notamment un `@id` stable, le nom légal, le logo canonique et les liens de profil `sameAs`, définissez [`seo.organization`](/fr/organize/settings-seo#seo) dans votre `docs.json`.
...
<div id="folder-scoped-meta-tags">
  ### Balises méta limitées à un dossier
</div>

Pour appliquer des balises méta différentes à un sous-ensemble de pages, comme une documentation versionnée ou un domaine produit spécifique, ajoutez `seo.paths` à votre `docs.json`. Chaque entrée cible un chemin de dossier relatif à la racine de la documentation et applique ses balises à toutes les pages sous ce dossier.
...
Les balises par dossier l'emportent sur `seo.metatags` pour la même clé, les chemins plus longs l'emportent sur les plus courts, et le frontmatter de la page l'emporte sur les deux. Consultez [`seo.paths`](/fr/organize/settings-seo#param-seo-paths) pour les règles de chemin et leurs exceptions.

<div id="verify-site-ownership">
  ### Vérifier la propriété du site
</div>

Pour vérifier votre site auprès de services comme Google Search Console, Bing Webmaster Tools ou d'autres moteurs de recherche, ajoutez la balise meta de vérification à `seo.metatags` dans votre `docs.json`. Mintlify injecte la balise dans chaque page.
...
Le frontmatter peut aussi remplacer toute balise méta personnalisée déclarée dans votre `docs.json` sous [`seo.metatags`](/fr/organize/settings-seo#param-seo-metatags) ou dans une entrée correspondante de [`seo.paths`](/fr/organize/settings-seo#param-seo-paths). Attribuez à la même clé une valeur de type chaîne non vide pour remplacer la valeur globale ou celle du dossier sur cette page. Le frontmatter ne peut pas ajouter de balises méta personnalisées que votre configuration ne déclare pas, et Mintlify ignore les valeurs qui ne sont pas des chaînes.
...
Par défaut, Mintlify n'indexe que les pages incluses dans la navigation de votre `docs.json`. Il exclut les [pages masquées](/fr/organize/hidden-pages) — les pages absentes de votre navigation, les pages situées sous un onglet ou un groupe masqué, et les pages dont le frontmatter contient `hidden: true` — de :
...
<div id="trailing-slash-urls">
  ### URL avec barre oblique finale
</div>

Si votre hébergement sert chaque page avec une barre oblique finale (par exemple, `/quickstart/` au lieu de `/quickstart`), activez `seo.trailingSlash` dans votre `docs.json` pour que les URL émises par Mintlify pour le SEO correspondent :
...
Lorsque cette option est activée, Mintlify ajoute une barre oblique finale à :

- Les balises canoniques et les balises meta `og:url`
- Les entrées `<loc>` du `sitemap.xml` généré automatiquement
- Les liens internes relatifs à la racine entre les pages, en préservant toute chaîne de requête ou tout fragment

Activer `trailingSlash` n'affecte que la façon dont les URL sont émises. Cela ne modifie pas les routes des pages, ne redirige pas les URL existantes et ne s'applique pas aux valeurs `canonical` explicites définies par page dans le [frontmatter](/fr/organize/pages). Laissez `trailingSlash` non défini ou à `false` si votre site sert des URL sans barre oblique finale.
...
Pour masquer l’intégralité de votre projet aux moteurs de recherche tout en le maintenant accessible publiquement, activez **Don't index project** sur la page [Général](https://app.mintlify.com/settings/project/general) de votre tableau de bord.

Lorsqu’il est activé, Mintlify :

- Rend chaque page avec les balises meta robots `noindex, nofollow`.
- Renvoie un `sitemap.xml` vide.
- Renvoie `404` pour `llms.txt`, y compris à l’emplacement `/.well-known/llms.txt`.
- Retire vos pages de [Mintlify Index](/fr/search-index), de sorte que les agents de code ne peuvent plus y récupérer votre contenu.
...
Votre recherche intégrée au produit, l’[assistant IA](/fr/assistant/index) et le [serveur MCP](/fr/ai/model-context-protocol) continuent de fonctionner. Pour retirer des pages de ces services, utilisez `noindex: true` ou `searchable: false` sur les pages individuelles.

<Note>
  **Don't index project** ne bloque pas [`llms-full.txt`](/fr/ai/llmstxt#llms-fulltxt), qui reste disponible à l’adresse `/llms-full.txt`. L’export Markdown de chaque page conserve également son pointeur vers `/llms.txt`, même si ce fichier renvoie `404` tant que le paramètre est activé. Si votre `llms.txt` renvoie `404` alors que `llms-full.txt` fonctionne, vérifiez ce paramètre. Consultez [Corriger un llms.txt qui renvoie 404 alors que llms-full.txt fonctionne](/fr/help-center/llms-txt-returns-404-but-llms-full-txt-works).
</Note>

Utilisez cette option lorsque vous souhaitez conserver un projet en ligne pour une relecture interne, une mise en préproduction ou un partage limité, sans qu’il apparaisse dans les résultats de recherche publics ni qu’il soit utilisé comme données d’entraînement ou de récupération pour les outils d’IA.
