...
Les balises par dossier l'emportent sur `seo.metatags` pour la même clé. Lorsqu'une page est incluse dans plusieurs entrées, les chemins plus longs l'emportent sur les plus courts. Le frontmatter de la page l'emporte toujours sur les deux. `canonical` provient toujours de `seo.metatags` et ne peut pas être défini par dossier. Consultez [`seo.paths`](/fr/organize/settings-seo#seo) pour le schéma complet.
...
Le frontmatter peut aussi remplacer toute balise méta personnalisée déclarée dans votre `docs.json` sous [`seo.metatags`](/fr/organize/settings-seo#seo) ou dans une entrée correspondante de [`seo.paths`](/fr/organize/settings-seo#seo). Fournissez une valeur de type chaîne non vide avec la même clé pour remplacer la valeur globale ou limitée à un dossier sur cette page. Le frontmatter ne peut pas introduire de balises méta que la configuration ne déclare pas, et les valeurs qui ne sont pas des chaînes sont ignorées.
...
Pour masquer l’intégralité de votre déploiement aux moteurs de recherche tout en le maintenant accessible publiquement, activez **Don't index project** sur la page [Général](https://app.mintlify.com/settings/deployment/general) de votre tableau de bord.
