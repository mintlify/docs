...
<Note>
  Pour les sites de documentation entièrement authentifiés, les sitemaps et les fichiers `robots.txt` requièrent également une authentification pour y accéder, et les sitemaps excluent les pages appartenant à des [groupes d'utilisateurs](/fr/deploy/authentication-setup#control-access-with-groups).

  Pour les sites partiellement authentifiés qui incluent des pages publiques, les sitemaps et les fichiers `robots.txt` sont accessibles publiquement. Les visiteurs anonymes ne voient que les pages publiques dans le sitemap. Les utilisateurs authentifiés voient également les pages non groupées.
</Note>
