...
<div id="configure-analytics-privacy">
  ## Configurer la confidentialité des analyses
</div>

Mintlify capture des analyses d'utilisation internes du widget. Par défaut, chaque événement inclut le chemin (pathname) actuel de la page qui intègre le widget afin que vous puissiez voir quelles pages génèrent l'utilisation de l'assistant. Le chemin est limité à 500 caractères et envoyé sans sa chaîne de requête ni son fragment. Les paramètres de requête, les fragments, les URL complètes et les référents ne sont jamais collectés.

Si votre site place des tokens, des données personnelles ou d'autres valeurs privées dans les segments du chemin, désactivez la capture du chemin à l'initialisation :
...
Modifier `analytics.capturePathname` à l'exécution nécessite d'appeler `destroy()` puis un nouvel `init()`.
