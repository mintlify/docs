...
Utilisez `filter` pour restreindre ce que l'assistant récupère lorsque votre documentation est organisée par langue ou par [versions](/organize/navigation#versions). Omettez un champ pour rechercher dans toutes ses valeurs.
...
`language` doit être un [code de langue pris en charge](/organize/settings-reference#navigation-global-languages), tel que `en`, `es`, `fr` ou `zh-Hans`. `version` correspond au nom de la version configurée dans votre tableau de bord.
...
| Option     | Type             | Description                                                                                                                                                       |
| ---------- | ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `language` | string or `null` | Restreint la recherche à un [code de langue pris en charge](/organize/settings-reference#navigation-global-languages), tel que `en`. Omettez pour rechercher dans toutes les langues. |
