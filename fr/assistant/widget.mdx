---
title: "Intégrer et configurer le widget IA Mintlify"
sidebarTitle: "Widget"
description: "Intégrez et configurez le widget IA Mintlify pour proposer un assistant entraîné sur votre contenu dans un site web, une application ou un tableau de bord."
...
Si votre site utilise une Content Security Policy, autorisez les origines requises par les fonctionnalités activées de votre widget :
...
Par exemple, utilisez le même nonce généré pour les deux scripts de la page et transmettez-le à `init()` :
...
Votre politique doit autoriser `https://widget.mintlify.com` dans `script-src`, le nonce généré dans `style-src` et les origines requises du widget et de l’API dans `connect-src`.
