...
<AccordionGroup>
  ...
  <Accordion title="Conflit">
    ...
    Pour résoudre un conflit dans l'éditeur web, ouvrez la pull request concernée chez votre fournisseur Git et choisissez la version à conserver depuis la vue de conflit. En local, éditez le fichier en conflit, supprimez les marqueurs de conflit, puis préparez (stage) et créez un commit du correctif.
  </Accordion>
  ...
  <Accordion title="Rebase">
    Rejouez les commits d'une branche sur la pointe d'une autre, en réécrivant l'historique comme si le travail avait démarré depuis la base la plus récente. Le rebase produit un historique linéaire sans commit de fusion. Consultez [Méthode de fusion par défaut](/fr/editor/settings#default-merge-method) pour savoir quand utiliser le rebase dans l'éditeur.
  </Accordion>
  ...
  <Accordion title="Squash">
    Combinez tous les commits d'une branche en un seul commit lors de la fusion. Le squash conserve un historique compact de votre branche de déploiement, mais fait perdre les messages de commit individuels de la branche de fonctionnalité. Consultez [Méthode de fusion par défaut](/fr/editor/settings#default-merge-method) pour savoir quand utiliser le squash dans l'éditeur.
  </Accordion>
