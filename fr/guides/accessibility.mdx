...
description: "Créez une documentation accessible selon les directives WCAG avec HTML sémantique, navigation clavier, texte alternatif et contenu inclusif."
...
Chaque page doit comporter un seul titre H1, qui provient de la propriété `title:` dans le frontmatter de la page. Utilisez ensuite les niveaux de titres dans l'ordre sans en sauter. Par exemple, ne passez pas de H2 à H4.
...
```mdx Bonne structure de tableau
...
```mdx Structure de tableau médiocre
| Recherche | Actif | 2024-03-15 |
| Analytics | Actif | 2024-03-10 |
| Exports | Bêta | 2024-03-20 |
```
...
Le texte alternatif rend les images accessibles aux utilisateurs de lecteurs d'écran et s'affiche lorsque les images ne se chargent pas. Les images de votre documentation doivent comporter un texte alternatif qui décrit l'image et précise clairement pourquoi vous l'avez incluse. Même avec un texte alternatif, ne vous fiez pas uniquement aux images pour transmettre des informations. Assurez-vous que votre contenu décrit ce que l'image communique.
...
- **Évitez les redondances** : Ne commencez pas par « Image de » car les lecteurs d'écran savent déjà que le texte alternatif appartient à une image. En revanche, incluez des formulations comme « Capture d'écran de » ou « Schéma de » si ce contexte est important pour l'image.
...
```json Exemple de contraste insuffisant
{
  "colors": {
    "primary": "#FFCC00",
    "background": {
...
Pour connaître les flags qui limitent l'analyse à un seul type de vérification, consultez la [référence de `mint a11y`](/fr/cli/commands#mint-a11y).
...
Les lecteurs d'écran lisent **le texte alternatif**, et il s'affiche lorsque les images ne se chargent pas. Il décrit ce que montre l'image et pourquoi elle est pertinente. Le texte alternatif est une exigence pour l'accessibilité.
...
Oui, mais avec parcimonie. Les lecteurs d'écran annoncent à voix haute le nom des emojis ; plusieurs emojis à la suite deviennent donc gênants. Par exemple, 🎉 devient « party popper » et 🚀 devient « rocket », si bien que 🎉 🚀 devient « party popper rocket ». Évitez d'utiliser des emojis pour transmettre des informations essentielles. Si vous supprimez l'emoji, le sens devrait rester clair. En cas de doute, préférez le texte.
</Accordion>
