...
Chaque exécution d'automatisation consomme des crédits. Les coûts varient selon la quantité de contenu que l'agent lit, le nombre de fichiers qu'il modifie et la durée de l'exécution. Les exécutions complexes qui mettent à jour de nombreuses pages coûtent davantage. Pour connaître le coût réel de vos automatisations, consultez le journal d'activité sur la page [Usage](https://app.mintlify.com/settings/organization/usage). Consultez [Crédits](/fr/credits) pour plus de détails sur la tarification des crédits.
