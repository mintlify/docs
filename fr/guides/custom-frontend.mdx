...
description: "Créez un frontend de documentation headless avec Astro en utilisant Mintlify pour la gestion de contenu, la recherche IA et les fonctionnalités d'assistant."
...
| AI assistant | Included | Le composant `Assistant` fournit une interface de chat IA alimentée par votre contenu. Disponible avec les [offres Pro et Enterprise](https://mintlify.com/pricing). |
...
* Un [compte Mintlify](https://app.mintlify.com)
...
<Steps>
  ...
  <Step title="S’inscrire sur Mintlify">
    Si vous n’avez pas de compte Mintlify, inscrivez-vous sur [app.mintlify.com/signup](https://app.mintlify.com/signup).
  </Step>

  <Step title="Installer la GitHub App">
    Sur la page [Git settings](https://app.mintlify.com/settings/deployment/git-settings) de votre tableau de bord Mintlify, installez la [Mintlify GitHub app](/fr/deploy/github). Si l’application est déjà installée, désinstallez-la puis réinstallez-la afin d’être prêt à connecter votre nouveau référentiel.
  </Step>

  <Step title="Connecter votre référentiel">
    1. Sur la page [Git settings](https://app.mintlify.com/settings/deployment/git-settings), sélectionnez le référentiel que vous avez créé à partir du template de démarrage.
  ...
  <Step title="Configurer les variables d’environnement">
    ...
    Votre sous-domaine correspond au nom de domaine de votre projet. C’est la partie de l’URL de votre tableau de bord après le nom de l’organisation. Par exemple, si l’URL de votre tableau de bord est `https://app.mintlify.com/org-name/domain-name`, votre sous-domaine est `domain-name`.

    Si vous avez une offre Pro ou Enterprise, générez une clé d’API de l’Assistant sur la page [API keys](https://app.mintlify.com/settings/organization/api-keys) de votre tableau de bord. La clé d’API de l’Assistant commence par `mint_dsc_`.
  </Step>
