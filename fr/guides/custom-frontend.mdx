...
<Steps>
  ...
  <Step title="Installer la GitHub App">
    Sur la page [Git settings](https://app.mintlify.com/settings/project/git-settings) de votre tableau de bord Mintlify, installez la [Mintlify GitHub app](/fr/deploy/github). Si l’application est déjà installée, désinstallez-la puis réinstallez-la afin d’être prêt à connecter votre nouveau référentiel.
  </Step>

  <Step title="Connecter votre référentiel">
    1. Sur la page [Git settings](https://app.mintlify.com/settings/project/git-settings), sélectionnez le référentiel que vous avez créé à partir du template de démarrage.
  ...
  <Step title="Configurer les variables d’environnement">
    ...
    Générez une clé d’API de l’Assistant sur la page [API keys](https://app.mintlify.com/settings/organization/api-keys) de votre tableau de bord. La clé d’API de l’Assistant commence par `mint_dsc_`.
  </Step>
...
* `unwrapNav()`: Aplatit l’arborescence de navigation en une liste pour le rendu de la barre latérale.
