---
title: "Cursor"
description: "Configurez Cursor pour en faire votre Assistant rédactionnel"
icon: "box"
---
...
Les règles Cursor fournissent un context persistant pour votre documentation, garantissant des suggestions plus cohérentes conformes à vos standards et à votre style.

* **Règles de projet** : stockées dans le référentiel de votre documentation et partagées avec votre équipe.
...
Créez des fichiers de règles dans le répertoire `.cursor/rules` de votre référentiel de documentation. Consultez la [documentation des règles Cursor](https://docs.cursor.com/context/rules) pour des instructions complètes de configuration.
...
Cette règle fournit à Cursor le context nécessaire pour formater correctement les composants Mintlify et appliquer les bonnes pratiques de rédaction technique.

Vous pouvez utiliser cet exemple tel quel ou le personnaliser pour votre documentation :

* **Normes de rédaction** : Mettez à jour les lignes directrices linguistiques pour qu’elles correspondent à votre guide de style.
...
Ajoutez cette règle, avec toutes les modifications nécessaires, en tant que fichier `.mdc` dans le répertoire `.cursor/rules` de votre dépôt de documentation.
