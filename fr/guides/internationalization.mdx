...
Si vous travaillez avec vos propres fournisseurs de traduction ou traducteurs régionaux, vous pouvez intégrer leur flux de travail à votre documentation Mintlify à l'aide de GitHub Actions ou d'outils CI/CD similaires.
...
Ce workflow GitHub Actions exporte automatiquement le contenu anglais modifié pour traduction lorsque des PRs sont fusionnées dans main.
...
**Bonnes pratiques pour les workflows de traduction externe**

- **Préserver le frontmatter** : Assurez-vous que les traducteurs conservent le frontmatter YAML intact, en ne traduisant que les valeurs de `title` et `description`.
- **Protéger les blocs de code** : Indiquez à vos prestataires de ne pas traduire les blocs de code.
- **Utiliser une mémoire de traduction** : Fournissez des glossaires avec les termes techniques qui doivent rester en anglais ou avoir des traductions spécifiques.
- **Automatiser la validation** : Utilisez des vérifications CI pour valider la syntaxe MDX et le frontmatter avant de fusionner les traductions.
- **Contrôle de version** : Suivez la version source de chaque traduction pour identifier le contenu obsolète.
