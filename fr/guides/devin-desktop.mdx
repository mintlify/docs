...
description: "Configurez l'IA Cascade de Devin Desktop avec des workspace rules pour rédiger une documentation Mintlify conforme à votre guide de style et au MDX."
keywords: ["Devin Desktop", "Windsurf", "Cascade", "workspace rules", "documentation IA", "documentation Devin"]
---

Transformez Devin Desktop en expert de la documentation qui comprend votre guide de style, vos composants et le contexte de votre projet grâce aux workspace rules et aux memories.

<Info>
  Cognition a racheté Windsurf et l'a renommé [Devin Desktop](https://devin.ai/desktop). L'assistant IA Cascade et le système de workspace rules restent identiques.
</Info>

<div id="use-devin-desktop-with-mintlify">
  ## Utilisez Devin Desktop avec Mintlify
</div>

Vous pouvez paramétrer l'assistant IA Cascade de Devin Desktop pour rédiger une documentation conforme à vos standards en utilisant les composants Mintlify. Les règles d'espace de travail et les mémoires fournissent un context persistant sur votre projet, garantissant des suggestions de Cascade plus cohérentes.

* **Règles d'espace de travail** : elles se trouvent dans votre référentiel de documentation, donc toute votre équipe les partage.
* **Mémoires** : fournissent un context individuel qui s'accumule au fil du temps.

Configurez des règles d'espace de travail pour définir des standards de documentation partagés. Vous pouvez enrichir vos mémoires au fil de votre travail, mais comme elles ne sont pas partagées, elles ne seront pas cohérentes entre les membres de l'équipe.

Créez des règles d'espace de travail dans le répertoire `.devin/rules` de votre référentiel de documentation. Devin Desktop continue de prendre en charge les règles dans `.windsurf/rules` pour la rétrocompatibilité. Consultez [Memories & Rules](https://docs.devin.ai/desktop/cascade/memories) dans la documentation Devin Desktop pour plus d'informations.
...
Enregistrez votre règle en tant que fichier `.md` dans le répertoire `.devin/rules` de votre dépôt de documentation.
...
Une fois vos règles configurées, vous pouvez utiliser Cascade pour vous aider dans diverses tâches de documentation. Consultez [Cascade](https://docs.devin.ai/desktop/cascade/cascade) dans la documentation de Devin Desktop pour en savoir plus.
...
Connectez le serveur MCP de Mintlify à Devin Desktop pour permettre à Cascade de rechercher dans la documentation Mintlify tout en vous aidant à rédiger. Lorsque vous connectez le serveur MCP, Cascade interroge la documentation Mintlify à jour pour obtenir du contexte, afin que vous n'ayez pas à quitter votre IDE pour la consulter.

Ajoutez le serveur MCP à `~/.codeium/windsurf/mcp_config.json` :
...
Consultez [Model Context Protocol](https://docs.devin.ai/desktop/cascade/mcp) dans la documentation Devin Desktop et [Model Context Protocol](/fr/ai/model-context-protocol) dans la documentation Mintlify pour plus de détails.
