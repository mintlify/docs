...
Votre fichier `robots.txt` contrôle quels bots peuvent explorer votre site. S'il bloque les agents utilisateurs IA, des outils comme ChatGPT, Claude et Perplexity ne peuvent pas lire votre documentation ni la citer dans les réponses.
...
```txt Mauvais — bloque tous les agents IA
User-agent: *
Disallow: /
```

Pour bloquer des robots spécifiques tout en autorisant les agents IA, ciblez uniquement les bots que vous souhaitez restreindre :

```txt Bon — autorise les agents IA
User-agent: BadBot
Disallow: /
...
Exécutez [`mint score`](/fr/cli/commands#mint-score) pour vérifier si le `robots.txt` de votre site autorise les agents IA. La vérification `robotsTxtAllowsAI` réussit lorsqu'aucun agent utilisateur IA n'est bloqué.
