...
Un widget réutilisable qui intègre l’[assistant](/assistant) directement dans votre application. Le widget propose :
...
Stockez les identifiants et les clés des fils de conversation pour maintenir l’historique des conversations entre les sessions.
...
Vous devez capturer et conserver les deux valeurs dès la première réponse. Pour chaque message suivant, incluez à la fois `threadId` et `threadKey` dans le corps de la requête. Si vous envoyez un `threadId` sans le `threadKey` correspondant, le serveur renvoie une erreur `404`.
...
export function AssistantWidget({ domain, docsURL }) {
  ...
  useEffect(() => {
    // Récupérer l'ID et la clé du fil sauvegardés dans localStorage
