---
title: "Liens"
description: "Créez des liens internes, référencez des endpoints d'API et maintenez l'intégrité des liens dans votre documentation avec des chemins relatifs et des ancres."
...
<div id="view-deep-links">
  ### Liens profonds vers les vues
</div>

Chaque composant `View` génère automatiquement un ID d'ancrage à partir de sa propriété `title`. Vous pouvez créer un lien direct vers une vue spécifique ou un élément à l'intérieur d'une vue en utilisant un hash dans l'URL :
...
Lorsque le hash de l'URL correspond à un ID de vue ou à un titre, un accordéon ou un onglet à l'intérieur d'une vue, la vue correspondante s'active automatiquement et la page défile jusqu'à la cible. Le hash de l'URL se met également à jour lorsque les utilisateurs changent de vue via le menu déroulant.

Utilisez la propriété `id` pour définir un hash personnalisé :
...
Cette vue est accessible via `#js-guide` au lieu du `#javascript` généré automatiquement.

Voir [Vue](/fr/components/view) pour plus d'informations.
