...
Lorsque vous utilisez des composants `View` sur une page, chaque vue obtient automatiquement un identifiant compatible avec les URL basé sur son titre. Liez directement à une vue spécifique en utilisant le hash :

```text
https://votre-url-docs/chemin-de-page#python
```

Vous pouvez également lier à des titres ou d'autres éléments ancrés à l'intérieur d'une vue. La vue correcte s'active automatiquement et la page défile jusqu'à la cible :

```text
https://votre-url-docs/chemin-de-page#installer-le-sdk
```

Lorsqu'un utilisateur change de vue via le menu déroulant, le hash de l'URL est mis à jour pour correspondre à la vue sélectionnée. Consultez [Vue](/fr/components/view#deep-links) pour plus de détails.
