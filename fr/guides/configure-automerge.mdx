...
<div id="automerge-setup-banner">
  ## Bannière de configuration de l'automerge
</div>

Si un workflow tente un automerge et que la fusion échoue parce que la GitHub App ne dispose pas des permissions de bypass (ou qu'un autre problème de configuration empêche la fusion), la page **Workflows** de votre dashboard affiche une bannière sous le sélecteur d'onglets.

La bannière renvoie vers les paramètres de branche de votre dépôt afin que vous puissiez vérifier vos rulesets et ajouter la GitHub App Mintlify à la liste de bypass. Une fois la configuration correcte et la prochaine tentative d'automerge réussie, la bannière est retirée automatiquement.

Vous pouvez fermer la bannière avec le bouton de fermeture. Elle réapparaît à la prochaine tentative d'automerge échouée pour ne pas manquer une nouvelle régression de configuration.
