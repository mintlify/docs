...
description: "Liste d’autorisation de domaines et configurations d’en-têtes pour les proxys inversés, pare-feu et réseaux appliquant des politiques de sécurité strictes"
---
...
- `connect-src` : détermine vers quelles URL il est possible d’établir une connexion pour les appels à l’API et les connexions WebSocket
- `frame-src` : détermine quelles URL peuvent être intégrées dans des frames ou des iframes
- `default-src` : valeur par défaut utilisée pour les autres directives lorsqu’elles ne sont pas définies explicitement
...
| `cdn.jsdelivr.net` | Ressources d’émojis pour images OG | `script-src`, `img-src` | Obligatoire |
...
<Note>
  Incluez uniquement les domains des services que vous utilisez. Supprimez tous les domains Analytics que vous n’avez pas configurés pour votre documentation.
</Note>
...
La plupart des proxys inverse prennent en charge l’ajout d’en-têtes personnalisés.
