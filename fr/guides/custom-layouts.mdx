...
Les pages en mode custom n'ont pas de typographie par défaut. Les éléments HTML natifs comme `<h2>` et `<p>` s'affichent avec les styles par défaut du navigateur, alors stylisez-les explicitement avec des classes Tailwind.
...
<Note>
  L'aperçu en direct de l'éditeur web ne génère pas le CSS des classes Tailwind spécifiques à une page, donc les mises en page personnalisées peuvent sembler sans styles pendant que vous éditez. Vérifiez vos styles avec `mint dev`, un déploiement de prévisualisation ou un déploiement de production. Consultez [Classes Tailwind non appliquées dans l'aperçu en direct de l'éditeur](/fr/help-center/tailwind-classes-not-applying-in-editor-preview).
</Note>
