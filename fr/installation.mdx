...
Par défaut, le navigateur s’ouvre automatiquement. Pour empêcher le navigateur de s’ouvrir, utilisez l’option `--no-open` :
...
<Note>
  Seules les URL HTTPS sont prises en charge dans les déploiements de production. L’option `--local-schema` est réservée au développement local uniquement.
</Note>
