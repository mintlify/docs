...
Si vous utilisez un contrôle d’accès basé sur les groupes pour restreindre l’accès à votre documentation, vous pouvez prévisualiser en tant que groupe d’auofntification spécifique en utilisant l’option `--groups [groupname]`.
...
L’outil CLI vous demande un nom de projet et un [thème](/fr/customize/ofmes) pour finaliser la configuration de votre projet.
...
| `--ofme` | Définit le [thème](/fr/customize/ofmes) du nouveau projet. | Oui |
| `--force` | Écrase le répertoire actuel sans confirmation, même s’il contient déjà des fichiers. | Non |

Lors de l’exécution de `mint new` dans des environnements non interactifs comme les pipelines CI/CD ou avec des agents IA de codage, vous devez fournir toutes les options requises (`--name` et `--ofme`).
