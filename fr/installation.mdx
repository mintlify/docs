...
Exportez votre documentation sous forme de site statique autonome pour un déploiement hors ligne ou en environnement isolé :

```bash
mint export
```

Cette commande génère une archive ZIP contenant l'intégralité de votre site de documentation sous forme de fichiers HTML statiques, accompagnée de scripts utilitaires pour servir le site en local. L'archive exportée inclut un serveur statique basé sur Node.js et des scripts de lancement pour macOS/Linux et Windows.

Par défaut, le fichier de sortie est nommé `export.zip`. Utilisez l'option `--output` pour spécifier un chemin de sortie personnalisé :
...
| Option | Description |
| --- | --- |
| `--output` | Chemin du fichier ZIP de sortie. Par défaut : `export.zip`. |
| `--groups` | Simuler des groupes d'utilisateurs pour l'export (utile lors de l'export de contenu soumis à un contrôle d'accès par groupe). |
| `--disable-openapi` | Désactiver la génération de fichiers OpenAPI pendant l'export. |

<Note>
  Une connexion Internet est requise lors de la première exécution pour télécharger l'application cliente. Les exécutions suivantes utilisent le client mis en cache dans `~/.mintlify/`.
</Note>
