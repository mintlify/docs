...
Par défaut, seuls les liens internes sont vérifiés. Utilisez les options suivantes pour étendre la portée :

| Option | Description |
| --- | --- |
| `--check-anchors` | Valide également les liens d'ancrage (par exemple, `/page#section`) par rapport aux slugs des titres. |
| `--check-external` | Vérifie également les liens externes pour détecter les URLs brisées. |
