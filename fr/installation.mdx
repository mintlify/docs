...
Par défaut, seuls les liens internes sont vérifiés. Utilisez des options pour étendre la portée :

| Flag               | Description                                                                                           |
| ------------------ | ----------------------------------------------------------------------------------------------------- |
| `--check-anchors`  | Vérifie également les liens d’ancrage (par exemple, `/page#section`) par rapport aux slugs de titres. |
...
Extrayez du contenu d'un site de documentation externe ou d'une spécification OpenAPI à l'aide des commandes `mint scrape`. Cela est utile lorsque vous migrez votre documentation depuis une autre plateforme vers Mintlify.
...
Utilisez l'option `--filter` (ou `-f`) pour limiter l'extraction aux URL correspondant à un préfixe de chemin spécifique :
...
**Générer des pages à partir d'une spécification OpenAPI :**
...
| `--no-writeFiles` | Prévisualiser la sortie sans écrire de fichiers. |

<div id="upgrade-configuration">
  ### Mettre à jour la configuration
</div>

Convertissez un fichier de configuration `mint.json` au format actuel `docs.json` :

```bash
mint upgrade
```

Consultez [Paramètres globaux](/fr/organize/settings) pour plus d'informations sur `docs.json`.

<div id="create-a-workflow">
  ### Créer un workflow
</div>

Créez des fichiers de workflow de manière interactive avec la commande suivante :

```bash
mint workflow
```

Cette commande vous guide dans la création d'un fichier de workflow dans `.mintlify/workflows/`. Elle vous demande le nom du workflow, le type de déclencheur (cron ou push), les référentiels de contexte, les paramètres d'auto-fusion, et ouvre votre éditeur par défaut pour rédiger les instructions de l'agent.

<Tip>
  L'interface en ligne de commande (CLI) détecte automatiquement les environnements non interactifs. Dans les environnements non interactifs, elle affiche les instructions d'utilisation au lieu de rester bloquée en attente de réponses aux invites.
</Tip>

Consultez [Workflows](/fr/agent/workflows) pour plus d'informations sur les fichiers de workflow et les options de configuration.

<div id="check-version">
  ### Vérifier la version
</div>

Affichez la version actuelle du CLI et du client :
