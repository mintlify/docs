...
La CLI collecte des données de télémétrie d'utilisation anonymes pour aider à améliorer Mintlify. Les données de télémétrie incluent le nom de la commande, la version de la CLI, le système d'exploitation et l'architecture. Mintlify ne collecte **pas** d'informations personnelles identifiables, de contenu de projet ni de chemins de fichiers.

Par défaut, la CLI collecte des données de télémétrie. Vous pouvez vous désinscrire à tout moment en utilisant l'option `--telemetry` :
...
| --- | --- | --- |
| `MINTLIFY_TELEMETRY_DISABLED` | `1` | Désactive la télémétrie de la CLI Mintlify. |
| `DO_NOT_TRACK` | `1` | Désactive la télémétrie en utilisant le standard [Console Do Not Track](https://consoledonottrack.com/). |
