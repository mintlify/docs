...
Les enregistrements `TXT` sont généralement validés en 5 minutes. Si la validation entre dans une période d’attente, la prochaine tentative automatique peut prendre plusieurs heures. Si vos enregistrements sont toujours en attente après 5 minutes et que vous avez vérifié qu’ils sont corrects, relancez la validation manuellement depuis votre Dashboard pour contourner l’attente.
