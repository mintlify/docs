...
description: "Hébergez votre documentation sur un domaine personnalisé avec configuration DNS, certificats TLS automatiques et routage par sous-domaine ou sous-chemin."
...
<div id="managing-a-custom-domain">
  ## Gérer un domaine personnalisé
</div>

<div id="default-subdomain-remains-active">
  ### Le sous-domaine par défaut reste actif
</div>

L’URL par défaut `<subdomain>.mintlify.site` de votre déploiement ne peut aujourd’hui être ni désactivée ni redirigée, même après la mise en service d’un domaine personnalisé. Les deux URL servent le même contenu. Définissez une [URL canonique](#set-a-canonical-url) pour que les moteurs de recherche indexent le domaine personnalisé comme URL principale.

<div id="renaming-a-project">
  ### Renommer un projet
</div>

Renommer un projet dans votre dashboard modifie uniquement le nom d’affichage. Cela ne change ni l’URL de déploiement ni le nom d’hôte par défaut `<subdomain>.mintlify.site`.

Si vous renommez un projet et voyez ensuite une erreur `Disallowed origin host` à votre URL par défaut, c’est que le nom d’hôte du déploiement ne correspond plus à ce que votre dashboard attend. Pour changer l’URL, supprimez le déploiement et recréez-le avec le nom souhaité. Il n’existe pas de renommage sur place de l’URL de déploiement.

<div id="releasing-a-domain">
  ### Libérer un domaine
</div>

Un domaine personnalisé ne peut être attaché qu’à un seul déploiement Mintlify à la fois. Si votre domaine est encore attaché à un déploiement ou une organisation précédent (par exemple, après un déplacement vers un nouveau workspace), le nouveau déploiement ne peut pas le revendiquer tant que l’ancien ne l’a pas libéré.

Pour libérer un domaine que vous contrôlez :

1. Contactez le [support](mailto:support@mintlify.com) et demandez la libération du domaine.
2. Le support fournit un enregistrement `TXT` de vérification avec une valeur unique.
3. Ajoutez l’enregistrement chez votre fournisseur DNS :

   ```text
   TXT | <votre-domaine> | <valeur fournie par le support>
   ```

4. Répondez au support une fois l’enregistrement en ligne. Le support vérifie l’enregistrement et détache le domaine du déploiement précédent. Vous pouvez ensuite l’ajouter à votre nouveau déploiement en suivant [Ajoutez votre domaine personnalisé](#add-your-custom-domain).
