...
Vous pouvez charger du JavaScript externe à partir d'une URL en ajoutant `customScripts` à l'objet `integrations` dans votre fichier `docs.json`. Chaque entrée nécessite une URL HTTPS `src` et accepte une option `strategy` pour contrôler le moment de chargement du script.
...
<ResponseField name="src" type="string" required>
  L'URL HTTPS du script à charger. Les URL pointant vers des fichiers `.svg`, `.exe` ou `.zip` ne sont pas autorisées.
</ResponseField>

<ResponseField name="strategy" type="string">
  Contrôle le moment de chargement du script. Options :
  - `afterInteractive` — charge le script une fois la page interactive. C'est la valeur par défaut.
  - `lazyOnload` — charge le script pendant les temps d'inactivité, après le chargement complet du contenu de la page.
</ResponseField>

Vous pouvez inclure jusqu'à 20 scripts personnalisés.
