...
Chargez des scripts tiers depuis des URLs externes sur chaque page de votre site de documentation. C'est utile pour ajouter des services d'analyse, des widgets de chat, des gestionnaires de consentement ou d'autres services fournissant un script hébergé.

Vous pouvez gérer les scripts externes depuis le [tableau de bord](https://dashboard.mintlify.com) dans **Settings > Add-ons > Custom scripts**. Pour chaque script, fournissez une URL HTTPS et choisissez une stratégie de chargement :
...
<Note>
  Vous pouvez ajouter jusqu'à 20 scripts personnalisés. Chaque URL doit utiliser HTTPS et ne doit pas pointer vers des types de fichiers interdits tels que `.svg`, `.exe` ou `.zip`.
</Note>
