...
Chargez des scripts tiers depuis des URLs externes en utilisant l'option `customScripts` dans `docs.json`. C'est utile pour ajouter des services d'analyse, des widgets de chat ou d'autres services fournissant un script hébergé.

Chaque entrée nécessite une URL `src` (HTTPS obligatoire) et accepte une option `strategy` pour contrôler le moment du chargement :
...
<Note>
  Vous pouvez ajouter jusqu'à 20 scripts personnalisés. L'URL `src` doit utiliser HTTPS et ne doit pas pointer vers des types de fichiers interdits tels que `.svg`, `.exe` ou `.zip`.
</Note>
