...
<AccordionGroup>
  ...
  <Accordion title="Sélecteurs">
    ...
    **Attributs de données**
    ...
    Certains éléments exposent un attribut `data-active` lorsqu'ils sont dans un état actif.
    ...
    - TableOfContentsItem (le plus profond) : `toc-item[data-active-deepest]`

    L'attribut `data-active` sur les éléments de la table des matières est défini sur la section actuellement visible et toutes ses sections parentes. Utilisez `data-active-deepest` pour cibler uniquement la section active la plus profonde.

    Les éléments de la table des matières exposent également un attribut `data-depth` indiquant leur niveau d'imbrication, à partir de `0` pour les titres de premier niveau.
  </Accordion>
</AccordionGroup>
