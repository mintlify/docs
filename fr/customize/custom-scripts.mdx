...
<div id="style-components-with-classname">
  ### Styliser les composants avec `className`
</div>
...
Trois composants n'acceptent pas `className` : [Banner](/fr/components/banner), [MDX](/fr/components/mdx) et [Visibility](/fr/components/visibility).

<div id="arbitrary-values-and-variants">
  ### Valeurs arbitraires et variantes
</div>
...
<Note>
  L'aperçu en direct de l'éditeur web ne génère pas le CSS des classes Tailwind propres à une page ; une page stylisée peut donc sembler sans style pendant que vous la modifiez. Consultez [Les classes Tailwind ne s'appliquent pas dans l'aperçu en direct de l'éditeur](/fr/help-center/tailwind-classes-not-applying-in-editor-preview).
</Note>
...
Vous pouvez combiner des noms de classes personnalisés avec les classes [Tailwind CSS](/fr/customize/custom-scripts#style-with-tailwind-css) sur le même élément.

Le CSS personnalisé s'applique à chaque page de votre site, y compris les pages en mode personnalisé et les pages d'accueil. Pour limiter les styles à une page ou une section spécifique, utilisez le sélecteur d'attribut `html[data-current-path="..."]` décrit dans [Attributs de données](#data-attributes).
...
<Accordion title="Sélection de texte">
  - `#text-selection-tooltip`—Infobulle qui apparaît lorsque vous sélectionnez du texte sur la page.
...
<AccordionGroup>
  <Accordion title="Composants de contenu">
    ...
    - `code-block-copy-button`—Bouton de copie affiché sur un bloc de code. Masquez-le avec `.code-block-copy-button { display: none; }`.
    - `code-block-fade-overlay`—Superposition de fondu affichée au bas d'un bloc de code réduit.
  ...
  <Accordion title="Navigation de la barre supérieure">
    - `nav-logo`—Image du logo dans la barre de navigation. Pour remplacer ses utilitaires de taille par défaut, ciblez `img.nav-logo` et utilisez `!important`.
  ...
  <Accordion title="Attributs de données">
    ...
    - `[data-component-part="card-content-container"]`—Wrapper interne autour du contenu d'une carte.
    - `[data-component-part="card-icon"]`—Icône à l'intérieur d'une carte. Utilisez-le pour styliser les icônes de carte, car le sélecteur d'élément `icon` ne correspond pas aux icônes à l'intérieur des cartes.
    - `[data-component-part="card-title"]`—Texte du titre à l'intérieur d'une carte.
    - `[data-component-part="card-content"]`—Contenu de la description à l'intérieur d'une carte.
    - `[data-component-part="card-image"]`—Image affichée en haut d'une carte.
    - `[data-component-part="card-cta"]`—Libellé d'appel à l'action à l'intérieur d'une carte.

    Parties du panneau assistant :
...
Par exemple, vous pouvez ajouter le fichier `ga.js` suivant pour activer [Google Analytics](https://marketingplatform.google.com/about/analytics) sur l'ensemble de la documentation.
...
<div id="access-personalized-user-data">
  ### Accéder aux données personnalisées de l'utilisateur
</div>

Si votre site utilise l'[authentification](/fr/deploy/authentication-setup) ou la [personnalisation](/fr/create/personalization), les scripts personnalisés peuvent lire le visiteur identifié depuis `window.mintlify.user`. Il s'agit du même objet que celui exposé aux pages MDX via la [variable `user`](/fr/create/personalization#dynamic-mdx-content) : il reflète le champ `content` de vos données utilisateur.
