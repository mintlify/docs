...
Vous pouvez charger des scripts externes sur chaque page en utilisant le champ `integrations.customScripts` dans votre `docs.json`. Chaque entrée nécessite une URL HTTPS dans `src` et accepte une `strategy` optionnelle pour contrôler le moment du chargement du script.

| Stratégie | Description |
|---|---|
| `beforeInteractive` | Injecte le script dans le `<head>` avant que la page ne devienne interactive. Utilisez-le pour les scripts qui doivent s'exécuter avant le chargement du contenu de la page. |
| `afterInteractive` | Charge le script après que la page est devenue interactive. C'est le comportement par défaut. |
| `lazyOnload` | Diffère le chargement jusqu'à ce que le navigateur soit inactif. |
...
Vous pouvez ajouter jusqu'à 20 scripts externes. Le `src` doit être une URL HTTPS et ne peut pas pointer vers des fichiers `.svg`, `.exe` ou `.zip`.
