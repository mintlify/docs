...
Vous pouvez également référencer des variables d’extrait à l’intérieur des blocs de code. Utilisez la même syntaxe `{variableName}` dans le corps d’un bloc de code délimité, et la valeur transmise en propriété remplace l’espace réservé au rendu de la page. Les espaces réservés qui ne correspondent à aucune propriété restent inchangés.

1. Ajoutez à votre extrait un bloc de code qui référence une variable.

    ```mdx shared/install-snippet.mdx
    Installez la CLI avec la commande suivante :
...
2. Importez l’extrait et transmettez la valeur en propriété.

    ```mdx destination-file.mdx
    ---
    title: "Une page d'exemple"
    description: "Ceci est une page d'exemple qui importe un extrait avec une variable de bloc de code."
    ---

    import InstallSnippet from "/shared/install-snippet.mdx";

    <InstallSnippet installCommand="npm i -g mintlify" />
    ```
