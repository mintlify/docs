...
Le nom que vous attribuez dans l’instruction `import` est la balise JSX utilisée pour rendre le snippet. Il doit être un identifiant de composant JSX valide : commencez par une lettre majuscule et utilisez la PascalCase (par exemple, `MySnippet`, pas `mySnippet` ni `my-snippet`). Les noms commençant par une minuscule sont traités comme des éléments HTML et ne rendent pas votre snippet.
