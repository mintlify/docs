...
Vous pouvez également référencer des variables d’extrait à l’intérieur des blocs de code. Utilisez la même syntaxe `{variableName}` dans le corps d’un bloc de code délimité. Au rendu de la page, Mintlify remplace chaque espace réservé par la valeur de la propriété correspondante et laisse intacts ceux sans correspondance.
