...
L’utilisation d’accolades dans une instruction `import` dépend de la façon dont le snippet est exporté. Importez le contenu rendu d’un fichier `.md` ou `.mdx` en tant qu’export par défaut sans accolades (`import MySnippet from "..."`). Utilisez des accolades pour les exports nommés, comme les variables ou les composants React (`import { myName } from "..."` ou `import { MyComponent } from "..."`).
