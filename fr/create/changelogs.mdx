...
Un `Update` ne s'affiche que si chacun de ses tags est inclus dans la sélection de filtres actifs. Les `Update` sans tags sont masqués dès qu'au moins un filtre est actif. Pour qu'un `Update` apparaisse sous plusieurs filtres, listez chacun de ces tags sur l'`Update`.
