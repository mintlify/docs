...
Par défaut, les IDs d'ancrage sont générés automatiquement à partir du texte du titre. Vous pouvez remplacer l'ID généré par un ID personnalisé en utilisant la syntaxe `{#custom-id}` :
...
<Note>
  Les IDs de titre personnalisés sont pris en charge pour les niveaux de titre 1 à 4 (`#` à `####`). Les niveaux 5 et 6 ne sont pas pris en charge.
</Note>
