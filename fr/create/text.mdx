...
Par défaut, Mintlify génère automatiquement les IDs d'ancrage à partir du texte du titre. Vous pouvez remplacer l'ID généré par un ID personnalisé en utilisant la syntaxe `{#custom-id}` :
...
Les IDs personnalisés sont conservés exactement tels que vous les écrivez, y compris la casse et les underscores. Vous pouvez utiliser camelCase, kebab-case, snake_case ou n'importe quelle combinaison :
...
Ces titres sont accessibles à `#quickStart`, `#setup_steps` et `#advanced-options` respectivement. Respectez exactement la casse lorsque vous liez à un ID personnalisé — les ancrages sont sensibles à la casse.

<Tip>
  La conservation de la casse est utile lors d'une migration depuis des outils de documentation qui utilisent des ancrages camelCase ou snake_case. Conservez vos IDs personnalisés existants pour éviter de casser les liens entrants.
</Tip>
