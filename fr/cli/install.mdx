...
<Visibility for="agents">
  ...
  Commencez par exécuter `mint status`. Préservez toute session déjà authentifiée. Sinon, exécutez `mint login` pour un compte existant ou recueillez le vrai nom, l'entreprise et l'adresse e-mail de l'utilisateur, puis exécutez `mint signup` pour un nouveau compte. L'utilisateur doit cliquer sur le lien de vérification reçu par e-mail et approuver l'authentification dans le navigateur, mais vous devez effectuer les étapes CLI restantes. Consultez [`mint signup`](/fr/cli/commands#mint-signup) pour connaître les flags et les conseils concernant les processus en arrière-plan.
</Visibility>
