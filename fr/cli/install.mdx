...
<Note>
  Le nom du package est `mint`, pas `mintlify`. Le package hérité `mintlify` est obsolète. Si vous avez précédemment installé `mintlify`, consultez [Package mintlify versus mint](#troubleshooting) dans la section de dépannage ci-dessous.
</Note>
