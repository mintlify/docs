...
| `openApiSpec` | Toute spécification OpenAPI ou Swagger servie à un chemin standard est valide. Cette vérification réussit lorsqu'aucune spécification n'est servie et n'échoue que lorsqu'un fichier de spécification servi n'est pas un document OpenAPI ou Swagger valide. |
...
| `pagesHaveMarkdown` | Les pages échantillonnées depuis le plan du site ont une contrepartie Markdown correspondante (`path.md` ou `path/index.md`) qui sert du contenu non HTML. Réussit à 90 % ou plus de couverture. |
