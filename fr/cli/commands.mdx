...
Formate chaque fichier `.mdx` du répertoire courant selon le style canonique de Mintlify. La commande analyse chaque fichier avec le même convertisseur que celui utilisé par l'éditeur web, puis le réécrit sur place si la sortie canonique diffère. Les fichiers formatés correspondent à la sortie que l'éditeur web écrit lors de la publication, donc publier depuis l'éditeur sur un dépôt formaté produit un diff qui ne contient que les lignes modifiées.

```bash
mint format
```

Exécutez la commande depuis la racine de votre projet de documentation. Elle parcourt tous les sous-répertoires, en ignorant les chemins correspondant à `.gitignore` et à toute règle d'exclusion Mintlify. Les fichiers qui correspondent déjà à la sortie canonique restent inchangés. Si le formatage devait supprimer le frontmatter d'un fichier, la commande laisse ce fichier inchangé et le signale comme un échec.

<Warning>
  `mint format` réécrit les fichiers sur place. Committez ou stashez vos modifications avant de l'exécuter afin de pouvoir examiner le diff. La première exécution sur un projet existant peut produire un diff ponctuel volumineux lors de la normalisation des fichiers, notamment les guillemets du frontmatter, l'espacement des listes et l'indentation des commentaires. Après cette exécution, les exécutions suivantes laissent intacts les fichiers déjà formatés.
</Warning>

Une fois terminé, la commande affiche le nombre de fichiers MDX reformatés et le nombre de fichiers en échec. Si un fichier échoue, la commande se termine avec le code `1` et affiche le chemin du fichier ainsi que l'erreur, ce qui vous permet de l'exécuter en CI pour imposer un formatage cohérent.
