...
Ouvre une fenêtre de navigateur pour compléter l'authentification. Si le navigateur ne s'ouvre pas, la CLI affiche une URL à ouvrir manuellement et un champ pour coller le code d'autorisation. Après une connexion réussie, la CLI vous invite à sélectionner un projet par défaut lorsque votre organisation a plusieurs projets, ou le définit automatiquement s'il n'y en a qu'un. Les identifiants sont enregistrés dans `~/.config/mintlify/config.json`.
