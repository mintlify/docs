...
Exécutez la commande sans flags pour vous inscrire de manière interactive — la CLI vous demande votre prénom, votre nom, le nom de l'entreprise et votre adresse e-mail, puis ouvre une fenêtre du navigateur pour vérifier votre e-mail. Après avoir cliqué sur le lien de vérification, l'authentification s'effectue automatiquement et vos identifiants sont enregistrés dans `~/.config/mintlify/config.json`. Une fois inscrit, vous êtes connecté et pouvez exécuter `mint status` pour confirmer la session.

Passez les flags ci-dessus pour ignorer les invites et vous inscrire de manière non interactive, ce qui est utile lorsqu'un agent de codage exécute la commande en votre nom :

```bash
mint signup --firstName Ada --lastName Lovelace --company Analytical --email ada@example.com
```

Une fois l'inscription terminée, visitez le [tableau de bord Mintlify](https://dashboard.mintlify.com) pour connecter votre dépôt GitHub et finaliser la configuration de votre déploiement.
