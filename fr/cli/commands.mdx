...
Exécutez la commande depuis votre répertoire de documentation. Elle analyse vos pages à la recherche de blocs de code délimités, vérifie quelles pages contiennent des exemples testables, demande à un agent de codage de générer des tests pour chaque page, exécute les tests générés et indique quelles pages réussissent ou échouent.
...
Dans les terminaux interactifs, la commande ouvre une interface guidée où vous sélectionnez un agent de codage et les pages à tester, suivez la progression en direct et examinez les résultats, y compris des aperçus des fichiers de test générés. Si une exécution précédente a été interrompue, vous pouvez la reprendre, et les exécutions passées sont enregistrées pour que vous puissiez consulter à nouveau leurs rapports.

Dans les environnements non interactifs, comme la CI, la commande exécute Claude sur des pages sélectionnées automatiquement, affiche un résumé des pages réussies et échouées, et se termine avec le code `0` si tous les tests réussissent ou `1` sinon.
