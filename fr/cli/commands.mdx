...
Exécuter des vérifications de préparation pour les agents sur un site de documentation public. Renvoie un score et une répartition des vérifications individuelles. Nécessite une authentification avec `mint login`.

```bash
mint score --url <url> [flags]
```

| Flag | Description |
| --- | --- |
| `--url` | URL du site de documentation à vérifier. Obligatoire. |
| `--format` | Format de sortie : `table` (par défaut, sortie colorée), `plain` (TSV pour canalisation), ou `json`. |
