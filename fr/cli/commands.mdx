...
Formate chaque fichier `.mdx` du répertoire courant selon le style canonique de Mintlify. La commande analyse chaque fichier avec le même convertisseur que celui utilisé par l'éditeur web, puis le réécrit sur place si la sortie canonique diffère. Les fichiers formatés correspondent à la sortie que l'éditeur web écrit lors de la publication. Publier depuis l'éditeur sur un dépôt formaté produit un diff qui ne contient que les lignes modifiées.
