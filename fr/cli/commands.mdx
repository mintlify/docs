...
Les commandes se terminent avec le code `0` en cas de succès et avec un code non nul en cas d'échec. Les vérifications telles que `mint validate`, `mint broken-links`, `mint a11y`, `mint deslop` et `mint format` se terminent avec un code non nul lorsqu'elles détectent des problèmes. Cela vous permet de les exécuter en CI pour faire échouer un pipeline en cas de régressions.
