...
La CLI `mint` nécessite [Node.js](https://nodejs.org) v20.17.0 ou une version supérieure. Installez la CLI globalement avec npm :

```bash
npm i -g mint
```

Exécutez les commandes depuis la racine de votre projet de documentation (le répertoire contenant `docs.json`), sauf si une commande accepte explicitement un argument de chemin. Consultez le [Guide de démarrage rapide](/fr/quickstart) pour un tutoriel complet.
