...
<ResponseField name="integrations.customScripts" type="array of object">
  Fichiers JavaScript externes à charger sur chaque page. Vous pouvez inclure jusqu'à 20 scripts.

  <Expandable title="customScripts item">
    <ResponseField name="src" type="string (HTTPS URL)" required>
      L'URL du script à charger. Doit utiliser HTTPS. Les URL pointant vers des fichiers `.svg`, `.exe` ou `.zip` ne sont pas autorisées.
    </ResponseField>
    <ResponseField name="strategy" type="string">
      Contrôle le moment de chargement du script. Options : `afterInteractive` (par défaut), `lazyOnload`.
    </ResponseField>
  </Expandable>
</ResponseField>
