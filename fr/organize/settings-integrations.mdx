...
<ResponseField name="integrations.customScripts" type="array of object">
  Chargez des scripts tiers personnalisés sur votre site de documentation. Maximum de 20 scripts.

  <Expandable title="integrations.customScripts">
    <ResponseField name="src" type="string (uri)" required>
      L'URL HTTPS du script à charger. Ne doit pas pointer vers un type de fichier non autorisé (`.svg`, `.exe`, `.zip`).
    </ResponseField>
    <ResponseField name="strategy" type="string">
      Contrôle le moment où le script se charge. `afterInteractive` ou `lazyOnload`. Par défaut : `afterInteractive`.
    </ResponseField>
  </Expandable>
</ResponseField>
