---
title: "Intégrations"
description: "Connectez des plateformes d'analyse, des outils de chat et d'autres services tiers à votre site de documentation dans votre fichier `docs.json`."
...
<ResponseField name="integrations.customScripts" type="array of objects">
  Chargez des fichiers JavaScript externes sur chaque page. Vous pouvez ajouter jusqu'à 20 scripts personnalisés.

  <Expandable title="integrations.customScripts">
    <ResponseField name="src" type="string (uri)" required>
      URL HTTPS du script à charger. Ne doit pas pointer vers un type de fichier interdit (par ex. `.svg`, `.exe`, `.zip`).
    </ResponseField>
    <ResponseField name="strategy" type="string">
      Quand charger le script. L'une des valeurs suivantes :
      - `beforeInteractive` — Charger avant que la page ne devienne interactive. Utilisez pour les scripts qui doivent s'exécuter le plus tôt possible, comme le consentement aux cookies ou les gestionnaires de balises.
      - `afterInteractive` — Charger après que la page est devenue interactive. C'est le comportement par défaut.
      - `lazyOnload` — Charger pendant les temps d'inactivité. Utilisez pour les scripts de faible priorité comme les outils d'analyse qui n'ont pas besoin de s'exécuter immédiatement.
    </ResponseField>
  </Expandable>
</ResponseField>
