---
title: "Pages"
description: "Configurez les métadonnées, titres, descriptions, icônes et modes de mise en page via les propriétés YAML en haut de votre documentation MDX."
...
Tous les champs du frontmatter sont facultatifs. Si vous omettez `title`, Mintlify en génère un à partir du chemin du fichier. Vous pouvez omettre entièrement le bloc de frontmatter sur les pages qui n'ont pas besoin de métadonnées personnalisées.
...
<ResponseField name="noindex" type="boolean">
  Définir sur `true` pour exclure la page de la recherche sur le site, des sitemaps, de l’indexation par les moteurs de recherche et du contexte de l’assistant IA. La page reste visible dans la navigation. Voir [Désactiver l’indexation](/fr/optimize/seo#disable-indexing) pour plus de détails. Toutes les pages avec `hidden: true` dans leur frontmatter reçoivent automatiquement `noindex: true`.
</ResponseField>

<ResponseField name="searchable" type="boolean" default="true">
  Vaut `true` par défaut : les pages sont consultables. Définir sur `false` pour exclure la page des résultats de recherche de votre site de documentation et du contexte de l'assistant IA. La page reste indexable par les moteurs de recherche externes, listée dans votre sitemap et visible dans la navigation. Voir [Exclure une page de la recherche](/fr/optimize/search#exclude-a-page-from-search) pour plus de détails.
</ResponseField>

<ResponseField name="boost" type="number">
  Multiplie le classement de la page dans la recherche intégrée par ce facteur. Utilisez des valeurs supérieures à `1` pour prioriser la page et des valeurs comprises entre `0` et `1` pour la déprioriser. Voir [Recherche](/fr/optimize/search#boost-search-ranking) pour plus de détails. `boost` n'a aucun effet lorsque `searchable: false`, puisque la page n'apparaît pas dans les résultats de la recherche intégrée.
</ResponseField>
...
<ResponseField name="related" type="array or boolean">
  Une liste de pages associées à afficher dans la section **Sujets associés** en bas de la page, ou `false` pour masquer entièrement la section. Remplace les suggestions automatiques du déploiement. Nécessite l'activation du module complémentaire [Pages associées](/fr/organize/related-pages). Voir [Pages associées](/fr/organize/related-pages) pour plus de détails.
</ResponseField>
...
<ResponseField name="contextual" type="object">
  Remplace le [menu contextuel](/fr/ai/contextual-menu) défini à l’échelle du site pour cette page. Accepte le même tableau `options` et la même valeur `display` que le champ `contextual` dans `docs.json`. Omettez le champ pour hériter de la configuration globale, ou définissez `options: []` pour désactiver le menu contextuel sur cette page.
</ResponseField>
...
Le mode Assistant affiche la page comme une expérience d’[assistant](/fr/assistant) en plein écran. Une interface de chat remplace tout le contenu de la page. Utilisez ce mode pour créer une page dédiée permettant aux utilisateurs d’interagir avec l’assistant, comme une page « Ask AI ». Tous les thèmes prennent en charge le mode Assistant.

Pour utiliser le mode Assistant, vous devez activer l’[assistant](/fr/assistant) sur votre déploiement. Si vous n’activez pas l’assistant, le mode de page Assistant affiche un champ de recherche désactivé.

```yaml
---
title: "Demander à l’IA"
description: "Posez des questions sur la prise en main et l’utilisation du produit"
mode: "assistant"
---
```
...
Liez des sites externes directement depuis votre navigation avec la métadonnée `url`.
...
Mintlify génère automatiquement la plupart des balises méta SEO. Vous pouvez définir manuellement des balises méta SEO pour personnaliser votre approche du SEO, le partage sur les réseaux sociaux et la compatibilité des navigateurs.

<Note>
  Encadrez toujours de guillemets les balises méta contenant des deux-points.
</Note>
...
Aidez les utilisateurs à découvrir une page spécifique dans les résultats de recherche en ajoutant `keywords` dans vos métadonnées. Ces mots-clés n’apparaissent pas dans le contenu de la page. Si des utilisateurs recherchent ces mots-clés, la page s’affiche dans les résultats de recherche.
...
La date affichée est déterminée dans l’ordre suivant :

1. Le champ de frontmatter `lastUpdatedDate`, s’il est défini sur la page.
2. Pour les déploiements adossés à GitHub ou GitLab, la date du dernier commit Git ayant modifié le fichier source de la page.
3. L’horodatage du dernier déploiement.

Pour afficher une date spécifique au lieu de la date du commit Git, définissez le champ de frontmatter `lastUpdatedDate`. Utilisez ce champ lorsque votre historique Git ne reflète pas le moment où le contenu a changé, par exemple pour du contenu importé ou synchronisé depuis un autre système.
...
Utilisez une date au format `YYYY-MM-DD` ou une chaîne datetime ISO 8601 complète. Si la valeur est manquante, vide ou impossible à analyser, Mintlify utilise la date du commit Git, puis l’horodatage du déploiement. Mintlify accepte aussi `last_updated_at` comme alias de `lastUpdatedDate`.
