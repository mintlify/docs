...
<ResponseField name="related" type="array or boolean">
  Une liste de pages associées à afficher dans la section **Sujets associés** en bas de la page, ou `false` pour masquer entièrement la section. Remplace les suggestions automatiques du projet. Nécessite l'activation du module complémentaire [Pages associées](/fr/organize/related-pages). Voir [Pages associées](/fr/organize/related-pages) pour plus de détails.
</ResponseField>
...
Pour utiliser le mode Assistant, vous devez activer l’[assistant](/fr/assistant) sur votre projet. Si vous n’activez pas l’assistant, le mode de page Assistant affiche un champ de recherche désactivé.
...
2. Pour les projets adossés à GitHub ou GitLab, la date du dernier commit Git ayant modifié le fichier source de la page.
