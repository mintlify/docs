...
Ajoutez un indicateur &quot;Dernière modification le [date]&quot; à toutes les pages en activant `metadata.timestamp` dans vos [paramètres globaux](/fr/organize/settings#metadata).
...
Remplacez le paramètre global d’horodatage pour des pages individuelles à l’aide du champ de frontmatter `timestamp`. Utilisez ce champ pour afficher ou masquer les horodatages au cas par cas sur des pages spécifiques.
...
Lorsque la valeur est définie sur `true`, l’horodatage s’affiche même si le paramètre global est `false`. Lorsque la valeur est définie sur `false`, l’horodatage est masqué même si le paramètre global est `true`.
