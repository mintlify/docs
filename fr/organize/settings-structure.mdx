...
<ResponseField name="navigation.mode" type='"auto" | "tree"'>
  Définit le mode de navigation par défaut pour tous les groupes. Lorsque défini sur `"tree"`, les groupes avec une page `root` affichent une arborescence de navigation sur la page racine. Hérité par tous les groupes descendants sauf s'il est remplacé. Par défaut : `"auto"`.
</ResponseField>
