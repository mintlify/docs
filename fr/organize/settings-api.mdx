...
description: "Configurez les spécifications OpenAPI et AsyncAPI, le playground interactif, les exemples de code et l'authentification dans `docs.json`."
...
<ResponseField name="api.playground" type="object">
  ...
  <Expandable title="api.playground">
    ...
    <ResponseField name="credentials" type="boolean">
      Indique s'il faut inclure les cookies et les en-têtes d'authentification pour les requêtes cross-origin lorsque le proxy est désactivé. Valeur par défaut : `false`. Ne prend effet que lorsque `proxy` est `false`.
    </ResponseField>
  </Expandable>
</ResponseField>
