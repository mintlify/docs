...
| `integrations.customScripts` | array of objects (max 20) | Non | Scripts externes à charger sur chaque page. |
| `integrations.customScripts[].src` | string (uri) | Oui | URL HTTPS du script. Ne doit pas pointer vers un type de fichier interdit. |
| `integrations.customScripts[].strategy` | `"beforeInteractive"` \| `"afterInteractive"` \| `"lazyOnload"` | Non | Quand charger le script. |
