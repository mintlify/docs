...
| `integrations.customScripts` | array of objects | Non | Scripts externes à charger. Max 20 éléments. Chaque objet a `src` (requis, URL HTTPS ; `.svg`, `.exe`, `.zip` non autorisés) et `strategy` (`"afterInteractive"` ou `"lazyOnload"`). |
