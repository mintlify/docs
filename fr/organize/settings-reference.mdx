...
| `integrations.customScripts` | array of object | Non | Scripts tiers personnalisés à charger. Maximum 20. |
| `integrations.customScripts[].src` | string (HTTPS uri) | Oui | URL HTTPS du script. Ne doit pas se terminer par `.svg`, `.exe` ou `.zip`. |
