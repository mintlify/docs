...
| `integrations.customScripts` | array | Non | Liste de scripts externes à charger sur chaque page. Maximum 20 entrées. |
| `integrations.customScripts[].src` | string (URL HTTPS) | Oui | URL du script. Doit être une URL HTTPS. |
| `integrations.customScripts[].strategy` | string | Non | Stratégie de chargement. `beforeInteractive`, `afterInteractive` ou `lazyOnload`. Utilisez `beforeInteractive` pour injecter le script dans le `<head>` avant que la page ne devienne interactive. |
