...
Disposition de répertoire pour les pages racines de groupe. Lorsque défini sur `"accordion"` ou `"card"`, les groupes avec une page `root` affichent une liste de répertoire de leurs pages enfants. Définissez sur `"none"` pour désactiver. S'hérite récursivement à travers la hiérarchie de navigation ; les descendants peuvent remplacer cette valeur.
