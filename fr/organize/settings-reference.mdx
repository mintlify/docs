...
| `integrations.customScripts` | array of object | Non | Fichiers JavaScript externes à charger sur chaque page. Max 20. |
| `integrations.customScripts[].src` | string (URL HTTPS) | Oui | URL du script. Doit être HTTPS. Les extensions `.svg`, `.exe`, `.zip` sont bloquées. |
| `integrations.customScripts[].strategy` | `"afterInteractive"` \| `"lazyOnload"` | Non | Moment de chargement du script. Défaut : `afterInteractive`. |
