...
Utilisez `"download-spec"` pour ajouter une option Télécharger qui récupère la spécification OpenAPI du déploiement. Si plusieurs spécifications sont configurées, elles sont regroupées dans `api-specs.zip` dans le navigateur.
