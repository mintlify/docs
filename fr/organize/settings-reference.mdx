...
Votre fichier `docs.json` se trouve à la racine de votre dépôt de documentation. Consultez [Paramètres globaux](/fr/organize/settings) pour les bases de la configuration.

<Tip>
  Ajoutez `"$schema": "https://mintlify.com/docs.json"` en haut de votre `docs.json` pour bénéficier de l'autocomplétion et de la validation dans des éditeurs comme VS Code. Pour valider `docs.json` localement, exécutez [`mint dev`](/fr/cli/commands#mint-dev) ou [`mint validate`](/fr/cli/commands#mint-validate) depuis la racine de votre projet — les erreurs de schéma apparaissent au démarrage.
</Tip>
