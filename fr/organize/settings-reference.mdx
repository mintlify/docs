...
Cette page suit le dernier schéma `docs.json` publié. Référencez-le dans votre fichier avec `"$schema": "https://mintlify.com/docs.json"` pour bénéficier de la validation et de l'autocomplétion dans votre éditeur.
...
| `markdown.schema` | boolean | Non | `true` |
| `markdown.instructions` | string ou tableau de chaînes | Non | Aucun |
| `seo.indexing` | `"navigable"` \| `"all"` | Non | `"navigable"` |
| `seo.metatags` | object | Non | Aucun |
| `seo.organization` | object | Non | Aucun |
...
Voir [Diviser la configuration avec `$ref`](/fr/organize/settings#split-configuration-with-%24ref) pour plus d'exemples.
...
Sélecteur de produits. Chaque entrée requiert un champ `product` et peut contenir des groups, pages, un tableau `menu` (même structure que `navigation.tabs[].menu`, pour des menus déroulants de produit multi-colonnes), des icônes ou des liens externes.

**Type :** array of object—chacun avec : `product` (string, requis), `description` (string), `icon` (string), `iconType` (string), `href` (string uri), `groups` (array), `pages` (array), `menu` (array)
...
| `icon` | string | Non | Nom d'icône, emoji, URL, chemin ou SVG. |
...
<Note>
  La substitution de variables ne s'applique qu'aux pages MDX et aux snippets. Elle ne s'applique pas aux fichiers de spécification OpenAPI (`.yaml`, `.yml`, `.json`) ni aux autres ressources statiques.
</Note>
...
Affiche une date de dernière modification sur toutes les pages. Pour les projets adossés à GitHub ou GitLab, la date reflète le dernier commit git ayant modifié le fichier source de chaque page, et utilise par défaut l’horodatage du dernier déploiement lorsqu’aucune date de commit git n’est disponible.
...
**Type :** string | array of string or object | object avec `source` (string), `directory` (string) et `overlays` (array of string)
...
Paramètres pour le Markdown que Mintlify sert aux outils et agents d'IA. Voir [Export Markdown](/fr/ai/markdown-export).

**Type :** object

#### `markdown.schema`

Indique s'il faut inclure la spécification OpenAPI ou AsyncAPI complète dans l'export Markdown des pages de référence d'API.
...
Instructions personnalisées pour les agents ajoutées au Markdown généré de chaque page, ainsi qu'à vos fichiers `llms.txt` et `llms-full.txt`. Fournissez une seule chaîne ou un tableau de chaînes, que Mintlify assemble avec des sauts de ligne. Voir [Instructions personnalisées pour les agents](/fr/ai/markdown-export#custom-agent-instructions).

**Type :** string ou tableau de chaînes
...
Organisation utilisée comme entité éditrice dans les données structurées (JSON-LD) de chaque page. Accepte `id`, `name`, `legalName`, `url`, `logo` et `sameAs`. Voir [SEO et recherche](/fr/organize/settings-seo#seo).
