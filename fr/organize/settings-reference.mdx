...
**Type :** array of object—chacun avec : `tab` (string, requis), `icon` (string), `iconType` (string), `hidden` (boolean), `searchable` (boolean), `href` (string uri, requis)

Définissez `searchable: true` avec `hidden: true` pour conserver l’accessibilité des pages descendantes via la recherche, `sitemap.xml`, l’assistant IA et `llms.txt`, tout en gardant l’onglet masqué dans la navigation rendue. Voir [Pages masquées](/fr/organize/hidden-pages#include-pages-under-specific-hidden-tabs-or-groups).
...
Groupes pour organiser le contenu en sections étiquetées. Chaque groupe peut inclure `group` (string), `hidden` (boolean), `searchable` (boolean), `pages` (array) et d’autres propriétés de navigation.

**Type :** array of object

Définissez `searchable: true` avec `hidden: true` pour conserver l’accessibilité des pages descendantes via la recherche, `sitemap.xml`, l’assistant IA et `llms.txt`, tout en gardant le groupe masqué dans la navigation rendue. Voir [Pages masquées](/fr/organize/hidden-pages#include-pages-under-specific-hidden-tabs-or-groups).
