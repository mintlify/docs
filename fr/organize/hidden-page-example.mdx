...
description: "Explorez les cas d’usage des pages cachées : documentation bêta, fichiers de contexte IA, archives héritées et références d’outils internes."
---

Ceci est une page cachée. Elle n’apparaît pas dans la navigation de `docs.json`, vous ne pouvez donc y accéder que via son URL.
...
* **Contexte pour les outils d’IA** : Si vous indexez les pages cachées, les outils d’IA peuvent s’y référer comme contexte. Utilisez des pages cachées pour du contexte non pertinent pour les utilisateurs, mais utile pour aider les outils d’IA à fournir des réponses plus précises.
...
<Warning>
  Il s'agit d'un point de terminaison interne. Ne l'utilisez jamais en production.
</Warning>
````
