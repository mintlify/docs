...
Pour toutes les méthodes de personnalisation, la déconnexion est gérée côté client. Lorsque les utilisateurs cliquent sur le bouton de déconnexion, Mintlify efface leurs données de session stockées dans le navigateur. Vous n'avez pas besoin d'implémenter un point de terminaison de déconnexion pour la personnalisation.

Si vous devez limiter la durée de conservation des données de personnalisation par Mintlify, définissez le champ `expiresAt` dans la réponse contenant les informations utilisateur. Lorsque l'heure actuelle dépasse `expiresAt`, les données stockées de l'utilisateur sont automatiquement supprimées et il doit s'authentifier à nouveau pour accéder au contenu personnalisé. Cela ne révoque pas les sessions ni les jetons auprès de votre fournisseur d'identité. Gérez la révocation côté serveur dans votre propre système, si nécessaire.
...
<Tabs>
  ...
  <Tab title="OAuth 2.0">
    ...
    **Créez un endpoint d’informations utilisateur** à `api.foo.com/docs/user-info`, qui requiert un jeton d’accès OAuth avec le scope `provider.users.docs` et renvoie les données personnalisées de l’utilisateur :
    ...
    <Note>
      Contrôlez la durée de la session avec le champ `expiresAt` dans votre réponse contenant les informations utilisateur. Il s’agit d’un timestamp Unix — nombre de secondes depuis l’époque (epoch) — indiquant quand la session doit expirer. Lorsque la session expire, les données stockées de l&#39;utilisateur sont supprimées et il doit se reconnecter.
    </Note>
    ...
    * Votre documentation est hébergée sur le même domaine ou sous-domaine que votre Dashboard.
    ...
    <Steps>
      <Step title="Create user info API endpoint.">
        ...
        * Répond aux requêtes `GET`.
        * Utilise vos cookies de session existants pour identifier les utilisateurs. Mintlify appelle cet endpoint avec les informations d’authentification incluses, de sorte que vos cookies de session sont envoyés automatiquement. Aucun en-tête `Authorization` n’est utilisé.
