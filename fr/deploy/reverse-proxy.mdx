...
Ne réécrivez pas uniquement `/docs/llms.txt` vers un `/llms.txt` hébergé à la racine. Définissez `/docs` comme chemin de base du projet et transférez la route `/docs/*` complète. Cela maintient les liens de pages et les liens d'index `/docs/_llms/*` générés sur le même préfixe public.
