...
- **Héberger sur `/docs`** : utilisez `mintlify.dev` comme cible du proxy. Activez l’option **Héberger sur `/docs`** sur la page [Configuration du domaine personnalisé](https://dashboard.mintlify.com/settings/deployment/custom-domain) de votre Dashboard. Il s’agit d’une configuration plus simple avec moins de routes.
...
1. Accédez à la page [Configuration du domain personnalisé](https://dashboard.mintlify.com/settings/deployment/custom-domain) dans votre Dashboard.
2. Activez l’option **Héberger sur `/docs`**.
3. Saisissez votre domain et sélectionnez **Ajouter un domain**.

<Warning>
  L’activation de **Héberger sur `/docs`** désactive l’invalidation du cache sur `mintlify.app`. Vous devez configurer un proxy vers `mintlify.dev` pour que les mises à jour s’affichent.
</Warning>
...
Redirigez (via proxy) ces chemins vers votre sous-domaine Mintlify :

| Chemin                            | Destination                          | Cache       |
| --------------------------------- | ------------------------------------ | ----------- |
...
Les routes `/.well-known/skills/*` et `/skill.md` sont optionnelles. Incluez-les uniquement si vous voulez publier des fichiers de compétences d’IA à la racine, par exemple à des chemins comme `your-domain.com/skills.md`, plutôt que sous votre sous-chemin de documentation comme `your-domain.com/docs/skills.md`.
...
**Cause** : Vous avez **Host at `/docs`** activé dans votre Dashboard, mais votre proxy inverse pointe vers `mintlify.app` au lieu de `mintlify.dev`.
