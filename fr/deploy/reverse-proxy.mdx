...
Mintlify prend en charge deux configurations de proxy inverse en fonction de vos exigences de sous-chemin.

- **Héberger sur `/docs`** : utilisez `mintlify.dev` comme cible du proxy. Activez le paramètre **Héberger sur `/docs`** sur la page [Configuration du domaine personnalisé](https://dashboard.mintlify.com/settings/deployment/custom-domain) de votre Dashboard. Il s'agit d'une configuration plus simple avec moins de routes.
...
Utilisez cette configuration lorsque vous voulez servir la documentation sur le chemin `/docs` de votre domaine.

Avant de configurer votre proxy inverse :

1. Accédez à [Configuration du domaine personnalisé](https://dashboard.mintlify.com/settings/deployment/custom-domain) dans votre Dashboard.
2. Activez l’option **Host at `/docs`**.
3. Saisissez votre domaine et sélectionnez **Add domain**.

<Warning>
  L’activation de **Host at `/docs`** interrompt l’invalidation du cache sur `mintlify.app`. Vous devez configurer un proxy vers `mintlify.dev` pour que les mises à jour apparaissent.
</Warning>
...
Configurez un proxy pour ces chemins vers votre sous-domaine Mintlify :
...
| `/.well-known/skills/*` (optionnel)| `<your-subdomain>.mintlify.dev/docs` | Sans cache |
| `/skill.md` (optionnel)           | `<your-subdomain>.mintlify.dev/docs` | Sans cache |

Les routes `/.well-known/skills/*` et `/skill.md` sont facultatives. Ne les incluez que si vous souhaitez servir les fichiers de compétences IA à des chemins racine comme `your-domain.com/skills.md` plutôt que sous votre sous-chemin de documentation, comme `your-domain.com/docs/skills.md`.
...
**Symptômes** : Vous publiez des mises à jour de la documentation, mais les modifications ne s'affichent pas sur votre site.
