...
Mintlify prend en charge deux configurations de proxy inverse en fonction de vos exigences en matière de sous-chemin. Les deux ciblent votre hostname `<your-subdomain>.mintlify.site`.
...
- **Sous-chemin personnalisé** : utilisez-le lorsque vous avez besoin d’un sous-chemin autre que `/docs` (par exemple `/help` ou `/resources`). Cette approche nécessite des règles de routage supplémentaires.
...
<Warning>
  Lorsque vous activez **Host at `/docs`**, l’URL canonique de votre documentation devient `<your-subdomain>.mintlify.site`. Configurez le proxy vers ce hostname pour que les invalidations de cache et les mises à jour atteignent votre site.
</Warning>
...
**Cause** : Votre proxy inverse pointe vers un hostname autre que votre sous-domaine Mintlify canonique, de sorte que les invalidations de cache n’atteignent pas votre site.
