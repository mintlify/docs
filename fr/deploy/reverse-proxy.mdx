...
Mintlify prend en charge deux configurations de proxy inverse en fonction de vos besoins en matière de sous-chemin.

- **Héberger sur `/docs`** : utilisez `mintlify.dev` comme cible du proxy. Activez l’option **Héberger sur `/docs`** sur la page [Configuration de domaine personnalisé](https://dashboard.mintlify.com/settings/deployment/custom-domain) de votre Dashboard. Il s’agit d’une configuration plus simple avec moins de routes à gérer.
...
<Warning>
  L’activation de **Host at `/docs`** interrompt l’invalidation du cache sur `mintlify.app`. Vous devez configurer votre proxy vers `mintlify.dev` pour que les mises à jour apparaissent.
</Warning>
...
Configurez un proxy de ces chemins vers votre sous-domaine Mintlify :
...
Les routes `/.well-known/skills/*` et `/skill.md` sont facultatives. Incluez-les uniquement si vous souhaitez servir des fichiers de compétences d’IA sur des chemins à la racine comme `your-domain.com/skills.md` plutôt que dans votre sous-chemin de documentation comme `your-domain.com/docs/skills.md`.
...
**Cause** : Vous avez activé **Host at `/docs`** dans votre Dashboard, mais votre reverse proxy pointe vers `mintlify.app` au lieu de `mintlify.dev`.
