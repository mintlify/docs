...
- **Héberger à `/docs`** : utilisez `mintlify.dev` comme cible du proxy. Activez l’option **Héberger à `/docs`** sur la page [Configuration du domain personnalisé](https://dashboard.mintlify.com/settings/deployment/custom-domain) de votre Dashboard. Il s'agit d'une configuration plus simple avec moins de routes.
...
Utilisez cette configuration lorsque vous souhaitez servir la documentation à l’emplacement `/docs` sur votre domaine.
...
<Warning>
  L’activation de **Host at `/docs`** désactive l’invalidation du cache sur `mintlify.app`. Vous devez configurer votre proxy vers `mintlify.dev` pour que les mises à jour soient prises en compte.
</Warning>
...
| `/.well-known/skills/*` (optional)| `<your-subdomain>.mintlify.dev/docs` | Aucune mise en cache |
| `/skill.md` (optional)            | `<your-subdomain>.mintlify.dev/docs` | Aucune mise en cache |

Les routes `/.well-known/skills/*` et `/skill.md` sont facultatives. Incluez-les uniquement si vous souhaitez servir des fichiers de compétences IA à des chemins racine comme `your-domain.com/skills.md` plutôt que sous votre sous-chemin de documentation, par exemple `your-domain.com/docs/skills.md`.
...
Lorsque vous avez besoin d’un sous-chemin autre que `/docs` (tel que `/help` ou `/resources`), utilisez la configuration de routage suivante.
