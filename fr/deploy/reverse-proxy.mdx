...
- **Héberger sur `/docs`** : utilisez `mintlify.dev` comme cible du proxy. Activez l’option **Héberger sur `/docs`** sur la page [Custom domain setup](https://dashboard.mintlify.com/settings/deployment/custom-domain) de votre Dashboard. Il s’agit d’une configuration plus simple avec moins de routes.
...
<div id="host-at-docs-subpath">
  ## Héberger au sous-chemin `/docs`
</div>

Utilisez cette configuration lorsque vous souhaitez servir la documentation au chemin `/docs` sur votre domain.
...
2. Activez le toggle **Host at `/docs`**.
3. Saisissez votre domain et sélectionnez **Add domain**.

<Warning>
  L’activation de **Host at `/docs`** désactive l’invalidation du cache sur `mintlify.app`. Vous devez effectuer un proxy vers `mintlify.dev` pour que les mises à jour apparaissent.
</Warning>
...
| `/.well-known/skills/*` (facultatif)| `<your-subdomain>.mintlify.dev/docs` | Aucun cache  |
| `/skill.md` (facultatif)          | `<your-subdomain>.mintlify.dev/docs` | Aucun cache  |

Les routes `/.well-known/skills/*` et `/skill.md` sont facultatives. Incluez-les uniquement si vous souhaitez servir les fichiers de skills d’IA à des chemins à la racine comme `your-domain.com/skills.md` au lieu de dans un sous-chemin de vos docs comme `your-domain.com/docs/skills.md`.
...
**Cause** : L’option **Host at `/docs`** est activée dans votre Dashboard, mais votre reverse proxy pointe vers `mintlify.app` au lieu de `mintlify.dev`.
