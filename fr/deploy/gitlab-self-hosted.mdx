...
description: "Connectez une instance GitLab auto-hébergée à Mintlify via OAuth pour cloner des dépôts, pousser des commits et ouvrir des merge requests automatisés."
...
<Note>
  Ce guide concerne uniquement l'intégration **OAuth** qui alimente les automatisations. Vous devez configurer la connexion côté déploiement (utilisée pour la synchronisation du contenu et les prévisualisations) séparément avec un jeton d’accès, voir le [guide GitLab](/fr/deploy/gitlab). L'intégration OAuth dépend de la connexion côté déploiement.
</Note>
...
- Une organisation Mintlify avec un **plan Pro ou supérieur**.
- Une organisation Mintlify pour laquelle la fonctionnalité GitLab auto-hébergé est activée. Contactez le support si vous ne voyez pas la section **Self-hosted GitLab** dans la page de votre tableau de bord [Git settings](https://app.mintlify.com/settings/project/git-settings).

## Prérequis réseau

L'intégration nécessite du trafic dans les deux directions entre votre instance GitLab et Mintlify.

### Entrant : de Mintlify vers votre instance GitLab

Mintlify se connecte à votre instance GitLab via HTTPS (port 443) pour échanger des tokens OAuth et appeler l'API GitLab. Ces connexions proviennent de l'adresse IP statique d'egress de Mintlify : `54.242.90.151`.

Si un pare-feu ou une liste d'adresses IP autorisées restreint l'accès à votre instance GitLab, autorisez le trafic entrant HTTPS (port 443) depuis `54.242.90.151`.

<Note>
  Les exécutions d'automatisation clonent et poussent depuis une infrastructure de sandbox distincte qui utilise par défaut des adresses IP dynamiques. Si votre instance GitLab applique une liste d'adresses IP autorisées, voir [Résoudre les exécutions d'agent bloquées par une liste d'adresses IP autorisées](/fr/help-center/agent-blocked-by-git-provider-ip-allow-list).
</Note>

### Sortant : de votre instance GitLab vers Mintlify

Votre instance GitLab délivre les événements de webhook à Mintlify via HTTPS (port 443) à `app.mintlify.com`. Autorisez ce trafic par nom d'hôte. Mintlify ne publie pas de liste d'adresses IP statiques pour ce point de terminaison.
...
<Steps>
  ...
  <Step title="Authorize">
    Cliquez sur **Authorize self-hosted GitLab**. GitLab vous redirige vers votre instance, vous invite à vous connecter si nécessaire, et affiche un écran de consentement listant les portées demandées.
...
<div id="invalid_client-after-authorizing">
  ### L'autorisation échoue avec `invalid_client`
</div>
...
GitLab a refusé d'enregistrer le webhook car sa liste d'autorisation des requêtes sortantes a rejeté l'URL envoyée par Mintlify (`https://app.mintlify.com/gitlab-oauth-webhook`). Les instances auto-hébergées rejettent les URL « locales » sauf si l'administrateur les autorise explicitement.
...
- L'application porte le drapeau **Trusted** sur GitLab. Les applications approuvées sautent le consentement pour tous les utilisateurs. Décochez **Trusted** dans les paramètres de l'application si vous souhaitez que les utilisateurs voient et confirment les portées.
