...
Si vous utilisez une instance GitLab auto-hébergée, vous pouvez la connecter à Mintlify en fournissant l'URL de votre instance.

<Steps>
  <Step title="Activer la configuration auto-hébergée">
    Dans le [Tableau de bord Mintlify](https://dashboard.mintlify.com/settings/deployment/git-settings), activez l'option **Set up as self-hosted** dans le panneau de configuration GitLab.
  </Step>
  <Step title="Saisir l'URL de votre hôte">
    Saisissez l'URL de votre instance GitLab auto-hébergée dans le champ **Host URL** (par exemple, `https://gitlab.example.com`).
  </Step>
  <Step title="Terminer la configuration">
    Renseignez votre ID de projet, votre jeton d'accès et votre branche de déploiement comme décrit dans la [configuration de la connexion ci-dessus](#set-up-the-connection), puis cliquez sur **Enregistrer les modifications**.
  </Step>
</Steps>

<Note>
  Les étapes de configuration du jeton d'accès et du webhook sont identiques pour les instances GitLab auto-hébergées. Assurez-vous que votre instance est accessible depuis Internet pour que Mintlify puisse s'y connecter.
</Note>
