...
description: "Configurez les en-têtes CSP pour autoriser les ressources Mintlify tout en préservant la sécurité des serveurs mandataires inverses, pare-feu et réseaux appliquant des politiques de sécurité strictes."
---

La Content Security Policy (CSP) est une norme de sécurité qui aide à prévenir les attaques de cross-site scripting (XSS) en contrôlant les ressources qu’une page web est autorisée à charger. Mintlify applique par défaut une CSP qui protège la plupart des sites. Si vous hébergez votre documentation derrière un serveur mandataire inverse (reverse proxy) ou un pare-feu qui remplace cette CSP par défaut, vous devrez peut-être configurer les en-têtes CSP pour que les fonctionnalités fonctionnent correctement.
