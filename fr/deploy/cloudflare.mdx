...
<div id="firewall-troubleshooting">
...
- La page de documentation se charge initialement mais plante avec une erreur 500 après 30 à 60 secondes.
...
- Messages de défi de sécurité Cloudflare mentionnant "malformed data" ou "suspicious URL patterns".

<div id="root-cause">
  ### Cause principale
</div>

Le Web Application Firewall (WAF) de Cloudflare et le Bot Fight Mode peuvent signaler les requêtes de ressources Mintlify comme suspectes pour les raisons suivantes :

- Plusieurs symboles `%` dans les paramètres d'URL encodés.
- Longues chaînes de requête contenant des caractères spéciaux.
- Requêtes automatisées provenant d'onglets inactifs.

<div id="create-a-firewall-exception">
  ### Créer une exception de pare-feu
</div>

Créez une règle de pare-feu Cloudflare pour exempter les ressources Mintlify des contrôles de sécurité.
...
5. Configurez la règle avec ces paramètres :
    ...
    **Nom de la règle :** Allow Mintlify assets

    **Lorsque les requêtes entrantes correspondent à :**
    - Champ : `Hostname`
    - Opérateur : `equals`
    - Valeur : `docs.yourdomain.com` (remplacez par votre véritable domaine de documentation)
...
- Assurez-vous que le hostname correspond exactement à votre domaine de documentation.
...
- Utiliser l'opérateur `contains` avec `/mintlify-assets/*`. Le `*` est traité comme un caractère littéral, pas comme un caractère générique.
- Utiliser `equals` pour URI Path. Cela ne correspond qu'au chemin exact `/mintlify-assets/` et pas aux sous-chemins.
- Oublier d'ignorer le Bot Fight Mode. Incluez-le explicitement dans l'action skip.
- Définir un hostname incorrect. Il doit correspondre au domaine réel de votre documentation.
...
3. Réglez temporairement le Security Level sur "Essentially Off" pour confirmer que Cloudflare est en cause.
4. Examinez les Page Rules personnalisées qui pourraient remplacer l'exception de pare-feu.
