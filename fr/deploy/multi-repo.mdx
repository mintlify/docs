...
Ajoutez une entrée `sourceRef` à un tableau de navigation dans le `docs.json` de la source de base. La valeur doit être l'identifiant `owner/repo` d'une autre source de référentiel du déploiement.

```json
{ "sourceRef": "acme/api-docs" }
```

La comparaison est insensible à la casse et les barres obliques de tête ou de fin sont ignorées. La valeur doit comporter exactement deux segments (`owner/repo`) ; les autres formats font échouer la fusion.
...
Dans cet exemple, la source de base (détenue par `acme` sur le référentiel `docs`) est diffusée à `/docs` et affiche un onglet `Guides`. Le `sourceRef` intègre les `tabs` d'une seconde source (`acme/api-docs`) configurée à `/api`.
...
- Les valeurs de `sourceRef` doivent utiliser le format `owner/repo` et correspondre à une source de référentiel configurée dans le même déploiement.
...
- Deux sources de référentiel d'un même déploiement ne peuvent pas partager le même `owner/repo`. Si c'est le cas, `sourceRef` ne peut pas être résolu sans ambiguïté et la fusion échoue.
...
Les entrées `sourceRef` sont résolues lors d'un déploiement multi-référentiels, lorsque Mintlify a accès à toutes les sources. L'aperçu local avec `mint dev` ne voit qu'un seul référentiel et ne peut donc pas intégrer la navigation référencée.

Lorsque vous exécutez `mint dev` dans un référentiel dont le `docs.json` contient des entrées `sourceRef`, Mintlify supprime ces entrées de l'aperçu et affiche un avertissement. Le reste de la navigation s'affiche normalement, mais les sections provenant d'une autre source sont absentes jusqu'au déploiement.
