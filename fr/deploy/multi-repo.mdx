...
Mintlify prend en charge `sourceRef` dans `anchors`, `tabs`, `groups`, `pages`, `products` et les tableaux sous `navigation.global`. Les autres structures de navigation, y compris les entrées `menu` à l'intérieur d'un onglet, n'acceptent pas `sourceRef`.
