...
<Warning>
  Ne placez pas `{/* vale off */}` ou d'autres commentaires d'expression MDX similaires en tant qu'enfants directs d'un composant JSX entre des éléments frères. Par exemple, entre deux éléments `<Step>` à l'intérieur d'un composant `<Steps>`.
