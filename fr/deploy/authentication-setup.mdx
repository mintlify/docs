...
<Tabs>
    ...
    * Vos exigences de sécurité autorisent le partage de mots de passe entre les utilisateurs.
    ...
    <Steps>
      <Step title="Créer un mot de passe.">
        1. Dans votre Dashboard, accédez à [Access](https://app.mintlify.com/settings/project/authentication).
        2. Définissez **Visibility** sur **Private**.
        3. Définissez **Method** sur **Password**.
  ...
  <Tab title="Authentification privée">
    ...
    * Toutes les personnes qui doivent accéder à votre site doivent être membres de votre organisation Mintlify.
    ...
    <Steps>
      <Step title="Activer l’authentification privée.">
        1. Dans votre Dashboard, accédez à [Access](https://app.mintlify.com/settings/project/authentication).
        2. Définissez **Visibility** sur **Private**.
        3. Définissez **Method** sur **Authenticated**.
        4. Cliquez sur **Save**.
    ...
    **Activez l’authentification privée** dans les paramètres de votre Tableau de bord Mintlify.
  ...
  <Tab title="OAuth 2.0">
    ...
    * Capacité à créer un endpoint d'API accessible via des jetons d'accès OAuth (facultatif, pour activer le contrôle d'accès basé sur les groupes).
    ...
    <Steps>
      <Step title="Configurez vos paramètres OAuth.">
        1. Dans votre Dashboard, allez dans [Access](https://app.mintlify.com/settings/project/authentication).
        2. Définissez **Visibility** sur **Private**.
        3. Définissez **Method** sur **Custom**.
        4. Cliquez sur **OAuth**.
        ...
        * **Info API URL** (facultatif) : Endpoint sur votre serveur que Mintlify appelle pour récupérer les informations utilisateur. Utilisez ce champ pour l’approche Info API du contrôle d’accès basé sur les groupes. Vous pouvez également utiliser les claims des jetons OAuth. Si aucune de ces options n’est configurée, le flux OAuth vérifie uniquement l’identité.
      ...
      <Step title="Configurez votre serveur OAuth.">
        1. Copiez la **Redirect URL** à partir de vos [paramètres Access](https://app.mintlify.com/settings/project/authentication).
        2. Ajoutez cette URL de redirection comme URL de redirection autorisée pour votre serveur OAuth.
      </Step>

      <Step title="Créez votre endpoint d’informations utilisateur pour l’accès basé sur les groupes (facultatif).">
        Pour utiliser l’approche Info API du contrôle d’accès basé sur les groupes, créez un endpoint d’API qui :
        ...
        * Renvoie les données utilisateur au format `User`. Voir [User data format](#user-data-format) pour plus d’informations.
        ...
        Ajoutez l’URL de cet endpoint dans le champ **Info API URL** de vos [paramètres Access](https://app.mintlify.com/settings/project/authentication).
      </Step>
    </Steps>
    ...
    Si votre fournisseur d’identité inclut l’appartenance aux groupes dans l’ID token ou l’access token, vous pouvez utiliser ces claims à la place d’une URL Info API. Cette option est disponible pour les configurations OAuth qui utilisent un secret client.

    Lors de la configuration des claims des jetons OAuth pour votre projet, utilisez des valeurs telles que :
    ...
    * `groupsDelimiter` : Délimiteur facultatif de 1 à 4 caractères. Mintlify l’utilise uniquement pour diviser les valeurs de claim qui sont des chaînes.
    ...
    Mintlify supprime les espaces autour de chaque groupe et ignore les segments vides. Sans `groupsDelimiter`, la chaîne complète est traitée comme un seul groupe. Chaque élément de type chaîne d’un claim sous forme de tableau est traité comme un groupe distinct et n’est pas divisé.

    Omettez `groupsDelimiter` lorsque le délimiteur peut faire partie du nom d’un groupe.
    ...
    * Un service backend capable de créer des URL de redirection.
    ...
    <Steps>
      <Step title="Générez une clé privée.">
        ...
        6. Pour proposer plusieurs flux de connexion, cliquez sur **Add login URL** et saisissez un nom d’affichage et une URL pour chaque option. Vous pouvez configurer jusqu’à 10 URL de connexion.
        7. Cliquez sur **Save**.
...
<div id="exclude-groups">
  ### Exclure des groupes
</div>

Pour bloquer des groupes spécifiques sur une page, utilisez des objets de règle avec `includes` et `excludes` au lieu d’une liste de noms de groupes.

* `includes` : les utilisateurs doivent appartenir à au moins un de ces groupes. Omettez `includes` pour autoriser tous les utilisateurs authentifiés qui ne sont pas exclus.
* `excludes` : les utilisateurs appartenant à l’un de ces groupes ne peuvent pas accéder à la page, même s’ils appartiennent aussi à un groupe inclus.

Mintlify vérifie `excludes` avant `includes`. Les utilisateurs exclus reçoivent une erreur 404.

```mdx Example page for staff, excluding contractors highlight={3}
---
title: "Feuille de route interne"
groups: [{ includes: ["staff"], excludes: ["contractor"] }]
---
```

Dans cet exemple, un utilisateur appartenant à la fois aux groupes `staff` et `contractor` ne peut pas accéder à la page.

Pour masquer une page à un groupe tout en la laissant ouverte à tous les autres utilisateurs authentifiés, omettez `includes`.

```mdx Example page hidden from contractors highlight={3}
---
title: "Guide de rémunération"
groups: [{ excludes: ["contractor"] }]
---
```

Une valeur `groups` est soit une liste de noms de groupes, soit une liste d’objets de règle. Vous ne pouvez pas mélanger les deux formes dans une même liste. Le caractère générique `*` n’est pas autorisé dans `excludes`. Une liste `includes` vide n’est pas valide. Pour refuser l’accès à tout le monde, définissez `groups: []`.

Si une page contient des objets de règle non valides, Mintlify ne met pas à jour votre déploiement et affiche `Page groups rules must be { includes, excludes } objects and * is not allowed in excludes. Deployment not updated`. L’éditeur web refuse également d’enregistrer une page contenant des objets de règle non valides.
...
* Les pages avec `public: true` sont accessibles à tout le monde. Mintlify ignore les noms de groupes et `includes` sur les pages publiques.
* Les pages avec `public: true` et `excludes` sont accessibles à tout le monde, sauf aux utilisateurs connectés appartenant à un groupe exclu. Les visiteurs anonymes peuvent toujours consulter la page. Les utilisateurs connectés exclus reçoivent une erreur 404. Si la session d’un utilisateur connecté a expiré et ne peut pas être renouvelée, Mintlify le redirige vers la connexion.
...
Lorsque vous utilisez l’authentification OAuth ou JWT, ou la personnalisation autonome, votre système renvoie des données utilisateur qui contrôlent la durée de la session et l’appartenance à des groupes, ainsi que la [personnalisation du contenu](/fr/create/personalization).
...
<ParamField path="groups" type="string[]">
  Liste des groupes auxquels l’utilisateur appartient. Avec l’authentification, les pages dont le frontmatter contient un `groups` correspondant sont accessibles à cet utilisateur. Avec la personnalisation autonome, les groupes contrôlent la visibilité des pages et du contenu, mais ne restreignent pas l’accès à l’URL directe d’une page.
...
| [Aperçu en direct](/fr/editor/review#live-preview)                 | Prise en charge complète | Pris en charge avec authentification automatique de l’éditeur                                        | Pris en charge avec authentification automatique de l’éditeur                                               |
