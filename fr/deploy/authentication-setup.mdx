...
Le sélecteur de visibilité du site dans le tableau de bord ne propose que **Public** ou **Private**. L'authentification partielle n'est pas un mode distinct. Pour la configurer, passez la visibilité du site sur **Private** afin que toutes les pages nécessitent une authentification par défaut. Marquez ensuite les pages ou groupes de navigation que vous souhaitez laisser ouverts à tout le monde avec `public: true`. Consultez [Rendre des pages publiques](#make-pages-public) pour la syntaxe du frontmatter et de `docs.json`.
