...
description: "Configurez l'authentification utilisateur pour votre site afin de contrôler l'accès aux pages et aux références d'API. Rendez les pages privées pour votre organisation Mintlify, ou utilisez l'authentification par mot de passe, OAuth ou JWT."
...
Avant de partager l'URL avec les utilisateurs, vérifiez que l'authentification fonctionne comme prévu :

- **Déploiements de prévisualisation** : Les [déploiements de prévisualisation](/fr/deploy/preview-deployments) respectent les mêmes paramètres d'authentification que votre site de production. Poussez une branche et ouvrez l'URL de prévisualisation pour confirmer que le flux de connexion fonctionne de bout en bout.
- **Tester avec un compte réel** : Connectez-vous avec un utilisateur de test ou un membre de chaque groupe qui devrait avoir accès, et vérifiez que les pages restreintes apparaissent ou sont masquées comme prévu.
- **Développement local** : `mint dev` sert votre site sans appliquer l'authentification pour vous permettre d'itérer sur le contenu. Vérifiez toujours les règles d'accès sur un déploiement de prévisualisation ou de production, et non sur le serveur local.
