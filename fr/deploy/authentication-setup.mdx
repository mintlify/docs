...
<Tabs>
  ...
  <Tab title="OAuth 2.0">
    ...
    * Capacité à créer un endpoint d'API accessible via des jetons d'accès OAuth (facultatif, pour activer le contrôle d'accès basé sur les groupes).
    ...
    Lorsque des claims de jetons sont configurés, Mintlify lit les groupes directement à partir du jeton renvoyé par votre échange de jeton et n’appelle pas l’URL Info API, même si une URL est définie. Pour configurer des claims de jetons pour votre déploiement, contactez le support Mintlify.

    Utilisez des valeurs telles que :
