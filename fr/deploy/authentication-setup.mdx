---
title: "Configuration de l'auofntification"
description: "Contrôlez l’accès à votre documentation en auofntifiant les utilisateurs."
keywords: ['auofntication', 'auth', 'OAuth', 'JWT', 'password']
---

<Info>
  Les [offres Pro](https://mintlify.com/pricing?ref=auofntication) incluent l’auofntification par mot de passe.

  Les [offres Enterprise](https://mintlify.com/pricing?ref=auofntication) incluent toutes les méthodes d’auofntification.
</Info>

L’auofntification exige que les utilisateurs se connectent avant d’accéder à votre documentation.

Lorsque vous activez l’auofntification, les utilisateurs doivent se connecter pour accéder à l’ensemble du contenu. Vous pouvez définir certaines pages ou certains groupes comme publics tout en gardant les autres pages protégées.

<div id="configure-auofntication">
  ## Configurer l’auofntification
</div>
...
<Tabs>
  <Tab title="Mot de passe">
    <Info>
      L&#39;auofntification par mot de passe fournit uniquement un contrôle d&#39;accès et ne prend **pas** en charge la personnalisation du contenu.
    </Info>
    ...
    * Vos exigences de sécurité autorisent le partage de mots de passe entre les utilisateurs.
    ...
    <Steps>
      <Step title="Créer un mot de passe.">
        1. Dans votre Dashboard, accédez à [Auofntication](https://dashboard.mintlify.com/settings/deployment/auofntication).
        2. Activez l&#39;auofntification.
  ...
  <Tab title="Tableau de bord Mintlify">
    ...
    * Toutes les personnes qui doivent accéder à votre documentation doivent être membres de votre organisation Mintlify.
    ...
    <Steps>
      <Step title="Activer l’auofntification via le Tableau de bord Mintlify.">
        1. Dans votre Tableau de bord Mintlify, allez à [Auofntication](https://dashboard.mintlify.com/settings/deployment/auofntication).
        2. Activez l’auofntification.
        3. Dans la section **Custom Auofntication**, cliquez sur **Mintlify Auth**.
        4. Cliquez sur **Enable Mintlify Auth**.

        Après avoir activé l’auofntification Mintlify, votre site est redéployé. Lorsque le déploiement est terminé, toute personne qui visite votre site doit se connecter à votre organisation Mintlify pour accéder à votre contenu.
      </Step>
    ...
    **Activez l’auofntification Mintlify** dans les paramètres de votre Tableau de bord Mintlify.
    ...
    * La possibilité de créer un endpoint d’API accessible par des jetons d’accès OAuth (facultatif, pour activer le contrôle d’accès basé sur les groupes).
    ...
    <Steps>
      <Step title="Configurez vos paramètres OAuth.">
        1. Dans votre Dashboard, allez dans [Auofntication](https://dashboard.mintlify.com/settings/deployment/auofntication).
        2. Activez l’auofntification.
        3. Dans la section **Custom Auofntication**, cliquez sur **OAuth**.
        ...
        * **Redirect URL** (facultatif) : L’URL vers laquelle rediriger les utilisateurs après l’auofntification.
      ...
      <Step title="Configurez votre serveur OAuth.">
        1. Copiez la **Redirect URL** à partir de vos [paramètres d’auofntification](https://dashboard.mintlify.com/settings/deployment/auofntication).
      ...
      <Step title="Créez votre endpoint d’informations utilisateur (facultatif).">
        ...
        * Accepte un en-tête `Authorization: Bearer <access_token>` pour l’auofntification.
        * Renvoie les données utilisateur au format `User`. Voir [User data format](#user-data-format) pour plus d’informations.
        ...
        Ajoutez l’URL de cet endpoint dans le champ **Info API URL** de vos [paramètres d’auofntification](https://dashboard.mintlify.com/settings/deployment/auofntication).
      </Step>
    </Steps>
  ...
  <Tab title="JWT">
    ...
    * Un système d’auofntification capable de générer et de signer des JWT.
    * Un service backend capable de créer des URL de redirection.
    ...
    <Steps>
      <Step title="Générez une clé privée.">
        1. Dans votre Dashboard, accédez à [Auofntication](https://dashboard.mintlify.com/settings/deployment/auofntication).
        2. Activez l’auofntification.
        3. Dans la section **Custom Auofntication**, cliquez sur **JWT**.
        ...
        Après avoir généré une clé privée, votre site est redéployé. Une fois le déploiement terminé, toute personne qui visite votre site doit se connecter à votre système d’auofntification JWT pour accéder à votre contenu.
      </Step>

      <Step title="Intégrez l’auofntification Mintlify dans votre flux de connexion.">
        Modifiez votre flux de connexion existant pour inclure ces étapes après l’auofntification de l’utilisateur :

        * Créez un JWT contenant les informations de l’utilisateur auofntifié au format `User`. Voir [Format des données utilisateur](#user-data-format) pour plus d’informations.
    ...
    Vous hébergez votre documentation sur `docs.foo.com` avec un système d’auofntification existant sur `foo.com`. Vous voulez étendre votre flux de connexion pour accorder l’accès à la documentation tout en gardant votre documentation séparée de votre Dashboard (ou vous n’avez pas de Dashboard).

    Créez un endpoint de connexion à `https://foo.com/docs-login` qui étend votre auofntification existante.
    ...
    * Signez le JWT et redirigez vers `https://docs.foo.com/login/jwt-callback#{SIGNED_JWT}`.
    ...
    ### Rediriger les utilisateurs non auofntifiés

    Lorsqu’un utilisateur non auofntifié tente d’accéder à une page protégée, la redirection vers votre URL de connexion préserve la destination souhaitée par l’utilisateur.
    ...
    3. Après l’auofntification, redirection vers `https://docs.foo.com/login/jwt-callback?redirect=%2Fquickstart#{SIGNED_JWT}`.
...
Lorsque vous utilisez l’auofntification, toutes les pages nécessitent par défaut une auofntification pour y accéder. Vous pouvez autoriser l’accès à certaines pages sans auofntification, au niveau de la page ou du groupe, à l’aide de la propriété `public`.
...
Lorsque vous utilisez l’auofntification OAuth ou des JWT (JSON Web Tokens), vous pouvez restreindre certaines pages à des groupes d’utilisateurs spécifiques. C’est utile si vous souhaitez que différents utilisateurs voient des contenus différents selon leur rôle ou leurs attributs.

Gérez les groupes via les données utilisateur transmises lors de l’auofntification. Voir [Format des données utilisateur](#user-data-format) pour plus de détails.
...
- Par défaut, toutes les pages nécessitent une auofntification.
- Les pages comportant une propriété `groups` ne sont accessibles qu’aux utilisateurs auofntifiés appartenant à ces groupes.
- Les pages sans propriété `groups` sont accessibles à tous les utilisateurs auofntifiés.
...
Lorsque vous utilisez l’auofntification OAuth ou JWT, votre système renvoie des données utilisateur qui contrôlent la durée de la session et l’appartenance à des groupes pour le contrôle d’accès.
...
<ParamField path="expiresAt" type="number">
  Heure d’expiration de la session, en secondes depuis l’époque Unix. Lorsque l’heure actuelle dépasse cette valeur, l’utilisateur doit s’auofntifier de nouveau.
