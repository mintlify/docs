...
description: "Configurez l'authentification utilisateur pour votre site avec OAuth, JWT, mot de passe ou des liens partagés afin de contrôler l'accès aux pages et aux références d'API."
keywords: ['authentication', 'auth', 'OAuth', 'JWT', 'password']
---

<Info>
  L'authentification au tableau de bord Mintlify est disponible sur toutes les offres.
...
<Warning>
  L'authentification n'est disponible que pour les sites hébergés sur un domaine personnalisé ou un sous-domaine Mintlify (par exemple, `docs.exemple.com` ou `exemple.mintlify.dev`). L'authentification **n'est pas prise en charge** lors de l'utilisation d'un [chemin de base personnalisé](/fr/deploy/docs-subpath) (par exemple, `exemple.com/docs`).
</Warning>
