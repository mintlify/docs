...
<Warning>
  L'authentification n'est disponible que pour la documentation hébergée sur un domaine personnalisé ou un sous-domaine Mintlify (par exemple, `docs.exemple.com` ou `exemple.mintlify.cc`). L'authentification **n'est pas prise en charge** lors de l'utilisation d'un [chemin de base personnalisé](/fr/deploy/docs-subpath) (par exemple, `exemple.com/docs`).
</Warning>
