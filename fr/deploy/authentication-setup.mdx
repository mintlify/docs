...
<Tabs>
  ...
  <Tab title="JWT">
    ...
    * Un service backend capable de créer des URL de redirection.
    ...
    <div id="offer-multiple-login-urls">
      ### Proposer plusieurs URL de connexion
    </div>

    Si votre organisation dispose de plusieurs fournisseurs d’identité ou de flux de connexion spécifiques à une région, vous pouvez configurer entre 2 et 10 URL de connexion nommées pour votre site authentifié par JWT. Lorsque plusieurs URL sont configurées, les visiteurs non authentifiés voient un écran de sélection de connexion aux couleurs de votre site, et chaque option redirige vers l’URL que vous avez configurée.

    Pour configurer plusieurs URL de connexion, fournissez une liste de paires `{ name, url }` lors de la mise à jour de vos paramètres d’authentification JWT. Chaque nom est affiché aux utilisateurs sur l’écran de sélection et doit contenir de 1 à 100 caractères. Chaque URL doit être une URL valide. Avec une seule URL de connexion, les visiteurs continuent d’être redirigés directement vers cette URL comme auparavant.

    ```json
    {
      "type": "jwt",
      "loginUrl": "https://global.example.com/login",
      "loginUrls": [
        { "name": "Global", "url": "https://global.example.com/login" },
        { "name": "EU", "url": "https://eu.example.com/login" }
      ]
    }
    ```

    Plusieurs URL de connexion s’appliquent uniquement à l’authentification JWT à l’échelle du site. La [personnalisation](/fr/create/personalization) JWT accepte une seule `loginUrl`.
