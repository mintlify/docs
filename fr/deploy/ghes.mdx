...
L'intégration nécessite du trafic dans les deux sens entre votre instance GHES et Mintlify. Configurez votre pare-feu pour autoriser les deux connexions suivantes.

<div id="outbound-your-ghes-instance-to-mintlify">
  #### Sortant : de votre instance GHES vers Mintlify
</div>

Votre instance GHES envoie les événements de webhook à Mintlify via HTTPS (port 443) à l'adresse `leaves.mintlify.com`.

* Autorisez le HTTPS sortant (port 443) vers `leaves.mintlify.com`. Autorisez ce trafic par nom d'hôte. Mintlify ne publie pas de liste d'adresses IP statiques pour ce point de terminaison, et ses adresses IP sous-jacentes peuvent changer.
...
Mintlify se connecte à votre instance GHES via HTTPS (port 443) pour appeler l'API GitHub et cloner des dépôts. Ces connexions proviennent de l'adresse IP de sortie statique de Mintlify : `54.242.90.151`.

Si un pare-feu ou une liste d'autorisation d'adresses IP restreint l'accès à votre instance GHES, autorisez le HTTPS entrant (port 443) depuis `54.242.90.151`.
