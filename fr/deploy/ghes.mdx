...
description: "Installez et configurez l’application GitHub Mintlify sur votre instance GitHub Enterprise Server pour déployer et synchroniser votre documentation."
...
L'intégration nécessite du trafic dans les deux directions entre votre instance GHES et Mintlify. Configurez votre pare-feu pour autoriser les deux connexions suivantes.

#### Sortant : de votre instance GHES vers Mintlify

Votre instance GHES délivre les événements de webhook à Mintlify via HTTPS (port 443) à `leaves.mintlify.com`.

* Autorisez le trafic sortant HTTPS (port 443) vers `leaves.mintlify.com`. Autorisez ce trafic par nom d'hôte. Mintlify ne publie pas de liste d'adresses IP statiques pour ce point de terminaison, et ses adresses IP sous-jacentes peuvent changer.
* Assurez-vous que votre instance GHES peut résoudre `leaves.mintlify.com` via DNS. Si vos serveurs utilisent un résolveur DNS interne, vérifiez qu'il résout les noms d'hôtes publics.

#### Entrant : de Mintlify vers votre instance GHES

Mintlify se connecte à votre instance GHES via HTTPS (port 443) pour appeler l'API GitHub, remonter les statuts des check runs et des déploiements, et lire le contenu du dépôt. Ces connexions proviennent de l'adresse IP statique d'egress de Mintlify : `54.242.90.151`.

Si un pare-feu ou une liste d'adresses IP autorisées restreint l'accès à votre instance GHES, autorisez le trafic entrant HTTPS (port 443) depuis `54.242.90.151`.

<Note>
  Les exécutions de l'[agent](/fr/agent) clonent et poussent depuis une infrastructure de sandbox distincte qui utilise par défaut des adresses IP dynamiques. Si votre instance GHES applique une liste d'adresses IP autorisées, voir [Résoudre les exécutions d'agent bloquées par une liste d'adresses IP autorisées](/fr/help-center/agent-blocked-by-git-provider-ip-allow-list).
</Note>
...
<Steps>
  <Step title="Définir les autorisations du dépôt">
    Définissez les autorisations suivantes pour l'application. L'application n'a besoin d'aucune autorisation au niveau de l'organisation, du compte ou de l'entreprise :
...
<Steps>
  <Step title="Créer l'application">
    ...
    GitHub vous redirige vers la page de paramètres de l'application.
  </Step>
...
* Installation owner : le nom de l’organisation GitHub propriétaire de l’installation de l’app
* Clé privée : l’intégralité du contenu du fichier `.pem` (partagez-la via un transfert de fichier sécurisé)
...
* Repository names: Dépôts spécifiques où vous avez installé l’application
...
* Intégrer votre environnement GHES à un projet Mintlify existant.
* Intégrer votre environnement GHES à un nouveau projet Mintlify que nous mettons en place pour vous.

Une fois que vous avez intégré votre environnement GHES à un projet Mintlify, vous êtes prêt à activer les webhooks pour votre GitHub App.
...
<AccordionGroup>
  ...
  <Accordion title="Les webhooks ne sont pas reçus">
    ...
    * Vérifiez que le secret du webhook correspond à celui que vous avez configuré.
  ...
  <Accordion title="L'application s'installe, mais ne réagit pas aux événements.">
    * Assurez-vous que notre serveur reçoit et accuse réception des webhooks avec un code de réponse 200.
    * Vous avez accordé les autorisations requises lors de l'installation.
  </Accordion>
  ...
  <Accordion title="Comment mettre à jour les autorisations de l'application ultérieurement ?">
    ...
    * Informez-nous de toute modification d'autorisations afin que nous puissions vous conseiller sur les étapes supplémentaires que vous pourriez devoir suivre.
  </Accordion>
