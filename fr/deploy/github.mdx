...
<div id="alternative-download-and-create-your-own-repository">
  ### Alternative : télécharger et créer votre propre référentiel
</div>

Si vous voulez un référentiel privé sans accorder d’autorisations OAuth étendues, vous pouvez transférer votre documentation manuellement :

1. Allez dans [Git Settings](https://dashboard.mintlify.com/settings/deployment/git-settings) dans votre Dashboard.
2. Téléchargez votre documentation au format fichier ZIP.
3. Créez un nouveau référentiel sur GitHub ou GitLab.
4. Extrayez le contenu du ZIP et poussez les fichiers vers votre nouveau référentiel.
5. Revenez à [Git Settings](https://dashboard.mintlify.com/settings/deployment/git-settings) et connectez votre nouveau référentiel à Mintlify.
6. Installez la GitHub App en suivant les étapes décrites dans [Installer la GitHub App](#install-the-github-app) ci-dessous.
...
Les fonctionnalités de feedback via les suggestions d’édition et le signalement de problèmes ne sont disponibles que pour les référentiels GitHub publics. Si ces options sont désactivées dans votre Tableau de bord Mintlify, vérifiez la visibilité de votre référentiel.
