...
[Les composants React](https://react.dev) sont un moyen puissant de créer des éléments interactifs et réutilisables dans votre documentation.
...
Le compteur s’affiche en tant que composant React interactif.

<Counter />

## Importation de composants

Vous pouvez importer des composants depuis votre dossier `snippets`. Contrairement à React classique, vous ne pouvez pas importer des composants depuis n’importe quel fichier `MDX`. Les composants réutilisables doivent être référencés à partir de fichiers situés dans le dossier `snippets`. En savoir plus sur les [fragments réutilisables (snippets)](/fr/reusable-snippets).
...
export const ColorGenerator = () => {
  const [hue, setHue] = useState(180)  const [saturation, setSaturation] = useState(50)
  ...
  const copyToClipboard = (color) => {
    navigator.clipboard
      .writeText(color)
      .then(() => {
        console.log(`Couleur copiée dans le presse-papiers : ${color}`)
      })
      .catch((err) => {
...
```mdximport { ColorGenerator } from "/snippets/color-generator.jsx"
...
<AccordionGroup>
  <Accordion title="Impact du rendu côté client">
    Les composants à base de hooks React s’exécutent côté client, ce qui entraîne plusieurs implications :

    - **SEO** : Les moteurs de recherche pourraient ne pas indexer pleinement le contenu dynamique.
    - **Chargement initial** : Les visiteurs peuvent voir un flash de contenu de chargement avant que les composants ne se rendent.
    - **Accessibilité** : Assurez-vous que les changements de contenu dynamique sont annoncés aux lecteurs d’écran.
  </Accordion>
  ...
  <Accordion title="Bonnes pratiques de performance">
    ...
    - **Chargement différé** : Envisagez de charger les composants complexes de façon différée pour améliorer le temps de chargement initial de la page.
  </Accordion>
</AccordionGroup>
