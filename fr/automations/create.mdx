...
Déclenchez une exécution manuelle avant de vous fier à un calendrier ou à un événement afin de détecter les problèmes en amont. Les exécutions manuelles utilisent la configuration actuelle de l'automatisation et apparaissent dans l'historique des exécutions aux côtés des exécutions planifiées. Consultez [Exécuter une automatisation manuellement](/fr/automations/manage#run-an-automation-manually) pour les étapes.

Ouvrez l'exécution dans la page [Automations](https://app.mintlify.com/products/automations?tab=runs) pour examiner le prompt, les fichiers que l'agent a lus ou modifiés, ainsi que toute pull request qu'il a ouverte. Si le résultat ne correspond pas à ce que vous attendiez, modifiez les instructions et relancez l'automatisation.
