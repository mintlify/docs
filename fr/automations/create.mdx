...
Testez une nouvelle automatisation avec une exécution manuelle afin de ne pas avoir à attendre que son déclencheur se produise. Les exécutions manuelles utilisent la configuration actuelle de l'automatisation et apparaissent dans l'historique des exécutions aux côtés des exécutions automatisées. Vous ne pouvez pas exécuter manuellement les automatisations déclenchées par des événements d'intégration. Consultez [Exécuter une automatisation manuellement](/fr/automations/manage#run-an-automation-manually) pour choisir la portée d'une exécution de test.
