...
- **Message dans un canal Slack** : s'exécute lorsqu'une personne publie un nouveau message de premier niveau dans un canal Slack sélectionné. Disponible pour les automatisations personnalisées. Sélectionnez un ou plusieurs canaux dont l'[application Mintlify pour Slack](/fr/agent/slack) est membre. Mintlify ignore les réponses en fil de discussion, les messages provenant de bots et les messages qui mentionnent l'agent Mintlify, et déduplique par identifiant d'événement Slack afin qu'un même message ne déclenche chaque automatisation correspondante qu'une seule fois au plus. Lorsque l'automatisation agit, elle répond dans le fil déclencheur avec un résumé et un lien vers toute pull request qu'elle ouvre.
