...
description: "Activez, désactivez, déclenchez et supprimez des automatisations depuis votre dashboard. Configurez déclencheurs, calendriers, dépôts et automerge."
...
Pour les automatisations personnalisées et l'automatisation **Update from code changes**, vous pouvez activer les [intégrations](/fr/automations/integrations) connectées afin que l'agent puisse récupérer du contexte depuis des outils comme Slack, Notion ou Linear lorsque l'automatisation s'exécute.

Pour activer les intégrations d'une automatisation, ouvrez ses paramètres et sélectionnez les intégrations que vous souhaitez que l'agent utilise dans la section **Available integrations**. Seules les intégrations déjà connectées pour votre organisation apparaissent dans la liste. Connectez les intégrations sur la page [Intégrations](https://app.mintlify.com/settings/organization/integrations) dans les paramètres de votre tableau de bord. Consultez [Intégrations](/fr/automations/integrations) pour plus d'informations.
...
<div id="troubleshoot-a-failed-run">
  ### Dépanner une exécution en échec
</div>

Si une exécution affiche le statut **Failed**, ouvrez-la depuis la page [Automation Runs](https://app.mintlify.com/products/automations) pour voir le prompt de l'agent, les appels d'outils et la sortie d'erreur.

Causes courantes :

- Absence d'accès à un dépôt de contexte ou de code source. Réinstallez la [GitHub App Mintlify](/fr/deploy/github) sur le dépôt concerné.
- Un jeton d'intégration révoqué. Reconnectez l'intégration sur la page [Integrations](https://app.mintlify.com/settings/organization/integrations).
- Un `docs.json` invalide ou une erreur de validation dans le dépôt du projet.
- Un calendrier ou un déclencheur qui se déclenche avant la fin de l'exécution précédente.

Après avoir résolu le problème sous-jacent, cliquez sur le bouton d'exécution <Icon icon="play" /> à côté de l'automatisation pour la relancer. Les exécutions en échec ne comptent pas dans votre limite quotidienne d'exécutions, les relances sont donc sans risque.
