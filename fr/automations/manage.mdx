...
### Délimiter la portée d'une exécution manuelle
</div>

Une exécution manuelle lancée depuis le bouton d'exécution examine tout ce qui a changé depuis la dernière exécution terminée de l'automatisation. Pour exécuter une automatisation avec une portée différente, demandez à l'[agent](/fr/editor/agent) de lancer l'exécution. L'agent prend en charge trois portées :

- **Plage de dates** : examine les changements d'une fenêtre de temps spécifique au lieu de la fenêtre par défaut. Par exemple, `run the changelog automation over the last two weeks`.
- **Tout** : examine tous les changements sans restriction de date. Par exemple, `run the style guide automation across everything`.
- **Une seule pull request** : s'exécute à partir d'une pull request d'un dépôt de code source connecté. Par exemple, `run the update from code changes automation on PR #42 of acme/api`. Pour les automatisations avec un déclencheur **Changement de code**, la pull request doit se trouver dans l'un des dépôts sources configurés.

Une exécution utilise exactement une portée. Fournir une portée vous permet aussi d'exécuter manuellement des automatisations qui n'ont pas de déclencheur de calendrier. Les exécutions avec portée comptent dans votre utilisation de crédits et apparaissent dans l'historique des exécutions comme toute autre exécution.
