...
- **Webhook** : s'exécute lorsqu'une requête `POST` authentifiée atteint le point de terminaison webhook de l'automatisation. Disponible uniquement pour les automatisations personnalisées. Utilisez-le pour déclencher une exécution depuis un pipeline CI/CD, un script de publication ou un outil interne. Consultez [Déclencher un webhook d'automatisation](/fr/api/automations/webhook) pour le point de terminaison, l'authentification et les limites de débit.
