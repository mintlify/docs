...
4. Sélectionnez une portée d'exécution, puis cliquez sur **Run now**.

Les exécutions manuelles enregistrent la configuration actuelle de l'automatisation avant de démarrer, sont décomptées de votre utilisation de crédits et apparaissent dans l'historique des exécutions aux côtés des exécutions planifiées. Après le démarrage d'une exécution, cliquez sur **View runs** dans la confirmation pour ouvrir l'historique des exécutions filtré sur cette automatisation.

Les automatisations avec un déclencheur **Intégration** ne peuvent pas être exécutées manuellement. Elles s'exécutent lorsque leur déclencheur se produit.

<div id="scope-a-manual-run">
### Délimiter la portée d'une exécution manuelle
</div>

Chaque exécution manuelle utilise exactement une portée, que vous sélectionnez au démarrage de l'exécution :

- **Depuis une date** : examine le contenu modifié depuis la date sélectionnée jusqu'à maintenant. Par défaut, la date correspond à la dernière exécution de l'automatisation.
- **Tout** : s'exécute sur l'ensemble de votre site sans restriction de date. Cela prend généralement plus de temps et utilise plus de crédits qu'une exécution ciblée.
- **Pull requests spécifiques** : limite l'exécution à une pull request d'un dépôt de code source connecté. Sélectionnez un dépôt, puis recherchez ses pull requests par titre ou par numéro. Pour les automatisations avec un déclencheur **Changement de code**, le dépôt doit être l'un des dépôts sources configurés.

Vous pouvez aussi demander à l'[agent](/fr/editor/agent) de lancer une exécution avec une portée. Par exemple, `run the changelog automation over the last two weeks` ou `run the update from code changes automation on PR #42 of acme/api`.
