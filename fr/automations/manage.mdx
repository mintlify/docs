...
- **Message Slack** : s'exécute lorsqu'un coéquipier publie un message de premier niveau dans l'un des canaux Slack sélectionnés. Disponible pour les automatisations personnalisées. Nécessite l'[application Slack Mintlify](/fr/agent/slack). Sélectionnez un ou plusieurs canaux à surveiller. Le déclencheur ignore les réponses de fil, les modifications et les messages provenant de bots ou de l'application Mintlify elle-même. L'agent lit chaque nouveau message et décide s'il doit agir.
