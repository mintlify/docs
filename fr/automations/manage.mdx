...
Vous ne pouvez supprimer que les [automatisations personnalisées](/fr/automations/create). Pour arrêter une automatisation prédéfinie, désactivez-la à la place. La suppression d'une automatisation est définitive et retire sa configuration ainsi que son historique d'exécutions.

1. Ouvrez la page [Automations](https://app.mintlify.com/products/automations) dans votre dashboard.
2. Cliquez sur le bouton <Icon icon="settings" /> de paramètres à côté de l'automatisation.
...
- **Failed** : l'agent n'a pas pu terminer l'exécution. Les exécutions en échec ne comptent pas dans votre limite quotidienne d'exécutions. Pour réessayer, [exécutez l'automatisation manuellement](#run-an-automation-manually) ou attendez son prochain déclenchement planifié.
