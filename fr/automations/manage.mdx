...
Lorsque vous sélectionnez **Calendrier personnalisé**, choisissez l'une des options de calendrier suivantes :
...
Utilisez un préréglage pour une fréquence récurrente courante. Sélectionnez **Cron personnalisé** lorsque vous avez besoin d'un calendrier que les préréglages ne couvrent pas. Le panneau des paramètres affiche une description compréhensible des préréglages. Il affiche la saisie cron uniquement lorsque **Cron personnalisé** est sélectionné.

Les expressions cron personnalisées utilisent cinq champs dans cet ordre : `minute hour day month weekday`. Par exemple, l'expression suivante exécute l'automatisation à 9:00 UTC du lundi au vendredi :

```text
0 9 * * 1-5
```

Mintlify valide l'expression avant que vous ne l'enregistriez. Un calendrier existant qui ne correspond à aucun préréglage s'ouvre avec **Cron personnalisé** sélectionné afin que vous puissiez examiner ou modifier l'expression.
