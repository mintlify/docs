...
- **Message dans un canal Slack** : s'exécute lorsqu'une personne publie un nouveau message de premier niveau dans un canal Slack sélectionné. Disponible pour les automatisations personnalisées. Invitez l'[application Mintlify pour Slack](/fr/agent/slack) dans chaque canal que vous souhaitez sélectionner, puis choisissez les canaux dans le panneau des paramètres. Si l'application n'est pas membre d'un canal sélectionné, le panneau des paramètres empêche l'enregistrement tant que vous ne l'y avez pas invitée. Mintlify ignore les réponses en fil de discussion, les messages provenant de bots et les messages qui mentionnent l'agent Mintlify. Mintlify déduplique par identifiant d'événement Slack afin qu'un même message ne déclenche chaque automatisation correspondante qu'une seule fois au plus. Lorsque l'automatisation agit, elle répond dans le fil déclencheur avec un résumé et un lien vers toute pull request qu'elle ouvre. Le détail de l'exécution dans le tableau de bord affiche le message déclencheur avec un lien permanent vers Slack.
