...
La suppression d'une automatisation la retire de votre projet, ainsi que sa configuration et son historique d'exécutions. Désactivez plutôt une automatisation si vous prévoyez de la réactiver plus tard.

1. Ouvrez la page [Automations](https://app.mintlify.com/products/automations) dans votre dashboard.
2. Cliquez sur le bouton <Icon icon="settings" /> paramètres à côté de l'automatisation que vous souhaitez supprimer.
3. Faites défiler jusqu'en bas de la fenêtre des paramètres et cliquez sur **Delete automation**.
4. Confirmez la suppression lorsqu'on vous le demande.

Vous pouvez également supprimer des automatisations depuis le terminal avec [`mint workflow`](/fr/cli/commands#mint-workflow).
