...
<Tabs>
  ...
  <Tab title="Tableau de bord Mintlify">
    ...
    * Les utilisateurs de votre documentation sont également vos éditeurs de documentation.
    ...
    <Steps>
      ...
      <Step title="Ajouter des utilisateurs autorisés.">
        1. Dans votre Dashboard, accédez à [Members](https://dashboard.mintlify.com/settings/organization/members).
    ...
    Votre documentation est hébergée sur `docs.foo.com` et votre équipe utilise le Dashboard pour modifier votre documentation. Vous souhaitez limiter l’accès aux seuls membres de l’équipe.

    **Activez l’authentification Mintlify** dans les paramètres de votre Dashboard.

    **Vérifiez l’accès de l’équipe** en vous assurant que tous les membres de l’équipe ont été ajoutés à votre organisation.
  </Tab>
    ...
    * La possibilité de créer un endpoint d’API accessible via des jetons d’accès OAuth (optionnel, pour activer les fonctionnalités de personnalisation).
    ...
    <Steps>
      <Step title="Configure your OAuth settings.">
        ...
        * **Scopes** : permissions à demander. Copiez la chaîne de scope **entière** (par exemple, pour un scope comme `provider.users.docs`, copiez l’intégralité de `provider.users.docs`). Utilisez plusieurs scopes si vous avez besoin de niveaux d’accès différents.
        * **Token URL** : votre endpoint d’échange de jeton OAuth.
        * **Info API URL** (optionnel) : endpoint pour récupérer les informations utilisateur à des fins de personnalisation. S’il est omis, le flux OAuth sera uniquement utilisé pour vérifier l’identité et les informations utilisateur resteront vides.
        * **Logout URL** : l’URL de déconnexion native de votre fournisseur OAuth. Si votre fournisseur propose un paramètre `returnTo` ou similaire, faites-le pointer vers l’URL de vos docs.
      ...
      <Step title="Create your user info endpoint (optional).">
        ...
        * Renvoie des données utilisateur au format `User`. Voir [User data format](/fr/authentication-personalization/personalization-setup#user-data-format) pour plus d’informations.
    ...
    * Un service backend capable de créer des URL de redirection.
    ...
    <Steps>
      <Step title="Générez une clé privée.">
        ...
        6. Stockez votre key en toute sécurité afin qu’elle soit accessible par votre backend.
      </Step>
      ...
      <Step title="Intégrez l’authentification Mintlify à votre flux de connexion.">
        ...
        * Créez un JWT contenant les informations de l’utilisateur authentifié au format `User`. Voir [Format des données utilisateur](/fr/authentication-personalization/personalization-setup#user-data-format) pour plus d’informations.
    ...
    Votre documentation est hébergée sur `docs.foo.com` avec un système d’authentification existant sur `foo.com`. Vous souhaitez étendre votre flux de connexion pour accorder l’accès à la documentation tout en gardant votre documentation séparée de votre Dashboard (ou vous n’avez pas de Dashboard).
    ...
    * Signez le JWT et redirigez vers `https://docs.foo.com/login/jwt-callback#{SIGNED_JWT}`.
    ...
    <CodeGroup>
      ...
      export async function handleRequest(req: Request, res: Response) {
        const user = {
          expiresAt: Math.floor((Date.now() + TWO_WEEKS_IN_MS) / 1000), // expiration de la session de 2 semaines
      ...
      async def return_mintlify_auth_status(current_user):
        jwt_token = jwt.encode(
          payload={
            ...
            'expiresAt': int((datetime.now() + timedelta(weeks=2)).timestamp()), # expiration de la session d'1 semaine
    ...
    2. Redirection vers votre URL de connexion avec un paramètre de requête de redirection : `https://foo.com/docs-login?redirect=%2Fquickstart`.
...
Lorsque vous utilisez une authentification partielle, toutes les pages sont protégées par défaut. Vous pouvez rendre certaines pages accessibles sans authentification, au niveau de la page ou du groupe, grâce à la propriété `public`.
