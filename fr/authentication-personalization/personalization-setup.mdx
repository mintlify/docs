...
```jsx
Bon retour, {user.firstName} ! Votre offre {user.org?.plan} inclut...
```
...
<ParamField path="expiresAt" type="number">
  ...
  <Warning><b>Pour les échanges JWT :</b> Cela diffère de l’attribut `exp` du JWT, qui détermine quand un JWT est considéré invalide. Par sécurité, définissez l’attribut `exp` du JWT sur une durée très courte (10 secondes ou moins). Utilisez `expiresAt` pour la durée réelle de la session (de quelques heures à plusieurs semaines).</Warning>
</ParamField>
...
<ParamField path="apiPlaygroundInputs" type="object">
  ...
  **Exemple** :
  ...
  Si un utilisateur envoie des requêtes sur un sous-domaine spécifique, vous pouvez transmettre `{ server: { subdomain: 'foo' } }` comme champ `apiPlaygroundInputs`. Cette valeur sera préremplie sur toute page d’API comportant le champ `subdomain`.
...
<Tabs>
    ...
    * Un service backend capable de créer des URL de redirection
    ...
    <Steps>
      <Step title="Generate a private key.">
        ...
        6. Stockez votre clé en lieu sûr afin qu’elle soit accessible par votre backend.
      </Step>
    ...
    Générez un secret JWT. Ensuite, créez un endpoint de connexion à `https://foo.com/docs-login` qui lance un parcours de connexion vers votre documentation.
  ...
  <Tab title="OAuth 2.0">
    ...
    * La capacité de créer un endpoint d’API accessible via des jetons d’accès OAuth
    ...
    <Steps>
      ...
      <Step title="Configure your OAuth personalization settings.">
        ...
        * **Scopes** : permissions à demander. Copiez la chaîne de scope **entière** (par exemple, pour un scope comme `provider.users.docs`, copiez l’intégralité de `provider.users.docs`). Doit correspondre aux scopes de l’endpoint configuré à l’étape 1.
    ...
    **Configurez les paramètres de votre serveur OAuth** dans votre tableau de bord :

    * **URL d’autorisation** : `https://auth.foo.com/authorization`
    * **ID client** : `ydybo4SD8PR73vzWWd6S0ObH`
    * **Scopes** : `['docs-user-info']`
    * **URL de jeton** : `https://auth.foo.com/exchange`
    * **URL de l’API d’informations** : `https://api.foo.com/docs/user-info`

    **Configurez votre serveur OAuth** pour autoriser les redirections vers votre URL de rappel (callback).
  </Tab>
    ...
    * Vos docs sont hébergées sur le même domaine ou sous-domaine que votre tableau de bord.
    ...
    <Steps>
      ...
      <Step title="Configure your personalization settings">
        ...
        4. Saisissez votre **Info API URL**, c’est-à-dire le point de terminaison de la première étape.
