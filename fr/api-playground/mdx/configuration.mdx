...
Vous pouvez définir manuellement des endpoints d’API dans des fichiers `MDX` individuels plutôt que d’utiliser une spécification OpenAPI. Cette méthode offre de la flexibilité pour du contenu personnalisé, mais nous recommandons de générer la documentation d’API à partir d’un fichier de spécification OpenAPI pour la plupart des projets, car c’est plus facile à maintenir et plus riche en fonctionnalités. Cependant, créer des pages `MDX` pour une API peut être utile pour documenter de petites API ou pour du prototypage.
...
<Steps>
  ...
  <Step title="Créez les pages de vos endpoints">
    ...
    Vous pouvez remplacer le mode d’affichage défini globalement pour le bac à sable d’API sur une page en ajoutant `playground` au frontmatter :
