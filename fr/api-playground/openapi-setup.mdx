...
Vous pouvez référencer plusieurs spécifications OpenAPI pour créer des pages pour votre API. Organisez des sections pour chaque spécification dans votre navigation, ou référencez des endpoints spécifiques provenant de différentes spécifications.
...
Générez des sections d’API dédiées en ajoutant un champ `openapi` à un élément de navigation, sans ajouter d’autres pages. Tous les points de terminaison de la spécification seront inclus :
...
<Note>
  ...
  enregistrées dans votre dépôt de documentation. S’il n’est pas défini, il est remplacé par défaut par le répertoire `api-reference`
  de votre dépôt.
</Note>
...
Faites référence à des points de terminaison spécifiques sans définir de spécification OpenAPI par défaut en incluant le chemin du fichier. Vous pouvez référencer des points de terminaison issus de plusieurs spécifications OpenAPI dans la même section de documentation.
...
* Mélanger des points de terminaison issus de plusieurs spécifications OpenAPI au sein de la même section de navigation.
* Inclure uniquement certains points de terminaison de différentes spécifications plutôt que tous.
* Intégrer des points de terminaison d’API aux côtés d’autres types de pages de documentation.
