...
description: "Générez une documentation d’API interactive à partir de vos fichiers de spécification OpenAPI."
...
Contrôlez comment les endpoints individuels de votre API sont exposés comme outils dans votre [serveur MCP](/ai/model-context-protocol) en utilisant `x-mint: mcp`. Par défaut, tous les endpoints sont disponibles comme outils MCP. Utilisez cette extension pour désactiver des endpoints spécifiques, ou pour personnaliser le nom et la description de l'outil que les applications d'IA voient lorsqu'elles se connectent à votre serveur MCP.

L'extension `x-mint: mcp` accepte les propriétés suivantes :

| Propriété | Type | Description |
| :--- | :--- | :--- |
| `enabled` | boolean | Indique si le endpoint est disponible comme outil MCP. `true` par défaut. |
| `name` | string | Nom personnalisé pour l'outil MCP. Remplace le nom généré par défaut. |
| `description` | string | Description personnalisée pour l'outil MCP. Remplace la description générée par défaut. |
...
Pour masquer un endpoint de votre serveur MCP tout en le gardant visible dans votre documentation, définissez `enabled` sur `false` :
...
<Note>
  L'extension `x-mint: mcp` remplace l'extension obsolète `x-mcp`. Si les deux sont présentes sur une opération, `x-mint: mcp` a la priorité.
</Note>
