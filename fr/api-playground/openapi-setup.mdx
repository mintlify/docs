...
Contrôlez quels endpoints d'API sont exposés en tant qu'outils MCP (Model Context Protocol) à l'aide de `x-mint: mcp`. Cela permet aux agents IA de découvrir et d'utiliser directement vos endpoints d'API.
...
<ResponseField name="x-mint.mcp.enabled" type="boolean">
  Indique s'il faut exposer cet endpoint en tant qu'outil MCP.
</ResponseField>

<ResponseField name="x-mint.mcp.name" type="string">
  Nom personnalisé pour l'outil MCP. Si non spécifié, un nom est généré à partir de l'opération.
</ResponseField>

<ResponseField name="x-mint.mcp.description" type="string">
  Description personnalisée pour l'outil MCP. Si non spécifiée, le résumé ou la description de l'opération est utilisé.
</ResponseField>
