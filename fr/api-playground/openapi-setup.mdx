...
Contrôlez quelles opérations API sont exposées comme outils via votre [serveur MCP](/ai/model-context-protocol) en utilisant `x-mint: mcp`. Par défaut, les opérations ne sont pas exposées comme outils MCP. Ajoutez `x-mint: mcp` avec `enabled: true` pour activer des opérations spécifiques.

<ParamField body="enabled" type="boolean">
  Indique s'il faut exposer cette opération comme un outil MCP. Par défaut `false`.
</ParamField>

<ParamField body="name" type="string">
  Nom personnalisé pour l'outil MCP. S'il n'est pas défini, le nom est dérivé de l'opération.
</ParamField>

<ParamField body="description" type="string">
  Description personnalisée pour l'outil MCP. Si elle n'est pas définie, la description est dérivée du résumé ou de la description de l'opération.
</ParamField>
      ...
      "get": {
        ...
        "x-mint": {
          "mcp": {
            "enabled": true,
            "name": "list-users",
            "description": "Récupérer une liste paginée de tous les utilisateurs"
          }
        },
        "responses": {
          "200": {
...
<Note>
  L'extension obsolète `x-mcp` est toujours prise en charge, mais `x-mint: mcp` a la priorité. Utilisez `x-mint: mcp` pour les nouvelles configurations.
</Note>
