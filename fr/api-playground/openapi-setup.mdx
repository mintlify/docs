...
Contrôlez quelles opérations d'API sont exposées comme outils via votre [serveur MCP](/ai/model-context-protocol) en utilisant `x-mint: mcp`. Par défaut, toutes les opérations sont incluses. Utilisez cette extension pour désactiver des opérations spécifiques ou personnaliser leur apparence pour les outils d'IA.
...
L'objet `mcp` accepte les propriétés suivantes :

| Propriété | Type | Description |
| :---------- | :------ | :---------- |
| `enabled` | boolean | Indique si cette opération est exposée comme outil MCP. Par défaut `true`. |
| `name` | string | Nom personnalisé pour l'outil MCP. Remplace le nom généré par défaut. |
| `description` | string | Description personnalisée pour l'outil MCP. Remplace la `description` de l'opération. |

Pour exclure un endpoint de votre serveur MCP, définissez `enabled` sur `false` :
...
<Note>
  L'extension `x-mint: mcp` remplace l'ancienne extension `x-mcp`. Si les deux sont présentes sur une opération, `x-mint: mcp` a la priorité.
</Note>
