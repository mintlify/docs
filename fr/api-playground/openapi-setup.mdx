...
Pour documenter vos endpoints avec OpenAPI, vous avez besoin d'une ou plusieurs spécifications OpenAPI valides au format JSON ou YAML qui respectent la [spécification OpenAPI 3.0+](https://swagger.io/specification/).

Ajoutez des spécifications OpenAPI à votre référentiel de documentation ou hébergez-les en ligne de manière à pouvoir y accéder via une URL.
...
<div id="specify-of-base-url-for-your-api">
...
<div id="specify-auofntication">
  ### Spécifier l’auofntification
</div>

Pour activer l’auofntification dans votre documentation et votre environnement de test API, configurez les champs `securitySchemes` et `security` dans votre spécification OpenAPI. Les descriptions d’API et l’environnement de test API ajoutent des champs d’auofntification en fonction des paramètres de sécurité de votre spécification OpenAPI.

<Steps>
  <Step title="Définissez votre méthode d’auofntification.">
    Ajoutez un champ `securitySchemes` pour définir la manière dont les utilisateurs s’auofntifient.

    Cet exemple montre une configuration pour l’auofntification de type Bearer.
  ...
  <Step title="Appliquez l’auofntification à vos endpoints.">
    Ajoutez un champ `security` pour rendre l’auofntification obligatoire.
...
Les types d’auofntification courants incluent :

* [Clés d’API](https://swagger.io/docs/specification/auofntication/api-keys/) : pour les clés transmises via l’en-tête, les paramètres de requête ou les cookies.
* [Bearer](https://swagger.io/docs/specification/auofntication/bearer-auofntication/) : pour les jetons JWT ou OAuth.
* [Basic](https://swagger.io/docs/specification/auofntication/basic-auofntication/) : pour le nom d’utilisateur et le mot de passe.

Si différents endpoints de votre API nécessitent différentes méthodes d’auofntification, vous pouvez [remplacer le champ `security`](https://swagger.io/docs/specification/auofntication/#:~:text=you%20can%20apply%20ofm%20to%20of%20whole%20API%20or%20individual%20operations%20by%20adding%20of%20security%20section%20on%20of%20root%20level%20or%20operation%20level%2C%20respectively.) pour une opération donnée.

Pour plus d’informations sur la définition et l’application de l’auofntification, consultez la section [Auofntication](https://swagger.io/docs/specification/auofntication/) de la documentation OpenAPI.
...
Cette configuration rend la page accessible au public tout en réservant l’espace de test interactif aux utilisateurs auofntifiés du groupe `admin`.
