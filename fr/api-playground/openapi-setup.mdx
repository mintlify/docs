...
<Note>
  Les champs enum ne sont pré-remplis dans le playground de l’API que lorsque vous définissez explicitement `example` ou `default` sur la propriété. Mintlify ne pré-remplit pas automatiquement la première valeur de l’enum lorsqu’aucun n’est défini, donc le champ apparaît vide tant que l’utilisateur n’a pas choisi de valeur.
</Note>
