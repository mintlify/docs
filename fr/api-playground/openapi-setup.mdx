...
Dans une spécification OpenAPI, des chemins comme `/users/{id}` ou `/` identifient les différents points de terminaison (endpoints) d’API. L’URL de base indique où les clients ajoutent ces chemins. Pour en savoir plus sur la configuration du champ `servers`, consultez la page [API Server and Base Path](https://swagger.io/docs/specification/api-host-and-base-path/) de la documentation OpenAPI.
...
Les overlays sont des fichiers JSON ou YAML distincts qui décrivent des modifications à appliquer à votre spécification OpenAPI. Utilisez des overlays pour modifier une spécification sans éditer son fichier source. Cela est utile lorsque la spécification est générée par un autre outil ou maintenue par une autre équipe. Les usages courants incluent le renommage de chemins, la définition d’URL de serveur, la suppression d’endpoints internes et l’amélioration des descriptions pour la documentation.

Mintlify prend en charge les versions 1.0 et 1.1 de la [spécification OpenAPI Overlay](https://spec.openapis.org/overlay/v1.1.0.html). Mintlify applique les overlays après l’analyse d’une spécification et avant sa validation. Les pages d’endpoints générées, la navigation, les références `openapi` dans le frontmatter et `mint validate` utilisent tous le document transformé.

<div id="create-an-overlay-document">
  ### Créer un document d’overlay
</div>

Un document d’overlay comporte un champ de version `overlay` de premier niveau, un objet `info` avec un `title` et une `version`, et un tableau `actions`. Chaque action sélectionne des nœuds de la spécification avec une expression `target` au format [JSONPath RFC 9535](https://www.rfc-editor.org/rfc/rfc9535) et applique un modificateur :
...
* `copy` : copie le nœud sélectionné par une autre expression JSONPath dans chaque nœud ciblé. Le modificateur `copy` nécessite Overlay 1.1.
...
Le champ facultatif `extends` associe un overlay à une spécification pour la [découverte automatique](#auto-discover-overlays). Définissez `extends` sur un chemin relatif au fichier d’overlay ou sur l’URL exacte que votre `docs.json` utilise pour référencer une spécification hébergée.

<div id="reference-overlays-in-your-docsjson">
  ### Référencer les overlays dans votre docs.json
</div>

Listez explicitement les overlays en utilisant la forme objet du champ `openapi`. La forme objet fonctionne partout où le champ `openapi` est accepté, y compris à l’intérieur de tableaux. Les overlays s’appliquent dans l’ordre où vous les listez.
...
Les chemins d’overlay doivent pointer vers des fichiers situés dans votre dépôt de documentation. Les URL d’overlay doivent utiliser `https`. Si deux entrées `openapi` référencent la même spécification avec des listes `overlays` différentes, la génération échoue.

<div id="auto-discover-overlays">
  ### Découverte automatique des overlays
</div>

Mintlify traite tout fichier JSON ou YAML de votre dépôt comportant une clé `overlay` de premier niveau comme un document d’overlay. Un overlay dont le champ `extends` correspond à une spécification de votre documentation s’applique automatiquement à cette spécification. Les overlays découverts automatiquement s’appliquent dans l’ordre alphabétique de leurs chemins de fichier. Les overlays sans champ `extends` ne s’appliquent jamais automatiquement.

Une liste `overlays` explicite pour une spécification remplace la découverte automatique pour cette spécification. Définissez `"overlays": []` pour désactiver tous les overlays d’une spécification, y compris ceux découverts automatiquement.

Les overlays explicites et découverts automatiquement échouent différemment. Si un overlay d’une liste `overlays` explicite ne peut pas être chargé ou appliqué, la spécification échoue à la validation et le déploiement signale une erreur de spécification. Si un overlay découvert automatiquement échoue, Mintlify l’ignore et publie la spécification sans lui.

<div id="example-rename-a-path">
  ### Exemple : renommer un chemin
</div>
...
Référencez la spécification transformée partout dans votre documentation. Par exemple, une référence dans le frontmatter d’une page doit utiliser le chemin post-overlay : `openapi: "POST /credit/accounts"`.

Lorsque vous exécutez `mint dev`, la modification ou la suppression d’un fichier d’overlay reconstruit les spécifications concernées. `mint validate` et `mint openapi-check` valident le document transformé, de sorte que les erreurs de validation référencent votre spécification après l’application des overlays.
...
Lorsque l’option est activée, un clic dessus télécharge directement votre spécification OpenAPI. Les déploiements comportant plusieurs spécifications les reçoivent regroupées dans `api-specs.zip`. Sur les déploiements protégés par `auth` ou `userAuth`, seuls les lecteurs authentifiés peuvent télécharger la spécification.

<Warning>
  La spécification OpenAPI téléchargée n'est pas filtrée et ne respecte pas les [groupes d'authentification](/fr/deploy/authentication-setup). Tout lecteur authentifié capable d'ouvrir le menu contextuel reçoit la spécification complète, y compris les points de terminaison et schémas qui seraient autrement masqués pour son groupe. N'activez pas `download-spec` sur un site authentifié si votre spécification OpenAPI contient des points de terminaison ou champs que vous jugez sensibles.
</Warning>
...
<Warning>
  **Spécifiez toujours le chemin du fichier OpenAPI lorsque vous avez plusieurs spécifications OpenAPI dans votre référentiel.**
  ...
  Mintlify charge chaque spécification OpenAPI de votre référentiel de documentation, même si elle n’est pas référencée dans `docs.json`. Lorsqu’une page MDX utilise `openapi: "MÉTHODE /chemin"` sans chemin de fichier, Mintlify recherche une opération correspondante dans toutes les spécifications chargées. Si plus d’une spécification contient la même méthode et le même chemin, la première correspondance l’emporte selon l’ordre alphabétique du nom de fichier, ce qui peut ne pas être la spécification attendue.

  Pour éviter toute ambiguïté, procédez de l’une des façons suivantes :
  ...
  - Supprimez le chemin en conflit de l’autre spécification.
  - Supprimez de votre référentiel les fichiers OpenAPI dont vous n’avez plus besoin.
</Warning>
