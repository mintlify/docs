---
title: "Configurer OpenAPI pour une documentation API interactive"
...
Mintlify affiche un champ de fichier dans le playground de l’API lorsque le schéma du corps de votre requête identifie un fichier. Utilisez les champs de fichier pour les endpoints qui acceptent des images, des documents, des archives ou d’autres données binaires.

Pour OpenAPI 3.1, définissez `type` sur `string` et ajoutez un champ `contentMediaType`.

Ajoutez `contentEncoding: base64` ou `contentEncoding: base64url` lorsque votre API attend un fichier encodé en base64.

Pour OpenAPI 3.0, utilisez plutôt `format: binary` ou `format: base64`.
...
Le playground prend en charge les types MIME courants pour les images, l’audio, la vidéo et les applications binaires. Les utilisateurs peuvent faire glisser un fichier vers le champ de téléversement ou cliquer sur le champ pour en sélectionner un.
...
Lorsque l’option est activée, un clic dessus télécharge directement votre spécification OpenAPI. Les déploiements comportant plusieurs spécifications les reçoivent regroupées dans `api-specs.zip`. Mintlify masque l’option pour les déploiements protégés par `auth` ou `userAuth` afin d’éviter la fuite du contenu des spécifications depuis une documentation authentifiée.
