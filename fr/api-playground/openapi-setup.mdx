...
Lorsque l’option est activée, un clic dessus télécharge directement votre spécification OpenAPI. Les projets comportant plusieurs spécifications les reçoivent regroupés dans `api-specs.zip`. Sur les projets protégés par `auth` ou `userAuth`, seuls les lecteurs authentifiés peuvent télécharger la spécification.
