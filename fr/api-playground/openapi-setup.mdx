...
<div id="use-a-localhost-specification">
  ### Utiliser une spécification en localhost
</div>

Pendant le développement local, vous pouvez générer un document OpenAPI depuis un service qui s'exécute sur votre machine et référencer son URL en localhost :
...
Passez `--local-schema` pour permettre à la CLI de récupérer la spécification via HTTP :

```bash
mint dev --local-schema
```

Pour valider la même configuration sans démarrer une prévisualisation, exécutez `mint validate --local-schema`. Les déploiements en production exigent que les URL OpenAPI utilisent HTTPS.
