...
<AccordionGroup>
  ...
  <Accordion title="Ma build échoue avec &quot;Failed to fetch OpenAPI file for anchor or tab&quot;">
    Cette erreur signifie que Mintlify n'a pas pu télécharger le document OpenAPI à l'URL indiquée dans le champ `openapi` de votre `docs.json` pendant la build. Causes courantes :

    - L'hôte est inaccessible ou n'est résolu que depuis un réseau privé.
    - L'URL requiert une authentification (jeton, cookie de session ou IP autorisée).
    - Le certificat est invalide ou le domaine a un problème DNS.
    - L'origine a renvoyé une erreur `5xx` transitoire ou a expiré.
    - La spécification était en cours de republication au moment où la build s'est exécutée, si bien que l'URL a servi une réponse partielle ou vide.

    Les builds téléchargent l'URL depuis l'internet public sans identifiants, donc vérifiez que le document se télécharge avec `curl` depuis une machine hors de votre réseau. Pour corriger l'erreur, adoptez l'un de ces modèles :

    - **Ajoutez la spécification à votre dépôt de docs.** C'est le modèle recommandé quand l'URL source est protégée par authentification. Pointez le champ `openapi` vers le chemin relatif au dépôt (par exemple, `"openapi": "openapi.json"`) et mettez à jour le fichier dans le même commit que celui qui change votre API.
    - **Servez la spécification depuis une URL publique HTTPS stable.** Hébergez-la sur un CDN ou un bucket de stockage d'objets qui ne requiert pas d'authentification, a un certificat TLS valide et renvoie le document complet à chaque requête.

    Pour un guide de diagnostic complet, y compris les échecs intermittents dus au CI qui publie la spécification après avoir déclenché la build, consultez [La build échoue avec "Failed to fetch OpenAPI file for anchor or tab"](/fr/help-center/openapi-url-fetch-fails-during-build).
  </Accordion>
