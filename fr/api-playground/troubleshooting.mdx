---
title: "Dépannage"
description: "Résolvez les problèmes courants du playground API : erreurs de validation OpenAPI, endpoints manquants et problèmes d'authentification fréquents."
...
<AccordionGroup>
  ...
  <Accordion title="Mon build échoue avec &quot;Failed to fetch OpenAPI file for anchor or tab&quot;">
    Cette erreur signifie que Mintlify n’a pas pu télécharger le document OpenAPI à l’URL indiquée dans le champ `openapi` de votre `docs.json` lors du build. Les causes fréquentes sont :

    - L’hôte est injoignable ou n’est résolu que depuis un réseau privé.
    - L’URL nécessite une authentification (un token, un cookie de session ou une liste d’adresses IP autorisées).
    - Le certificat est invalide ou le domaine présente un problème DNS.
    - L’origine a renvoyé une erreur 5xx transitoire ou a expiré.
    - La spécification était en cours de republication au moment où le build s’est exécuté, de sorte que l’URL a servi une réponse partielle ou vide.

    Pour cerner la cause, reproduisez le téléchargement en dehors de Mintlify depuis un réseau public :
    ...
    Puis validez le document téléchargé en local avec la CLI Mint :

    ```bash
    mint validate
    ```

    Si l’URL n’est pas accessible depuis l’internet public, ou n’est accessible qu’avec des identifiants, adoptez l’un de ces schémas :

    - **Committez la spécification dans votre dépôt de docs.** C’est le schéma recommandé lorsque l’URL source est derrière une authentification. Faites pointer le champ `openapi` vers le chemin relatif au dépôt (par exemple, `"openapi": "openapi.json"`) et mettez à jour le fichier dans le même commit que celui qui modifie votre API.
    - **Servez la spécification depuis une URL HTTPS publique et stable.** Hébergez-la sur un CDN ou un bucket de stockage d’objets qui ne nécessite pas d’authentification, dispose d’un certificat TLS valide et renvoie le document complet à chaque requête.

    Si vous générez la spécification en CI, séquencez le pipeline pour que la spécification soit entièrement publiée à son URL publique avant d’appeler l’endpoint d’API [Trigger deployment](/fr/api/update/trigger). Déclencher le déploiement en premier fait que le build récupère une spécification obsolète ou manquante.
  </Accordion>
  ...
  <Accordion title="La description du champ Authorization est générique ou peu utile">
    Mintlify génère automatiquement une description pour le champ `Authorization` du bac à sable d'API en fonction du type de schéma de sécurité. Vous pouvez la remplacer en ajoutant une `description` au schéma de sécurité dans votre spécification OpenAPI.
      ...
      "components": {
        "securitySchemes": {
          "bearerAuth": {
            "type": "http",
            "scheme": "bearer",
            "description": "Votre clé d'API depuis le [tableau de bord](https://example.com/dashboard). Envoyez-la sous la forme `Bearer <key>`."
          }
        }
      }
    }
    ...
    La `description` prend en charge Markdown et remplace le texte généré automatiquement sur chaque endpoint qui utilise ce schéma de sécurité.
  </Accordion>
