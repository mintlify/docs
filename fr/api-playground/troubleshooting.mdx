...
<AccordionGroup>
  ...
  <Accordion title="L’une de mes pages OpenAPI s’affiche complètement vide">
    ...
    Mintlify tolère les différences de barre oblique finale — `/users/{id}` et `/users/{id}/` correspondent au même endpoint. Cependant, si votre spécification définit les deux comme des opérations distinctes, la correspondance exacte a la priorité.
