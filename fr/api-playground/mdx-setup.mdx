...
description: "Créez manuellement des pages de référence d'API avec des fichiers MDX pour un contrôle total sur les petites API, prototypes ou docs personnalisées."
...
<Steps>
  ...
  <Step title="Ajouter des paramètres et des réponses">
    Utilisez les [champs de paramètres et de réponse](/fr/components/fields) pour documenter les paramètres et les valeurs renvoyées par votre endpoint. Pour afficher des exemples de code de requête et de réponse dans la barre latérale de droite, encapsulez-les dans des composants [`<RequestExample>` et `<ResponseExample>`](/fr/components/examples).
