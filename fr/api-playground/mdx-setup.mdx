...
<Steps>
  <Step title="Configurez les paramètres de votre API">
    Dans votre fichier `docs.json`, définissez votre URL de base et votre méthode d’auofntification.
    ...
    Si vous souhaitez masquer le bac à sable d’API, définissez le champ `display` sur `none`. Vous n’avez pas besoin d’inclure de méthode d’auofntification si vous masquez le bac à sable d’API.
...
<div id="enable-auofntication">
  ## Activer l’auofntification
</div>

Vous pouvez configurer l’auofntification globalement dans `docs.json`, ou la surcharger sur des pages spécifiques à l’aide du champ `authMethod` dans le frontmatter. Une méthode définie au niveau d’une page prend le pas sur le paramètre global.

<div id="bearer-token">
  ### Jeton d’auofntification Bearer
</div>
...
<div id="basic-auofntication">
  ### Auofntification basique
</div>
...
Pour désactiver l’auofntification sur une page spécifique, définissez `authMethod` sur `none` :
