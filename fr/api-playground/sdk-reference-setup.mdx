...
<div id="preview-and-troubleshoot">
  ## Prévisualiser et dépanner
</div>

Exécutez [`mint dev`](/fr/cli/commands#mint-dev) pour prévisualiser localement les pages de référence du SDK. La CLI analyse l’artefact au démarrage et reconstruit les pages lorsque vous modifiez `docs.json` ou le fichier d’artefact.

Si les pages n’apparaissent pas ou si le build signale des erreurs :

- Vérifiez que `format` correspond à l’artefact produit par votre outil.
- Vérifiez que `source` pointe vers le fichier ou le répertoire correct par rapport à votre dépôt de documentation, ou vers une URL HTTPS accessible.
- Pour les formats à répertoire, vérifiez que l’artefact contient la sortie attendue de l’outil (par exemple, YAML `docfx metadata`, sortie du builder `json` de Sphinx).
- Pour les sources distantes, vérifiez que le téléchargement respecte les limites de 50 Mo / 200 Mo extraits.

Exécutez [`mint validate`](/fr/cli/commands#mint-validate) pour faire remonter les erreurs de configuration en même temps que le reste du build de votre documentation.
