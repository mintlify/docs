...
Déclarez `sdk` sur un groupe pour limiter les pages générées à une sous-section d’un onglet existant. Les groupes héritent de la configuration `sdk` de leur parent, ce qui permet aux groupes imbriqués de remplacer ou d’étendre l’artefact défini au niveau de l’onglet.
...
<Note>
  Vous pouvez déclarer `sdk` sur un [onglet](/fr/organize/navigation#tabs) ou un [groupe](/fr/organize/navigation#groups). Un onglet ou un groupe avec `sdk` peut inclure des `groups` imbriqués, mais aucune autre structure de navigation, telle que `pages`, `versions` ou `languages`. Il ne peut pas non plus inclure une propriété `openapi`, `asyncapi` ou `graphql`.
</Note>
...
Lorsqu’au moins une page SDK rédigée manuellement existe sous un onglet ou un groupe avec `sdk`, Mintlify ignore la génération en masse pour ce propriétaire. Déplacez une page hors de ce propriétaire si vous souhaitez que le reste de la bibliothèque soit rempli automatiquement.
...
La forme chaîne est analysée comme `[source] kind name`. Omettez `source` pour l’hériter (ainsi que `format`) de la configuration `sdk` de l’onglet ou du groupe. Pour les cibles `method` et `property`, qualifiez le nom avec un parent, par exemple `method Auth0Client.loginWithRedirect`.
...
<ParamField path="format" type="string">
  Remplace le `format` hérité. Nécessaire uniquement lorsque la page ne se trouve pas sous un onglet ou un groupe avec `sdk`.
</ParamField>

<ParamField path="source" type="string">
  Remplace le `source` hérité. Nécessaire uniquement lorsque la page ne se trouve pas sous un onglet ou un groupe avec `sdk`.
</ParamField>
