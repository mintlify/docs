...
<ResponseField name="playground" type="object">
  ...
  <Expandable title="playground" defaultOpen="True">
    ...
    <ResponseField name="credentials" type="boolean">
      ...
      Lorsque `true` et que `proxy` est `false`, le playground envoie les requêtes avec les identifiants (cookies, en-têtes d'autorisation ou certificats client TLS) inclus. Ceci est utile lorsque votre API repose sur l'authentification par cookies ou la gestion de sessions et que vous avez besoin que le navigateur envoie ces identifiants directement. N'a aucun effet lorsque `proxy` est `true`.
    </ResponseField>
  </Expandable>
</ResponseField>
