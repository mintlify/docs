...
<ResponseField name="playground" type="object">
  ...
  <Expandable title="playground" defaultOpen="True">
    ...
    <ResponseField name="credentials" type="boolean">
      Indique s'il faut inclure les cookies et les en-têtes d'authentification pour les requêtes cross-origin lorsque le proxy est désactivé. Valeur par défaut : `false`.

      Lorsque `true` et que `proxy` est `false`, le playground envoie les requêtes avec les identifiants (cookies, en-têtes d'autorisation ou certificats client TLS) inclus. Cela est utile lorsque votre API repose sur l'authentification par cookies ou les jetons de session et que vous avez besoin que le navigateur les joigne automatiquement. Cette option n'a aucun effet lorsque `proxy` est `true`.
    </ResponseField>
  </Expandable>
</ResponseField>
