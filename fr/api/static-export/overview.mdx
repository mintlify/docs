...
<Note>
  L'exportation statique n'est pas disponible pour les déploiements dont l'authentification est activée. Si votre déploiement utilise une authentification partielle ou complète, [Démarrer une tâche d'exportation statique](/fr/api/static-export/start-job) renvoie une réponse `403`. Contactez [support@mintlify.com](mailto:support@mintlify.com) si vous devez exporter un déploiement authentifié.
</Note>

<div id="page-urls-in-static-bundles">
  ## URL des pages exportées
</div>

Les bundles d’exportation statique utilisent des URL de pages explicites avec l’extension `.html`. Ainsi, `/guides/getting-started` devient `/guides/getting-started.html`, et la page d’accueil devient `/index.html`.

Cette structure correspond aux fichiers du bundle. Elle permet un hébergement S3 ou statique simple, sans nécessiter la réécriture d’URL sans extension. Conservez les noms de fichiers `.html` lors du téléversement. L’API d’exportation statique active automatiquement ce format, vous n’avez donc pas besoin de modifier la configuration de votre documentation.

Les déploiements avec CloudFront préservent la navigation côté client et le préchargement des pages. Les hébergeurs de fichiers simples prennent en charge la navigation par pages complètes grâce aux liens `.html` générés. Les sites de production hébergés normalement dans le cloud restent inchangés.

La navigation interne intégrée utilise également des destinations en `.html`. Cela concerne notamment les sélecteurs de version et de langue, les résultats de recherche, les cartes, les citations de l’assistant et les liens du playground d’API. Les chaînes de requête sont conservées. Les liens du playground d’API continuent donc de fonctionner.

<Note>
  Les URL canoniques et celles du sitemap restent sans extension. Vos URL personnalisées de navigation côté client peuvent également rester sans extension. Les déploiements reposant sur CloudFront les résolvent, mais les hébergeurs qui servent uniquement des fichiers peuvent nécessiter des réécritures.
</Note>
...
<Steps>
  ...
  <Step title="Générer le bundle">
    Appelez [Générer le bundle d'exportation](/fr/api/static-export/generate-bundle) avec le `jobId`. L'API empaquète l'exportation dans une seule archive et renvoie `bundleUrl`, un lien S3 présigné vers le bundle d'exportation statique, ainsi qu'un horodatage `expiresAt`. Téléchargez le bundle avant `expiresAt` — passé ce délai, rappelez [Générer le bundle d'exportation](/fr/api/static-export/generate-bundle) pour obtenir un nouveau lien. La tâche sous-jacente et ses fichiers restent réutilisables ; seul le lien est limité dans le temps.
  </Step>
</Steps>
...
Mintlify auto-hébergé se déploie avec le chart Helm du dépôt [`mintlify/enterprise`](https://github.com/mintlify/enterprise). Une fois qu'une tâche d'exportation statique produit un bundle, vous pointez le chart vers le bundle et le déploiement le sert depuis votre propre infrastructure.
