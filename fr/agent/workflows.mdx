...
| **Automerge** | Non | Désactivé par défaut, ce qui ouvre une pull request (demande de fusion) pour révision. Si activé, ouvre une pull request (demande de fusion) et la fusionne automatiquement. |
| **Notify** | Non | Configuration des notifications. Envoyez des messages Slack lorsque les workflows sont terminés. |

Pour les référentiels GitHub, vous devez avoir installé la GitHub App Mintlify sur chaque dépôt répertorié dans les champs de contexte ou de déclencheur. Ajoutez de nouveaux dépôts sur la page [GitHub app](https://dashboard.mintlify.com/settings/organization/github-app) de votre dashboard Mintlify.
...
Spécifiez chaque référentiel au format `owner/repo`. Vous pouvez facultativement spécifier une branche à surveiller. Si vous ne spécifiez pas de branche, le workflow se déclenche lorsque des modifications sont poussées vers la branche par défaut du référentiel.
...
Ces exemples de prompts illustrent des cas d'utilisation courants de workflows. Créez un workflow dans le dashboard et collez le prompt dans le champ d'instructions.
...
**Déclencheur :** Lors d'une poussée vers `your-org/your-product` (branche main)
...
**Déclencheur :** Lors d'une poussée vers `your-org/your-docs` (branche main)
...
**Déclencheur :** Planification cron `0 9 * * 3` (les mercredis à 9 h 00 UTC)
...
**Déclencheur :** Planification cron `0 9 * * 1` (les lundis à 9 h 00 UTC)
