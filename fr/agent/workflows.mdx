...
Pour les référentiels GitLab, connectez vos projets GitLab via OAuth sur la page [GitLab OAuth](https://dashboard.mintlify.com/settings/organization/gitlab-oauth) de votre dashboard Mintlify. Vous pouvez également utiliser l’[intégration GitLab](/fr/deploy/gitlab) avec un jeton d’accès et un webhook.
...
<Note>
  ...
  Pour les référentiels GitLab, l'automerge utilise les identifiants de votre connexion [GitLab OAuth](https://dashboard.mintlify.com/settings/organization/gitlab-oauth) ou le jeton d'accès configuré dans votre [intégration GitLab](/fr/deploy/gitlab). Assurez-vous que le compte ou le jeton dispose de permissions suffisantes pour fusionner les merge requests sur la branche cible.
</Note>
