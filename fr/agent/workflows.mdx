...
4. Dans GitHub, examinez et fusionnez la pull request (demande de fusion) qui ajoute le fichier de workflow à votre référentiel.
...
Par défaut, l'agent ouvre une pull request pour chaque exécution de workflow afin que vous puissiez examiner les modifications avant leur mise en production. Activez `automerge: true` pour fusionner automatiquement la pull request après sa création.

```yaml
automerge: true
```

Lorsque `automerge` est activé, le workflow crée toujours une pull request, exécute les vérifications configurées, puis la fusionne. Cela vous permet de conserver un historique des modifications dans les pull requests de votre référentiel tout en automatisant l'étape de fusion.
