...
L'agent s'exécute dans un bac à sable isolé avec un accès Internet restreint. Lors de la rédaction de prompts, ne faites référence qu'aux outils disponibles pour l'agent.

- Utilitaires shell standards (`grep`, `sed`, `awk`, `curl` et autres)
- `git` et la CLI GitHub (`gh`)
- La CLI Mintlify (`mint`)
- Node.js et Bun

L'agent ne peut pas installer de packages ou d'outils supplémentaires lors de l'exécution. Les registres de packages et autres services externes ne sont pas accessibles depuis le bac à sable.
