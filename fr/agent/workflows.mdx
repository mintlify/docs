...
| `automerge` | Non | La valeur par défaut est `false`, ce qui ouvre une pull request pour révision. Si `true`, ouvre une pull request et la fusionne automatiquement. |

Vous devez avoir installé la GitHub App Mintlify sur chaque référentiel répertorié dans les champs `context` ou `on.push.repo`. Ajoutez de nouveaux référentiels sur la page [GitHub app](https://dashboard.mintlify.com/settings/organization/github-app) de votre Dashboard Mintlify.
