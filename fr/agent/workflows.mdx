...
3. Configurez le déclencheur, le prompt et les autres paramètres, puis cliquez sur **Create workflow**.
...
Si vous avez installé la [CLI Mintlify](/fr/installation), exécutez la commande suivante depuis votre référentiel de documentation pour créer un fichier de workflow de manière interactive dans la CLI.

```bash
mint workflow
```

La CLI vous demande des informations sur le workflow et crée un fichier `.md` dans le répertoire `.mintlify/workflows/`. Validez (commit) et poussez (push) le fichier pour activer le workflow.

<Tip>
  Si vous exécutez `mint workflow` dans un environnement non interactif tel qu'un pipeline CI/CD ou un agent de codage IA, la CLI renvoie les instructions d'utilisation et le format du fichier de workflow au lieu des invites interactives.
</Tip>
...
| `automerge` | Non | La valeur par défaut est `false`, ce qui ouvre une pull request (demande de fusion) pour examen. Si `true`, ouvre une pull request et la fusionne automatiquement. |
| `notify` | Non | Configuration des notifications. Envoie des messages Slack lorsque les workflows se terminent. |
...
Envoyez des messages Slack lorsqu'un workflow se termine ou échoue. Les notifications incluent l'état du workflow, un lien vers la pull request et un résumé des modifications.
...
| `channel_ids` | IDs des canaux à notifier. |
| `users` | Noms d'utilisateur, noms d'affichage ou noms réels pour envoyer un DM. |
| `user_ids` | IDs d'utilisateur Slack pour envoyer un DM. |

<Note>
Les notifications Slack nécessitent une installation Slack sur votre organisation. Connectez Slack sur la page [intégration Slack](https://dashboard.mintlify.com/settings/organization/integrations/slack) de votre Tableau de bord.
</Note>
...
S'exécute chaque semaine pour générer un journal des modifications et notifier votre équipe sur Slack lorsque le workflow se termine. Cet exemple montre comment combiner des workflows planifiés avec des notifications Slack.
