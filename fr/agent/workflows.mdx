...
<Info>
  Les workflows sont en bêta. Ils sont actuellement gratuits pour tous les plans pendant la période bêta. Les fonctionnalités, la disponibilité et la tarification des workflows sont susceptibles d'évoluer.
</Info>
...
Vous pouvez avoir jusqu’à 10 workflows actifs. Chaque workflow peut s’exécuter jusqu’à 50 fois par jour. Les exécutions qui échouent ne sont pas comptabilisées dans cette limite.
...
L'agent s'exécute dans un sandbox isolé avec un accès internet restreint. Lors de la rédaction de prompts, ne faites référence qu'aux outils disponibles pour l'agent.

- Utilitaires shell standard (`grep`, `sed`, `awk`, `curl`, et autres)
- `git` et la CLI GitHub (`gh`)
- La CLI Mintlify (`mint`)
- Node.js et Bun

L'agent ne peut pas installer de packages ou d'outils supplémentaires lors de l'exécution. Les registres de packages et autres services externes ne sont pas accessibles depuis le sandbox.

Les prompts qui demandent à l'agent d'exécuter des outils non disponibles peuvent produire des résultats inattendus ou échouer.
