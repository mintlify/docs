---
title: "Workflows"
description: "Automatisez la maintenance de la documentation avec des tâches d’agent planifiées ou déclenchées par des événements."
keywords: ["automation", "automate", "cron", "auto-update"]
---

Les workflows exécutent automatiquement l'agent selon une planification ou lors d'une poussée vers un référentiel. Chaque workflow définit un prompt pour l'agent ainsi qu'un déclencheur indiquant quand l'exécuter.

Lorsqu'un workflow s'exécute, l'agent clone tous les référentiels spécifiés comme contexte, suit le prompt, puis ouvre soit une pull request (demande de fusion), soit pousse directement les modifications vers votre branche de déploiement.
...
<div id="monitor-workflow-runs">
  ## Surveiller les exécutions de workflows
</div>

La page [Workflows](https://dashboard.mintlify.com/products/workflows) du tableau de bord comporte deux onglets : **Historique des exécutions** et **Workflows**.

<div id="run-history">
  ### Historique des exécutions
</div>

L'onglet **Historique des exécutions** est la vue par défaut. Il affiche une liste consolidée des exécutions de tous vos workflows avec les colonnes suivantes :

- **Heure** : date et heure de début et de fin de l'exécution.
- **Workflow** : nom du workflow et événement déclencheur, comme une pull request fusionnée.
- **Statut** : état actuel de l'exécution. Les statuts possibles sont :
  - **En file d'attente** : l'exécution attend de démarrer.
  - **En cours** : l'exécution est en cours.
  - **Révision nécessaire** : l'exécution a terminé et a ouvert une pull request qui nécessite une révision.
  - **PR fusionnée** : la pull request de l'exécution a été fusionnée.
  - **PR fermée** : la pull request de l'exécution a été fermée sans fusion.
  - **Aucune action nécessaire** : l'exécution s'est terminée sans ouvrir de pull request.
  - **Échec** : l'exécution a rencontré une erreur.
- **Résumé** : description de ce que l'exécution a fait, avec des boutons d'action pour voir la pull request, ouvrir les modifications dans l'éditeur ou fermer la pull request.

Par défaut, l'historique des exécutions affiche uniquement les exécutions avec des pull requests ouvertes afin que vous puissiez vous concentrer sur les éléments nécessitant une action. Activez **Afficher toutes les exécutions** pour voir toutes les exécutions historiques, y compris les pull requests fusionnées et fermées.

Utilisez le menu déroulant de filtre par workflow pour afficher les exécutions d'un workflow spécifique.

<div id="workflows-tab">
  ### Onglet Workflows
</div>

L'onglet **Workflows** liste vos workflows actifs. Cliquez sur un workflow pour voir ou modifier sa configuration.
...
Si vous avez installé l’[interface en ligne de commande Mintlify (CLI)](/fr/installation), exécutez la commande suivante depuis votre référentiel de documentation pour créer un fichier de workflow de manière interactive via la CLI.
...
1. Activez **Notify on completion** pour activer les notifications Slack.
2. Recherchez et sélectionnez les canaux que vous souhaitez notifier.
