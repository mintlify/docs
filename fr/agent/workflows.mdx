...
<Note>
  Les workflows ignorent automatiquement les événements push causés par leurs propres pull requests fusionnées. Cela empêche un workflow de se redéclencher lui-même dans une boucle infinie.
</Note>
...
Par défaut, l'agent ouvre une pull request (demande de fusion) pour chaque exécution de workflow afin que vous puissiez examiner les modifications avant leur mise en production. Activez `automerge: true` pour fusionner automatiquement la pull request par squash sans approbation manuelle. Cela vous permet de conserver une trace des modifications dans l'historique des pull requests de votre référentiel tout en automatisant l'étape de fusion.
