...
Pour les référentiels GitHub, vous devez avoir installé la GitHub App Mintlify sur chaque dépôt répertorié dans les champs de contexte ou de déclencheur push. Ajoutez de nouveaux dépôts sur la page [GitHub app](https://dashboard.mintlify.com/settings/organization/github-app) de votre dashboard Mintlify.

Pour les référentiels GitLab, chaque projet doit être connecté via OAuth sur la [page de configuration OAuth GitLab](https://dashboard.mintlify.com/settings/organization/gitlab-oauth). Voir [Configuration GitLab](#gitlab-setup).
...
* Spécifiez le référentiel au format `owner/repo`.
* Spécifiez éventuellement une branche à surveiller. Si vous ne spécifiez pas de branche, le workflow se déclenche lorsque des modifications sont poussées vers la branche par défaut du référentiel.
...
Accordez à l'agent un accès en lecture à des référentiels supplémentaires lors de l'exécution du workflow. Cela est utile lorsque votre prompt doit examiner du code ou du contenu en dehors de votre référentiel de documentation.
...
Par défaut, l'agent ouvre une pull request (demande de fusion) pour chaque exécution de workflow afin que vous puissiez examiner les modifications avant leur mise en production. Activez l'automerge pour fusionner automatiquement la pull request sans approbation manuelle. Cela vous permet de conserver une trace des modifications dans l'historique des pull requests de votre référentiel tout en automatisant l'étape de fusion.
...
<Note>
  ...
  Pour les référentiels GitLab, l'automerge utilise le jeton d'accès configuré dans votre [intégration GitLab](/fr/deploy/gitlab). Assurez-vous que le jeton dispose de permissions suffisantes pour fusionner des merge requests sur la branche cible.
</Note>
...
Modifiez ou supprimez les workflows depuis la page [Workflows](https://dashboard.mintlify.com/products/workflows) de votre dashboard.
...
**Référentiels de contexte :** `your-org/your-docs`
...
**Déclencheur :** Planification cron (`0 9 * * 3` — chaque mercredi à 9 h 00 UTC)
...
**Déclencheur :** Planification cron (`0 9 * * 1` — chaque lundi à 9 h 00 UTC)
...
**Référentiels de contexte :** `your-org/your-product`

**Automerge :** Désactivé

**Notifications Slack :** Notifier le canal `#documentation` et l'utilisateur `tech-writer`
