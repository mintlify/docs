...
L'agent peut lire le code source de n'importe quel dépôt connecté à votre projet Mintlify, même s'il ne s'agit pas de votre dépôt de documentation. Cela lui permet de construire un contexte plus approfondi pour guider les mises à jour de la documentation.

- Pour GitHub, l'agent peut cloner tout dépôt sur lequel la GitHub App Mintlify est installée.
- Pour GitLab, l'agent peut cloner tout projet auquel le compte GitLab connecté a accès.
...
<Note>
  Pour les déploiements connectés à GitLab, l'agent reste lié au déploiement à partir duquel vous avez ouvert le fil. Ouvrez un nouveau fil pour changer de déploiement au lieu de le faire en cours de conversation.
</Note>
