...
3. Cliquez sur le lien, connectez-vous au service tiers et approuvez les autorisations demandées. Après l'autorisation, votre navigateur affiche une page de confirmation indiquant que la connexion est terminée.
...
Mintlify maintient le statut de vos services connectés synchronisé avec Composio. Une connexion est automatiquement marquée comme déconnectée lorsque :

- Le fournisseur tiers expire ou révoque les identifiants.
- Vous supprimez la connexion depuis le tableau de bord Composio.
- Le fournisseur marque la connexion comme inactive.

Si une connexion n'est plus active, la prochaine fois que vous demandez à l'agent d'utiliser ce service, il répond avec un nouveau lien d'autorisation. Cliquez sur le lien, réautorisez le service et l'agent reprend là où vous vous étiez arrêté.
