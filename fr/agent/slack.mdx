...
## Dépôts hébergés par Mintlify

Si votre documentation se trouve dans un dépôt hébergé par Mintlify (l'option par défaut lorsque vous n'avez pas connecté votre propre compte GitHub ou GitLab), l'agent Slack fonctionne sans configuration supplémentaire :

- **Aucune installation de la GitHub App requise** : L'agent utilise l'accès géré par Mintlify à votre dépôt hébergé, vous n'avez donc pas besoin d'installer ni d'autoriser une intégration Git distincte.
- **Les modifications sont publiées directement** : Au lieu d'ouvrir une pull request, l'agent valide les modifications directement sur votre branche de déploiement. Votre site en production est mis à jour dès que les modifications sont déployées.
- **Les mises à jour de statut n'incluent pas les détails du dépôt** : Comme il n'y a pas de dépôt externe à référencer, les messages de progression de l'agent dans Slack se concentrent sur les fichiers et les modifications plutôt que sur un chemin de dépôt.
- **Lien de finalisation** : Lorsque l'agent termine, il publie un lien vers votre site en production afin que vous puissiez examiner la modification publiée.

Pour passer d'un dépôt hébergé par Mintlify à votre propre dépôt GitHub ou GitLab, suivez le guide de migration sur la page [Git Settings](https://dashboard.mintlify.com/settings/deployment/git-settings). Après la migration, l'agent passe au flux standard des pull requests décrit ci-dessus.
