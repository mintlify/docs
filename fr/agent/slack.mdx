...
<Tip>
  Cette page explique comment ajouter l'agent en tant que bot à votre espace de travail Slack. Pour donner aux automatisations l'accès au contenu des canaux Slack en tant que source de données, consultez [Intégrations](/fr/automations/integrations).
</Tip>
...
Pour supprimer l'agent de votre espace de travail Slack :

1. Dans Slack, accédez à **Settings & administration > Manage apps** et ouvrez l'application `mintlify`.
2. Cliquez sur **Remove app** pour la désinstaller de l'espace de travail.

Après la suppression, l'agent cesse de répondre aux mentions et aux messages directs. L'historique des fils existants reste dans Slack, mais l'agent ne peut plus y accéder. Réinstallez l'application depuis votre dashboard Mintlify pour vous reconnecter.
...
- Envoyer des messages directs à l'agent pour l'utiliser en privé afin de mettre à jour votre documentation. Les messages directs fonctionnent dès que l'application est installée.
- Mentionner `@mintlify` dans un canal pour l'utiliser publiquement et de manière collaborative. L'agent ne répond que dans les canaux auxquels il a été invité — exécutez `/invite @mintlify` dans un canal pour l'ajouter.
