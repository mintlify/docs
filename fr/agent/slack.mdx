...
Si vous devez réautoriser l'agent—par exemple pour accorder de nouvelles autorisations après une mise à jour des scopes Slack ou pour récupérer après un jeton révoqué—réinstallez l'application depuis votre tableau de bord Mintlify.

Lorsque vous réinstallez l'agent dans un espace de travail déjà connecté à votre organisation, Mintlify met à jour le jeton de bot stocké sans le remplacer. Vos canaux, fils et intégrations par utilisateur existants restent intacts.
...
Mintlify synchronise le statut de vos services connectés avec Composio. Une connexion est déconnectée lorsque :
...
Si une connexion n'est plus active, la prochaine fois que vous demanderez à l'agent d'utiliser ce service, il répondra avec un lien d'autorisation. Cliquez sur le lien, réautorisez le service et l'agent reprendra la connexion.
...
Posez à l'agent des questions sur votre produit ou votre documentation et recevez des réponses dans le fil.
