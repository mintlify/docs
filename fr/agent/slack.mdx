...
Pour retirer l'agent de votre espace de travail Slack, ouvrez la page [Automations](https://app.mintlify.com/products/automations) de votre dashboard et cliquez sur **Disconnect Slack**. Cette action révoque le bot token stocké et empêche l'agent de répondre dans Slack. Un propriétaire de l'espace de travail Slack peut également désinstaller l'application `mintlify` depuis la page **Manage apps** de Slack, ce qui produit le même effet.

Une reconnexion ultérieure restaure l'agent sans recréer vos canaux ni l'historique des fils de discussion.
