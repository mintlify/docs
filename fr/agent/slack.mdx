...
Les intégrations que vous connectez depuis Slack sont définies au niveau de votre utilisateur Slack individuel : chaque coéquipier configure ses propres connexions et l'agent utilise les connexions de la personne qui a envoyé le message. Si un utilisateur Slack n'a pas connecté un service, l'agent se rabat sur les intégrations au niveau de l'organisation installées depuis la page [Integrations](https://dashboard.mintlify.com/settings/organization/integrations) du tableau de bord.

Pour installer, supprimer ou réautoriser une intégration au niveau de l'organisation, allez dans **Settings** > **Workspace** > **Integrations**. Connectez un service pour le rendre disponible à l'agent dans tout l'espace de travail, ou déconnectez un service pour révoquer son accès. Seuls les toolkits qui prennent en charge l'authentification gérée apparaissent dans la liste, donc chaque intégration affichée peut être connectée directement depuis le tableau de bord.
