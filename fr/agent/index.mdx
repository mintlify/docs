...
1. Ouvrez la page [Agent](https://app.mintlify.com/products/agent) dans votre dashboard.
2. Installez la GitHub App Mintlify sur les dépôts avec lesquels vous souhaitez que l'agent travaille.
3. Envoyez votre première requête depuis le panneau de l'agent, depuis Slack, ou en ouvrant une pull request dans un dépôt connecté.
4. Relisez la pull request ouverte par l'agent et fusionnez-la lorsque vous êtes prêt.

<div id="how-the-agent-stays-safe">
  ## Comment l'agent reste sûr
</div>

- N'ouvre que des pull requests. L'agent ne fait jamais de commit directement sur votre branche de déploiement.
- N'accède qu'aux dépôts que vous connectez via la GitHub App Mintlify.
- Traite les dépôts qui ne sont pas de documentation en lecture seule, en les utilisant comme contexte sans jamais y écrire.
- Nécessite une relecture et une fusion humaines avant qu'un changement n'atteigne votre site en production.
