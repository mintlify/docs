...
Si la bannière Transcend n’apparaît pas ou si l’analytique ne se charge jamais, vérifiez les points suivants :

- **Identifiant de bundle** : Vérifiez que le `bundleId` de votre `docs.json` correspond au déploiement dans votre tableau de bord Transcend. Une faute de frappe ou un bundle provenant d’un autre environnement empêche Airgap de se charger.
- **Script Airgap** : Ouvrez les outils de développement de votre navigateur et vérifiez, dans l’onglet réseau, la requête vers `https://transcend-cdn.com/cm/<bundleId>/airgap.js`. Si la requête échoue ou renvoie un 404, l’identifiant de bundle est invalide ou le déploiement n’est pas publié dans Transcend.
- **Nom de la finalité** : Si vous avez défini `analyticsPurpose`, assurez-vous qu’il correspond exactement au nom de la finalité dans Transcend, casse et espaces compris. Une finalité qui ne correspond pas signifie que Mintlify attend un consentement que Transcend ne résout jamais.
- **Erreurs dans la console** : Recherchez dans la console du navigateur les erreurs provenant d’`airgap` ou de Transcend. Elles indiquent généralement un bundle mal configuré ou un script bloqué.
- **Content Security Policy** : Si votre site définit une CSP personnalisée, veillez à ce qu’elle autorise `transcend-cdn.com` pour les scripts.
