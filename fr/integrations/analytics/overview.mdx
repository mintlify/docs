...
<Note>
  Les intégrations Analytics ne nécessitent que des clés API publiques, qui sont accessibles à toute personne consultant votre site de documentation. Vos clés API privées, qui sont associées aux clés publiques côté fournisseurs Analytics, doivent rester privées. N’incluez pas de clés API privées dans votre `docs.json`.
</Note>
