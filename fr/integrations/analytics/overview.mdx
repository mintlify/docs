...
Après avoir déployé votre `docs.json` mis à jour, chargez une page de votre site en production et vérifiez que les événements arrivent dans la vue temps réel ou débogage de votre fournisseur. Si les événements n'apparaissent pas, vérifiez que :

- Vous utilisez la clé publique ou l'ID du fournisseur, et non une clé privée ou de serveur.
- Le nom de la propriété dans `docs.json` correspond à celui documenté par votre fournisseur.
- La valeur n'est pas entourée de guillemets supplémentaires ni d'espaces.
