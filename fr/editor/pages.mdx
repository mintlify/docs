...
description: "Créez des pages, modifiez le contenu, téléversez des images et des vidéos, gérez vos fichiers et recherchez dans votre contenu avec l'éditeur web Mintlify."
...
<div id="duplicate-a-page">

### Dupliquer une page

Dupliquez une page lorsque vous souhaitez réutiliser sa structure et son contenu comme point de départ. La copie conserve le corps de la page, le titre, la description et l'icône. Son titre se termine par `(copy)` pour vous permettre de l'identifier et de le renommer.

Pour dupliquer une page, utilisez l'une de ces méthodes :

- Ouvrez la page, cliquez sur **More options** dans la barre supérieure, puis sélectionnez **Duplicate**.
- Dans la vue **Navigation** ou **Files**, faites un clic droit sur la page et sélectionnez **Duplicate**.

Si vous dupliquez la page actuellement ouverte, la copie inclut vos modifications non enregistrées. Lorsque l'original est répertorié, l'éditeur place la copie à côté de celui-ci dans l'arborescence des fichiers et la navigation. Vérifiez la copie, mettez à jour ses paramètres, puis publiez-la.

<Note>
  Vous devez disposer d'un accès en modification et travailler sur une branche autorisant les changements. La duplication est disponible pour les pages Markdown et MDX, sans configuration supplémentaire.
</Note>
