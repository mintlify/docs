...
S'il n'y a aucune modification en attente ou si une pull request (demande de fusion) est déjà ouverte pour la branche actuelle, le bouton **Publish** n'est pas cliquable.

<Note>
  Votre site de documentation en production se met à jour après que Mintlify a compilé et déployé vos modifications publiées. Cela prend généralement entre 30 secondes et quelques minutes. Consultez le statut du déploiement de vos modifications sur votre [tableau de bord](https://dashboard.mintlify.com).
</Note>
...
Les conflits surviennent lorsque vous tentez de fusionner des branches comportant des modifications incompatibles sur les mêmes fichiers.

- Vous et un autre membre de l'équipe modifiez les mêmes lignes dans un fichier sur des branches différentes.
- Vous déplacez, renommez ou supprimez des fichiers sur une branche et les modifiez différemment sur une autre branche.

<div id="resolve-conflicts-steps">
  ### Résoudre les conflits
</div>
