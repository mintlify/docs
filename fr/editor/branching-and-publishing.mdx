...
1. Dans le menu de publication, cliquez sur l'URL d'aperçu. Le format de l'URL est `organization-branch-name.mintlify.site`.
    <Frame>
...
L'éditeur affiche un avertissement lorsque des conflits bloquent la publication. Pour les résoudre :

1. Ouvrez l'avertissement de conflit dans le menu de publication pour voir la liste des fichiers en conflit.
2. Cliquez sur un fichier pour l'ouvrir. L'éditeur affiche vos modifications aux côtés des modifications entrantes de la branche de déploiement.
3. Pour chaque conflit, choisissez **Keep mine**, **Keep theirs**, ou modifiez la section manuellement pour combiner les deux.
4. Enregistrez le fichier pour effacer le conflit, puis publiez à nouveau.
...
Pour annuler une modification après sa publication, revenez sur le commit dans votre fournisseur Git :

- **GitHub** : Ouvrez la pull request fusionnée ou le commit dans le référentiel et cliquez sur **Revert**. Fusionnez la pull request d'annulation pour publier le rollback.
- **GitLab** : Ouvrez la merge request ou le commit et cliquez sur **Revert**. Fusionnez le commit d'annulation résultant.

L'éditeur synchronise automatiquement l'annulation, et votre site en production se redéploie avec le contenu précédent.
