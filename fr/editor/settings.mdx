---
title: "Paramètres de l'éditeur pour l'apparence, la publication et le chat"
description: "Configurez l'apparence de l'éditeur, les consignes pour les commits et les pull requests, les pull requests en brouillon, la méthode de fusion par défaut et les instructions du chat IA de l'éditeur web Mintlify."
keywords: ["éditeur", "paramètres", "apparence", "navigation", "aperçu", "ai", "chat", "instructions", "publication", "pull request", "commit", "brouillon", "fusion"]
---

L'éditeur web comporte deux niveaux de paramètres :

- **Vos paramètres** ne s'appliquent qu'à vous et contrôlent l'apparence de l'éditeur et la façon dont le chat IA vous assiste pendant vos modifications.
...
Les préférences d'apparence sont liées à votre compte utilisateur, donc chaque membre de l'équipe gère les siennes.

<div id="default-sidebar-view">
  ### Vue par défaut de la barre latérale
</div>

Choisissez ce que la barre latérale de l'éditeur affiche à l'ouverture :

- **File explorer** : Une arborescence à plat de tous les fichiers de votre dépôt. Utilisez cette vue pour modifier des fichiers qui ne sont pas du contenu, comme des snippets, des images ou des fichiers de configuration.
- **Navigation** : La structure de navigation définie dans `docs.json`. Utilisez cette vue pour réorganiser des pages, groupes, onglets, anchors ou menus.

Vous pouvez basculer entre les vues à tout moment grâce au commutateur en haut de la barre latérale, ou appuyez sur <kbd>Shift</kbd> + <kbd>Ctrl/Cmd</kbd> + <kbd>F</kbd>.

<div id="full-width-editor">
  ### Éditeur pleine largeur
</div>

Activez cette option pour étendre la zone d'édition à toute la largeur disponible. Utilisez-la lorsque vous voulez plus de place pour des tableaux, des blocs de code ou des diagrammes larges. Laissez-la désactivée pour conserver une largeur de contenu proche du rendu des pages sur le site publié.

<div id="open-live-preview-in-a-new-tab">
  ### Ouvrir l'aperçu en direct dans un nouvel onglet
</div>

Activez cette option pour que le bouton <Icon icon="play" /> d'aperçu en direct ouvre un onglet de navigateur séparé au lieu d'un panneau intégré à l'éditeur. Utilisez un nouvel onglet pour garder l'aperçu visible sur un second écran ou pendant que vous naviguez entre les pages de l'éditeur. Voir [Aperçu en direct](/fr/editor/live-preview) pour plus de détails.

<div id="ai-chat-instructions">
  ## Instructions du chat IA
</div>

Les instructions du chat IA sont des consignes persistantes que l'éditeur envoie à l'IA avec votre requête. Utilisez-les pour fixer des règles de style et de ton que vous ne voulez pas répéter à chaque fois, comme la voix, la terminologie ou les conventions de mise en forme.
