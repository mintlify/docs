...
<Warning>
  Si votre site nécessite une [authentification](/fr/deploy/authentication-setup), `llms.txt` et `llms-full.txt` sont servis derrière l'authentification. La plupart des outils d'IA et des LLM ne peuvent pas s'authentifier sur votre site et ne peuvent donc pas accéder à ces fichiers. Cela signifie que les outils d'IA comme ChatGPT, Claude et Cursor ne pourront pas découvrir ni indexer votre documentation.

  Il n'est pas possible de rendre `llms.txt` ou `llms-full.txt` publics sur un site authentifié. Si vous avez besoin que les outils d'IA indexent votre documentation, envisagez d'utiliser une [authentification partielle](/fr/deploy/authentication-setup#make-pages-public) pour garder certaines pages publiques au lieu d'authentifier entièrement votre site.

  Les fichiers excluent également les pages qui appartiennent à des [groupes d'utilisateurs](/fr/deploy/authentication-setup#control-access-with-groups).
</Warning>
