...
<Note>
  Le comportement de `llms.txt` et `llms-full.txt` dépend de la configuration de votre site :

  - **Sites publics** : Les deux fichiers sont accessibles publiquement et listent toutes les pages de votre documentation.
  - **Sites partiellement authentifiés** (certaines pages sont publiques) : Les deux fichiers sont accessibles publiquement et ne listent que les pages publiques.
  - **Sites entièrement authentifiés** : `llms.txt` renvoie 404. Les utilisateurs authentifiés peuvent récupérer `llms-full.txt` pour s'en servir comme contexte, et le fichier omet toutes les pages restreintes à des [groupes d'utilisateurs](/fr/deploy/authentication-setup#control-access-with-groups) spécifiques ou masquées de la recherche.

  Sur les sites avec authentification, les liens des spécifications OpenAPI et AsyncAPI sont omis de `llms.txt`.
