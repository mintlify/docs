---
title: "llms.txt"
description: "Optimisez votre documentation pour permettre aux LLM de la lire et de l’indexer."
...
* **Liens vers les spécifications API** vers vos spécifications OpenAPI et AsyncAPI (si configurées).
...
Si votre projet inclut des spécifications [OpenAPI](/fr/api-playground/openapi-setup) ou [AsyncAPI](/fr/api-playground/asyncapi-setup), Mintlify inclut automatiquement des liens vers ces spécifications dans votre fichier `llms.txt`. Cela permet aux LLM d'accéder à l'ensemble de votre schéma API pour des réponses plus précises concernant vos endpoints, paramètres et types de données.

Les fichiers de spécifications sont dédupliqués par nom de fichier, le chemin le plus court étant prioritaire lorsque plusieurs fichiers partagent le même nom dans différents répertoires.
