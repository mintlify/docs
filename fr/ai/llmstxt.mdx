...
<Note>
  Comment l'authentification affecte `llms.txt` et `llms-full.txt` :

  - **Sites publics** : Les deux fichiers sont accessibles publiquement et incluent toutes les pages.
  - **Sites partiellement authentifiés** (certaines pages marquées `public: true`) : Les deux fichiers sont accessibles publiquement et incluent uniquement les pages publiques. Les pages qui appartiennent à des [groupes d'utilisateurs](/fr/deploy/authentication-setup#control-access-with-groups) ou qui nécessitent une authentification sont exclues.
  - **Sites entièrement authentifiés** (toutes les pages protégées) : Les deux fichiers renvoient un `404` et ne sont pas servis. Cela évite de divulguer les titres et les descriptions des pages aux robots d'indexation non authentifiés.
