...
<Note>
  Le fichier `llms.txt` généré automatiquement est limité à 100 000 caractères. Lorsqu'un fichier approche de cette limite, Mintlify raccourcit d'abord les descriptions des pages, puis supprime des entrées et ajoute une note indiquant ce qui a été omis. Cette limite ne s'applique pas à `llms-full.txt`. Pour lister toutes les pages sans troncature, ajoutez un [fichier `llms.txt` personnalisé](#custom-files) à la racine de votre projet.
</Note>
