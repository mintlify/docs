...
<Note>
  Le comportement de `llms.txt` et `llms-full.txt` dépend de votre configuration d'authentification :

  - **Sites publics** : Les deux fichiers sont accessibles publiquement et incluent toutes les pages.
  - **Sites partiellement authentifiés** (sites ayant au moins une page `public: true`) : Les deux fichiers sont accessibles publiquement, mais ne listent et n'incluent que le contenu des pages publiques. Les spécifications OpenAPI et AsyncAPI sont exclues.
  - **Sites entièrement authentifiés** (aucune page publique) : `llms.txt` n'est servi qu'aux utilisateurs authentifiés et exclut les pages restreintes à des [groupes d'utilisateurs](/fr/deploy/authentication-setup#control-access-with-groups), ainsi que les spécifications OpenAPI et AsyncAPI. `llms-full.txt` n'est pas disponible.
