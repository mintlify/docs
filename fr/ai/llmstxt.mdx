...
<Note>
  La façon dont `llms.txt` et `llms-full.txt` sont servis dépend de votre configuration d'authentification :

  - **Sites publics** : Les deux fichiers sont disponibles publiquement et incluent toutes les pages.
  - **Sites partiellement authentifiés** (certaines pages marquées [`public`](/fr/deploy/authentication-setup#make-pages-public)) : Les deux fichiers sont accessibles publiquement sans authentification, mais n'incluent que les pages publiques. Les spécifications OpenAPI et AsyncAPI sont omises.
  - **Sites entièrement authentifiés** (aucune page publique) : `llms.txt` n'est servi qu'après authentification et exclut toute page appartenant à un [groupe d'utilisateurs](/fr/deploy/authentication-setup#control-access-with-groups). `llms-full.txt` renvoie un 404 et n'est pas disponible. Les spécifications OpenAPI et AsyncAPI sont omises de `llms.txt`.

  Les LLM et les outils d'IA qui ne peuvent pas s'authentifier sur votre site ne peuvent pas accéder au contenu protégé.
