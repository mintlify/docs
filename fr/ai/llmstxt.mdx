...
Utilisez `llms.txt` et `llms-full.txt` à des fins différentes :

- **`llms.txt`** est un index léger de votre documentation. Les outils d'IA l'utilisent pour découvrir quelles pages existent et ne récupérer que celles qui sont pertinentes pour la requête d'un utilisateur. À privilégier pour les agents capables de suivre les liens et souhaitant limiter l'utilisation du contexte.
- **`llms-full.txt`** est le texte complet de chaque page concaténé dans un seul fichier. À privilégier lorsque vous souhaitez charger l'intégralité de votre documentation dans le contexte d'un outil d'IA en une seule requête, par exemple pour de l'évaluation, de l'analyse hors ligne ou pour des outils qui ne suivent pas les liens.
