...
<Note>
  L'authentification influe sur la façon dont `llms.txt` et `llms-full.txt` sont servis :

  - **Sites entièrement authentifiés** : les deux fichiers nécessitent une authentification pour être consultés. Les LLM et les outils d'IA qui ne peuvent pas s'authentifier sur votre site ne peuvent pas accéder à ces fichiers.
  - **Sites partiellement authentifiés** : les deux fichiers sont accessibles publiquement sans authentification, mais leur contenu est limité à vos pages publiques. Les pages qui appartiennent à des [groupes d'utilisateurs](/fr/deploy/authentication-setup#control-access-with-groups) sont exclues.
