...
<Note>
  Aucun fichier d'index `llms.txt` ne peut dépasser 100 000 caractères. Lorsque l'index complet dépasserait ce budget, Mintlify le divise en fichiers d'index enfants scopés au lieu de supprimer des pages. Toutes les pages restent accessibles via les index divisés. Cette limite ne s'applique pas à `llms-full.txt`.
</Note>

### Index scopés pour les sites volumineux

Lorsque l'index complet dépasse le budget de caractères, Mintlify partitionne votre documentation en index scopés :

- Chaque index scopé est servi à l'adresse `/_llms/<scope>.md`, où `<scope>` identifie le sous-arbre qu'il couvre.
- Mintlify divise d'abord par sous-arbre de navigation (groupes, onglets et autres conteneurs structurels de votre `docs.json`). Les grandes sections plates sans sous-structure sont ensuite divisées par segment de chemin d'URL, afin qu'aucun index ne dépasse le budget.
- Le fichier `llms.txt` racine ajoute une section `## Indexes` qui répertorie chaque index scopé accessible directement depuis la racine, avec un fil d'Ariane, le nombre de pages uniques et la description de chaque index. Ce répertoire est précédé d'une instruction indiquant aux outils d'IA de suivre les liens de manière récursive jusqu'aux pages de documentation.
- Les index scopés suivent le même format que le fichier racine, de sorte qu'un index scopé peut lui-même contenir une autre section `## Indexes`. Les outils d'IA doivent parcourir ces liens de manière récursive pour atteindre chaque page.

```mdx Exemple de section Indexes dans llms.txt
