...
<Note>
  Si votre site est entièrement authentifié, `llms.txt` et `llms-full.txt` sont également soumis à une authentification pour être consultés. Les LLM et les outils d'IA qui ne peuvent pas s'authentifier sur votre site ne peuvent pas accéder à ces fichiers. Si votre site est partiellement authentifié et comporte au moins une page publique, `llms.txt` et `llms-full.txt` sont accessibles publiquement. Les fichiers excluent les pages qui appartiennent à des [groupes d'utilisateurs](/fr/deploy/authentication-setup#control-access-with-groups).
