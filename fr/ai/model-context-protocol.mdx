...
Votre serveur MCP expose des outils permettant aux applications d'IA de rechercher dans votre documentation et de parcourir un système de fichiers en lecture seule de vos pages de documentation. Vos utilisateurs doivent connecter votre serveur MCP à leurs outils.

<div id="how-mcp-servers-work">
  ### Fonctionnement des serveurs MCP
</div>

Lorsqu'une application d'IA se connecte à votre serveur MCP de documentation, elle peut rechercher dans votre documentation et parcourir vos fichiers de documentation directement en réponse à la requête d'un utilisateur, au lieu de s'appuyer sur des informations issues de ses données d'entraînement ou d'effectuer une recherche générique sur le web. Votre serveur MCP donne accès à tout le contenu indexé de votre site de documentation.
...
- **Search** : Recherche dans votre documentation pour trouver du contenu pertinent, en renvoyant des extraits avec des titres et des liens. Utilisez cet outil pour les requêtes larges ou conceptuelles.
- **Query docs filesystem** : Exécute des commandes shell en lecture seule sur un système de fichiers virtuel contenant vos pages de documentation sous forme de fichiers `.mdx` et vos spécifications OpenAPI. Les agents IA utilisent cet outil pour lire des pages complètes (`head -80 /quickstart.mdx`), explorer la structure de la documentation (`tree / -L 2`), rechercher des mots-clés exacts (`rg -il "rate limit" /`) et lire plusieurs pages en un seul appel. Chaque appel est sans état et aucune donnée n'est écrite.
...
- **`command`** : Une commande shell à exécuter sur le système de fichiers virtuel de documentation. Par exemple, `head -80 /api-reference/create-customer.mdx` pour lire une page, `tree / -L 2` pour lister la structure des répertoires, ou `rg -il "keyword" /` pour rechercher dans le contenu des fichiers.
