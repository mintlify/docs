...
<Steps>
  ...
  <Step title="Ajouter des domaines de redirection">
    ...
    Lorsque vous ajoutez un nom d’hôte sans protocole comme `claude.ai`, le protocole HTTPS est utilisé par défaut. Pour autoriser un protocole personnalisé, incluez l’URL complète avec le protocole, par exemple `custom-app://callback.example.com`. Ceci est utile pour les outils d’IA natifs ou de bureau qui utilisent des schémas d’URL personnalisés pour les callbacks OAuth.
