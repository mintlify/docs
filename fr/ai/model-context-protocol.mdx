...
Si votre documentation utilise une authentification partielle avec des pages publiques et des pages protégées, vous disposez de deux endpoints pour le serveur MCP :

- `/mcp` : Ne nécessite pas d’authentification. Renvoie uniquement le contenu public. Partagez ce endpoint avec les utilisateurs qui ont besoin d’accéder au contenu public.
- `/authed/mcp` : Nécessite toujours une authentification. Renvoie le contenu selon les permissions de chaque utilisateur en fonction de ses [groupes d’utilisateurs](/fr/deploy/authentication-setup). Partagez ce endpoint avec les utilisateurs qui ont besoin d’accéder au contenu protégé.

Le endpoint `/authed/mcp` utilise son propre flux OAuth à `/authed/mcp/oauth/*`. Les domaines de redirection configurés pour votre serveur MCP s’appliquent à la fois à `/mcp` et à `/authed/mcp`.
