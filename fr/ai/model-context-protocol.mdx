...
<Tip>
  Pour permettre à des agents de confiance de modifier votre contenu, utilisez le [serveur Admin MCP](/fr/ai/mintlify-mcp). Pour fournir aux agents de programmation une source de récupération unique sur tous les sites Mintlify, utilisez [Mintlify Index](/fr/search-index).
</Tip>
...
<Info>
  WebMCP nécessite une [offre Pro ou Enterprise](https://mintlify.com/pricing?ref=webmcp).
</Info>

WebMCP enregistre vos outils MCP directement sur la page en utilisant l'API `navigator.modelContext` du navigateur. Les navigateurs et agents dans le navigateur compatibles avec WebMCP peuvent appeler ces outils pendant qu'un utilisateur consulte votre site, sans se connecter d'abord à votre serveur Search MCP hébergé.

WebMCP expose cinq outils :

- **`search_docs`** : Recherche sur votre site et renvoie des extraits avec des URL absolues.
- **`read_page`** : Renvoie le contenu Markdown d'une page spécifique.
- **`get_site_overview`** : Renvoie le fichier `llms.txt` de votre site.
- **`read_skill`** : Renvoie le contenu d'un [fichier skill.md](/fr/ai/skillmd).
- **`navigate_page`** : Navigue l'onglet actuel vers une autre page de votre site.

WebMCP ne nécessite aucune configuration. Mintlify enregistre automatiquement ces outils pour les sites éligibles lorsque le navigateur d'un visiteur prend en charge la spécification WebMCP.
...
Pour la compatibilité avec les agents, le même document de découverte est également disponible à l’adresse `/.well-known/mcp.json`. Mintlify expose en plus une carte de serveur MCP à `/.well-known/mcp/server-card.json` et une liste de cartes de serveur à `/.well-known/mcp/server-cards.json`. Mintlify sert tous les endpoints de découverte automatiquement et aucune configuration n’est requise.

<div id="server-card-endpoints">
  #### Endpoints des cartes de serveur
</div>

`GET /.well-known/mcp/server-card.json` renvoie une seule carte de serveur décrivant votre serveur MCP. `GET /.well-known/mcp/server-cards.json` renvoie un tableau `servers` contenant une carte par endpoint disponible (inclut toujours une carte de serveur publique, ainsi qu’une carte de serveur authentifiée lorsque votre site utilise l’authentification).
  ...
  <Tab title="Cursor">
    <Steps>
      ...
      <Step title="Publiez l'URL de votre serveur MCP pour vos utilisateurs">
        ...
        1. Utilisez <kbd>Cmd</kbd> + <kbd>Shift</kbd> + <kbd>P</kbd> (<kbd>Ctrl</kbd> + <kbd>Shift</kbd> + <kbd>P</kbd> sous Windows) pour ouvrir la palette de commandes.
