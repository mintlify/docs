...
L&#39;outil de recherche MCP prend en charge des paramètres de filtrage optionnels que les applications d&#39;IA peuvent utiliser pour affiner les résultats de recherche. Les paramètres disponibles s'adaptent dynamiquement en fonction de la configuration de votre documentation.

**Toujours disponibles :**
* **`apiReferenceOnly`** : Ne renvoie que la documentation de référence de l'API.
* **`codeOnly`** : Ne renvoie que les extraits de code.

**Disponibles lorsqu'ils sont configurés :**
* **`version`** : Filtre les résultats pour une version spécifique de la documentation (par exemple, `'v0.7'`). Disponible uniquement lorsque votre documentation utilise des [versions](/fr/organize/versions). Ne renvoie que le contenu marqué avec la version spécifiée ou le contenu disponible pour toutes les versions.
* **`language`** : Filtre les résultats pour un code de langue spécifique (par exemple, `'en'`, `'zh'` ou `'es'`). Disponible uniquement lorsque votre documentation utilise des [langues](/fr/organize/languages). Ne renvoie que le contenu dans la langue spécifiée ou le contenu disponible dans toutes les langues.

Le schéma de l'outil de recherche MCP inclut dynamiquement uniquement les paramètres applicables à votre déploiement. Les applications d&#39;IA déterminent quand appliquer ces filtres en fonction du contexte de la requête de l&#39;utilisateur.
