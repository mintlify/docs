...
Lorsqu'une application d'IA est connectée à votre serveur MCP de documentation, elle peut rechercher directement dans votre documentation en réponse à la requête d'un utilisateur, plutôt que de s'appuyer sur les informations de ses données d'entraînement ou d'effectuer une recherche générique sur le web. Votre serveur MCP donne accès à tout le contenu indexé de votre site de documentation.

* Les applications d'IA peuvent rechercher de manière proactive dans votre documentation pendant qu'elles génèrent une réponse, même si on ne leur demande pas explicitement de rechercher dans votre documentation.
...
* Chaque recherche, également appelée appel d'outil, a lieu pendant le processus de génération, de sorte que l'application d'IA recherche des informations à jour dans votre documentation pour générer sa réponse.
...
L'outil de recherche MCP prend en charge des paramètres de filtrage optionnels que les applications d'IA utilisent pour affiner les résultats de recherche.
...
* Pour la documentation avec authentification partielle, où certaines pages sont publiques et d'autres protégées, vous devez activer votre serveur MCP avant qu'il ne soit accessible aux utilisateurs. Les utilisateurs non authentifiés peuvent rechercher le contenu public. Les utilisateurs qui s'authentifient peuvent rechercher tout le contenu auquel ils ont accès en fonction de leurs [groupes d'utilisateurs](/fr/deploy/authentication-setup).
* Pour la documentation où toutes les pages requièrent une authentification, vous devez activer votre serveur MCP avant qu'il ne soit accessible aux utilisateurs. Les utilisateurs doivent s'authentifier avant de se connecter à votre serveur MCP. Votre serveur MCP ne recherche que le contenu auquel chaque utilisateur a accès en fonction de ses [groupes d'utilisateurs](/fr/deploy/authentication-setup).
...
<div id="enable-authenticated-mcp">
  ### Activer MCP authentifié
</div>

Si votre documentation requiert une authentification, votre serveur MCP exige que les utilisateurs s'authentifient avant de se connecter. Lorsqu'un utilisateur ajoute l'URL de votre serveur MCP à son outil d'IA, il doit se connecter avec ses identifiants existants. Après l'authentification, une redirection le renvoie vers son outil. Le serveur MCP ne renvoie que le contenu auquel chaque utilisateur a le droit d'accéder en fonction de ses [groupes d'utilisateurs](/fr/deploy/authentication-setup).

Si votre documentation dispose d'une authentification partielle avec des pages publiques et protégées, les utilisateurs peuvent se connecter à votre serveur MCP sans s'authentifier pour accéder au contenu public. Les utilisateurs qui s'authentifient obtiennent l'accès au contenu protégé qui leur est accessible.

Par défaut, votre serveur MCP n'est disponible que pour les outils en localhost. Pour permettre aux outils web de se connecter, ajoutez les domaines de redirection des outils d'IA. Un domaine de redirection est le nom d'hôte qu'un outil d'IA utilise après qu'un utilisateur a terminé l'authentification, comme `claude.ai` ou `app.cursor.ai`. Les outils d'IA de vos utilisateurs ne peuvent pas terminer l'authentification si leur domaine de redirection ne figure pas dans cette liste.

<Steps>
  <Step title="Activez MCP authentifié dans votre Dashboard">
    1. Accédez à la [page du serveur MCP](https://dashboard.mintlify.com/products/mcp) dans votre Dashboard.
    2. Cliquez sur le bouton **Enable MCP Server**.
  </Step>
  <Step title="Ajoutez des domaines de redirection">
    Ajoutez les domaines de redirection des outils d'IA auxquels vous souhaitez accorder l'accès à vos utilisateurs. Les outils d'IA de vos utilisateurs ne peuvent pas terminer l'authentification si leur domaine de redirection ne figure pas dans cette liste. Les domaines de redirection courants incluent `claude.ai` et `vscode.dev/redirect`.
...
Pour la documentation qui requiert une authentification, votre serveur MCP n'indexe que les pages des [groupes d'utilisateurs publics](/fr/deploy/authentication-setup#make-pages-public). Pour la documentation avec authentification partielle, votre serveur MCP indexe les pages publiques pour les utilisateurs non authentifiés et toutes les pages des groupes d'utilisateurs publics.
