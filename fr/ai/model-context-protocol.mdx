...
Par défaut, votre serveur MCP est uniquement disponible pour les outils localhost. Pour autoriser les outils web et les clients de bureau à se connecter, configurez les URIs de redirection OAuth autorisées. Une URI de redirection est l’adresse qu’un outil d’IA utilise une fois l’authentification terminée. Les outils d’IA de vos utilisateurs ne peuvent pas finaliser l’authentification si leur URI de redirection n’est pas autorisée.
...
<Steps>
  ...
  <Step title="Activer les clients prédéfinis">
    Le Dashboard inclut des boutons bascule prédéfinis pour les outils d’IA courants comme Claude Desktop et Cursor. Activez le bouton bascule pour chaque client que vous souhaitez autoriser. Chaque bouton bascule ajoute automatiquement l’URI de redirection appropriée pour ce client.
  </Step>

  <Step title="Ajouter des domaines personnalisés">
    Pour les outils d’IA non listés comme clients prédéfinis, ajoutez leurs domaines de redirection dans la section **Custom Domains**. Les outils d’IA de vos utilisateurs ne peuvent pas finaliser l’authentification si leur domaine de redirection ne figure pas dans cette liste.
