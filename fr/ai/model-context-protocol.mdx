...
<Steps>
  ...
  <Step title="Ajouter des domaines de redirection">
    ...
    Par défaut, les domaines de redirection utilisent `https://`. Pour autoriser un schéma de protocole personnalisé (par exemple, un callback d’application native comme `myapp://`), incluez le protocole complet dans l’entrée du domaine (par exemple, `myapp://callback`). Les protocoles dangereux tels que `javascript:`, `data:` et `file:` sont toujours bloqués.
