...
- `/mcp` : Ne nécessite pas d’authentification. Renvoie uniquement le contenu public. Partagez cet endpoint avec les utilisateurs qui ont besoin d’accéder au contenu public.
- `/authed/mcp` : Nécessite toujours une authentification. Renvoie le contenu limité aux permissions de chaque utilisateur en fonction de ses [groupes d’utilisateurs](/fr/deploy/authentication-setup). Partagez cet endpoint avec les utilisateurs qui ont besoin d’accéder au contenu protégé.

L’endpoint `/authed/mcp` utilise son propre flux OAuth via `/authed/mcp/oauth/*`. Les domaines de redirection configurés pour votre serveur MCP s’appliquent à la fois à `/mcp` et à `/authed/mcp`.
