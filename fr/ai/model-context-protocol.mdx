...
<Steps>
  ...
  <Step title="Ajouter des domaines personnalisés">
    Pour les outils d’IA non listés comme clients prédéfinis, ajoutez leurs domaines de redirection dans la section **Custom Domains**. Saisissez le nom d’hôte du domaine de redirection (par exemple, `example.com`). Les outils d’IA de vos utilisateurs ne peuvent pas finaliser l’authentification si leur domaine de redirection ne figure pas dans cette liste.
