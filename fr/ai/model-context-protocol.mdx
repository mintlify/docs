...
- `/mcp` : Ne nécessite pas d’authentification. Ne renvoie que le contenu public. Partagez-le avec les utilisateurs qui ont besoin d’accéder au contenu public.
- `/authed/mcp` : Nécessite toujours une authentification. Renvoie le contenu limité aux permissions de chaque utilisateur en fonction de ses [groupes d’utilisateurs](/fr/deploy/authentication-setup). Partagez-le avec les utilisateurs qui ont besoin d’accéder au contenu protégé.
