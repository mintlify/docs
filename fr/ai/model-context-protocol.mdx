...
description: "Connectez Claude, Cursor, ChatGPT et d’autres outils d’IA à votre serveur MCP de recherche hébergé pour qu’ils interrogent et récupèrent votre contenu."
keywords: ["MCP", "outils d’IA", "Claude", "Cursor", "recherche"]
---
...
<Tip>
  Pour permettre aux agents de confiance de modifier votre contenu, utilisez le [serveur Admin MCP](/fr/ai/mintlify-mcp). Pour fournir aux agents de codage une source unique permettant de récupérer du contexte à partir de tous les sites Mintlify, utilisez [Mintlify Index](/fr/search-index).
</Tip>
...
- **Submit feedback** : Signale un problème de documentation à votre équipe lorsqu'un agent trouve une page incorrecte, obsolète, confuse ou incomplète. L'agent fournit le chemin de la page et une description du problème, que Mintlify enregistre comme retour négatif dans votre tableau de bord d'analytics.

Les agents déterminent quand utiliser chaque outil en fonction du contexte de la conversation. Par exemple, un agent peut d'abord rechercher sur votre site pour trouver des pages pertinentes, puis utiliser l'outil query docs filesystem pour lire le contenu complet des résultats les plus pertinents. S'il découvre un problème avec la page en cours de route, il peut soumettre un retour.
...
Si votre documentation utilise l'[authentification](/fr/deploy/authentication-setup), les skills avec un champ `groups` ne sont exposés qu'aux utilisateurs finaux authentifiés dont les groupes correspondent. Voir [Restreindre les skills à des groupes d'utilisateurs](/fr/ai/skillmd#restrict-skills-to-user-groups).
...
* Pour un contenu avec authentification partielle, où certaines pages sont publiques et d'autres nécessitent une connexion, vous devez activer votre serveur Search MCP avant que les utilisateurs puissent y accéder. Les utilisateurs non authentifiés peuvent rechercher dans le contenu public. Les utilisateurs qui s'authentifient peuvent rechercher dans tout le contenu auquel ils ont l'autorisation d'accéder en fonction de leurs [groupes d'utilisateurs](/fr/deploy/authentication-setup#control-access-with-groups).
* Pour un contenu où toutes les pages requièrent une authentification, vous devez activer votre serveur Search MCP avant qu'il soit accessible aux utilisateurs. Les utilisateurs doivent s'authentifier avant de se connecter à votre serveur Search MCP. Votre serveur Search MCP recherche uniquement dans le contenu auquel chaque utilisateur a accès en fonction de ses [groupes d'utilisateurs](/fr/deploy/authentication-setup#control-access-with-groups).

Si votre site utilise l'authentification, configurez votre serveur Search MCP hébergé depuis **Settings → Security & access → MCP** dans votre Dashboard. Le serveur Search MCP authentifié est disponible au chemin `/authed/mcp` de l'URL de votre site.

<Note>
  La page de paramètres **MCP** n'apparaît que lorsque vous activez l'[authentification](/fr/deploy/authentication-setup) pour votre site. Si votre site ne nécessite pas d'authentification, votre serveur Search MCP reste accessible au chemin `/mcp` de l'URL de votre site.
</Note>

Mintlify réserve les chemins `/mcp` et `/authed/mcp` aux serveurs Search MCP. Les autres éléments de navigation ne peuvent pas utiliser ces chemins.
...
| `servers` | Serveurs MCP disponibles. Inclut toujours une entrée `public`. Lorsque votre site utilise l’authentification, inclut également une entrée `authenticated`. |
| `servers[].name` | Identifiant de l’entrée du serveur. |
| `servers[].authentication` | Méthode d’authentification de l’entrée du serveur. |

Pour la compatibilité avec les agents, le même document de découverte est également disponible à l’adresse `/.well-known/mcp.json`. Mintlify expose en plus une carte de serveur MCP à `/.well-known/mcp/server-card.json` et une liste de cartes de serveur à `/.well-known/mcp/server-cards.json`. Mintlify sert automatiquement tous les points de terminaison de découverte et aucune configuration n’est requise.

<div id="server-card-endpoints">
  #### Points de terminaison des cartes de serveur
</div>

`GET /.well-known/mcp/server-card.json` renvoie une seule carte de serveur décrivant votre serveur MCP. `GET /.well-known/mcp/server-cards.json` renvoie un tableau `servers` contenant une carte par point de terminaison disponible (inclut toujours une carte de serveur publique, ainsi qu’une carte de serveur authentifiée lorsque votre site utilise l’authentification).
...
```json
{
  ...
  "url": "https://your-docs.com/mcp",
  "transport": "http",
  "description": "Rechercher et récupérer votre documentation",
  "capabilities": {
    "tools": true,
    "resources": true
  },
  ...
  "tools": [
    {
      ...
      "description": "Rechercher dans votre site de documentation.",
      "inputSchema": {
        "type": "object",
        "properties": {
...
| Champ | Description |
| --- | --- |
| `tools[].name` | Nom de l’outil annoncé via MCP. Les noms d’outils sont spécifiques à chaque site ; utilisez donc la valeur renvoyée par le point de terminaison de découverte plutôt que de la coder en dur. |
...
Les outils intégrés de recherche et de requête du système de fichiers de documentation restent en lecture seule et non destructifs, avec les annotations suivantes :
...
Si votre site nécessite une authentification, votre serveur Search MCP demande aux utilisateurs de s'authentifier avant de se connecter. Lorsqu'un utilisateur ajoute l'URL de votre serveur MCP à son outil d'IA, il doit se connecter avec ses identifiants existants. Après l'authentification, une redirection le renvoie vers son outil. Le serveur MCP ne renvoie que le contenu auquel chaque utilisateur est autorisé à accéder en fonction de ses [groupes d'utilisateurs](/fr/deploy/authentication-setup#control-access-with-groups).

Si votre site utilise une authentification partielle avec des pages publiques et des pages protégées, vous disposez de deux points de terminaison de serveur Search MCP :
...
- `/authed/mcp` : Nécessite toujours une authentification. Renvoie le contenu en fonction des permissions de chaque utilisateur selon ses [groupes d’utilisateurs](/fr/deploy/authentication-setup#control-access-with-groups). Partagez-le avec les utilisateurs qui ont besoin d’accéder au contenu protégé.

Le point de terminaison `/authed/mcp` utilise son propre flux OAuth à `/authed/mcp/oauth/*`. Les domaines de redirection configurés pour votre serveur MCP s’appliquent à la fois à `/mcp` et à `/authed/mcp`.

Les clients MCP populaires tels que Claude, ChatGPT, Cursor et Devin sont toujours autorisés à finaliser l’authentification auprès de votre serveur MCP. Vous n’avez pas besoin d’ajouter ces clients à une liste d’autorisations.

Pour permettre à d’autres outils web de se connecter, ajoutez leurs domaines de redirection dans votre Dashboard. Un domaine de redirection est le nom d’hôte qu’un outil d’IA utilise une fois l’authentification terminée. Les outils d’IA de vos utilisateurs ne peuvent pas finaliser l’authentification si leur domaine de redirection ne figure pas dans cette liste ou si l’outil ne fait pas partie des clients toujours autorisés.

Mintlify active automatiquement votre serveur MCP hébergé lorsque votre site a l'[authentification](/fr/deploy/authentication-setup) configurée.

<Steps>
  <Step title="Ouvrir les paramètres du MCP hébergé">
    Dans votre Dashboard, accédez à **Settings → Security & access → MCP**.
  </Step>

  <Step title="Ajouter des domaines de redirection personnalisés">
    Sous **Allowed OAuth redirect URIs**, ajoutez les domaines de redirection des outils d’IA supplémentaires auxquels vous souhaitez donner accès à vos utilisateurs. Vous n’avez besoin d’ajouter que les domaines des clients qui ne figurent pas déjà dans la liste des clients toujours autorisés.
...
Les identifiants client s'authentifient auprès du point de terminaison `/authed/mcp` et renvoient l'ensemble du contenu des pages publiques et des pages authentifiées qui ne sont pas restreintes à des groupes spécifiques.

<Note>
  Les identifiants client MCP sont distincts des [clés d'API Mintlify](/fr/api/introduction). Les clés d'API admin et assistant authentifient les requêtes à l'API REST de Mintlify et ne peuvent pas être utilisées pour s'authentifier auprès de votre serveur MCP. Pour appeler votre serveur MCP de manière programmatique, créez un identifiant client depuis **Settings → Security & access → MCP** dans votre Dashboard et échangez-le contre un jeton d'accès en suivant les étapes ci-dessous.
</Note>

<Steps>
  <Step title="Créer un identifiant client">
    1. Dans votre Dashboard, accédez à **Settings → Security & access → MCP**.
    2. Dans la section **Client credentials**, cliquez sur **Generate**.
    3. Entrez un libellé pour identifier l'objet de l'identifiant, puis confirmez.
    4. Copiez le **client ID** et le **client secret** depuis la boîte de dialogue **Secret generated**. Le client secret n'est affiché qu'une seule fois. Vous ne pourrez plus le récupérer ensuite.
  </Step>
  <Step title="Échanger les identifiants contre un jeton d'accès">
    Envoyez une requête POST au point de terminaison de jeton de votre serveur MCP avec votre client ID et votre secret. Votre point de terminaison de jeton se trouve au chemin `/authed/mcp/oauth/token` de l'URL de votre site.
  ...
  <Step title="Se connecter au serveur MCP">
    Utilisez le jeton d'accès comme bearer token lors de la connexion au point de terminaison `/authed/mcp`.
...
Vous pouvez gérer vos identifiants client depuis **Settings → Security & access → MCP** dans votre Dashboard.

- **Supprimer un identifiant** pour révoquer définitivement l'accès. Cette action est irréversible.
- **Faire tourner un identifiant** en créant un nouvel identifiant, en mettant à jour les intégrations qui utilisent l'ancien avec le nouveau client ID et le nouveau secret, puis en supprimant l'ancien identifiant. Chaque identifiant possède un secret unique ; créer un nouvel identifiant n'affecte donc pas les identifiants existants tant que vous ne les avez pas supprimés.
...
| Search par site (domaine) | 10 000 requêtes par heure | Limite le nombre total d'appels à l'outil de recherche effectués par tous les utilisateurs de votre serveur MCP. |
| Query docs filesystem par site (domaine) | 10 000 requêtes par heure | Limite le nombre total d'appels à l'outil query docs filesystem effectués par tous les utilisateurs de votre serveur MCP. |
| Search authentifié par site (domaine) | 5 000 requêtes par heure | Limite le nombre total d'appels authentifiés à l'outil de recherche effectués par tous les utilisateurs de votre serveur MCP. |
| Query docs filesystem authentifié par site (domaine) | 5 000 requêtes par heure | Limite le nombre total d'appels authentifiés à l'outil query docs filesystem effectués par tous les utilisateurs de votre serveur MCP. |

Si vous dépassez une limite, le serveur MCP renvoie une réponse HTTP `429 Too Many Requests`. Les clients MCP doivent effectuer un back-off et réessayer après un court délai — un back-off exponentiel commençant à quelques secondes est un choix par défaut sûr. Les limites se réinitialisent sur une fenêtre horaire glissante, la plupart des appelants se rétablissent donc automatiquement en moins d'une heure.

Si votre site atteint régulièrement ces limites, contactez [support@mintlify.com](mailto:support@mintlify.com) pour demander des limites plus élevées.
...
<Tabs>
  ...
  <Tab title="Claude">
    <Steps>
      <Step title="Obtenez l'URL de votre serveur MCP">
        L'URL de votre serveur MCP est le chemin `/mcp` de l'URL de votre site. Par exemple, `https://your-docs.com/mcp`. Si votre site nécessite une authentification, utilisez le chemin `/authed/mcp` à la place.
      </Step>
  ...
  <Tab title="Codex">
    <Steps>
      ...
      <Step title="Publiez l'URL de votre serveur MCP pour vos utilisateurs">
        Créez un guide pour vos utilisateurs qui inclut l'URL de votre serveur MCP et la procédure pour le connecter à Codex CLI.
    ...
    Voir la [documentation MCP de Codex](https://developers.openai.com/codex/mcp) pour plus de détails.
  </Tab>
...
<Tabs>
  ...
  <Tab title="Codex">
    Pour connecter le serveur MCP de Mintlify à Codex CLI, ajoutez-le à votre fichier de configuration `~/.codex/config.toml` :
    ...
    Testez la connexion en démarrant une session Codex et en posant la question :

    ```text
    What tools do you have available?
    ```

    Codex devrait alors lister le serveur MCP de Mintlify parmi les outils disponibles.

    Consultez la [documentation MCP de Codex](https://developers.openai.com/codex/mcp) pour plus de détails.
  </Tab>
  ...
  <Tab title="Cursor">
    ...
    Voir [Installer des serveurs MCP](https://docs.cursor.com/en/context/mcp#installing-mcp-servers) dans la documentation de Cursor pour plus de détails.
  </Tab>
