...
Le Model Context Protocol (MCP) est un protocole ouvert qui établit des connexions normalisées entre des applications d’IA et des services externes, tels que la documentation. Mintlify génère un serveur MCP à partir de votre documentation et de vos spécifications OpenAPI, afin de préparer votre contenu à l’écosystème IA plus large, où n’importe quel client MCP (comme Claude, Cursor, Goose et autres) peut se connecter à votre documentation et à vos API.
...
Tous les serveurs MCP incluent par défaut l’outil `search`, qui permet aux utilisateurs d’interroger votre documentation depuis d’autres outils.
...
Pour exposer des endpoints en tant qu’outils MCP, utilisez l’objet `mcp` au sein de l’extension `x-mint`, au niveau du fichier ou de l’endpoint. Par exemple, le serveur MCP de Mintlify inclut des outils pour créer des conversations avec l’Assistant, obtenir des mises à jour de statut et déclencher des mises à jour.

Les serveurs MCP adoptent une approche priorisant la sécurité : les endpoints d’API ne sont pas exposés par défaut. Vous devez explicitement activer les endpoints pour les rendre disponibles en tant qu’outils MCP. N’exposez que les endpoints sûrs pour un accès public via des outils d’IA.
...
<ResponseField name="mcp" type="object">
  ...
  <Expandable title="MCP">
    <ResponseField name="enabled" type="boolean">
      Indique s’il faut exposer l’endpoint en tant qu’outil MCP. Cette option prévaut sur la configuration au niveau du fichier.
    </ResponseField>
...
Activez MCP pour tous les endpoints par défaut dans un fichier de spécification OpenAPI, puis excluez sélectivement certains endpoints :
...
<div id="endpoint-level-configuration">
  ### Configuration au niveau des points de terminaison
</div>

Activez le Model Context Protocol (MCP) pour des points de terminaison spécifiques :
...
<Tabs>
  ...
  <Tab title="Claude">
    ...
    <Steps>
      ...
      <Step title="Accéder au serveur MCP dans votre conversation">
  ...
  <Tab title="Cursor">
    ...
    Voir [Installing MCP servers](https://docs.cursor.com/en/context/mcp#installing-mcp-servers) dans la documentation de Cursor pour en savoir plus.
  </Tab>
...
Lorsque vous activez un endpoint d’API pour le Model Context Protocol (MCP), le serveur inclut les exigences d’authentification définies dans vos éléments `securitySchemes` et `securityRequirement` d’OpenAPI. Toute key est gérée directement par l’outil et n’est ni stockée ni traitée par Mintlify.

Si un utilisateur demande à son outil d’IA d’appeler un endpoint protégé, l’outil lui demandera alors les identifiants d’authentification nécessaires.

<div id="monitoring-your-mcp-server">
  ## Surveiller votre serveur MCP
</div>

Vous pouvez afficher tous les outils MCP disponibles dans la section **Outils disponibles** de la [page MCP Server](https://dashboard.mintlify.com/products/mcp) de votre Dashboard.

<Frame>
  <img src="/images/mcp/mcp-server-page-light.png" alt="Dashboard MCP avec la section Outils disponibles mise en évidence" class="block dark:hidden" />

  <img src="/images/mcp/mcp-server-page-dark.png" alt="Dashboard MCP avec la section Outils disponibles mise en évidence" class="hidden dark:block" />
</Frame>
...
<AccordionGroup>
  <Accordion title="Le serveur MCP n’affiche que l’outil de recherche">
    ...
    3. Consultez vos journaux de déploiement pour détecter des erreurs de traitement OpenAPI.

    Si le traitement OpenAPI échoue, le serveur poursuit avec l’outil de recherche seul afin de préserver la fonctionnalité.
  </Accordion>
  ...
  <Accordion title="Descriptions d’outils manquantes ou peu claires">
    ...
    2. Assurez-vous que les noms de paramètres et leurs descriptions sont explicites.
