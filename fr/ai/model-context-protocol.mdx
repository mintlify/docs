...
Si l'authentification est activée, configurez votre serveur Search MCP hébergé depuis **Settings → Security & access → MCP** dans votre Dashboard. Le serveur Search MCP authentifié est disponible au chemin `/authed/mcp` de l'URL de votre site.
...
Votre serveur MCP hébergé est activé automatiquement lorsque votre site a l'[authentification](/fr/deploy/authentication-setup) configurée. Pour permettre aux outils d'IA de terminer l'authentification, ajoutez leurs domaines de redirection dans votre Dashboard.
