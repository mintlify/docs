...
- **Chat** : Envoyez des instructions à l'agent pour mettre à jour votre documentation. L'agent crée des pull requests (demandes de fusion) en fonction de vos consignes et affiche des liens pour consulter les pull requests ou ouvrir les modifications dans l'éditeur web.
- **History** : Parcourez les conversations passées et continuez à travailler sur les requêtes précédentes. Cliquez sur une conversation pour la charger dans la vue Chat.
...
2. Dans la section d’intégration Slack, cliquez sur **Connect**.
3. Suivez les instructions affichées par Slack pour ajouter l’application `mintlify` à votre espace de travail.
4. Suivez les instructions affichées par Slack pour lier votre compte Mintlify à votre espace de travail Slack.
...
3. Sélectionnez le bouton **Connect** dans la section d’intégration Slack.
...
Utilisez les endpoints de l’agent pour [créer des tâches](/api-reference/agent/create-agent-job), [récupérer une tâche spécifique](/api-reference/agent/get-agent-job) et [récupérer toutes les tâches](/api-reference/agent/get-all-jobs).

Lors de la création de tâches via l’API, vous pouvez contrôler si des pull requests (demandes de fusion) sont créées en mode brouillon à l’aide du paramètre `asDraft` (dont la valeur par défaut est `true`). Définissez `asDraft: false` pour créer des pull requests (demandes de fusion) non brouillon, prêtes pour une revue immédiate et une fusion dans des workflows automatisés.
...
Intégrez l'agent à vos outils d'automatisation existants pour mettre automatiquement à jour la documentation lors de changements de code, déclencher des relectures de contenu ou synchroniser les mises à jour de documentation entre plusieurs dépôts.

Découvrez comment faire dans le tutoriel [Mettre à jour automatiquement la documentation lorsque du code est fusionné](/guides/automate-agent).