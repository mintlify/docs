...
Ajoutez un fichier `AGENTS.md` dans votre référentiel pour personnaliser le comportement de l’agent. L’agent lit ce fichier et applique toutes les instructions que vous y fournissez.

Créez le fichier `AGENTS.md` à la racine de votre référentiel. Si votre projet est un monorepo, créez le fichier à la racine du répertoire de documentation.

Ajoutez toutes les instructions que vous souhaitez que l’agent suive. L’agent ajoute ces instructions à son prompt système, de sorte qu’elles influencent l’ensemble de ses réponses et de ses actions.
