---
title: "Agent"
description: "Automatisez les mises à jour de la documentation avec l'agent. Créez des pull requests à partir de messages Slack, de pull requests ou d'appels à l'API."
keywords: ["automatisation", "automatiser", "intégration Slack", "maintenance"]
---

<Info>
  L&#39;agent est disponible sur les [offres Pro et Custom](https://mintlify.com/pricing?ref=agent) pour toute personne ayant accès à votre Dashboard.
</Info>

L&#39;agent crée des pull requests avec des modifications proposées pour votre documentation à partir de vos prompts. Lorsque vous envoyez une demande à l&#39;agent, il s&#39;appuie sur votre documentation, les dépôts connectés et les messages Slack pour créer du contenu conforme aux meilleures pratiques de rédaction technique et au schéma Mintlify. Accédez à l&#39;agent directement dans votre Dashboard, dans votre espace de travail Slack, ou intégrez-le dans des applications personnalisées via l&#39;API.

Utilisez l&#39;agent pour :

* Rédiger de nouveaux contenus à partir de vos prompts, de liens vers des pull requests ou de discussions Slack
* Réviser des exemples de code et des références d&#39;API obsolètes
...
Accédez au panneau de l&#39;agent directement depuis votre Dashboard à l&#39;aide du raccourci clavier <kbd>⌘</kbd>+<kbd>I</kbd> (Mac) ou <kbd>Ctrl</kbd>+<kbd>I</kbd> (Windows/Linux), ou en cliquant sur le bouton **Ask agent**.
...
* **History** : Affichez et poursuivez les conversations précédentes.
* **Settings** : Configurez les autorisations de la GitHub App et l&#39;intégration Slack.

Lorsque l&#39;agent apporte des modifications, vous pouvez afficher la pull request (demande de fusion) directement depuis le chat ou ouvrir les modifications dans l&#39;éditeur web.

<div id="add-the-agent-to-your-slack-workspace">
  ## Ajouter l’agent à votre espace de travail Slack
</div>

Vous pouvez également utiliser l’agent dans Slack pour collaborer avec votre équipe sur les mises à jour de la documentation.

<Note>
  Si le propriétaire de l’espace de travail Slack exige l’approbation d’un administrateur pour installer des applications, demandez-lui d’approuver l’application Mintlify avant de la connecter.
</Note>

1. Ouvrez le panneau de l’agent dans votre Dashboard et accédez à **Settings**.
2. Sélectionnez le bouton **Connect** dans la section d’intégration Slack.
3. Suivez les instructions de Slack pour ajouter l’application `mintlify` à votre espace de travail.
4. Suivez les instructions de Slack pour lier votre compte Mintlify à votre espace de travail Slack.
5. Vérifiez que l’agent fonctionne correctement et répond lorsque vous :
   * Lui envoyez un message direct.
   * Le mentionnez dans un canal avec `@mintlify`.

<div id="connect-repositories-as-context">
  ## Connecter des dépôts comme contexte
</div>

L&#39;agent ne peut accéder qu&#39;aux dépôts que vous connectez via la GitHub App Mintlify. Configurez les dépôts auxquels l&#39;agent peut accéder dans l&#39;onglet **Settings** du panneau de l&#39;agent ou dans les [paramètres de la GitHub App](https://github.com/apps/mintlify/installations/new).
...
Intégrez l’agent à vos outils d’automatisation existants pour mettre à jour automatiquement la documentation lors de modifications de code, déclencher des revues de contenu ou synchroniser les mises à jour de la documentation entre plusieurs dépôts.

Découvrez comment dans le tutoriel [Mettre à jour automatiquement la documentation lors de la fusion de code](/fr/guides/automate-agent).