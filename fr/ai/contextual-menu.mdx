...
description: "Ajoutez à votre documentation un menu contextuel avec des intégrations d'IA en un clic pour ChatGPT, Claude, Perplexity, Google AI Studio et les outils MCP."
...
Les mêmes champs et règles de validation s’appliquent que dans `docs.json`, y compris les [options personnalisées](#adding-custom-options) et le paramètre `display`. Si un remplacement au niveau d’une page est invalide, Mintlify revient à la configuration `contextual` globale.
