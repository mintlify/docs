...
description: "Ajoutez à votre documentation un menu contextuel avec des intégrations d'IA en un clic pour ChatGPT, Claude, Perplexity, Google AI Studio, Devin, Devin Desktop et les outils MCP."
keywords: ["Outils d'IA", "ChatGPT", "Claude", "Perplexity", "MCP", "Grok", "cursor", "vscode", "vs code", "Google AI Studio", "aistudio", "Devin", "Windsurf", "Devin Desktop", "pdf", "copier la page", "bouton copier", "copier le markdown", "copier en markdown", "afficher en markdown", "menu contextuel", "menu de page", "menu IA", "actions IA", "ouvrir dans", "télécharger pdf", "exporter pdf", "demander à l'IA", "openapi", "télécharger la spec"]
---

import { PreviewButton } from "/snippets/previewbutton.jsx"
import IconsRequired from "/snippets/fr/icons-required.mdx";

Le menu contextuel offre un accès rapide à un objet optimisé par l’IA et à des intégrations directes avec des outils d’IA populaires. Lorsque les utilisateurs ouvrent le menu contextuel sur n’importe quelle page, ils peuvent copier l’objet comme contexte pour des outils d’IA ou lancer des conversations dans ChatGPT, Claude, Perplexity, Google AI Studio, Grok, Devin, Devin Desktop, ou un outil personnalisé de votre choix, avec votre documentation déjà chargée comme contexte.
...
| **Télécharger la spécification d'API**    | `download-spec` | Télécharge la spécification OpenAPI de votre déploiement. S'il y a plusieurs spécifications, les télécharge dans une archive zip. N'apparaît que sur les pages de référence d'API. |
| **Options personnalisées**                | Objet        | Ajoutez des options personnalisées au menu contextuel                                 |

<div id="enable-the-contextual-menu">
  ## Activer le menu contextuel
</div>

Ajoutez le champ `contextual` à votre fichier `docs.json` et indiquez les options que vous souhaitez inclure. Les options apparaissent dans le menu dans l'ordre où vous les listez.
...
<ResponseField name="href" type="string | object" required>
  ...
  <Expandable title="href object">
    ...
    <ResponseField name="query" type="object[]">
      Un tableau d'objets de paramètres de requête à ajouter à l'URL de base.
...
<div id="override-on-individual-pages">
  ## Remplacer sur une page individuelle
</div>

Remplacez le menu contextuel global sur une page spécifique en ajoutant un champ `contextual` au frontmatter de la page. Utilisez cette approche pour mettre en avant des actions propres à la page, comme `download-pdf` sur une page de conditions d’utilisation, ou pour masquer entièrement le menu sur une page d’accueil.

L’objet `contextual` au niveau de la page remplace celui défini globalement pour cette page. Omettez le champ pour hériter de `docs.json`, ou définissez `options: []` pour désactiver le menu contextuel sur cette page.
...
Les mêmes champs et règles de validation s’appliquent que dans `docs.json`, y compris les [options personnalisées](#add-custom-options) et le paramètre `display`. Si un remplacement au niveau d’une page est invalide, Mintlify revient à la configuration `contextual` globale.
