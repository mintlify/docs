...
Le menu contextuel offre un accès rapide à du contenu optimisé pour l’IA et à des intégrations directes avec des outils d’IA populaires. Lorsque les utilisateurs ouvrent le menu contextuel sur n’importe quelle page, ils peuvent copier du contenu pour l’utiliser comme contexte dans des outils d’IA, ou lancer des conversations dans ChatGPT, Claude, Perplexity, ou un outil personnalisé de votre choix, avec votre documentation déjà chargée comme contexte.
...
<ResponseField name="href" type="string | object" required>
  L’attribut href de l’option. Utilisez une chaîne pour les liens simples ou un objet pour les liens dynamiques avec des paramètres de requête.
