...
keywords: ["MCP", "accès en écriture", "IA", "édition", "Claude", "ChatGPT", "Cursor", "branche", "pull request"]
---
...
L'Admin MCP est un service Mintlify hébergé à l'adresse `https://mcp.mintlify.com`. Chaque client se connecte au même point de terminaison et s'authentifie avec votre compte Mintlify.
...
Consultez la [référence du MCP Mintlify Index](/fr/search-index/mcp) pour connaître les paramètres de ses outils et ses limites de débit.
...
<Tip>
  Si votre connexion a accès à plusieurs déploiements, chaque déploiement sur lequel vous effectuez un checkout conserve sa propre session et sa propre branche en mémoire simultanément.
