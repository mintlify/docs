...
La [spécification skill.md](https://agentskills.io/specification) est un format structuré et lisible par les machines qui rend explicites les capacités des produits, les entrées requises et les contraintes. Les agents peuvent ainsi utiliser ces produits de manière plus fiable.
...
Lorsque les utilisateurs se connectent à votre serveur Model Context Protocol (MCP), leurs agents peuvent découvrir et utiliser vos fichiers `skill.md` en tant que [ressources MCP](/fr/ai/model-context-protocol#mcp-resources) sans installer les skills séparément.
...
<Prompt description="Installez les fichiers skill.md d'un produit dans votre agent." actions={["copy", "cursor"]}>
Installe les fichiers `skill.md` d'un site de documentation hébergé sur Mintlify dans le contexte de mon agent pour que tu saches ce que ce produit peut faire.

1. Demande-moi l'URL du site de documentation si je ne l'ai pas fournie, puis exécute `npx skills add <url>` avec cette URL.
2. Affiche la liste des skills installés, en incluant le nom et la description de chaque skill.
3. Résume les capacités clés en un paragraphe pour que je sache ce que tu peux maintenant m'aider à accomplir avec ce produit.
4. Si un skill déclare des entrées obligatoires ou des contraintes dans son frontmatter, signale-les-moi avant que je te demande de l'utiliser.
</Prompt>
