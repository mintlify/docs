...
`GET /.well-known/agent-card.json` renvoie un document JSON conforme au schéma 0.2.0 de la carte d'agent A2A. Chaque entrée du tableau `skills` correspond à une skill provenant de vos endpoints de découverte de skills.

Les agents compatibles A2A récupèrent `/.well-known/agent-card.json` pour découvrir votre site par son nom et sa description, suivent `documentationUrl` pour obtenir du contenu lisible par un humain, et parcourent `skills` pour récupérer chaque fichier `skill.md`.
