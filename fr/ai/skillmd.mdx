...
Les URL de la carte (`url`, `documentationUrl`, `provider.url` et chaque URL de skill) sont construites à partir du domaine personnalisé configuré lorsqu'il est défini, afin que la carte publiée annonce toujours le domaine canonique au lieu du sous-domaine `*.mintlify.app`.
