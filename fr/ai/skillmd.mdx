...
<Note>
  Si vous utilisez un [proxy inverse](/fr/deploy/reverse-proxy), configurez-le pour qu'il redirige les chemins `/skill.md` et `/.well-known/skills/*` vers votre sous-domaine Mintlify.
</Note>
...
<div id="skills-discovery-endpoints">
  ## Endpoints de découverte de skills
</div>

Mintlify héberge un répertoire de skills à l'adresse `/.well-known/skills/` que les agents peuvent utiliser pour découvrir et récupérer vos fichiers de skill de manière programmatique.

<div id="skills-index">
  ### Index des skills
</div>

`GET /.well-known/skills/index.json` renvoie un manifeste JSON répertoriant toutes les skills disponibles :
...
Le champ `name` est un slug sécurisé pour les URL, dérivé du champ `name` dans le frontmatter de votre `skill.md`.

<div id="individual-skill-files">
  ### Fichiers de skill individuels
</div>

`GET /.well-known/skills/{name}/skill.md` renvoie le fichier `skill.md` d'une skill spécifique identifiée par son nom slugifié dans l'index.
