...
L'accès aux journaux d'audit nécessite les deux conditions suivantes :

- **Plan Enterprise** — votre organisation doit disposer du droit d'accès aux journaux d'audit.
- **Permission** — votre rôle doit inclure la permission `org.auditLogs.read` (administrateur par défaut).

Si l'une de ces conditions n'est pas remplie, vous êtes redirigé vers la page des paramètres avec une notification expliquant pourquoi l'accès a été refusé. Mettez à niveau votre plan ou contactez l'administrateur de votre organisation pour obtenir l'accès.
...
| Integration      | Installations et suppressions de GitHub, Slack, Linear et Notion. |
