...
description: "Mintlify est une plateforme de documentation pour les développeurs et l’IA, avec recherche intelligente, playgrounds d’API interactifs et analytiques."
...
**Votre site**, propulsé par Mintlify. Mintlify génère votre site à partir de votre référentiel et le déploie par défaut sous une URL en `.mintlify.site`. Lorsque vous êtes prêt, vous pouvez associer un domaine personnalisé à votre site.
...
L’**agent** aide votre équipe à créer et à maintenir le contenu en générant des mises à jour à partir d’automatisations planifiées, de pull requests (demandes de fusion) fusionnées dans votre référentiel de fonctionnalités, ou de fils de discussion Slack.
