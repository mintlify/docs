...
sidebarTitle: "AWS"
description: "Hébergez la documentation sur un sous-chemin personnalisé à l'aide des services AWS"
---

import Propagating from "/snippets/fr/custom-subpath-propagating.mdx";

Pour héberger votre documentation à un sous-chemin personnalisé tel que `yoursite.com/docs` avec AWS Route 53 et CloudFront, vous devez configurer votre fournisseur DNS pour qu’il pointe vers votre distribution CloudFront.
...
<Frame>
    ![Page CloudFront « Create distribution » affichant « acme.mintlify.dev » comme Origin domain.](/images/cloudfront/origin-name.png)
  </Frame>
...
2. Recherchez votre URL d’environnement de préproduction qui reflète le domaine principal. Cela varie fortement selon l’hébergement de votre landing page. Par exemple, l’URL de préproduction de Mintlify est [mintlify-landing-page.vercel.app](https://mintlify-landing-page.vercel.app).

<Info>
  Si votre landing page est hébergée sur Webflow, utilisez l’URL de préproduction fournie par Webflow. Elle se termine par `.webflow.io`.
...
Pour "Cache policy", sélectionnez **CachingDisabled** afin de vous assurer que ces requêtes de vérification transitent sans mise en cache.

<Frame>
  ![Page CloudFront « Create behavior » avec un « Path pattern » « /.well-known/* » et « Origin and origin groups » pointant vers l’URL de staging.](/images/cloudfront/well-known-policy.png)
</Frame>
...
Créez un comportement avec un **Path pattern** correspondant au sous-chemin de votre choix suivi de `/*`, par exemple `/docs/*`, et des **Origin and origin groups** pointant vers la même URL en `.mintlify.dev`.
...
<Frame>
  ![Une distribution CloudFront avec le comportement « Default (*) » sélectionné et le bouton Edit mis en avant.](/images/cloudfront/default-behavior-1.png)
</Frame>

1. Modifiez le paramètre **Origin and origin groups** du comportement par défaut pour pointer vers l’URL de préproduction (dans notre cas `mintlify-landing-page.vercel.app`).
...
Vous pouvez maintenant vérifier si votre distribution est correctement configurée en allant dans l’onglet « General » et en ouvrant l’URL du **Distribution domain name**.
...
Toutes les pages devraient rediriger vers votre page d’accueil principale, mais si vous ajoutez le sous-chemin que vous avez choisi, par exemple `/docs`, à l’URL, vous devriez être redirigé vers votre instance de documentation Mintlify.

<div id="connect-with-route53">
  ## Connecter avec Route53
</div>

Nous allons maintenant faire pointer la distribution CloudFront vers votre domaine principal.
