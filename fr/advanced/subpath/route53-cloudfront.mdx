...
sidebarTitle: "AWS"
description: "Hébergez votre documentation sur un sous-chemin personnalisé avec les services AWS"
---

import Propagating from "/snippets/fr/custom-subpath-propagating.mdx";

Pour héberger votre documentation sur un sous-chemin personnalisé tel que `yoursite.com/docs` avec AWS Route 53 et CloudFront, vous devez configurer votre fournisseur DNS pour qu’il pointe vers votre distribution CloudFront.
...
Vos fichiers de documentation doivent être organisés dans votre dépôt pour correspondre à la structure de sous-chemin choisie. Par exemple, si vous souhaitez que votre documentation se trouve à l’adresse `yoursite.com/docs`, créez un répertoire `docs/` contenant l’ensemble de vos fichiers de documentation.

<div id="high-level-overview">
  ## Vue d’ensemble
</div>

Acheminer le trafic vers ces chemins avec une stratégie de cache **CachingDisabled** :
...
Acheminer le trafic vers ce chemin avec une stratégie de cache **CachingEnabled** :
...
2. Trouvez votre URL de préproduction qui reflète le domaine principal. Cela varie fortement selon la manière dont votre page d’accueil est hébergée. Par exemple, l’URL de préproduction de Mintlify est [mintlify-landing-page.vercel.app](https://mintlify-landing-page.vercel.app).
...
Créez un comportement avec un **modèle de chemin (Path pattern)** correspondant au sous-chemin de votre choix, par exemple `/docs`, avec **Origine et groupes d’origine** pointant vers l’URL `.mintlify.dev` (dans notre cas `acme.mintlify.dev`).

- Définissez « Cache policy » sur **CachingOptimized**.
- Définissez « Origin request policy » sur **AllViewerExceptHostHeader**.
- Définissez la stratégie de protocole du visiteur sur **Redirect HTTP to HTTPS**.
...
Créez un comportement avec un **motif de chemin** correspondant au sous-chemin de votre choix suivi de `/*`, par exemple `/docs/*`, et des **origines et groupes d’origines** pointant vers la même URL `.mintlify.dev`.
...
1. Modifiez **Origin and origin groups** du comportement par défaut pour utiliser l’URL de staging (dans notre cas `mintlify-landing-page.vercel.app`).
...
Vous pouvez maintenant vérifier que votre distribution est correctement configurée en allant dans l’onglet « General » et en ouvrant l’URL **Distribution domain name**.
...
Toutes les pages devraient renvoyer vers votre page d’accueil principale. En revanche, si vous ajoutez le sous-chemin que vous avez choisi — par exemple `/docs` — à l’URL, vous devriez être redirigé vers votre instance de documentation Mintlify.
...
<Note>
  ...
  d’Amazon Route 53 pour router le trafic vers une distribution
