...
Utilisez CSS pour mettre en forme les éléments HTML, ou ajoutez du CSS et du JavaScript personnalisés pour personnaliser entièrement l’apparence et le comportement de votre documentation.
...
Ajoutez des fichiers CSS à votre dépôt : les classes qu’ils définissent seront prises en compte et disponibles dans tous vos fichiers `MDX`.
...
<Tip>
  Utilisez l’outil Inspecter l’élément pour trouver les références aux éléments que vous souhaitez personnaliser.
</Tip>
...
<Warning>
  Les références et le style des éléments courants sont susceptibles d’évoluer au fil de la plateforme. Utilisez les styles personnalisés avec prudence.
</Warning>
...
Le JavaScript personnalisé vous permet d’ajouter du code exécutable globalement. C’est l’équivalent d’ajouter une balise `<script>` contenant du code JavaScript sur chaque page.

<div id="adding-custom-javascript">
  ### Ajout de JavaScript personnalisé
</div>

Tout fichier `.js` placé dans le répertoire de contenu de votre documentation sera inclus sur chaque page de la documentation. Par exemple, vous pouvez ajouter le fichier `ga.js` suivant pour activer [Google Analytics](https://marketingplatform.google.com/about/analytics) sur l’ensemble de la documentation.
...
<Warning>
  Veillez à l’utiliser avec prudence afin de ne pas introduire de vulnérabilités de sécurité.
</Warning>
