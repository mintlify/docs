...
<Warning>
  If you have disabled telemetry in your `docs.json` file by setting `integrations.telemetry.enabled` to `false`, the feedback feature will not appear on your documentation pages, even if it is enabled in your dashboard.
</Warning>
