...
<Update label="March 20, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Contextual menu expansions, agent PR comments, self-hosted GitLab UI, and more" }}>
  ...
  ## Agent comments on pull requests

  The [agent](/agent/index) now leaves comments directly on the pull requests it creates, providing a summary of the changes and the reasoning behind them. This makes it easier to review agent-generated documentation updates.

  ## Self-hosted GitLab configuration

  You can now configure [self-hosted GitLab](/deploy/gitlab) instances directly from the dashboard. GitLab API requests are now proxied through the server, removing the need for browser-based authentication to your GitLab instance.
  ...
  Profile pictures are now displayed throughout the dashboard for a more personalized experience.

  ## Custom IDs in MDX

  You can now assign custom `id` attributes to [accordion](/components/accordions), [tab](/components/tabs), and [step](/components/steps) components for more precise anchor linking and deep-linking into specific sections of your documentation.

  ## AI-generated PR descriptions

  The agent now generates descriptive titles and summaries for the pull requests it creates, giving reviewers better context at a glance.
  ...
  - Workflows run page redesigned with a cleaner layout
  - Slack notifications for workflows now configurable from the dashboard
  - Analytics page loading states improved with skeleton UI and smoother transitions
  - Editor typing latency and IndexedDB persistence performance improved
  - Login organization selection page redesigned
  - Card layout shift on initial page load eliminated
  - Dashboard analytics date-picker responsiveness improved
  - Viewer role now properly enforced in the editor
  ...
  - Fixed accordion group node parsing in the editor
