...
<Update label="May 29, 2026" tags={["New releases","Improvements"]} rss={{ title:"Mintlify MCP, GitLab workflows, preview sharing, and more" }}>
  ## New features

  - **Mintlify MCP server**: Connect Claude, Cursor, or any MCP client to your Mintlify project to draft edits, restructure navigation, update `docs.json`, and open pull requests. Every change happens on a branch and is reviewable like any other commit. See [Mintlify MCP](https://www.mintlify.com/docs/ai/mintlify-mcp).
  - **GitLab workflows**: [Workflows](https://www.mintlify.com/docs/workflows/enable) now support GitLab repositories alongside GitHub. You can connect GitLab.com or a [self-hosted GitLab instance](https://www.mintlify.com/docs/deploy/gitlab-self-hosted) via OAuth, and use automerge for merge requests with the Maintainer role.
  - **Share a preview**: A new [Share a preview](https://www.mintlify.com/docs/editor/share-a-preview) page documents preview deployment URLs and editor links so reviewers can see your changes before they go live. You can also restrict preview access to your Mintlify organization from the Add-ons page.
  - **Download API spec**: Add `"download-spec"` to `contextual.options` to give visitors a one-click download of your OpenAPI spec from any API reference page. Single specs download as a file; multiple specs bundle into `api-specs.zip`. See [Let visitors download your spec](https://www.mintlify.com/docs/api-playground/openapi-setup#let-visitors-download-your-spec).
  - **OpenAPI callbacks**: API reference pages now render operation-level OpenAPI `callbacks` automatically in a collapsible section between the request body and responses. No configuration required. See [Callbacks](https://www.mintlify.com/docs/api-playground/openapi-setup#callbacks).
  - **Agent discovery endpoints**: Mintlify now serves an agent card at `/.well-known/agent-card.json` (A2A 0.2.0) so agents can discover your skills without a session, plus additional MCP discovery URLs at `/.well-known/mcp.json`, `/.well-known/mcp/server-card.json`, and `/.well-known/mcp/server-cards.json`. All endpoints are served automatically with no setup. See [skill.md](https://www.mintlify.com/docs/ai/skillmd#agent-card) and [Model Context Protocol](https://www.mintlify.com/docs/ai/model-context-protocol).
  - **Content-Signal directives in robots.txt**: The auto-generated `robots.txt` now includes Content Signals (`ai-train=yes`, `search=yes`, `ai-input=yes`) so AI crawlers know your docs are available for training, search, and answer generation. You can override the defaults with a custom `robots.txt`. See [Content-Signal directives](https://www.mintlify.com/docs/optimize/seo#content-signal-directives).

  ## Updates

  - **Editor publishing**: The default commit message now lists the files you created, updated, moved, or deleted, replacing the AI-generated message. The **Commit message instructions** publishing setting has been removed. See [Branching and publishing](https://www.mintlify.com/docs/editor/branching-and-publishing).
  - **Editor tutorial**: A new [How to use the editor](https://www.mintlify.com/docs/editor/tutorial) tutorial walks you through creating a branch, editing pages, sharing a preview, and publishing.
  - **Markdown export authentication**: The Markdown export docs now include a table covering how `.md` URLs behave under no, partial, and full authentication. See [Markdown export](https://www.mintlify.com/docs/ai/markdown-export#authentication).
  - **Dashboard URL**: All Mintlify dashboard links now use `app.mintlify.com` instead of `dashboard.mintlify.com`. Existing links continue to redirect.
</Update>
...
<Update label="May 1, 2026" tags={["New releases","Improvements"]} rss={{ title:"Revamped workflows and editor, agent score, new Mintlify MCP, and more" }}>
  ## Workflows
