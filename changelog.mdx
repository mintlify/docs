...
<Update label="July 16, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Visibility component, related pages, per-page contextual menu, drafts, and more" }}>

  ## New features

  - **Related pages:** Automatically surface related pages at the bottom of docs to help readers discover more relevant content.
  - **Drafts:** Preview unpublished changes on a per-page basis without creating a branch. A new announcement dialog and dashboard toggle guide teams onto the new workflow.
  - **Per-page contextual menu overrides:** Configure the [contextual menu](/ai/contextual-menu) per page in frontmatter to override the site-wide options.
  ...
  - **Trailing-slash SEO config:** New `seo.trailingSlash` option in [`docs.json`](/settings) enforces consistent trailing-slash URLs across your site.
  - **Universal Search API:** New API for querying content across your docs, with per-deployment opt-out.
  - **S3 key prefixes:** The S3 integration now supports key prefixes so you can scope uploads to a specific path in your bucket.
  - **Pylon support:** [Pylon](/automations/integrations) is now available as a connected integration for support-driven [automations](/automations).

  ## Editor

  - **Review request card in live preview:** Reviewers see a request-review card directly on the live preview, with a shared changes list showing draft changes at a glance.
  - **Lock the deploy branch:** The editor now prompts to lock your deploy branch when branch protections are detected, and a new toggle blocks direct edits to `main`.
  - **Better undo/redo in the [editor agent](/editor):** Content and navigation-tree changes made by the agent are now cleanly undoable and redoable.
  - **Collapsible heading state persists per user:** Your expanded and collapsed section preferences carry across editor sessions.
  - **Cached-content read-only mode:** The visual editor now goes read-only while showing cached (unsynced) content to prevent conflicting writes.
  - **Hidden pages CTA:** A new banner in the editor calls out pages hidden from navigation so you can find and manage them.

  ## Automations and Slack agent

  - **Agent configuration moved into Automations:** Agent settings now live under [Automations](/automations). Old agent-page URLs redirect automatically.
  - **Slack agent onboarding:** When the agent is enabled, org members now receive a Slack DM and a `#mintlify` channel is created with an onboarding message. The agent also sends follow-up nudges when the channel goes quiet.
  ...
  - **Headless Assistant client:** A new framework-independent [Assistant](/assistant) core and browser loader lets you embed the assistant with a headless client API and direct-mount lifecycle for custom UIs.
  - **Widget dashboard:** Configure and preview the Assistant widget from a dedicated dashboard page and playground.
  - **Filter MCP skill resources by end-user groups:** Restrict which skill resources are exposed through the [MCP server](/ai/model-context-protocol) based on end-user group membership.
  - **Group-scoped custom skills for authed MCP:** Scope custom skills to specific end-user groups on authenticated MCP deployments.
  - **Auto-merge admin MCP saves:** Admin-MCP edits can now auto-merge to `main` when configured.

  ## Analytics

  - **Analytics v7 improvements:** Consolidated Overview and Traffic sections, unified CSV export menu, synced date-range selectors across sections, and a new Feedback section.
  - **Assistant Chat history tab:** Dig into individual assistant conversations from the [analytics](/optimize/analytics) v7 assistant card, with feedback and pagination in the topic drawer.
  - **Search breakdown and rankings:** New search-breakdown and rankings cards, plus zero-result-search endpoints and unique-query and click-through-rate metrics.
  - **CSV exports:** Export MCP search, agent-visitor, and zero-result-search reports as CSV.
  - **Version dropdown tracking:** Version switches on your site are now tracked as `docs.version.change` events for analytics.

  ## Bug fixes

  - Fixed OAuth redirect URIs on preview deployments so authenticated docs sign-in works on `.mintlify.site` preview hosts.
  - Cached MDX no longer stores fallback compiles after a parse failure, so subsequent edits recover cleanly.
  ...
  - Chat history deep links open correctly from analytics v7.
  - Rankings page views in analytics v7 aggregate human and agent traffic, and zero-result delta colors reflect the correct direction.
  - Assistant no longer throws max-update-depth errors while a response is streaming.
  ...
  - Fixed navigation sidebar tree overflow, delete-deployment button overflow, and topbar button padding.
  - Referenced-navigation reloads are noticeably faster.
