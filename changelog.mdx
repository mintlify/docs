...
<Update label="September 11, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"OpenAPI Overlays, flat AI pricing, and more" }}>
  ...
  - **Free editor agent:** The editor agent is free to use, and the Slack agent is free per write. Free agent events no longer appear in your usage history.
  - **Fixed credits for assistant answers:** Assistant answers are billed at a fixed number of credits.
  - **Flat automation pricing:** Automations are billed a flat 250 credits per write.
  - **Zero-cost BYOK:** AI usage that runs on your own model key costs zero credits. See [Credits](/credits).

  ## Editor and collaboration

  - **Recents and Starred everywhere:** The editor's Recents and Starred sections now sync across your devices and appear in the workspace sidebar. Both are scoped to workspace pages across branches, and you can reorder sidebar sections by dragging them.
  ...
  - **Comments panel:** Open a document-wide comments panel to review open and resolved threads for the current page. Source mode now uses the same comment cards and popovers as visual mode, and reaction picks surface the reactions your team uses most. See [Comments](/editor/collaborate#comments).
  ...
  - **Page icons as tab favicons:** The editor tab favicon now uses the page's icon, and page and navigation icon frontmatter accept emoji values.
  - **Card icon fields:** The `Card` component's properties panel now exposes the `icon` and related fields inline.
  - **Editor polish:** Suggestion deletions coalesce as you type, editor search results show the recipient list scoped to your organization, share recipients outside your organization are skipped, live preview works for pages that are not in the navigation, and view-only pages show a **View-only mode** dropdown.
  ...
  - **Default model:** New automation runs now default to Opus 4.7.
  ...
  - **GitLab agent tokens:** Mintlify revokes GitLab agent project access tokens after each workflow run, sweeps stale tokens every hour, and names tokens by workflow instance. Token liveness is now determined by heartbeat instead of workflow status.
  ...
  - **Dashboard fixes:** Code-block caret placement is fixed for both hoisted and input-rule cases, SSO configuration resets when you remove a connection, only verified domains can be selected when SSO is configured, and the page header restores the **Add description** affordance.
  ...
  - **Miscellaneous:** Private page headers are preserved when reloading empty bodies, emitted asset URLs and cache bypass paths resolve under deployment base paths, and the Gmail search shortcut for Mintlify emails works again.
