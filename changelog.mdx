...
<Update label="November 15" tags={["New releases", "Improvements"]} rss={{ title: "Weekly Updates", description: "Badge component, API playground improvements, custom language support, and performance enhancements" }}>
  ...
  - **Badge component**: New `<Badge />` component for displaying status indicators, labels, and tags in your documentation
  ...
  - **LaTeX configuration**: Control LaTeX/math rendering through `docs.json` with the new `latex` field, allowing you to disable LaTeX processing when not needed for improved performance
  ...
  - Fixed rendering of MDX request and response examples in API playground
  - Better error messages when invalid URLs are provided in API configurations
  - Fixed AsyncAPI pages compatibility with new API page design
  - Array property children now have proper padding for better visual hierarchy
  - Prevented page pop-in on new API pages for smoother loading experience
  - Fixed text overflow issues in API playground for long parameter names and values

  ## Navigation and UI enhancements

  - Tab hover indicators now have improved styling for better visual feedback
  - Fixed contextual menu styling for better consistency across themes
  - Long words in documentation now wrap properly with `overflow-wrap` to prevent horizontal scrolling
  - Step component now handles cases where no title is provided
  - Logo section spacing improved with better padding and `justify-between` layout
  ...
  - Icons now align properly with text along the x-axis
  - Fixed theme toggle colors for better visibility in both light and dark modes
  - Multi-view dropdown now aligns left for consistent positioning
  ...
  - CardGroup replaced with Columns component in editor dropdown for better layout control

  ## Component and styling enhancements

  - Images can now be zoomed with improved overlay styling for both light and dark themes
  - Fixed keyboard shortcut styles inside callouts for dark theme
  - Fixed badge positioning inside `<GroupDropdown />` components
  - Improved OG image generation with better truncation logic and Google Fonts support
  - Tables now use `tabular-nums` font feature for better number alignment in data-heavy tables
  - Fixed scroll behavior for tables inside tabs and steps inside tabs
  - Palm theme sidebar styling improvements

  ## Performance and infrastructure

  - KaTeX CSS is now conditionally loaded only when math expressions are present on the page, reducing bundle size for pages without LaTeX
  ...
  - YAML language support added to Shiki for better syntax highlighting in configuration examples

  ## Bug fixes and reliability

  - Fixed security vulnerabilities related to path traversal and internal route access
  - Fixed persistent auth keys in playground input store to prevent data loss
  ...
  - Hardened static assets route for better security
  - Fixed OAuth redirects to properly include `basePath` for subdirectory deployments
  - Removed dual basepath in auth login links
  ...
  - Removed ESM expression props from MDX when removing JavaScript for server-side rendering
  - Only simple expressions are now allowed in SSR mode for ESTrees to prevent rendering issues
  - Fixed dashboard design inconsistencies and improved spacing
  - Fixed email validation in discovery signup endpoint for better password validation
  - Added `.nvmrc` file for consistent Node.js version across development environments

  ## Localization

  - Added localization support for additional languages including Auth0 integration improvements
