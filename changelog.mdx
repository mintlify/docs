...
<Update label="October 31" tags={["Improvements"]} rss={{ title: "Weekly Updates", description: "API playground enhancements, table improvements, performance optimizations, and bug fixes" }}>
  ...
  - **Prefill default values**: The API playground now automatically fills in default values for parameters, saving time when testing endpoints.
  ...
  - **MDX descriptions in modals**: API playground modals now display rich MDX-formatted descriptions at the top, giving better context for endpoints with formatted text, links, and code examples.

  ## OpenAPI and API reference

  - **New schema graph rendering**: Individual OpenAPI schema pages now use an optimized schema graph renderer that loads faster, especially for complex schemas with many nested objects or references.

  ## Performance and infrastructure

  - **Faster static file serving**: All static files (images, videos, PDFs, documents) are now served with improved caching headers, which results in faster page loads on repeat visits and reduced bandwidth usage.
  - **Optimized deployment caching**: Deployments now complete faster due to parallelized Redis operations, which reduces the time between pushing changes and seeing them live.
  - **Better cache management**: Improved handling of signed URLs and cache expiration so that assets stay fresh when updated while still benefiting from caching for unchanged content.
  ...
  - **Better table styling**: Tables now have better alignment, consistent spacing, and enhanced readability. Table headers align properly with their content columns.
