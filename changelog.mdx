...
<Update label="December 12, 2025" tags={["New releases", "Improvements"]} rss={{ title: "Discord bot enhancements", description: "Multi-turn conversations, #ask-ai channel support, and improved UX for Discord bot" }}>
  ## Discord bot enhancements

  The Discord bot now supports multi-turn conversations within threads. When you continue a conversation in an existing thread, the bot remembers the context from previous messages, providing more coherent and contextual responses.

  Additional Discord improvements:
  - **#ask-ai channel support**: The bot now automatically responds to all messages in channels named `#ask-ai` without requiring an @mention, similar to the Slack bot behavior.
  - **Collapsible search results**: Search results now display as collapsible embeds with a "Show Details" button, keeping threads cleaner while still providing access to source links.
  - **Visual feedback**: The bot now adds reaction indicators (👀 while processing, ✅ when complete) to show processing status.
  - **Customizable bot appearance**: Configure your Discord bot's name and avatar through the dashboard settings.
  ...
  ```mdx
  <Tooltip headline="Pro Tip" tip="You can use keyboard shortcuts to navigate faster.">
    Hover for tip
  </Tooltip>
  ```
  ...
  - Fixed IME input handling in the assistant to prevent accidental message submission during composition (for languages like Japanese, Chinese, Korean).
  ...
  - Fixed an issue where the first undo (Ctrl+Z) in the editor would incorrectly clear the file.
