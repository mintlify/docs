...
<Update label="July 2, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Assistant custom skills, SCIM provisioning, and more" }}>
  ...
  - **AI preview editor:** Start a new documentation site from a prompt and iterate with an AI agent in an interactive preview. The agent can ask clarifying questions, resume after a connection interruption, and start directly from onboarding. Provide source URLs, repositories, and uploaded files as context for the first generation. Interrupted generations now resume automatically after worker failures. If retries are exhausted, Mintlify restores the starter deployment.
