...
<Update label="April 10, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Directory listings, agent feedback, invisible captcha, Slack improvements, and more" }}>

  ## Directory listings

  Navigation groups with a [root page](/organize/navigation#root-page) can now display a [directory listing](/organize/navigation#directory-listings) of their child pages. Set the `directory` property on a group, tab, or the top-level `navigation` object to `"accordion"` or `"card"` to automatically generate an overview of the section's content. Useful for help centers, support centers, and any section where users need a quick way to browse available pages.

  ## Agent feedback

  The [agent](/agent) now returns structured feedback alongside documentation updates. When the agent creates or updates content, it surfaces actionable feedback about what changed and why, so you can review the reasoning behind each documentation update directly in the dashboard.

  ## Invisible captcha

  A new bot protection option for the [assistant](/ai/assistant). Enable invisible captcha from the dashboard to automatically verify that assistant messages come from real users without requiring any user interaction. Replaces visible CAPTCHA challenges with a seamless background check.
  ...
  ## GitLab support for workflows

  [Workflows](/agent/workflows) now support GitLab repositories. The agent can clone GitLab repositories as context, follow prompts, and open merge requests or push changes directly to your deployment branch.
  ...
  Added Finnish (fi) as a supported [localization](/guides/internationalization) language.
  ...
  - The [assistant](/ai/assistant) is now available in `mint dev` local previews.
  ...
  - Fixed an editor loading issue in source mode.
