...
<Update label="September 8, 2026" tags={["New releases"]} rss={{ title:"Outcome-based credit pricing" }}>

  ## Outcome-based pricing

  - **Fixed credit prices per action:** Every AI action has a fixed credit price, and you pay only when an action produces a result. An assistant answer costs 25 credits. An automation run that updates your docs costs 250 credits. Actions that produce nothing cost nothing, including assistant messages that can't be answered and automation runs that find nothing to update or fail.
  - **Free agent conversations:** Conversations with the editor agent and the agent in Slack don't consume credits on Pro and Enterprise plans.
  - The price of an action is the same regardless of how much content the agent reads or how long the action runs, so you can project usage from volume alone. Your plan, its included credits, and the price per credit stay the same. See [Credit pricing](/credits).
