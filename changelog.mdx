...
<Update label="May 15, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Editor AI instructions and publishing controls" }}>

  ## Editor settings for AI and publishing

  The web editor now has a dedicated **Settings** menu with two layers of configuration:

  - **Your settings** are scoped to your account and shape how the editor's AI assists with your edits.
  - **Publishing settings** are configured per deployment by an admin and shape how commits and pull requests are generated and merged.

  See [Editor settings for AI and publishing](/editor/settings) for the full reference.

  ## New features

  - **AI instructions**: Add persistent guidance that the editor sends to the AI on every **Edit with AI** action and agent session. Use it to lock in voice, terminology, component preferences, or formatting rules so you don't have to repeat them in every prompt.
  - **Commit message instructions**: Guide the AI when it generates commit messages on publish — useful for matching Conventional Commits, ticket prefixes, or stricter subject-length conventions.
  - **Pull request instructions**: Standardize AI-generated pull request titles and descriptions whenever the editor opens a pull request, including from **Create pull request** and **Merge and publish** flows.

  ## Improvements

  - **Draft pull requests by default**: Admins can configure the editor to open every new pull request as a draft, so reviewers see a clear "not ready yet" signal until you mark it ready.
  - **Default merge method**: Pick **Merge**, **Squash**, or **Rebase** as the default for **Merge and publish**. Match this to your Git provider's branch protection rules to avoid failed merges from the editor.
