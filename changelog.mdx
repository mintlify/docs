...
<Update label="March 6, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Auto-generate documentation from public GitHub repositories, assistant multimodality, and more improvements" }}>
  ...
  - [Full base URL](/organize/settings#param-api) now supported in the API playground by configuring `api.url` in `docs.json`.
...
<Update label="February 27, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Automate documentation tasks with workflows and collaborate in the editor with comments and suggestions" }}>
  ...
  Automate documentation tasks with [workflows](/agent/workflows). Set up scheduled or event-triggered automations that run Mintie bot to update your documentation.
...
<Update label="February 20, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Agent configuration files and custom frontend" }}>
  ...
  Use `.mintlify/Assistant.md` and `.mintlify/AGENTS.md` files in your repository to customize Mintie bot and assistant behaviors. See [Customize assistant behavior](/ai/assistant#customize-assistant-behavior) and [Customize Mintie bot behavior](/agent/customize) for more information.
...
<Update label="January 9, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Implicit snippets and assistant conversation insights for all" }}>
  ...
  Conversation insights are now available for all documentation sites that have both Mintie bot and assistant enabled. Mintie bot analyzes patterns in assistant conversations to identify documentation gaps and suggest improvements, helping you proactively address common user questions.
...
<Update label="December 23, 2025" tags={["New releases", "Improvements"]} rss={{ title: "Web editor improvements and assistant insights" }}>
  ...
  ## Mintie bot suggestions from assistant conversations

  [Mintie bot](/agent) can now suggest documentation updates based on assistant conversations. When enabled, Mintie bot analyzes questions users ask the assistant to identify patterns that indicate missing or unclear documentation.

  This adds a second source for Mintie bot suggestions alongside repository monitoring, helping you proactively address common user questions.
...
<Update label="December 19, 2025" tags={["Improvements"]} rss={{ title: "Mintie bot, assistant, and editor improvements" }}>
  ## Mintie bot improvements

  - Added support for reading [custom instructions](/agent/customize) from an `AGENTS.md` file.
  - Mintie bot now opens PRs under your GitHub account if you connect your account on the dashboard.
...
<Update label="December 12, 2025" tags={["New releases", "Improvements"]} rss={{  title: "Mintie bot suggestions and Q&A bots" }}>
  ## Mintie bot suggestions
  
  Mintie bot can now monitor Git repositories and suggest documentation updates. When user-facing code changes are detected, suggestions appear in the Mintie bot panel. Suggestions include the relevant pull request and a list of proposed documentation updates. Add suggestions as context for Mintie bot to create pull requests or dismiss them.
...
<Update label="October 11, 2025" tags={["Improvements"]} rss={{ title: "Weekly Updates", description: "API playground improvements, analytics enhancements, web editor updates, and bug fixes" }}>
  ## Assistant and Mintie bot AI features

  - Added list_pull_requests and list_commits tools for Mintie bot such that it can document a date range of PRs or multiple PRs at once
  - Upgraded Mintie bot and assistant to Claude Sonnet 4.5
