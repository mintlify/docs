...
<Update label="September 18, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Scoped automation triggers, editor agent integrations, and more" }}>
  ...
  - **Scoped GitHub triggers:** Custom automations can now trigger on GitHub topics, per-repository code changes, and per-repository push events with path and author filters. Template automations also accept per-repository filters. See [Create an automation](/automations/create).
  ...
  - **Connected integrations as tools:** The agent can call your connected third-party apps while researching and editing. Native Slack tools are included, and the agent can prompt you to connect a new integration when it needs one. Integration tool calls show friendly labels in the agent panel.
  - **Analytics with charts:** The agent can query your MintQL analytics and render results as inline charts when you ask questions about page views, referrers, or feedback.
  - **Roll back edits:** Ask the agent to undo a change and it reverts the pages it edited in the current session.
  - **Current page context:** The agent now sees the page you have open and its unsaved edits, so requests like "fix this section" apply to what you are looking at.

  ## Editor and collaboration

  - **Teamspace (Enterprise):** Teamspace pages and folders let a team share drafts in the dashboard, gated by an organization rollout flag. Contact support to enable it for your organization.
  ...
  - **Version history redesign:** The version history panel has a refreshed layout that's easier to scan when comparing revisions. See [Version history](/editor/pages#version-history).
  - **Faster editor rendering:** Visual mode renders around the viewport, sidebar navigation reuses unchanged rows, and cached pages restore with complete source, so switching pages and scrolling feel snappier.

  ## Dashboard

  - **Rebuilt Activity page:** The Activity page has a new layout that groups events, updates in place, and shows richer detail in the drawer.
  - **Unified top banners:** Trial, plan, and status banners now use a single consistent style across the dashboard.
  ...
  - **Sort and export improvements:** Page ratings support sort order, the feedback drawer shows the page URL, and assistant analytics exports include feedback ratings.
  - **Page filter polish:** The page filter opens on the currently selected page, resets cleanly with the clear button, and shows **All pages** as the default label.
  ...
  - **Universal Search:** Universal Search is now available to all customers without an entitlement gate.
  ...
  - **Security:** Session cookies set `Secure` on HTTPS, MintQL uses a stricter parser to close bypasses, JSON parser errors are sanitized, GitLab credentials are covered by SSRF checks, editor `web_fetch` is hardened against SSRF, GitLab and Bitbucket OAuth return paths are validated, and assistant thread reads require ownership.
