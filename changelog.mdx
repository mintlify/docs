...
<Update label="July 10, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Product search filter, granular PDF exports, API key controls, and more" }}>

  ## New releases

  - **Product search filter:** Enterprise deployments can now define [search filters](/optimize/search) so readers can scope results to a specific product, section, or audience directly from the search bar.
  - **Granular PDF exports:** Choose exactly which pages to include when [exporting to PDF](/optimize/pdf-exports) using a file tree selection modal, and see partial vs. full export metadata in your export history. Locales are respected when generating exports.
  - **Custom base path UI:** Configure a custom base path (subpath) for your docs deployment inline from the dashboard, with a live hosting-mode description that reflects your setup. See [docs subpath](/deploy/docs-subpath).
  - **System theme option:** Readers can now match their docs [theme](/customize/themes) to their operating system's light/dark preference.
  - **JSON-LD on docs pages:** Every docs page now emits auto-generated [schema.org JSON-LD metadata](/optimize/seo) to improve how docs surface in search engines and AI answers.
  - **Visitor geo on `window.mintlify.geo`:** [Custom components and scripts](/customize/custom-scripts) can now read the visitor's country, region, and continent from `window.mintlify.geo` for personalized experiences.
  - **`/call` page:** Docs with a support phone number now expose a `/call` page that forwards visitors straight to the phone dialer, replacing the old `showCallNumber` assistant tool.
  - **Paste JSON into the API playground body:** Pasting a full JSON payload into the [API playground](/api-playground/overview) body now fills every matching field, including nested objects, array rows, and `oneOf` variants — no more field-by-field entry.
  - **Support-docs-updates automation:** A new [automation](/automations) proposes docs changes based on incoming support tickets, so recurring questions turn into docs updates.
  - **Onboarding for existing repos:** [Onboarding](/deploy/github) now supports connecting an existing Git repository and renaming it, on top of creating a fresh one.

  ## Improvements

  - **Identity & access consolidation:** SSO, SCIM, and network access settings are now unified on a single [Identity & access page](/dashboard/sso) in the dashboard, and the page loads noticeably faster.
  - **Configurable dashboard session lifetimes:** Admins can now set the [dashboard session duration](/dashboard/sso) and inactivity timeout from Identity & access.
  - **Network access IP allowlist UI:** Manage the org-wide [session IP allowlist](/dashboard/network-access) from a dedicated Network access tab.
  - **API key IP allowlist, scopes, and expiration:** When creating an [external or discovery API key](/api-playground/overview), you can now attach an IP allowlist, choose read/write scopes, and set an explicit expiration date. All key lifecycle events are recorded in the [audit log](/dashboard/audit-logs).
  - **SCIM audit events and downgrades:** SCIM user changes now emit [audit log events](/dashboard/audit-logs), and downgrade paths are supported via SCIM.
  - **Integrations moved to organization settings:** Agent [integrations](/organize/settings-integrations) now live under a dedicated **Integrations** tab at `/settings/organization/integrations`, out of the agent tab.
  - **Slack agent quality-of-life:** The [Slack agent](/agent/slack) is now time-aware, resolves user and channel names, honors thread timezones for reads, retries file fetches on upload races, and honors deploy branch protection when choosing a review process.
  - **`.mp4` attachments in support:** You can now attach `.mp4` recordings when contacting support.
  - **Assistant clarifying questions:** The [AI assistant](/assistant) can now ask a clarifying question with 2–4 clickable options when a request is ambiguous, and streaming responses render more smoothly.
  - **Contact form opt-out for the assistant:** Disable the assistant's in-chat contact form from the [assistant settings](/assistant/configure) while still keeping the sales form available.
  - **Faster editor loads:** The editor now caches the nav tree and page contents in IndexedDB for faster first paint, and virtualizes the file tree and publish dropdown for large sites.
  - **Editor comments restored:** Inline comments are back in the [editor](/editor), with cleaner storage that no longer persists comment markup in the underlying doc.
  - **Mobile drawers:** The mobile sidebar and mobile AI assistant now use a native drawer interaction that feels closer to a real app.
  - **Reset editor state:** A **Reset editor state** action is available in the editor UI to recover from a stuck local session.

  ## Bug fixes

  - Assistant streams now surface errors instead of failing silently, and retries regenerate the response instead of appending a duplicate turn.
  - Three-way merge in the editor no longer duplicates content in text blocks, and hanging visual-mode diffs on large pages are resolved.
  - Nested `llms.txt` files no longer 404 (regression from an earlier release).
  - MCP OAuth2 authentication is now correctly reported in MCP discovery for auth-enabled deployments, and per-deployment redirect allowlists are honored on org-wide admin MCP consent.
  - The **Copy anchor link** icon on `<Update>` blocks in the changelog now copies the correct URL.
  - Multi-source deployments no longer re-run a full update when a push doesn't touch the matched source's content directory.
  - Cursor's web callback is now allowlisted so trial users can complete MCP auth.
  - The billing page renders the plan from the current deployment instead of the org.
  - `<Steps>` children now inherit `titleSize` from the parent so the table of contents renders consistently.
  - The Assistant and Analytics tabs are swapped back to their expected order in the sidebar.
