...
<Update label="April 9, 2026" tags={["New releases"]} rss={{ title: "Directory listings for navigation group root pages" }}>

  ## Directory listings for group root pages

  Navigation groups with a [root page](/organize/navigation#root-page) can now display an automatic directory of their child pages. Set the new [`directory`](/organize/navigation#directory-listings) property on any navigation node to `"accordion"` or `"card"` to render a listing below the root page content. The value inherits recursively, so you can set it once at a high level and override it on individual groups with `"none"`.
