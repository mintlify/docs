...
<Update label="August 7, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Bitbucket, editor version history, deeper analytics, and more" }}>

  ## New features

  - **Bitbucket support:** Connect a Bitbucket repository from the dashboard to sync your docs, on top of GitHub and GitLab. See [CI](/deploy/ci) for more information.
  - **Editor version history:** Browse past editor versions, compare against the current draft, and restore a previous version from the page.
  - **Duplicate page:** Duplicate a page from the page options menu in the editor.
  - **Authenticated live previews:** Preview drafts of pages that live behind auth from inside the editor, without logging out.
  ...
  - **Prefill API playground server variables:** A new client-side API lets custom scripts prefill server variables in the API playground.
  - **Multiple JWT login URLs:** Configure more than one login URL per deployment for [JWT authentication](/deploy/authentication-setup), and pass extra `loginOptions` metadata for downstream login flows.
  - **Pylon integration:** Connect [Pylon](/automations/integrations) as a read-only integration so the agent can use Pylon context in Slack and automations.
  - **New integration allowlist entries:** [LaunchDarkly](/automations/integrations), [PostHog](/automations/integrations), and [Salesforce Service Cloud](/automations/integrations) are now available as agent integrations, each with a proper logo.
  - **User flow analytics:** A new user-flow paths card in analytics shows the routes visitors take through your docs, with drop-off surfaced at each step and an "Other" bucket for the long tail.
  - **Funnel analytics:** New conversion funnel cards cover the user funnel, search funnel, and assistant deflection funnel, including a "took an action" stage.
  - **Interactions overview:** A new interactions overview card summarizes CTA clicks, search, and assistant activity in one place.

  ## Improvements

  - **Impressions renamed to User engagements:** The Impressions section in [analytics](/optimize/analytics) is now called User engagements, and now includes an actions line and action-rate column alongside CTA click-through rate.
  - **Editor comments redesign:** Comments in the editor have a refreshed layout, and standard keyboard shortcuts now work for comment, suggest, and source modes.
  - **Suggesting mode pill:** A new pill in the editor top bar makes it clearer which authoring mode you're in.
  - **Branches dropdown:** The editor branches dropdown has redesigned rows, fuzzy search, and a live label for the branch you're on.
  - **Nested private pages:** Create nested private pages and move nodes with cascading, grant-aware permissions.
  - **Private page @-mentions:** Reference private pages by title with `@` in the editor agent composer.
  - **Agent-authored edits:** The editor renders comments and suggestions created by the agent inline, and the send action is now optimistic.
  ...
  - **Integration logos:** Salesforce Service Cloud and OneDrive now show their real logos.
  - **MDX comments:** Inline MDX comments are stripped from rendered output.
