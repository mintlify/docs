...
<Update label="May 1, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Workflows v2, web editor video and onboarding improvements, configurable OpenAPI Markdown export, and more" }}>

  ## Workflows v2

  The redesigned [workflows](/workflows) experience is now the default at `/products/workflows` in the dashboard. New capabilities include:

  - **Custom workflows** alongside built-in templates.
  - **Searchable repository dropdown** when configuring sources, triggers, and context repos.
  - **Update mode** that triggers on the deployment's connected Git source.
  - **Runs tab filters** by schema, status, and URL parameters with live polling.
  - **Source PR links** and one-click accept or close on workflow runs.
  - **Confirmation prompt** before closing a workflow run's pull request.
  - **Global Slack notifications** for workflow events.
  - **GitLab repository selection** when configuring workflow sources.
  - The default daily run limit is now 500.

  ## Vimeo support in the web editor

  The web editor now supports Vimeo embeds alongside YouTube and Loom. Paste a Vimeo URL to insert a player directly.

  ## Copy branch button

  The branch view in the web editor now includes a button to copy the current branch name, making it easier to switch to it locally or share it with teammates.

  ## Configurable OpenAPI in Markdown export

  You can now control whether OpenAPI reference pages are included in your [Markdown export](/ai/markdown-export) and `llms.txt`, giving you finer control over what AI agents see.

  ## Section-level search boost

  Boost specific sections of your documentation in search results in addition to entire pages. See [search boost](/optimize/search-boost) for details.

  ## Related pages frontmatter

  The `relatedPages` frontmatter field has been renamed to `related`. Existing `relatedPages` values continue to work, but new pages should use `related`. The related pages link icon has also been updated.

  ## Improvements

  - The Mintlify [agent](/agent) UI no longer requires the legacy Composio install path—integrations are managed through the standard flow.
  - Onboarding now includes a template selection step so new projects can start from a curated starter.
  - Editor v7 introduces remote diff viewing, live preview, an editable tree component, and recovery for invalid MDX.
  - The `mint` CLI now shares the agent score response contract through `@mintlify/models` for consumers building on top of it.

  ## Bug fixes

  - Fixed an XSS vulnerability in the assistant.
  - Fixed copying personalized code blocks so the rendered values are copied instead of the template.
  - Fixed offset for blocks placed inside a [`Column`](/components/columns).
  - Fixed AsyncAPI MDX content positioning.
  - Fixed prose styling inside [`Tab`](/components/tabs) components.
  - Fixed search result snippets that duplicated the page title prefix.
  - Fixed `_next/image` requests being blocked by `robots.txt`, so `og:image` previews now render correctly.
  - Reduced memory usage during link-rot checks on large sites.
