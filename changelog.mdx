...
<Update label="September 25, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Narration, webhook-triggered automations, image reading for agents, and more" }}>

  ## New releases

  - **Webhook triggers for automations:** Custom automations can now run when an external service sends a request to the automation's webhook. See [Trigger a webhook automation](/automations/manage#trigger-a-webhook-automation).
  - **Image reading for agents:** The editor agent and the [assistant](/assistant) can now read images in your docs and use what they see in their answers and edits. See [View images](/editor/agent#view-images).
  - **Narration:** Readers can listen to a page from a **Listen to this page** player at the top of supported pages. Mintlify highlights each sentence as it plays and skips code blocks, tables, and other non-prose content. Available on Pro and Enterprise plans. See [Narration](/assistant/narration).
  - **MDX checks in source mode:** Source mode flags misspelled components, unknown or duplicate props, invalid prop values, missing required props, and unclosed tags. Quick fixes let you apply the suggested correction in one click.
  - **Source mode hover definitions and autocomplete:** Source mode shows component and prop definitions on hover and suggests completions as you type.
  - **Formatting suggestions:** The editor suggests inline formatting while you write, and you can preview a suggestion before you accept it.

  ## Improvements

  - **List shortcuts:** <kbd>Cmd</kbd> + <kbd>Shift</kbd> + <kbd>9</kbd> creates a task list, and <kbd>Cmd</kbd> + <kbd>Shift</kbd> + <kbd>7</kbd> creates a numbered list. <kbd>Tab</kbd> and <kbd>Shift</kbd> + <kbd>Tab</kbd> indent or outdent every selected list item. See [Keyboard shortcuts](/editor/keyboard-shortcuts).
  - **Comments:** Unsent comment drafts survive page reloads, replies stay put when a thread collapses, and comments can contain only a mention. See [Comments](/editor/collaborate#comments).
  - **Copy and paste:** Copying keeps the structure, numbering, and formatting of selected lists.
  - **Switching projects:** The editor reopens the page you last had open when you switch projects.
  - **Faster editor:** Pages open faster, typing no longer reparses the whole page, and navigation loads without waiting on GitHub data.
  - **Redesigned agent panel:** The [editor agent](/editor/agent) panel is faster in long chats, asks smarter clarifying questions, renders MintQL charts and tables inline, folds long user messages so they no longer take over the chat, and lets you click anywhere in the panel to start typing.
  - **Granular publishing:** Selective publishing is more granular and now defaults to publishing only your own changes.
  - **Private page roles:** The **Publisher** role on private pages is now called **Full Access**.
  - **Automations:** Editors can now delete automations, automation triggers link to their repositories and channels, and automations use Opus 5.5 by default.
  - **Rollbacks for editors:** Editors can now [roll back](/deploy/rollbacks) a deployment, not just admins.
  - **Rename your organization:** You can now update your organization's name.
  - **Multi-repo partial updates:** Deployments with multiple repositories rebuild only what changed instead of running a full update.

  ## Bug fixes

  - **Editor:** HTML tables no longer lose rows on save, new page titles keep focus while you type, ordered lists in table cells keep their starting number, task lists written with `[X]` are no longer flagged as invalid MDX, checkboxes line up with bullets, image paste works on new private pages, and the editor no longer leaks memory when you return to it.
  - **Publishing:** Publishing works around navigation errors instead of blocking, the editor refuses publish selections that would break navigation, and the publish button no longer gets stuck in a render loop.
  ...
  - **Updates:** Mintlify removes stale OpenAPI pages when a spec changes.
