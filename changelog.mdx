...
<Update label="December 12, 2025" tags={["Improvements"]} rss={{ title: "Tooltip component updates", description: "Enhanced tooltip styling with new headline prop and improved accessibility" }}>
  ## Tooltip component updates

  The [Tooltip component](/components/tooltips) has been redesigned with improved styling and new functionality:

  - **New headline prop**: Add an optional headline to tooltips for better content organization.
  - **Refreshed design**: Updated styling with improved spacing, colors, and visual hierarchy that matches the latest design system.
  - **Better accessibility**: Enhanced `aria-label` attributes that now include headline text when present for improved screen reader support.
  - **Improved CTA styling**: Call-to-action links now use a chevron icon with refined spacing and hover states.
