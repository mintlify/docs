...
<Update label="March 6, 2026" tags={["New releases", "Improvements"]} rss={{ title: "File attachments in assistant, contextual starter questions, and editor improvements" }}>

  ## File attachments in assistant

  Upload images and files directly in the [assistant](/ai/assistant) chat. Paste or drag files into the assistant to include them as context when asking questions, making it easier to get help with code, diagrams, and screenshots.

  ## Contextual starter questions

  The assistant now displays suggested questions based on the page you're viewing. Configure starter questions in your dashboard to guide users toward helpful prompts.

  ## Related pages

  Pages now show related content recommendations to help users discover relevant documentation. Related pages appear automatically based on content similarity.

  ## TypeScript React components

  You can now use [custom TypeScript React components](/customize/custom-scripts) in your documentation with full type support.

  ## Editor improvements

  - Redesigned editor search bar with improved UX.
  - New secondary sidebar with tab navigation for easier file management.
  - Nested drag-and-drop support for list items.
  - Improved code block editing with focus states and line numbers.
  - URL-based file paths for better navigation and sharing.

  ## Improvements

  - [Full base URL display](/organize/settings#param-api) can now be configured in `docs.json` using `api.url`.
  - Workflow run limit increased from 20 to 50 per day.
  - Branch names now support underscores.
  - Dashboard layout refreshed with improved topbar, sidebar, and navigation.
  - Deployment history now distinguishes between queued and in-progress states.
  - Cancel assistant messages with keyboard shortcuts.

  ## Bug fixes

  - Fixed cursor jumping when pasting content in the editor.
  - Fixed link border colors in callout components.
  - Fixed tooltip display for analytics cards.
  - Fixed link handling for uncommon URL protocols.
