...
<Update label="August 5, 2026" tags={["New releases"]} rss={{ title:"Introducing Mintlify Index" }}>
  ...
  [Mintlify Index](/search-index/overview) gives coding agents one retrieval layer for publisher-maintained technical documentation and the web.

  - **Public MCP server:** [Connect](/search-index/connect) Claude Code or Cursor without an API key. The `context` tool returns source-cited material for implementation tasks.
