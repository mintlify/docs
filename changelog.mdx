...
<Update label="April 17, 2026" tags={["New releases","Improvements"]} rss={{ title:"Visibility component, MCP filesystem access, agent feedback, Git setup wizard, and more" }}>
  ...
  Enable [agent feedback](/optimize/feedback#agent-feedback) in your deployment settings to let agents submit structured feedback about your documentation. When turned on, your search MCP server provides a `submit_feedback` tool that connected agents can use to report documentation issues. Available as a beta feature in the dashboard under deployment add-ons.
