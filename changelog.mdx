...
<Update label="September 11, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Recents and Starred sync, automation merge conflict resolution, and more" }}>
  ...
  - **Comments panel:** Open a document-wide comments panel to review open and resolved threads for the current page. Reaction picks now surface the reactions your team uses most.
  ...
  - **Desktop app login handoff:** Sign in to the Mintlify desktop app by confirming a one-time code from your browser, then switch organizations from the same consent screen.
  ...
  - **Opt-in branch dropdown:** The editor branch dropdown is now opt-in per user; enable it from your editor settings when you want it back.

  ## API playground and OpenAPI

  - **OpenAPI Overlays 1.1.0:** OpenAPI Overlay 1.1.0 documents are now applied in update and schema-graph workflows, with `$ref` prefixes and array entries handled correctly under base paths and mounts. See [OpenAPI setup](/api-playground/openapi-setup).
  - **Playground labels:** The `oneOf` variant dropdown uses title-aware labels so each option is easier to identify.

  ## Assistant and AI

  - **Free editor agent, flat AI pricing:** The editor agent is now free to use, the Slack agent is free per write, assistant answers are billed at fixed credits, and automations are billed a flat 250 credits per write. BYOK AI usage costs zero credits. Free agent events are hidden from usage history, and the usage page announces the new flat credit pricing.
  - **Assistant Playground presets:** The Assistant Playground includes streaming presets so you can compare configurations without editing each field.
  - **Universal ranking:** Search ranks page-title matches higher so exact-title lookups surface first.
  ...
  - **Assistant fixes:** Assistant drafts are preserved while a response is streaming, code blocks stay highlighted while streaming and skip unchanged renders, tables and compact code blocks scroll independently, file attachments render as horizontal cards, suggestion links render inside answers, and the send and stop controls are combined into one button.
  ...
  - **Publishing:** Publish checkboxes are now independent per row, partial publishes apply selected pending actions to the baseline navigation tree, and content-less create actions retire after publish so they cannot block later publishes.
  ...
  - **OpenAPI:** Deleting an OpenAPI spec now removes its generated assets, memory usage during OpenAPI candidate detection is reduced, and API reference pages slice the schema graph to the page's target.
  - **Miscellaneous:** Organization logos are stored in the platform bucket, private page headers are preserved when reloading empty bodies, cache bypass paths match under deployment base paths, and the Gmail search shortcut for Mintlify emails works again.
