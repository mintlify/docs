...
<Update label="February 26, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Contextual starter questions and editor improvements" }}>

  ## Contextual starter questions

  The AI assistant now generates contextual starter questions based on the page users are viewing. Instead of showing the same questions across your entire documentation, the assistant suggests relevant questions specific to the current page's content. This helps users discover answers faster and explore topics more naturally.

  Additionally, starter questions now support path-based configuration, allowing different questions to appear on different sections of your documentation.

  ## Current page context

  The assistant now includes the current page's content in its system prompt, providing more accurate and relevant responses based on where users are in your documentation.

  ## Search filtering

  New search filter UI makes it easier to narrow down search results across your documentation.

  ## Improvements

  - Added `x-code-samples` support in markdown representation of API routes for better AI agent consumption.
  - Performance improvements in the web editor for icon picker, comments panel, and table of contents.
  - Improved caching for GitHub requests in the editor.

  ## Bug fixes

  - Fixed resize handle visibility for non-editor paths.
  - Fixed race conditions in editor suggestions with pending writes.
  - Fixed spacing issues when the assistant bar is not present.
  - Fixed search dropdown appearing incorrectly for GitHub links.
