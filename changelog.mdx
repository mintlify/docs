...
<Update label="October 31" tags={["Improvements"]} rss={{ title: "Weekly Updates", description: "API playground enhancements, table improvements, performance optimizations, and bug fixes" }}>
  ...
  - **Prefill default values**: The API playground now automatically fills in default values for parameters, saving you time when testing endpoints. You no longer need to manually enter common default values—just click "Send" to test with sensible defaults.
  - **Better handling of complex schemas**: Fixed issues with `allOf`, `oneOf`, and nested object schemas that previously caused rendering problems or incorrect field displays. Complex API schemas now render correctly with all fields visible and properly structured.
  - **Improved type labels**: Enum and nullable field types now display accurate labels in the playground, making it clearer what values are accepted and whether fields are optional.
  - **Enhanced nested object labels**: When working with nested objects in API requests, you'll now see proper descriptive labels for each field instead of generic placeholders, making it easier to understand what data to provide.
  - **Better table rendering**: Enum const tables and API field displays now render with improved formatting and alignment, making it easier to scan available options and understand field requirements.
  - **MDX descriptions in modals**: API playground modals now display rich MDX-formatted descriptions at the top, giving you better context about endpoints with formatted text, links, and code examples.

  ## Table improvements

  - **Better tables feature**: Tables throughout your documentation now have improved styling with better alignment, consistent spacing, and enhanced readability. Table headers align properly with their content columns, eliminating visual confusion.

  ## OpenAPI & API reference

  - **New schema graph rendering**: Individual OpenAPI schema pages now use an optimized schema graph renderer that loads significantly faster, especially for complex schemas with many nested objects or references.

  ## Performance & infrastructure

  - **Faster static file serving**: All static files (images, videos, PDFs, documents) are now served with improved caching headers, resulting in faster page loads on repeat visits and reduced bandwidth usage.
  - **Optimized deployment caching**: Deployments now complete faster thanks to parallelized Redis operations, reducing the time between pushing changes and seeing them live.
  - **Better cache management**: Improved handling of signed URLs and cache expiration means your assets stay fresh when updated while still benefiting from aggressive caching for unchanged content.

  ## Assistant improvements

  - **Help button**: A new dedicated help button provides easier access to the AI assistant from anywhere in your docs, making it more discoverable for users who need quick answers.
  - **Improved conversation quality**: Fixed JSON formatting issues that could cause confusing responses, and added support for generating starter questions to help users know what to ask.

  ## Analytics

  - **Deferred analytics loading**: PostHog and LogRocket now load after your documentation content, improving initial page load times and Core Web Vitals scores without sacrificing analytics data.
  - **Conditional session recording**: PostHog session recording can now be enabled conditionally based on user properties or page context, giving you more control over when recordings happen.
  ...
  - Fixed `/llms.txt` routing so LLM agents can properly access your documentation
