...
<Update label="April 10, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Directory listings, CLI templates, multi-skill files, workflows for GitLab, and more" }}>

  ## Directory listings for navigation groups

  Navigation groups with a [`root` page](/organize/navigation#root-page) can now display a directory listing of all child pages. Set `"directory": "accordion"` or `"directory": "card"` on any navigation group in `docs.json` to automatically render a browsable page index below the root page content. The setting inherits through nested groups and can be overridden with `"none"`.

  ## CLI project templates

  Initialize new documentation projects from pre-built templates with `mint new --template`. Choose from a selection of starter templates to get up and running faster. See [CLI commands](/cli/commands) for details.

  ## Multi-skill support

  Define multiple AI [skill files](/ai/skillmd) by placing `SKILL.md` files in subdirectories under `.mintlify/skills/`. A manifest endpoint at `/.well-known/skills/index.json` lists all available skills. Single `skill.md` files at the project root continue to work as before.

  ## Workflows for GitLab

  [Workflows](/agent/workflows) now support GitLab repositories in addition to GitHub, including webhook-driven triggers and merge request tracking.

  ## MCP client credentials

  The [MCP](/ai/model-context-protocol) OAuth endpoint now supports the `client_credentials` grant type for machine-to-machine authentication. Configure client credentials from the dashboard to enable server-side integrations with your MCP server.

  ## Assistant and search in local development

  The AI [assistant](/ai/assistant) and search are now available during local development with `mint dev` when you are logged in, so you can test the full experience locally.

  ## Improvements

  - New [publish flow](/editor/publish) in the web editor with branch creation, change review, and pull request support.
  - Create new pages directly in the web editor without leaving the browser.
  - Source (MDX) editing mode now available alongside the visual editor for switching between WYSIWYG and code views.
  - Discard unpublished changes in the editor with a visual diff preview before reverting.
  - Live presence indicators show which teammates are viewing the same page in the editor.
  - `docs.json` can now be edited collaboratively in the web editor with real-time sync.
  - Redesigned slash command menu in the editor with grouped items and component previews.
  - [Workflow](/agent/workflows) run notifications now appear in the dashboard inbox. The workflow list now shows PR titles as the primary identifier.
  - [Slack bot](/ai/slack-bot) now supports configuring a preferred deployment per channel and switching deployments mid-conversation.
  - Bot protection settings for the [assistant](/ai/assistant) are now configurable from the dashboard.
  - `mint analytics` CLI command for viewing stats, search queries, feedback, and conversations. Supports table, JSON, and graph output formats. See [CLI analytics](/cli/analytics).
  - CLI access tokens now auto-refresh when expired, removing the need to manually re-login.
  - [`mint export`](/deploy/export) now includes OpenAPI-generated API reference pages.
  - `<hr>` horizontal rule elements are now supported in MDX content.
  - Custom heading IDs now render correctly in all contexts.
  - Added Finnish language support for localization.

  ## Bug fixes

  - Fixed SEO and metadata audit workflow template incorrectly modifying API endpoint pages.
  - Fixed source editor failing to load in certain scenarios.
  - Fixed inline formatting with leading or trailing spaces being incorrectly applied.
  - Fixed directory folder icon colors not matching the configured primary theme color.
  - Fixed badge illustration and keyboard shortcut display issues.
  - Fixed editor bug when switching branches with stale file state.
  - Fixed API response example generation logic for certain schema types.
  - Fixed RSS feed fetching blame information from local clones instead of the Git provider API.
  - Fixed sitemap incorrectly including hidden pages when `seo.indexing` is set to `"all"`.
  - Fixed page node reordering in the editor.
...
<Update label="April 3, 2026" tags={["New releases", "Improvements"]} rss={{ title: "get_page MCP tool, password-protected previews, SAML group role mappings, and more" }}>
  ...
  - [Workflows](/agent/workflows) can now be disabled and re-enabled from the dashboard.
  ...
  - Fixed [workflow](/agent/workflows) automerge toggle not properly disabling when set to off.
...
<Update label="March 27, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Linear and Notion integrations, offline exports, inline AI editing, and more" }}>
  ...
  Connect your Linear workspace from the agent settings page to give the [agent](/agent/linear) access to your Linear issues. The agent can reference issues and comments as context when generating documentation updates or you can delegate issues to the agent to directly in your Linear workspace.

  ## Notion integration

  Connect your Notion workspace from the agent settings page to give the [agent](/agent/notion) read access to your Notion content. The agent can reference Notion pages as context when generating documentation updates.
...
<Update label="March 13, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Workflow templates and CLI command, Agent API v2, and more" }}>
  ...
  Create [workflows](/agent/workflows) faster with pre-built templates in the [dashboard](https://dashboard.mintlify.com/products/workflows). Choose from templates for changelog generation, API docs sync, feature documentation drafts, broken link detection, SEO audits, and more. Templates include optimized prompts and trigger configurations to get you started quickly.

  ## Workflow CLI command

  Create [workflows](/agent/workflows) files from the command line with the `mint workflow` CLI command. Interactive prompts guide you through name, trigger, context, and instructions.
...
<Update label="March 6, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Auto-generate documentation from public GitHub repositories, assistant multimodality, and more improvements" }}>
  ...
  The [assistant](/ai/assistant) now supports multi-modal input. Attach files to messages to provide additional context. Drag and drop or paste images, PDFs, and code files directly into the chat.

  The assistant can now generate page-specific [example questions](/ai/assistant#add-sample-questions).
