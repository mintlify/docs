...
<Update label="February 13, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Prompt component, Sequoia theme, Tabler icons, and editor improvements" }}>
  ...
  New [`<Prompt />`](/components/prompt) MDX component for displaying copyable prompts. Use it to share AI prompts, command-line instructions, or any text users need to copy.

  ## Sequoia theme

  Introducing the [Sequoia theme](/customize/themes#sequoia)—a new theme featuring a stone color palette, AI assistant sidebar, redesigned table of contents, and aligned navigation tabs.
  ...
  The [contextual menu](/ai/contextual-menu) now includes an option to add your docs as an MCP server directly from any page.

  ## `root` in groups

  Navigation groups now support a [`root`](/organize/navigation#root-pages) property to set a landing page for the group. Clicking the group header navigates to the root page instead of expanding the group.

  ## AI merge conflict resolution

  The web editor now automatically resolves merge conflicts using AI when pulling changes from your repository.

  ## Editor improvements

  - New Table, Accordion, and Card components with improved editing experience
  - Global block selection for easier multi-block editing
  - Redesigned image toolbar
  - Redesigned icons popover

  ## Editor publish dropdown redesign

  Improved branch creation and pull request workflow with clearer status indicators and streamlined publishing options.

  ## `Columns` component

  The [`<Columns />`](/components/columns) component is now the recommended way to create multi-column layouts. `CardGroup` continues to work but `Columns` offers more flexibility.
