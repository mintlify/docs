...
<Update label="June 12, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Editor drafts, revamped agent onboarding, Workflows GA, and more" }}>
  ...
  - **Media drop uploads:** Drag images and other media into the file tree to upload them in place.

  ## Agent onboarding

  - **Revamped 3-step flow:** New users get a guided 3-step onboarding for the agent, with a welcome screen, integration setup, and a final review step.
  - **Connect integrations during onboarding:** Add third-party [agent integrations](/agent/integrations) — including Confluence, Notion, and Plain — as part of setup.
  - **Quick setup re-entry:** A banner returns you to the right onboarding step if you leave partway through.
  - **New dashboard checklist:** A redesigned checklist on the dashboard tracks setup progress after onboarding.

  ## Workflows GA

  - **Beta badge removed:** [Workflows](/workflows) is out of beta.
  - **Stale run reaper:** Long-queued or stalled workflow runs now time out reliably after 25 hours.

  ## Sign-in redesign

  - **Email and password by default:** The login screen now defaults to email and password, with a toggle to switch to a magic link.
  ...
  - **Renamed contextual menu action:** "Open in Windsurf" in the [contextual menu](/ai/contextual-menu) is now "Open in Devin Desktop" and uses the `devin://` deep link.
  ...
  - **Project-level noindex:** Site owners can hide an entire project from search engines from the [docs UI](/optimize/seo).
  ...
  - **CSV exports:** Export dashboard analytics to CSV for offline reporting.
  - **Agent feedback filter:** Filter agent analytics by user feedback.

  ## Billing

  - **Higher self-serve credit tiers:** Self-serve credit tiers now go up to 52,500. See [credits](/credits) for details.
  - **First credits discount:** New customers see a first-purchase discount on paywall entry points.

  ## Bug fixes

  - Renaming files between `.mdx` and other extensions in the editor now works.
  - Click-to-reposition the cursor in the file tree rename input.
  - Fixed an editor white screen on certain navigations.
  - Adding files to `docs.json` no longer requires a reset.
  - Image renames in the editor now work.
  - The active page node is revealed in the navigation tree on load.
  ...
  - Confluence connections now route through the Nango Connect UI.
  - Workflows home banner shows a toast when a custom workflow delete fails, and prefers `errorMessage` over a summary for failed runs.
  - "Ran x ago" labels tick during polling.
  - Fixed a double-count in the usage overview when credits rolled over.
  ...
  - PDF route auth metadata lookup is fixed.
  - The `@mintlify` comment bot only triggers on real mentions, not scoped package names.
</Update>
