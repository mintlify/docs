...
<Update label="January 30, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Simplified skills installation, revamped analytics, editor improvements, and more" }}>
  ...
  Install skills using just a docs domain—no file path needed:
  ...
  Faster, smoother experience when navigating between analytics tabs with improved loading states. Deep link directly to specific analytics tabs for easier sharing and bookmarking.

  ## Editor improvements

  - Editable icons
  - WYSIWYG title/description editing for Cards, Accordions, and Steps
  - Hide and unhide pages from your navigation
  - Local image support in live preview
  - Code block triggering with \`\`\`
  - Drag handle selection and delete improvements
  - Stability improvements

  ## Updated assistant page

  Manage all assistant functionality in one page with a cleaner layout and better UX.

  ## MCP filtering

  AI agents can now filter MCP search results by parameters automatically—no action needed on your part.

  ## Other

  - "Open in Grok" support
  - Zip file upload support in onboarding

  ## Bug fixes

  - **Sidebar navigation state**: Fixed a caching issue that caused the sidebar to show incorrect expanded/collapsed states when navigating between pages.
  - **Custom page scrolling**: Resolved scroll position issues on custom pages so content displays correctly.
  - **Mermaid diagrams**: Fixed conflicts when multiple Mermaid diagrams appeared on the same page.
  - **Tabs component**: Fixed a rendering loop that could cause performance issues with the Tabs component.
  - **Password reset**: Fixed the password reset button not working.
  - **Print view**: The AI assistant no longer appears when printing pages.
  - **Dark mode cursor**: Fixed the gap cursor color being invisible in dark mode.
  - **MDX style objects**: Style attributes in MDX are now preserved correctly instead of being stripped.
  - **Last modified dates**: Fixed an issue where page "last modified" dates updated incorrectly during global config changes.
  - **AI assistant follow-ups**: Follow-up messages in the AI assistant now work correctly.
  - **Index page authentication**: Fixed authentication issues with index pages.
  - **GitHub status in onboarding**: GitHub status is now properly surfaced during onboarding when the service is down.
