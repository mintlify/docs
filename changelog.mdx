...
<Update label="July 24, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"GraphQL and SDK references, editor review flow, assistant widget, and more" }}>
  ...
  - **Editor agent reads your code repos:** Give the [editor agent](/editor/agent) read-only access to connected code repositories so it can ground documentation edits in your source.
  - **Top-level page comments:** Start a comment thread on a page itself, not just on selected text.
  - **Mention yourself in comments:** `@`-mention your own account in [editor comments](/editor/comments) to leave reminders.
  - **New keyboard shortcuts:** `Option+S` opens a [suggestion](/editor/suggestions) on the selected text and `Option+C` opens a [comment](/editor/comments).
  - **Cursor focuses at top on open:** Opening a page now places the cursor at the top of the document.
  - **Faster load:** The editor no longer waits on the git file tree to render.
  - **View without GitHub permissions:** Users without repo write access can now open the editor in read-only mode instead of getting blocked.

  ## Assistant and widget

  - **Simplified [Assistant Widget](/guides/assistant-embed):** Cleaner configuration, framework-neutral loader, configurable initial theme, improved Markdown rendering, and a refreshed composer, trigger, and playground UI.
  - **Pre-filled contact form:** When the assistant hands off to support, it now pre-fills the contact form message from the conversation so users don't have to re-explain their issue.
  - **ChatGPT MCP connector:** Add Mintlify docs as a [ChatGPT connector](/ai/mintlify-mcp) so ChatGPT can search and cite your documentation.
  - **Claude in automations:** Automations now support Claude as a model provider.

  ## Automations and Slack agent

  - **Automation analytics dashboard:** A new analytics view for [automations](/automations) with per-workflow run history, outcomes, and trends.
  - **Composio integrations:** Third-party integrations for automations are now powered by Composio, with a shared connect flow across the dashboard and Slack agent.
  - **Integration triggers:** Trigger automations from third-party events (for example, a Slack message or a support ticket update) via [Composio triggers](/automations/integrations).
  - **User-scoped integrations:** Connect third-party tools under your own account and pick them from the automations picker, alongside org-wide connections.
  - **One-shot scheduled workflows:** Schedule an automation to run once at a future time and auto-delete after it finishes.
  - **Filtered changelog URLs:** Share a [changelog](/changelog) link filtered to a specific tag.
  ...
  - **`mint format`:** Format MDX files with Mintlify's bundled editor converter for consistent structure across your docs. See [CLI commands](/cli/commands).
  - **`mint deslop`:** Detect AI-sounding prose and get humanization suggestions before publishing.
  - **`mint add-domain`:** Configure a [custom domain](/customize/custom-domain) from the CLI without leaving your terminal.
  - **Nicer `--help` output:** Consistent `--help` formatting across every command and a friendlier prompt when the CLI is invoked by an AI agent.

  ## Analytics

  - **Feedback revamp:** The [analytics feedback](/optimize/feedback) section now surfaces per-page ratings and opens a detailed feedback drawer with the underlying comments.
  - **Human vs. agent page rankings:** Page rankings now split human and agent traffic into separate columns so you can see who's really reading each page.
  - **Analytics streaming config:** Pick which deployments to include when [streaming analytics](/optimize/analytics) to your own warehouse.

  ## Deployments and search

  - **Search on preview deployments:** Preview deployments now serve docs search, so you can validate search results before merging.
  - **Deep-link PR previews:** GitHub PR preview links now open directly on the page you changed.
  - **GHES repo selection:** Self-hosted GitHub Enterprise Server repositories can now be selected in [git settings](/deploy/ghes).
  - **Ignore `.mintlify` by default:** The `.mintlify` directory is ignored on deploy by default, except for `skills` and `specs`.

  ## Bug fixes

  - Search domain path casing is preserved end-to-end.
  - Fuzzy search now only falls back to near-typo matches instead of loose fuzzy hits.
  - Stuck search indexing jobs are recovered by a new watchdog, and search AST uploads retry on transient S3 failures instead of silently dropping pages.
  - Search emits per-context version and product tags for pages that are cross-listed across contexts.
  - Preview builds now surface branch deletion clearly instead of showing a misleading build error.
  - The editor now round-trips task list checkboxes, table escapes, LaTeX math, callout `type` attributes, video sources, image titles, and inline code with pipes inside table cells.
  - The editor allows block content inside `Columns` so nested blocks don't get silently deleted.
  - PDF exports are now rendered in light mode regardless of the reader's theme.
  - `.md` page internal links and the `Link` header respect your `basePath`.
  - Authenticated docs correctly refresh expired end-user JWTs in the search proxy and serve a genuine 404 for missing pages on allowlisted deployments.
  - The Assistant Widget waits for hCaptcha to be ready before submitting and preserves historical response actions across reloads.
  - Subpath domain setup preserves its pending state during DNS provisioning and hides the DNS migration step where it doesn't apply.
  - The `luma` and `sequoia` themes correctly center content in center page mode, and the `maple` theme keeps the API side panel below the tabs navigation.
  - Duplicate page names in the dashboard now append a slug instead of colliding, and the products redirect preserves the authentication segment.
  - Password auth renders read-only when unentitled instead of crashing the settings page.
