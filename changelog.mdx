...
<Update label="March 18, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Linear integration, related pages, self-hosted GitLab, and contextual menu updates" }}>

  ## Linear integration

  The agent can now monitor Linear for issue updates and suggest documentation changes based on engineering work tracked in Linear.

  ## Related pages

  Configure related pages in the dashboard to surface contextually relevant content to readers, helping them discover more of your documentation.

  ## Self-hosted GitLab support

  Connect a self-hosted GitLab instance to Mintlify directly from the dashboard. Enable the **Set up as self-hosted** toggle and enter your GitLab instance's host URL during setup.

  ## Contextual menu updates

  Added "Open in Devin", "Open in Windsurf", "Connect to Devin" MCP, and "Open in Google AI Studio" options to the [contextual menu](/ai/contextual-menu). Users can now send documentation pages directly to more AI tools.

  ## Invite multiple members

  Invite multiple team members to your organization at once from the dashboard.

  ## Workflows improvements

  - New workflows run page to view execution history and status
  - Configure Slack notifications for workflows directly from the dashboard

  ## Editor improvements

  - Viewers with the [viewer role](/ai/contextual-menu) can now access the editor in read-only mode
  - Fixed editor typing latency and IndexedDB persistence blocking for a smoother editing experience

  ## Improvements

  - Agent now comments directly on pull requests with documentation suggestions
  - Upgraded search infrastructure for faster and more accurate results

  ## Bug fixes

  - Fixed card layout shift on initial page load
  - Fixed Luma theme banner layout shift
  - Fixed list rendering in API playground markdown descriptions
  - Fixed code block tooltip appearing on prompt and Mermaid components
  - Fixed title attribute prioritization in MDX serialization
  - Fixed dashboard analytics date-picker delay
  - Fixed blockquote placeholder rendering
  - Fixed focus styles for advanced tabs
  - Fixed security issue with page mode data attribute
  - Fixed page mode flicker
  - Fixed OpenAPI file matching for edge cases
