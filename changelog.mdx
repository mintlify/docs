...
<Update label="March 12, 2026" tags={["New releases", "Improvements"]} rss={{ title: "New workflow creation flow, YouTube and Loom embeds in editor, Hungarian and Catalan language support" }}>

  ## New workflow creation flow

  The [workflow creation experience](/agent/workflows) has been redesigned with a cleaner interface and improved usability for setting up automated documentation tasks.

  ## YouTube and Loom embeds in editor

  The slash command menu in the [web editor](/editor/index) now includes options to quickly embed YouTube and Loom videos directly into your documentation.

  ## New language support

  Added support for Hungarian and Catalan languages in the documentation interface.

  ## Improvements

  - Spellcheck can now be enabled per-page in the editor using the `spellcheck:true` attribute.
  - Page views are now displayed on the analytics page for better content performance insights.
  - Localized footer and navbar content is now supported for multi-language documentation sites.
  - Logo href now matches the first page in the selected locale for better navigation consistency.
  - Webhooks now support server preference configuration.
  - Dashboard UI is now more responsive across different screen sizes.

  ## Bug fixes

  - Fixed PDF rendering style issues.
  - Fixed footer visibility for custom pages on Mint, Maple, Willow, and Linden themes.
  - Fixed `.md` route fallback to use properly generated titles.
  - Fixed editor popover component behavior.
  - Fixed assistant attach file icon.
  - Fixed typo in error messages ("occurred").
  - Fixed proper HTTP status codes for redirects (308/307).
  - Fixed thumbs-only feedback filtering in analytics.
  - Fixed internal props leaking into markdown output.
