...
<Update label="September 4, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Collaborative source mode, comment reactions, mint test for everyone, and more" }}>

  ## Editor and collaboration

  - **Collaborative source mode:** Source mode now supports real-time collaboration. Teammates' cursors are labeled in the source pane, and frontmatter edits sync live across visual and source mode. See [Visual and source mode](/editor#visual-and-source-mode).
  ...
  - **Selective publish:** Select individual changes in the publish menu to publish part of your pending work instead of everything at once. See [Branching and publishing](/editor/publish).
  - **Share updates:** Mintlify notifies you when someone shares a page with you, and the share dialog searches recipients by email address.
  - **Editor polish:** Search results show page titles and reveal the file in the sidebar, the sidebar lists only Markdown pages by default, the emoji suggestion menu has better search relevance, and the agent panel stays available in site settings.
  ...
  - **Failure emails:** Mintlify emails you when an automation run fails, then holds further failure emails until a run succeeds again.

  ## Analytics

  - **Page filter:** Filter your analytics to a specific page with the new page dropdown in the dashboard. See [Analytics overview](/analytics).

  ## API playground

  - **Optional field toggles:** Long request forms list required fields first and collapse optional fields behind toggles, so the inputs that matter are easier to find.
  ...
  - **Assistant citations:** The assistant repairs citation links that dropped a leading path segment, so citations open the correct page.
  - **Authenticated previews:** Preview deployments for docs behind authentication serve your changed files.
  - **Translations:** Translated pages link to locale-prefixed internal paths.
