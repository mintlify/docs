...
<Update label="April 27, 2026" tags={["Improvements"]} rss={{ title: "Workflow settings refresh and custom workflows in the dashboard" }}>

  ## Workflow settings refresh

  The settings panel for each [workflow](/agent/workflows) in the dashboard has been redesigned:

  - **Choose how updates are applied** on every workflow type. Pick **Automatic** to commit changes directly, or **Require review** to open a pull request first.
  - **Pick the trigger** for workflows that support both options. Workflows like translations, brand tone, grammar, broken links, and SEO now default to **On update** (recommended) so they run whenever your content changes, with **On a custom schedule** still available behind the scheduling option.
  - **Add custom instructions** under **More configurations**. The text you enter is appended to the workflow's base prompt every run, so you can pin voice, formatting rules, or edge cases without forking the workflow.

  ## Custom workflows in the dashboard

  Custom workflows now appear alongside the predefined ones on the [Workflows](https://dashboard.mintlify.com/products/workflows) page. Each card shows a preview of the workflow's prompt, and you can toggle it on or off or open the same settings panel to edit the trigger, review behavior, and instructions.
