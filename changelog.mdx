...
<Update label="February 20, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Editor search, Willow theme, workflows observability, and comments improvements" }}>

  ## Editor search

  Search across all your documentation pages directly in the [web editor](/editor/index). Find content instantly with highlighting, breadcrumb navigation, and a current page badge.

  ## Willow theme updates

  The [Willow theme](/customize/themes) now features a redesigned side navigation, improved topbar, and enhanced content layout with better table of contents styling and pagination.

  ## Workflows observability

  Monitor your [agent workflows](/agent/workflows) with new observability features in the dashboard, including run history and status tracking.

  ## Comments and collaboration

  - Support for @-mentions in editor comments to notify team members
  - Comment counts now display in the branch selector
  - Toggle the editor sidebar for a cleaner editing experience
  - Improved comment deeplinks for sharing specific discussions

  ## Notifications inbox

  New notifications inbox in the editor to track comment replies and collaboration activity.

  ## API playground improvements

  - Audio files now display as playable response examples
  - Support for boolean enums with single field selection
  - CSS variable support for API playground pages

  ## Editor improvements

  - Drag to add table rows and columns with visual tooltips
  - Improved code block focus and placeholder behavior
  - Fixed accordion IDs for duplicate titles to prevent anchor conflicts
  - Better anchor validation for ParamField, ResponseField, and Accordion components

  ## Rollover billing

  New billing UI to support rollover credits for assistant message allotments.

  ## Bug fixes

  - Fixed Sequoia theme layout shift issues
  - Fixed Windows path separator handling in CLI
  - Resolved large navigation cache handling
  - Fixed folder recreation after moving all files out
  - Fixed banner display for live preview and config sheet validation
