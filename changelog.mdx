...
<Update label="March 27, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Offline export, inline AI editing, visual diffs, Notion integration, and more" }}>
  ...
  The [web editor](/editor) now shows a rich visual diff when reviewing changes before publishing. Added, deleted, and modified blocks are highlighted with colored gutters and a minimap for navigation. Toggle between visual diff and markdown diff modes from the editor toolbar.
  ...
  - [Custom heading IDs](/create/text#custom-heading-ids) now work with `h5` and `h6` headings. Search results correctly deep-link to sections that use custom IDs.
