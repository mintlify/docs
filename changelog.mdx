...
<Update label="March 27, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Linear integration, API playground credentials, composable docs.json, and more" }}>

  ## Linear integration

  The [agent](/agent/linear) now integrates with Linear. Connect your Linear workspace and mention `@mintlify` in issues or comments to trigger documentation updates directly from your project management workflow.

  ## API playground credentials

  A new [`credentials`](/organize/settings-api) option is available for the API playground. When enabled with proxy disabled, API requests include cookies, auth headers, and TLS client certificates for cross-origin requests — useful for cookie-based auth or session tokens.

  ## Composable `docs.json`

  Split your `docs.json` configuration into multiple files using [`$ref`](/organize/settings) references. This makes large configurations easier to manage and reuse across documentation projects.

  ## Agent skills discovery endpoint

  A new `/.well-known/agent-skills/` [discovery endpoint](/ai/skillmd) follows a standardized format (v0.2.0) with `$schema`, `type`, `url`, and `digest` fields, making it easier for AI agents to discover your documentation's capabilities.

  ## Custom heading IDs

  Override auto-generated anchor IDs on headings using the `{#custom-id}` [syntax](/create/text). This gives you stable anchor links that don't break when heading text changes.

  ## Custom page layouts guide

  A new [guide](/guides/custom-layouts) covers page modes, hero sections, Cards and Columns, React components, custom CSS, and full landing page examples.

  ## Custom protocol support for MCP redirects

  [MCP redirect domains](/ai/model-context-protocol) now support custom protocol schemes (e.g., `myapp://`) in addition to `https://`.

  ## Headless feature availability chart

  A new [chart](/guides/custom-frontend) clarifies which features are available, unavailable, or must be self-built for headless documentation projects.

  ## Improvements

  - New `scoreThreshold` parameter (0–1) for filtering low-relevance [search](/api/assistant/search) results, and `pageSize` constraints (1–50) now documented.
  - [MCP](/ai/model-context-protocol) rate limits increased from 1,000 to 10,000 requests per hour per domain.
  - [Heap analytics](/integrations/analytics/heap) integration now tracks page views, search queries, feedback, context menu interactions, and navigation clicks.
  - Agent now [searches existing documentation](/agent/workflows) during the research phase for better context.
  - New CSS data attributes (`data-active-deepest`, `data-badge`, `data-color`, `data-size`) for [styling](/customize/custom-scripts) table of contents and badge components.
  - Notion and Linear added to [audit log](/dashboard/audit-logs) integration category.
  - Clarified that automerge requires [bypass permissions](/guides/configure-automerge) on all rulesets targeting the deploy branch.
  - OG image documentation clarified: `og:image` sets a static image, while `thumbnails.background` keeps the Mintlify overlay.

  ## Bug fixes

  - Fixed SEO metadata with overly long titles and descriptions across multiple pages.
  - Removed period delimiter support from variable names since they break parsing.
