...
<Update label="January 9, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Implicit snippets and assistant conversation insights for all" }}>
  ...
  Snippets now work without explicit configuration. Previously, you needed to define snippets in a specific `/snippets` directory. Now, any file can be imported as a snippet from anywhere in your docs for more flexible content reuse. See [Reusable snippets](/create/reusable-snippets) for more information.

  ## Assistant suggestions for all users

  [Conversation insights](/ai/suggestions#conversation-insights) are now available for all documentation sites that have both the agent and assistant enabled. The agent analyzes patterns in assistant conversations to identify documentation gaps and suggest improvements, helping you proactively address common user questions.
