...
<Update label="April 24, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Preview Deployment API, public repos in workflow context, Git onboarding redesign, web editor improvements, and more" }}>
  ...
  ## Public GitHub repos as workflow context

  [Workflows](/agent/workflows) now accept any public GitHub repository as a [context repository](/agent/workflows#context-repositories), even if it isn't connected to your organization. Add repositories by URL or in `owner/repo` format to reference open-source libraries, SDKs, or external documentation while the workflow runs. Public repositories can contain prompt injection or poisoned context, so only add ones you trust.
