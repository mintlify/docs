...
<Update label="April 3, 2026" tags={["New releases", "Improvements"]} rss={{ title: "get_page MCP tool, CLI authentication and analytics, password-protected previews, SAML group role mappings, and more" }}>

  ## `get_page` MCP tool

  The [MCP server](/ai/model-context-protocol) now includes a `get_page` tool that lets AI agents fetch full page content by path. Agents can retrieve complete documentation pages for deeper context beyond search results.

  ## CLI authentication

  Authenticate with Mintlify directly from the command line. Run `mint login` to sign in via the browser, `mint status` to check your session, and `mint logout` to sign out. The CLI now also checks for available upgrades when you run `mint dev`.

  ## CLI analytics

  View [analytics](/optimize/analytics) directly from the command line with `mint analytics`. Subcommands include `stats` for page view metrics, `search` for search query data, `feedback` for user feedback, and `conversations` for assistant conversation analytics.

  ## Password-protected preview links

  [Preview deployments](/deploy/preview-deployments) can now be password-protected, so you can share preview links with external reviewers without exposing unfinished content publicly.
  ...
  ## Spec-generated pages in the editor

  Pages auto-generated from OpenAPI specs now appear in the web editor as read-only entries, so you can see them in your navigation tree without risk of accidental edits.

  ## Redesigned configuration layout

  The dashboard configuration settings page has been redesigned with a cleaner layout and autosave support.

  ## Improvements

  - `mint openapi-check` is now part of `mint validate`. The standalone command is deprecated.
  ...
  - Editor navigation is now copied when creating a new branch, preserving your sidebar structure.
  ...
  - Authentication groups are now filtered to only include groups present in your docs, including for [MCP](/ai/model-context-protocol) authenticated search.
  - Slack agent responses now show tool call summaries instead of raw tool calls for a cleaner experience.
  ...
  - Fixed MCP OAuth redirect URI comparison when switching between `localhost` and `127.0.0.1`.
  - Fixed Badge and Icon visual diff detection in the editor.
  - Fixed video upload limit display to show the correct 20 MB maximum.
  - Fixed OpenAPI title and description not resolving for pages referenced by file path.
  - Fixed secure cookie handling for authentication flows.
