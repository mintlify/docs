...
<Update label="December 12, 2025" tags={["Improvements"]} rss={{ title: "Navigation configuration sheet", description: "Configure navigation properties directly in the web editor" }}>
  ## Navigation configuration sheet

  The web editor now includes a configuration sheet for navigation nodes, allowing you to edit properties directly in the UI:

  - Configure properties for all navigation types (tabs, anchors, groups, pages, versions, languages, products, dropdowns, items)
  - Edit titles, icons, colors, descriptions, and visibility settings
  - Manage OpenAPI/AsyncAPI specifications, keywords, and custom URLs
  - Toggle default states, public access, and expansion behavior
  - Real-time validation with error messages for invalid inputs

  Access the configuration sheet by selecting any navigation node in the editor sidebar.
