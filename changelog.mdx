...
<Update label="May 22, 2026" tags={["New releases","Improvements"]} rss={{ title:"Maximum search results setting and searchable hidden tabs and groups" }}>
  ## Maximum search results

  A new **Maximum search results** setting on the dashboard Search page lets you control how many results the in-product search bar returns per query. Tune this alongside search boosts to fit how readers navigate your docs. See [Search](/optimize/search) for details.

  ## Searchable hidden tabs and groups

  Hidden tabs and groups now support a `searchable: true` option that exposes their descendant pages to in-product search, the sitemap, and AI context — without making the rest of your hidden content discoverable. Use it when you rely on hidden tabs as a navigation-layout workaround (for example, per-product isolated sidebars) and still want those pages indexed. See [Hidden pages](/organize/hidden-pages) for the precedence rules and an example.
</Update>
