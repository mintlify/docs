...
<Update label="April 17, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Visibility component, MCP filesystem access, agent feedback, Git setup wizard, and more" }}>
  ...
  The new [`Visibility`](/components/visibility) component lets you show different content to human readers and AI agents on the same page. Wrap content in `<Visibility for="humans">` to display it only on the web, or `<Visibility for="agents">` to include it only in [Markdown output](/ai/markdown-export) consumed by AI tools. Useful for tailoring instructions, examples, or formatting to each audience without maintaining separate pages.

  ## MCP filesystem access

  [MCP servers](/ai/model-context-protocol) now include a query docs filesystem tool that lets AI agents browse and read your documentation's file structure directly. Agents can navigate directories, read full page content, and batch-read multiple pages in a single call—replacing the previous `get_page` tool with a more flexible interface.

  ## Agent feedback

  Enable agent feedback in your deployment settings to let AI agents submit structured feedback about your documentation quality. When turned on, feedback instructions are included in your `llms-full.txt` so agents know how to report issues they encounter. Available as a beta feature in the dashboard under deployment add-ons.
  ...
  - The web editor now shows a diff view so you can review changes before publishing.
  - Pasting valid MDX components into the editor now preserves component structure instead of converting to plain text.
  - Long navigation titles now hyphenate automatically to prevent overflow.
  - The editor handles deleted pages gracefully with a read-only banner and preserved content.
  - Relative links in `index.mdx` files now resolve correctly.
  ...
  - Fixed GitLab subgroup repositories failing to push in workflow sandboxes.
