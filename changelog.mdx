...
- Fixed `deepObject` parameter handling by passing style and explode props to ArrayParamField
- Moved PlaygroundModalWrapper to portal for better rendering
...
- Fixed video responses display in API playground
- Continued API playground refactor with improved OpenAPI document mapping

## Analytics and insights improvements

- Removed click-to-load requirement for analytics
- Added category filters in assistant bar chart
- Reversed order in bar chart tooltip for better readability
- Added empty state for analytics feedback (thumbs up/down)
- Fixed feedback page to respect new date filters
- Improved analytics data accuracy with server-side timestamps
- Added better error logging for analytics
- Dual-read implementation for analytics routes (KPIs, popular pages, feedback, referrals) with ClickHouse
- Fixed filtered chat thread pagination by overquerying
...
- Added support for monthly to yearly billing frequency changes (and vice versa)
- Fixed public page marking based on navigation configuration
- Added debug logging for public pages

## Performance and infrastructure

- Added in-memory OpenAPI file validation cache
- Improved update workflow performance with better logging
- Swapped to AWS ECR for Fluentbit log containers
- Set up Grafana for monitoring
- Added OpenTelemetry traces for better observability
- Bumped update worker cluster capacity

## Assistant and AI features

- Added error handling for assistant 429 rate limit responses
- Fixed assistant rate limiting to properly handle sliding time windows
- Added support for Claude 4.5 model
- Improved assistant search to query docs in parallel
- Added structured logging for writer errors
- Upgraded Agent to Claude Sonnet 4.5
- Added starter questions feature (server-side implementation)
...
- Made search engines ignore preview deployments
...
- Removed comments in final markdown stored in MongoDB
- Fixed directory reading support for read_external_files
- Added try-catch blocks to prevent silent failures
- Fixed stripMonorepoPath from preview links

## Component and styling

- Added dropdowns support in products division
- Fixed products and menu in middleware nav util function
- Used initial data from server components for better performance
- Added toggle support in client for starter questions
- Changed 404 page copy to be more clear
- Updated scraper targeting for GitBook

## Developer experience

- Adopted Unleash for feature flagging
- Deleted 'src' folder from CLI before zipping to reduce package size
- Upgraded Lucide icon version
- Bumped FontAwesome icon version
- Cleaned up request input merging logic
- Added unit tests for various features
- Added e2e test for AsyncAPI Playground
- Automated dangerous code change comments in PRs
- Added list_pull_requests and list_commits tools for AI writer
