...
<Update label="July 10, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Search filters, custom subpath hosting, network access controls, and more" }}>
  ...
  - **Support-triggered docs automation:** A new [automation](/automations) drafts documentation updates from customer support conversations, so recurring questions turn into docs fixes.
  ...
  - **`/call` page:** Sites configured with a phone contact now expose a `/call` route that forwards visitors to the dialer with the correct number and pause formatting.
  ...
  - **Pro plan relaunch:** The Pro plan and upgrade experience have been refreshed, with a prorated cost preview shown before you change credit tiers.

  ## Improvements

  - **Comments in the editor:** Inline comments are back in the [editor](/editor), with cleaner color contrast on delete suggestions and refined toolbar spacing.
  - **Assistant contact form opt-out:** Turn off the in-chat contact form from [assistant deflection settings](/optimize/analytics) if you don't want the assistant to hand off to support.
  - **Faster editor first paint:** The editor caches the navigation tree in the browser and streams docs from a local copy, so opening the editor loads noticeably faster.
  - **Mobile drawer redesign:** The mobile sidebar and mobile AI assistant now use a native drawer primitive for smoother open/close and better touch behavior.
  - **API playground:** Paste a JSON body into the [playground](/api-playground/overview) to auto-fill request body fields.
  - **Publish dropdown virtualization:** The editor publish list virtualizes long change sets so it stays responsive on large publishes.
  ...
  - Broken links automation copy is clarified to note it only checks internal links.
  - The `/call` page renders the site favicon and formats phone numbers with a two-comma dial pause.
  - Removed the first-credits discount UI from the credits purchase CTA when it no longer applies.
  - Client mono font swap flicker fixed.
