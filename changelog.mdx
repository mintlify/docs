...
<Update label="July 24, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"GraphQL and SDK references, editor review flow, and scoped navigation" }}>

  ## New features

  - **GraphQL API references:** Generate reference pages from a GraphQL schema alongside your existing OpenAPI references. See [GraphQL setup](/api-playground/graphql-setup).
  - **SDK reference pages:** Turn TypeDoc, DocFX, Javadoc, Sphinx, and PHPDoc artifacts into full reference pages at build time from a new `sdk` navigation tab. See [SDK reference setup](/api-playground/sdk-reference-setup).
  - **Editor review flow:** Approve drafts, publish them, or request changes with a dedicated review UI in the [editor](/editor). Reviews work on both drafts and branch-backed PRs.
  - **Top-level page comments:** Leave a [comment](/editor) on the whole page in the editor instead of only on selected text.
  - **`mint format` command:** Format every `.mdx` file in your project to Mintlify's canonical style with the same parser the editor uses. See [`mint format`](/cli/commands#mint-format).
  - **Filtered changelog URLs:** [Changelog](/create/changelogs) filters now persist in the URL so readers can share filtered views.
  - **ChatGPT connector for admin MCP:** Install the Mintlify [admin MCP](/ai/mintlify-mcp) as a ChatGPT connector to edit docs and open PRs directly from a chat.

  ## Improvements

  - **Scoped navigation for all deployments:** Path-scoped navigation is now enabled for both public and authenticated deployments, keeping large sidebars responsive as readers move between products and versions.
  - **Editor keyboard shortcuts:** New <kbd>Option</kbd> + <kbd>C</kbd> shortcut to [comment](/editor) on selected text and <kbd>Option</kbd> + <kbd>S</kbd> to toggle [suggesting mode](/editor/suggestions).
  - **Comment editing menu:** Comments now have a resolve button and an actions menu with edit and delete, so you can update a comment without deleting and reposting.
  - **@self mentions:** You can now @mention yourself in editor [comments](/editor).
  - **Editor agent moved to the right:** The [editor agent](/editor) panel now opens on the right side of the workspace for a more natural review layout.
  - **Cursor focuses at top of page:** Opening a page in the [editor](/editor) now places the cursor at the top of the document so you can start typing immediately.
  - **Docs search on preview deployments:** Preview builds now index for search, so you can test search behavior before merging.
  - **PR preview deep links:** GitHub PR preview links now open directly to the page changed in the PR instead of the site root.
  - **Deep feedback analytics:** The [feedback](/optimize/feedback) section adds per-page ratings and a detailed feedback drawer for reviewing reader responses.
  - **Human vs. agent page rankings:** Analytics page rankings split traffic into human views and agent views so you can see how each audience uses your docs.
  - **Real automation analytics:** The [Automations](/automations) page now shows live run analytics instead of placeholder data.
  - **Slack channel update recommendations:** Toggle recommended channel updates on or off directly from the Slack config modal in Automations.
  - **Deployable OpenAPI and AsyncAPI retirement metadata:** Add a `retiredAt` field to OpenAPI and AsyncAPI specs to mark specs as retired.

  ## Bug fixes

  - Fixed a client-side crash when a URL contained malformed percent-encoded characters.
  - Fixed self-referencing breadcrumbs so a page no longer links to itself in the trail.
  - Fixed centered content alignment in center page mode for the Luma and Sequoia themes.
  - PDF exports now force light mode so exported pages render correctly regardless of the reader's theme.
  - Internal links in raw `.md` pages and the `Link` header now include the configured base path when docs are hosted at a subpath.
  - Fixed a topbar CSS issue that could distort the header on narrow viewports.
  - Search now refreshes an expired end-user JWT in the authed search proxy instead of returning an error.
  - Editor suggesting mode now lets you type at the suggestion boundary immediately after exiting the mode.
  - Editor round-trips for pipes in inline code, latex math, table escapes, callout `type` attributes, video source children, task list checkboxes, and image titles now preserve source exactly.
  - Password-authenticated docs render as read-only when unentitled instead of crashing.
  - Fixed preview builds so they surface branch deletions instead of misleading errors.
  - `Cmd`+`I` in the editor only italicizes when text is selected.
