...
<Update label="May 12, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Editor agent attachments and history, expanded Slack agent context, MCP discovery, and more" }}>

  ## Editor agent improvements

  The [editor](/editor) agent now supports file attachments in chat, so you can drop in images, PDFs, or other reference material for the agent to use as context. The agent history panel is back, letting you revisit previous conversations from the editor. A new **Add to Assistant** action in the page toolbar sends the current page to the [assistant](/assistant) in one click.

  Editor agent usage now flows through the same credit-based billing as the rest of Mintlify, with a unified usage view across editor and assistant credits.

  ## File tree improvements

  You can now open unlisted Markdown files (pages not in `docs.json`) directly from the file tree, making it easier to find and edit orphaned content. Images, videos, and other media files in the file tree can be opened and previewed in place.

  ## Slack agent

  The [Slack agent](/agent/slack) gets several context and access improvements:

  - **Viewer-role access**: Users with viewer roles can now interact with the Slack agent using read-only tools, so the whole team can ask questions without needing edit access.
  - **Image context in threads**: The agent now includes prior images shared in a Slack thread when answering follow-ups, so screenshots stay in context across the conversation.
  - **Reactions in history**: Slack message reactions are now surfaced in the agent's history and thread tools, giving it better signal on which answers landed.

  ## MCP discovery endpoint

  Mintlify-hosted sites now serve a `/.well-known/mcp` discovery endpoint, so MCP clients can automatically find your [MCP server](/ai/model-context-protocol) without manual configuration.

  ## Custom domain TLS

  [Custom domains](/customize/custom-domain) now enforce a minimum TLS version on Cloudflare-managed hostnames, ensuring older insecure TLS versions are rejected for traffic to your docs.

  ## Assistant

  - Overages are now [disabled by default](/assistant/configure#allow-overages) for new [assistant](/assistant) plans. Enable overages from the dashboard if you want usage to continue past your included credits.
  - Fixed analytics that were double-counting thumbs-up/down feedback on assistant responses.

  ## Bug fixes

  - Fixed the editor agent paywall incorrectly appearing for paid customers.
  - Fixed editor tool call inputs not rendering when reloading a page mid-run.
  - Fixed live collaboration for hidden blocks in the editor.
  - Fixed editor OG images.
  - Fixed editor mobile layout issues, including drag-and-drop attachments and the agent panel default open state.
  - Fixed `Cmd+B` to reliably toggle the navigation sidebar.
  - Fixed Slack event acknowledgement so installs and reinstalls complete more reliably.
