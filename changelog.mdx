...
<Update label="December 5" tags={["Improvements"]} rss={{ title: "Weekly Updates", description: "API playground enhancements, web editor improvements, and bug fixes" }}>
  ## API playground improvements

  - Fixed duplicate query parameter descriptions that were appearing in API documentation
  - Fixed handling of falsy query parameters to properly filter them out
  - Fixed unique tab naming for discriminator patterns in OpenAPI schemas
  - Fixed iframe aspect ratio rendering issues
  - Improved handling of undefined response schemas to still show descriptions and dropdowns

  ## Web editor enhancements

  - Fixed image uploading from visual mode in the editor
  - Fixed sync operation to properly update currently opened files
  - Fixed branch creation workflow to correctly create folders when not bringing changes
  ...
  - Added editor debug menu for troubleshooting

  ## Assistant improvements

  - Fixed assistant submission to prevent empty messages from being sent
  - Fixed message submission during IME composition (for languages like Japanese, Chinese, Korean)
  - Improved assistant prompt handling for better responses
  - Fixed action menu dropdown dismissal behavior

  ## Component and styling fixes

  - Fixed styles for code inside links
  - Fixed nested lists rendering inside accordions
  - Fixed Steps component to hide line for last step item when content is empty
  - Fixed Japanese translations
  - Improved Box component resize handling

  ## Bug fixes and reliability

  - Fixed OpenAPI file matching to prevent incorrect filename associations
  - Removed gray-matter dependency in favor of safe front-matter for improved security
  - Fixed handling of complex Mermaid diagrams
  - Improved error boundaries in API playground v2
  - Fixed hash handling for URLs and table of contents
