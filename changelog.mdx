...
<Update label="September 25, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Webhook-triggered automations, image reading for agents, and more" }}>
  ...
  - **Editor:** HTML tables no longer lose rows on save, new page titles keep focus while you type, ordered lists in table cells keep their starting number, task lists written with `[X]` are no longer flagged as invalid MDX, checkboxes line up with bullets, image paste works on new private pages, and a memory leak when returning to the editor is fixed.
  - **Publishing:** Publishing works around navigation errors instead of blocking, publish selections that would break navigation are refused with a clear message, and the publish button no longer gets stuck in a render loop.
  ...
  - **Updates:** Stale OpenAPI pages are removed when a spec changes.
