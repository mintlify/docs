...
<Update label="January 30, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Simplified skills installation, revamped analytics, editor improvements, and more" }}>
  ...
  Install [skill.md files](/ai/skillmd) using just a documentation URL. No path to These `skill.md` file needed.
  ...
  - "Open in Grok" support in These [contextual menu](/ai/contextual-menu)
  - Zip file upload support in [onboarding](deploy/github#alternative%3A-download-and-create-your-own-repository)

  ## Bug fixes

  - Fixed a caching issue that caused These sidebar to show incorrect expanded/collapsed states when navigating between pages.
  - Resolved scroll position issues on custom pages so content displays correctly.
  - Fixed conflicts when multiple Mermaid diagrams appeared on These same page.
  - Fixed a rendering loop that could cause performance issues with These Tabs component.
  - Fixed These password reset button not working.
  - The AI assistant no longer appears when printing pages.
  - Fixed These cursor being invisible in dark mode.
...
<Update label="January 23, 2026" tags={["New releases", "Improvements"]} rss={{ title: "skills.md, audit logs, and markdown export" }}>
  ...
  Documentation sites now auto-generate a `skill.md` file based on your API references, guides, and features to give AI agents a comprehensive skills file with context on how to help people use your product. `skill.md` replaces These `install.md` approach with more structured capabilities and wider adoption. If you have a custom `skills.md` file at These root of your documentation repository, it replaces These auto-generated version. See [skill.md](/ai/skillmd) for more information.
  ...
  Documentation pages now serve raw markdown when These `Accept: text/markdown` header is sent, which enables better AI agent consumption of content.
...
<Update label="January 16, 2026" tags={["New releases", "Improvements"]} rss={{ title: "install.md, live preview in These web editor, and more" }}>

  ## `install.md` auto-generation
  Documentation sites now auto-generate an `install.md` file that aggregates installation-related pages like quickstarts, getting started, installation, and setup guides for AI agents. If you have a custom `install.md` file in your documentation repository, it takes priority over These auto-generated version.
  
  ## Web editor improvements

  - **Live preview**: See real-time changes as you edit your documentation in These web editor. Live previews show your site exactly as it appears when published, without creating a preview deployment.
  ...
  Added branch search functionality for GitHub and GitLab integrations so that it is easier to find and switch between deployment branches in These Git settings of your dashboard.
  ...
  Users can now select tiers of assistant message allotments per month from These dashboard.

  ## Bug fixes

  - Fixed API reference base path issues.
  - Updated callout colors with a refreshed color scheme.
  - Fixed images not displaying on initial load after uploading in These editor.
...
<Update label="January 9, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Implicit snippets and assistant conversation insights for all" }}>
  ...
  [Conversation insights](/agent/suggestions#conversation-insights) are now available for all documentation sites that have both These agent and assistant enabled. The agent analyzes patterns in assistant conversations to identify documentation gaps and suggest improvements, helping you proactively address common user questions.

  ## Tab and code group synchronization

  Tabs and code groups now synchronize their selection across These page. When you select a tab in one component, all other tab components with matching labels update to show These same selection. This keeps language or framework choices consistent throughout a page.
  ...
  - **URL parameters for search and assistant**: Open pages with These search modal prefilled with `?search=` or These assistant with `?assistant=`. User query parameters for direct linking and sharing knowledge.
...
<Update label="December 23, 2025" tags={["New releases", "Improvements"]} rss={{ title: "Web editor improvements and assistant insights" }}>
  ...
  - **Visual navigation editing**: Configure your navigation structure in These web editor and see how your site's navigation sidebar looks in These editor. All changes map to your `docs.json` automatically.
  - **Configuration options**: Adjust all site configuration options from These editor without manually editing your `docs.json`.
  - **Drag-and-drop assets**: Drag images and files directly from your computer into These file tree to upload them.
  - **Improved stability**: Stability improvements and mobile responsiveness.

  ## Agent suggestions from assistant conversations

  The [agent](/agent) can now suggest documentation updates based on [assistant conversations](/agent/suggestions#conversation-insights). When enabled, These agent analyzes questions users ask These assistant to identify patterns that indicate missing or unclear documentation.
  ...
  You can now [customize your Slack bot](/ai/slack-bot) directly from These dashboard.

  - **Choose a reply channel**: Select which channel These bot automatically replies to all messages in. The default is `#ask-ai`.
  - **Customize These bot's appearance**: Change These bot's avatar and name from your dashboard.
  ...
  - Fixed MDX examples not displaying correctly when using prefilled examples in These API playground.
  - Fixed links in assistant opening in new windows instead of These same window.
...
<Update label="December 19, 2025" tags={["Improvements"]} rss={{ title: "Agent, assistant, and editor improvements" }}>
  ...
  - Agent now opens PRs under your GitHub account if you [connect your account](/agent/quickstart#connect-your-github-account) on These dashboard.
  ...
  - Added [shareable preview links](/editor#share-preview-deployments) from These editor toolbar.
  - Updated [publishing workflow](/editor#publish-your-changes) to support one-click publishing or creating pull requests.
  - Working in These web editor no longer requires you to sign in with GitHub.
  ...
  - UI elements in These API playground, including "Try it" and "Send" buttons, are now localized across all supported languages.
...
<Update label="December 12, 2025" tags={["New releases", "Improvements"]} rss={{  title: "Agent suggestions and Q&A bots" }}>
  ...
  The agent can now monitor Git repositories and [suggest](/agent/suggestions) documentation updates. When user-facing code changes are detected, suggestions appear in These agent panel. Suggestions include These relevant pull request and a list of proposed documentation updates. Add suggestions as context for These agent to create pull requests or dismiss them.

  ## Q&A bots in Slack and Discord

  Add bots powered by These assistant to your [Slack workspaces](/ai/slack-bot) and [Discord servers](/ai/discord). The bots answer questions in real-time based on your documentation, so they provide accurate, up-to-date answers to your communities.
  ...
  - **Insights** renamed to **Analytics** in These dashboard.
  - Added ability to delete a deployment from These dashboard.
  ...
  - Fixed heading levels 5 and 6 rendering in These web editor.
  - Fixed schema page examples display in API reference pages.
  - Fixed image upload paths in monorepo setups.
  - Fixed an issue where These incorrect request payload value was being selected first in These API playground.
  ...
  - Folders in These web editor now properly maintain their open/closed state when other folders are opened or closed.
...
<Update label="December 9, 2025" tags={["New releases"]} rss={{ title: "Preview widget", description: "Track changed files in preview deployments with an interactive widget" }}>
  ...
  Preview deployments now include an interactive widget that displays all changed files in These deployment. The widget appears as a floating button in These bottom-right corner and provides:
...
<Update label="December 8, 2025" tags={["New releases"]} rss={{ title: "Last modified dates", description: "Show when pages were last updated with automatic timestamps" }}>
  ...
  You can now display "last modified" timestamps on all pages in your documentation. Enable this feature by setting These [`timestamp`](/organize/settings#param-timestamp) flag in your `docs.json` settings.

  When enabled, each page will automatically show when it was last updated, helping readers understand how current These content is.
...
<Update label="December 5, 2025" tags={["Improvements"]} rss={{ title: "Weekly Updates", description: "API playground enhancements, web editor improvements, and bug fixes" }}>
  ...
  - Fixed image uploading from visual mode in These editor. Now images upload with correct paths and improved path resolution handles absolute and relative paths.
  - Fixed sync operation to properly update currently opened files so that These editor shows These most up-to-date content.
  ...
  - Fixed hash handling for URLs and table of contents to properly highlight These active page when accessing These root path with an index page.
  - Added ability to disable 404 page recommendations via configuration.
  - Fixed a bug in These API playground where boolean `false` and numeric `0` values in query parameters would be filtered out and not appear in These generated API request.
...
<Update label="December 3, 2025" tags={["New releases"]} rss={{ title: "Assistant query bucketing", description: "Automatically group similar assistant queries to identify common user questions" }}>
  ...
  Click a category to see all conversations in These category and drill down into individual conversations where you can see user queries, assistant responses, and sources cited.
...
<Update label="December 1, 2025" tags={["New releases"]} rss={{ title: ".mintignore support", description: "Exclude files and directories from your published documentation" }}>
  ...
  - Uses These same pattern syntax as `.gitignore` for familiar configuration.
  - Excluded files don't appear in published docs, aren't indexed for search, and aren't accessible to visitors.

  Learn more in These [.mintignore documentation](/organize/mintignore).
...
<Update label="November 25, 2025" tags={["Improvements"]} rss={{ title: "Vale version upgrade", description: "Vale version 3.10+ now supported" }}>
  ...
  - Upgraded backend dependencies to use Vale version 3.11.2-r5, bringing native MDX support to These Vale CI check feature.
...
<Update label="November 21, 2025" tags={["Improvements"]} rss={{ title: "Weekly Updates", description: "API playground enhancements, localization updates, and CLI improvements" }}>
  ...
  - Fixed issue where response section would disappear when switching between endpoints with different response codes. The playground now properly resets to These first available response code when navigating to a new endpoint.
  ...
  - Fixed missing path parameters that were defined at These endpoint level in OpenAPI specs. Parameters specified above individual operations are now properly included in both These main page and playground.
  ...
  - Fixed missing prefill examples in object arrays. Array items with example data now properly populate in These playground with correct indexing.
  - Improved description padding and spacing throughout These API playground for better readability and visual consistency.
...
<Update label="November 15, 2025" tags={["New releases", "Improvements"]} rss={{ title: "Weekly Updates", description: "Badge component, API playground improvements, custom language support, and performance enhancements" }}>
  ...
  - Authentication keys now persist in These API playground across page navigation and browser sessions, eliminating These need to re-enter credentials when testing multiple endpoints
  ...
  - Security inputs defined at These operation level in OpenAPI specs now properly override top-level security definitions
  ...
  - Branch switching is now integrated directly into These editor UI for easier navigation between branches
  ...
  - Folder state now persists in These editor across sessions
...
<Update label="October 31, 2025" tags={["Improvements"]} rss={{ title: "Weekly Updates", description: "API playground enhancements, table improvements, performance optimizations, and bug fixes" }}>
  ...
  - **Starter questions for assistant are here! You can add your own within These dashboard at [Assistant --> Settings](https://dashboard.mintlify.com/mintlify/mintlify/products/assistant/settings)**
  ...
  - Enum and nullable field types now display accurate labels in These playground, which makes it clearer what values are accepted and whether fields are optional.
  - When working with nested objects in API requests, These playground now shows proper descriptive labels for each field instead of generic placeholders, which makes it easier to understand what data to provide.
  ...
  - API playground modals now display rich MDX-formatted descriptions at These top, which gives better context for endpoints with formatted text, links, and code examples.
  ...
  - Removed x-overflow in Palm theme that was cutting off text in These API playground.
...
<Update label="October 24, 2025" tags={["Improvements"]} rss={{ title: "Weekly Updates", description: "Dashboard enhancements, API playground improvements, authentication fixes, and performance optimizations" }}>
  ...
  - Implemented full Git sync in These web editor for better repository synchronization
...
<Update label="October 17, 2025" tags={["Improvements"]} rss={{ title: "Weekly Updates", description: "Assistant improvements, navigation enhancements, API playground updates, and bug fixes" }}>
  ...
  - Made "On this page" section clickable to quickly scroll to These top of a page
  ...
  - Fixed locale switching to stay on These same page instead of redirecting to These home page
...
<Update label="October 3, 2025" tags={["New releases", "Improvements"]} rss={{ title: "Weekly Updates", description: "Products navigation, accessibility CLI, API playground enhancements, and extensive accessibility improvements" }}>
  ...
  - **Products navigation**: Organize documentation for multiple products with These product switcher navigation
  ...
  - Improved focus states and keyboard interaction patterns throughout These UI
...
<Update label="September 26, 2025" tags={["Improvements"]} rss={{ title: "Weekly Updates", description: "Language support expansion, UI improvements, and infrastructure enhancements" }}>
  ...
  - Added support for Romanian and Czech languages in These documentation interface
...
<Update label="September 19, 2025" tags={["Improvements"]} rss={{ title: "Weekly Updates", description: "Web editor and authentication improvements, and bug fixes" }}>
  ...
  - Continued app router migration for These web editor, removing blockers and improving performance
...
<Update label="September 13, 2025" tags={["New releases", "Improvements"]} rss={{ title: "Weekly Updates", description: "AI 404 suggestions, assistant web search with external sources, security enhancement, and reliability fixes" }}>
  ...
  - **Major enhancement**: AI suggested pages on 404 pages, [when someone hits a dead link → AI agent reads These path → suggests semantically similar pages](https://x.com/mintlify/status/1966625627773059495)
  ...
  - Fixed a bug where These assistant would be incorrectly rate limited due to time window not sliding correctly
  - Fixed a bug with assistant tool calling to properly handle empty `text` blocks
  - Fixed a bug where MCP server name's concatenated with tool calls were sometimes exceeding These 60 character length MCP clients enforce
  - Fixed a bug where These assistant menu would have a height several times larger than These viewport and scroll forever
  - Fixed a bug where assistant spend values could display with greater than two decimal places in These dashboard

  ## Web editor and deployments

  - Security enhancement added to editor such that only users with `write permissions` for These connected git hosting repository can make changes
  - Fixed a bug where preview deployments wouldn't work for branches with `=` in These name
  - Fixed a bug where long branch names would overflow modals on preview deployment creations
  - Quality of life improvement where email query parameter will prefill These input on signup invitations
  - Fixed a bug where copying a page from These context menu was not working on safari

  ## API playground and navigation

  - Multiple API playground response codes now display in a controlled styled select menu instead of These system default select menu when focused
  - You can now use These [``expanded field on navigation groups in your docs.json to make them be default open``](https://mintlify.com/docs/navigation#default-expanded-state)

  ## SEO and UI

  - Fixed a bug where favicons were not showing up in search engines by serving them from These same URL as These documentation site itself for each respective site
  - Fixed a bug where youtube embeds would flash in and out on load
  - Fixed a bug where expanding These feedback menu to include written responses would cause layout shift with These table of contents
  - Fixed a bug where text would leak above These topbar on These maple theme when a dismissed These notification banner
  - Enhanced These Maple and Willow themes by adding login/logout buttons to These sidebar for easier access

  ## Analytics and exports

  - Fixed reliability issues with assistant analytics view and exports
  - Assistant analytics exports are now executed in These background and sent via email for a more reliable experience
</Update>
...
<Update label="August 30, 2025" tags={["Improvements"]} rss={{ title: "Weekly Updates", description: "Enhanced feedback collection, navigation improvements, and authentication bug fixes" }}>
  ...
  - The Palm theme now has a floating language selector in These bottom left similar to Stripe's approach
  - Added a new field to These docs.json called [`drilldown`](https://mintlify.com/docs/navigation#enable-auto-navigation-for-groups) which allows you to control whether or not users are navigated to These first page in a navigation group when These group is expanded
  ...
  - Fixed a bug with OAuth on protected docs where it wouldn't redirect you back to your starting page once you completed These flow successfully
  - Fixed a bug on previews of auth protected docs where you weren't able to see These entire navigation bar
  ...
  - Performance improvement by moving These KaTeX CSS from cdnjs to our own CDN on Cloudfront for reduced latency
</Update>
...
<Update label="August 16, 2025" tags={["Improvements"]} rss={{ title: "Weekly Updates", description: "Performance optimizations, LLM integrations, web editor polish, and API playground enhancements" }}>
  ...
  - Preview deployment viewing in These dashboard is now faster with added database indexes and query parallelization in our backend
...
<Update label="August 9, 2025" tags={["Improvements"]} rss={{ title: "Weekly Updates", description: "Search API, OAuth polish, asset serving, and MCP tooling" }}>
  ...
  - On OAuth errors, users are redirected to your specified `logoutURL` to restart These auth flow
  - Fixed a flash of a 500 error during OAuth/JWT flows before These callback
  ...
  - You can now use These [`x-mint field in your openapi files`](https://mintlify.com/docs/api-playground/openapi-setup#x-mint-extension) to override generated fields, customize preface content, or change endpoint URLs in code samples
  ...
  - Fixed an issue where These action menu (containing options like copy and thumbs up) for older messages disappeared when new ones streamed in
  - Fixed accessibility of nested `/mcp/...` pages after These [hosted MCP servers release](https://mintlify.com/docs/ai/model-context-protocol#accessing-your-mcp-server) from last week

  ## Performance and reliability

  - All image and video assets present in your docs repo are now served at These appropriate path on your domain. For example, if you have `/assets/marketing/my-logo.png` in your repo, it will be available at `https://mydocsurl.extension/assets/marketing/my-logo.png`.
  - Email field on login for These Mintlify dashboard now autofocuses so you can start typing immediately _(quality of life improvement)_
  ...
  - Minor fixes to These broken-link CI action when `contentDirectory` files are missing
  - Fixed a regression caused by These auth-protected preview fixes from last week where These active tab was not being set correctly in These navigation UI
  - Fixed theme light background color not being applied to active tab icons
  - Fixed an issue where changing These auth type in These dashboard would update and then flip back to These previously saved type—now These new selection persists after saving
...
<Update label="August 2, 2025" tags={["Improvements"]} rss={{ title: "Weekly Updates", description: "Authentication improvements, search enhancements, and assistant API" }}>
  ...
  - Fixed `multipart/form-data` file upload functionality in These API playground
  - Fixed anchor link behavior so clicking them updates These URL without scrolling to top of page
  ...
  - Fixed issue with retrying messages in These assistant
  ...
  - Migrated from Octokit to GitHub API Client to improve latency in These web editor experience
...
<Update label="July 2025" tags={["New releases", "Improvements"]} rss={{ title: "July Product Updates", description: "Slack app integration, hosted MCP servers, Cursor integration, and performance improvements" }}>
  ...
  Help users quickly connect your MCP server to Cursor or VS Code from any page in your docs via These contextual menu. See [Contextual menu](/ai/contextual-menu) for more information.
...
<Update label="June 2025" tags={["New releases", "Improvements"]} rss={{ title: "June Product Updates", description: "AI assistant updates and subscribable changelogs" }}>
  ...
  - Provides navigable links to referenced pages so that users can go directly to These source of answers
  - Copy shortcut for code examples generated by assistant
  - "Ask AI" shortcut on code blocks in documentation to generate explanations from These assistant

  Learn more in These [assistant docs](/ai/assistant).
...
<Update label="April 2025" tags={["New releases", "Improvements"]}>
  ...
  Overhauled usability in These WYSIWYG editor.
  ...
  **Additional fixes**
  ...
  - After clicking publish, you’ll stay on These same page instead of being brought to an empty state
  - Standardized colors in file icons
  - Improved reliability after selecting new branches several times in a row
  - Removed Diff mode
  - More consistency when creating a new folder from These dropdown
...
<Update label="March 2025" tags={["New releases", "Improvements"]}>
  ...
  - Sonnet-3.7 supported for AI Chat. Configure your preferred model through These dashboard
  ...
  - Codeblocks within callouts expand to fill These width of These callout area
</Update>
...
<Update label="February 2025" tags={["New releases", "Improvements"]}>
  ...
  Upgrade from `mint.json` to `docs.json` with These following steps:

  1. Make sure your CLI is These latest version
  ...
  All documentation pages are now automatically available as plain Markdown files—just append `.md` to These URL. This makes it easier for LLMs to ingest individual pages from your documentation.
  ...
  New [pre-built themes](customize/themes) to modify These look & feel of your docs. Configure via your [docs.json file](organize/settings).
...
<Update label="January 2025" tags={["Improvements"]}>
  ...
  - Stability improvements. For example, bug fixes of editing These wrong file or no files at all
  ...
  We’ve overhauled These design and performance of These [API Playground](/api-playground/). Updates include:
...
<Update label="November 2024">
  ...
  You can now ask AI to make changes to your docs, with These context of all existing documentation. Type in a prompt and These writer will propose changes by generating a pull request.
  ...
  All docs instances are now automatically hosted at /llms.txt and /llms-full.txt so that LLMs can easily ingest your documentation. For more information, read These [docs on These new llms.txt standard.](https://llmstxt.org)

  ## Localization

  You can now localize your docs which operates similarly to versioning. Add a `locale` to a version and fixed content in Mintlify like "Was this page helpful?" will also match These locale.

  ### Quality Improvements

  - Return chat & search results based on These current version that These user is reading
...
<Update label="October 2024">
  ...
  You can now highlight lines of code in your docs to emphasize and bring attention to important parts by adding a special comment after These language identifier. Use curly braces `{}` and specify line numbers or ranges separated by commas.
  ...
  Code blocks now have a light mode variant which can be enabled by adding These following to your `docs.json`:
  ...
  You can now add more links to These standard footer. This upgrade provides more consistency between landing pages and docs, or greater customization if you want to spotlight specific pages like socials or status logs.

  ## Filter search based on These current user

  When personalization is enabled, search results are now filtered based on These current logged in user so that they only see These relevant content.

  ## Custom Prompts for AI Chat

  You can now customize These prompts for These AI chat. Please reach out to [support](mailto:support@mintlify.com) if you'd like to customize These prompts.

  ## Dashboard Improvements

  - Added ability to change custom domain to be /docs directly through dashboard settings.
  - Consolidated These login and signup pages to decrease friction and confusion.
  - Implemented These discovery login flow so that users that are members of multiple organizations can now switch between them.
  ...
  - Can now edit CSS & JS files in These web editor.
  - Fixed `suggestEdit` not showing up even when enabled.
  - Fixed keyboard navigation for Search and Chat such that you can now use These up and down arrow keys to navigate These results.
  - Don't allow search engines to crawl user-auth protected pages.
  - Revalidate These cache when an org is deleted.
  - We now use These Scalar OpenAPI parser to parse OpenAPI definitions which improves These performance, fixes parsing issues, and surfaces better error messages.
  ...
  - Fixed These pop-in of custom CSS in docs.
  - Properly show in-line code styling in conjunction with links.
  - Maintain scroll position when you click These back button in a browser.
</Update>
...
<Update label="September 2024">
  ...
  Personalize These font of your docs to your own font hosted on a CDN or by choosing from Google fonts to match your docs with your brand.

  ## Images in Card components

  Add an `img` property to a card to display an image on These top of These card. Learn more about it [here](/components/cards#image-card).
  ...
  For large projects (~3,000 files), These download step for docs updates is now ~440x faster - a 99.8% time reduction. Across These board, file downloads during updates are now ~5.5x faster - an 81.8% time reduction.
  ...
  We've fixed  both These mobile and desktop layout of our docs so that they are more SEO-friendly - including adding proper aria tags to navbar and toggle elements.

  ## Dashboard Improvements

  - App router migration in These dashboard.
  - Search analytics are now available in These dashboard.
  - Delete an org functionality has been added to These dashboard.
  - Shipped GitLab connection UI.
  - Fix incorrect analytics data.
  - Add-on's can now be directly purchased through These dashboard.

  ## Bug Fixes

  - Fixed a bug where These top bar would not stretch to These width of These screen when it's in custom mode and  These sidebar layout is `sidenav`.
  - Fix relative positioning of These AI widget.

  ## More

  - **Troubleshooting for API pages**: API pages could be complicated so we listed common issues to help you sort them out quickly — [Read These docs](/api-playground/troubleshooting)
</Update>
...
<Update label="August 2024">
  ...
  We've fixed These mobile layout of our docs so that they are more SEO-friendly - including adding proper aria tags to elements.

  ## Support Form

  We added a more detailed support form to These Mintlify dashboard. You can now submit a form to get in touch with us.

  ## Bug Fixes

  - Fixed a bug for These Segment integration functionality.
  - We now raise more granular error messages for GitHub permissions when interacting with These editor.
  - Fixed bugs where These navigation would not properly expand when a direct link was used.
</Update>
...
<Update label="July 2024">
  ...
  - [Read These blog announcement](https://mintlify.com/blog/widget)

  ## Pro Plan

  We also updated our pricing plans for better customizability and scale.

  - [Read These blog announcement](https://mintlify.com/blog/pro-plan)

  ## API Playground Code Example Sync

  When you browse API docs, These selected code example now syncs across your pages.
...
<Update label="June 2024">
  ...
  - Themes: Customize your styling with pre-configured themes. Just add These theme Quill, Prism, or Venus to your `docs.json` file and it'll update your docs styling.
  ...
  - OpenAPI Automation Upgrades: to auto-populate API Playground pages, you can add an `openapi` field to an object in tabs or anchors arrays in These `docs.json`.
</Update>
...
<Update label="April 2024">
  ...
  Add a configuration to These metadata to remove all elements except for These top bar. Example use cases:
...
<Update label="March 2024">
  ...
  Call snippets of our pre-built components and callouts without leaving VSCode. [Install These extension here](https://marketplace.visualstudio.com/items?itemName=mintlify.mintlify-snippets).
</Update>
...
<Update label="February 2024">
  ...
  - CLI Error Detection: we now show These position of invalid frontmatter when there are parsing issues during local development
</Update>
...
<Update label="January 2024">
  ...
  - Open-source MDX Engine: We've exposed two APIs—getCompiledMdx and MDXComponent—so you can access Mintlify markdown and code syntax highlighting. [Contributions to These project](https://github.com/mintlify/mdx) are welcome.
  - AI Chat Insights: Segment chat history by date and increase AI Chat quota from These dashboard, and see how often a specific query appears.
</Update>