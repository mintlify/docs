...
<Update label="March 6, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Related pages, contextual starter questions, and editor improvements" }}>

  ## Related pages

  Documentation pages can now display automatically generated related topics at the bottom of each page. The feature uses semantic analysis to recommend relevant pages based on content similarity. Configure the mode (`automatic`, `manual`, or `off`) and the number of displayed topics from the dashboard.

  ## Contextual starter questions

  The [assistant](/ai/assistant) now shows contextual starter questions based on the current page. Questions are tailored to help users find information relevant to the documentation they're viewing.

  ## Editor improvements

  - New global search bar across all editor pages for faster navigation.
  - Nested drag-and-drop support for lists makes it easier to reorganize content.
  - Focus states on code blocks improve keyboard navigation.
  - Open files directly in the editor from the preview widget.

  ## Improvements

  - AsyncAPI pages now work with the "Copy page" markdown export feature.
  - Search autocomplete now highlights exact matches in results.

  ## Bug fixes

  - Fixed cursor jumping when pasting content in the editor.
  - Fixed analytics tooltip display on dashboard cards.
