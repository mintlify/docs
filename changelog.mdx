...
<Update label="July 3, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Custom base paths, voice/iMessage agent tools, AsyncAPI expansion, and more" }}>

  ## New releases

  - **Custom base paths for hosted docs:** Serve your docs under a custom subpath on your domain. Navigation, in-content links, `llms.txt`, and redirects all respect the configured mount path. See [docs subpath](/deploy/docs-subpath).
  - **Voice and iMessage agent tools:** The [assistant API](/ai/assistant) now includes a `text_call_number` tool and a token-gated cross-site docs search endpoint so voice and iMessage agents can look up answers and route callers to the right number.
  - **Native Slack read tools for the Slack agent:** The [Slack agent](/agent/slack) uses a native bot token to read Slack context directly, improving retrieval quality and reasoning-message reliability.
  - **AI-generated preview editor:** Onboard by describing your docs and letting Mintlify draft a preview site from your code. The [preview editor](/editor) streams progress, asks clarifying questions when needed, and lets you publish when ready.
  - **Group-to-role mapping UI:** Configure SAML/OIDC group-to-role mappings from the [SSO](/dashboard/sso) settings page instead of contacting support.
  - **Scheduled automations via webhook:** Trigger a scheduled [automation](/automations) run from an external system through a webhook, in addition to its cron schedule.
  - **`<Meter />` primitive:** Render progress or usage meters inline with the new `<Meter />` component.
  - **Mixpanel data residency region:** Choose the Mixpanel data-residency region when configuring [analytics integrations](/integrations/analytics/mixpanel).

  ## Improvements

  - **CIDR ranges in session IP allowlist:** The [SSO session IP allowlist](/dashboard/sso) now accepts CIDR ranges in addition to individual IPs.
  - **AsyncAPI `expanded: 'all'`:** Set `api.params.expanded: 'all'` to expand every parameter group by default for [AsyncAPI](/api-playground/asyncapi-setup) reference pages, matching existing OpenAPI behavior.
  - **Deployment-level agent review process:** Choose the review process the [agent](/agent) uses per deployment from agent settings.
  - **Assistant contact form lead capture:** The in-chat [assistant contact form](/assistant) collects lead fields (name, email, company) and forwards them to the support or sales email.
  - **Search breadcrumbs across divisions:** Search results include the full division path in breadcrumbs so users can tell which product a result belongs to.
  - **Auto-apply updates for Mintlify-hosted repos:** Automation updates apply automatically for deployments hosted on Mintlify-managed repos, without GitHub links or manual PR review.
  - **Preview sandbox revival and attachments:** Stopped preview sandboxes are revived on next use, and the preview editor accepts message attachments.
  - **"Open in Devin Cloud":** The Devin handoff button is renamed to "Open in Devin Cloud" for accuracy.

  ## Bug fixes

  - `nullable: true` is now preserved inside `allOf` compositions when rendering OpenAPI schemas.
  - Multi-repo deployments correctly attribute RSS `lineBlame` to the source repo.
  - Slack agent errors surface the correct GitLab provider name.
  - AI preview editor sessions stuck in `initializing` now recover automatically.
  - `docs.json` warning badge in the publish dropdown wraps properly on long text.
  - The dashboard uses the correct Inter font weight throughout.
