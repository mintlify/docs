...
<Update label="December 10, 2025" tags={["Improvements"]} rss={{ title: "Dashboard improvements", description: "Enhanced agent panel with suggestions UI and repository monitoring" }}>
  ## Agent panel improvements

  - Added new suggestions panel in the agent tab to view and manage documentation suggestions from monitored repositories
  - Improved repository monitoring configuration with organization-level management and bulk enable/disable actions
  - Enhanced PR suggestion workflow with "Add to chat" functionality for seamless integration with the agent
  - Added skeleton loading states for better user experience while fetching suggestions
  - Improved visual hierarchy and spacing throughout the agent panel interface
