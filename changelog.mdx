...
<Update label="November 21" tags={["Improvements"]} rss={{ title: "Weekly Updates", description: "API playground enhancements, localization updates, and CLI improvements" }}>
  ...
  - Enhanced translations for contextual menu and assistant across all supported languages including Arabic, Czech, German, Spanish, French, Hindi, Indonesian, Italian, Japanese, Korean, Latvian, Dutch, Norwegian, Portuguese, Romanian, Russian, Swedish, Turkish, Ukrainian, and Chinese (Simplified and Traditional)
  - Added missing translations for "Searching for", "Searched", "Found results for", "Generating", and assistant disclaimer text
  - Completed translations for MCP Server and IDE connection options (Cursor and VS Code) across all languages

  ## CLI improvements

  - Added error message for users running `mint dev` on Node.js versions below 20.17, providing clear guidance to upgrade to an LTS version
  - Improved error handling to help users quickly identify and resolve Node.js version compatibility issues

  ## Bug fixes and reliability

  - Fixed issue where primitive parameters without content would show unnecessary separators in the API playground
  - Improved handling of response section rendering when switching between endpoints with different available response codes
  - Enhanced type labels for security scheme parameters to display correct format information
