...
<Update label="December 23, 2025" tags={["New releases", "Improvements"]} rss={{ title: "Web editor improvements and assistant insights" }}>
  ...
  - **Reorganized documentation**: Editor documentation is now split into dedicated pages for [pages](/editor/pages), [media](/editor/media), [navigation](/editor/navigation), [configurations](/editor/configurations), [collaboration](/editor/collaborate), [publishing](/editor/publish), and [keyboard shortcuts](/editor/keyboard-shortcuts).
  - **Drag-and-drop assets**: Drag images and files directly from your computer into the file tree to upload them.
  - **Resizable navigation panel**: Resize the navigation sidebar to your preferred width.
  - **Improved navigation tree**: Better styling, scrollbar behavior, and accordion list display.
  - **Duplicate content shortcuts**: Use keyboard shortcuts to quickly duplicate content in the editor.

  ## Assistant insights

  The [agent](/ai/agent) can now suggest documentation updates based on [assistant conversations](/ai/suggestions#conversation-insights). When enabled, the agent analyzes questions users ask the assistant to identify patterns that indicate missing or unclear documentation.

  This adds a second source for agent suggestions alongside pull request monitoring, helping you proactively address common user questions.

  ## Slack bot customization

  You can now [customize your Slack bot](/ai/slack-bot) directly from the dashboard:

  - **Choose your channel**: Select which channel the bot automatically replies to all messages in, instead of only `#ask-ai`.
  - **Customize appearance**: Change the bot's avatar and name from the Mintlify dashboard.
