...
<Update label="October 17" tags={["Improvements"]} rss={{ title: "Weekly Updates", description: "Assistant improvements, navigation enhancements, API playground updates, and bug fixes" }}>
  ...
  - Improved assistant positioning to display underneath pagination instead of overlapping content
  - Fixed assistant disappearing bug and improved visibility on small pages
  - Added session tracking improvements for better analytics

  ## Navigation and UI enhancements

  - Added stable scrollbar gutter to navigation components for consistent layout
  - Made "On this page" section clickable to scroll content container to top
  - Fixed assistant bar positioning across all themes (Almond, Aspen, Linden, Maple, Palm, Willow, Mint)
  - Improved assistant bar to use portal rendering to prevent overlapping issues
  ...
  - Fixed locale switching to stay on the same page instead of redirecting to home
  - Fixed Spanish translation typo (busca → buscar)
  ...
  - Fixed localStorage failures in e2e tests with isomorphic storage utility

  ## Developer experience improvements

  - Added development panel with feature flag toggles (dev only)
  - Re-enabled feature flags in local client for easier testing
  - Added super-secret key command for dev panel access
  - Improved error handling and logging for better debugging
  - Added HubSpot tracker integration
  - Updated contact email routing in dashboard for add-on requests
</Update>
