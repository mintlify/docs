...
<Update label="August 21, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Branches v2, editor wiki mode, bulk invites, and more" }}>

  ## Editor and collaboration

  - **Branches v2 and version history:** A refreshed branches experience with a new welcome, rename and delete controls, per-user branch names, recency-ordered branch dropdown, and a redesigned version history. See [Branching and publishing](/editor/branching-and-publishing).
  - **Wiki mode:** Turn on the new wiki mode in editor settings to autocommit changes to your deployment branch and skip the publish step. See [Editor settings](/editor/settings).
  - **Editor top bar and sidebar rework:** A new breadcrumbs row and consolidated page controls in the top bar, a **Breadcrumbs** preference, sticky sidebar section headers, and a dedicated bottom bar for settings and **Ask agent**. The agent panel now docks on the left.
  - **Mobile navigation rework:** The editor's mobile navigation has been redesigned for smaller screens.
  - **Page access requests:** Request edit access on a private or protected page directly from the editor, and review incoming requests from a new approval modal.
  - **Discardable autosaved changes:** Discard autosaved changes on a branch when you no longer want to keep them.
  - **Draft PRs by default:** Pull requests opened from the editor now default to draft. Requesting review on a draft PR no longer errors.
  - **Bring pending changes on branch create:** A new modal offers to move your pending changes when you create a branch.
  - **Clickable @mentions in comments:** Mentions in editor comments now link to the mentioned user.
  - **Profile pictures in comments:** Comment cards show the author's profile picture.
  - **Live preview:** Run multiple local live previews at the same time. The agent panel docks on the left, and the preview stays visible on onboarding follow-up turns.

  ## Automations and integrations

  - **Slack message trigger:** Run automations from messages in a connected Slack channel. See [Automations](/automations).
  - **Redesigned automations landing page:** A refreshed landing page with run count and creator surfaced on each automation card.

  ## Docs and delivery

  - **`noCopy` code blocks:** Add a `noCopy` meta flag on a fenced code block, or use a bare `no-copy` fence for unhighlighted blocks like ASCII diagrams, to hide the copy button. See [Code blocks](/create/code).
  - **Highlighted search snippets:** Search results can show a highlighted content snippet from the matching page. Turn on **Show content snippets** in your dashboard search settings.
  - **Scoped `llms.txt` indexes:** Grouped `llms.txt` indexes are served by URL segment so agents can pull a scoped slice of your docs.
  - **Assistant widget usage:** The assistant [widget](/assistant/widget) is now a paid add-on, and widget analytics record the page each conversation started on.
  - **Bulk member invites:** Invite teammates in bulk from the dashboard with a CSV upload and a review step before sending.
  - **Analytics in self-hosted deployments:** The dashboard analytics tab is now available on self-hosted deployments.
  - **Starter plan:** The Hobby plan has been renamed to Starter. Pricing and entitlements are unchanged.

  ## Bug fixes

  - **API playground:** OpenAPI 3.1 examples and enum values now appear in generated response samples. Markdown lists in the endpoint modal no longer render double bullets. Custom heading anchors indented inside JSX components resolve correctly.
  - **GraphQL:** Input type pages are generated and served on GraphQL API reference sites.
  - **Steps component:** Copy-pasting inside a `Step` no longer creates nested steps.
  - **Tabs:** Tabs no longer jump on click.
  - **Anchor icons:** Anchor link icons stay white on dark hover.
  - **Assistant widget loading:** The widget stays interactive while it's loading.
  - **Live preview freshness:** Navigation updates propagate to open live previews.
  - **Settings sidebar search:** Search in the settings sidebar has been restored.
  - **Editor share menu:** Editor share and page actions are consolidated into one menu.
  - **Discord bot:** The Discord assistant no longer crashes on expired interactions.
  - **Editor branch names:** Auto-generated branch names use a cleaner `Draft <date>` format.
...
<Update label="August 7, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Private pages, faster live previews, deeper analytics, and more" }}>
  ...
  - **Authenticated live previews:** Preview drafts of pages behind authentication from the editor [live preview](/editor/live-preview).
