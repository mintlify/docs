...
<Update label="July 17, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Markdown syntax for tree, Assistant widget refresh, and more" }}>

  ## New features

  - **Markdown syntax for the tree component:** Author file and folder structures inside a [`Tree`](/components/tree) using standard Markdown lists. Trailing `/` marks folders, and nested lists become nested folders — no `Tree.Folder` or `Tree.File` tags required.
  - **Refreshed Assistant widget:** The embeddable [Assistant widget](/assistant) now shares the same conversation UI and capabilities as the in-product assistant, including live session context.
  - **S3 key prefixes for audit log streaming:** Configure a custom key prefix when streaming [audit logs](/dashboard/audit-logs) to Amazon S3 to organize objects under a specific path.

  ## Improvements

  - **Incident banner promoted to the top of the dashboard:** Active incident notices now appear at the top of the dashboard and editor so they're visible from every page.
  - **Shorter build logs:** Deployment build logs are more concise and easier to scan.
  - **Faster editor navigation updates:** Gitsync-driven navigation updates in the [editor](/editor) are batched and applied incrementally, reducing lag on large sites.
  - **Editor agent picks up unlinked files:** The editor agent now loads files that aren't yet part of your navigation, so you can work on new pages before wiring them up.

  ## Bug fixes

  - Analytics delta tags no longer render when the value is 0 with no change.
  - CTA click events now emit consistently across all themes in [analytics](/optimize/analytics).
  - OAuth redirects now use the canonical display domain and stay on the browser-facing host after logout.
  - The `/login` page is no longer cached as a 404 when the upstream returns a 5xx error.
  - Fixed section reordering in the [Assistant widget](/assistant).
  - Restored the zoom exemption for the [Tile](/components/tiles) component so tiles render at the intended size.
  - File-change comment API calls in the editor now retry on transient failures.
