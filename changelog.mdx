...
<Update label="March 30, 2026" tags={["New releases", "Improvements"]} rss={{ title: "SAML group role mappings, Slack agent live progress, visual diffs minimap, and more" }}>

  ## SAML group role mappings

  Organizations using SAML SSO can now map identity provider groups to Mintlify [roles](/dashboard/roles). Assign admin, editor, or viewer roles based on IdP group membership so that new users provisioned through SSO automatically receive the correct access level. When roles are managed by your IdP, they are locked in the dashboard to prevent manual overrides.

  ## Slack agent live progress

  The [Slack agent](/agent/slack) now provides real-time feedback while working on your request. A live-updating message shows what the agent is doing, including files searched, read, and edited. Multi-step requests display a checklist so you can track each task. If the agent needs clarification, it presents options directly in the thread. When finished, it posts a summary with links to the pull request and the web editor.

  ## Visual diffs minimap

  The web editor now includes a minimap when reviewing visual diffs, making it easier to navigate large changesets and jump to specific changes.

  ## Improvements

  - [Feedback analytics](/optimize/feedback) dashboard redesigned with a cleaner layout, persistent table state, and simplified email collection settings.
  - SEO audit [workflow](/agent/workflows) template updated with stricter metadata checks, including minimum description length, boilerplate title detection, and duplicate description flagging.
  - Improved styling for API components in the web editor.
  - Editor collaboration reliability improved with better conflict resolution for multi-user editing sessions.

  ## Bug fixes

  - Fixed `broken-links` CLI not detecting custom heading IDs correctly.
  - Fixed live preview not loading in monorepo setups.
  - Fixed links inside reusable snippets not working in the editor.
  - Fixed minor tab styling issues in the editor.
  - Fixed invalid `head` elements being insertable in the editor.
