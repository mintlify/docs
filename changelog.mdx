...
<Update label="March 12, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Workflow CLI command, new workflow creation flow, Hungarian language support, and more" }}>

  ## New workflow creation flow

  Creating [workflows](/agent/workflows) just got easier. The redesigned workflow creation experience includes guided setup, templates for common automation tasks like detecting broken links and generating weekly changelogs, and clearer options for automerge settings.

  ## `mint workflow` CLI command

  Create workflow files directly from your terminal with the new `mint workflow` CLI command. The interactive command guides you through setting up workflow name, trigger type (cron or push), context repositories, automerge settings, and agent instructions.

  ## Slack notifications for workflows

  [Workflows](/agent/workflows) can now send Slack notifications when they complete. Get notified about pull requests created by the agent, whether they need review or were auto-merged.

  ## Slack bot improvements

  The [Slack bot](/ai/slack-bot) can now answer questions about any deployment in your organization, not just the primary one.

  ## Hungarian language support

  Added Hungarian (Magyar) as a new supported [localization](/guides/internationalization) language.

  ## Improvements

  - Page views now display on the [analytics](/optimize/analytics) page with charts and tables.
  - YouTube and Loom embeds can now be added directly from the editor slash command menu.
  - Spellcheck is now enabled by default in the web editor for code blocks and prompts.
  - [MCP](/ai/model-context-protocol) now works with partially authenticated docs.
  - Webhook-level servers in OpenAPI specs now properly override document-level servers.
  - Pages without titles in [llms.txt](/ai/llmstxt) now fall back to a generated title from the page slug.
  - Long descriptions in llms.txt are now truncated to prevent oversized files.

  ## Bug fixes

  - Fixed PDF export styling issues with code blocks, badges, frames, and other components.
  - Fixed footer appearing on custom pages for Mint, Maple, Willow, and Linden themes.
  - Fixed snippet handling for edge cases with bare identifiers and re-export specifiers.
  - Fixed `.md` route pages falling back to slug-based titles when metadata title is missing.
