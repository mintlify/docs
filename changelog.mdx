...
<Update label="July 2, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"AI preview editor, assistant contact form, automations webhook triggers, SCIM, and more" }}>

  ## New releases

  - **AI preview editor:** Kick off a new docs site from a prompt and iterate with an AI agent in an interactive preview. The agent can now ask clarifying questions, resume streams across reconnects, and be launched directly from the onboarding flow.
  - **Assistant contact form:** The [AI assistant](/assistant) can now surface an in-chat contact form to hand off to support or sales when a question can't be answered from your docs.
  - **Themed query buckets in assistant analytics:** [Assistant analytics](/assistant) groups related questions into themes and shows thumbs-up/thumbs-down sentiment per bucket so you can spot where the assistant is helping — or missing — at a glance.
  - **Automations API webhook trigger:** Trigger scheduled [automations](/automations) on demand via an external API webhook, in addition to their cron schedule.
  - **Automation per-run updates:** [Automation](/automations) runs show credits used per run and post per-run status updates as they progress.
  - **Session IP allowlist (Enterprise):** Restrict dashboard sessions to allowed IP ranges as part of your [SSO and session security policy](/dashboard/sso).
  ...
  - **Custom base paths for docs and `llms.txt`:** Serve docs, [`llms.txt`, and `llms-full.txt`](/ai/llmstxt) under a custom base path, with nav and in-content links rewritten to include the prefix.
  - **`searchable` frontmatter field:** Set `searchable: false` on a [page](/organize/pages) to keep it out of internal search results.

  ## Improvements

  - **Admin MCP consent UI:** Redesigned [admin MCP](/ai/model-context-protocol) consent screen with a multi-select deployment picker, per-deployment scopes, and a new `list_deployments` discovery tool. Consent, auth paths, and code-mode SDK client are now scoped to the selected org and deployment.
  - **Hosted MCP admin connect page:** New admin connect page and hosted MCP auth settings in the dashboard under Settings.
  - **`@mintlify` PR bot on fork branches:** The `@mintlify` GitHub bot now pushes comment edits directly to fork branches when the app is installed on the fork, and supports PRs opened against docs repos.
  ...
  - **Mintlify-hosted repo automations:** [Automations](/automations) on Mintlify-hosted repos support auto-apply updates and hide GitHub-specific links that don't apply.
  ...
  - **SSO copy refresh:** The [SSO](/dashboard/sso) settings page has cleaner sections and updated GitLab configuration instructions.
  ...
  - Assistant bucket occurrence counts now respect the selected date range.
  - Removed invalid `charset`, `canonical`, and `og:*` meta tags from rendered pages.
  - Static asset response headers are lowercased to avoid duplicated `Content-Type`.
  - Usage charts hide legends and bars for products with zero usage.
