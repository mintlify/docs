...
<Update label="April 27, 2026" tags={["New releases"]} rss={{ title: "Global Slack notifications for workflows" }}>

  ## Global Slack notifications for workflows

  Configure Slack channels once and receive notifications for every [workflow](/agent/workflows) in your organization. Open the **Notifications** modal from the Workflows page to connect Slack and pick the channels that should receive updates when workflows open or merge pull requests. The Workflows page shows a dismissable banner when no channels are configured so it's easy to see whether notifications are set up. Per-workflow Slack notifications continue to work alongside the global setting.
