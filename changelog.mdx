...
<Update label="April 24, 2026" tags={["New releases", "Improvements"]} rss={{ title: "AccordionGroup reordering, broken-links redirect checks, branch diff list, offline editing, and more" }}>

  ## AccordionGroup reordering

  Accordions inside an [`AccordionGroup`](/components/accordions) can now be reordered directly in the web editor. Drag and drop to rearrange items without editing the underlying MDX.
  ...
  A public API for generating preview deployments is now available. Use it to trigger previews from your own tooling—useful when your Git workflow doesn't map cleanly to standard branch-based preview triggers.

  ## Git onboarding and settings redesign

  The Git settings page has been redesigned with a cleaner layout. Mintlify-hosted customers migrating to their own GitHub or GitLab repository now get a step-by-step flow that walks through the full migration process.

  ## Branch diff in the editor

  The web editor now shows the full git diff for the current branch—including changes already pushed, not just local edits. Review everything modified in one place before publishing or opening a pull request.

  ## Offline editing

  The web editor now persists your changes locally using IndexedDB. Edits made while offline are preserved and sync automatically when your connection returns.

  ## Editor emoji trigger

  Type `:` in the web editor to open an emoji picker and insert emojis inline without leaving the keyboard.

  ## Editable Color component

  The `Color` component is now editable directly in the web editor, so you can update color swatches visually.

  ## Improvements

  - Added a reconnection indicator in the editor when the collaboration server disconnects, with automatic reconnection when the page regains focus.
  - SVG files can now be uploaded from the editor configuration view.
  ...
  - Sites without a configured URL are now scored against their subdomain so onboarding users get feedback before their custom domain is set up.
  - AI-generated pull request titles and descriptions are back for agent-opened PRs.
  - Hobby plan organizations are now limited to 100 [workflow](/agent/workflows) runs per month.
  ...
  - Fixed the create PR button not re-enabling after saving changes on a branch.
  - Fixed frontmatter occasionally falling out of sync between the editor and the source file.
  - Fixed source mode occasionally showing stale content after switching from visual mode.
  - Fixed the publish button not activating when a file was changed to be empty.
  - Fixed creating files on git setups with limited access. Publish now surfaces a clear error instead of silently failing.
  - Fixed git changes not syncing correctly when `contentDirectory` is set, including monorepo setups with duplicated `contentDirectory` on publish.
  - Fixed pages not updating when git changes are synced to an already-open editor.
  - Fixed new pages not persisting into the decorated navigation after creation.
  - Fixed discarding navigation changes incorrectly resetting open documents.
  - Fixed navigating back to a branch after creation when the branch isn't persisted in git.
  - Fixed symlink handling for skill files stored outside the content directory.
  - Fixed pending actions not staying consistent after merging with git.
  - Fixed editor collaboration connecting to the wrong room after renaming files.
  - Fixed several editor polish issues: tab order and focus in editor blocks, caret position after inserting `/tabs`, keyboard shortcuts for global search and inline links, TOC trigger area, Accordion open-on-create behavior, Accordion enter key handling, ordered list merging, column placeholders, Step placeholder titles in Firefox, Steps margins, icon and API playground popover rendering, and live caret UX.
  - Fixed undo not scrolling the viewport back to the cursor position.
  - Fixed related pages resolving the wrong subdomain for custom domains.
  - Fixed the Luma integration table of contents position.
  - Fixed add-ons redirect logic in the dashboard.
...
<Update label="April 10, 2026" tags={["New releases", "Improvements"]} rss={{ title: "CLI analytics, directory listings, multi-skill support, Slack bot improvements, and more" }}>
  ...
  - [Workflows](/agent/workflows) now surface in the dashboard inbox for easier tracking. The workflow list also displays PR names for quicker identification.
