...
<Update label="July 17, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Markdown FileTree syntax and Assistant widget" }}>

  ## New features

  - **Markdown FileTree syntax:** Author file and folder trees using a plain markdown list inside a `<Tree>` (or `<FileTree>`) tag. Trailing slashes and nested lists mark folders. See [Tree](/components/tree).
  - **Assistant widget:** A new browser loader embeds the [assistant](/assistant) directly in your app or docs site, and a companion dashboard page lets you configure and manage widget deployments. See the [assistant embed tutorial](/guides/assistant-embed) to get started.

  ## Improvements

  - **Clearer workflow PR merge errors:** Merging a workflow PR from the dashboard now returns explicit errors for merge conflicts and branch protection so you can resolve them without guessing.
  - **Resilient PR comments:** File-change PR comments now retry through transient GitHub outages (429/502/503/504) and reconcile duplicates so short blips don't block your PRs.

  ## Bug fixes

  - Fixed OAuth redirect URLs to use your canonical display domain so authenticated docs sign-in returns you to the right host.
  - `/login` no longer caches a 404 when the upstream returns a 5xx, so login recovers as soon as the upstream does.
