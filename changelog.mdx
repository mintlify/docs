...
<Update label="February 27, 2026" tags={["New releases", "Improvements"]} rss={{ title: "GitHub Enterprise support, editor improvements, and CLI enhancements" }}>

  ## GitHub Enterprise support

  Mintlify now supports GitHub Enterprise Server. Connect your self-hosted GitHub Enterprise instance to deploy documentation from private repositories.

  ## Table of contents in editor

  The web editor now includes a floating table of contents that highlights your current position as you scroll. Navigate to any heading with a single click.

  ## Editor content sync across tabs

  Edits in the web editor now sync across browser tabs in real-time. Changes made in one tab instantly appear in all other open tabs, eliminating conflicts when working in multiple windows.

  ## Comment filtering

  Filter comments in the editor by page or view all comments across your documentation. The comments pane now includes a dropdown to switch between viewing comments on the current page or all pages.

  ## CLI broken link checker improvements

  The `mint broken-links` command now checks snippets and external links in addition to internal links. Use `--check-snippets` to verify snippet imports and `--check-external` to validate external URLs.

  ## Improvements

  - Webhook request body examples now support tab switching when multiple examples are defined in your OpenAPI spec.
  - Navigation group `root` pages are now properly displayed as read-only in the editor.
  - Sidebar titles now wrap correctly instead of being cut off.
  - Search analytics now include the query text for better insights.
  - Improved performance when fetching snippets and API specs in the editor.

  ## Bug fixes

  - Fixed Willow theme styling inconsistencies.
  - Fixed assistant panel positioning under global banners.
  - Fixed broken anchor link detection for custom components and HTML elements with `id` attributes.
  - Fixed editor toolbar placement across different browsers.
  - Fixed Almond theme layout shift with the assistant pane.
