...
<Update label="March 18, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Linear integration, contextual menu expansion, agent PR comments, and more" }}>

  ## Linear integration

  The [agent](/agent/index) now integrates with Linear. When a Linear issue is created or updated, the agent can automatically generate documentation pull requests based on the issue context. Connect Linear from the [workflows dashboard](https://dashboard.mintlify.com/products/workflows) to get started.

  ## Contextual menu expansion

  The [contextual menu](/ai/contextual-menu) now includes options to open documentation in [Devin](https://devin.ai), [Windsurf](https://windsurf.com), and [Google AI Studio](https://aistudio.google.com). Users can also connect their hosted MCP server directly to Devin from the menu.

  ## Agent comments on PRs

  The [agent](/agent/index) now adds summary comments to pull requests it creates, giving reviewers a quick overview of what changed and why.

  ## Self-hosted GitLab support

  Connect self-hosted GitLab instances to Mintlify directly from the dashboard. Previously, only GitLab.com was supported. See [GitLab](/deploy/gitlab) for setup instructions.
  ...
  ## Agent available on trial plans

  The [agent](/agent/index) is now accessible during your trial period so you can evaluate it before committing to a plan.

  ## Improvements

  - Faster deployments when only `docs.json` configuration changes are detected
  - Improved editor typing performance with reduced latency
  - Viewer role users can now access the [editor](/editor/index) in read-only mode
  - Authenticated docs now support [MCP](/ai/model-context-protocol) server connections
  - Dashboard analytics date picker is now more responsive

  ## Bug fixes

  - Fixed card layout shift on initial page load
  - Fixed Luma theme banner layout shift
  - Fixed lists not rendering correctly in API playground markdown descriptions
  - Fixed base path not being applied to internal links
  - Fixed code block tooltip incorrectly appearing for prompt and Mermaid components
  - Fixed hidden pages losing their child pages in the navigation sidebar
  - Fixed blockquote placeholder rendering in the editor
