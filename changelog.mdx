...
<Update label="May 7, 2026" tags={["New releases"]} rss={{ title: "Self-hosted GitLab OAuth for workflows" }}>

  ## Self-hosted GitLab OAuth

  You can now connect a self-hosted GitLab instance to Mintlify through OAuth. Register an OAuth application on your GitLab instance, save the client ID and secret in [Git Settings](https://dashboard.mintlify.com/settings/deployment/git-settings), then authorize. Connected projects can be used as documentation sources, trigger repositories, or context repositories in [workflows](/workflows), with the same group and project browsing experience available for `gitlab.com`. See [Self-hosted GitLab OAuth](/deploy/gitlab#self-hosted-gitlab-oauth) for setup instructions. Available on Enterprise plans.
