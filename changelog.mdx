...
<Update label="May 8, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Slack agent permission requests, expanded MCP access, AsyncAPI nested groups, and more" }}>

  ## Slack agent permission requests

  When the [Slack agent](/agent/slack) runs a third-party action that writes data—such as sending an email or creating a ticket—it now asks you to approve or deny the action before proceeding. The permission request is only visible to the person who triggered it, so the rest of the thread isn't interrupted.
  ...
  - TLS certificate provisioning for [custom domains](/customize/custom-domain#automatic-tls-provisioning) is now handled directly by Mintlify instead of routing through Vercel.
  ...
  - The [assistant](/assistant) billing experience has been updated to use credit-based packages across all plans, with consistent terminology in the dashboard and usage controls.
