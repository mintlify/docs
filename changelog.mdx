...
<Update label="July 17, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"FileTree list syntax, refreshed Assistant widget, and more" }}>

  ## New features

  - **FileTree list syntax:** Author [file trees](/components/tree) with a familiar Markdown list inside a `<FileTree>` block. Trailing slashes and nested lists become folders automatically, so you can skip the explicit `Tree.Folder` and `Tree.File` tags.

  ## Improvements

  - **Refreshed Assistant widget:** The embeddable [Assistant widget](/guides/assistant-embed) now matches the in-product Assistant conversation UI and session state stays in sync with the live context.
  - **Shorter build logs:** Deployment build logs are more concise so you can find the important information faster.
  - **Incident banner placement:** The status incident banner now appears at the top of the [dashboard](https://app.mintlify.com) for better visibility.

  ## Bug fixes

  - Fixed OAuth redirect URLs so authenticated docs sign-in uses the canonical display domain and keeps you on the browser-facing host after logout.
  - Fixed a caching issue that could serve a stale `/login` 404 when the upstream returned a 5xx.
  - Analytics no longer shows delta tags when a metric is zero with no change, and `cta_click` events are emitted consistently across all themes.
  - Assistant widget section reordering now saves correctly.
  - Slack app installs no longer request unapproved scopes.
