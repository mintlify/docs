...
<Update label="October 22" tags={["Improvements"]} rss={{ title: "Weekly Updates", description: "TinyBase persistence for editor state management" }}>
  ## Editor state management improvements

  - Added TinyBase database with IndexedDB persistence for editor state
  - Implemented persistent storage for editor state across browser sessions
  - Added state hydration on editor initialization to restore previous sessions
  - Integrated XState machine with TinyBase for robust state management
  - Added React contexts for global access to editor database and state machine
  - Improved editor initialization with mock data support for testing
  - Enhanced file actor management with persistent storage
  - Added database readiness checks before state operations
</Update>
