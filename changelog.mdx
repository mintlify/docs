...
<Update label="March 27, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Offline export, inline AI editing, visual diffs, Notion integration, and more" }}>

  ## Offline export

  Package your entire documentation site into a self-contained zip archive with the new [`mint export`](/deploy/export) CLI command. Recipients can view your docs in their browser without an internet connection or a Mintlify account.

  ## Inline AI editing

  Use [inline AI editing](/editor/inline-ai) to generate, rewrite, and restructure content directly in the visual editor. Select text and choose from preset actions like **Improve writing** or **Auto-structure**, or type a custom instruction. Changes stream in as a highlighted diff so you can review before applying.

  ## Visual diffs in the editor

  The [web editor](/editor) now shows a rich visual diff when reviewing changes before publishing. Added, deleted, and modified blocks are highlighted with colored gutters. Toggle between visual diff and markdown diff modes from the editor toolbar.
  ...
  Preview deployments triggered from GitHub pull requests now post a status comment on the PR with the deployment status, a link to the preview URL, and a link to the dashboard. The comment updates in-place on subsequent pushes.

  ## Composable `docs.json`

  Split large configurations into smaller files using [`$ref` references](/organize/settings#split-configuration-with-%24ref). Each reference points to a separate JSON file that gets resolved at build time, making complex navigation structures easier to manage.

  ## Custom heading IDs for h5 and h6

  [Custom heading IDs](/create/text#custom-heading-ids) now work with `h5` and `h6` headings, and search results correctly link to sections that use custom IDs.

  ## Improvements

  - Workflows dashboard redesigned with clearer run history, action dropdowns, visual schedule pickers, and improved empty states.
  ...
  - Feedback analytics now support filtering by feedback type (contextual vs. thumbs).
  - Dashboard UI improvements for analytics and assistant pages.
  - Workflow PR status now reconciles automatically with GitHub.
  - Agent now respects Git push rules in commit messages.
  ...
  - Fixed dashboard global search not respecting the selected branch.
