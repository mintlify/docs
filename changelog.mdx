...
<Update label="May 1, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Workflows v2, Admin MCP, onboarding redesign, search improvements, and more" }}>

  ## Workflows v2

  Workflows have been rebuilt with a new dashboard experience at **Settings → Workflows**. The new version replaces the previous workflows page and adds:

  - **Custom workflows** in addition to built-in update and SEO workflows.
  - **Runs tab** with status filtering (running, succeeded, failed), schema-only filter, sort by last run, and direct links to source pull requests.
  - **Settings modal** with a searchable repository dropdown, scroll-friendly layout, and confirmation before closing an open workflow PR.
  - **GitLab support** alongside GitHub, plus support for **public GitHub repositories** as triggers and as additional context repos.
  - **Global Slack notifications** for workflow runs across a deployment.
  - Daily run limit raised to **500 runs per 24 hours**.

  Workflows can now be triggered against the deployment's own Git source, and any trigger kind (push, PR, manual, schedule) can drive a changelog workflow.

  ## Admin MCP

  Mintlify's admin MCP server is now served at a single `/mcp` URL on each deployment, with a redesigned dashboard that includes a deployment picker. New tools let agents make structured edits to your docs:

  - `checkout` (formerly `create_branch`), `save` (formerly `submit_pr`), `get_session_state`, and `discard_session` for managing edit sessions.
  - `move_node`, `update_node`, `delete_node`, and `update_config` for modifying navigation and configuration.
  - `diff` and `list_branches` for inspecting work in progress.
  - Streaming responses, branch divergence visibility in search, and OAuth-based connection approval.

  ## Configurable OpenAPI in Markdown export

  You can now control whether OpenAPI-generated reference pages are included in your [Markdown export](/ai/llmstxt) and `llms-full.txt`. Useful when you'd rather have agents pull schema details from the OpenAPI spec directly instead of from rendered reference pages.

  ## Section-level search boost

  Search indexing now supports section-level boosts in addition to page-level boosts, letting you promote specific headings within a page so they rank higher for relevant queries.

  ## Onboarding redesign

  The onboarding flow has a new look and a few new steps:

  - **Template selection** step lets new users start from a curated set of documentation templates.
  - **Role selection** during signup tailors the rest of onboarding to your role.
  - **Manual setup escape hatch** for users who'd rather configure everything themselves.
  - **Custom domain step** automatically detects an existing CNAME and skips the manual instructions when your DNS is already pointed at Mintlify.
  - **Consolidated Git provider settings** page for connecting GitHub or GitLab.

  ## Search improvements

  - Matched query terms are now highlighted directly in search result headers.
  - Search now indexes content inside `<img>` alt text and other inline image markup.
  - Tag-based search filters are case-insensitive.
  - Snippets no longer duplicate the page title at the start of the result.

  ## Improvements

  - The `relatedPages` frontmatter field has been renamed to `related`. The old name continues to work for backward compatibility.
  - Pricing and member limit copy in the dashboard has been clarified.
  ...
  - The web editor adds a copy-branch button on the branch view, Vimeo embed support, and a new editable tree component for navigation editing.
  ...
  - Fixed an XSS vector in the Assistant.
  ...
  - Fixed a 405 response on the revalidate route when reached via GET through Cloudflare.
  - Fixed self-serve deployment errors in the dashboard.
  - Fixed a workflow card toggle that could blank the dashboard page.
  - Fixed Stripe webhooks running for organizations migrated to the new billing system.
...
<Update label="March 13, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Workflow templates and CLI command, Agent API v2, and more" }}>
  ...
  Create [workflows](/workflows) faster with pre-built templates in the [dashboard](https://dashboard.mintlify.com/products/workflows). Choose from templates for changelog generation, API docs sync, feature documentation drafts, broken link detection, SEO audits, and more. Templates include optimized prompts and trigger configurations to get you started quickly.
