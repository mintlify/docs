...
<Update label="August 28, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Bring your own model, the MDX component, custom component styles, and more" }}>

  ## AI models and credits

  - **Bring your own model:** Supply your own LLM provider API key and choose the model behind the assistant, agent, and automations. Usage is metered at half the credit rate, and your provider bills you for inference directly. OpenRouter is the recommended provider. Anthropic and OpenAI-compatible endpoints are also supported, with narrower feature coverage. Available on Enterprise plans as a contract add-on.
  - **Referral program:** Refer a team to Mintlify from the **Referrals** page in dashboard settings. When a referred team converts to a paid plan, both organizations receive 50,000 credits. You can earn the reward for up to three referrals. Available on Pro plans.

  ## Authoring

  - **`<MDX>` component:** Wrap content in `<MDX>` to render Markdown formatting inside JSX expressions, such as conditional content. Headings inside an `<MDX>` block don't appear in the page's table of contents yet.
  - **Custom styles on components:** Built-in components now accept a `className` prop, and dynamic Tailwind utilities work inside it. Classes such as `bg-red-500` and bracket syntax such as `w-[calc(100%-2rem)]` previously failed silently on components.
