...
<Update label="February 20, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Automated workflows, editor comments, Willow theme, and Mermaid ELK layout" }}>

  ## Automated workflows

  Configure automated documentation workflows with scheduled triggers. Define workflows in `.mintlify/workflows/` using markdown files with YAML frontmatter. Workflows support:
  - Cron-based schedules for recurring documentation tasks
  - Merge-based triggers when PRs are merged to specified branches
  - Automerge option for trusted workflow outputs

  ## Editor comments

  Collaborate on documentation with in-editor comments. New features include:
  - @-mention teammates in comments to notify them
  - Comment counts displayed in the branch selector
  - Deep links to specific comments
  - Comments sync to PR descriptions when publishing

  ## Willow theme updates

  The [Willow theme](/customize/themes) has been refreshed with a redesigned sidebar navigation and topbar.

  ## Mermaid ELK layout

  [Mermaid diagrams](/components/mermaid-diagrams) now support the ELK layout engine for improved automatic diagram layouts.

  ## Audio responses in API playground

  The [API playground](/api-playground/overview) now displays audio file responses, expanding support for multimedia API responses.

  ## Editor improvements

  - Drag table edges to add rows and columns
  - Live Mermaid diagram preview in the editor
  - Reorder tabs via drag and drop
  - Toggle sidebar visibility
  - Search within the editor
  - Notifications inbox for collaboration updates

  ## Variables in configuration

  Use [variables](/organize/settings#variables) in your `docs.json` with a find-and-replace utility for consistent values across your configuration.

  ## Assistant behavior customization

  Configure the assistant with an `Assistant.md` file in `.mintlify/Assistant.md` to customize its persona, tone, and product-specific context. See [Customize assistant behavior](/ai/assistant#customize-assistant-behavior) for details.

  ## Bug fixes

  - Fixed Sequoia theme layout shift issues
  - Fixed accordion ID duplication for components with duplicate titles
  - Fixed anchor validation for ParamField, ResponseField, and Accordion components
  - Fixed Windows path separator handling
  - Improved Mermaid rendering inside CodeGroup components
