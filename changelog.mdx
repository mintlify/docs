...
<Update label="September 4, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Bring your own model, notification preferences, collaborative source mode, and more" }}>

  ## Bring your own model

  - Supply your own LLM provider API key and choose the model that powers the [assistant](/assistant), [agent](/agent), and [automations](/automations/index) on a deployment. Usage on your own key costs half the Mintlify credits. OpenRouter covers every AI product; Anthropic and OpenAI-compatible providers cover a subset. Available on Enterprise plans. See [Bring your own model](/ai/bring-your-own-model).

  ## Notifications

  - **Notification preferences:** Choose which emails you receive for each event, including comment replies and mentions, thread resolutions, suggestions submitted, accepted, or rejected, automation runs, and product feedback.
  ...
  - **Collaborative source mode:** Source mode now supports real-time collaboration. Teammates' cursors are labeled in the source pane, and frontmatter edits sync live across visual and source mode. See [Real-time editing](/editor/collaborate#real-time-editing).
  ...
  - **`mint test` for everyone:** Generate tests for the code blocks in your docs from the CLI, now available to all users. Runs are saved so you can revisit past reports. See [`mint test`](/cli/commands#mint-test).
...
<Update label="July 16, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Page improvements, new analytics dashboard, and more" }}>
  ...
  - Fixed OAuth redirect URIs on preview deployments so authenticated docs sign-in works on `.mintlifysite.com` preview hosts.
