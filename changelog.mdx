...
<Update label="March 14, 2026" tags={["New releases"]} rss={{ title: "Workflows run history dashboard" }}>

  ## Workflows run history

  The [Workflows](https://dashboard.mintlify.com/products/workflows) page in the dashboard now defaults to a **Run history** tab that shows a consolidated view of runs across all your workflows. Filter by workflow or toggle between open pull requests and all historical runs.

  Each run row displays richer status information including "Review needed", "PR merged", and "PR closed" states, along with action buttons to view the pull request, open changes in the editor, or close the pull request directly from the dashboard.

  The previous workflow card list is available under the **Workflows** tab. See [Monitor workflow runs](/agent/workflows#monitor-workflow-runs) for details.
