...
<Update label="March 26, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Inline AI editing, Notion integration, mint export, and more" }}>

  ## Inline AI editing

  Use [inline AI editing](/editor/inline-ai) in the web editor to generate, improve, or restructure content without leaving the visual editor. AI changes appear as a highlighted diff so you can review them before applying.

  ## Notion integration

  Connect Notion to your Mintlify dashboard to sync content between your Notion workspace and your documentation. Configure the integration directly from the dashboard.

  ## `mint export` CLI command

  Use [`mint export`](/deploy/export) to package your entire documentation site into a self-contained zip archive for offline viewing. Recipients can unzip and view docs in their browser without an internet connection or a Mintlify account.

  ## Heap analytics integration

  [Heap](/integrations/analytics/heap) is now available as an analytics integration. Add your Heap app ID to `docs.json` to start tracking user behavior on your documentation site.

  ## Preview deployment PR comments

  Preview deployments now post a comment on your pull request with a link to the preview, similar to Vercel-style deployment comments. This makes it easier for reviewers to find and check preview deployments directly from the PR.

  ## API playground credentials option

  A new [`credentials`](/api-playground/overview) option is available in the API playground configuration. When enabled with proxy disabled, the playground sends requests with browser-managed credentials like cookies and session tokens included.

  ## Improvements

  - Workflow creation UX redesigned with a cleaner layout and step-by-step flow.
  - Workflows table now includes an action dropdown and improved empty state.
  - Editor navigation now syncs with the server-side navigation API for more accurate sidebar rendering.
  - Editor-only sync mode for faster file synchronization.
  - Feedback statuses updated in the dashboard analytics table.
  - OpenAPI file validation added when uploading specs through the editor.
  - H5 and H6 heading support added to the MDX parser.

  ## Bug fixes

  - Fixed custom heading IDs causing parsing errors.
  - Fixed nested code block parsing.
  - Fixed Luma theme primary button color not applying from `docs.json`.
  - Fixed navigation item wrapping with badges.
  - Fixed text-selection tooltip positioning.
  - Fixed padding for tables inside tabs.
  - Fixed sidebar shadow pointer style.
  - Fixed Frame hint icon vertical alignment with text.
  - Fixed links in static HTML exports.
  - Fixed XSS vulnerability in live preview.
  - Fixed redirect when hitting the `/explore` route directly.
  - Fixed branch-aware global search in the dashboard.
  - Fixed default division selection and caching.
