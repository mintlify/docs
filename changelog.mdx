...
<Update label="April 24, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Preview Deployment API, Git onboarding redesign, web editor improvements, and more" }}>
  ...
  Changes are now persisted locally, so edits made while offline are preserved and sync automatically when your connection returns.

  - **Emoji picker**: Type `:` to open an emoji picker and insert emojis inline.
  - **AccordionGroup reordering**: Drag and drop to rearrange accordions inside an [`AccordionGroup`](/components/accordions) without editing MDX.
  - **Editable Color component**: Update color swatches directly in the editor without editing MDX.
