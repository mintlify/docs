...
<Update label="May 29, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Editor agent upgrades, new workflows, API playground callbacks, and more" }}>
  ## Editor
  ...
  - **Frontmatter panel:** A new file frontmatter panel in the file explorer view lets you view and edit page metadata without leaving the tree.
  ...
  - **Searches Mintlify docs:** The editor agent can now search the Mintlify documentation to ground its answers and edits.
  - **Edit and delete non-docs files:** The agent can manage non-docs files in your repository in addition to pages.
  - **File attachments:** Attach files directly to the agent — they're sent through the API rather than inlined in the prompt.
  - **Interrupt mid-stream:** Stop the agent while it's streaming a response if you want to redirect it.

  ## Workflows

  - **Improve docs from feedback** is now a dedicated workflow that reviews recent [page feedback](https://www.mintlify.com/docs/optimize/feedback) and opens targeted documentation updates from what users report. See [Workflows](https://www.mintlify.com/docs/workflows/enable#improve-docs-from-feedback).
  - The **Sync content with code** workflow has been renamed to **Self-updating site** to better reflect what it does.
  - Workflow titles and copy across the dashboard have been refreshed for clarity.

  ## API playground

  - **Callbacks on API reference pages:** OpenAPI `callbacks` now render on API reference pages so readers can see the requests your API sends back. See [OpenAPI setup](https://www.mintlify.com/docs/api-playground/openapi-setup).
  - **Download API spec:** A new **Download API spec** option is available from the page context menu on API reference pages.

  ## Dashboard

  - **Switch organizations without signing out:** You can now move between organizations in a single click. No more re-authenticating to switch contexts.

  ## AI and MCP

  - **MCP server card:** Mintlify now serves an MCP server card at the well-known paths MCP scanners probe, including server info and customer name, making it easier for clients to discover your [MCP server](https://www.mintlify.com/docs/ai/model-context-protocol).
  ...
  - **Snippet variables in code blocks:** [Reusable snippets](https://www.mintlify.com/docs/create/reusable-snippets) now support variables inside code blocks, so you can parameterize examples consistently.
