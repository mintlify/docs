...
<Update label="February 6, 2026" tags={["New releases", "Improvements"]} rss={{ title: "Self-serve SSO, Viewer role, Analytics toggle, and slash command redesign" }}>

  ## Self-serve SSO configuration

  Enterprise customers can now configure SAML SSO (Okta or Microsoft Entra) directly from the dashboard, including JIT provisioning, provider switching, and connection removal.

  ## Viewer role

  New permission level with read-only access to documentation and settings. Viewers cannot edit docs, invite members, or modify assistant/SSO configuration.

  ## Human/AI traffic toggle

  Analytics page now shows separate Human and AI traffic tabs (removed "All Traffic" view).

  ## Slash command menu redesign

  Editor slash menu now features type-to-search, improved keyboard navigation, better positioning, and a "No results" state.

  ## Bug fixes

  - **Fixed table cell overflow in dashboard tables** – Long text and unbroken strings in Activity and Status columns now wrap properly instead of overflowing, making dashboard tables readable at narrow widths.
  - **Fixed file renaming not working in visual editor** – Renaming files via the config sheet now properly saves when you have the visual editor open; also fixed an issue where editing one page's config would mistakenly update a different page.
  - **Fixed editor error clearing** – MDX parsing errors (like unclosed `<Card>` tags) now update dynamically as you fix them instead of persisting incorrectly across reloads.
  - **Fixed anchor resolution for editor comments** – Comments now track their location as content changes using fuzzy matching, preventing them from becoming orphaned or attached to the wrong text.
  - **Fixed drag handle alignment** – Drag-and-drop handles in the editor now align correctly with different content types (headings, paragraphs, etc.) with consistent sizing and better hit areas.
