...
<Update label="July 3, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Call-in from the assistant, autocommit for unprotected deploy branches, and more" }}>
  ...
  - **AsyncAPI honors `api.params.expanded`:** Setting [`api.params.expanded: "all"`](/settings) in `docs.json` now expands parameters and message bindings on [AsyncAPI](/api-playground/asyncapi-setup) pages too, matching the OpenAPI behavior.
