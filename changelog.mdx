...
<Update label="May 15, 2026" tags={["New releases","Improvements"]} rss={{ title:"Editor AI instructions and publishing controls" }}>
  ...
  - **Usage breakdown by category**: All customers, including Hobby plans, can now break down credit usage by category in the dashboard to see exactly where credits are being spent.
  - **Out-of-credits banner**: A banner now appears on the Workflows runs tab when your deployment's remaining credit balance hits zero, with a direct link to purchase a credit package. The banner is suppressed if overages are enabled on your plan.
  ...
  - **Dashboard sidebar reorganized:** The sidebar now has three sections — top (Home, Editor, Analytics), **Automate** (Workflows, Agent, Assistant), and **Admin** (MCP, Settings). The same grouping is mirrored in the mobile menu.
  - **Search language fallback**: Search now falls back to a language path prefix when an exact match isn't found, improving results on multilingual sites.
