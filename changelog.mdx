...
<Update label="July 10, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Search filters, custom subpath hosting, editor comments and suggestions, and more" }}>

  ## New releases

  - **Search filters:** Add [filters to your docs search](/optimize/search) so readers can narrow results by product, category, or other facets you configure in the dashboard.
  - **Custom subpath hosting:** Host your docs at a custom base path (for example, `example.com/docs`) directly from the dashboard, with an inline base path input in the deployment hosting settings. See [subpath hosting](/deploy/docs-subpath).
  - **Comments and suggestions in the editor:** Leave threaded comments on any text in the [editor](/editor), reply, resolve, and @mention teammates with email notifications. Suggestion mode lets reviewers propose edits for authors to accept or reject. Available to all customers.
  - **Require SSO with break-glass access:** Enforce SSO sign-in for your entire organization, with designated break-glass members who can still sign in directly if your identity provider is unavailable. See [SSO](/dashboard/sso). Available on the Enterprise plan.
  - **Network access IP allowlists:** Restrict dashboard access to trusted IP ranges with the new [network access](/dashboard/network-access) settings, and apply IP allowlists to external API keys for granular control. Available on the Enterprise plan.
  - **Dashboard session lifetimes and revocation:** Configure how long dashboard sessions stay valid and revoke individual sessions from the members page, complementing the existing [SSO controls](/dashboard/sso). Available on the Enterprise plan.
  - **API key expiration and scopes:** Create external API keys with expiration dates and read or read/write scopes for tighter credential hygiene.
  - **Update from support feedback automation:** The new [Update from support feedback automation](/automations) reviews resolved support conversations from Plain or Intercom and drafts documentation updates, so recurring questions turn into docs fixes. Available on the Enterprise plan.
  - **Granular PDF exports:** [Export PDFs](/optimize/pdf-exports) for specific pages or locales instead of the entire site. Available on the Enterprise plan.
  - **Clarifying questions in the assistant:** The [assistant](/assistant) can now ask a targeted clarifying question when a request is ambiguous, presented as selectable option cards in chat.
  - **`/call` page:** Sites configured with a phone contact now expose a `/call` route that forwards visitors to the dialer with the correct number, dial pause formatting, and your site favicon.
  - **Consolidated Identity & access page:** SSO, SCIM, and network access settings now live together on a single [Identity & access](/dashboard/sso) page in the dashboard.
  - **Schema.org JSON-LD on docs pages:** Docs pages now include auto-generated schema.org structured data to improve search-engine understanding of your content.
  - **System theme:** Docs sites can now follow each reader's operating system light or dark mode preference with the new `system` theme option.

  ## Improvements

  - **Faster editor:** Opening the editor is dramatically faster: the navigation tree is cached in the browser and pages render from a local copy, and navigation updates now sync instantly across collaborators. Large docs sites see an order-of-magnitude improvement.
  - **Collapsible headings in the editor:** Collapse and expand sections under headings while editing to navigate long pages more easily.
  - **Assistant contact form opt-out:** Turn off the in-chat contact form from [assistant deflection settings](/assistant/configure) if you don't want the assistant to hand off to support.
  ...
  - **Search boosting:** Per-page `boost` values set in frontmatter are now applied to search rankings.
  ...
  - **Assistant retry:** Retrying an assistant response now regenerates the answer instead of appending a duplicate turn.
  - **Custom domain UX copy:** Clearer copy and inline descriptions on custom domain and hosting-mode settings in the dashboard.
  ...
  - The [assistant](/assistant) now surfaces stream errors instead of failing silently, and no longer shows stale content from a previous session after navigation.
  - The assistant answers from the context it has gathered instead of returning an empty message when it reaches its tool call limit.
  - Toggling an automation on or off no longer fails with a "workflow not found" error.
  - Three-way merges in the editor no longer duplicate content inside text blocks.
  - Visual-mode diffs no longer hang on large pages in the editor.
  - End-user auth logout now performs a full-page navigation so state is cleared reliably.
  - MCP discovery correctly reports OAuth2 authentication on auth-enabled deployments.
  - The billing page renders the plan for the current deployment instead of the org default.
  - PDF export links are normalized, and the table of contents no longer renders empty for certain deployments.
  - Search no longer promotes down-boosted pages via title matches.
  - Default search filters are cleared correctly when reset.
  ...
  - Removed the first-credits discount UI from the credits purchase CTA when it no longer applies.
