...
<Update label="July 3, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Call-in from the assistant, autocommit for unprotected deploy branches, and more" }}>

  ## New

  - **Call-in tool for the assistant:** The [assistant](/assistant) can now offer a phone call for questions it can't answer from your content. It surfaces your site's call-in number and per-site extension as a tap-to-call link — extensions are pre-dialed on mobile.
  - **Autocommit for unprotected deploy branches:** When your deploy branch isn't protected, the [editor](/editor) now autocommits saves directly to it under the draft-and-publish flow, so quick fixes ship without a manual publish step. Protected branches continue to require a draft and explicit publish.
  - **Drafts-only deployment picker:** In the draft-and-publish flow on `main`, the manual **Publish** CTA is gone and the deployment dropdown lists drafts only, matching the autocommit behavior above.
  - **`<Meter />` component:** A new [Meter](/components) primitive for rendering progress and usage indicators in your docs.
  - **AsyncAPI honors `api.params.expanded`:** Setting [`api.params.expanded: "all"`](/organize/settings-reference) in `docs.json` now expands parameters and message bindings on [AsyncAPI](/api-playground/asyncapi-setup) pages too, matching the OpenAPI behavior.
  - **Session published card in AI preview:** After the AI preview editor publishes a session, a summary card is shown in-chat so you can jump straight to the changes.
  - **Preview sandboxes revive on demand:** Stopped AI preview sandboxes now come back automatically when you resume a session, and messages support file attachments so you can share screenshots or specs directly in chat.
  - **Contact form lead fields:** The assistant [contact form](/assistant) captures optional lead fields (company, use case, etc.) and includes them in the notification email routed to your team.

  ## Improvements

  - **Slack agent — native Slack read tools:** The [Slack agent](/agent/slack) now reads Slack via native bot-token tools with better thread pagination and clearer error messages. Internal reasoning stays out of user-facing replies.
  - **Slack agent — GitLab-aware context:** The Slack agent correctly identifies deployments backed by GitLab (instead of defaulting to GitHub) and surfaces real GitLab API errors when permissions are misconfigured.
  - **Collapsible answered questions and subagent task list:** The assistant chat now collapses resolved questions and shows a compact task list when a subagent batch is running.
  - **Slimmer AI preview chat header:** The preview editor chat has a redesigned slim topbar, and the composer stays pinned below the header as content scrolls.
  - **Chat pinned to bottom on load:** The AI preview chat opens scrolled to the latest message instead of the top.
  - **Faster CLI prebuild:** [`mint`](/cli) has a faster cold start and an improved prebuild UI.
  - **Dashboard typography:** The dashboard is now on the correct Inter font, tightening up spacing across the UI.

  ## Bug fixes

  - Toggling the `/docs` base path in the dashboard now queues a rebuild so routing updates immediately.
  - `.mdx` and `.md` extensions are stripped from created page hrefs so the AI editor can resolve newly-created pages by path.
  - OpenAPI specs with both `nullable: true` and `allOf` (common in Marshmallow-generated specs) preserve the null variant when rendered in the [API playground](/api-playground/openapi-setup).
  - RSS line-blame now uses the composite git service on multi-repo deployments so recent-changes feeds attribute commits correctly.
  - AI preview sessions no longer get stuck in "initializing" and wait for the hosted repo seed commit before starting.
...
<Update label="July 2, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Assistant custom skills, SCIM provisioning, and more" }}>
  ...
  - **Themed query categories in assistant analytics:** [Assistant analytics](/optimize/analytics) groups related questions into category and shows thumbs-up/thumbs-down sentiment per category, so you can review frequent topics your users are asking about.
