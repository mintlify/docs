...
<Update label="December 22, 2025" tags={["Improvements"]} rss={{ title: "Assistant improvements and bug fixes" }}>
  ## Assistant improvements

  - Improved assistant link handling to open pages in the same window for better navigation flow.
  - Enhanced assistant bar with better formatting for relative links and suggestions.
  - Fixed assistant placeholder display issues.
  - Fixed assistant bar regression that affected input focus and clickable areas.

  ## API playground improvements

  - Fixed Ask AI button visibility in playground modal for cleaner interface.
  - Improved code block tooltip text color for better readability.
  - Fixed missing request examples for API endpoints.
  - Fixed MDX examples display when prefill is enabled.
  - Improved handling of example data with deep merge of examples and inputs.

  ## Editor improvements

  - Added new editor feature flag toggle for better control over editor features.
  - Fixed markdown link rendering in assistant.
  - Disabled React grab functionality for improved stability.

  ## Navigation improvements

  - Added mobile theme switcher and logo display in navigation.
  - Fixed header slug generation to properly preserve special characters.

  ## Bug fixes

  - Fixed combobox focus issues.
  - Fixed unique keys replacement for additional items in API schemas.
  - Fixed resize handle visibility on the right-hand side.
  - Improved configuration for respecting docs config "required" setting for examples.
...
<Update label="December 19, 2025" tags={["Improvements"]} rss={{ title: "Agent, assistant, and editor improvements" }}>
  ...
  - Added support for reading [custom instructions](/ai/agent#customize-agent-behavior) from an `AGENTS.md` file.
  - Agent now opens PRs under your GitHub account if you [connect your account](/ai/agent#connect-your-github-account) on the dashboard.
...
<Update label="December 12, 2025" tags={["New releases", "Improvements"]} rss={{  title: "Agent suggestions and Q&A bots" }}>
  ...
  The agent can now monitor Git repositories and [suggest](/ai/suggestions) documentation updates. When user-facing code changes are detected, suggestions appear in the agent panel. Suggestions include the relevant pull request and a list of proposed documentation updates. Add suggestions as context for the agent to create pull requests or dismiss them.
