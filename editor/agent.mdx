...
Type <kbd>@</kbd> to mention a specific page, and the agent focuses on it. Without a mention, it uses the page you currently have open. On dashboard pages, the agent knows which page you're viewing, including settings screens and product pages like Analytics, Assistant, and Automations, so you can ask about the current page without naming it.
