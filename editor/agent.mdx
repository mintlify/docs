...
When the agent needs more direction before continuing, it asks a question in the chat. Select one of the suggested options, or type your own answer in the chat box if the question allows it. Click **Skip** to let the agent continue without the extra detail.

Questions stay open until you answer or skip them. A pending question remains available after the agent finishes its response, after you reload the page, and when you return to the conversation from [session history](#session-history). Your answer continues the same conversation.
