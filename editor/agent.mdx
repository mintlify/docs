...
keywords: ["editor", "AI", "assistant", "agent", "chat", "comments", "suggestions", "code mode", "file upload", "Slack"]
---
...
If your team has installed the [Mintlify Slack app](/agent/slack), the editor agent can also search and read that same workspace (channels, threads, and members) while it researches or answers your questions. This is separate from the [Integrations](/automations/integrations) connected on the Integrations settings page.

Unlike the [Slack agent](/agent/slack), which can post messages and add reactions for editors and admins, the editor agent's Slack access is always read-only. It can list and search channels, read message history and threads, and look up members, but it never posts messages or adds reactions.

Ask the agent to use Slack naturally in your request, such as:

- `check the #eng channel for context on the new auth flow`
- `summarize what the team decided about rate limiting in yesterday's thread`

If the Slack app isn't installed yet, ask the agent to connect it. The agent returns an authorization link. Complete the flow, then send another message so the agent can confirm the connection and continue.

The agent can only read channels that include the Slack app. If it can't access a channel, add `@mintlify` to that channel and try again.
