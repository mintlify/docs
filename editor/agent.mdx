...
Type <kbd>@</kbd> to mention a specific page, and the agent focuses on it. Without a mention, it uses the page you currently have open. On dashboard settings pages, the agent knows which settings screen you're viewing, so you can ask about the visible setting without naming it.
...
When the agent needs more direction before continuing, it asks a question in the chat. Select one of the suggested options, or type your own answer in the chat box if the question allows it. Click **Skip** to let the agent continue with the default it stated in the question.

The agent pauses its work while a question is pending. It waits for your reply instead of continuing with a guess. Questions stay open until you answer or skip them. A pending question remains available after the agent finishes its response, after you reload the page, and when you return to the conversation from [session history](#session-history). Your answer continues the same conversation.

Skipped and unanswered questions appear in the chat as a collapsible summary. Expand the summary and click **Answer questions** to reopen them.
