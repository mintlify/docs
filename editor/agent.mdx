...
If your organization has [connected the Mintlify Slack app](/agent/slack), the editor agent can read from Slack while it works. Ask it to pull context from a channel, thread, or teammate to inform the changes it makes.

Example prompts:

- `check #launches for the latest details on the billing revamp and update the pricing page`
- `summarize the API design thread in #eng and add the decisions to the API overview`
- `who owns the search team? find them in Slack and mention them in the review notes`

The editor agent has read-only access to Slack. It can list and search channels the app is a member of, fetch messages and threads, and look up workspace members, but it never posts messages or reacts on your behalf.

If Slack isn't connected yet, ask the agent to connect it. The agent returns a **Connect to Slack** link that starts the install flow. After the workspace owner completes it, send another message so the agent can read from Slack.

Slack access follows your dashboard role. If your role can't read or install Slack integrations in the dashboard, the agent can't do it either.
