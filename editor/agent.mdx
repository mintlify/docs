...
Private pages are organized into two sidebar sections. **Personal** contains pages and folders you own. **Shared with me** contains pages and folders shared with you. Each section can be collapsed independently. Use the add menu on the **Personal** header to create a private page or folder. The **Shared with me** header does not include this menu.

You can also right-click the **Personal** section header in the sidebar to create a private page or folder if you have permission to create them.

To promote a private page or folder from the sidebar, open its **More options** menu and select **Promote to workspace**. The action is available for items in your **Personal** section when you have **Manager** access to the item and all of its descendants. After you confirm, Mintlify adds the page or folder at the workspace root. Promoting a folder also promotes all nested folders and pages, recreating their structure and copying their content. Mintlify generates page paths from titles and automatically resolves duplicate paths. The confirmation dialog warns that promotion is permanent. You cannot move the promoted content back to private pages.
...
To create a private folder in **Personal**, use a human-readable title as the final path segment. The folder appears under a new `private-folder-<uuid>` path after you list the directory again:
...
Within `/home/daytona/private`, use `mv` to reparent a private page or folder in **Personal** into another private folder. The item keeps its UUID-based name. Manager access is required. You cannot rename pages or move a folder into itself:
