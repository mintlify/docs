...
Click **Ask agent** in the editor toolbar, or press <kbd>Cmd</kbd> + <kbd>I</kbd> (macOS) or <kbd>Ctrl</kbd> + <kbd>I</kbd> (Windows). The **Ask agent** button also appears on settings pages in your dashboard, so the agent stays available while you configure settings. The agent is available to editors and admins. [Viewers](/dashboard/roles) don't see the **Ask agent** button.
