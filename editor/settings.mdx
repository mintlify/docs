...
- The editor and source view become read-only for anyone viewing the deployment branch.
- The `docs.json` configuration panel stops saving changes.
- Navigation edits — adding pages, reordering, and drag-and-drop — are hidden.
- A banner prompts contributors to create a draft branch before making edits.

Your deployment branch stays in sync with Git, so pushes from your repository still update your live site. Locking only affects edits made from the editor.

<Warning>
  Turning on the lock discards any unpublished editor changes on the deployment branch and force-syncs the editor with your Git repository. Publish or move pending edits to a draft branch before you enable the lock.
</Warning>

While the lock is on, incoming Git pushes to the deployment branch overwrite the editor's live state, so any in-flight edits on that branch are replaced by the latest commit.
...
To edit while the deployment branch is locked, use the branch picker in the top bar to create or switch to a draft branch. You publish those changes by opening a pull request from your draft branch, the same as any other editor branch.
