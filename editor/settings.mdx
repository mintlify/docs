...
Granular publishing controls what the **Publish** menu shows on your deployment branch when several people have pending changes.

- **Off (default)**: The **Publish** menu lists only the pending changes you touched, along with shared files others also edited and any files without an author. Publishing commits and deploys only those changes. **Discard all** discards only those changes. A dismissible notice tells you when teammates have other pending changes that are hidden.
- **On**: The **Publish** menu splits pending changes into **Your changes** and **Modified by others**, with a checkbox on every row so you can choose exactly which changes to include when you publish.
