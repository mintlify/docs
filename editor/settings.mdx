...
- **Show all files**: The sidebar lists Markdown pages by default. Turn this on to also show images, videos, spec and data files, code, PDFs, and dotfiles. You can also press <kbd>Cmd</kbd> + <kbd>Shift</kbd> + <kbd>.</kbd> (macOS) or <kbd>Ctrl</kbd> + <kbd>Shift</kbd> + <kbd>.</kbd> (Windows) to toggle.
...
Selective publishing lets you choose which pending changes to include when you publish on your deployment branch. When you turn this setting on, the **Publish** menu shows a checkbox for each pending change so you can choose which changes to include when you publish. See [Publish specific changes](/editor/publish#publish-specific-changes) for more information.

This setting only takes effect while you work on your deployment branch and that branch has no branch protections. It's off by default.
...
When this setting is on, pull requests created from the editor open as drafts. Authors mark them ready for review when they finish editing. This setting is on by default.

If you want editor pull requests to open ready for review immediately, turn off draft pull requests.

## Selective publishing

{/* TODO(ethan): verify current rollout status before publishing — as of the Sept 2026 Slack #shipped announcement this was still behind the `scoped-publishing` feature flag and rolling out gradually, not GA for all customers yet */}

Selective publishing lets you choose which pending changes to include when you publish directly to your deployment branch. When this setting is on, the publish flow shows a checkbox next to each pending change. Deselect any changes you want to hold back. Deselected changes stay pending so you can publish them later. This setting is off by default.

Selective publishing applies only when you publish directly on a deployment branch without branch protections. Changes that depend on each other select and deselect together. For example, deleting a page removes both the page file and its navigation entry as one selection.
