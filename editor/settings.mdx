---
title: "Editor settings for appearance, publishing, and chat"
description: "Configure editor appearance, commit and pull request guidance, draft pull requests, the default merge method, and AI chat instructions for the Mintlify web editor."
keywords: ["editor", "settings", "appearance", "navigation", "preview", "ai", "chat", "instructions", "publishing", "pull request", "commit", "draft", "merge"]
---

The web editor has two layers of settings:

- **Your settings** apply only to you and control the editor's appearance and how the AI chat assists with your edits.
...
Appearance preferences are scoped to your user account, so each teammate maintains their own.

### Default sidebar view

Choose what the editor sidebar shows when you open the editor:

- **File explorer**: A flat tree of every file in your repository. Use this view when you want to edit non-content files like snippets, images, or configuration.
- **Navigation**: The navigation structure defined in `docs.json`. Use this view when you want to reorganize pages, groups, tabs, anchors, or menus.

You can switch between views at any time using the toggle at the top of the sidebar, or press <kbd>Shift</kbd> + <kbd>Ctrl/Cmd</kbd> + <kbd>F</kbd>.

### Full-width editor

Turn this on to expand the editing area to fill the available horizontal space. Use it when you want more room for tables, code blocks, or wide diagrams. Leave it off to keep the content width close to how pages render on the published site.

### Open live preview in a new tab

Turn this on to make the <Icon icon="play" /> live preview button open in a separate browser tab instead of an in-editor panel. Use a new tab when you want to keep the preview visible on a second monitor or while switching between editor pages. See [Live preview](/editor/live-preview) for more.

## AI chat instructions

AI chat instructions are persistent guidance that the editor sends to the AI alongside your request. Use them to capture style and tone rules you don't want to repeat every time, like voice, terminology, or formatting conventions.
