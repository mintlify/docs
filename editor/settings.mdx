...
The **Danger zone** section of the editor settings panel contains actions that can't be undone. Open it from the <Icon icon="settings" /> settings icon in the editor toolbar, then scroll to **Danger zone**.

### Reset editor

**Reset editor** forces the editor to discard its local state and resync from your Git repository. Use it when the editor gets stuck out of sync with your repo — for example, when the file tree is empty or shows errors like `Unable to find docs.json or mint.json` even though the file exists on your deployment branch and your live site is building normally.

<Warning>
  Resetting the editor discards any unpublished changes that haven't been committed to Git. If you have pending edits you want to keep, publish them first.
</Warning>
...
<Tip>
  The **Reset editor** action lives in the editor's settings panel, not in the [Danger zone](https://app.mintlify.com/settings/organization/danger-zone) of your dashboard settings. The dashboard Danger zone is only used to [delete a deployment](/deploy/deployments#delete-a-deployment).
</Tip>
