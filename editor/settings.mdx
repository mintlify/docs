...
You can configure these settings from the settings panel. To open it, click the <Icon icon="settings" /> settings icon in the editor toolbar, then select a section.
...
Enable **Show hidden files** when you need to edit dotfiles or other hidden files. Mintlify displays them in the navigation sidebar. You can also press <kbd>⌘⇧.</kbd> on macOS or <kbd>Ctrl+Shift+.</kbd> on other platforms to toggle this preference.
