...
Turn this on to have the editor automatically commit changes made on the deployment branch (usually `main`) shortly after editing stops. When off, changes on the deployment branch stay pending in the editor until you publish them.

Use this setting when your team edits the deployment branch directly and wants updates to reach Git without a manual publish step. Leave it off if you want to review changes in the editor before they land on the deployment branch.

This setting only affects the deployment branch. Edits on feature branches created from the editor continue to autocommit regardless of this setting, and locking the deployment branch takes precedence.
