...
- Your organization has a Mintlify deployment connected to GitHub or GitLab. Follow the [quickstart](/quickstart) if you haven't set one up yet.
- You have an [admin or editor role](/dashboard/roles) in the organization. Viewers can browse and leave [suggestions](/editor/suggestions), but cannot publish.
- Your Git provider allows the editor to open pull requests on the repository.
...
If you decide not to publish your changes, you can abandon the branch instead of merging it. Switch back to your deployment branch, then delete the branch from your Git provider (GitHub or GitLab). Any unpublished commits on the branch are removed with it, and your live site is unaffected.

See [Branching and publishing](/editor/branching-and-publishing) for more on managing branches and pull requests.
