...
Suggesting mode applies to page text, including headings, paragraphs, list items, and inline formatting. Changes to components, images, code blocks, and frontmatter are not tracked as suggestions — those edits apply directly to the branch. Editors and admins can accept or reject any suggestion, but viewers cannot. See [roles](/dashboard/roles) for details.

Pending suggestions appear as highlighted text in the editor. The sidebar lists them alongside comments so reviewers can find them, and the editor delivers a notification to reviewers when a new suggestion is created on a branch.
