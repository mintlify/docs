...
4. To exit suggesting mode, click the <Icon icon="pen" /> **Switch to Editing** button in the floating toolbar or the **X** on the **Suggesting** pill in the top bar.

<Note>
  [Viewers](/dashboard/roles) are always in suggesting mode. Suggesting mode activates automatically when a viewer opens the editor. The **Suggesting** pill is visible but not interactive for viewers, so they remain in suggesting mode. This lets viewers propose changes without directly editing content.
</Note>
...
Accepting a suggestion resolves the thread and applies the suggested change. The suggestion and any replies remain in the thread history. The accepted change becomes part of your current branch and goes live the next time you [publish](/editor/branching-and-publishing).

You can reply to a suggestion to discuss it before accepting or rejecting it. Replies remain in the resolved thread history.
...
Rejecting a suggestion resolves the thread and discards the proposed change. The suggestion and any replies remain in the thread history.
