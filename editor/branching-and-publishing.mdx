...
Reviewers can approve a pull request directly from the editor without leaving the publish menu. Click **Approve** to submit an approving review on the open pull request for the current branch. The review status updates to **Approved** and the pull request becomes eligible for **Merge and publish**.

To approve a pull request from the editor:

- The pull request must be open or in draft status.
- You can't approve your own pull request. GitHub blocks self-approvals.
- For GitHub repositories, you must connect your GitHub account to Mintlify in your [account settings](https://dashboard.mintlify.com/settings/account) so the approval is attributed to you. Approvals fall back to opening the pull request in GitHub if your account isn't connected.
- For GitLab repositories, in-editor approvals aren't supported. Open the merge request in GitLab to approve it.

If GitHub rejects the approval—for example, because branch protection rules require a different reviewer—the editor surfaces the error and links you to the pull request in GitHub to finish the review.
