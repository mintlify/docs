...
<Note>
  ...
  If there are no checkboxes next to pending changes, turn on [granular publishing](#publish-specific-changes).
</Note>
...
When you publish directly on your deployment branch and several people have pending changes, the [**Granular publishing**](/editor/settings#granular-publishing) setting controls which pending changes the **Publish** menu shows and commits.

**Granular publishing off (default)**: The **Publish** menu lists only the pending changes you touched, along with shared files others also edited and any files without an author. Clicking **Publish** commits and deploys only those changes, and **Discard all** discards only those changes. A dismissible notice tells you when teammates have other pending changes that are hidden. Turn on granular publishing to see and publish everyone's pending changes.

**Granular publishing on**: The **Publish** menu splits pending changes into **Your changes** and **Modified by others**, with a checkbox on every row so you can choose which specific changes to include when you publish.

- Granular publishing is only available while you work directly on your deployment branch and that branch has no branch protections. On feature branches or when your deployment branch requires pull requests, **Save in branch** and **Create pull request** still include every pending change.
- Deleting a folder deletes every page inside it. Those deletions must publish as one group of changes. You can't partially delete a folder.
- Some changes depend on each other, like a page and its navigation entry. If your selection leaves out a change that a selected change depends on, Mintlify rejects the publish before creating a commit. Adjust your selection to include the dependency, then publish again.
...
- **Publish**: Commits and deploys your changes to your live site immediately. By default, this includes only pending changes you touched, shared files, and unattributed files; turn on [granular publishing](/editor/settings#granular-publishing) to choose from everyone's pending changes.
