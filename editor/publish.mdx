...
The editor checks your navigation for errors, like a tab placed next to groups. On your deployment branch, what happens depends on which pending change caused the error.

- **Your change caused it**: The editor disables **Publish**. An alert names the change, for example `Fix "API reference" to publish`. Click **Review** to go to the item with the error in the navigation, then fix it.
- **A teammate's change caused it**: You can still publish. Their change stays pending and the rest publishes. With granular publishing on, their change shows a warning icon, and you can't select it. Hover over the icon to see the error.
- **The error is already on your live site**: The alert reads "Your navigation has N errors." It doesn't block publishing.

On feature branches and deployment branches with branch protections, any navigation error caused by a pending change blocks publishing and requesting review.
