...
The web editor respects commit message rules configured in your Git provider. If your
repository enforces a commit message pattern, the editor automatically formats commit
messages to match.

- **GitHub**: The editor reads
  [commit message patterns](https://docs.github.com/en/repositories/configuring-branches-and-merges-in-your-repository/managing-rulesets/available-rules-for-rulesets#commit-message)
  defined in repository rulesets for the target branch.
- **GitLab**: The editor reads the
  [commit message regex](https://docs.gitlab.com/user/project/repository/push_rules/#commit-messages-with-a-specific-reference)
  from your project's push rules.

When a rule is detected, the editor adjusts AI-generated commit messages to comply
with the pattern. If a message doesn't match, the editor automatically adds a
conventional commit prefix like `docs:` or `docs(editor):` as a fallback.

<Note>
  If you provide a custom commit message that doesn't match your repository's pattern,
  the editor attempts to add a prefix to make it compliant. If no prefix produces a
  match, the editor uses your original message as-is.
</Note>
