...
<Note>
  If you don't see a **Publish** button, your deployment has [auto publish](#auto-publish) turned on and your edits go live automatically. If you don't see a branch selector, turn it on from the page options menu. See [Show the branch selector](#show-the-branch-selector).
</Note>
...
The branch selector appears in the editor toolbar when your deployment branch has branch protections or when you're already working on a feature branch. If you're on an unprotected deployment branch, the selector is hidden by default.

To show the branch selector, click <Icon icon="ellipsis" /> **More options** in the top-right of the page, then turn on **Branches**. This preference applies only to you.
