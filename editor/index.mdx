...
<Note>
  If the editor ever appears out of sync with your repository — for example, the file tree is empty or shows `Unable to find docs.json or mint.json` even though the file is present on your deployment branch — use [Reset editor](/editor/settings#reset-editor) to force a resync from Git.
</Note>
