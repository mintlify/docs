...
**Source mode** gives you direct access to the MDX. Use it for precise control over component properties and frontmatter. Type `<` to autocomplete components: the menu lists documented components with each one's description, props, and an example. Inside a tag, the menu suggests that component's props and values, including page links, image paths, and icon names. Hover over a component or prop name to see its documentation in a popover. To open the popover at your cursor, press <kbd>Cmd</kbd> + <kbd>.</kbd> (macOS) or <kbd>Ctrl</kbd> + <kbd>.</kbd> (Windows).
