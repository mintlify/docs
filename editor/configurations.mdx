...
To open the panel, select the <Icon icon="sliders-horizontal" /> configurations icon in the editor toolbar.
...
- **Theme**: Select a [theme](/customize/themes) for your site's overall appearance.
...
- **Page eyebrow**: Show small labels preceding page titles. Select `section` or `breadcrumbs`.
...
- **Drilldown**: Auto-navigate to the first page when a user selects a navigation group.
...
- **Indexing**: Select whether to index hidden pages.
