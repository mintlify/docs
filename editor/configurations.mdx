...
- **Status**: The HTTP status code for the redirect.
  - `308`: Permanent redirect. Use this for content that has moved permanently. Selected by default.
  - `307`: Temporary redirect. Use this when the move is temporary.

To add a redirect, select **Add redirect**, fill in the source and destination, and select **Save**. To edit or remove an existing redirect, hover over the row and select the pencil or trash icon. Use the search field to filter redirects by source or destination when you have many entries.
