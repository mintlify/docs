...
Configurations write directly to your `docs.json` file. When you publish, the editor commits the updated `docs.json` along with your other changes.
...
Open `docs.json` from the file explorer to edit the raw configuration. Use this when you want to change a setting that isn't exposed in the Configurations panel, or when you want to add, remove, or restructure top-level keys.

Edits flow through the same publish path as the Configurations panel. The editor tracks pending `docs.json` changes and surfaces a banner until you apply them, then includes the updated file when you publish.

A page's frontmatter and `docs.json` stay in sync: if you remove a key in the editor, it is removed from your published `docs.json` instead of being silently retained. Reserved keys like `navigation` and `$schema` are protected and cannot be deleted from the file.

For the full reference of supported settings, see [Settings](/organize/settings).
