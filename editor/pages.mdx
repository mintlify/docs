...
Duplicate a page when you want to reuse its structure and content as a starting point for a new page. The copy includes the page body, title, description, and icon. Its title ends with `(copy)` so you can identify and rename it.

To duplicate a page, use either method:

- Open the page, click **More options** in the top bar, then select **Duplicate**.
- In the **Navigation** or **Files** view, right-click the page and select **Duplicate**.

When you duplicate the page you currently have open, the copy includes your unsaved edits. The editor places the copy next to the original in the file tree and navigation when the original is listed. Review the copy, update its settings, and publish it when you're ready.

<Note>
  You need edit access and a branch that allows changes. Duplication is available for Markdown and MDX pages, and doesn't require additional configuration.
</Note>
