...
Deleting a page removes both the file and its navigation entry. Despite the **Move to trash** label, there's no trash view to restore pages from. If you delete a page by mistake, use the undo prompt that appears after deleting. The deletion commits to your repository when you publish, so after publishing you can only restore the page from your repository's history.
