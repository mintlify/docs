...
Browse pages in the **Navigation** tab of the left panel. Switch to the **File explorer** tab to see every file in your repository, including non-content files like snippets, configuration, and images. You can also press <kbd>Shift</kbd> + <kbd>Ctrl/Cmd</kbd> + <kbd>F</kbd> to toggle the file explorer. Click the search icon or press <kbd>Cmd</kbd> + <kbd>K</kbd> (macOS) or <kbd>Ctrl</kbd> + <kbd>K</kbd> (Windows) to search by filename or page content.

To change which view opens by default, see [Default sidebar view](/editor/settings#default-sidebar-view).
...
Open most text files in your repository directly from the file explorer. Common formats are editable, including:

- Configuration files like `docs.json`, `yaml`, `toml`, and `ini`.
- Snippets and reusable Markdown.
- Source files like `js`, `ts`, `tsx`, `py`, `rb`, `go`, `sh`, `sql`, `html`, and `css`.

Edits sync in real time the same way as MDX pages, so teammates see your changes immediately. When you publish, the editor commits the file like any other change.

Binary and executable files (for example, `pdf`, fonts, archives, office documents, `exe`, `dll`, `dmg`, `jar`, and `iso`) open with a **No preview available** pane. You can see them in the file explorer but can't edit them in the editor or upload new ones through it.
