...
## Edit source and config files

The web editor opens common source code, configuration, and plaintext files in a source editor so you can update them alongside your documentation. Use this for files like sample code, config snippets, and project metadata that live in your docs repository.

Supported file types include:

- **Documentation**: `mdx`, `md`, `rst`, `tex`, `txt`
- **Data and config**: `json`, `jsonl`, `ndjson`, `yaml`, `yml`, `toml`, `ini`, `conf`, `cfg`, `env`, `properties`, `csv`, `tsv`, `xml`
- **Web**: `html`, `htm`, `css`, `scss`, `sass`, `less`, `vue`, `svelte`
- **JavaScript and TypeScript**: `js`, `jsx`, `mjs`, `cjs`, `ts`, `tsx`, `mts`, `cts`
- **Other languages**: `py`, `rb`, `go`, `rs`, `java`, `c`, `cc`, `cpp`, `h`, `hpp`, `cs`, `swift`, `kt`, `php`, `lua`, `r`, `scala`, `dart`, `ex`, `clj`, `hs`
- **Shell**: `sh`, `bash`, `zsh`, `fish`, `ps1`, `bat`, `cmd`
- **Query and schema**: `sql`, `graphql`, `gql`, `proto`
- **Diffs and logs**: `patch`, `diff`, `log`, `lock`

### Files without a preview

Binary files such as PDFs, fonts, archives, and Office documents stay visible in the navigation tree but open to a **No preview available** pane instead of the editor. To update these files, replace them through Git.

### Blocked file types

Executables, installers, and shared libraries—including `exe`, `msi`, `dll`, `dmg`, `pkg`, `deb`, `rpm`, `apk`, `jar`, `iso`, and similar formats—cannot be uploaded through the editor. The editor rejects them when you drag and drop, and never fetches their contents if they already exist in the repository. This protects collaborators from accidentally distributing executable content through your docs site.
