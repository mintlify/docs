...
- **Unlist**: Right-click a page and select **Unlist**, or open its settings sheet and click the unlist icon next to the delete button. The editor removes the page from your navigation but keeps the file in your repository, so anyone who knows the URL can still reach it. Use this for [hidden pages](/organize/hidden-pages) you don't want listed in the sidebar. To relist an unlisted page, add it back from the **Add existing file** menu on a navigation element.
