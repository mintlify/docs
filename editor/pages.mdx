...
Mint records a new entry when the editor saves content that differs from the latest recorded version. Saving unchanged content doesn't create a duplicate entry. A saved page can also be empty. Each entry shows the author and relative save time. The most recent entry is marked **Latest**.
...
Restore applies the selected snapshot to the current page. For public pages, it writes to the active site, deployment, and branch. For private pages, it updates the shared private content instead of a branch-specific copy. It doesn't create a new page or merely preview the saved content.
...
Restore isn't available on blocked deploy branches or the **Latest** version.
