...
Tables include additional actions in the drag handle menu:

- **Copy table**: Copy the table's Markdown to your clipboard.
- **Fit to editor width**: Toggle on to constrain the table to the editor's width with equal-width columns. Useful for tables with many columns that would otherwise scroll horizontally. Toggle off to return to the default scrollable layout.
- **Full-screen editing**: Open the table in a full-screen overlay for easier editing of wide tables. The table remains fully editable. Press <kbd>Esc</kbd> or click the close button to exit.
