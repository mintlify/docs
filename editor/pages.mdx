...
### Create pages and folders inside a folder

To add content inside a folder in the file tree, click the <Icon icon="plus" /> plus button next to the folder, then choose:

- **Create a page**: Adds a new `.mdx` file inside the folder. The page is created as unlisted, so it doesn't appear in the sidebar until you add it to navigation. The editor seeds a unique filename (for example, `untitled-page.mdx`) so new pages don't collide with existing siblings.
- **New folder**: Adds a new subfolder inside the parent. The editor seeds a unique name (for example, `untitled-folder`).

Rename the page or folder after creating it to match your content.
