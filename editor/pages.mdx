...
When a page contains MDX that the editor can't parse, the visual editor shows a **Couldn't open this page** message with the underlying error. This typically happens when an unclosed component, malformed JSX, or invalid frontmatter is committed to your repository. You have two ways to recover without leaving the editor.

### Fix with AI

Click **Fix with AI** to send the page contents to Mintlify's AI, which attempts to repair the syntax automatically. The fix applies as soon as it's ready, so the page reopens in visual mode if the repair succeeds.

- If the fix resolves some but not all errors, the message updates to show the remaining error so you can run **Fix with AI** again.
- If the AI returns the same content unchanged, the editor prompts you to retry or switch to the source editor.
- Click the **X** next to the progress bar to cancel a long-running fix.

### Resolve in source editor

Click **Resolve in source editor** to switch to Markdown mode with the unparsable file loaded. A banner at the top of the editor shows the parse error so you can find the cause.

1. Edit the MDX directly to fix the syntax.
1. Click **Mark fixed** to commit your changes.
1. If the file parses successfully, the editor automatically returns to visual mode. If errors remain, the banner updates with the new error.

Use this flow when you want manual control over the fix or when AI repair isn't producing the result you want.
