...
The file tree also surfaces files that aren't listed in your `docs.json` navigation:

- **Unlisted Markdown files** — Click any `.md` or `.mdx` file to open it for editing, even if it isn't referenced in `docs.json`. Unlisted pages open without the page header section because there's no navigation entry to source the title, icon, or description from.
- **Images and videos** — Click an image (PNG, JPG, JPEG, GIF, WebP, SVG) or video (MP4, WebM) to preview it inline.
- **Other file types** — Files with unsupported extensions, like `.json`, `.css`, or `.py`, aren't clickable in the file tree. Edit them in your preferred code editor and push the changes to your repository.
