...
The web editor supports collaborative editing for MDX pages and other plain text files in your repository, including `.yml`, `.yaml`, `.json`, `.js`, `.jsx`, `.css`, `.txt`, and `.csv` files. Changes from every collaborator sync in real time, and incoming Git changes merge automatically.
...
### Edit non-MDX text files

Open any supported text file from the navigation tree to edit it directly in the web editor. The editor opens these files in a plain text view without the visual/Markdown toggle.

Supported file types include configuration files (`.yml`, `.yaml`, `.json`), source files (`.js`, `.jsx`, `.css`), and plain data files (`.txt`, `.csv`).

Text files share the same collaboration model as MDX pages:

- **Real-time co-editing**: Multiple teammates can edit the same file at once. Live cursors show who is editing and where.
- **Automatic Git sync**: Incoming changes pushed from outside the editor merge into your draft automatically using a line-level three-way merge.
- **Conflict markers**: If the same lines change in both the editor and Git, the editor inserts standard Git conflict markers so you can resolve the conflict manually before publishing:

  ```text
  <<<<<<< editor
  Your in-editor changes
  =======
  Incoming changes from Git
  >>>>>>> git
  ```

  Search for `<<<<<<< editor` in the file to find any unresolved conflicts. Delete the markers and keep the lines you want before publishing.
