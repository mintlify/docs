...
- **Unlist**: Right-click a page and select **Unlist**, or open page settings and click the <Icon icon="eye-slash" /> icon next to the trash. The page is removed from navigation but the underlying `.mdx` file stays in your repository, so you can re-add it later from the **Add existing file** menu. Only `docs.json` changes when you publish. Click **Undo** on the toast to restore the page to its original position. The action is hidden for pages without an underlying file, such as external links.
