...
- **Insert or edit a link**: Select text and press <kbd>Cmd</kbd> + <kbd>K</kbd> (macOS) or <kbd>Ctrl</kbd> + <kbd>K</kbd> (Windows), or click an existing link to open the link editor. The link editor has two fields:
  - **Page or URL**: Start typing to search pages in your project, or paste an external URL. Select a result with the arrow keys and press <kbd>Enter</kbd>, or press <kbd>Enter</kbd> to save a URL you typed.
  - **Link title**: Edit the text that displays in the page. Press <kbd>Enter</kbd> to save, or <kbd>Esc</kbd> to close the editor without changes. Click **Remove link** to unlink the text.

  You can also drag a page from the navigation tree into the editor to create a link with the page title.
