...
Browse files in the **File explorer** tab of the left panel. New editor sessions open here by default. Switch to the **Navigation** tab to see the structure defined in `docs.json`. Click a page to open it. Click the search icon or press <kbd>Cmd</kbd> + <kbd>K</kbd> (macOS) or <kbd>Ctrl</kbd> + <kbd>K</kbd> (Windows) to search by filename or page content.
