...
Use version history to review saved versions of the page you're editing. Mint records a new entry when the editor saves content that differs from the latest recorded version. Saving unchanged content doesn't create a duplicate entry. A saved page can also be empty. Each entry shows the author and relative save time. The most recent entry is marked **Latest**.
...
The drawer shows versions for the current page on the current branch. Click **Show more versions** to load older entries. Older versions include a **Restore** action when you have permission to edit the page. Click **Restore**, then confirm to replace the current page content with that saved version. This overwrites unsaved changes. Restore isn't available on blocked deploy branches or the **Latest** version. You can't compare versions from the drawer.
