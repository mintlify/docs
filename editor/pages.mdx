...
### Add an existing file to navigation

If your repository contains pages that aren't listed in `docs.json`, you can add them to the sidebar without leaving the editor:

1. Click the <Icon icon="plus" /> plus button in the navigation element where you want the page to appear.
1. Click **Add existing file**.
1. Search for the file by filename or path, then select it.

The editor inserts the file as a page in the chosen location and updates `docs.json` when you publish. Files that are already listed in navigation, files inside `snippets/` or hidden directories, and files like `README` and `LICENSE` are excluded from the list.
