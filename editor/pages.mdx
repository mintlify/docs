...
Send a passage from the page you're editing to the [agent](/agent) without leaving the editor. Use this to ask the agent to rewrite, expand, or explain a specific paragraph instead of describing it from memory.

1. Highlight the text you want to share.
1. In the floating toolbar that appears above the selection, click **Add to Assistant**.

The selection attaches to the agent panel as a chip. The panel opens with the chat input focused so you can start typing a prompt right away. Add more text snippets by repeating the steps above before sending your message.

The agent receives your selection alongside any prompt you write, so you can ask things like "Tighten this paragraph" or "Turn this into a bulleted list" and the agent works against the exact text you sent.
