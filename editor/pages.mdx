...
To set a custom icon or emoji on a folder in **Personal** or **Workspace**, open its options menu in the navigation tree and select **Change icon**. Pick an icon from the **Icons** tab or an emoji from the **Emojis** tab, or select **Remove** to clear it. Folder icons persist when you drag a folder from **Personal** into **Workspace**.
