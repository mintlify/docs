...
- **Preview the site on your `.mintlify.site` subdomain.** Every deployment gets a Mintlify-hosted URL like `example.mintlify.site`. Walk through the site as a reader would before you point your custom domain at it.
- **Run `mint broken-links` locally.** See the [CLI reference](/cli/commands) for setup. Fix any broken links it reports.
- **Verify redirects resolve to the correct destinations.** Test each source path against your preview URL.
- **Confirm authentication and page-level access.** If you set up authentication, sign in as a user in each access group to verify they see the right pages.
