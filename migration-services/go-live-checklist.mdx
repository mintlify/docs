...
- Review your Mintlify site access.
  ...
  - **Test authenticated access before launch.** Sign in as a test user in each user group and confirm they can only reach the pages you expect. Verify that a signed-out visitor is redirected or blocked from private pages.
...
- If required, set up [redirects](/create/redirects).
  - **Test your redirects before launch.** Redirects apply on [preview deployments](/deploy/preview-deployments), so open a preview and visit each source path to confirm it lands on the expected destination. You can also test locally by running `mint dev` and visiting the source paths in your browser.
...
- **Enable add-ons.** Go to the [General](https://app.mintlify.com/settings/deployment/general) page in your dashboard. Enable relevant add-ons such as agent feedback, [CI checks](/deploy/ci), related topics, and more.
